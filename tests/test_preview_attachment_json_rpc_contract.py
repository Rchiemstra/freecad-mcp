"""Agent-to-FreeCAD wire contract for the ``preview_attachment`` MCP operation."""
...
from freecad_mcp.generated.capabilities.register_modules import tools_partdesign_b
...
def _invoke_registered(monkeypatch, transport, params):
    ...
    monkeypatch.setattr(tools_partdesign_b, "server_connection", lambda: connection)
    monkeypatch.setattr(tools_partdesign_b, "server_state", lambda: SimpleNamespace(only_text_feedback=True))
    bind_instrumented_fast_mcp(InstrumentedFastMCP)
    mcp = InstrumentedFastMCP("preview_attachment-contract")
    exports = {}
    tools_partdesign_b._register_preview_attachment(mcp, dependencies=object(), exports=exports)

    async def invoke():
        async with create_connected_server_and_client_session(mcp._mcp_server) as client:
            listing = await client.list_tools()
            tool = next(tool for tool in listing.tools if tool.name == "preview_attachment")
            assert "doc_name" in tool.inputSchema.get("required", ["doc_name"])
            return await client.call_tool("preview_attachment", params)
...
def test_preview_attachment_sends_authenticated_json_rpc_to_freecad(monkeypatch):
    transport = _RecordingFreeCADTransport()
    result = _invoke_registered(monkeypatch, transport, {"doc_name": "AgentDocument", "datum_name": "Value"})
    ...
    assert envelope["method"] == "preview_attachment"
...
def test_mcp_schema_rejects_non_string_doc_name_before_json_rpc(monkeypatch):
    transport = _RecordingFreeCADTransport()
    params = dict({"doc_name": "AgentDocument", "datum_name": "Value"})
    params["doc_name"] = 42
