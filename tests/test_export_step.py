"""Unit coverage for the typed ``export_step`` mutation slice."""
...
from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops import export_step as subject
...
def _collaborators(document, events, *, validator=None, final_result=None, monkeypatch=None):
    if monkeypatch is not None:
        monkeypatch.setattr(measure_io_actions, "export_step", _fake_payload)
...
def test_export_step_runs_apply_recompute_inspect_validate_then_commits(monkeypatch):
    events = []
    document = _Document(events)
    collaborators, api = _collaborators(document, events, monkeypatch=monkeypatch)
    result = run_export_step(collaborators, "Doc", "/tmp/out.step")
...
def test_invalid_names_abort_without_recompute_or_commit(monkeypatch):
    ...
    result = run_export_step(collaborators, "Doc", "")
...
def test_export_step_has_typed_rpc_handler():
    assert subject.TYPED_RPC_HANDLER[0] == "export_step"
    assert callable(subject.TYPED_RPC_HANDLER[1])


def test_unknown_native_evidence_cannot_release_success():
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops.export_step_mutation import (
        _export_step_native_result,
        _NativeMutationState,
    )
    result = _export_step_native_result({"status": "FutureStatus", "committed": False}, _NativeMutationState(postcondition_passed=True))
    assert result["success"] is False
