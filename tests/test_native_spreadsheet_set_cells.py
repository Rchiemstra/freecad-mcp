"""Native qualification matrix for ``spreadsheet_set_cells``."""
...
def test_spreadsheet_set_cells_native_success_inspects_after_recompute(monkeypatch):
    _require_native()
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops import spreadsheet_set_cells as subject

    document = FreeCAD.newDocument("MCPSpreadsheetSetCellsNativeSuccess")
    events = []
    ...
    _prepare(document)
    events.clear()
    original_apply = subject.apply_spreadsheet_set_cells
    original_read = subject.read_spreadsheet_set_cells_result
    ...
    monkeypatch.setattr(subject, "apply_spreadsheet_set_cells", tracked_apply)
    monkeypatch.setattr(subject, "read_spreadsheet_set_cells_result", tracked_read)
    try:
        result = subject.run_spreadsheet_set_cells(_collaborators(FreeCAD, lambda _d: events.append("validate")), document.Name, "Target", [{"address": "A1", "value": 1}])
...
def test_spreadsheet_set_cells_native_validation_failure_restores_complete_state():
    _require_native()
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops.spreadsheet_set_cells import run_spreadsheet_set_cells

    document = FreeCAD.newDocument("MCPSpreadsheetSetCellsNativeRollback")
    _prepare(document)
    ...
    try:
        result = run_spreadsheet_set_cells(
            ...
            document.Name, "Target", [{"address": "A1", "value": 1}],
        )
...
def test_spreadsheet_set_cells_native_apply_failure_restores(monkeypatch):
    _require_native()
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops import spreadsheet_set_cells as subject

    document = FreeCAD.newDocument("MCPSpreadsheetSetCellsNativeApplyFailure")
    _prepare(document)
    state_before = _model_state(document)
    original = subject.apply_spreadsheet_set_cells
    ...
    monkeypatch.setattr(subject, "apply_spreadsheet_set_cells", mutates_then_raises)
    try:
        result = subject.run_spreadsheet_set_cells(_collaborators(FreeCAD, lambda _d: None), document.Name, "Target", [{"address": "A1", "value": 1}])
        assert result["success"] is False
...
def test_spreadsheet_set_cells_native_recompute_failure_rolls_back(monkeypatch):
    _require_native()
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops import spreadsheet_set_cells as subject

    document = FreeCAD.newDocument("MCPSpreadsheetSetCellsNativeRecomputeFailure")
    ...
    monkeypatch.setattr(subject, "apply_spreadsheet_set_cells", arm)
    try:
        result = subject.run_spreadsheet_set_cells(_collaborators(FreeCAD, lambda _d: None), document.Name, "Target", [{"address": "A1", "value": 1}])
        assert result["success"] is False
...
def test_spreadsheet_set_cells_native_rollback_failure_is_uncertain_and_fences(monkeypatch):
    _require_native()
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops import spreadsheet_set_cells as subject

    document = FreeCAD.newDocument("MCPSpreadsheetSetCellsNativeRollbackFailure")
    ...
    try:
        result = subject.run_spreadsheet_set_cells(collaborators, document.Name, "Target", [{"address": "A1", "value": 1}])
        proxy.armed = False
        fenced = subject.run_spreadsheet_set_cells(collaborators, document.Name, "Target", [{"address": "A1", "value": 1}])
...
def test_spreadsheet_set_cells_native_inspection_failure_rolls_back(monkeypatch):
    _require_native()
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops import spreadsheet_set_cells as subject

    document = FreeCAD.newDocument("MCPSpreadsheetSetCellsNativeInspectionFailure")
    _prepare(document)
    state_before = _model_state(document)

    def reject(_document, _receipt):
        raise subject.SpreadsheetSetCellsError("CREATED_OBJECT_WRONG_TYPE", "forced inspection failure")

    monkeypatch.setattr(subject, "read_spreadsheet_set_cells_result", reject)
    try:
        result = subject.run_spreadsheet_set_cells(_collaborators(FreeCAD, lambda _d: None), document.Name, "Target", [{"address": "A1", "value": 1}])
        assert result["success"] is False
...
def test_spreadsheet_set_cells_native_postcondition_cannot_write():
    _require_native()
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops.spreadsheet_set_cells import run_spreadsheet_set_cells

    document = FreeCAD.newDocument("MCPSpreadsheetSetCellsReadOnlyPostcondition")
    ...
    try:
        result = run_spreadsheet_set_cells(_collaborators(FreeCAD, validate), document.Name, "Target", [{"address": "A1", "value": 1}])
