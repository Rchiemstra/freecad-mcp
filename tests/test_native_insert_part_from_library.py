...
def test_insert_part_from_library_native_commit_succeeds():
    _require_native_collaboration()

    import FreeCAD

    from addon.FreeCADMCP.collaboration_api import CollaborationAPI
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops.insert_part_from_library import run_insert_part_from_library

    document = FreeCAD.newDocument("MCPTypedInsertPartFromLibrary")
    events: list[str] = []
    saved_path = ""
    try:
        
        if "insert_part_from_library" == "reload_document":
            saved_path = document.Name + ".FCStd"
            document.saveAs(saved_path)
        if "insert_part_from_library" == "open_document":
            saved_path = document.Name + ".FCStd"
            document.saveAs(saved_path)
        ...
        result = run_insert_part_from_library(collab, document.Name, "Parts/Box.FCStd")
