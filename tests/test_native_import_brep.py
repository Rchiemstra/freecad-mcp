...
def test_import_brep_native_success_inspects_after_recompute(monkeypatch):
    ...
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops import import_brep as module

    document = FreeCAD.newDocument("MCPImportBrepNative")
    events: list[str] = []
    ...
    monkeypatch.setattr(module, "apply_import_brep", tracked_apply)
    collaborators = _collaborators(FreeCAD, lambda _document: events.append("validate"))
    try:
        result = module.run_import_brep(collaborators, document.Name, "/tmp/in.brep")
...
def test_import_brep_native_validation_failure_restores(monkeypatch):
    _require_native_collaboration()
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops import import_brep as module

    document = FreeCAD.newDocument("MCPImportBrepNativeRollback")
    original_apply = module.apply_import_brep

    def tracked_apply(admitted_document, request):
        obj = admitted_document.addObject("App::FeaturePython", "RejectedNative")
        return original_apply(admitted_document, request) if False else module.ImportBrepReceipt(
...
def test_import_brep_native_missing_document_keeps_typed_error():
    ...
    result = run_import_brep(collaborators, "MissingNativeDoc", "/tmp/in.brep")
    assert result["success"] is False
