"""Sensitivity checks for the pocket_feature static and architecture gate."""
...
from ci.check_pocket_feature_contract import scan_pocket_feature_architecture

pytestmark = pytest.mark.unit

ROOT = Path(__file__).resolve().parents[1]
LEAF = "addon/FreeCADMCP/rpc_server/methods/cad_methods_ops/pocket_feature.py"
MUTATION = "addon/FreeCADMCP/rpc_server/methods/cad_methods_ops/pocket_feature_mutation.py"
BRIDGE = "addon/FreeCADMCP/collaboration_api.py"
PUBLIC_ADAPTER = "src/freecad_mcp/operations/parametric_ops/pocket_feature.py"
...
def test_pocket_feature_architecture_gate_accepts_the_production_path() -> None:
    assert scan_pocket_feature_architecture(ROOT) == []
...
def test_gate_rejects_each_known_bad_variant_for_the_intended_reason(
    ...
    assert expected in scan_pocket_feature_architecture(
        ROOT,
...
def test_gate_rejects_transaction_or_recompute_ownership_in_the_leaf() -> None:
    source = _read(LEAF)
    marker = '    """Create a Pocket without recomputing or managing a transaction."""'
...
def test_gate_rejects_any_on_the_pocket_feature_specific_surface() -> None:
    source = _read(LEAF)
    old = 'def build_pocket_feature_request(\n    doc_name: object,'
