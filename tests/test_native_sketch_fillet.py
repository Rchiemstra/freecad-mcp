"""Native FreeCAD qualification for ``sketch_fillet``."""
...
def test_sketch_fillet_native_success_on_idle_sketch():
    _require_native_collaboration()
    import FreeCAD
    import Part
    import Sketcher

    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops.sketch_fillet import run_sketch_fillet

    document, sketch = _idle_sketch(FreeCAD, Part, "MCPSketchFilletNativeSuccess")
    before = sketch.GeometryCount, sketch.ConstraintCount
    try:
        result = run_sketch_fillet(
            _collaborators(FreeCAD, Part, Sketcher, lambda _doc: None),
            document.Name, sketch.Name, 0, 1, 1.0,
        )
...
def test_sketch_fillet_native_validation_failure_restores_geometry_counts():
    _require_native_collaboration()
    import FreeCAD
    import Part
    import Sketcher
    ...
    document, sketch = _idle_sketch(FreeCAD, Part, "MCPSketchFilletNativeValidation")
    ...
    try:
        result = run_sketch_fillet(_collaborators(FreeCAD, Part, Sketcher, fail), document.Name, sketch.Name, 0, 1, 1.0)
...
def test_sketch_fillet_native_missing_document_keeps_typed_error():
    _require_native_collaboration()
    import FreeCAD
    import Part
    import Sketcher
...
def test_sketch_fillet_native_failure_isolated_and_healthy_document_succeeds():
    _require_native_collaboration()
    import FreeCAD
    import Part
    import Sketcher
    ...
    failed_document, failed_sketch = _idle_sketch(FreeCAD, Part, "MCPSketchFilletIsolatedFailure")
    healthy_document, healthy_sketch = _idle_sketch(FreeCAD, Part, "MCPSketchFilletHealthyDocument")
    ...
    try:
        failed = run_sketch_fillet(collab, failed_document.Name, failed_sketch.Name, 0, 1, 1.0)
        isolated = run_sketch_fillet(collab, healthy_document.Name, healthy_sketch.Name, 0, 1, 1.0)
...
def test_sketch_fillet_native_postcondition_cannot_write():
    _require_native_collaboration()
    import FreeCAD
    import Part
    import Sketcher
    ...
    document, sketch = _idle_sketch(FreeCAD, Part, "MCPSketchFilletReadOnlyPostcondition")
