"""Unit coverage for the typed ``spreadsheet_set_alias`` mutation slice."""
...
def test_spreadsheet_set_alias_runs_apply_recompute_inspect_validate_then_commits():
    events: list[str] = []
    document = _Document(events)
    _seed(document)
    collaborators, _api = _collaborators(document, events)

    result = run_spreadsheet_set_alias(collaborators, "Doc", "Target", "A1", "Wall")
...
def test_creation_that_changes_then_raises_is_rolled_back(monkeypatch):
    ...
    monkeypatch.setattr(subject, "apply_spreadsheet_set_alias", mutate_then_raise)
    result = run_spreadsheet_set_alias(collaborators, "Doc", "Target", "A1", "Wall")
    assert result["success"] is False
...
def test_unknown_or_contradictory_native_evidence_cannot_release_success():
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops.spreadsheet_set_alias_mutation import (
        _spreadsheet_set_alias_native_result,
    ...
    for native_result in (
        ...
        result = _spreadsheet_set_alias_native_result(native_result, _NativeMutationState(postcondition_passed=True))
        assert result["success"] is False
