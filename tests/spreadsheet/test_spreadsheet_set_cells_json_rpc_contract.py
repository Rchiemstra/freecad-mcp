"""Agent-to-FreeCAD wire contract for the ``spreadsheet_set_cells`` MCP operation."""
...
def _invoke_registered(monkeypatch, transport, params):
    ...
    mcp = InstrumentedFastMCP("spreadsheet_set_cells-contract")
    exports = {}
    tools_parametric_sheet_a._register_spreadsheet_set_cells(mcp, dependencies=object(), exports=exports)

    async def invoke():
        async with create_connected_server_and_client_session(mcp._mcp_server) as client:
            listing = await client.list_tools()
            tool = next(tool for tool in listing.tools if tool.name == "spreadsheet_set_cells")
            assert "doc_name" in tool.inputSchema.get("required", ["doc_name"])
            return await client.call_tool("spreadsheet_set_cells", params)
...
def test_spreadsheet_set_cells_sends_authenticated_json_rpc_to_freecad(monkeypatch):
    transport = _RecordingFreeCADTransport()
    result = _invoke_registered(monkeypatch, transport, {"doc_name": "AgentDocument", "sheet_name": "Value", "cells": [{"address": "A1", "value": 1}]})
    ...
    assert envelope["method"] == "spreadsheet_set_cells"
