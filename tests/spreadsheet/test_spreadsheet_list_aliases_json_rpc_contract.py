"""Agent-to-FreeCAD wire contract for the ``spreadsheet_list_aliases`` MCP operation."""
...
def _invoke_registered(monkeypatch, transport, params):
    ...
    mcp = InstrumentedFastMCP("spreadsheet_list_aliases-contract")
    exports = {}
    tools_parametric_sheet_b._register_spreadsheet_list_aliases(mcp, dependencies=object(), exports=exports)

    async def invoke():
        async with create_connected_server_and_client_session(mcp._mcp_server) as client:
            listing = await client.list_tools()
            tool = next(tool for tool in listing.tools if tool.name == "spreadsheet_list_aliases")
            assert "doc_name" in tool.inputSchema.get("required", ["doc_name"])
            return await client.call_tool("spreadsheet_list_aliases", params)
...
def test_spreadsheet_list_aliases_sends_authenticated_json_rpc_to_freecad(monkeypatch):
    ...
    assert envelope["method"] == "spreadsheet_list_aliases"
