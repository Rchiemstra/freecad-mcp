"""Agent-to-FreeCAD wire contract for the ``spreadsheet_create`` MCP operation."""
...
class _RecordingFreeCADTransport:
    ...
    def request(self, path, payload, headers):
        ...
        if body_result is _DEFAULT_RESPONSE:
            body_result = dict({"contract_version": 1, "success": True, "ok": True, "outcome": "committed", "committed": True, "retry_safe": False, "sheet": "Value", "label": "Value"}, **{
                key: envelope["params"][key]
                for key in envelope["params"]
                if key in ['sheet', 'label']
            })
            body_result = {"contract_version": 1, "success": True, "ok": True, "outcome": "committed", "committed": True, "retry_safe": False, "sheet": "Value", "label": "Value"}
        response = {
            "jsonrpc": "2.0",
            "id": request["id"],
            "result": {
...
def _invoke_registered(monkeypatch, transport, params):
    ...
    mcp = InstrumentedFastMCP("spreadsheet_create-contract")
    exports = {}
    tools_parametric_sheet_a._register_spreadsheet_create(mcp, dependencies=object(), exports=exports)

    async def invoke():
        async with create_connected_server_and_client_session(mcp._mcp_server) as client:
            listing = await client.list_tools()
            tool = next(tool for tool in listing.tools if tool.name == "spreadsheet_create")
            assert "doc_name" in tool.inputSchema.get("required", ["doc_name"])
            return await client.call_tool("spreadsheet_create", params)
...
def test_spreadsheet_create_sends_authenticated_json_rpc_to_freecad(monkeypatch):
    transport = _RecordingFreeCADTransport()
    result = _invoke_registered(monkeypatch, transport, {"doc_name": "AgentDocument", "sheet_name": "Value"})
    ...
    assert envelope["method"] == "spreadsheet_create"
...
def test_mcp_schema_rejects_non_string_doc_name_before_json_rpc(monkeypatch):
    transport = _RecordingFreeCADTransport()
    params = dict({"doc_name": "AgentDocument", "sheet_name": "Value"})
    params["doc_name"] = 42
