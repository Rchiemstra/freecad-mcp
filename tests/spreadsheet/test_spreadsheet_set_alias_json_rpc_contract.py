"""Agent-to-FreeCAD wire contract for the ``spreadsheet_set_alias`` MCP operation."""
...
class _RecordingFreeCADTransport:
    ...
    def request(self, path, payload, headers):
        ...
        if body_result is _DEFAULT_RESPONSE:
            body_result = dict({"contract_version": 1, "success": True, "ok": True, "outcome": "committed", "committed": True, "retry_safe": False, "sheet": "Value", "address": "Value", "alias": "Value"}, **{
                key: envelope["params"][key]
                for key in envelope["params"]
                if key in ['sheet', 'address', 'alias']
            })
            body_result = {"contract_version": 1, "success": True, "ok": True, "outcome": "committed", "committed": True, "retry_safe": False, "sheet": "Value", "address": "Value", "alias": "Value"}
        response = {
            "jsonrpc": "2.0",
            "id": request["id"],
            "result": {
...
def _invoke_registered(monkeypatch, transport, params):
    ...
    monkeypatch.setattr(tools_parametric_sheet_a, "server_connection", lambda: connection)
    monkeypatch.setattr(tools_parametric_sheet_a, "server_state", lambda: SimpleNamespace(only_text_feedback=True))
    bind_instrumented_fast_mcp(InstrumentedFastMCP)
    mcp = InstrumentedFastMCP("spreadsheet_set_alias-contract")
    exports = {}
    tools_parametric_sheet_a._register_spreadsheet_set_alias(mcp, dependencies=object(), exports=exports)

    async def invoke():
        async with create_connected_server_and_client_session(mcp._mcp_server) as client:
            listing = await client.list_tools()
            tool = next(tool for tool in listing.tools if tool.name == "spreadsheet_set_alias")
            assert "doc_name" in tool.inputSchema.get("required", ["doc_name"])
            return await client.call_tool("spreadsheet_set_alias", params)
...
def test_spreadsheet_set_alias_sends_authenticated_json_rpc_to_freecad(monkeypatch):
    transport = _RecordingFreeCADTransport()
    result = _invoke_registered(monkeypatch, transport, {"doc_name": "AgentDocument", "sheet_name": "Value", "address": "Value", "alias": "Value"})
    ...
    assert envelope["method"] == "spreadsheet_set_alias"
...
def test_mcp_schema_rejects_non_string_doc_name_before_json_rpc(monkeypatch):
    transport = _RecordingFreeCADTransport()
    params = dict({"doc_name": "AgentDocument", "sheet_name": "Value", "address": "Value", "alias": "Value"})
    params["doc_name"] = 42
