...
MUTATION = "addon/FreeCADMCP/rpc_server/methods/cad_methods_ops/spreadsheet_list_aliases_mutation.py"
...
def test_spreadsheet_list_aliases_architecture_gate_accepts_the_production_path() -> None:
...
def test_gate_rejects_inspection_before_recompute() -> None:
    ...
    assert "SPREADSHEET_LIST_ALIASES004 missing typed postcondition=inspect" in scan_spreadsheet_list_aliases_architecture(
...
def test_gate_rejects_cached_success_after_native_rejection() -> None:
    ...
    assert "SPREADSHEET_LIST_ALIASES007 cached success escaped native rejection" in scan_spreadsheet_list_aliases_architecture(
...
def test_gate_rejects_any_on_the_leaf() -> None:
    ...
    assert source.count(old) == 1
    mutated = source.replace(old, "from typing import Any\n\ndef build_spreadsheet_list_aliases_request(", 1)
    # Force an explicit Any on the leaf surface.
    mutated = mutated.replace("doc_name: object", "doc_name: Any", 1)
    assert any("Any" in item for item in scan_spreadsheet_list_aliases_architecture(ROOT, source_overrides={LEAF: mutated}))
