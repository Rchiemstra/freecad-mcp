"""Agent-to-FreeCAD wire contract for the ``spreadsheet_get_cells`` MCP operation."""
...
def _invoke_registered(monkeypatch, transport, params):
    ...
    mcp = InstrumentedFastMCP("spreadsheet_get_cells-contract")
    exports = {}
    tools_parametric_sheet_a._register_spreadsheet_get_cells(mcp, dependencies=object(), exports=exports)

    async def invoke():
        async with create_connected_server_and_client_session(mcp._mcp_server) as client:
            listing = await client.list_tools()
            tool = next(tool for tool in listing.tools if tool.name == "spreadsheet_get_cells")
            assert "doc_name" in tool.inputSchema.get("required", ["doc_name"])
            return await client.call_tool("spreadsheet_get_cells", params)
...
def test_spreadsheet_get_cells_sends_authenticated_json_rpc_to_freecad(monkeypatch):
    transport = _RecordingFreeCADTransport()
    result = _invoke_registered(monkeypatch, transport, {"doc_name": "AgentDocument", "sheet_name": "Value", "addresses": ["A1"]})
    ...
    assert envelope["method"] == "spreadsheet_get_cells"
...
def test_mcp_schema_rejects_non_string_doc_name_before_json_rpc(monkeypatch):
    transport = _RecordingFreeCADTransport()
    params = dict({"doc_name": "AgentDocument", "sheet_name": "Value", "addresses": ["A1"]})
    params["doc_name"] = 42
