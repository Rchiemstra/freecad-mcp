"""Adversarial wire cases for the ``spreadsheet_list_aliases`` contract."""
...
def test_unknown_response_never_proves_rejection(raw):
    result = parse_spreadsheet_list_aliases_response(raw)
...
def test_contract_version_must_be_the_exact_integer(version):
    assert parse_spreadsheet_list_aliases_response(dict(_success(), contract_version=version))["success"] is False
...
def test_contradictory_or_malformed_success_cannot_escape(change):
    result = parse_spreadsheet_list_aliases_response(dict(_success(), **change))
    assert result["success"] is False
    assert result["outcome"] == "uncertain"
    assert result["retry_safe"] is False
...
def test_valid_success_round_trips():
    raw = _success()
    assert parse_spreadsheet_list_aliases_response(raw) == raw
...
def test_transport_failure_preserves_unknown_model_state():
    ...
    response = spreadsheet_list_aliases_operation(_Conn(), True, "Doc", "Value")
    assert response.isError is True
    data = response.structuredContent["data"]
    assert data["error_code"] == "SPREADSHEET_LIST_ALIASES_TRANSPORT_UNCERTAIN"
