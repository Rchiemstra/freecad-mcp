"""Native qualification matrix for ``sketch_add_external_projection``."""
...
def test_sketch_add_external_projection_native_success_inspects_after_recompute(monkeypatch):
    ...
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops import sketch_add_external_projection as subject

    document = FreeCAD.newDocument("MCPSketchAddExternalProjectionNativeSuccess")
    events = []
    ...
    _prepare(document)
    events.clear()
    original_apply = subject.apply_sketch_add_external_projection
    original_read = subject.read_sketch_add_external_projection_result
...
def test_sketch_add_external_projection_native_validation_failure_restores_complete_state():
    _require_native_collaboration()
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops.sketch_add_external_projection import run_sketch_add_external_projection

    document = FreeCAD.newDocument("MCPSketchAddExternalProjectionNativeRollback")
    _prepare(document)
...
def test_sketch_add_external_projection_native_apply_failure_restores(monkeypatch):
    _require_native_collaboration()
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops import sketch_add_external_projection as subject

    document = FreeCAD.newDocument("MCPSketchAddExternalProjectionNativeApplyFailure")
    _prepare(document)
    state_before = _model_state(document)
    original = subject.apply_sketch_add_external_projection
...
def test_sketch_add_external_projection_native_recompute_failure_rolls_back(monkeypatch):
    _require_native_collaboration()
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops import sketch_add_external_projection as subject

    document = FreeCAD.newDocument("MCPSketchAddExternalProjectionNativeRecomputeFailure")
...
def test_sketch_add_external_projection_native_rollback_failure_is_uncertain_and_fences(monkeypatch):
    _require_native_collaboration()
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops import sketch_add_external_projection as subject

    document = FreeCAD.newDocument("MCPSketchAddExternalProjectionNativeRollbackFailure")
...
def test_sketch_add_external_projection_native_inspection_failure_rolls_back(monkeypatch):
    _require_native_collaboration()
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops import sketch_add_external_projection as subject

    document = FreeCAD.newDocument("MCPSketchAddExternalProjectionNativeInspectionFailure")
    _prepare(document)
    state_before = _model_state(document)

    def reject(_document, _receipt):
        raise subject.SketchAddExternalProjectionError("CREATED_OBJECT_WRONG_TYPE", "forced inspection failure")
...
def test_sketch_add_external_projection_native_postcondition_cannot_write():
    _require_native_collaboration()
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops.sketch_add_external_projection import run_sketch_add_external_projection

    document = FreeCAD.newDocument("MCPSketchAddExternalProjectionReadOnlyPostcondition")
