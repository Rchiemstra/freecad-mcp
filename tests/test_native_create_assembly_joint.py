...
def test_create_assembly_joint_native_success_inspects_after_recompute(monkeypatch):
    ...
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops import create_assembly_joint as module

    document = FreeCAD.newDocument("MCPCreateAssemblyJointNative")
    events: list[str] = []
    ...
    monkeypatch.setattr(module, "apply_create_assembly_joint", tracked_apply)
    collaborators = _collaborators(FreeCAD, lambda _document: events.append("validate"))
    try:
        result = module.run_create_assembly_joint(collaborators, document.Name, "Assembly", "Fixed", "A", "B")
...
def test_create_assembly_joint_native_validation_failure_restores(monkeypatch):
    _require_native_collaboration()
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops import create_assembly_joint as module

    document = FreeCAD.newDocument("MCPCreateAssemblyJointNativeRollback")
    original_apply = module.apply_create_assembly_joint

    def tracked_apply(admitted_document, request):
        obj = admitted_document.addObject("App::FeaturePython", "RejectedNative")
        return original_apply(admitted_document, request) if False else module.CreateAssemblyJointReceipt(
...
def test_create_assembly_joint_native_missing_document_keeps_typed_error():
    ...
    result = run_create_assembly_joint(collaborators, "MissingNativeDoc", "Assembly", "Fixed", "A", "B")
    assert result["success"] is False
