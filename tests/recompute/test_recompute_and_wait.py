"""Unit coverage for the typed ``recompute_and_wait`` mutation slice."""
...
from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops.recompute_and_wait_mutation import (
    _recompute_and_wait_native_result,
...
def test_recompute_and_wait_runs_apply_recompute_validate_then_commits():
    events: list[str] = []
    document = FakeDocument(events)
    collab, _api = collaborators(document, events)

    result = run_recompute_and_wait(collab, "Doc")
...
def test_invalid_arguments_abort_without_commit():
    ...
    result = run_recompute_and_wait(collab, "")
...
def test_missing_document_fails_without_entering_apply():
    ...
    if "recompute_and_wait" in {"create_document", "open_document"}:
...
def test_native_capability_is_required_before_apply():
    ...
    if "recompute_and_wait" not in {"create_document", "open_document"}:
...
def test_unknown_or_contradictory_native_evidence_cannot_release_success(native_result):
    result = _recompute_and_wait_native_result(native_result, _NativeMutationState(postcondition_passed=True))
    assert result["success"] is False
    assert result["outcome"] == "uncertain"
    assert result["retry_safe"] is False


def test_recompute_and_wait_has_apply_entry_point():
    assert callable(apply_recompute_and_wait)
