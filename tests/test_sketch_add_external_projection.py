"""Unit coverage for the typed ``sketch_add_external_projection`` mutation slice."""
...
def test_sketch_add_external_projection_runs_apply_recompute_inspect_validate_then_commits():
    events: list[str] = []
    document = _Document(events)
    _seed(document)
    collaborators, _api = _collaborators(document, events)

    result = run_sketch_add_external_projection(collaborators, "Doc", "Seed", "Seed", "auto", False, True)
...
def test_creation_that_changes_then_raises_is_rolled_back(monkeypatch):
    ...
    monkeypatch.setattr(subject, "apply_sketch_add_external_projection", mutate_then_raise)
    result = run_sketch_add_external_projection(collaborators, "Doc", "Seed", "Seed", "auto", False, True)
    assert result["success"] is False
...
def test_unknown_or_contradictory_native_evidence_cannot_release_success():
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops.sketch_add_external_projection_mutation import (
        _sketch_add_external_projection_native_result,
    ...
    for native_result in (
        ...
        result = _sketch_add_external_projection_native_result(native_result, _NativeMutationState(postcondition_passed=True))
        assert result["success"] is False
