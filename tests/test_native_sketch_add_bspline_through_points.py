"""Native FreeCAD qualification for ``sketch_add_bspline_through_points``."""
...
def test_sketch_add_bspline_through_points_native_success_on_idle_sketch():
    _require_native_collaboration()
    import FreeCAD
    import Part
    import Sketcher

    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops.sketch_add_bspline_through_points import run_sketch_add_bspline_through_points

    document, sketch = _idle_sketch(FreeCAD, Part, "MCPSketchAddBsplineThroughPointsNativeSuccess")
    before = sketch.GeometryCount, sketch.ConstraintCount
    try:
        result = run_sketch_add_bspline_through_points(
            _collaborators(FreeCAD, Part, Sketcher, lambda _doc: None),
            document.Name, sketch.Name, [{'x': 0.0, 'y': 0.0}, {'x': 5.0, 'y': 4.0}, {'x': 10.0, 'y': 0.0}], 3, False, False,
        )
...
def test_sketch_add_bspline_through_points_native_validation_failure_restores_geometry_counts():
    _require_native_collaboration()
    import FreeCAD
    import Part
    import Sketcher
    ...
    document, sketch = _idle_sketch(FreeCAD, Part, "MCPSketchAddBsplineThroughPointsNativeValidation")
    ...
    try:
        result = run_sketch_add_bspline_through_points(_collaborators(FreeCAD, Part, Sketcher, fail), document.Name, sketch.Name, [{'x': 0.0, 'y': 0.0}, {'x': 5.0, 'y': 4.0}, {'x': 10.0, 'y': 0.0}], 3, False, False)
...
def test_sketch_add_bspline_through_points_native_missing_document_keeps_typed_error():
    _require_native_collaboration()
    import FreeCAD
    import Part
    import Sketcher
    ...
    result = run_sketch_add_bspline_through_points(
        ...
        "Sketch",
        [{'x': 0.0, 'y': 0.0}, {'x': 5.0, 'y': 4.0}, {'x': 10.0, 'y': 0.0}], 3, False, False,
    )
    assert result["error_code"] == "DOCUMENT_NOT_FOUND"
    assert result["outcome"] == "rejected"


def test_sketch_add_bspline_through_points_native_failure_isolated_and_healthy_document_succeeds():
    _require_native_collaboration()
    import FreeCAD
    import Part
    import Sketcher
    ...
    failed_document, failed_sketch = _idle_sketch(FreeCAD, Part, "MCPSketchAddBsplineThroughPointsIsolatedFailure")
    healthy_document, healthy_sketch = _idle_sketch(FreeCAD, Part, "MCPSketchAddBsplineThroughPointsHealthyDocument")
    ...
    try:
        failed = run_sketch_add_bspline_through_points(collab, failed_document.Name, failed_sketch.Name, [{'x': 0.0, 'y': 0.0}, {'x': 5.0, 'y': 4.0}, {'x': 10.0, 'y': 0.0}], 3, False, False)
        isolated = run_sketch_add_bspline_through_points(collab, healthy_document.Name, healthy_sketch.Name, [{'x': 0.0, 'y': 0.0}, {'x': 5.0, 'y': 4.0}, {'x': 10.0, 'y': 0.0}], 3, False, False)
...
def test_sketch_add_bspline_through_points_native_postcondition_cannot_write():
    _require_native_collaboration()
    import FreeCAD
    import Part
    import Sketcher
    ...
    document, sketch = _idle_sketch(FreeCAD, Part, "MCPSketchAddBsplineThroughPointsReadOnlyPostcondition")
