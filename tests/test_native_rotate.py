...
def test_rotate_native_success_inspects_after_recompute(monkeypatch):
    ...
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops import rotate as module

    document = FreeCAD.newDocument("MCPRotateNative")
    events: list[str] = []
    ...
    monkeypatch.setattr(module, "apply_rotate", tracked_apply)
    collaborators = _collaborators(FreeCAD, lambda _document: events.append("validate"))
    try:
        result = module.run_rotate(collaborators, document.Name, "Box", 0.0, 0.0, 1.0, 90.0)
...
def test_rotate_native_validation_failure_restores(monkeypatch):
    _require_native_collaboration()
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops import rotate as module

    document = FreeCAD.newDocument("MCPRotateNativeRollback")
    original_apply = module.apply_rotate

    def tracked_apply(admitted_document, request):
        obj = admitted_document.addObject("App::FeaturePython", "RejectedNative")
        return original_apply(admitted_document, request) if False else module.RotateReceipt(
...
def test_rotate_native_missing_document_keeps_typed_error():
    ...
    result = run_rotate(collaborators, "MissingNativeDoc", "Box", 0.0, 0.0, 1.0, 90.0)
    assert result["success"] is False
