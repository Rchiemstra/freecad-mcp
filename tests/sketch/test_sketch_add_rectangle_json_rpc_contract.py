"""Agent-to-FreeCAD wire contract for the ``sketch_add_rectangle`` MCP operation."""
...
_TOOL_PARAMS = {'doc_name': 'AgentDocument', 'sketch_name': 'Sketch', 'x1': 0.0, 'y1': 0.0, 'x2': 10.0, 'y2': 5.0, 'construction': False}
...
def _invoke_registered(monkeypatch, transport, arguments):
    ...
    mcp = InstrumentedFastMCP("sketch_add_rectangle-contract")
    exports = {}
    tools_sketch_primitives._register_sketch_add_rectangle(mcp, dependencies=object(), exports=exports)

    async def invoke():
        async with create_connected_server_and_client_session(mcp._mcp_server) as client:
            listing = await client.list_tools()
            tool = next(tool for tool in listing.tools if tool.name == "sketch_add_rectangle")
            assert set(tool.inputSchema["required"]) == {'x2', 'doc_name', 'x1', 'sketch_name', 'y2', 'y1'}
            return await client.call_tool("sketch_add_rectangle", arguments)
...
def test_sketch_add_rectangle_sends_exact_authenticated_json_rpc_values_to_freecad(monkeypatch):
    ...
    assert envelope["method"] == "sketch_add_rectangle"
