"""Agent-to-FreeCAD wire contract for the ``sketch_fillet`` MCP operation."""
...
_TOOL_PARAMS = {'doc_name': 'AgentDocument', 'sketch_name': 'Sketch', 'geo1': 0, 'geo2': 1, 'radius': 1.0}
...
def _invoke_registered(monkeypatch, transport, arguments):
    ...
    mcp = InstrumentedFastMCP("sketch_fillet-contract")
    exports = {}
    tools_sketch_curves_b2._register_sketch_fillet(mcp, dependencies=object(), exports=exports)

    async def invoke():
        async with create_connected_server_and_client_session(mcp._mcp_server) as client:
            listing = await client.list_tools()
            tool = next(tool for tool in listing.tools if tool.name == "sketch_fillet")
            assert set(tool.inputSchema["required"]) == {'radius', 'geo2', 'doc_name', 'sketch_name', 'geo1'}
            return await client.call_tool("sketch_fillet", arguments)
...
def test_sketch_fillet_sends_exact_authenticated_json_rpc_values_to_freecad(monkeypatch):
    ...
    assert envelope["method"] == "sketch_fillet"
