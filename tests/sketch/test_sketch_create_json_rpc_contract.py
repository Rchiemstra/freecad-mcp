"""Agent-to-FreeCAD wire contract for the ``sketch_create`` MCP operation."""
    ...
    def request(self, path, payload, headers):
        ...
        if body_result is _DEFAULT_RESPONSE:
            body_result = {
                ...
                "label": envelope["params"]["sketch_name"],
            }
        response = {
            "jsonrpc": "2.0",
            "id": request["id"],
...
def _invoke_registered(monkeypatch, transport, arguments):
    ...
    mcp = InstrumentedFastMCP("sketch_create-contract")
    exports = {}
    tools_sketch_create_1._register_sketch_create(
        mcp,
        dependencies=object(),
        exports=exports,
    )
    ...
    async def invoke():
        async with create_connected_server_and_client_session(mcp._mcp_server) as client:
            ...
            tool = next(tool for tool in listing.tools if tool.name == "sketch_create")
            assert set(tool.inputSchema["required"]) == {'doc_name', 'sketch_name'}
            return await client.call_tool("sketch_create", arguments)
...
def test_sketch_create_sends_exact_authenticated_json_rpc_values_to_freecad(monkeypatch):
    transport = _RecordingFreeCADTransport()
    result = _invoke_registered(monkeypatch, transport, {'doc_name': 'AgentDocument', 'sketch_name': 'MainSketch'})
    ...
    assert envelope == {
        ...
        "params": {'doc_name': 'AgentDocument', 'sketch_name': 'MainSketch', 'body_name': None, 'attach_to': None},
        "lease_credentials": [],
        "operation": {"name": "Create sketch"},
    }
...
def test_public_sketch_create_route_never_turns_rejection_into_success(monkeypatch):
    response_result = {
        "contract_version": 1,
        "success": False,
        "ok": False,
...
def test_public_route_marks_committed_but_invalid_response_non_retryable(monkeypatch):
    ...
    result = _invoke_registered(monkeypatch, transport, {'doc_name': 'AgentDocument', 'sketch_name': 'MainSketch'})
    assert result.isError is True
    assert result.structuredContent["error_code"] == "SKETCH_CREATE_COMMITTED_RESPONSE_INVALID"
...
def test_mcp_schema_rejects_non_string_before_json_rpc(monkeypatch, sketch_name):
    transport = _RecordingFreeCADTransport()
    arguments = dict({'doc_name': 'AgentDocument', 'sketch_name': 'MainSketch'})
