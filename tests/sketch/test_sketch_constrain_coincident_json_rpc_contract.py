"""Agent-to-FreeCAD wire contract for the ``sketch_constrain_coincident`` MCP operation."""
...
from freecad_mcp.generated.capabilities.register_modules import tools_sketch_constraints_1
...
_TOOL_PARAMS = {'doc_name': 'AgentDocument', 'sketch_name': 'Sketch', 'geo1': 0, 'pos1': 2, 'geo2': 1, 'pos2': 1}
...
def _invoke_registered(monkeypatch, transport, arguments):
    ...
    monkeypatch.setattr(tools_sketch_constraints_1, "server_connection", lambda: connection)
    monkeypatch.setattr(
    ...
    mcp = InstrumentedFastMCP("sketch_constrain_coincident-contract")
    exports = {}
    tools_sketch_constraints_1._register_sketch_constrain_coincident(mcp, dependencies=object(), exports=exports)

    async def invoke():
        async with create_connected_server_and_client_session(mcp._mcp_server) as client:
            listing = await client.list_tools()
            tool = next(tool for tool in listing.tools if tool.name == "sketch_constrain_coincident")
            assert set(tool.inputSchema["required"]) == {'geo2', 'pos2', 'doc_name', 'sketch_name', 'geo1', 'pos1'}
            return await client.call_tool("sketch_constrain_coincident", arguments)
...
def test_sketch_constrain_coincident_sends_exact_authenticated_json_rpc_values_to_freecad(monkeypatch):
    ...
    assert envelope["method"] == "sketch_constrain_coincident"
