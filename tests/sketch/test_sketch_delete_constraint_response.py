"""Adversarial wire cases for the sketch_delete_constraint contract."""
...
def _success():
    return make_sketch_delete_constraint_success(SketchName("Sketch"), 1)


@pytest.mark.parametrize("raw", [None, [], 1, "timeout", {}, {1: "bad key"}])
def test_unknown_response_never_proves_rejection(raw):
    result = parse_sketch_delete_constraint_response(raw)
...
def test_contract_version_must_be_the_exact_integer(version):
    assert (
        parse_sketch_delete_constraint_response(dict(_success(), contract_version=version))["success"] is False
    )
...
def test_contradictory_or_malformed_success_cannot_escape(change):
    result = parse_sketch_delete_constraint_response(dict(_success(), **change))
    assert result["success"] is False
    assert result["outcome"] == "uncertain"
    assert result["retry_safe"] is False
...
@pytest.mark.parametrize(
    ...
    [
        _success(),
        make_sketch_delete_constraint_failure("BUSY", "Retry after recompute", native_status="Busy"),
        make_sketch_delete_constraint_failure("INVALID_ARGUMENT", "Invalid name", retry_safe=False),
        *[
            make_sketch_delete_constraint_uncertain(
                "STATE_UNKNOWN",
...
def test_valid_variants_round_trip_without_changing_evidence(raw):
    assert parse_sketch_delete_constraint_response(raw) == raw
    assert parse_sketch_delete_constraint_response(parse_sketch_delete_constraint_response(raw)) == raw
...
def test_contradictory_failure_never_proves_rejection(change):
    result = parse_sketch_delete_constraint_response(
        dict(make_sketch_delete_constraint_failure("REJECTED", "Rejected"), **change)
    )
    assert result["outcome"] == "uncertain"
    assert result["retry_safe"] is False
...
def test_gui_completion_timeout_preserves_unknown_model_state():
    ...
    response = sketch_delete_constraint_operation(
        SimpleNamespace(sketch_delete_constraint=lambda *_args, **_kwargs: raw), True, "Doc", "Sketch", [0], None
    )
...
def test_transport_failure_preserves_unknown_model_state():
    ...
    response = sketch_delete_constraint_operation(
        SimpleNamespace(sketch_delete_constraint=lost_response), True, "Doc", "Sketch", [0], None
    )
    assert response.isError is True
    data = response.structuredContent["data"]
    assert data["error_code"] == "SKETCH_DELETE_CONSTRAINT_TRANSPORT_UNCERTAIN"
