"""Agent-to-FreeCAD wire contract for the ``sketch_add_regular_polygon`` MCP operation."""
...
_TOOL_PARAMS = {'doc_name': 'AgentDocument', 'sketch_name': 'Sketch', 'cx': 0.0, 'cy': 0.0, 'radius': 10.0, 'sides': 6, 'angle': 0.0, 'construction': False}
...
def _invoke_registered(monkeypatch, transport, arguments):
    ...
    mcp = InstrumentedFastMCP("sketch_add_regular_polygon-contract")
    exports = {}
    tools_sketch_curves_b._register_sketch_add_regular_polygon(mcp, dependencies=object(), exports=exports)

    async def invoke():
        async with create_connected_server_and_client_session(mcp._mcp_server) as client:
            listing = await client.list_tools()
            tool = next(tool for tool in listing.tools if tool.name == "sketch_add_regular_polygon")
            assert set(tool.inputSchema["required"]) == {'radius', 'cy', 'sides', 'doc_name', 'sketch_name', 'cx'}
            return await client.call_tool("sketch_add_regular_polygon", arguments)
...
def test_sketch_add_regular_polygon_sends_exact_authenticated_json_rpc_values_to_freecad(monkeypatch):
    ...
    assert envelope["method"] == "sketch_add_regular_polygon"
