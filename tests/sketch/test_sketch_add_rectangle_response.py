"""Adversarial wire cases for the ``sketch_add_rectangle`` contract."""
...
def _success():
    return make_sketch_add_rectangle_success(SketchName("Sketch"), [0, 1])


@pytest.mark.parametrize("raw", [None, [], 1, "timeout", {}, {1: "bad key"}])
def test_unknown_response_never_proves_rejection(raw):
    result = parse_sketch_add_rectangle_response(raw)
...
def test_contract_version_must_be_the_exact_integer(version):
    assert parse_sketch_add_rectangle_response(dict(_success(), contract_version=version))["success"] is False
...
def test_contradictory_or_malformed_success_cannot_escape(change):
    result = parse_sketch_add_rectangle_response(dict(_success(), **change))
    assert result["success"] is False
    assert result["outcome"] == "uncertain"
    assert result["retry_safe"] is False
...
@pytest.mark.parametrize(
    ...
    [
        _success(),
        make_sketch_add_rectangle_failure("BUSY", "Retry after recompute", native_status="Busy"),
        make_sketch_add_rectangle_failure("INVALID_ARGUMENT", "Invalid name", retry_safe=False),
        *[
            make_sketch_add_rectangle_uncertain("STATE_UNKNOWN", "Reconcile before replay", committed=committed)
            for committed in (None, False, True)
        ],
    ],
)
def test_valid_variants_round_trip_without_changing_evidence(raw):
    assert parse_sketch_add_rectangle_response(raw) == raw
    assert parse_sketch_add_rectangle_response(parse_sketch_add_rectangle_response(raw)) == raw
...
def test_gui_completion_timeout_preserves_unknown_model_state():
    ...
    response = sketch_add_rectangle_operation(
        SimpleNamespace(sketch_add_rectangle=lambda *_args, **_kwargs: raw), True, "Doc", "Sketch", 0.0, 0.0, 10.0, 5.0, False
    )
...
def test_transport_failure_preserves_unknown_model_state():
    ...
    response = sketch_add_rectangle_operation(
        SimpleNamespace(sketch_add_rectangle=lost_response), True, "Doc", "Sketch", 0.0, 0.0, 10.0, 5.0, False
    )
    assert response.isError is True
    data = response.structuredContent["data"]
    assert data["error_code"] == "SKETCH_ADD_RECTANGLE_TRANSPORT_UNCERTAIN"
