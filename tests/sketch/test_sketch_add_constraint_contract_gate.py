...
@pytest.mark.parametrize(
    ...
    [
        (
            ...
            "            self.inspect,",
            "            self.apply,",
            "SKETCH_ADD_CONSTRAINT004 missing typed postcondition=inspect",
        ),
        (
            BRIDGE,
            "            return _unsupported(\n"
            ...
            "SKETCH_ADD_CONSTRAINT006 postcondition path can reach non-native callback fallback",
        ),
        (
            BRIDGE,
            '        """Run a typed native mutation with apply and inspect on one document."""\n\n        document = self._resolve_admitted_document(document_name)',
            '        """Run a typed native mutation with apply and inspect on one document."""\n\n        document = self._resolve_admitted_document(document_name)\n        document = self._resolve_admitted_document(document_name)',
            "SKETCH_ADD_CONSTRAINT005 bridge must resolve the admitted document exactly once",
        ),
        (
            PUBLIC_ADAPTER,
            "result = parse_sketch_add_constraint_response(raw_result)",
            "result = raw_result",
            "SKETCH_ADD_CONSTRAINT008 public adapter bypasses response validation",
        ),
        (
            MUTATION,
            ...
            "    if committed is not False or status not in _REJECTED_STATUSES:",
            "SKETCH_ADD_CONSTRAINT007 cached success escaped native rejection",
        ),
        (
            MUTATION,
            "if state.postcondition_passed and state.failure is None:",
            "if True:",
            "SKETCH_ADD_CONSTRAINT015 commit escaped without a successful postcondition",
        ),
    ],
    ids=(
...
def test_gate_rejects_transaction_or_recompute_ownership_in_the_leaf() -> None:
    ...
    assert "SKETCH_ADD_CONSTRAINT001 leaf owns forbidden execution: recompute" in (
...
def test_gate_rejects_any_on_the_sketch_add_constraint_specific_surface() -> None:
    ...
    assert "SKETCH_ADD_CONSTRAINT014 typed sketch_add_constraint surface contains Any: sketch_add_constraint leaf" in (
        scan_sketch_add_constraint_architecture(
            ROOT,
