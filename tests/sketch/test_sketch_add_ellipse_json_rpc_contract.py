"""Agent-to-FreeCAD wire contract for the ``sketch_add_ellipse`` MCP operation."""
...
_TOOL_PARAMS = {'doc_name': 'AgentDocument', 'sketch_name': 'Sketch', 'cx': 0.0, 'cy': 0.0, 'major_radius': 8.0, 'minor_radius': 4.0, 'angle': 0.0, 'construction': False}
...
def _invoke_registered(monkeypatch, transport, arguments):
    ...
    mcp = InstrumentedFastMCP("sketch_add_ellipse-contract")
    exports = {}
    tools_sketch_curves_a2._register_sketch_add_ellipse(mcp, dependencies=object(), exports=exports)

    async def invoke():
        async with create_connected_server_and_client_session(mcp._mcp_server) as client:
            listing = await client.list_tools()
            tool = next(tool for tool in listing.tools if tool.name == "sketch_add_ellipse")
            assert set(tool.inputSchema["required"]) == {'cy', 'doc_name', 'sketch_name', 'major_radius', 'cx', 'minor_radius'}
            return await client.call_tool("sketch_add_ellipse", arguments)
...
def test_sketch_add_ellipse_sends_exact_authenticated_json_rpc_values_to_freecad(monkeypatch):
    ...
    assert envelope["method"] == "sketch_add_ellipse"
