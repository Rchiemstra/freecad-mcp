"""Agent-to-FreeCAD wire contract for the ``sketch_add_parametric_curve`` MCP operation."""
...
from freecad_mcp.generated.capabilities.register_modules import tools_sketch_curves_b
...
_TOOL_PARAMS = {'doc_name': 'AgentDocument', 'sketch_name': 'Sketch', 'x_expr': 't', 'y_expr': '0', 't_start': 0.0, 't_end': 10.0, 'samples': 20, 'construction': False}
...
def _invoke_registered(monkeypatch, transport, arguments):
    ...
    monkeypatch.setattr(tools_sketch_curves_b, "server_connection", lambda: connection)
    monkeypatch.setattr(
    ...
    mcp = InstrumentedFastMCP("sketch_add_parametric_curve-contract")
    exports = {}
    tools_sketch_curves_b._register_sketch_add_parametric_curve(mcp, dependencies=object(), exports=exports)

    async def invoke():
        async with create_connected_server_and_client_session(mcp._mcp_server) as client:
            listing = await client.list_tools()
            tool = next(tool for tool in listing.tools if tool.name == "sketch_add_parametric_curve")
            assert set(tool.inputSchema["required"]) == {'t_end', 'doc_name', 't_start', 'sketch_name', 'x_expr', 'y_expr'}
            return await client.call_tool("sketch_add_parametric_curve", arguments)
...
def test_sketch_add_parametric_curve_sends_exact_authenticated_json_rpc_values_to_freecad(monkeypatch):
    ...
    assert envelope["method"] == "sketch_add_parametric_curve"
