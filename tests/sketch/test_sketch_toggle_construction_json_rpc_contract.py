"""Agent-to-FreeCAD wire contract for the ``sketch_toggle_construction`` MCP operation."""
...
_TOOL_PARAMS = {'doc_name': 'AgentDocument', 'sketch_name': 'Sketch', 'geo_indices': [0], 'construction': True}
...
def _invoke_registered(monkeypatch, transport, arguments):
    ...
    mcp = InstrumentedFastMCP("sketch_toggle_construction-contract")
    exports = {}
    tools_sketch_curves_b._register_sketch_toggle_construction(mcp, dependencies=object(), exports=exports)

    async def invoke():
        async with create_connected_server_and_client_session(mcp._mcp_server) as client:
            listing = await client.list_tools()
            tool = next(tool for tool in listing.tools if tool.name == "sketch_toggle_construction")
            assert set(tool.inputSchema["required"]) == {'doc_name', 'sketch_name', 'geo_indices'}
            return await client.call_tool("sketch_toggle_construction", arguments)
...
def test_sketch_toggle_construction_sends_exact_authenticated_json_rpc_values_to_freecad(monkeypatch):
    ...
    assert envelope["method"] == "sketch_toggle_construction"
