"""Sensitivity checks for the sketch_toggle_construction static and architecture gate."""
...
LEAF = 'addon/FreeCADMCP/rpc_server/methods/cad_methods_ops/sketch_toggle_construction.py'
MUTATION = 'addon/FreeCADMCP/rpc_server/methods/cad_methods_ops/sketch_toggle_construction_mutation.py'
BRIDGE = "addon/FreeCADMCP/collaboration_api.py"
PUBLIC_ADAPTER = 'src/freecad_mcp/operations/parametric_ops/sketch_toggle_construction.py'
...
def test_sketch_toggle_construction_architecture_gate_accepts_the_production_path() -> None:
    assert scan_sketch_op_architecture(ROOT, "sketch_toggle_construction") == []
...
@pytest.mark.parametrize(
    ...
    [
        (
            ...
            "            self.inspect,",
            "            self.apply,",
            "SKETCH_TOGGLE_CONSTRUCTION004 missing typed postcondition=inspect",
        ),
        (
            BRIDGE,
            "            return _unsupported(\n"
            ...
            "SKETCH_TOGGLE_CONSTRUCTION006 postcondition path can reach non-native callback fallback",
        ),
        (
            PUBLIC_ADAPTER,
            "result = parse_sketch_toggle_construction_response(raw_result)",
            "result = raw_result",
            "SKETCH_TOGGLE_CONSTRUCTION008 public adapter bypasses response validation",
        ),
        (
            MUTATION,
            ...
            "    if committed is not False or status not in _REJECTED_STATUSES:",
            "SKETCH_TOGGLE_CONSTRUCTION007 cached success escaped native rejection",
        ),
        (
            MUTATION,
            "if state.postcondition_passed and state.failure is None:",
            "if True:",
            "SKETCH_TOGGLE_CONSTRUCTION015 commit escaped without a successful postcondition",
        ),
    ],
)
...
def test_gate_rejects_transaction_or_recompute_ownership_in_the_leaf() -> None:
    ...
    assert "SKETCH_TOGGLE_CONSTRUCTION001 leaf owns forbidden execution: recompute" in (
        scan_sketch_op_architecture(ROOT, "sketch_toggle_construction", source_overrides={LEAF: mutated})
    )
