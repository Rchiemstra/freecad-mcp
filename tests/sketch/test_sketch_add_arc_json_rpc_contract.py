"""Agent-to-FreeCAD wire contract for the ``sketch_add_arc`` MCP operation."""
...
from freecad_mcp.generated.capabilities.register_modules import tools_sketch_primitives
...
_TOOL_PARAMS = {'doc_name': 'AgentDocument', 'sketch_name': 'Sketch', 'cx': 0.0, 'cy': 0.0, 'radius': 5.0, 'start_angle': 0.0, 'end_angle': 90.0, 'construction': False}
...
def _invoke_registered(monkeypatch, transport, arguments):
    ...
    monkeypatch.setattr(tools_sketch_primitives, "server_connection", lambda: connection)
    monkeypatch.setattr(
    ...
    mcp = InstrumentedFastMCP("sketch_add_arc-contract")
    exports = {}
    tools_sketch_primitives._register_sketch_add_arc(mcp, dependencies=object(), exports=exports)

    async def invoke():
        async with create_connected_server_and_client_session(mcp._mcp_server) as client:
            listing = await client.list_tools()
            tool = next(tool for tool in listing.tools if tool.name == "sketch_add_arc")
            assert set(tool.inputSchema["required"]) == {'radius', 'cy', 'start_angle', 'doc_name', 'sketch_name', 'cx', 'end_angle'}
            return await client.call_tool("sketch_add_arc", arguments)
...
def test_sketch_add_arc_sends_exact_authenticated_json_rpc_values_to_freecad(monkeypatch):
    ...
    assert envelope["method"] == "sketch_add_arc"
