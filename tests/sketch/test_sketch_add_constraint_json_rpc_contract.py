"""Agent-to-FreeCAD wire contract for the ``sketch_add_constraint`` MCP operation."""
...
def _invoke_registered(monkeypatch, transport, arguments):
    ...
    mcp = InstrumentedFastMCP("sketch_add_constraint-contract")
    exports = {}
    tools_sketch_create_1._register_sketch_add_constraint(
        mcp,
        dependencies=object(),
        exports=exports,
    )
    ...
    async def invoke():
        async with create_connected_server_and_client_session(mcp._mcp_server) as client:
            ...
            tool = next(tool for tool in listing.tools if tool.name == "sketch_add_constraint")
            assert set(tool.inputSchema["required"]) == {'constraints', 'doc_name', 'sketch_name'}
            return await client.call_tool("sketch_add_constraint", arguments)
...
def test_sketch_add_constraint_sends_exact_authenticated_json_rpc_values_to_freecad(monkeypatch):
    transport = _RecordingFreeCADTransport()
    result = _invoke_registered(monkeypatch, transport, {'doc_name': 'AgentDocument', 'sketch_name': 'MainSketch', 'constraints': [{'type': 'Radius', 'geo': 0, 'value': 5.0, 'name': 'R'}]})
    ...
    assert envelope == {
        ...
        "method": "sketch_add_constraint",
        "params": {'doc_name': 'AgentDocument', 'sketch_name': 'MainSketch', 'constraints': [{'type': 'Radius', 'geo': 0, 'value': 5.0, 'name': 'R'}]},
        "lease_credentials": [],
        "operation": {"name": "Add sketch constraint"},
    }
...
def test_public_sketch_add_constraint_route_never_turns_rejection_into_success(monkeypatch):
    response_result = {
        "contract_version": 1,
        "success": False,
        "ok": False,
...
def test_public_route_marks_committed_but_invalid_response_non_retryable(monkeypatch):
    ...
    result = _invoke_registered(monkeypatch, transport, {'doc_name': 'AgentDocument', 'sketch_name': 'MainSketch', 'constraints': [{'type': 'Radius', 'geo': 0, 'value': 5.0, 'name': 'R'}]})
    assert result.isError is True
    assert result.structuredContent["error_code"] == "SKETCH_ADD_CONSTRAINT_COMMITTED_RESPONSE_INVALID"
...
def test_mcp_schema_rejects_non_string_before_json_rpc(monkeypatch, sketch_name):
    transport = _RecordingFreeCADTransport()
    arguments = dict({'doc_name': 'AgentDocument', 'sketch_name': 'MainSketch', 'constraints': [{'type': 'Radius', 'geo': 0, 'value': 5.0, 'name': 'R'}]})
