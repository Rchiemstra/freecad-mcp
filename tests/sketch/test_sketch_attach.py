...
def _call(collaborators, *extra):
    if extra:
        args = list(('Doc', 'Sketch', 'Box:Face1'))
        args[1] = extra[0]
        return run_sketch_attach(collaborators, *tuple(args))
    return run_sketch_attach(collaborators, *('Doc', 'Sketch', 'Box:Face1'))


def test_sketch_attach_runs_apply_recompute_inspect_validate_then_commits():
    events = []
    ...
    assert result["attached_kind"] == "face_ref"
    assert result["attached_object"] == "Box"
...
def test_creation_that_changes_then_raises_is_rolled_back():
    ...
    assert result["success"] is False
    assert result["error_code"] == 'SKETCH_ATTACH_FAILED'
    assert "commit" not in events
...
def test_sketch_attach_has_no_uncoordinated_source_entry_point():
...
def test_unknown_or_contradictory_native_evidence_cannot_release_success(native_result):
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops.sketch_attach_mutation import (
        _sketch_attach_native_result,
        _NativeSketchAttachMutationState,
    )

    result = _sketch_attach_native_result(native_result, _NativeSketchAttachMutationState(postcondition_passed=True))
    assert result["success"] is False
