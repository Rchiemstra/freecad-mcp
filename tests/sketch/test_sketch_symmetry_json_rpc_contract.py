"""Agent-to-FreeCAD wire contract for the ``sketch_symmetry`` MCP operation."""
...
from freecad_mcp.generated.capabilities.register_modules import tools_sketch_curves_b2
...
_TOOL_PARAMS = {'doc_name': 'AgentDocument', 'sketch_name': 'Sketch', 'geo_indices': [0], 'symmetry_geo': 1, 'copy': True}
...
def _invoke_registered(monkeypatch, transport, arguments):
    ...
    monkeypatch.setattr(tools_sketch_curves_b2, "server_connection", lambda: connection)
    monkeypatch.setattr(
    ...
    mcp = InstrumentedFastMCP("sketch_symmetry-contract")
    exports = {}
    tools_sketch_curves_b2._register_sketch_symmetry(mcp, dependencies=object(), exports=exports)

    async def invoke():
        async with create_connected_server_and_client_session(mcp._mcp_server) as client:
            listing = await client.list_tools()
            tool = next(tool for tool in listing.tools if tool.name == "sketch_symmetry")
            assert set(tool.inputSchema["required"]) == {'doc_name', 'sketch_name', 'symmetry_geo', 'geo_indices'}
            return await client.call_tool("sketch_symmetry", arguments)
...
def test_sketch_symmetry_sends_exact_authenticated_json_rpc_values_to_freecad(monkeypatch):
    ...
    assert envelope["method"] == "sketch_symmetry"
