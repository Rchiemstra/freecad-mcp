"""Unit coverage for the typed ``sketch_add_ellipse`` mutation slice."""
...
from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops import sketch_add_ellipse as subject
...
def test_sketch_add_ellipse_runs_apply_recompute_inspect_validate_then_commits():
    events = []
    document = FakeDocument(events)
    _prepare(document, events)
    collab, api = collaborators(document, events)

    result = run_sketch_add_ellipse(collab, "Doc", "Sketch", 0.0, 0.0, 8.0, 4.0, 0.0, False)
...
def test_invalid_names_abort_without_recompute_or_commit(sketch_name):
    ...
    result = run_sketch_add_ellipse(collab, "Doc", sketch_name, 0.0, 0.0, 8.0, 4.0, 0.0, False)
...
def test_missing_document_fails_without_entering_the_apply_callback():
    ...
    result = run_sketch_add_ellipse(collab, "Missing", "Sketch", 0.0, 0.0, 8.0, 4.0, 0.0, False)
...
def test_sketch_add_ellipse_has_no_uncoordinated_source_entry_point():
    assert not hasattr(subject, "sketch_add_ellipse_gui")
    template = Path(__file__).parents[2] / 'src/freecad_mcp/templates/p1_curves/sketch_add_ellipse.py.txt'
...
def test_unknown_or_contradictory_native_evidence_cannot_release_success(native_result):
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops.sketch_add_ellipse_mutation import (
        _sketch_add_ellipse_native_result,
        _NativeSketchAddEllipseMutationState,
    )

    result = _sketch_add_ellipse_native_result(
        native_result, _NativeSketchAddEllipseMutationState(postcondition_passed=True)
    )
    assert result["success"] is False
