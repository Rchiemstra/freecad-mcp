"""Agent-to-FreeCAD wire contract for the ``sketch_constrain_parallel`` MCP operation."""
...
def _invoke_registered(monkeypatch, transport, arguments):
    ...
    mcp = InstrumentedFastMCP("sketch_constrain_parallel-contract")
    exports = {}
    tools_sketch_constraints_2._register_sketch_constrain_parallel(mcp, dependencies=object(), exports=exports)

    async def invoke():
        async with create_connected_server_and_client_session(mcp._mcp_server) as client:
            listing = await client.list_tools()
            tool = next(tool for tool in listing.tools if tool.name == "sketch_constrain_parallel")
            assert set(tool.inputSchema["required"]) == {'doc_name', 'geo1', 'sketch_name', 'geo2'}
            return await client.call_tool("sketch_constrain_parallel", arguments)
...
def test_sketch_constrain_parallel_sends_exact_authenticated_json_rpc_values_to_freecad(monkeypatch):
    ...
    assert envelope["method"] == "sketch_constrain_parallel"
