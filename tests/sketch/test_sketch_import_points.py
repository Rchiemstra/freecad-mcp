"""Unit coverage for the typed ``sketch_import_points`` mutation slice."""
...
from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops import sketch_import_points as subject
...
def test_sketch_import_points_runs_apply_recompute_inspect_validate_then_commits():
    events = []
    document = FakeDocument(events)
    _prepare(document, events)
    collab, api = collaborators(document, events)

    result = run_sketch_import_points(collab, "Doc", "Sketch", [{'x': 1.0, 'y': 2.0}, {'x': 3.0, 'y': 4.0}], False)
...
def test_invalid_names_abort_without_recompute_or_commit(sketch_name):
    ...
    result = run_sketch_import_points(collab, "Doc", sketch_name, [{'x': 1.0, 'y': 2.0}, {'x': 3.0, 'y': 4.0}], False)
...
def test_missing_document_fails_without_entering_the_apply_callback():
    ...
    result = run_sketch_import_points(collab, "Missing", "Sketch", [{'x': 1.0, 'y': 2.0}, {'x': 3.0, 'y': 4.0}], False)
...
def test_sketch_import_points_has_no_uncoordinated_source_entry_point():
    assert not hasattr(subject, "sketch_import_points_gui")
    template = Path(__file__).parents[2] / 'src/freecad_mcp/templates/p1_curves/sketch_import_points.py.txt'
...
def test_unknown_or_contradictory_native_evidence_cannot_release_success(native_result):
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops.sketch_import_points_mutation import (
        _sketch_import_points_native_result,
        _NativeSketchImportPointsMutationState,
    )

    result = _sketch_import_points_native_result(
        native_result, _NativeSketchImportPointsMutationState(postcondition_passed=True)
    )
    assert result["success"] is False
