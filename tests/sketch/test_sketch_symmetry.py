"""Unit coverage for the typed ``sketch_symmetry`` mutation slice."""
...
from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops import sketch_symmetry as subject
...
def test_sketch_symmetry_runs_apply_recompute_inspect_validate_then_commits():
    events = []
    document = FakeDocument(events)
    _prepare(document, events)
    collab, api = collaborators(document, events)

    result = run_sketch_symmetry(collab, "Doc", "Sketch", [0], 1, True)
...
def test_invalid_names_abort_without_recompute_or_commit(sketch_name):
    ...
    result = run_sketch_symmetry(collab, "Doc", sketch_name, [0], 1, True)
...
def test_missing_document_fails_without_entering_the_apply_callback():
    ...
    result = run_sketch_symmetry(collab, "Missing", "Sketch", [0], 1, True)
...
def test_sketch_symmetry_has_no_uncoordinated_source_entry_point():
    assert not hasattr(subject, "sketch_symmetry_gui")
    template = Path(__file__).parents[2] / 'src/freecad_mcp/templates/p2_editing/sketch_symmetry.py.txt'
...
def test_unknown_or_contradictory_native_evidence_cannot_release_success(native_result):
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops.sketch_symmetry_mutation import (
        _sketch_symmetry_native_result,
        _NativeSketchSymmetryMutationState,
    )

    result = _sketch_symmetry_native_result(
        native_result, _NativeSketchSymmetryMutationState(postcondition_passed=True)
    )
    assert result["success"] is False
