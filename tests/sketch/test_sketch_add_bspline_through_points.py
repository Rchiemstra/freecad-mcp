"""Unit coverage for the typed ``sketch_add_bspline_through_points`` mutation slice."""
...
from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops import sketch_add_bspline_through_points as subject
...
def test_sketch_add_bspline_through_points_runs_apply_recompute_inspect_validate_then_commits():
    events = []
    document = FakeDocument(events)
    _prepare(document, events)
    collab, api = collaborators(document, events)

    result = run_sketch_add_bspline_through_points(collab, "Doc", "Sketch", [{'x': 0.0, 'y': 0.0}, {'x': 5.0, 'y': 4.0}, {'x': 10.0, 'y': 0.0}], 3, False, False)
...
def test_invalid_names_abort_without_recompute_or_commit(sketch_name):
    ...
    result = run_sketch_add_bspline_through_points(collab, "Doc", sketch_name, [{'x': 0.0, 'y': 0.0}, {'x': 5.0, 'y': 4.0}, {'x': 10.0, 'y': 0.0}], 3, False, False)
...
def test_missing_document_fails_without_entering_the_apply_callback():
    ...
    result = run_sketch_add_bspline_through_points(collab, "Missing", "Sketch", [{'x': 0.0, 'y': 0.0}, {'x': 5.0, 'y': 4.0}, {'x': 10.0, 'y': 0.0}], 3, False, False)
...
def test_sketch_add_bspline_through_points_has_no_uncoordinated_source_entry_point():
    assert not hasattr(subject, "sketch_add_bspline_through_points_gui")
    template = Path(__file__).parents[2] / 'src/freecad_mcp/templates/p1_curves/sketch_add_bspline_through_points.py.txt'
...
def test_unknown_or_contradictory_native_evidence_cannot_release_success(native_result):
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops.sketch_add_bspline_through_points_mutation import (
        _sketch_add_bspline_through_points_native_result,
        _NativeSketchAddBsplineThroughPointsMutationState,
    )

    result = _sketch_add_bspline_through_points_native_result(
        native_result, _NativeSketchAddBsplineThroughPointsMutationState(postcondition_passed=True)
    )
    assert result["success"] is False
