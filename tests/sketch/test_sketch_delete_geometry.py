...
def _call(collaborators, *extra):
    if extra:
        ...
        args[1] = extra[0]
        return run_sketch_delete_geometry(collaborators, *tuple(args))
    return run_sketch_delete_geometry(collaborators, *('Doc', 'Sketch', [0]))


def test_sketch_delete_geometry_runs_apply_recompute_inspect_validate_then_commits():
    events = []
...
def test_creation_that_changes_then_raises_is_rolled_back():
    ...
    assert result["success"] is False
    assert result["error_code"] == 'SKETCH_DELETE_GEOMETRY_FAILED'
    assert "commit" not in events
...
def test_sketch_delete_geometry_has_no_uncoordinated_source_entry_point():
    assert not hasattr(subject, 'sketch_delete_geometry_gui')
...
def test_unknown_or_contradictory_native_evidence_cannot_release_success(native_result):
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops.sketch_delete_geometry_mutation import (
        _sketch_delete_geometry_native_result,
        _NativeSketchDeleteGeometryMutationState,
    )

    result = _sketch_delete_geometry_native_result(native_result, _NativeSketchDeleteGeometryMutationState(postcondition_passed=True))
    assert result["success"] is False
