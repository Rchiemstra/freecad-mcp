"""Agent-to-FreeCAD wire contract for the ``sketch_delete_geometry`` MCP operation."""
...
def _invoke_registered(monkeypatch, transport, arguments):
    ...
    mcp = InstrumentedFastMCP("sketch_delete_geometry-contract")
    exports = {}
    tools_sketch_create_2._register_sketch_delete_geometry(
        mcp,
        dependencies=object(),
        exports=exports,
    )
    ...
    async def invoke():
        async with create_connected_server_and_client_session(mcp._mcp_server) as client:
            ...
            tool = next(tool for tool in listing.tools if tool.name == "sketch_delete_geometry")
            assert set(tool.inputSchema["required"]) == {'doc_name', 'geometry_indices', 'sketch_name'}
            return await client.call_tool("sketch_delete_geometry", arguments)
...
def test_sketch_delete_geometry_sends_exact_authenticated_json_rpc_values_to_freecad(monkeypatch):
    transport = _RecordingFreeCADTransport()
    result = _invoke_registered(monkeypatch, transport, {'doc_name': 'AgentDocument', 'sketch_name': 'MainSketch', 'geometry_indices': [0]})
    ...
    assert envelope == {
        ...
        "method": "sketch_delete_geometry",
        "params": {'doc_name': 'AgentDocument', 'sketch_name': 'MainSketch', 'geometry_indices': [0]},
        "lease_credentials": [],
        "operation": {"name": "Delete sketch geometry"},
    }
...
def test_public_sketch_delete_geometry_route_never_turns_rejection_into_success(monkeypatch):
    response_result = {
        "contract_version": 1,
        "success": False,
        "ok": False,
...
def test_public_route_marks_committed_but_invalid_response_non_retryable(monkeypatch):
    ...
    result = _invoke_registered(monkeypatch, transport, {'doc_name': 'AgentDocument', 'sketch_name': 'MainSketch', 'geometry_indices': [0]})
    assert result.isError is True
    assert result.structuredContent["error_code"] == "SKETCH_DELETE_GEOMETRY_COMMITTED_RESPONSE_INVALID"
...
def test_mcp_schema_rejects_non_string_before_json_rpc(monkeypatch, sketch_name):
    transport = _RecordingFreeCADTransport()
    arguments = dict({'doc_name': 'AgentDocument', 'sketch_name': 'MainSketch', 'geometry_indices': [0]})
