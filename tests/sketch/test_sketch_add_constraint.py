...
def _call(collaborators, *extra):
    if extra:
        args = list(('Doc', 'Sketch', [{'type': 'Radius', 'geo': 0, 'value': 5.0, 'name': 'R2'}]))
        args[1] = extra[0]
        return run_sketch_add_constraint(collaborators, *tuple(args))
    return run_sketch_add_constraint(collaborators, *('Doc', 'Sketch', [{'type': 'Radius', 'geo': 0, 'value': 5.0, 'name': 'R2'}]))


def test_sketch_add_constraint_runs_apply_recompute_inspect_validate_then_commits():
    events = []
...
def test_creation_that_changes_then_raises_is_rolled_back():
    ...
    assert result["success"] is False
    assert result["error_code"] == 'SKETCH_ADD_CONSTRAINT_FAILED'
    assert "commit" not in events
...
def test_sketch_add_constraint_has_no_uncoordinated_source_entry_point():
    assert not hasattr(subject, 'sketch_add_constraint_gui')
...
def test_unknown_or_contradictory_native_evidence_cannot_release_success(native_result):
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops.sketch_add_constraint_mutation import (
        _sketch_add_constraint_native_result,
        _NativeSketchAddConstraintMutationState,
    )

    result = _sketch_add_constraint_native_result(native_result, _NativeSketchAddConstraintMutationState(postcondition_passed=True))
    assert result["success"] is False
