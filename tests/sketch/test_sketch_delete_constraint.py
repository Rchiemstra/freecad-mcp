...
def _call(collaborators, *extra):
    if extra:
        args = list(('Doc', 'Sketch', [0], []))
        args[1] = extra[0]
        return run_sketch_delete_constraint(collaborators, *tuple(args))
    return run_sketch_delete_constraint(collaborators, *('Doc', 'Sketch', [0], []))


def test_sketch_delete_constraint_runs_apply_recompute_inspect_validate_then_commits():
    events = []
...
def test_creation_that_changes_then_raises_is_rolled_back():
    ...
    assert result["success"] is False
    assert result["error_code"] == 'SKETCH_DELETE_CONSTRAINT_FAILED'
    assert "commit" not in events
...
@pytest.mark.parametrize(
    ...
    [(_MissingAfterRecomputeDocument, 'CREATED_OBJECT_MISSING'), (_ReplacingAfterRecomputeDocument, 'CREATED_OBJECT_REPLACED')],
)
...
def test_response_uses_the_actual_name_assigned_by_freecad():
    events = []
    document = _Document(events, assigned_name='Sketch')
    collaborators, _api = _collaborators(document, events)
...
def test_sketch_delete_constraint_has_no_uncoordinated_source_entry_point():
    assert not hasattr(subject, 'sketch_delete_constraint_gui')
...
def test_unknown_or_contradictory_native_evidence_cannot_release_success(native_result):
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops.sketch_delete_constraint_mutation import (
        _sketch_delete_constraint_native_result,
        _NativeSketchDeleteConstraintMutationState,
    )

    result = _sketch_delete_constraint_native_result(native_result, _NativeSketchDeleteConstraintMutationState(postcondition_passed=True))
    assert result["success"] is False
