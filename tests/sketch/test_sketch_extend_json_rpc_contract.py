"""Agent-to-FreeCAD wire contract for the ``sketch_extend`` MCP operation."""
...
_TOOL_PARAMS = {'doc_name': 'AgentDocument', 'sketch_name': 'Sketch', 'geo_index': 0, 'increment': 2.0, 'end_point': 2}
...
def _invoke_registered(monkeypatch, transport, arguments):
    ...
    mcp = InstrumentedFastMCP("sketch_extend-contract")
    exports = {}
    tools_sketch_curves_b2._register_sketch_extend(mcp, dependencies=object(), exports=exports)

    async def invoke():
        async with create_connected_server_and_client_session(mcp._mcp_server) as client:
            listing = await client.list_tools()
            tool = next(tool for tool in listing.tools if tool.name == "sketch_extend")
            assert set(tool.inputSchema["required"]) == {'doc_name', 'increment', 'sketch_name', 'geo_index'}
            return await client.call_tool("sketch_extend", arguments)
...
def test_sketch_extend_sends_exact_authenticated_json_rpc_values_to_freecad(monkeypatch):
    ...
    assert envelope["method"] == "sketch_extend"
