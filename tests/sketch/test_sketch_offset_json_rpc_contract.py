"""Agent-to-FreeCAD wire contract for the ``sketch_offset`` MCP operation."""
...
class _RecordingFreeCADTransport:
    ...
    def request(self, path, payload, headers):
        ...
        if body_result is _DEFAULT_RESPONSE:
            from tests.sketch.test_sketch_offset_response import _success
...
def _invoke_registered(monkeypatch, transport, arguments):
    ...
    mcp = InstrumentedFastMCP("sketch_offset-contract")
    exports = {}
    tools_sketch_curves_b2._register_sketch_offset(mcp, dependencies=object(), exports=exports)

    async def invoke():
        async with create_connected_server_and_client_session(mcp._mcp_server) as client:
            listing = await client.list_tools()
            tool = next(tool for tool in listing.tools if tool.name == "sketch_offset")
            assert set(tool.inputSchema["required"]) == {"doc_name", "sketch_name", "geo_indices", "offset"}
            return await client.call_tool("sketch_offset", arguments)
...
def test_sketch_offset_sends_exact_authenticated_json_rpc_values_to_freecad(monkeypatch):
    ...
    assert envelope["method"] == "sketch_offset"
...
def test_mcp_schema_rejects_non_string_before_json_rpc(monkeypatch, doc_name):
    transport = _RecordingFreeCADTransport()
    payload = dict(_DEFAULT_ARGUMENTS)
