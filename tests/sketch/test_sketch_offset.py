"""Unit coverage for the typed ``sketch_offset`` mutation slice."""
...
from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops import sketch_offset as subject
...
def _offset_geometry():
    ...
    return SimpleNamespace(
        StartPoint=start,
        EndPoint=end,
        toShape=lambda: SimpleNamespace(
...
def _prepare(document, events):
    ...
    document.sketch.addGeometry(_offset_geometry(), False)
    document.sketch.addGeometry(_offset_geometry(), False)
    sketch.fail_after_add, sketch.fail_after_constraint, sketch.fail_after_edit = previous
    events.clear()


def test_sketch_offset_runs_apply_recompute_inspect_validate_then_commits():
    events = []
    document = FakeDocument(events)
    _prepare(document, events)
    collab, api = collaborators(document, events)

    result = run_sketch_offset(collab, "Doc", "Sketch", [0], 1.0, True, False)
...
def test_invalid_names_abort_without_recompute_or_commit(sketch_name):
    ...
    result = run_sketch_offset(collab, "Doc", sketch_name, [0], 1.0, True, False)
...
def test_missing_document_fails_without_entering_the_apply_callback():
    ...
    result = run_sketch_offset(collab, "Missing", "Sketch", [0], 1.0, True, False)
...
def test_sketch_offset_has_no_uncoordinated_source_entry_point():
    assert not hasattr(subject, "sketch_offset_gui")
    template = Path(__file__).parents[2] / 'src/freecad_mcp/templates/p2_editing/sketch_offset.py.txt'
...
def test_unknown_or_contradictory_native_evidence_cannot_release_success(native_result):
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops.sketch_offset_mutation import (
        _sketch_offset_native_result,
        _NativeSketchOffsetMutationState,
    )

    result = _sketch_offset_native_result(
        native_result, _NativeSketchOffsetMutationState(postcondition_passed=True)
    )
    assert result["success"] is False
