"""Adversarial wire cases for the ``sketch_add_bspline`` contract."""
...
def test_contract_version_must_be_the_exact_integer(version):
    assert parse_sketch_add_bspline_response(dict(_success(), contract_version=version))["success"] is False
...
def test_contradictory_or_malformed_success_cannot_escape(change):
    result = parse_sketch_add_bspline_response(dict(_success(), **change))
    assert result["success"] is False
    assert result["outcome"] == "uncertain"
    assert result["retry_safe"] is False
...
@pytest.mark.parametrize(
    ...
    [
        _success(),
        make_sketch_add_bspline_failure("BUSY", "Retry after recompute", native_status="Busy"),
        make_sketch_add_bspline_failure("INVALID_ARGUMENT", "Invalid name", retry_safe=False),
        *[
            make_sketch_add_bspline_uncertain("STATE_UNKNOWN", "Reconcile before replay", committed=committed)
            for committed in (None, False, True)
        ],
    ],
)
def test_valid_variants_round_trip_without_changing_evidence(raw):
    assert parse_sketch_add_bspline_response(raw) == raw
    assert parse_sketch_add_bspline_response(parse_sketch_add_bspline_response(raw)) == raw
...
def test_gui_completion_timeout_preserves_unknown_model_state():
    ...
    response = sketch_add_bspline_operation(
        SimpleNamespace(sketch_add_bspline=lambda *_args, **_kwargs: raw), True, "Doc", "Sketch", [{'x': 0.0, 'y': 0.0}, {'x': 3.0, 'y': 4.0}, {'x': 7.0, 'y': 4.0}, {'x': 10.0, 'y': 0.0}], 3, None, None, None, False, False
    )
...
def test_transport_failure_preserves_unknown_model_state():
    ...
    response = sketch_add_bspline_operation(
        SimpleNamespace(sketch_add_bspline=lost_response), True, "Doc", "Sketch", [{'x': 0.0, 'y': 0.0}, {'x': 3.0, 'y': 4.0}, {'x': 7.0, 'y': 4.0}, {'x': 10.0, 'y': 0.0}], 3, None, None, None, False, False
    )
    assert response.isError is True
    data = response.structuredContent["data"]
    assert data["error_code"] == "SKETCH_ADD_BSPLINE_TRANSPORT_UNCERTAIN"
