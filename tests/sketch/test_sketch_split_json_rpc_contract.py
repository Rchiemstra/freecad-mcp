"""Agent-to-FreeCAD wire contract for the ``sketch_split`` MCP operation."""
...
_TOOL_PARAMS = {'doc_name': 'AgentDocument', 'sketch_name': 'Sketch', 'geo_index': 0, 'point_x': 5.0, 'point_y': 0.0}
...
def _invoke_registered(monkeypatch, transport, arguments):
    ...
    mcp = InstrumentedFastMCP("sketch_split-contract")
    exports = {}
    tools_sketch_curves_b2._register_sketch_split(mcp, dependencies=object(), exports=exports)

    async def invoke():
        async with create_connected_server_and_client_session(mcp._mcp_server) as client:
            listing = await client.list_tools()
            tool = next(tool for tool in listing.tools if tool.name == "sketch_split")
            assert set(tool.inputSchema["required"]) == {'geo_index', 'doc_name', 'sketch_name', 'point_y', 'point_x'}
            return await client.call_tool("sketch_split", arguments)
...
def test_sketch_split_sends_exact_authenticated_json_rpc_values_to_freecad(monkeypatch):
    ...
    assert envelope["method"] == "sketch_split"
