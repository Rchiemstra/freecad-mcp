"""Agent-to-FreeCAD wire contract for the ``sketch_add_polyline`` MCP operation."""
...
_TOOL_PARAMS = {'doc_name': 'AgentDocument', 'sketch_name': 'Sketch', 'points': [{'x': 0.0, 'y': 0.0}, {'x': 10.0, 'y': 0.0}], 'closed': False, 'construction': False}
...
def _invoke_registered(monkeypatch, transport, arguments):
    ...
    mcp = InstrumentedFastMCP("sketch_add_polyline-contract")
    exports = {}
    tools_sketch_curves_a._register_sketch_add_polyline(mcp, dependencies=object(), exports=exports)

    async def invoke():
        async with create_connected_server_and_client_session(mcp._mcp_server) as client:
            listing = await client.list_tools()
            tool = next(tool for tool in listing.tools if tool.name == "sketch_add_polyline")
            assert set(tool.inputSchema["required"]) == {'doc_name', 'sketch_name', 'points'}
            return await client.call_tool("sketch_add_polyline", arguments)
...
def test_sketch_add_polyline_sends_exact_authenticated_json_rpc_values_to_freecad(monkeypatch):
    ...
    assert envelope["method"] == "sketch_add_polyline"
