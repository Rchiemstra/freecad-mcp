"""Unit coverage for the typed ``sketch_constrain_distance`` mutation slice."""
...
from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops import sketch_constrain_distance as subject
...
def test_sketch_constrain_distance_runs_apply_recompute_inspect_validate_then_commits():
    events = []
    document = FakeDocument(events)
    _prepare(document, events)
    collab, api = collaborators(document, events)

    result = run_sketch_constrain_distance(collab, "Doc", "Sketch", 0, 10.0, None, None)
...
def test_invalid_names_abort_without_recompute_or_commit(sketch_name):
    ...
    result = run_sketch_constrain_distance(collab, "Doc", sketch_name, 0, 10.0, None, None)
...
def test_missing_document_fails_without_entering_the_apply_callback():
    ...
    result = run_sketch_constrain_distance(collab, "Missing", "Sketch", 0, 10.0, None, None)
...
def test_sketch_constrain_distance_has_no_uncoordinated_source_entry_point():
    assert not hasattr(subject, "sketch_constrain_distance_gui")
...
def test_unknown_or_contradictory_native_evidence_cannot_release_success(native_result):
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops.sketch_constrain_distance_mutation import (
        _sketch_constrain_distance_native_result,
        _NativeSketchConstrainDistanceMutationState,
    )

    result = _sketch_constrain_distance_native_result(
        native_result, _NativeSketchConstrainDistanceMutationState(postcondition_passed=True)
    )
    assert result["success"] is False
