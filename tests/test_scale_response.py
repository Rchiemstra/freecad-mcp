"""Adversarial wire cases for the scale contract."""
...
def test_valid_success_round_trips():
    raw = _success()
    assert parse_scale_response(raw) == raw
...
def test_transport_failure_preserves_unknown_model_state():
    ...
    response = scale_operation(SimpleNamespace(scale=lost_response), True, "Doc", "Box", 1.0, 1.0, 1.0)
    assert response.isError is True
    data = response.structuredContent["data"]
    assert data["error_code"] == "SCALE_TRANSPORT_UNCERTAIN"
    assert data["outcome"] == "uncertain"
    assert data["retry_safe"] is False


def test_contradictory_success_cannot_escape():
    result = parse_scale_response(dict(_success(), rollback_failed=True))
    assert result["success"] is False
    assert result["outcome"] == "uncertain"
