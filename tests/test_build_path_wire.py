"""Unit coverage for the typed ``build_path_wire`` mutation slice."""
...
def _item(name: str, *, label: str | None = None, type_id: str = "Part::Feature"):
    obj = SimpleNamespace(
        Name=name,
...
def test_build_path_wire_runs_apply_recompute_inspect_validate_then_commits():
    events: list[str] = []
...
def test_creation_that_changes_then_raises_is_rolled_back(monkeypatch):
    ...
    monkeypatch.setattr(subject, "apply_build_path_wire", mutate_then_raise)
    result = run_build_path_wire(collaborators, "Doc", "Created", [{"sketch": "Seed", "geo_index": 0}], 0.5, None, "error")
    assert result["success"] is False
...
def test_unknown_or_contradictory_native_evidence_cannot_release_success():
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops.build_path_wire_mutation import (
        _build_path_wire_native_result,
    ...
    for native_result in (
        ...
        result = _build_path_wire_native_result(native_result, _NativeMutationState(postcondition_passed=True))
        assert result["success"] is False
