"""Unit coverage for the typed ``center_of_mass`` mutation slice."""
...
from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops import center_of_mass as subject
...
def _collaborators(document, events, *, validator=None, final_result=None, monkeypatch=None):
    if monkeypatch is not None:
        monkeypatch.setattr(measure_io_actions, "center_of_mass", _fake_payload)
...
def test_center_of_mass_runs_apply_recompute_inspect_validate_then_commits(monkeypatch):
    events = []
    document = _Document(events)
    collaborators, api = _collaborators(document, events, monkeypatch=monkeypatch)
    result = run_center_of_mass(collaborators, "Doc", "Box")
...
def test_invalid_names_abort_without_recompute_or_commit(monkeypatch):
    ...
    result = run_center_of_mass(collaborators, "Doc", "")
...
def test_center_of_mass_has_typed_rpc_handler():
    assert subject.TYPED_RPC_HANDLER[0] == "center_of_mass"
    assert callable(subject.TYPED_RPC_HANDLER[1])


def test_unknown_native_evidence_cannot_release_success():
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops.center_of_mass_mutation import (
        _center_of_mass_native_result,
        _NativeMutationState,
    )
    result = _center_of_mass_native_result({"status": "FutureStatus", "committed": False}, _NativeMutationState(postcondition_passed=True))
    assert result["success"] is False
