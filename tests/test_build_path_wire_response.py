"""Adversarial wire cases for the ``build_path_wire`` contract."""
...
def test_contract_version_must_be_the_exact_integer(version):
    assert parse_build_path_wire_response(dict(_success(), contract_version=version))["success"] is False
...
def test_contradictory_or_malformed_success_cannot_escape(change):
    result = parse_build_path_wire_response(dict(_success(), **change))
    assert result["success"] is False
    assert result["outcome"] == "uncertain"
    assert result["retry_safe"] is False
...
def test_valid_success_round_trips():
    raw = _success()
    assert parse_build_path_wire_response(raw) == raw
...
def test_transport_failure_preserves_unknown_model_state():
    ...
    response = build_path_wire_operation(_Conn(), True, "Doc", "Value", [{"sketch": "Seed", "geo_index": 0}], 0.5, None, "error")
    assert response.isError is True
    data = response.structuredContent["data"]
    assert data["error_code"] == "BUILD_PATH_WIRE_TRANSPORT_UNCERTAIN"
