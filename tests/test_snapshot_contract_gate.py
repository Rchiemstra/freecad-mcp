...
MUTATION = "addon/FreeCADMCP/rpc_server/methods/cad_methods_ops/snapshot_mutation.py"
...
def test_snapshot_architecture_gate_accepts_the_production_path() -> None:
...
def test_gate_rejects_inspection_before_recompute() -> None:
    ...
    assert "SNAPSHOT004 missing typed postcondition=inspect" in scan_snapshot_architecture(
...
def test_gate_rejects_cached_success_after_native_rejection() -> None:
    ...
    assert "SNAPSHOT007 cached success escaped native rejection" in scan_snapshot_architecture(
