"""Native qualification matrix for ``spreadsheet_set_alias``."""
...
def test_spreadsheet_set_alias_native_success_inspects_after_recompute(monkeypatch):
    _require_native()
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops import spreadsheet_set_alias as subject

    document = FreeCAD.newDocument("MCPSpreadsheetSetAliasNativeSuccess")
    events = []
    ...
    _prepare(document)
    events.clear()
    original_apply = subject.apply_spreadsheet_set_alias
    original_read = subject.read_spreadsheet_set_alias_result
    ...
    monkeypatch.setattr(subject, "apply_spreadsheet_set_alias", tracked_apply)
    monkeypatch.setattr(subject, "read_spreadsheet_set_alias_result", tracked_read)
    try:
        result = subject.run_spreadsheet_set_alias(_collaborators(FreeCAD, lambda _d: events.append("validate")), document.Name, "Target", "A1", "Wall")
...
def test_spreadsheet_set_alias_native_validation_failure_restores_complete_state():
    _require_native()
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops.spreadsheet_set_alias import run_spreadsheet_set_alias

    document = FreeCAD.newDocument("MCPSpreadsheetSetAliasNativeRollback")
    _prepare(document)
    state_before = _model_state(document)
    try:
        result = run_spreadsheet_set_alias(
            _collaborators(FreeCAD, lambda _d: (_ for _ in ()).throw(RuntimeError("forced validation failure"))),
            document.Name, "Target", "A1", "Wall",
        )
...
def test_spreadsheet_set_alias_native_apply_failure_restores(monkeypatch):
    _require_native()
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops import spreadsheet_set_alias as subject

    document = FreeCAD.newDocument("MCPSpreadsheetSetAliasNativeApplyFailure")
    _prepare(document)
    state_before = _model_state(document)
    original = subject.apply_spreadsheet_set_alias
    ...
    monkeypatch.setattr(subject, "apply_spreadsheet_set_alias", mutates_then_raises)
    try:
        result = subject.run_spreadsheet_set_alias(_collaborators(FreeCAD, lambda _d: None), document.Name, "Target", "A1", "Wall")
        assert result["success"] is False
...
def test_spreadsheet_set_alias_native_recompute_failure_rolls_back(monkeypatch):
    _require_native()
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops import spreadsheet_set_alias as subject

    document = FreeCAD.newDocument("MCPSpreadsheetSetAliasNativeRecomputeFailure")
    ...
    monkeypatch.setattr(subject, "apply_spreadsheet_set_alias", arm)
    try:
        result = subject.run_spreadsheet_set_alias(_collaborators(FreeCAD, lambda _d: None), document.Name, "Target", "A1", "Wall")
        assert result["success"] is False
...
def test_spreadsheet_set_alias_native_rollback_failure_is_uncertain_and_fences(monkeypatch):
    _require_native()
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops import spreadsheet_set_alias as subject

    document = FreeCAD.newDocument("MCPSpreadsheetSetAliasNativeRollbackFailure")
    ...
    try:
        result = subject.run_spreadsheet_set_alias(collaborators, document.Name, "Target", "A1", "Wall")
        proxy.armed = False
        fenced = subject.run_spreadsheet_set_alias(collaborators, document.Name, "Target", "A1", "Wall")
...
def test_spreadsheet_set_alias_native_inspection_failure_rolls_back(monkeypatch):
    _require_native()
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops import spreadsheet_set_alias as subject

    document = FreeCAD.newDocument("MCPSpreadsheetSetAliasNativeInspectionFailure")
    _prepare(document)
    state_before = _model_state(document)

    def reject(_document, _receipt):
        raise subject.SpreadsheetSetAliasError("CREATED_OBJECT_WRONG_TYPE", "forced inspection failure")

    monkeypatch.setattr(subject, "read_spreadsheet_set_alias_result", reject)
    try:
        result = subject.run_spreadsheet_set_alias(_collaborators(FreeCAD, lambda _d: None), document.Name, "Target", "A1", "Wall")
        assert result["success"] is False
...
def test_spreadsheet_set_alias_native_postcondition_cannot_write():
    _require_native()
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops.spreadsheet_set_alias import run_spreadsheet_set_alias

    document = FreeCAD.newDocument("MCPSpreadsheetSetAliasReadOnlyPostcondition")
    ...
    try:
        result = run_spreadsheet_set_alias(_collaborators(FreeCAD, validate), document.Name, "Target", "A1", "Wall")
