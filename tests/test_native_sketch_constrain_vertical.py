"""Native FreeCAD qualification for ``sketch_constrain_vertical``."""
...
def test_sketch_constrain_vertical_native_success_on_idle_sketch():
    _require_native_collaboration()
    import FreeCAD
    import Part
    import Sketcher

    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops.sketch_constrain_vertical import run_sketch_constrain_vertical

    document, sketch = _idle_sketch(FreeCAD, Part, "MCPSketchConstrainVerticalNativeSuccess")
    before = sketch.GeometryCount, sketch.ConstraintCount
    try:
        result = run_sketch_constrain_vertical(
            _collaborators(FreeCAD, Part, Sketcher, lambda _doc: None),
            document.Name, sketch.Name, 1,
        )
...
def test_sketch_constrain_vertical_native_validation_failure_restores_geometry_counts():
    _require_native_collaboration()
    import FreeCAD
    import Part
    import Sketcher
    ...
    document, sketch = _idle_sketch(FreeCAD, Part, "MCPSketchConstrainVerticalNativeValidation")
    ...
    try:
        result = run_sketch_constrain_vertical(_collaborators(FreeCAD, Part, Sketcher, fail), document.Name, sketch.Name, 1)
...
def test_sketch_constrain_vertical_native_missing_document_keeps_typed_error():
    _require_native_collaboration()
    import FreeCAD
    import Part
    import Sketcher
...
def test_sketch_constrain_vertical_native_failure_isolated_and_healthy_document_succeeds():
    _require_native_collaboration()
    import FreeCAD
    import Part
    import Sketcher
    ...
    failed_document, failed_sketch = _idle_sketch(FreeCAD, Part, "MCPSketchConstrainVerticalIsolatedFailure")
    healthy_document, healthy_sketch = _idle_sketch(FreeCAD, Part, "MCPSketchConstrainVerticalHealthyDocument")
    ...
    try:
        failed = run_sketch_constrain_vertical(collab, failed_document.Name, failed_sketch.Name, 1)
        isolated = run_sketch_constrain_vertical(collab, healthy_document.Name, healthy_sketch.Name, 1)
...
def test_sketch_constrain_vertical_native_postcondition_cannot_write():
    _require_native_collaboration()
    import FreeCAD
    import Part
    import Sketcher
    ...
    document, sketch = _idle_sketch(FreeCAD, Part, "MCPSketchConstrainVerticalReadOnlyPostcondition")
