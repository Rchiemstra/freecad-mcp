"""Agent-to-FreeCAD wire contract for the ``validate_geometry`` MCP operation."""
...
class _RecordingFreeCADTransport:
    ...
    def request(self, path, payload, headers):
        ...
        if body_result is _DEFAULT_RESPONSE:
            from tests.validate_geometry.test_validate_geometry_response import _success
...
def _invoke(monkeypatch, transport, arguments):
    ...
    mcp = InstrumentedFastMCP("validate_geometry-contract")
    exports = {}
    tools_measure_b._register_validate_geometry(mcp, dependencies=object(), exports=exports)

    async def invoke():
        async with create_connected_server_and_client_session(mcp._mcp_server) as client:
            listing = await client.list_tools()
            tool = next(tool for tool in listing.tools if tool.name == "validate_geometry")
            return await client.call_tool("validate_geometry", arguments)
...
def test_validate_geometry_sends_authenticated_json_rpc_to_freecad(monkeypatch):
    ...
    assert envelope["method"] == "validate_geometry"
