"""Adversarial wire cases for the create_assembly contract."""
...
def _success():
    return make_create_assembly_success(assembly="Assembly", label="Assembly", type="Assembly::AssemblyObject", joint_group=None)


@pytest.mark.parametrize("raw", [None, [], 1, "timeout", {}, {1: "bad key"}])
def test_unknown_response_never_proves_rejection(raw):
    result = parse_create_assembly_response(raw)
...
def test_valid_success_round_trips():
    raw = _success()
    assert parse_create_assembly_response(raw) == raw
...
def test_transport_failure_preserves_unknown_model_state():
    ...
    response = create_assembly_operation(SimpleNamespace(create_assembly=lost_response), True, "Doc", "Assembly")
    assert response.isError is True
    data = response.structuredContent["data"]
    assert data["error_code"] == "CREATE_ASSEMBLY_TRANSPORT_UNCERTAIN"
    assert data["outcome"] == "uncertain"
    assert data["retry_safe"] is False


def test_contradictory_success_cannot_escape():
    result = parse_create_assembly_response(dict(_success(), rollback_failed=True))
    assert result["success"] is False
    assert result["outcome"] == "uncertain"
