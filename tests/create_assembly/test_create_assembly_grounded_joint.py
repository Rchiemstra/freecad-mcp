"""Unit coverage for the typed ``create_assembly_grounded_joint`` mutation slice."""
...
from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops import create_assembly_grounded_joint as subject
...
def _collaborators(document, events, *, validator=None, final_result=None, monkeypatch=None):
    if monkeypatch is not None:
        monkeypatch.setattr(assembly_actions, "create_grounded_joint", _fake_payload)
...
def test_create_assembly_grounded_joint_runs_apply_recompute_inspect_validate_then_commits(monkeypatch):
    events = []
    document = _Document(events)
    collaborators, api = _collaborators(document, events, monkeypatch=monkeypatch)
    result = run_create_assembly_grounded_joint(collaborators, "Doc", "Assembly", "Base")
...
def test_invalid_names_abort_without_recompute_or_commit(monkeypatch):
    ...
    result = run_create_assembly_grounded_joint(collaborators, "Doc", "", "Base")
...
def test_create_assembly_grounded_joint_has_typed_rpc_handler():
    assert subject.TYPED_RPC_HANDLER[0] == "create_assembly_grounded_joint"
    assert callable(subject.TYPED_RPC_HANDLER[1])


def test_unknown_native_evidence_cannot_release_success():
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops.create_assembly_grounded_joint_mutation import (
        _create_assembly_grounded_joint_native_result,
        _NativeMutationState,
    )
    result = _create_assembly_grounded_joint_native_result({"status": "FutureStatus", "committed": False}, _NativeMutationState(postcondition_passed=True))
    assert result["success"] is False
