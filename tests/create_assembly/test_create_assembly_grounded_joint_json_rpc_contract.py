"""Agent-to-FreeCAD wire contract for the ``create_assembly_grounded_joint`` MCP operation."""
...
from freecad_mcp._shared.protocol.create_assembly_grounded_joint_contract import make_create_assembly_grounded_joint_success
from freecad_mcp.freecad_client import FreeCADConnection
from freecad_mcp.freecad_client_ops.connection_methods.connection_headers_ops import (
    configure_rpc_session,
)
from freecad_mcp.generated.capabilities.register_modules import tools_assembly
...
class _RecordingFreeCADTransport:
    ...
    def request(self, path, payload, headers):
        ...
        if body_result is _DEFAULT_RESPONSE:
            from tests.create_assembly.test_create_assembly_grounded_joint_response import _success
...
def _invoke(monkeypatch, transport, arguments):
    ...
    monkeypatch.setattr(tools_assembly, "server_connection", lambda: connection)
    monkeypatch.setattr(
        tools_assembly,
        "server_state",
    ...
    mcp = InstrumentedFastMCP("create_assembly_grounded_joint-contract")
    exports = {}
    tools_assembly._register_create_assembly_grounded_joint(mcp, dependencies=object(), exports=exports)

    async def invoke():
        async with create_connected_server_and_client_session(mcp._mcp_server) as client:
            listing = await client.list_tools()
            tool = next(tool for tool in listing.tools if tool.name == "create_assembly_grounded_joint")
            return await client.call_tool("create_assembly_grounded_joint", arguments)
...
def test_create_assembly_grounded_joint_sends_authenticated_json_rpc_to_freecad(monkeypatch):
    ...
    if "create_assembly_grounded_joint" == "create_assembly_grounded_joint":
        arguments.update({"assembly_name": "Assembly", "component_name": "Base"})
    elif "create_assembly_grounded_joint" == "create_assembly_joint":
        arguments.update({
    ...
    elif "create_assembly_grounded_joint" == "solve_assembly":
        arguments.update({"assembly_name": "Assembly"})
    elif "create_assembly_grounded_joint" in {"create_helical_gear", "create_involute_gear", "create_spur_gear"}:
        arguments.update({"gear_name": "Gear", "teeth": 8, "module": 2.0, "width": 10.0})
    elif "create_assembly_grounded_joint" == "export_brep":
        arguments.update({"obj_name": "Box", "file_path": "/tmp/out.brep"})
    elif "create_assembly_grounded_joint" in {"export_step", "export_stl"}:
        arguments.update({"file_path": "/tmp/out"})
    elif "create_assembly_grounded_joint" in {"import_brep", "import_step"}:
        arguments.update({"file_path": "/tmp/in"})
    elif "create_assembly_grounded_joint" in {"bounding_box", "center_of_mass"}:
        arguments.update({"obj_name": "Box"})
    elif "create_assembly_grounded_joint" == "common_volume_along_path":
        arguments.update({"moving_object": "Mover", "obstacle_objects": ["Wall"], "samples": [{"x": 0, "y": 0, "z": 0}]})
    elif "create_assembly_grounded_joint" == "rotate":
        arguments.update({"obj_name": "Box", "axis_x": 0, "axis_y": 0, "axis_z": 1, "angle_deg": 90})
    elif "create_assembly_grounded_joint" == "scale":
        arguments.update({"obj_name": "Box", "sx": 1, "sy": 1, "sz": 1})
    elif "create_assembly_grounded_joint" == "translate":
    ...
    assert envelope["method"] == "create_assembly_grounded_joint"
    assert envelope["params"]["doc_name"] == "AgentDocument"
    assert envelope["session_token"] == "test-session-token"
    assert envelope["operation"]["name"] == "Create Grounded Joint"
