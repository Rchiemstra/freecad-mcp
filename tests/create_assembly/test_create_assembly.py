"""Unit coverage for the typed ``create_assembly`` mutation slice."""
...
from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops import create_assembly as subject
...
def _fake_payload(document, *args, **kwargs):
    ...
    obj = document.addObject("Assembly::AssemblyObject", "Created")
    return {
        "assembly": obj.Name,
        "label": obj.Label,
        "type": obj.TypeId,
...
def _collaborators(document, events, *, validator=None, final_result=None, monkeypatch=None):
    if monkeypatch is not None:
        monkeypatch.setattr(assembly_actions, "create_assembly", _fake_payload)
...
def test_create_assembly_runs_apply_recompute_inspect_validate_then_commits(monkeypatch):
    events = []
    document = _Document(events)
    collaborators, api = _collaborators(document, events, monkeypatch=monkeypatch)
    result = run_create_assembly(collaborators, "Doc", "Assembly")
...
def test_invalid_names_abort_without_recompute_or_commit(monkeypatch):
    ...
    result = run_create_assembly(collaborators, "Doc", "")
...
def test_create_assembly_has_typed_rpc_handler():
    assert subject.TYPED_RPC_HANDLER[0] == "create_assembly"
    assert callable(subject.TYPED_RPC_HANDLER[1])


def test_unknown_native_evidence_cannot_release_success():
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops.create_assembly_mutation import (
        _create_assembly_native_result,
        _NativeMutationState,
    )
    result = _create_assembly_native_result({"status": "FutureStatus", "committed": False}, _NativeMutationState(postcondition_passed=True))
    assert result["success"] is False
