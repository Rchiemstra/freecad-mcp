...
"""Native collaboration coverage for the typed ``sketch_add_geometry`` mutation."""
...
def _revision_state(document):
    ...
    session = document.beginEditSession("sketch_add_geometry-native-state-probe")
    try:
...
def test_sketch_add_geometry_native_success_inspects_after_recompute(monkeypatch):
    _require_native_collaboration()
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops import sketch_add_geometry as body_create_module

    document = FreeCAD.newDocument("MCPSketchAddGeometryNativePhaseOrder")
    events = []
    ...
    monkeypatch.setattr(body_create_module, "apply_sketch_add_geometry", tracked_apply)
    monkeypatch.setattr(body_create_module, "read_sketch_add_geometry_result", tracked_read)
    collaborators = _collaborators(FreeCAD, lambda _document: events.append("validate"))
    try:
        result = body_create_module.run_sketch_add_geometry(collaborators, document.Name, "Sketch", [{"type": "circle", "center": {"x": 0, "y": 0}, "radius": 8}])
...
def test_sketch_add_geometry_native_validation_failure_restores_complete_state():
    _require_native_collaboration()
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops.sketch_add_geometry import run_sketch_add_geometry

    document = FreeCAD.newDocument("MCPSketchAddGeometryNativeRollback")
    _idle(document, 'geometry')
    ...
    try:
        result = run_sketch_add_geometry(collaborators, document.Name, "Sketch", [{"type": "circle", "center": {"x": 0, "y": 0}, "radius": 8}])
...
def test_sketch_add_geometry_native_apply_failure_restores_all_attempted_effects(monkeypatch):
    _require_native_collaboration()
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops import sketch_add_geometry as body_create_module

    document = FreeCAD.newDocument("MCPSketchAddGeometryNativeApplyFailure")
    _idle(document, 'geometry')
    state_before = _model_state(document)
    revision_before = _revision_state(document)
    original_apply = body_create_module.apply_sketch_add_geometry
    ...
    monkeypatch.setattr(body_create_module, "apply_sketch_add_geometry", mutates_then_raises)
...
def test_sketch_add_geometry_native_recompute_failure_rolls_back(monkeypatch):
    _require_native_collaboration()
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops import sketch_add_geometry as body_create_module

    document = FreeCAD.newDocument("MCPSketchAddGeometryNativeRecomputeFailure")
    ...
    monkeypatch.setattr(body_create_module, "apply_sketch_add_geometry", arm_recompute_failure)
    collaborators = _collaborators(FreeCAD, lambda _document: None)
    try:
        result = body_create_module.run_sketch_add_geometry(collaborators, document.Name, "Sketch", [{"type": "circle", "center": {"x": 0, "y": 0}, "radius": 8}])
        assert result["success"] is False
...
def test_sketch_add_geometry_native_rollback_failure_is_uncertain_and_fences_document(monkeypatch):
    _require_native_collaboration()
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops import sketch_add_geometry as body_create_module

    document = FreeCAD.newDocument("MCPSketchAddGeometryNativeRollbackFailure")
    ...
    monkeypatch.setattr(body_create_module, "apply_sketch_add_geometry", arm_persistent_failure)
    collaborators = _collaborators(FreeCAD, lambda _document: None)
    try:
        result = body_create_module.run_sketch_add_geometry(collaborators, document.Name, "Sketch", [{"type": "circle", "center": {"x": 0, "y": 0}, "radius": 8}])
        proxy.armed = False
        fenced = body_create_module.run_sketch_add_geometry(collaborators, document.Name, "Sketch", [{"type": "circle", "center": {"x": 0, "y": 0}, "radius": 8}])
        assert result["success"] is False
...
def test_sketch_add_geometry_native_inspection_failure_rolls_back_after_recompute(monkeypatch):
    _require_native_collaboration()
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops import sketch_add_geometry as body_create_module

    document = FreeCAD.newDocument("MCPSketchAddGeometryNativeInspectionFailure")
    _idle(document, 'geometry')
    state_before = _model_state(document)
    revision_before = _revision_state(document)

    def reject_inspection(*_args, **_kwargs):
        raise body_create_module.SketchAddGeometryError(
            "CREATED_OBJECT_WRONG_TYPE", "forced post-recompute inspection failure"
        )

    monkeypatch.setattr(body_create_module, "read_sketch_add_geometry_result", reject_inspection)
    collaborators = _collaborators(FreeCAD, lambda _document: None)
    try:
        result = body_create_module.run_sketch_add_geometry(collaborators, document.Name, "Sketch", [{"type": "circle", "center": {"x": 0, "y": 0}, "radius": 8}])
        assert result["success"] is False
...
def test_sketch_add_geometry_native_failure_isolated_and_healthy_rollback_recovers():
    _require_native_collaboration()
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops.sketch_add_geometry import run_sketch_add_geometry

    failed_document = FreeCAD.newDocument("MCPSketchAddGeometryIsolatedFailure")
    healthy_document = FreeCAD.newDocument("MCPSketchAddGeometryHealthyDocument")
    _idle(failed_document, 'geometry')
    _idle(healthy_document, 'geometry')
    ...
    try:
        failed = run_sketch_add_geometry(collaborators, failed_document.Name, "Sketch", [{"type": "circle", "center": {"x": 0, "y": 0}, "radius": 8}])
        isolated = run_sketch_add_geometry(collaborators, healthy_document.Name, "Sketch", [{"type": "circle", "center": {"x": 0, "y": 0}, "radius": 8}])
        rejected_names.clear()
        recovered = run_sketch_add_geometry(collaborators, failed_document.Name, "Sketch", [{"type": "circle", "center": {"x": 0, "y": 0}, "radius": 8}])
...
def test_sketch_add_geometry_native_duplicate_and_missing_document_keep_typed_errors():
    _require_native_collaboration()
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops.sketch_add_geometry import run_sketch_add_geometry

    document = FreeCAD.newDocument("MCPSketchAddGeometryTypedErrors")
    _idle(document, 'geometry')
    collaborators = _collaborators(FreeCAD, lambda _: None)
    try:
        first = run_sketch_add_geometry(collaborators, document.Name, "Sketch", [{"type": "circle", "center": {"x": 0, "y": 0}, "radius": 8}])
        assert first["success"] is True
        before = _model_state(document), _revision_state(document)
        result = run_sketch_add_geometry(collaborators, document.Name, "MissingSketch", [{"type": "circle", "center": {"x": 0, "y": 0}, "radius": 8}])
        assert result["error_code"] == 'SKETCH_NOT_FOUND'
        assert result["retry_safe"] is True
        assert (_model_state(document), _revision_state(document)) == before
        missing = run_sketch_add_geometry(collaborators, "NoSuchAddGeometryQualificationDocument", "Sketch", [{"type": "circle", "center": {"x": 0, "y": 0}, "radius": 8}])
...
def test_sketch_add_geometry_native_rollback_restores_rich_model(monkeypatch, stage):
    _require_native_collaboration()
    import FreeCAD
    import Part
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops import sketch_add_geometry as subject

    document = FreeCAD.newDocument("MCPSketchAddGeometryRichRollback")
    ...
    _idle(document, 'geometry')
    before = _model_state(document), _revision_state(document)
    original_apply = subject.apply_sketch_add_geometry
    original_inspect = subject.read_sketch_add_geometry_result
    ...
    def inspect(*args, **kwargs):
        if stage == "inspection":
            raise subject.SketchAddGeometryError("INSPECTION_FAILED", "injected inspection failure")
    ...
    monkeypatch.setattr(subject, "apply_sketch_add_geometry", change_model)
    monkeypatch.setattr(subject, "read_sketch_add_geometry_result", inspect)
    try:
        result = subject.run_sketch_add_geometry(_collaborators(FreeCAD, validate), document.Name, "Sketch", [{"type": "circle", "center": {"x": 0, "y": 0}, "radius": 8}])
...
def test_sketch_add_geometry_native_postcondition_cannot_write(monkeypatch, write):
    _require_native_collaboration()
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops import sketch_add_geometry as subject

    document = FreeCAD.newDocument("MCPSketchAddGeometryReadOnlyPostcondition")
