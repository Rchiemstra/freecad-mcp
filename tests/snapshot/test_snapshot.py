"""Unit coverage for the typed ``snapshot`` mutation slice."""
...
def test_snapshot_runs_apply_recompute_inspect_validate_then_commits():
    events: list[str] = []
...
def test_unknown_or_contradictory_native_evidence_cannot_release_success():
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops.snapshot_mutation import (
        _snapshot_native_result,
    ...
    for native_result in (
        ...
        result = _snapshot_native_result(native_result, _NativeMutationState(postcondition_passed=True))
        assert result["success"] is False
