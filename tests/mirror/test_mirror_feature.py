"""Unit coverage for the typed ``mirror_feature`` mutation slice."""
...
from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops import mirror_feature as subject
from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops.mirror_feature import run_mirror_feature
from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops.mirror_feature_mutation import (
    _MirrorFeatureNativeMutationState,
    _mirror_feature_native_result,
)
...
def _run(collab, **overrides):
    ...
    return run_mirror_feature(collab, **payload)


def test_mirror_feature_runs_apply_recompute_inspect_validate_then_commits():
    events: list[str] = []
...
def test_creation_that_changes_then_raises_is_rolled_back():
    ...
    assert result["success"] is False
    assert result["error_code"] == "MIRROR_FEATURE_FAILED"
...
def test_unknown_or_contradictory_native_evidence_cannot_release_success(native_result):
    result = _mirror_feature_native_result(
        native_result, _MirrorFeatureNativeMutationState(postcondition_passed=True)
    )
    assert result["success"] is False
...
def test_typed_rpc_handler_is_registered():
    ...
    assert "mirror_feature" in discover_typed_rpc_handlers()
    assert subject.TYPED_RPC_HANDLER[0] == "mirror_feature"
