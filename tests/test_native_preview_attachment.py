"""Native qualification matrix for ``preview_attachment``."""
...
def test_preview_attachment_native_success_inspects_after_recompute(monkeypatch):
    _require_native_collaboration()
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops import preview_attachment as subject

    document = FreeCAD.newDocument("MCPPreviewAttachmentNativeSuccess")
    events = []
    ...
    _prepare(document)
    events.clear()
    original_apply = subject.apply_preview_attachment
    original_read = subject.read_preview_attachment_result
    ...
    monkeypatch.setattr(subject, "apply_preview_attachment", tracked_apply)
    monkeypatch.setattr(subject, "read_preview_attachment_result", tracked_read)
    try:
        result = subject.run_preview_attachment(_collaborators(FreeCAD, lambda _d: events.append("validate")), document.Name, "Target")
...
def test_preview_attachment_native_validation_failure_restores_complete_state():
    _require_native_collaboration()
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops.preview_attachment import run_preview_attachment

    document = FreeCAD.newDocument("MCPPreviewAttachmentNativeRollback")
    _prepare(document)
    state_before = _model_state(document)
    try:
        result = run_preview_attachment(
...
def test_preview_attachment_native_apply_failure_restores(monkeypatch):
    _require_native_collaboration()
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops import preview_attachment as subject

    document = FreeCAD.newDocument("MCPPreviewAttachmentNativeApplyFailure")
    _prepare(document)
    state_before = _model_state(document)
    original = subject.apply_preview_attachment
    ...
    monkeypatch.setattr(subject, "apply_preview_attachment", mutates_then_raises)
    try:
        result = subject.run_preview_attachment(_collaborators(FreeCAD, lambda _d: None), document.Name, "Target")
        assert result["success"] is False
...
def test_preview_attachment_native_recompute_failure_rolls_back(monkeypatch):
    _require_native_collaboration()
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops import preview_attachment as subject

    document = FreeCAD.newDocument("MCPPreviewAttachmentNativeRecomputeFailure")
    ...
    monkeypatch.setattr(subject, "apply_preview_attachment", arm)
    try:
        result = subject.run_preview_attachment(_collaborators(FreeCAD, lambda _d: None), document.Name, "Target")
        assert result["success"] is False
...
def test_preview_attachment_native_rollback_failure_is_uncertain_and_fences(monkeypatch):
    _require_native_collaboration()
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops import preview_attachment as subject

    document = FreeCAD.newDocument("MCPPreviewAttachmentNativeRollbackFailure")
    ...
    try:
        result = subject.run_preview_attachment(collaborators, document.Name, "Target")
        proxy.armed = False
        fenced = subject.run_preview_attachment(collaborators, document.Name, "Target")
...
def test_preview_attachment_native_inspection_failure_rolls_back(monkeypatch):
    _require_native_collaboration()
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops import preview_attachment as subject

    document = FreeCAD.newDocument("MCPPreviewAttachmentNativeInspectionFailure")
    _prepare(document)
    state_before = _model_state(document)

    def reject(_document, _receipt):
        raise subject.PreviewAttachmentError("CREATED_OBJECT_WRONG_TYPE", "forced inspection failure")

    monkeypatch.setattr(subject, "read_preview_attachment_result", reject)
    try:
        result = subject.run_preview_attachment(_collaborators(FreeCAD, lambda _d: None), document.Name, "Target")
        assert result["success"] is False
...
def test_preview_attachment_native_postcondition_cannot_write():
    _require_native_collaboration()
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops.preview_attachment import run_preview_attachment

    document = FreeCAD.newDocument("MCPPreviewAttachmentReadOnlyPostcondition")
    ...
    try:
        result = run_preview_attachment(_collaborators(FreeCAD, validate), document.Name, "Target")
