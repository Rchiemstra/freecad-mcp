"""Agent-to-FreeCAD wire contract for the ``sketch_attach`` MCP operation."""
...
def _invoke_registered(monkeypatch, transport, arguments):
    ...
    mcp = InstrumentedFastMCP("sketch_attach-contract")
    exports = {}
    tools_parametric_body._register_sketch_attach(
        mcp,
        dependencies=object(),
        exports=exports,
    )
    ...
    async def invoke():
        async with create_connected_server_and_client_session(mcp._mcp_server) as client:
            ...
            tool = next(tool for tool in listing.tools if tool.name == "sketch_attach")
            assert set(tool.inputSchema["required"]) == {'doc_name', 'sketch_name', 'support'}
            return await client.call_tool("sketch_attach", arguments)
...
def test_sketch_attach_sends_exact_authenticated_json_rpc_values_to_freecad(monkeypatch):
    transport = _RecordingFreeCADTransport()
    result = _invoke_registered(monkeypatch, transport, {'doc_name': 'AgentDocument', 'sketch_name': 'MainSketch', 'support': 'XY_Plane'})
    ...
    assert envelope == {
        ...
        "method": "sketch_attach",
        "params": {'doc_name': 'AgentDocument', 'sketch_name': 'MainSketch', 'support': 'XY_Plane'},
        "lease_credentials": [],
        "operation": {"name": "Attach sketch"},
    }
...
def test_public_sketch_attach_route_never_turns_rejection_into_success(monkeypatch):
    response_result = {
        "contract_version": 1,
        "success": False,
        "ok": False,
...
def test_public_route_marks_committed_but_invalid_response_non_retryable(monkeypatch):
    ...
    result = _invoke_registered(monkeypatch, transport, {'doc_name': 'AgentDocument', 'sketch_name': 'MainSketch', 'support': 'XY_Plane'})
    assert result.isError is True
    assert result.structuredContent["error_code"] == "SKETCH_ATTACH_COMMITTED_RESPONSE_INVALID"
...
def test_mcp_schema_rejects_non_string_before_json_rpc(monkeypatch, sketch_name):
    transport = _RecordingFreeCADTransport()
    arguments = dict({'doc_name': 'AgentDocument', 'sketch_name': 'MainSketch', 'support': 'XY_Plane'})
...
@pytest.mark.parametrize(
    ...
    [
        ...
        dict(_success_identity(), rollback_failed=True) if False else {
            ...
            "committed": True,
            "retry_safe": False,
            **{'sketch': 'x', 'attached_kind': 'x', 'attached_object': 'x', 'attached_subname': 'x'},
