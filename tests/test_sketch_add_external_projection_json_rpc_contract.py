"""Agent-to-FreeCAD wire contract for the ``sketch_add_external_projection`` MCP operation."""
...
class _RecordingFreeCADTransport:
    ...
    def request(self, path, payload, headers):
        ...
        if body_result is _DEFAULT_RESPONSE:
            body_result = dict({"contract_version": 1, "success": True, "ok": True, "outcome": "committed", "committed": True, "retry_safe": False, "sketch_name": "Value"}, **{
                key: envelope["params"][key]
                for key in envelope["params"]
                if key in ['sketch_name']
            })
            body_result = {"contract_version": 1, "success": True, "ok": True, "outcome": "committed", "committed": True, "retry_safe": False, "sketch_name": "Value"}
        response = {
            "jsonrpc": "2.0",
            "id": request["id"],
            "result": {
...
def _invoke_registered(monkeypatch, transport, params):
    ...
    mcp = InstrumentedFastMCP("sketch_add_external_projection-contract")
    exports = {}
    tools_partdesign_a2._register_sketch_add_external_projection(mcp, dependencies=object(), exports=exports)

    async def invoke():
        async with create_connected_server_and_client_session(mcp._mcp_server) as client:
            listing = await client.list_tools()
            tool = next(tool for tool in listing.tools if tool.name == "sketch_add_external_projection")
            required = tool.inputSchema.get("required") or []
            assert "doc_name" in required
            return await client.call_tool("sketch_add_external_projection", params)
...
def test_sketch_add_external_projection_sends_authenticated_json_rpc_to_freecad(monkeypatch):
    transport = _RecordingFreeCADTransport()
    result = _invoke_registered(monkeypatch, transport, {"doc_name": "AgentDocument", "sketch_name": "Value", "source_ref": "Value", "projection_mode": "auto", "defining": True, "allow_gui_geometry_loop": True})
    ...
    assert envelope["method"] == "sketch_add_external_projection"
...
def test_mcp_schema_rejects_non_string_doc_name_before_json_rpc(monkeypatch):
    transport = _RecordingFreeCADTransport()
    params = dict({"doc_name": "AgentDocument", "sketch_name": "Value", "source_ref": "Value", "projection_mode": "auto", "defining": True, "allow_gui_geometry_loop": True})
    params["doc_name"] = 42
