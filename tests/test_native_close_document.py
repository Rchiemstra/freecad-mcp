...
def test_close_document_native_commit_succeeds():
    _require_native_collaboration()

    import FreeCAD

    from addon.FreeCADMCP.collaboration_api import CollaborationAPI
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops.close_document import run_close_document

    document = FreeCAD.newDocument("MCPTypedCloseDocument")
    events: list[str] = []
    saved_path = ""
    try:
        
        if "close_document" == "reload_document":
            saved_path = document.Name + ".FCStd"
            document.saveAs(saved_path)
        if "close_document" == "open_document":
            saved_path = document.Name + ".FCStd"
            document.saveAs(saved_path)
        ...
        result = run_close_document(collab, document.Name)
