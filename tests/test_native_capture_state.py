"""Native qualification matrix for ``capture_state``."""
...
def test_capture_state_native_success_inspects_after_recompute(monkeypatch):
    _require_native_collaboration()
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops import capture_state as subject

    document = FreeCAD.newDocument("MCPCaptureStateNativeSuccess")
    events = []
    ...
    _prepare(document)
    events.clear()
    original_apply = subject.apply_capture_state
    original_read = subject.read_capture_state_result
...
def test_capture_state_native_validation_failure_restores_complete_state():
    _require_native_collaboration()
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops.capture_state import run_capture_state

    document = FreeCAD.newDocument("MCPCaptureStateNativeRollback")
    _prepare(document)
...
def test_capture_state_native_apply_failure_restores(monkeypatch):
    _require_native_collaboration()
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops import capture_state as subject

    document = FreeCAD.newDocument("MCPCaptureStateNativeApplyFailure")
    _prepare(document)
    state_before = _model_state(document)
    original = subject.apply_capture_state
...
def test_capture_state_native_recompute_failure_rolls_back(monkeypatch):
    _require_native_collaboration()
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops import capture_state as subject

    document = FreeCAD.newDocument("MCPCaptureStateNativeRecomputeFailure")
...
def test_capture_state_native_rollback_failure_is_uncertain_and_fences(monkeypatch):
    _require_native_collaboration()
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops import capture_state as subject

    document = FreeCAD.newDocument("MCPCaptureStateNativeRollbackFailure")
...
def test_capture_state_native_inspection_failure_rolls_back(monkeypatch):
    _require_native_collaboration()
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops import capture_state as subject

    document = FreeCAD.newDocument("MCPCaptureStateNativeInspectionFailure")
    _prepare(document)
    state_before = _model_state(document)

    def reject(_document, _receipt):
        raise subject.CaptureStateError("CREATED_OBJECT_WRONG_TYPE", "forced inspection failure")
...
def test_capture_state_native_postcondition_cannot_write():
    _require_native_collaboration()
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops.capture_state import run_capture_state

    document = FreeCAD.newDocument("MCPCaptureStateReadOnlyPostcondition")
