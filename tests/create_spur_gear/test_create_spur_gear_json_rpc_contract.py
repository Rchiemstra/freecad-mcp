"""Agent-to-FreeCAD wire contract for the ``create_spur_gear`` MCP operation."""
...
from freecad_mcp._shared.protocol.create_spur_gear_contract import make_create_spur_gear_success
...
class _RecordingFreeCADTransport:
    ...
    def request(self, path, payload, headers):
        ...
        if body_result is _DEFAULT_RESPONSE:
            from tests.create_spur_gear.test_create_spur_gear_response import _success
...
def _invoke(monkeypatch, transport, arguments):
    ...
    mcp = InstrumentedFastMCP("create_spur_gear-contract")
    exports = {}
    tools_gear_1._register_create_spur_gear(mcp, dependencies=object(), exports=exports)

    async def invoke():
        async with create_connected_server_and_client_session(mcp._mcp_server) as client:
            listing = await client.list_tools()
            tool = next(tool for tool in listing.tools if tool.name == "create_spur_gear")
            return await client.call_tool("create_spur_gear", arguments)
...
def test_create_spur_gear_sends_authenticated_json_rpc_to_freecad(monkeypatch):
    ...
    if "create_spur_gear" == "create_assembly_grounded_joint":
        arguments.update({"assembly_name": "Assembly", "component_name": "Base"})
    elif "create_spur_gear" == "create_assembly_joint":
        arguments.update({
