...
def test_scale_native_success_inspects_after_recompute(monkeypatch):
    ...
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops import scale as module

    document = FreeCAD.newDocument("MCPScaleNative")
    events: list[str] = []
    ...
    monkeypatch.setattr(module, "apply_scale", tracked_apply)
    collaborators = _collaborators(FreeCAD, lambda _document: events.append("validate"))
    try:
        result = module.run_scale(collaborators, document.Name, "Box", 1.0, 1.0, 1.0)
...
def test_scale_native_validation_failure_restores(monkeypatch):
    _require_native_collaboration()
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops import scale as module

    document = FreeCAD.newDocument("MCPScaleNativeRollback")
    original_apply = module.apply_scale

    def tracked_apply(admitted_document, request):
        obj = admitted_document.addObject("App::FeaturePython", "RejectedNative")
        return original_apply(admitted_document, request) if False else module.ScaleReceipt(
...
def test_scale_native_missing_document_keeps_typed_error():
    ...
    result = run_scale(collaborators, "MissingNativeDoc", "Box", 1.0, 1.0, 1.0)
    assert result["success"] is False
