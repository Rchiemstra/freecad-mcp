"""Agent-to-FreeCAD wire contract for the ``get_sketch_geometry`` MCP operation."""
...
class _RecordingFreeCADTransport:
    ...
    def request(self, path, payload, headers):
        ...
        if body_result is _DEFAULT_RESPONSE:
            from tests.sketch_geometry.test_get_sketch_geometry_response import _success
...
def _invoke(monkeypatch, transport, arguments):
    ...
    mcp = InstrumentedFastMCP("get_sketch_geometry-contract")
    exports = {}
    tools_partdesign_a2._register_get_sketch_geometry(mcp, dependencies=object(), exports=exports)

    async def invoke():
        async with create_connected_server_and_client_session(mcp._mcp_server) as client:
            listing = await client.list_tools()
            tool = next(tool for tool in listing.tools if tool.name == "get_sketch_geometry")
            return await client.call_tool("get_sketch_geometry", arguments)
...
def test_get_sketch_geometry_sends_authenticated_json_rpc_to_freecad(monkeypatch):
    ...
    assert envelope["method"] == "get_sketch_geometry"
