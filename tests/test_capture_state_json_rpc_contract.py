"""Agent-to-FreeCAD wire contract for the ``capture_state`` MCP operation."""
...
class _RecordingFreeCADTransport:
    ...
    def request(self, path, payload, headers):
        ...
        if body_result is _DEFAULT_RESPONSE:
            body_result = dict({"contract_version": 1, "success": True, "ok": True, "outcome": "committed", "committed": True, "retry_safe": False, "doc": "Value"}, **{
                key: envelope["params"][key]
                for key in envelope["params"]
                if key in ['doc']
            })
            body_result = {"contract_version": 1, "success": True, "ok": True, "outcome": "committed", "committed": True, "retry_safe": False, "doc": "Value"}
        response = {
            "jsonrpc": "2.0",
            "id": request["id"],
            "result": {
...
def _invoke_registered(monkeypatch, transport, params):
    ...
    mcp = InstrumentedFastMCP("capture_state-contract")
    exports = {}
    tools_advanced_b._register_capture_state(mcp, dependencies=object(), exports=exports)

    async def invoke():
        async with create_connected_server_and_client_session(mcp._mcp_server) as client:
            listing = await client.list_tools()
            tool = next(tool for tool in listing.tools if tool.name == "capture_state")
            assert "doc_name" in tool.inputSchema.get("required", ["doc_name"])
            return await client.call_tool("capture_state", params)
...
def test_capture_state_sends_authenticated_json_rpc_to_freecad(monkeypatch):
    ...
    assert envelope["method"] == "capture_state"
...
def test_mcp_schema_rejects_non_string_doc_name_before_json_rpc(monkeypatch):
    transport = _RecordingFreeCADTransport()
    params = dict({"doc_name": "AgentDocument"})
    params["doc_name"] = 42
