"""Native qualification matrix for ``clear_expression``."""
...
def test_clear_expression_native_success_inspects_after_recompute(monkeypatch):
    ...
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops import clear_expression as subject

    document = FreeCAD.newDocument("MCPClearExpressionNativeSuccess")
    events = []
    ...
    _prepare(document)
    events.clear()
    original_apply = subject.apply_clear_expression
    original_read = subject.read_clear_expression_result
    ...
    monkeypatch.setattr(subject, "apply_clear_expression", tracked_apply)
    monkeypatch.setattr(subject, "read_clear_expression_result", tracked_read)
    try:
        result = subject.run_clear_expression(_collaborators(FreeCAD, lambda _d: events.append("validate")), document.Name, "Seed", "Length")
...
def test_clear_expression_native_validation_failure_restores_complete_state():
    _require_native_collaboration()
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops.clear_expression import run_clear_expression

    document = FreeCAD.newDocument("MCPClearExpressionNativeRollback")
    _prepare(document)
    state_before = _model_state(document)
    try:
        result = run_clear_expression(
            _collaborators(FreeCAD, lambda _d: (_ for _ in ()).throw(RuntimeError("forced validation failure"))),
            document.Name, "Seed", "Length",
        )
...
def test_clear_expression_native_apply_failure_restores(monkeypatch):
    _require_native_collaboration()
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops import clear_expression as subject

    document = FreeCAD.newDocument("MCPClearExpressionNativeApplyFailure")
    _prepare(document)
    state_before = _model_state(document)
    original = subject.apply_clear_expression
    ...
    monkeypatch.setattr(subject, "apply_clear_expression", mutates_then_raises)
    try:
        result = subject.run_clear_expression(_collaborators(FreeCAD, lambda _d: None), document.Name, "Seed", "Length")
        assert result["success"] is False
...
def test_clear_expression_native_recompute_failure_rolls_back(monkeypatch):
    _require_native_collaboration()
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops import clear_expression as subject

    document = FreeCAD.newDocument("MCPClearExpressionNativeRecomputeFailure")
    ...
    monkeypatch.setattr(subject, "apply_clear_expression", arm)
    try:
        result = subject.run_clear_expression(_collaborators(FreeCAD, lambda _d: None), document.Name, "Seed", "Length")
        assert result["success"] is False
...
def test_clear_expression_native_rollback_failure_is_uncertain_and_fences(monkeypatch):
    _require_native_collaboration()
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops import clear_expression as subject

    document = FreeCAD.newDocument("MCPClearExpressionNativeRollbackFailure")
    ...
    try:
        result = subject.run_clear_expression(collaborators, document.Name, "Seed", "Length")
        proxy.armed = False
        fenced = subject.run_clear_expression(collaborators, document.Name, "Seed", "Length")
...
def test_clear_expression_native_inspection_failure_rolls_back(monkeypatch):
    _require_native_collaboration()
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops import clear_expression as subject

    document = FreeCAD.newDocument("MCPClearExpressionNativeInspectionFailure")
    _prepare(document)
    state_before = _model_state(document)

    def reject(_document, _receipt):
        raise subject.ClearExpressionError("CREATED_OBJECT_WRONG_TYPE", "forced inspection failure")

    monkeypatch.setattr(subject, "read_clear_expression_result", reject)
    try:
        result = subject.run_clear_expression(_collaborators(FreeCAD, lambda _d: None), document.Name, "Seed", "Length")
        assert result["success"] is False
...
def test_clear_expression_native_postcondition_cannot_write():
    _require_native_collaboration()
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops.clear_expression import run_clear_expression

    document = FreeCAD.newDocument("MCPClearExpressionReadOnlyPostcondition")
    ...
    try:
        result = run_clear_expression(_collaborators(FreeCAD, validate), document.Name, "Seed", "Length")
