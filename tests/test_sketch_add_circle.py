"""Unit coverage for the typed ``sketch_add_circle`` mutation slice."""
...
from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops import sketch_add_circle as subject
...
def test_sketch_add_circle_runs_apply_recompute_inspect_validate_then_commits():
    events = []
    document = FakeDocument(events)
    _prepare(document, events)
    collab, api = collaborators(document, events)

    result = run_sketch_add_circle(collab, "Doc", "Sketch", 0.0, 0.0, 5.0, False)
...
def test_invalid_names_abort_without_recompute_or_commit(sketch_name):
    ...
    result = run_sketch_add_circle(collab, "Doc", sketch_name, 0.0, 0.0, 5.0, False)
...
def test_missing_document_fails_without_entering_the_apply_callback():
    ...
    result = run_sketch_add_circle(collab, "Missing", "Sketch", 0.0, 0.0, 5.0, False)
...
def test_sketch_add_circle_has_no_uncoordinated_source_entry_point():
    assert not hasattr(subject, "sketch_add_circle_gui")
    template = Path(__file__).parents[1] / 'src/freecad_mcp/templates/core/sketch_add_circle.py.txt'
...
def test_unknown_or_contradictory_native_evidence_cannot_release_success(native_result):
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops.sketch_add_circle_mutation import (
        _sketch_add_circle_native_result,
        _NativeSketchAddCircleMutationState,
    )

    result = _sketch_add_circle_native_result(
        native_result, _NativeSketchAddCircleMutationState(postcondition_passed=True)
    )
    assert result["success"] is False
