...
def test_bounding_box_native_success_inspects_after_recompute(monkeypatch):
    ...
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops import bounding_box as module

    document = FreeCAD.newDocument("MCPBoundingBoxNative")
    events: list[str] = []
    ...
    monkeypatch.setattr(module, "apply_bounding_box", tracked_apply)
    collaborators = _collaborators(FreeCAD, lambda _document: events.append("validate"))
    try:
        result = module.run_bounding_box(collaborators, document.Name, "Box")
...
def test_bounding_box_native_validation_failure_restores(monkeypatch):
    _require_native_collaboration()
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops import bounding_box as module

    document = FreeCAD.newDocument("MCPBoundingBoxNativeRollback")
    original_apply = module.apply_bounding_box

    def tracked_apply(admitted_document, request):
        obj = admitted_document.addObject("App::FeaturePython", "RejectedNative")
        return original_apply(admitted_document, request) if False else module.BoundingBoxReceipt(
...
def test_bounding_box_native_missing_document_keeps_typed_error():
    ...
    result = run_bounding_box(collaborators, "MissingNativeDoc", "Box")
    assert result["success"] is False
