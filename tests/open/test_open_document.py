"""Unit coverage for the typed ``open_document`` mutation slice."""
...
from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops.open_document_mutation import (
    _open_document_native_result,
...
def test_open_document_runs_apply_recompute_validate_then_commits():
    events: list[str] = []
    collab, _api = collaborators(None, events)

    result = run_open_document(collab, "C:/models/part.FCStd")

    assert result["success"] is True
    assert result['path'] == 'C:/models/part.FCStd'
...
def test_missing_document_fails_without_entering_apply():
    events: list[str] = []
    collab, _api = collaborators(None, events)
    if "open_document" in {"create_document", "open_document"}:
...
def test_native_capability_is_required_before_apply():
    ...
    if "open_document" not in {"create_document", "open_document"}:
...
def test_unknown_or_contradictory_native_evidence_cannot_release_success(native_result):
    result = _open_document_native_result(native_result, _NativeMutationState(postcondition_passed=True))
    assert result["success"] is False
    assert result["outcome"] == "uncertain"
    assert result["retry_safe"] is False


def test_open_document_has_apply_entry_point():
    assert callable(apply_open_document)
