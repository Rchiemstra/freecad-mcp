...
MUTATION = "addon/FreeCADMCP/rpc_server/methods/cad_methods_ops/open_document_mutation.py"
...
def test_open_document_architecture_gate_accepts_the_production_path() -> None:
...
@pytest.mark.parametrize(
    ...
    [
        (
            ...
            "            self.inspect,",
            "            self.apply,",
            "OPEN_DOCUMENT004 missing typed postcondition=inspect",
        ),
        (
            PUBLIC_ADAPTER,
            "result = parse_open_document_response(raw_result)",
            "result = raw_result",
            "OPEN_DOCUMENT008 public adapter bypasses response validation",
        ),
        (
            MUTATION,
            ...
            "    if committed is not False or status not in _REJECTED_STATUSES:",
            "OPEN_DOCUMENT007 cached success escaped native rejection",
        ),
        (
            MUTATION,
            "if state.postcondition_passed and state.failure is None:",
            "if True:",
            "OPEN_DOCUMENT015 commit escaped without a successful postcondition",
        ),
    ],
)
def test_gate_rejects_each_known_bad_variant(relative, old, broken, expected) -> None:
    ...
    assert expected in scan_open_document_architecture(ROOT, source_overrides={relative: source.replace(old, broken)})
...
def test_gate_rejects_recompute_ownership_in_the_leaf() -> None:
    source = _read(LEAF)
    marker = '    """Record the opened document identity without recomputing."""'
    assert source.count(marker) == 1
    mutated = source.replace(marker, marker + "\n\n    doc.recompute()")
    assert "OPEN_DOCUMENT001 leaf owns forbidden execution: recompute" in scan_open_document_architecture(
...
def test_gate_rejects_any_on_the_typed_surface() -> None:
    source = _read(LEAF)
    old = 'def build_open_document_request(path: object)'
    broken = old.replace(": object", ": Any", 1)
    assert source.count(old) == 1
    assert "OPEN_DOCUMENT014 typed surface contains Any: leaf" in scan_open_document_architecture(
