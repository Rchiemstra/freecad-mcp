"""Agent-to-FreeCAD wire contract for the ``open_document`` MCP operation."""
...
class _RecordingFreeCADTransport:
    ...
    def request(self, path, payload, headers):
        ...
        if body_result is _DEFAULT_RESPONSE:
            ...
            extra = {"document_name": "part", "path": envelope["params"]["path"]}
...
def _invoke_registered(monkeypatch, transport, arguments):
    ...
    mcp = InstrumentedFastMCP("open_document-contract")
    exports = {}
    tools_gui_document_a._register_open_document(mcp, dependencies=object(), exports=exports)

    async def invoke():
        async with create_connected_server_and_client_session(mcp._mcp_server) as client:
            listing = await client.list_tools()
            tool = next(tool for tool in listing.tools if tool.name == "open_document")
            assert set(tool.inputSchema["required"]) == {'path'}
            return await client.call_tool("open_document", arguments)
...
def test_open_document_sends_exact_authenticated_json_rpc_values_to_freecad(monkeypatch):
    transport = _RecordingFreeCADTransport()
    result = _invoke_registered(monkeypatch, transport, {'path': 'C:/models/part.FCStd'})
    ...
    assert envelope["method"] == "open_document"
    assert envelope["params"] == {'path': 'C:/models/part.FCStd'}
    assert envelope["operation"] == {"name": 'Open document'}
...
def test_public_route_never_turns_rejection_into_success(monkeypatch):
    arguments = dict({'path': 'C:/models/part.FCStd'})
...
def test_public_route_marks_committed_but_invalid_response_non_retryable(monkeypatch):
    ...
    result = _invoke_registered(monkeypatch, transport, {'path': 'C:/models/part.FCStd'})
    assert result.isError is True
    assert result.structuredContent["error_code"] == 'OPEN_DOCUMENT_COMMITTED_RESPONSE_INVALID'
