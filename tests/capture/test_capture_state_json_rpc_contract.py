    ...
    def request(self, path, payload, headers):
        ...
        if body_result is _DEFAULT_RESPONSE:
            body_result = {
                ...
                "outcome": "observed",
                "retry_safe": False,
                "doc": envelope["params"]["doc_name"],
                "objects": {"Seed": {"name": "Seed"}},
            }
        response = {
            "jsonrpc": "2.0",
            "id": request["id"],
...
def _invoke_registered(monkeypatch, transport, arguments):
    ...
    async def invoke():
        async with create_connected_server_and_client_session(mcp._mcp_server) as client:
            ...
            return await client.call_tool("capture_state", arguments)
...
def test_capture_state_sends_exact_authenticated_json_rpc_values_to_freecad(monkeypatch):
    ...
    assert envelope["params"] == {"doc_name": "AgentDocument", "object_names": None}
...
def test_public_route_marks_committed_but_invalid_response_non_retryable(monkeypatch):
    ...
    result = _invoke_registered(monkeypatch, transport, {"doc_name": "AgentDocument"})
    assert result.isError is True
    assert result.structuredContent["error_code"] == "CAPTURE_STATE_COMMITTED_RESPONSE_INVALID"
...
def test_mcp_schema_rejects_non_string_doc_name_before_json_rpc(monkeypatch):
    transport = _RecordingFreeCADTransport()
    result = _invoke_registered(monkeypatch, transport, {"doc_name": 42})
    assert result.isError is True
