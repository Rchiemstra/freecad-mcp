...
def test_reload_document_architecture_gate_rejects_the_production_path_for_policy_reasons() -> None:
    violations = scan_reload_document_architecture(ROOT)
    assert violations != []
    assert any(item.startswith("LIFE reload_document") for item in violations)
