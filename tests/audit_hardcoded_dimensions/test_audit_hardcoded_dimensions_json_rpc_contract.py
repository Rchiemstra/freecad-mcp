"""Agent-to-FreeCAD wire contract for the ``audit_hardcoded_dimensions`` MCP operation."""
...
class _RecordingFreeCADTransport:
    ...
    def request(self, path, payload, headers):
        ...
        if body_result is _DEFAULT_RESPONSE:
            from tests.audit_hardcoded_dimensions.test_audit_hardcoded_dimensions_response import _success
...
def _invoke(monkeypatch, transport, arguments):
    ...
    mcp = InstrumentedFastMCP("audit_hardcoded_dimensions-contract")
    exports = {}
    tools_advanced_a._register_audit_hardcoded_dimensions(mcp, dependencies=object(), exports=exports)

    async def invoke():
        async with create_connected_server_and_client_session(mcp._mcp_server) as client:
            listing = await client.list_tools()
            tool = next(tool for tool in listing.tools if tool.name == "audit_hardcoded_dimensions")
            return await client.call_tool("audit_hardcoded_dimensions", arguments)
...
def test_audit_hardcoded_dimensions_sends_authenticated_json_rpc_to_freecad(monkeypatch):
    ...
    assert envelope["method"] == "audit_hardcoded_dimensions"
    assert envelope["params"]["doc_name"] == "AgentDocument"
    assert envelope["params"]["body_name"] == "Body"
