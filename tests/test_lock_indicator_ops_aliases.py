...
@pytest.mark.unit
def test_freecad_and_package_lock_indicator_share_registry(monkeypatch) -> None:
    ...
    assert sys.modules["lock_indicator"] is sys.modules[
        "addon.FreeCADMCP.lock_indicator"
    ]


@pytest.mark.unit
def test_direct_lock_indicator_state_modules_share_widget_state(monkeypatch) -> None:
    monkeypatch.syspath_prepend(str(_ADDON_ROOT))
    package_state = importlib.import_module(
        "addon.FreeCADMCP.lock_indicator_ops.state"
    )
    flat_state = importlib.import_module("lock_indicator_ops.state")

    assert package_state is flat_state
    assert package_state._deterred_actions is flat_state._deterred_actions


@pytest.mark.unit
@pytest.mark.parametrize(
    "facade_name",
    ("addon.FreeCADMCP.lock_indicator", "lock_indicator"),
)
def test_each_lock_indicator_spelling_retains_monkeypatch_surface(
    facade_name: str,
    monkeypatch,
) -> None:
    monkeypatch.syspath_prepend(str(_ADDON_ROOT))
    importlib.import_module("addon.FreeCADMCP.lock_indicator")
    importlib.import_module("lock_indicator")
    facade = importlib.import_module(facade_name)
    bindings = importlib.import_module(
        "addon.FreeCADMCP.lock_indicator_ops.facade_bindings"
    )
    sentinel = object()

    monkeypatch.setattr(facade, "_refresh_bridge", sentinel, raising=False)

    assert bindings.facade_attr("_refresh_bridge") is sentinel


@pytest.mark.unit
@pytest.mark.parametrize("submodule", _LOCK_INDICATOR_OPS_SUBMODULES)
def test_lock_indicator_ops_submodule_flat_package_identity(
