...
def test_recompute_document_native_commit_succeeds():
    _require_native_collaboration()

    import FreeCAD

    from addon.FreeCADMCP.collaboration_api import CollaborationAPI
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops.recompute_document import run_recompute_document

    document = FreeCAD.newDocument("MCPTypedRecomputeDocument")
    events: list[str] = []
    saved_path = ""
    try:
        
        if "recompute_document" == "reload_document":
            saved_path = document.Name + ".FCStd"
            document.saveAs(saved_path)
        if "recompute_document" == "open_document":
            saved_path = document.Name + ".FCStd"
            document.saveAs(saved_path)
        ...
        result = run_recompute_document(collab, document.Name)
