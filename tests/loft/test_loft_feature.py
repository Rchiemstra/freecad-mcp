"""Unit coverage for the typed ``loft_feature`` mutation slice."""
...
from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops import loft_feature as subject
from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops.loft_feature import run_loft_feature
from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops.loft_feature_mutation import (
    _LoftFeatureNativeMutationState,
    _loft_feature_native_result,
)
...
def _run(collab, **overrides):
    ...
    return run_loft_feature(collab, **payload)


def test_loft_feature_runs_apply_recompute_inspect_validate_then_commits():
    events: list[str] = []
    document = FeatureDocument(events)
    prepare_document('loft', document)
...
def test_creation_that_changes_then_raises_is_rolled_back():
    ...
    assert result["success"] is False
    assert result["error_code"] == "LOFT_FEATURE_FAILED"
...
def test_apply_and_inspect_use_the_native_admitted_document():
    ...
    prepare_document('loft', admitted)
...
def test_unknown_or_contradictory_native_evidence_cannot_release_success(native_result):
    result = _loft_feature_native_result(
        native_result, _LoftFeatureNativeMutationState(postcondition_passed=True)
    )
    assert result["success"] is False
...
def test_typed_rpc_handler_is_registered():
    ...
    assert "loft_feature" in discover_typed_rpc_handlers()
    assert subject.TYPED_RPC_HANDLER[0] == "loft_feature"
