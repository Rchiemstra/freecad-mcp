"""Agent-to-FreeCAD wire contract for the ``activate_document`` MCP operation."""
...
class _RecordingFreeCADTransport:
    ...
    def request(self, path, payload, headers):
        ...
        if body_result is _DEFAULT_RESPONSE:
            ...
            extra = {"document_name": envelope["params"]["doc_name"], "label": envelope["params"]["doc_name"]}
...
def _invoke_registered(monkeypatch, transport, arguments):
    ...
    mcp = InstrumentedFastMCP("activate_document-contract")
    exports = {}
    tools_gui_document_b._register_activate_document(mcp, dependencies=object(), exports=exports)

    async def invoke():
        async with create_connected_server_and_client_session(mcp._mcp_server) as client:
            listing = await client.list_tools()
            tool = next(tool for tool in listing.tools if tool.name == "activate_document")
            assert set(tool.inputSchema["required"]) == {'doc_name'}
            return await client.call_tool("activate_document", arguments)
...
def test_activate_document_sends_exact_authenticated_json_rpc_values_to_freecad(monkeypatch):
    ...
    assert envelope["method"] == "activate_document"
    assert envelope["params"] == {'doc_name': 'AgentDocument'}
    assert envelope["operation"] == {"name": 'Activate document'}
...
def test_public_route_marks_committed_but_invalid_response_non_retryable(monkeypatch):
    ...
    result = _invoke_registered(monkeypatch, transport, {'doc_name': 'AgentDocument'})
    assert result.isError is True
    assert result.structuredContent["error_code"] == 'ACTIVATE_DOCUMENT_COMMITTED_RESPONSE_INVALID'
