...
def test_activate_document_native_commit_succeeds():
    ...
    document = FreeCAD.newDocument("MCPTypedActivateDocument")
    events: list[str] = []
    saved_path = ""
    try:
        
        if "activate_document" == "reload_document":
            saved_path = document.Name + ".FCStd"
            document.saveAs(saved_path)
        if "activate_document" == "open_document":
            saved_path = document.Name + ".FCStd"
            document.saveAs(saved_path)
        ...
        result = run_activate_document(collab, document.Name)
