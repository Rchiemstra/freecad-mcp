...
"""Native collaboration coverage for the typed ``sketch_create`` mutation."""
...
def _revision_state(document):
    ...
    session = document.beginEditSession("sketch_create-native-state-probe")
    try:
...
def test_sketch_create_native_success_inspects_after_recompute(monkeypatch):
    _require_native_collaboration()
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops import sketch_create as body_create_module

    document = FreeCAD.newDocument("MCPSketchCreateNativePhaseOrder")
    events = []
    ...
    _idle(document, 'empty')
    events.clear()
    original_apply = body_create_module.apply_sketch_create
    original_read = body_create_module.read_sketch_create_result
    ...
    monkeypatch.setattr(body_create_module, "apply_sketch_create", tracked_apply)
    monkeypatch.setattr(body_create_module, "read_sketch_create_result", tracked_read)
    collaborators = _collaborators(FreeCAD, lambda _document: events.append("validate"))
    try:
        result = body_create_module.run_sketch_create(collaborators, document.Name, "NativeSketch")
        assert result["success"] is True
        assert result["committed"] is True
        assert result["retry_safe"] is False
        assert result['sketch'] == 'NativeSketch'
...
def test_sketch_create_native_validation_failure_restores_complete_state():
    ...
    document = FreeCAD.newDocument("MCPSketchCreateNativeRollback")
    _idle(document, 'empty')
    ...
    try:
        result = run_sketch_create(collaborators, document.Name, "NativeSketch")
...
def test_sketch_create_native_apply_failure_restores_all_attempted_effects(monkeypatch):
    _require_native_collaboration()
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops import sketch_create as body_create_module

    document = FreeCAD.newDocument("MCPSketchCreateNativeApplyFailure")
    _idle(document, 'empty')
    state_before = _model_state(document)
    revision_before = _revision_state(document)
    original_apply = body_create_module.apply_sketch_create
    ...
    monkeypatch.setattr(body_create_module, "apply_sketch_create", mutates_then_raises)
...
def test_sketch_create_native_recompute_failure_rolls_back(monkeypatch):
    _require_native_collaboration()
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops import sketch_create as body_create_module

    document = FreeCAD.newDocument("MCPSketchCreateNativeRecomputeFailure")
    ...
    monkeypatch.setattr(body_create_module, "apply_sketch_create", arm_recompute_failure)
    collaborators = _collaborators(FreeCAD, lambda _document: None)
    try:
        result = body_create_module.run_sketch_create(collaborators, document.Name, "NativeSketch")
        assert result["success"] is False
...
def test_sketch_create_native_rollback_failure_is_uncertain_and_fences_document(monkeypatch):
    _require_native_collaboration()
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops import sketch_create as body_create_module

    document = FreeCAD.newDocument("MCPSketchCreateNativeRollbackFailure")
    ...
    monkeypatch.setattr(body_create_module, "apply_sketch_create", arm_persistent_failure)
    collaborators = _collaborators(FreeCAD, lambda _document: None)
    try:
        result = body_create_module.run_sketch_create(collaborators, document.Name, "NativeSketch")
        proxy.armed = False
        fenced = body_create_module.run_sketch_create(collaborators, document.Name, "NativeSketch")
        assert result["success"] is False
...
def test_sketch_create_native_inspection_failure_rolls_back_after_recompute(monkeypatch):
    _require_native_collaboration()
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops import sketch_create as body_create_module

    document = FreeCAD.newDocument("MCPSketchCreateNativeInspectionFailure")
    _idle(document, 'empty')
    state_before = _model_state(document)
    revision_before = _revision_state(document)

    def reject_inspection(*_args, **_kwargs):
        raise body_create_module.SketchCreateError(
            "CREATED_OBJECT_WRONG_TYPE", "forced post-recompute inspection failure"
        )

    monkeypatch.setattr(body_create_module, "read_sketch_create_result", reject_inspection)
    collaborators = _collaborators(FreeCAD, lambda _document: None)
    try:
        result = body_create_module.run_sketch_create(collaborators, document.Name, "NativeSketch")
        assert result["success"] is False
...
def test_sketch_create_native_failure_isolated_and_healthy_rollback_recovers():
    _require_native_collaboration()
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops.sketch_create import run_sketch_create

    failed_document = FreeCAD.newDocument("MCPSketchCreateIsolatedFailure")
    healthy_document = FreeCAD.newDocument("MCPSketchCreateHealthyDocument")
    _idle(failed_document, 'empty')
    _idle(healthy_document, 'empty')
    ...
    try:
        failed = run_sketch_create(collaborators, failed_document.Name, "NativeSketch")
        isolated = run_sketch_create(collaborators, healthy_document.Name, "NativeSketch")
        rejected_names.clear()
        recovered = run_sketch_create(collaborators, failed_document.Name, "NativeSketch")
...
def test_sketch_create_native_duplicate_and_missing_document_keep_typed_errors():
    _require_native_collaboration()
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops.sketch_create import run_sketch_create

    document = FreeCAD.newDocument("MCPSketchCreateTypedErrors")
    _idle(document, 'empty')
    collaborators = _collaborators(FreeCAD, lambda _: None)
    try:
        first = run_sketch_create(collaborators, document.Name, "NativeSketch")
        ...
        missing = run_sketch_create(collaborators, "NoSuchSketchCreateQualificationDocument", "NativeSketch")
...
def test_sketch_create_native_rollback_restores_rich_model(monkeypatch, stage):
    _require_native_collaboration()
    import FreeCAD
    import Part
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops import sketch_create as subject

    document = FreeCAD.newDocument("MCPSketchCreateRichRollback")
    ...
    _idle(document, 'empty')
    before = _model_state(document), _revision_state(document)
    original_apply = subject.apply_sketch_create
    original_inspect = subject.read_sketch_create_result
    ...
    def inspect(*args, **kwargs):
        if stage == "inspection":
            raise subject.SketchCreateError("INSPECTION_FAILED", "injected inspection failure")
    ...
    monkeypatch.setattr(subject, "apply_sketch_create", change_model)
    monkeypatch.setattr(subject, "read_sketch_create_result", inspect)
    try:
        result = subject.run_sketch_create(_collaborators(FreeCAD, validate), document.Name, "NativeSketch")
...
def test_sketch_create_native_postcondition_cannot_write(monkeypatch, write):
    _require_native_collaboration()
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops import sketch_create as subject

    document = FreeCAD.newDocument("MCPSketchCreateReadOnlyPostcondition")
