"""Sensitivity checks for the sketch_symmetry static and architecture gate."""
...
LEAF = 'addon/FreeCADMCP/rpc_server/methods/cad_methods_ops/sketch_symmetry.py'
MUTATION = 'addon/FreeCADMCP/rpc_server/methods/cad_methods_ops/sketch_symmetry_mutation.py'
BRIDGE = "addon/FreeCADMCP/collaboration_api.py"
PUBLIC_ADAPTER = 'src/freecad_mcp/operations/parametric_ops/sketch_symmetry.py'
...
def test_sketch_symmetry_architecture_gate_accepts_the_production_path() -> None:
    assert scan_sketch_op_architecture(ROOT, "sketch_symmetry") == []
...
@pytest.mark.parametrize(
    ...
    [
        (
            ...
            "            self.inspect,",
            "            self.apply,",
            "SKETCH_SYMMETRY004 missing typed postcondition=inspect",
        ),
        (
            BRIDGE,
            "            return _unsupported(\n"
            ...
            "SKETCH_SYMMETRY006 postcondition path can reach non-native callback fallback",
        ),
        (
            PUBLIC_ADAPTER,
            "result = parse_sketch_symmetry_response(raw_result)",
            "result = raw_result",
            "SKETCH_SYMMETRY008 public adapter bypasses response validation",
        ),
        (
            MUTATION,
            ...
            "    if committed is not False or status not in _REJECTED_STATUSES:",
            "SKETCH_SYMMETRY007 cached success escaped native rejection",
        ),
        (
            MUTATION,
            "if state.postcondition_passed and state.failure is None:",
            "if True:",
            "SKETCH_SYMMETRY015 commit escaped without a successful postcondition",
        ),
    ],
)
...
def test_gate_rejects_transaction_or_recompute_ownership_in_the_leaf() -> None:
    ...
    assert "SKETCH_SYMMETRY001 leaf owns forbidden execution: recompute" in (
        scan_sketch_op_architecture(ROOT, "sketch_symmetry", source_overrides={LEAF: mutated})
    )
