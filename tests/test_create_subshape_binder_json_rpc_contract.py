"""Agent-to-FreeCAD wire contract for the ``create_subshape_binder`` MCP operation."""
...
def _invoke_registered(monkeypatch, transport, params):
    ...
    mcp = InstrumentedFastMCP("create_subshape_binder-contract")
    exports = {}
    tools_partdesign_a._register_create_subshape_binder(mcp, dependencies=object(), exports=exports)

    async def invoke():
        async with create_connected_server_and_client_session(mcp._mcp_server) as client:
            listing = await client.list_tools()
            tool = next(tool for tool in listing.tools if tool.name == "create_subshape_binder")
            assert "doc_name" in tool.inputSchema.get("required", ["doc_name"])
            return await client.call_tool("create_subshape_binder", params)
...
def test_create_subshape_binder_sends_authenticated_json_rpc_to_freecad(monkeypatch):
    transport = _RecordingFreeCADTransport()
    result = _invoke_registered(monkeypatch, transport, {"doc_name": "AgentDocument", "binder_name": "Value", "source_object": "Value", "relative": True, "sync_placement": True, "if_exists": "error"})
...
def test_mcp_schema_rejects_non_string_doc_name_before_json_rpc(monkeypatch):
    transport = _RecordingFreeCADTransport()
    params = dict({"doc_name": "AgentDocument", "binder_name": "Value", "source_object": "Value", "relative": True, "sync_placement": True, "if_exists": "error"})
    params["doc_name"] = 42
