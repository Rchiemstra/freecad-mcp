"""Native qualification matrix for ``create_placement_binder``."""
...
def test_create_placement_binder_native_success_inspects_after_recompute(monkeypatch):
    ...
    document = FreeCAD.newDocument("MCPCreatePlacementBinderNativeSuccess")
    events = []
    ...
    _prepare(document)
    events.clear()
    original_apply = subject.apply_create_placement_binder
    original_read = subject.read_create_placement_binder_result
...
def test_create_placement_binder_native_validation_failure_restores_complete_state():
    _require_native_collaboration()
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops.create_placement_binder import run_create_placement_binder

    document = FreeCAD.newDocument("MCPCreatePlacementBinderNativeRollback")
    _prepare(document)
...
def test_create_placement_binder_native_apply_failure_restores(monkeypatch):
    _require_native_collaboration()
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops import create_placement_binder as subject

    document = FreeCAD.newDocument("MCPCreatePlacementBinderNativeApplyFailure")
    _prepare(document)
    state_before = _model_state(document)
    original = subject.apply_create_placement_binder
...
def test_create_placement_binder_native_recompute_failure_rolls_back(monkeypatch):
    _require_native_collaboration()
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops import create_placement_binder as subject

    document = FreeCAD.newDocument("MCPCreatePlacementBinderNativeRecomputeFailure")
...
def test_create_placement_binder_native_rollback_failure_is_uncertain_and_fences(monkeypatch):
    _require_native_collaboration()
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops import create_placement_binder as subject

    document = FreeCAD.newDocument("MCPCreatePlacementBinderNativeRollbackFailure")
...
def test_create_placement_binder_native_inspection_failure_rolls_back(monkeypatch):
    _require_native_collaboration()
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops import create_placement_binder as subject

    document = FreeCAD.newDocument("MCPCreatePlacementBinderNativeInspectionFailure")
    _prepare(document)
    state_before = _model_state(document)

    def reject(_document, _receipt):
        raise subject.CreatePlacementBinderError("CREATED_OBJECT_WRONG_TYPE", "forced inspection failure")
...
def test_create_placement_binder_native_postcondition_cannot_write():
    _require_native_collaboration()
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops.create_placement_binder import run_create_placement_binder

    document = FreeCAD.newDocument("MCPCreatePlacementBinderReadOnlyPostcondition")
