"""Unit coverage for the typed ``polar_pattern_feature`` mutation slice."""
...
from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops import polar_pattern_feature as subject
from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops.polar_pattern_feature import run_polar_pattern_feature
from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops.polar_pattern_feature_mutation import (
    _PolarPatternFeatureNativeMutationState,
    _polar_pattern_feature_native_result,
)
...
def _run(collab, **overrides):
    ...
    return run_polar_pattern_feature(collab, **payload)


def test_polar_pattern_feature_runs_apply_recompute_inspect_validate_then_commits():
    events: list[str] = []
...
def test_creation_that_changes_then_raises_is_rolled_back():
    ...
    assert result["success"] is False
    assert result["error_code"] == "POLAR_PATTERN_FEATURE_FAILED"
...
def test_apply_and_inspect_use_the_native_admitted_document():
    ...
    prepare_document('pattern', admitted)
...
def test_unknown_or_contradictory_native_evidence_cannot_release_success(native_result):
    result = _polar_pattern_feature_native_result(
        native_result, _PolarPatternFeatureNativeMutationState(postcondition_passed=True)
    )
    assert result["success"] is False
...
def test_typed_rpc_handler_is_registered():
    ...
    assert "polar_pattern_feature" in discover_typed_rpc_handlers()
    assert subject.TYPED_RPC_HANDLER[0] == "polar_pattern_feature"
