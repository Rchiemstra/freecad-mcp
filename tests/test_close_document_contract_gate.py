...
MUTATION = "addon/FreeCADMCP/rpc_server/methods/cad_methods_ops/close_document_mutation.py"
...
def test_close_document_architecture_gate_accepts_the_production_path() -> None:
    assert scan_close_document_architecture(ROOT) == []
...
@pytest.mark.parametrize(
    ...
    [
        (
            ...
            "            self.inspect,",
            "            self.apply,",
            "CLOSE_DOCUMENT004 missing typed postcondition=inspect",
        ),
        (
            PUBLIC_ADAPTER,
            "result = parse_close_document_response(raw_result)",
            "result = raw_result",
            "CLOSE_DOCUMENT008 public adapter bypasses response validation",
        ),
        (
            MUTATION,
            ...
            "    if committed is not False or status not in _REJECTED_STATUSES:",
            "CLOSE_DOCUMENT007 cached success escaped native rejection",
        ),
        (
            MUTATION,
            "if state.postcondition_passed and state.failure is None:",
            "if True:",
            "CLOSE_DOCUMENT015 commit escaped without a successful postcondition",
        ),
    ],
)
def test_gate_rejects_each_known_bad_variant(relative, old, broken, expected) -> None:
    ...
    assert expected in scan_close_document_architecture(ROOT, source_overrides={relative: source.replace(old, broken)})
...
def test_gate_rejects_recompute_ownership_in_the_leaf() -> None:
    source = _read(LEAF)
    marker = '    """Record the document identity; closing happens after native commit."""'
    assert source.count(marker) == 1
    mutated = source.replace(marker, marker + "\n\n    doc.recompute()")
    assert "CLOSE_DOCUMENT001 leaf owns forbidden execution: recompute" in scan_close_document_architecture(
...
def test_gate_rejects_any_on_the_typed_surface() -> None:
    ...
    assert source.count(old) == 1
    assert "CLOSE_DOCUMENT014 typed surface contains Any: leaf" in scan_close_document_architecture(
