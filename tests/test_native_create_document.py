...
def test_create_document_native_commit_succeeds():
    ...
    document = FreeCAD.newDocument("MCPTypedCreateDocument")
    events: list[str] = []
    saved_path = ""
    try:
        
        if "create_document" == "reload_document":
            saved_path = document.Name + ".FCStd"
            document.saveAs(saved_path)
        if "create_document" == "open_document":
            saved_path = document.Name + ".FCStd"
            document.saveAs(saved_path)
        ...
        result = run_create_document(collab, document.Name + "Created")
