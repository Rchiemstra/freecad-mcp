"""Unit coverage for the typed ``undo`` mutation slice."""
...
from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops.undo_mutation import (
    _undo_native_result,
...
def test_undo_runs_apply_recompute_validate_then_commits():
    events: list[str] = []
...
def test_missing_document_fails_without_entering_apply():
    ...
    if "undo" in {"create_document", "open_document"}:
...
def test_native_capability_is_required_before_apply():
    ...
    if "undo" not in {"create_document", "open_document"}:
...
def test_unknown_or_contradictory_native_evidence_cannot_release_success(native_result):
    result = _undo_native_result(native_result, _NativeMutationState(postcondition_passed=True))
    assert result["success"] is False
