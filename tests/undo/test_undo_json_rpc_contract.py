"""Agent-to-FreeCAD wire contract for the ``undo`` MCP operation."""
...
def _invoke_registered(monkeypatch, transport, arguments):
    ...
    mcp = InstrumentedFastMCP("undo-contract")
    exports = {}
    tools_document_history._register_undo(mcp, dependencies=object(), exports=exports)

    async def invoke():
        async with create_connected_server_and_client_session(mcp._mcp_server) as client:
            listing = await client.list_tools()
            tool = next(tool for tool in listing.tools if tool.name == "undo")
            assert set(tool.inputSchema["required"]) == {'doc_name'}
            return await client.call_tool("undo", arguments)
...
def test_undo_sends_exact_authenticated_json_rpc_values_to_freecad(monkeypatch):
...
def test_public_route_marks_committed_but_invalid_response_non_retryable(monkeypatch):
    ...
    result = _invoke_registered(monkeypatch, transport, {'doc_name': 'AgentDocument'})
    assert result.isError is True
    assert result.structuredContent["error_code"] == 'UNDO_COMMITTED_RESPONSE_INVALID'
