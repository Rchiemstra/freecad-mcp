"""Agent-to-FreeCAD wire contract for the ``linear_pattern_feature`` MCP operation."""
...
_TOOL_PARAMS = {'doc_name': 'AgentDocument', 'feature_name': 'Pad', 'pattern_name': 'Array', 'length': 40.0, 'occurrences': 5}
_REQUIRED = {'length', 'doc_name', 'occurrences', 'feature_name', 'pattern_name'}
...
def _invoke_registered(monkeypatch, transport, tool_params):
    ...
    mcp = InstrumentedFastMCP("linear_pattern_feature-contract")
    exports = {}
    tools_features_basic_1._register_linear_pattern_feature(
        mcp,
        dependencies=object(),
        exports=exports,
    )
    ...
    async def invoke():
        async with create_connected_server_and_client_session(mcp._mcp_server) as client:
            ...
            tool = next(tool for tool in listing.tools if tool.name == "linear_pattern_feature")
            assert set(tool.inputSchema.get("required", [])) == _REQUIRED
            return await client.call_tool("linear_pattern_feature", tool_params)
...
def test_linear_pattern_feature_sends_exact_authenticated_json_rpc_values_to_freecad(monkeypatch):
    ...
    assert envelope["method"] == "linear_pattern_feature"
    assert envelope["session_token"] == "test-session-token"
    assert envelope["operation"] == {"name": 'Linear pattern feature'}
...
def test_public_route_marks_committed_but_invalid_response_non_retryable(monkeypatch):
    ...
    result = _invoke_registered(monkeypatch, transport, dict(_TOOL_PARAMS))
    assert result.isError is True
    assert result.structuredContent["error_code"] == "LINEAR_PATTERN_FEATURE_COMMITTED_RESPONSE_INVALID"
