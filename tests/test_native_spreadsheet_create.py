"""Native qualification matrix for ``spreadsheet_create``."""
...
def test_spreadsheet_create_native_success_inspects_after_recompute(monkeypatch):
    _require_native_collaboration()
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops import spreadsheet_create as subject

    document = FreeCAD.newDocument("MCPSpreadsheetCreateNativeSuccess")
    events = []
    ...
    _prepare(document)
    events.clear()
    original_apply = subject.apply_spreadsheet_create
    original_read = subject.read_spreadsheet_create_result
    ...
    monkeypatch.setattr(subject, "apply_spreadsheet_create", tracked_apply)
    monkeypatch.setattr(subject, "read_spreadsheet_create_result", tracked_read)
    try:
        result = subject.run_spreadsheet_create(_collaborators(FreeCAD, lambda _d: events.append("validate")), document.Name, "Created")
...
def test_spreadsheet_create_native_validation_failure_restores_complete_state():
    _require_native_collaboration()
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops.spreadsheet_create import run_spreadsheet_create

    document = FreeCAD.newDocument("MCPSpreadsheetCreateNativeRollback")
    _prepare(document)
    state_before = _model_state(document)
    try:
        result = run_spreadsheet_create(
...
def test_spreadsheet_create_native_apply_failure_restores(monkeypatch):
    _require_native_collaboration()
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops import spreadsheet_create as subject

    document = FreeCAD.newDocument("MCPSpreadsheetCreateNativeApplyFailure")
    _prepare(document)
    state_before = _model_state(document)
    original = subject.apply_spreadsheet_create
    ...
    monkeypatch.setattr(subject, "apply_spreadsheet_create", mutates_then_raises)
    try:
        result = subject.run_spreadsheet_create(_collaborators(FreeCAD, lambda _d: None), document.Name, "Created")
        assert result["success"] is False
...
def test_spreadsheet_create_native_recompute_failure_rolls_back(monkeypatch):
    _require_native_collaboration()
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops import spreadsheet_create as subject

    document = FreeCAD.newDocument("MCPSpreadsheetCreateNativeRecomputeFailure")
    ...
    monkeypatch.setattr(subject, "apply_spreadsheet_create", arm)
    try:
        result = subject.run_spreadsheet_create(_collaborators(FreeCAD, lambda _d: None), document.Name, "Created")
        assert result["success"] is False
...
def test_spreadsheet_create_native_rollback_failure_is_uncertain_and_fences(monkeypatch):
    _require_native_collaboration()
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops import spreadsheet_create as subject

    document = FreeCAD.newDocument("MCPSpreadsheetCreateNativeRollbackFailure")
    ...
    try:
        result = subject.run_spreadsheet_create(collaborators, document.Name, "Created")
        proxy.armed = False
        fenced = subject.run_spreadsheet_create(collaborators, document.Name, "Created")
...
def test_spreadsheet_create_native_inspection_failure_rolls_back(monkeypatch):
    _require_native_collaboration()
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops import spreadsheet_create as subject

    document = FreeCAD.newDocument("MCPSpreadsheetCreateNativeInspectionFailure")
    _prepare(document)
    state_before = _model_state(document)

    def reject(_document, _receipt):
        raise subject.SpreadsheetCreateError("CREATED_OBJECT_WRONG_TYPE", "forced inspection failure")

    monkeypatch.setattr(subject, "read_spreadsheet_create_result", reject)
    try:
        result = subject.run_spreadsheet_create(_collaborators(FreeCAD, lambda _d: None), document.Name, "Created")
        assert result["success"] is False
...
def test_spreadsheet_create_native_postcondition_cannot_write():
    _require_native_collaboration()
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops.spreadsheet_create import run_spreadsheet_create

    document = FreeCAD.newDocument("MCPSpreadsheetCreateReadOnlyPostcondition")
    ...
    try:
        result = run_spreadsheet_create(_collaborators(FreeCAD, validate), document.Name, "Created")
