"""Native qualification matrix for ``spreadsheet_list_aliases``."""
...
def test_spreadsheet_list_aliases_native_success_inspects_after_recompute(monkeypatch):
    ...
    document = FreeCAD.newDocument("MCPSpreadsheetListAliasesNativeSuccess")
    events = []
    ...
    _prepare(document)
    events.clear()
    original_apply = subject.apply_spreadsheet_list_aliases
    original_read = subject.read_spreadsheet_list_aliases_result
    ...
    monkeypatch.setattr(subject, "apply_spreadsheet_list_aliases", tracked_apply)
    monkeypatch.setattr(subject, "read_spreadsheet_list_aliases_result", tracked_read)
    try:
        result = subject.run_spreadsheet_list_aliases(_collaborators(FreeCAD, lambda _d: events.append("validate")), document.Name, "Target")
...
def test_spreadsheet_list_aliases_native_validation_failure_restores_complete_state():
    _require_native_collaboration()
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops.spreadsheet_list_aliases import run_spreadsheet_list_aliases

    document = FreeCAD.newDocument("MCPSpreadsheetListAliasesNativeRollback")
    _prepare(document)
    state_before = _model_state(document)
    try:
        result = run_spreadsheet_list_aliases(
...
def test_spreadsheet_list_aliases_native_apply_failure_restores(monkeypatch):
    _require_native_collaboration()
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops import spreadsheet_list_aliases as subject

    document = FreeCAD.newDocument("MCPSpreadsheetListAliasesNativeApplyFailure")
    _prepare(document)
    state_before = _model_state(document)
    original = subject.apply_spreadsheet_list_aliases
    ...
    monkeypatch.setattr(subject, "apply_spreadsheet_list_aliases", mutates_then_raises)
    try:
        result = subject.run_spreadsheet_list_aliases(_collaborators(FreeCAD, lambda _d: None), document.Name, "Target")
        assert result["success"] is False
...
def test_spreadsheet_list_aliases_native_recompute_failure_rolls_back(monkeypatch):
    _require_native_collaboration()
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops import spreadsheet_list_aliases as subject

    document = FreeCAD.newDocument("MCPSpreadsheetListAliasesNativeRecomputeFailure")
    ...
    monkeypatch.setattr(subject, "apply_spreadsheet_list_aliases", arm)
    try:
        result = subject.run_spreadsheet_list_aliases(_collaborators(FreeCAD, lambda _d: None), document.Name, "Target")
        assert result["success"] is False
...
def test_spreadsheet_list_aliases_native_rollback_failure_is_uncertain_and_fences(monkeypatch):
    _require_native_collaboration()
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops import spreadsheet_list_aliases as subject

    document = FreeCAD.newDocument("MCPSpreadsheetListAliasesNativeRollbackFailure")
    ...
    try:
        result = subject.run_spreadsheet_list_aliases(collaborators, document.Name, "Target")
        proxy.armed = False
        fenced = subject.run_spreadsheet_list_aliases(collaborators, document.Name, "Target")
...
def test_spreadsheet_list_aliases_native_inspection_failure_rolls_back(monkeypatch):
    _require_native_collaboration()
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops import spreadsheet_list_aliases as subject

    document = FreeCAD.newDocument("MCPSpreadsheetListAliasesNativeInspectionFailure")
    _prepare(document)
    state_before = _model_state(document)

    def reject(_document, _receipt):
        raise subject.SpreadsheetListAliasesError("CREATED_OBJECT_WRONG_TYPE", "forced inspection failure")

    monkeypatch.setattr(subject, "read_spreadsheet_list_aliases_result", reject)
    try:
        result = subject.run_spreadsheet_list_aliases(_collaborators(FreeCAD, lambda _d: None), document.Name, "Target")
        assert result["success"] is False
...
def test_spreadsheet_list_aliases_native_postcondition_cannot_write():
    _require_native_collaboration()
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops.spreadsheet_list_aliases import run_spreadsheet_list_aliases

    document = FreeCAD.newDocument("MCPSpreadsheetListAliasesReadOnlyPostcondition")
    ...
    try:
        result = run_spreadsheet_list_aliases(_collaborators(FreeCAD, validate), document.Name, "Target")
