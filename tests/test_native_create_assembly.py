"""Native qualification matrix for typed ``create_assembly``."""
...
def test_create_assembly_native_success_inspects_after_recompute(monkeypatch):
    _require_native_collaboration()
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops import create_assembly as module

    document = FreeCAD.newDocument("MCPCreateAssemblyNative")
    events: list[str] = []
    ...
    monkeypatch.setattr(module, "apply_create_assembly", tracked_apply)
    collaborators = _collaborators(FreeCAD, lambda _document: events.append("validate"))
    try:
        result = module.run_create_assembly(collaborators, document.Name, "Assembly")
...
def test_create_assembly_native_validation_failure_restores(monkeypatch):
    _require_native_collaboration()
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops import create_assembly as module

    document = FreeCAD.newDocument("MCPCreateAssemblyNativeRollback")
    original_apply = module.apply_create_assembly

    def tracked_apply(admitted_document, request):
        obj = admitted_document.addObject("Assembly::AssemblyObject", "RejectedNative")
        return original_apply(admitted_document, request) if False else module.CreateAssemblyReceipt(
...
def test_create_assembly_native_missing_document_keeps_typed_error():
    _require_native_collaboration()
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops.create_assembly import run_create_assembly

    collaborators = _collaborators(FreeCAD, lambda _document: None)
    result = run_create_assembly(collaborators, "MissingNativeDoc", "Assembly")
    assert result["success"] is False
