"""Agent-to-FreeCAD wire contract for the ``solve_assembly`` MCP operation."""
...
from freecad_mcp._shared.protocol.solve_assembly_contract import make_solve_assembly_success
...
class _RecordingFreeCADTransport:
    ...
    def request(self, path, payload, headers):
        ...
        if body_result is _DEFAULT_RESPONSE:
            from tests.solve.test_solve_assembly_response import _success
...
def _invoke(monkeypatch, transport, arguments):
    ...
    mcp = InstrumentedFastMCP("solve_assembly-contract")
    exports = {}
    tools_assembly._register_solve_assembly(mcp, dependencies=object(), exports=exports)

    async def invoke():
        async with create_connected_server_and_client_session(mcp._mcp_server) as client:
            listing = await client.list_tools()
            tool = next(tool for tool in listing.tools if tool.name == "solve_assembly")
            return await client.call_tool("solve_assembly", arguments)
...
def test_solve_assembly_sends_authenticated_json_rpc_to_freecad(monkeypatch):
    ...
    if "solve_assembly" == "create_assembly_grounded_joint":
        arguments.update({"assembly_name": "Assembly", "component_name": "Base"})
    elif "solve_assembly" == "create_assembly_joint":
        arguments.update({
