...
def test_translate_native_success_inspects_after_recompute(monkeypatch):
    ...
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops import translate as module

    document = FreeCAD.newDocument("MCPTranslateNative")
    events: list[str] = []
    ...
    monkeypatch.setattr(module, "apply_translate", tracked_apply)
    collaborators = _collaborators(FreeCAD, lambda _document: events.append("validate"))
    try:
        result = module.run_translate(collaborators, document.Name, "Box", 1.0, 0.0, 0.0)
...
def test_translate_native_validation_failure_restores(monkeypatch):
    _require_native_collaboration()
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops import translate as module

    document = FreeCAD.newDocument("MCPTranslateNativeRollback")
    original_apply = module.apply_translate

    def tracked_apply(admitted_document, request):
        obj = admitted_document.addObject("App::FeaturePython", "RejectedNative")
        return original_apply(admitted_document, request) if False else module.TranslateReceipt(
...
def test_translate_native_missing_document_keeps_typed_error():
    ...
    result = run_translate(collaborators, "MissingNativeDoc", "Box", 1.0, 0.0, 0.0)
    assert result["success"] is False
