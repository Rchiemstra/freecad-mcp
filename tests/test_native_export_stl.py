...
def test_export_stl_native_success_inspects_after_recompute(monkeypatch):
    ...
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops import export_stl as module

    document = FreeCAD.newDocument("MCPExportStlNative")
    events: list[str] = []
    ...
    monkeypatch.setattr(module, "apply_export_stl", tracked_apply)
    collaborators = _collaborators(FreeCAD, lambda _document: events.append("validate"))
    try:
        result = module.run_export_stl(collaborators, document.Name, "/tmp/out.stl")
...
def test_export_stl_native_validation_failure_restores(monkeypatch):
    _require_native_collaboration()
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops import export_stl as module

    document = FreeCAD.newDocument("MCPExportStlNativeRollback")
    original_apply = module.apply_export_stl

    def tracked_apply(admitted_document, request):
        obj = admitted_document.addObject("App::FeaturePython", "RejectedNative")
        return original_apply(admitted_document, request) if False else module.ExportStlReceipt(
...
def test_export_stl_native_missing_document_keeps_typed_error():
    ...
    result = run_export_stl(collaborators, "MissingNativeDoc", "/tmp/out.stl")
    assert result["success"] is False
