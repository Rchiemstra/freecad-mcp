...
def test_delete_object_native_commit_succeeds():
    ...
    document = FreeCAD.newDocument("MCPTypedDeleteObject")
    events: list[str] = []
    saved_path = ""
    try:
        document.addObject("App::FeaturePython", "TypedDelete")
    document.recompute()
        if "delete_object" == "reload_document":
            saved_path = document.Name + ".FCStd"
            document.saveAs(saved_path)
        if "delete_object" == "open_document":
            saved_path = document.Name + ".FCStd"
            document.saveAs(saved_path)
        ...
        result = run_delete_object(collab, document.Name, "TypedDelete", False, False)
