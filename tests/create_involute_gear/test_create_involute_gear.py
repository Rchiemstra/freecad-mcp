"""Unit coverage for the typed ``create_involute_gear`` mutation slice."""
...
from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops import create_involute_gear as subject
from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops.create_involute_gear import run_create_involute_gear
...
def _collaborators(document, events, *, validator=None, final_result=None, monkeypatch=None):
    if monkeypatch is not None:
        monkeypatch.setattr(gear_actions, "create_involute_gear", _fake_payload)
...
def test_create_involute_gear_runs_apply_recompute_inspect_validate_then_commits(monkeypatch):
    events = []
    document = _Document(events)
    collaborators, api = _collaborators(document, events, monkeypatch=monkeypatch)
    result = run_create_involute_gear(collaborators, "Doc", "Gear", 8, 2.0, 10.0)
...
def test_invalid_names_abort_without_recompute_or_commit(monkeypatch):
    ...
    result = run_create_involute_gear(collaborators, "Doc", "", 8, 2.0, 10.0)
...
def test_create_involute_gear_has_typed_rpc_handler():
    assert subject.TYPED_RPC_HANDLER[0] == "create_involute_gear"
    assert callable(subject.TYPED_RPC_HANDLER[1])


def test_unknown_native_evidence_cannot_release_success():
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops.create_involute_gear_mutation import (
        _create_involute_gear_native_result,
        _NativeMutationState,
    )
    result = _create_involute_gear_native_result({"status": "FutureStatus", "committed": False}, _NativeMutationState(postcondition_passed=True))
    assert result["success"] is False
