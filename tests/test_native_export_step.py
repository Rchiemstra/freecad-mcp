...
def test_export_step_native_success_inspects_after_recompute(monkeypatch):
    ...
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops import export_step as module

    document = FreeCAD.newDocument("MCPExportStepNative")
    events: list[str] = []
    ...
    monkeypatch.setattr(module, "apply_export_step", tracked_apply)
    collaborators = _collaborators(FreeCAD, lambda _document: events.append("validate"))
    try:
        result = module.run_export_step(collaborators, document.Name, "/tmp/out.step")
...
def test_export_step_native_validation_failure_restores(monkeypatch):
    _require_native_collaboration()
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops import export_step as module

    document = FreeCAD.newDocument("MCPExportStepNativeRollback")
    original_apply = module.apply_export_step

    def tracked_apply(admitted_document, request):
        obj = admitted_document.addObject("App::FeaturePython", "RejectedNative")
        return original_apply(admitted_document, request) if False else module.ExportStepReceipt(
...
def test_export_step_native_missing_document_keeps_typed_error():
    ...
    result = run_export_step(collaborators, "MissingNativeDoc", "/tmp/out.step")
    assert result["success"] is False
