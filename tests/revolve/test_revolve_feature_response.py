"""Adversarial wire cases for the ``revolve_feature`` contract."""
...
def _success():
    return make_revolve_feature_success(FeatureName("Feature001"), "Main Feature")


@pytest.mark.parametrize("raw", [None, [], 1, "timeout", {}, {1: "bad key"}])
def test_unknown_response_never_proves_rejection(raw):
    result = parse_revolve_feature_response(raw)
...
def test_contract_version_must_be_the_exact_integer(version):
    assert parse_revolve_feature_response(dict(_success(), contract_version=version))["success"] is False
...
def test_contradictory_or_malformed_success_cannot_escape(change):
    result = parse_revolve_feature_response(dict(_success(), **change))
    assert result["success"] is False
    assert result["outcome"] == "uncertain"
    assert result["retry_safe"] is False
...
@pytest.mark.parametrize(
    ...
    [
        _success(),
        make_revolve_feature_failure("BUSY", "Retry after recompute", native_status="Busy"),
        make_revolve_feature_failure("INVALID_ARGUMENT", "Invalid name", retry_safe=False),
        *[
            make_revolve_feature_uncertain(
                "STATE_UNKNOWN",
...
def test_valid_variants_round_trip_without_changing_evidence(raw):
    assert parse_revolve_feature_response(raw) == raw
    assert parse_revolve_feature_response(parse_revolve_feature_response(raw)) == raw
...
def test_transport_failure_preserves_unknown_model_state():
    ...
    response = revolve_feature_operation(
        SimpleNamespace(revolve_feature=lost_response), True, "Doc", sketch_name='Sketch', revolve_name='Revolve', angle=360.0, axis='Z_Axis', body_name=None, symmetric=False, reversed_dir=False
    )
    assert response.isError is True
    data = response.structuredContent["data"]
    assert data["error_code"] == "REVOLVE_FEATURE_TRANSPORT_UNCERTAIN"
