"""Unit coverage for the typed ``revolve_feature`` mutation slice."""
...
from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops import revolve_feature as subject
from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops.revolve_feature import run_revolve_feature
from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops.revolve_feature_mutation import (
    _RevolveFeatureNativeMutationState,
    _revolve_feature_native_result,
)
...
def _run(collab, **overrides):
    ...
    return run_revolve_feature(collab, **payload)


def test_revolve_feature_runs_apply_recompute_inspect_validate_then_commits():
    events: list[str] = []
...
def test_creation_that_changes_then_raises_is_rolled_back():
    ...
    assert result["success"] is False
    assert result["error_code"] == "REVOLVE_FEATURE_FAILED"
...
def test_unknown_or_contradictory_native_evidence_cannot_release_success(native_result):
    result = _revolve_feature_native_result(
        native_result, _RevolveFeatureNativeMutationState(postcondition_passed=True)
    )
    assert result["success"] is False
...
def test_typed_rpc_handler_is_registered():
    ...
    assert "revolve_feature" in discover_typed_rpc_handlers()
    assert subject.TYPED_RPC_HANDLER[0] == "revolve_feature"
