...
def test_create_placement_datum_native_success_inspects_after_recompute(monkeypatch):
    ...
    try:
        result = subject.run_create_placement_datum(_collaborators(FreeCAD, lambda _d: events.append("validate")), document.Name, "Body", "Created", "Seed", True, None)
...
def test_create_placement_datum_native_validation_failure_restores_complete_state():
    ...
    try:
        result = run_create_placement_datum(
            ...
            document.Name, "Body", "Created", "Seed", True, None,
        )
...
def test_create_placement_datum_native_apply_failure_restores(monkeypatch):
    ...
    try:
        result = subject.run_create_placement_datum(_collaborators(FreeCAD, lambda _d: None), document.Name, "Body", "Created", "Seed", True, None)
        assert result["success"] is False
...
def test_create_placement_datum_native_rollback_failure_is_uncertain_and_fences(monkeypatch):
    ...
    try:
        result = subject.run_create_placement_datum(collaborators, document.Name, "Body", "Created", "Seed", True, None)
        proxy.armed = False
        fenced = subject.run_create_placement_datum(collaborators, document.Name, "Body", "Created", "Seed", True, None)
...
def test_create_placement_datum_native_postcondition_cannot_write():
    ...
    try:
        result = run_create_placement_datum(_collaborators(FreeCAD, validate), document.Name, "Body", "Created", "Seed", True, None)
