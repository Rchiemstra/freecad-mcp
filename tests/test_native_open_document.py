...
def test_open_document_native_commit_succeeds():
    ...
    document = FreeCAD.newDocument("MCPTypedOpenDocument")
    events: list[str] = []
    saved_path = ""
    try:
        
        if "open_document" == "reload_document":
            saved_path = document.Name + ".FCStd"
            document.saveAs(saved_path)
        if "open_document" == "open_document":
            saved_path = document.Name + ".FCStd"
            document.saveAs(saved_path)
        ...
        result = run_open_document(collab, saved_path)
