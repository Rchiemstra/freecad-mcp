"""Unit coverage for the typed ``repair_references`` mutation slice."""
...
from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops.repair_references_mutation import (
    _repair_references_native_result,
...
def test_invalid_arguments_abort_without_commit():
    ...
    result = run_repair_references(collab, "Doc", [])
...
def test_missing_document_fails_without_entering_apply():
    ...
    if "repair_references" in {"create_document", "open_document"}:
...
def test_native_capability_is_required_before_apply():
    ...
    if "repair_references" not in {"create_document", "open_document"}:
        seeded = FakeDocument(events)
        pad = seeded.addObject('Part::Feature', 'Pad')
        sketch = seeded.addObject('Sketcher::SketchObject', 'Sketch')
        sketch.PropertiesList = ['Support']
        sketch._prop_types['Support'] = 'App::PropertyLinkSubList'
        seeded.events.clear()
...
def test_unknown_or_contradictory_native_evidence_cannot_release_success(native_result):
    result = _repair_references_native_result(native_result, _NativeMutationState(postcondition_passed=True))
    assert result["success"] is False
