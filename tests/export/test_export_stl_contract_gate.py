...
def test_export_stl_architecture_gate_rejects_the_production_path_for_policy_reasons() -> None:
    violations = scan_export_stl_architecture(ROOT)
    assert violations != []
    assert any(item.startswith("EXT export_stl") for item in violations)
