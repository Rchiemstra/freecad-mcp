"""Agent-to-FreeCAD wire contract for the ``export_brep`` MCP operation."""
...
from freecad_mcp._shared.protocol.export_brep_contract import make_export_brep_success
from freecad_mcp.freecad_client import FreeCADConnection
from freecad_mcp.freecad_client_ops.connection_methods.connection_headers_ops import (
    configure_rpc_session,
)
from freecad_mcp.generated.capabilities.register_modules import tools_io_export
...
class _RecordingFreeCADTransport:
    ...
    def request(self, path, payload, headers):
        ...
        if body_result is _DEFAULT_RESPONSE:
            from tests.export.test_export_brep_response import _success
...
def _invoke(monkeypatch, transport, arguments):
    ...
    monkeypatch.setattr(tools_io_export, "server_connection", lambda: connection)
    monkeypatch.setattr(
        tools_io_export,
        "server_state",
    ...
    mcp = InstrumentedFastMCP("export_brep-contract")
    exports = {}
    tools_io_export._register_export_brep(mcp, dependencies=object(), exports=exports)

    async def invoke():
        async with create_connected_server_and_client_session(mcp._mcp_server) as client:
            listing = await client.list_tools()
            tool = next(tool for tool in listing.tools if tool.name == "export_brep")
            return await client.call_tool("export_brep", arguments)
...
def test_export_brep_sends_authenticated_json_rpc_to_freecad(monkeypatch):
    ...
    if "export_brep" == "create_assembly_grounded_joint":
        arguments.update({"assembly_name": "Assembly", "component_name": "Base"})
    elif "export_brep" == "create_assembly_joint":
        arguments.update({
