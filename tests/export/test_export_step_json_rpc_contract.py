"""Agent-to-FreeCAD wire contract for the ``export_step`` MCP operation."""
...
from freecad_mcp._shared.protocol.export_step_contract import make_export_step_success
...
class _RecordingFreeCADTransport:
    ...
    def request(self, path, payload, headers):
        ...
        if body_result is _DEFAULT_RESPONSE:
            from tests.export.test_export_step_response import _success
...
def _invoke(monkeypatch, transport, arguments):
    ...
    mcp = InstrumentedFastMCP("export_step-contract")
    exports = {}
    tools_io_export._register_export_step(mcp, dependencies=object(), exports=exports)

    async def invoke():
        async with create_connected_server_and_client_session(mcp._mcp_server) as client:
            listing = await client.list_tools()
            tool = next(tool for tool in listing.tools if tool.name == "export_step")
            return await client.call_tool("export_step", arguments)
...
def test_export_step_sends_authenticated_json_rpc_to_freecad(monkeypatch):
    ...
    if "export_step" == "create_assembly_grounded_joint":
        arguments.update({"assembly_name": "Assembly", "component_name": "Base"})
    elif "export_step" == "create_assembly_joint":
        arguments.update({
