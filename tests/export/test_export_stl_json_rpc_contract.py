"""Agent-to-FreeCAD wire contract for the ``export_stl`` MCP operation."""
...
from freecad_mcp._shared.protocol.export_stl_contract import make_export_stl_success
...
class _RecordingFreeCADTransport:
    ...
    def request(self, path, payload, headers):
        ...
        if body_result is _DEFAULT_RESPONSE:
            from tests.export.test_export_stl_response import _success
...
def _invoke(monkeypatch, transport, arguments):
    ...
    mcp = InstrumentedFastMCP("export_stl-contract")
    exports = {}
    tools_io_export._register_export_stl(mcp, dependencies=object(), exports=exports)

    async def invoke():
        async with create_connected_server_and_client_session(mcp._mcp_server) as client:
            listing = await client.list_tools()
            tool = next(tool for tool in listing.tools if tool.name == "export_stl")
            return await client.call_tool("export_stl", arguments)
...
def test_export_stl_sends_authenticated_json_rpc_to_freecad(monkeypatch):
    ...
    if "export_stl" == "create_assembly_grounded_joint":
        arguments.update({"assembly_name": "Assembly", "component_name": "Base"})
    elif "export_stl" == "create_assembly_joint":
        arguments.update({
