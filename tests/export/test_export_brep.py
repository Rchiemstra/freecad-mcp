"""Unit coverage for the typed ``export_brep`` mutation slice."""
...
from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops import export_brep as subject
...
def _collaborators(document, events, *, validator=None, final_result=None, monkeypatch=None):
    if monkeypatch is not None:
        monkeypatch.setattr(measure_io_actions, "export_brep", _fake_payload)
...
def test_export_brep_runs_apply_recompute_inspect_validate_then_commits(monkeypatch):
    events = []
    document = _Document(events)
    collaborators, api = _collaborators(document, events, monkeypatch=monkeypatch)
    result = run_export_brep(collaborators, "Doc", "Box", "/tmp/out.brep")
...
def test_invalid_names_abort_without_recompute_or_commit(monkeypatch):
    ...
    result = run_export_brep(collaborators, "Doc", "", "/tmp/out.brep")
...
def test_export_brep_has_typed_rpc_handler():
    assert subject.TYPED_RPC_HANDLER[0] == "export_brep"
    assert callable(subject.TYPED_RPC_HANDLER[1])


def test_unknown_native_evidence_cannot_release_success():
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops.export_brep_mutation import (
        _export_brep_native_result,
        _NativeMutationState,
    )
    result = _export_brep_native_result({"status": "FutureStatus", "committed": False}, _NativeMutationState(postcondition_passed=True))
    assert result["success"] is False
