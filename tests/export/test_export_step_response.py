"""Adversarial wire cases for the export_step contract."""
...
def test_valid_success_round_trips():
    raw = _success()
    assert parse_export_step_response(raw) == raw
...
def test_transport_failure_preserves_unknown_model_state():
    ...
    response = export_step_operation(SimpleNamespace(export_step=lost_response), "Doc", "/tmp/out.step")
    assert response.isError is True
    data = response.structuredContent["data"]
    assert data["error_code"] == "EXPORT_STEP_TRANSPORT_UNCERTAIN"
    assert data["outcome"] == "uncertain"
    assert data["retry_safe"] is False


def test_contradictory_success_cannot_escape():
    result = parse_export_step_response(dict(_success(), rollback_failed=True))
    assert result["success"] is False
    assert result["outcome"] == "uncertain"
