"""Unit coverage for the typed ``create_part_container`` mutation slice."""
...
def _item(name: str, *, label: str | None = None, type_id: str = "App::Part"):
    obj = SimpleNamespace(
        Name=name,
...
def test_create_part_container_runs_apply_recompute_inspect_validate_then_commits():
    events: list[str] = []
    document = _Document(events)
    _seed(document)
    collaborators, _api = _collaborators(document, events)

    result = run_create_part_container(collaborators, "Doc", "Created", None, "error")
...
def test_creation_that_changes_then_raises_is_rolled_back(monkeypatch):
    ...
    monkeypatch.setattr(subject, "apply_create_part_container", mutate_then_raise)
    result = run_create_part_container(collaborators, "Doc", "Created", None, "error")
    assert result["success"] is False
...
def test_unknown_or_contradictory_native_evidence_cannot_release_success():
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops.create_part_container_mutation import (
        _create_part_container_native_result,
    ...
    for native_result in (
        ...
        result = _create_part_container_native_result(native_result, _NativeMutationState(postcondition_passed=True))
        assert result["success"] is False
