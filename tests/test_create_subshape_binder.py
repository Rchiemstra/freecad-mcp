"""Unit coverage for the typed ``create_subshape_binder`` mutation slice."""
...
def test_create_subshape_binder_runs_apply_recompute_inspect_validate_then_commits():
    events: list[str] = []
    document = _Document(events)
    _seed(document)
    collaborators, _api = _collaborators(document, events)

    result = run_create_subshape_binder(collaborators, "Doc", "Created", "Seed", None, None, "Body", False, True, "error")
...
def test_creation_that_changes_then_raises_is_rolled_back(monkeypatch):
    ...
    monkeypatch.setattr(subject, "apply_create_subshape_binder", mutate_then_raise)
    result = run_create_subshape_binder(collaborators, "Doc", "Created", "Seed", None, None, "Body", False, True, "error")
    assert result["success"] is False
...
def test_unknown_or_contradictory_native_evidence_cannot_release_success():
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops.create_subshape_binder_mutation import (
        _create_subshape_binder_native_result,
