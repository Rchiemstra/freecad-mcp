"""Agent-to-FreeCAD wire contract for the ``recompute_document`` MCP operation."""
...
def _invoke_registered(monkeypatch, transport, arguments):
    ...
    mcp = InstrumentedFastMCP("recompute_document-contract")
    exports = {}
    tools_document_history._register_recompute_document(mcp, dependencies=object(), exports=exports)

    async def invoke():
        async with create_connected_server_and_client_session(mcp._mcp_server) as client:
            listing = await client.list_tools()
            tool = next(tool for tool in listing.tools if tool.name == "recompute_document")
            assert set(tool.inputSchema["required"]) == {'doc_name'}
            return await client.call_tool("recompute_document", arguments)
...
def test_recompute_document_sends_exact_authenticated_json_rpc_values_to_freecad(monkeypatch):
    ...
    assert envelope["method"] == "recompute_document"
...
def test_public_route_marks_committed_but_invalid_response_non_retryable(monkeypatch):
    ...
    result = _invoke_registered(monkeypatch, transport, {'doc_name': 'AgentDocument'})
    assert result.isError is True
    assert result.structuredContent["error_code"] == 'RECOMPUTE_DOCUMENT_COMMITTED_RESPONSE_INVALID'
