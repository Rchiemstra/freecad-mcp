"""Unit coverage for the typed ``import_step`` mutation slice."""
...
from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops import import_step as subject
...
def _collaborators(document, events, *, validator=None, final_result=None, monkeypatch=None):
    if monkeypatch is not None:
        monkeypatch.setattr(measure_io_actions, "import_step", _fake_payload)
...
def test_import_step_runs_apply_recompute_inspect_validate_then_commits(monkeypatch):
    events = []
...
def test_invalid_names_abort_without_recompute_or_commit(monkeypatch):
    ...
    result = run_import_step(collaborators, "Doc", "")
...
def test_unknown_native_evidence_cannot_release_success():
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops.import_step_mutation import (
        _import_step_native_result,
        _NativeMutationState,
    )
    result = _import_step_native_result({"status": "FutureStatus", "committed": False}, _NativeMutationState(postcondition_passed=True))
    assert result["success"] is False
