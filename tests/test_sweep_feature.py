"""Unit coverage for the typed ``sweep_feature`` mutation slice."""
...
from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops import sweep_feature as subject
from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops.sweep_feature import run_sweep_feature
from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops.sweep_feature_mutation import (
    _SweepFeatureNativeMutationState,
    _sweep_feature_native_result,
)
...
def _run(collab, **overrides):
    ...
    return run_sweep_feature(collab, **payload)


def test_sweep_feature_runs_apply_recompute_inspect_validate_then_commits():
    events: list[str] = []
    document = FeatureDocument(events)
    prepare_document('sweep', document)
...
def test_creation_that_changes_then_raises_is_rolled_back():
    ...
    assert result["success"] is False
    assert result["error_code"] == "SWEEP_FEATURE_FAILED"
...
def test_apply_and_inspect_use_the_native_admitted_document():
    ...
    prepare_document('sweep', admitted)
...
def test_unknown_or_contradictory_native_evidence_cannot_release_success(native_result):
    result = _sweep_feature_native_result(
        native_result, _SweepFeatureNativeMutationState(postcondition_passed=True)
    )
    assert result["success"] is False
...
def test_typed_rpc_handler_is_registered():
    ...
    assert "sweep_feature" in discover_typed_rpc_handlers()
    assert subject.TYPED_RPC_HANDLER[0] == "sweep_feature"
