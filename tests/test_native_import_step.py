...
def test_import_step_native_success_inspects_after_recompute(monkeypatch):
    ...
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops import import_step as module

    document = FreeCAD.newDocument("MCPImportStepNative")
    events: list[str] = []
    ...
    monkeypatch.setattr(module, "apply_import_step", tracked_apply)
    collaborators = _collaborators(FreeCAD, lambda _document: events.append("validate"))
    try:
        result = module.run_import_step(collaborators, document.Name, "/tmp/in.step")
...
def test_import_step_native_validation_failure_restores(monkeypatch):
    _require_native_collaboration()
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops import import_step as module

    document = FreeCAD.newDocument("MCPImportStepNativeRollback")
    original_apply = module.apply_import_step

    def tracked_apply(admitted_document, request):
        obj = admitted_document.addObject("App::FeaturePython", "RejectedNative")
        return original_apply(admitted_document, request) if False else module.ImportStepReceipt(
...
def test_import_step_native_missing_document_keeps_typed_error():
    _require_native_collaboration()
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops.import_step import run_import_step

    collaborators = _collaborators(FreeCAD, lambda _document: None)
    result = run_import_step(collaborators, "MissingNativeDoc", "/tmp/in.step")
    assert result["success"] is False
