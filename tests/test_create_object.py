"""Unit coverage for the typed ``create_object`` mutation slice."""
...
from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops.create_object_mutation import (
    _create_object_native_result,
...
def test_create_object_runs_apply_recompute_validate_then_commits():
    events: list[str] = []
...
def test_invalid_arguments_abort_without_commit():
    ...
    result = run_create_object(collab, "Doc", {"Name": "", "Type": "Part::Box"})
...
def test_missing_document_fails_without_entering_apply():
    ...
    if "create_object" in {"create_document", "open_document"}:
...
def test_native_capability_is_required_before_apply():
    ...
    if "create_object" not in {"create_document", "open_document"}:
...
def test_unknown_or_contradictory_native_evidence_cannot_release_success(native_result):
    result = _create_object_native_result(native_result, _NativeMutationState(postcondition_passed=True))
    assert result["success"] is False
