"""Unit coverage for the typed ``fillet_feature`` mutation slice."""
...
from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops import fillet_feature as subject
from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops.fillet_feature import run_fillet_feature
from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops.fillet_feature_mutation import (
    _FilletFeatureNativeMutationState,
    _fillet_feature_native_result,
)
...
def test_creation_that_changes_then_raises_is_rolled_back():
    ...
    assert result["success"] is False
    assert result["error_code"] == "FILLET_FEATURE_FAILED"
...
def test_unknown_or_contradictory_native_evidence_cannot_release_success(native_result):
    result = _fillet_feature_native_result(
        native_result, _FilletFeatureNativeMutationState(postcondition_passed=True)
    )
    assert result["success"] is False
...
def test_typed_rpc_handler_is_registered():
    ...
    assert "fillet_feature" in discover_typed_rpc_handlers()
    assert subject.TYPED_RPC_HANDLER[0] == "fillet_feature"
