"""Unit coverage for the typed ``import_brep`` mutation slice."""
...
from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops import import_brep as subject
from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops.import_brep import run_import_brep
...
def _collaborators(document, events, *, validator=None, final_result=None, monkeypatch=None):
    if monkeypatch is not None:
        monkeypatch.setattr(measure_io_actions, "import_brep", _fake_payload)
...
def test_import_brep_runs_apply_recompute_inspect_validate_then_commits(monkeypatch):
    events = []
    document = _Document(events)
    collaborators, api = _collaborators(document, events, monkeypatch=monkeypatch)
    result = run_import_brep(collaborators, "Doc", "/tmp/in.brep")
...
def test_invalid_names_abort_without_recompute_or_commit(monkeypatch):
    ...
    result = run_import_brep(collaborators, "Doc", "")
...
def test_import_brep_has_typed_rpc_handler():
    assert subject.TYPED_RPC_HANDLER[0] == "import_brep"
    assert callable(subject.TYPED_RPC_HANDLER[1])


def test_unknown_native_evidence_cannot_release_success():
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops.import_brep_mutation import (
        _import_brep_native_result,
        _NativeMutationState,
    )
    result = _import_brep_native_result({"status": "FutureStatus", "committed": False}, _NativeMutationState(postcondition_passed=True))
    assert result["success"] is False
