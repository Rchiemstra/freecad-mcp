...
from freecad_mcp.generated.capabilities.register_modules import tools_io_import
...
class _RecordingFreeCADTransport:
    ...
    def request(self, path, payload, headers):
        ...
        if body_result is _DEFAULT_RESPONSE:
            from tests.import_cad.test_import_brep_response import _success
...
def _invoke(monkeypatch, transport, arguments):
    ...
    monkeypatch.setattr(tools_io_import, "server_connection", lambda: connection)
    monkeypatch.setattr(
        tools_io_import,
        "server_state",
