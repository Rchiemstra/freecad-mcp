...
def test_create_involute_gear_native_success_inspects_after_recompute(monkeypatch):
    ...
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops import create_involute_gear as module

    document = FreeCAD.newDocument("MCPCreateInvoluteGearNative")
    events: list[str] = []
    ...
    monkeypatch.setattr(module, "apply_create_involute_gear", tracked_apply)
    collaborators = _collaborators(FreeCAD, lambda _document: events.append("validate"))
    try:
        result = module.run_create_involute_gear(collaborators, document.Name, "Gear", 8, 2.0, 10.0)
...
def test_create_involute_gear_native_validation_failure_restores(monkeypatch):
    _require_native_collaboration()
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops import create_involute_gear as module

    document = FreeCAD.newDocument("MCPCreateInvoluteGearNativeRollback")
    original_apply = module.apply_create_involute_gear

    def tracked_apply(admitted_document, request):
        obj = admitted_document.addObject("App::FeaturePython", "RejectedNative")
        return original_apply(admitted_document, request) if False else module.CreateInvoluteGearReceipt(
...
def test_create_involute_gear_native_missing_document_keeps_typed_error():
    ...
    result = run_create_involute_gear(collaborators, "MissingNativeDoc", "Gear", 8, 2.0, 10.0)
    assert result["success"] is False
