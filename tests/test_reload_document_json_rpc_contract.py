"""Agent-to-FreeCAD wire contract for the ``reload_document`` MCP operation."""
...
def _invoke_registered(monkeypatch, transport, arguments):
    ...
    mcp = InstrumentedFastMCP("reload_document-contract")
    exports = {}
    tools_gui_document_a._register_reload_document(mcp, dependencies=object(), exports=exports)

    async def invoke():
        async with create_connected_server_and_client_session(mcp._mcp_server) as client:
            listing = await client.list_tools()
            tool = next(tool for tool in listing.tools if tool.name == "reload_document")
            assert set(tool.inputSchema["required"]) == {'doc_name'}
            return await client.call_tool("reload_document", arguments)
...
def test_reload_document_sends_exact_authenticated_json_rpc_values_to_freecad(monkeypatch):
    ...
    assert envelope["method"] == "reload_document"
    assert envelope["params"] == {'doc_name': 'AgentDocument'}
    assert envelope["operation"] == {"name": 'Reload document'}
...
def test_public_route_marks_committed_but_invalid_response_non_retryable(monkeypatch):
    ...
    result = _invoke_registered(monkeypatch, transport, {'doc_name': 'AgentDocument'})
    assert result.isError is True
    assert result.structuredContent["error_code"] == 'RELOAD_DOCUMENT_COMMITTED_RESPONSE_INVALID'
