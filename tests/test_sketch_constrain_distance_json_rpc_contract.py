"""Agent-to-FreeCAD wire contract for the ``sketch_constrain_distance`` MCP operation."""
...
_TOOL_PARAMS = {'doc_name': 'AgentDocument', 'sketch_name': 'Sketch', 'geo': 0, 'value': 10.0, 'pos': None, 'name': None}
...
def _invoke_registered(monkeypatch, transport, arguments):
    ...
    mcp = InstrumentedFastMCP("sketch_constrain_distance-contract")
    exports = {}
    tools_sketch_constraints_1._register_sketch_constrain_distance(mcp, dependencies=object(), exports=exports)

    async def invoke():
        async with create_connected_server_and_client_session(mcp._mcp_server) as client:
            listing = await client.list_tools()
            tool = next(tool for tool in listing.tools if tool.name == "sketch_constrain_distance")
            assert set(tool.inputSchema["required"]) == {'doc_name', 'value', 'sketch_name', 'geo'}
            return await client.call_tool("sketch_constrain_distance", arguments)
...
def test_sketch_constrain_distance_sends_exact_authenticated_json_rpc_values_to_freecad(monkeypatch):
    ...
    assert envelope["method"] == "sketch_constrain_distance"
