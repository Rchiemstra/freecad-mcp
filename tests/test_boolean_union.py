"""Unit coverage for the typed ``boolean_union`` mutation slice."""
...
from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops import boolean_union as subject
from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops.boolean_union import run_boolean_union
from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops.boolean_union_mutation import (
    _BooleanUnionNativeMutationState,
    _boolean_union_native_result,
)
...
def _run(collab, **overrides):
    ...
    return run_boolean_union(collab, **payload)


def test_boolean_union_runs_apply_recompute_inspect_validate_then_commits():
    events: list[str] = []
...
def test_creation_that_changes_then_raises_is_rolled_back():
    ...
    assert result["success"] is False
    assert result["error_code"] == "BOOLEAN_UNION_FAILED"
...
def test_unknown_or_contradictory_native_evidence_cannot_release_success(native_result):
    result = _boolean_union_native_result(
        native_result, _BooleanUnionNativeMutationState(postcondition_passed=True)
    )
    assert result["success"] is False
...
def test_typed_rpc_handler_is_registered():
    ...
    assert "boolean_union" in discover_typed_rpc_handlers()
    assert subject.TYPED_RPC_HANDLER[0] == "boolean_union"
