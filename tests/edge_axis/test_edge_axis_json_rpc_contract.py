"""Agent-to-FreeCAD wire contract for the ``edge_axis`` MCP operation."""
...
class _RecordingFreeCADTransport:
    ...
    def request(self, path, payload, headers):
        ...
        if body_result is _DEFAULT_RESPONSE:
            from tests.edge_axis.test_edge_axis_response import _success
...
def _invoke(monkeypatch, transport, arguments):
    ...
    mcp = InstrumentedFastMCP("edge_axis-contract")
    exports = {}
    tools_partdesign_b2._register_edge_axis(mcp, dependencies=object(), exports=exports)

    async def invoke():
        async with create_connected_server_and_client_session(mcp._mcp_server) as client:
            listing = await client.list_tools()
            tool = next(tool for tool in listing.tools if tool.name == "edge_axis")
            return await client.call_tool("edge_axis", arguments)
...
def test_edge_axis_sends_authenticated_json_rpc_to_freecad(monkeypatch):
    ...
    assert envelope["method"] == "edge_axis"
    assert envelope["params"]["doc_name"] == "AgentDocument"
    assert envelope["params"]["edge"] == "Edge1"
