"""Agent-to-FreeCAD wire contract for the ``set_expression`` MCP operation."""
...
from freecad_mcp.generated.capabilities.register_modules import tools_parametric_sheet_b
...
class _RecordingFreeCADTransport:
    ...
    def request(self, path, payload, headers):
        ...
        if body_result is _DEFAULT_RESPONSE:
            body_result = dict({"contract_version": 1, "success": True, "ok": True, "outcome": "committed", "committed": True, "retry_safe": False, "object": "Value", "prop_path": "Value", "expression": "Value"}, **{
                key: envelope["params"][key]
                for key in envelope["params"]
                if key in ['object', 'prop_path', 'expression']
            })
            body_result = {"contract_version": 1, "success": True, "ok": True, "outcome": "committed", "committed": True, "retry_safe": False, "object": "Value", "prop_path": "Value", "expression": "Value"}
        response = {
            "jsonrpc": "2.0",
            "id": request["id"],
            "result": {
...
def _invoke_registered(monkeypatch, transport, params):
    ...
    monkeypatch.setattr(tools_parametric_sheet_b, "server_connection", lambda: connection)
    monkeypatch.setattr(tools_parametric_sheet_b, "server_state", lambda: SimpleNamespace(only_text_feedback=True))
    bind_instrumented_fast_mcp(InstrumentedFastMCP)
    mcp = InstrumentedFastMCP("set_expression-contract")
    exports = {}
    tools_parametric_sheet_b._register_set_expression(mcp, dependencies=object(), exports=exports)

    async def invoke():
        async with create_connected_server_and_client_session(mcp._mcp_server) as client:
            listing = await client.list_tools()
            tool = next(tool for tool in listing.tools if tool.name == "set_expression")
            assert "doc_name" in tool.inputSchema.get("required", ["doc_name"])
            return await client.call_tool("set_expression", params)
...
def test_set_expression_sends_authenticated_json_rpc_to_freecad(monkeypatch):
    transport = _RecordingFreeCADTransport()
    result = _invoke_registered(monkeypatch, transport, {"doc_name": "AgentDocument", "object_name": "Value", "prop_path": "Value", "expression": "Value"})
    ...
    assert envelope["method"] == "set_expression"
...
def test_mcp_schema_rejects_non_string_doc_name_before_json_rpc(monkeypatch):
    transport = _RecordingFreeCADTransport()
    params = dict({"doc_name": "AgentDocument", "object_name": "Value", "prop_path": "Value", "expression": "Value"})
    params["doc_name"] = 42
