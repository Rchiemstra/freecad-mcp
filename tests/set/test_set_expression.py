"""Unit coverage for the typed ``set_expression`` mutation slice."""
...
def test_set_expression_runs_apply_recompute_inspect_validate_then_commits():
    events: list[str] = []
    document = _Document(events)
    _seed(document)
    collaborators, _api = _collaborators(document, events)

    result = run_set_expression(collaborators, "Doc", "Seed", "Length", "1+1")
...
def test_creation_that_changes_then_raises_is_rolled_back(monkeypatch):
    ...
    monkeypatch.setattr(subject, "apply_set_expression", mutate_then_raise)
    result = run_set_expression(collaborators, "Doc", "Seed", "Length", "1+1")
    assert result["success"] is False
...
def test_unknown_or_contradictory_native_evidence_cannot_release_success():
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops.set_expression_mutation import (
        _set_expression_native_result,
