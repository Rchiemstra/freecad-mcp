"""Agent-to-FreeCAD wire contract for the ``clear_expression`` MCP operation."""
...
class _RecordingFreeCADTransport:
    ...
    def request(self, path, payload, headers):
        ...
        if body_result is _DEFAULT_RESPONSE:
            body_result = dict({"contract_version": 1, "success": True, "ok": True, "outcome": "committed", "committed": True, "retry_safe": False, "object": "Value", "prop_path": "Value"}, **{
                key: envelope["params"][key]
                for key in envelope["params"]
                if key in ['object', 'prop_path']
            })
            body_result = {"contract_version": 1, "success": True, "ok": True, "outcome": "committed", "committed": True, "retry_safe": False, "object": "Value", "prop_path": "Value"}
        response = {
            "jsonrpc": "2.0",
            "id": request["id"],
            "result": {
...
def _invoke_registered(monkeypatch, transport, params):
    ...
    mcp = InstrumentedFastMCP("clear_expression-contract")
    exports = {}
    tools_parametric_sheet_b._register_clear_expression(mcp, dependencies=object(), exports=exports)

    async def invoke():
        async with create_connected_server_and_client_session(mcp._mcp_server) as client:
            listing = await client.list_tools()
            tool = next(tool for tool in listing.tools if tool.name == "clear_expression")
            assert "doc_name" in tool.inputSchema.get("required", ["doc_name"])
            return await client.call_tool("clear_expression", params)
...
def test_clear_expression_sends_authenticated_json_rpc_to_freecad(monkeypatch):
    transport = _RecordingFreeCADTransport()
    result = _invoke_registered(monkeypatch, transport, {"doc_name": "AgentDocument", "object_name": "Value", "prop_path": "Value"})
    ...
    assert envelope["method"] == "clear_expression"
...
def test_mcp_schema_rejects_non_string_doc_name_before_json_rpc(monkeypatch):
    transport = _RecordingFreeCADTransport()
    params = dict({"doc_name": "AgentDocument", "object_name": "Value", "prop_path": "Value"})
    params["doc_name"] = 42
