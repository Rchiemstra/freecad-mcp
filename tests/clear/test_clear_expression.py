"""Unit coverage for the typed ``clear_expression`` mutation slice."""
...
def test_clear_expression_runs_apply_recompute_inspect_validate_then_commits():
    events: list[str] = []
    document = _Document(events)
    _seed(document)
    collaborators, _api = _collaborators(document, events)

    result = run_clear_expression(collaborators, "Doc", "Seed", "Length")
...
def test_creation_that_changes_then_raises_is_rolled_back(monkeypatch):
    ...
    monkeypatch.setattr(subject, "apply_clear_expression", mutate_then_raise)
    result = run_clear_expression(collaborators, "Doc", "Seed", "Length")
    assert result["success"] is False
...
def test_unknown_or_contradictory_native_evidence_cannot_release_success():
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops.clear_expression_mutation import (
        _clear_expression_native_result,
    ...
    for native_result in (
        ...
        result = _clear_expression_native_result(native_result, _NativeMutationState(postcondition_passed=True))
        assert result["success"] is False
