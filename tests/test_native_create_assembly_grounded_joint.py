"""Native qualification matrix for typed ``create_assembly_grounded_joint``."""
...
def test_create_assembly_grounded_joint_native_success_inspects_after_recompute(monkeypatch):
    _require_native_collaboration()
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops import create_assembly_grounded_joint as module

    document = FreeCAD.newDocument("MCPCreateAssemblyGroundedJointNative")
    events: list[str] = []
    ...
    original_apply = module.apply_create_assembly_grounded_joint

    def tracked_apply(admitted_document, request):
        events.append("apply")
        probe.touch()
        obj = admitted_document.addObject("App::FeaturePython", "CreatedNative")
        return module.CreateAssemblyGroundedJointReceipt(
            payload={
                "assembly": obj.Name,
                "label": obj.Label,
                "type": obj.TypeId,
    ...
    monkeypatch.setattr(module, "apply_create_assembly_grounded_joint", tracked_apply)
    collaborators = _collaborators(FreeCAD, lambda _document: events.append("validate"))
    try:
        result = module.run_create_assembly_grounded_joint(collaborators, document.Name, "Assembly", "Base")
...
def test_create_assembly_grounded_joint_native_validation_failure_restores(monkeypatch):
    _require_native_collaboration()
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops import create_assembly_grounded_joint as module

    document = FreeCAD.newDocument("MCPCreateAssemblyGroundedJointNativeRollback")
    original_apply = module.apply_create_assembly_grounded_joint

    def tracked_apply(admitted_document, request):
        obj = admitted_document.addObject("App::FeaturePython", "RejectedNative")
        return original_apply(admitted_document, request) if False else module.CreateAssemblyGroundedJointReceipt(
...
def test_create_assembly_grounded_joint_native_missing_document_keeps_typed_error():
    _require_native_collaboration()
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops.create_assembly_grounded_joint import run_create_assembly_grounded_joint

    collaborators = _collaborators(FreeCAD, lambda _document: None)
    result = run_create_assembly_grounded_joint(collaborators, "MissingNativeDoc", "Assembly", "Base")
    assert result["success"] is False
