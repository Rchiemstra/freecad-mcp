"""Agent-to-FreeCAD wire contract for the ``get_document_tree`` MCP operation."""
...
class _RecordingFreeCADTransport:
    ...
    def request(self, path, payload, headers):
        ...
        if body_result is _DEFAULT_RESPONSE:
            from tests.document_tree.test_get_document_tree_response import _success
...
def _invoke(monkeypatch, transport, arguments):
    ...
    mcp = InstrumentedFastMCP("get_document_tree-contract")
    exports = {}
    tools_io_export._register_get_document_tree(mcp, dependencies=object(), exports=exports)

    async def invoke():
        async with create_connected_server_and_client_session(mcp._mcp_server) as client:
            listing = await client.list_tools()
            tool = next(tool for tool in listing.tools if tool.name == "get_document_tree")
            return await client.call_tool("get_document_tree", arguments)
...
def test_get_document_tree_sends_authenticated_json_rpc_to_freecad(monkeypatch):
    ...
    assert envelope["method"] == "get_document_tree"
