"""Agent-to-FreeCAD wire contract for the ``bounding_box`` MCP operation."""
...
from freecad_mcp._shared.protocol.bounding_box_contract import make_bounding_box_success
from freecad_mcp.freecad_client import FreeCADConnection
from freecad_mcp.freecad_client_ops.connection_methods.connection_headers_ops import (
    configure_rpc_session,
)
from freecad_mcp.generated.capabilities.register_modules import tools_measure_a
...
class _RecordingFreeCADTransport:
    ...
    def request(self, path, payload, headers):
        ...
        if body_result is _DEFAULT_RESPONSE:
            from tests.bounding.test_bounding_box_response import _success
...
def _invoke(monkeypatch, transport, arguments):
    ...
    monkeypatch.setattr(tools_measure_a, "server_connection", lambda: connection)
    monkeypatch.setattr(
        tools_measure_a,
        "server_state",
    ...
    mcp = InstrumentedFastMCP("bounding_box-contract")
    exports = {}
    tools_measure_a._register_bounding_box(mcp, dependencies=object(), exports=exports)

    async def invoke():
        async with create_connected_server_and_client_session(mcp._mcp_server) as client:
            listing = await client.list_tools()
            tool = next(tool for tool in listing.tools if tool.name == "bounding_box")
            return await client.call_tool("bounding_box", arguments)
...
def test_bounding_box_sends_authenticated_json_rpc_to_freecad(monkeypatch):
    ...
    if "bounding_box" == "create_assembly_grounded_joint":
        arguments.update({"assembly_name": "Assembly", "component_name": "Base"})
    elif "bounding_box" == "create_assembly_joint":
        arguments.update({
