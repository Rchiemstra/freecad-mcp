...
class _RecordingFreeCADTransport:
    ...
    def request(self, path, payload, headers):
        ...
        if body_result is _DEFAULT_RESPONSE:
            body_result = dict({"contract_version": 1, "success": True, "ok": True, "outcome": "committed", "committed": True, "retry_safe": False, "source": "Value"}, **{
                key: envelope["params"][key]
                for key in envelope["params"]
                if key in ['source']
            })
            body_result = {"contract_version": 1, "success": True, "ok": True, "outcome": "committed", "committed": True, "retry_safe": False, "source": "Value"}
        response = {
            "jsonrpc": "2.0",
            "id": request["id"],
            "result": {
...
def _invoke_registered(monkeypatch, transport, params):
    ...
    monkeypatch.setattr(tools_advanced_a, "server_state", lambda: SimpleNamespace(only_text_feedback=True))
    ...
    async def invoke():
        async with create_connected_server_and_client_session(mcp._mcp_server) as client:
            ...
            return await client.call_tool("validate_movement_follow", params)
...
def test_validate_movement_follow_sends_authenticated_json_rpc_to_freecad(monkeypatch):
    transport = _RecordingFreeCADTransport()
    result = _invoke_registered(monkeypatch, transport, {"doc_name": "AgentDocument", "source": "Value", "dependents": ["Seed"], "translation": [0, 0, 1], "axis": [0, 0, 1], "angle_deg": 1.0, "restore": True, "tolerance": 1.0})
...
def test_mcp_schema_rejects_non_string_doc_name_before_json_rpc(monkeypatch):
    transport = _RecordingFreeCADTransport()
    params = dict({"doc_name": "AgentDocument", "source": "Value", "dependents": ["Seed"], "translation": [0, 0, 1], "axis": [0, 0, 1], "angle_deg": 1.0, "restore": True, "tolerance": 1.0})
    params["doc_name"] = 42
