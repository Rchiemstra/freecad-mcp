"""Unit coverage for the typed ``sketch_constrain_tangent`` mutation slice."""
...
from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops import sketch_constrain_tangent as subject
...
def test_sketch_constrain_tangent_runs_apply_recompute_inspect_validate_then_commits():
    events = []
    document = FakeDocument(events)
    _prepare(document, events)
    collab, api = collaborators(document, events)

    result = run_sketch_constrain_tangent(collab, "Doc", "Sketch", 0, 1)
...
def test_invalid_names_abort_without_recompute_or_commit(sketch_name):
    ...
    result = run_sketch_constrain_tangent(collab, "Doc", sketch_name, 0, 1)
...
def test_missing_document_fails_without_entering_the_apply_callback():
    ...
    result = run_sketch_constrain_tangent(collab, "Missing", "Sketch", 0, 1)
...
def test_sketch_constrain_tangent_has_no_uncoordinated_source_entry_point():
    assert not hasattr(subject, "sketch_constrain_tangent_gui")
...
def test_unknown_or_contradictory_native_evidence_cannot_release_success(native_result):
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops.sketch_constrain_tangent_mutation import (
        _sketch_constrain_tangent_native_result,
        _NativeSketchConstrainTangentMutationState,
    )

    result = _sketch_constrain_tangent_native_result(
        native_result, _NativeSketchConstrainTangentMutationState(postcondition_passed=True)
    )
    assert result["success"] is False
