"""Agent-to-FreeCAD wire contract for the ``run_fem_analysis`` MCP operation."""
...
def _invoke_registered(monkeypatch, transport, arguments):
    ...
    exports = {}
    tools_advanced_b2._register_run_fem_analysis(mcp, dependencies=object(), exports=exports)
...
def test_public_route_marks_committed_but_invalid_response_non_retryable(monkeypatch):
    ...
    result = _invoke_registered(monkeypatch, transport, dict(_DEFAULT_ARGUMENTS))
    assert result.isError is True
    assert result.structuredContent["error_code"] == "INVALID_RUN_FEM_ANALYSIS_RESPONSE"
