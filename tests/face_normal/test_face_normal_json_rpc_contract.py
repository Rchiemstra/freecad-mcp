"""Agent-to-FreeCAD wire contract for the ``face_normal`` MCP operation."""
...
class _RecordingFreeCADTransport:
    ...
    def request(self, path, payload, headers):
        ...
        if body_result is _DEFAULT_RESPONSE:
            from tests.face_normal.test_face_normal_response import _success
...
def _invoke(monkeypatch, transport, arguments):
    ...
    mcp = InstrumentedFastMCP("face_normal-contract")
    exports = {}
    tools_partdesign_b._register_face_normal(mcp, dependencies=object(), exports=exports)

    async def invoke():
        async with create_connected_server_and_client_session(mcp._mcp_server) as client:
            listing = await client.list_tools()
            tool = next(tool for tool in listing.tools if tool.name == "face_normal")
            return await client.call_tool("face_normal", arguments)
...
def test_face_normal_sends_authenticated_json_rpc_to_freecad(monkeypatch):
    ...
    assert envelope["method"] == "face_normal"
    assert envelope["params"]["doc_name"] == "AgentDocument"
    assert envelope["params"]["face"] == "Face1"
