"""Agent-to-FreeCAD wire contract for the ``sweep_pipe`` MCP operation."""
...
class _RecordingFreeCADTransport:
    ...
    def request(self, path, payload, headers):
        ...
        if body_result is _DEFAULT_RESPONSE:
            body_result = dict({"contract_version": 1, "success": True, "ok": True, "outcome": "committed", "committed": True, "retry_safe": False, "solid_name": "Value"}, **{
                key: envelope["params"][key]
                for key in envelope["params"]
                if key in ['solid_name']
            })
            body_result = {"contract_version": 1, "success": True, "ok": True, "outcome": "committed", "committed": True, "retry_safe": False, "solid_name": "Value"}
        response = {
            "jsonrpc": "2.0",
            "id": request["id"],
            "result": {
...
def _invoke_registered(monkeypatch, transport, params):
    ...
    mcp = InstrumentedFastMCP("sweep_pipe-contract")
    exports = {}
    tools_partdesign_a2._register_sweep_pipe(mcp, dependencies=object(), exports=exports)

    async def invoke():
        async with create_connected_server_and_client_session(mcp._mcp_server) as client:
            listing = await client.list_tools()
            tool = next(tool for tool in listing.tools if tool.name == "sweep_pipe")
            assert "doc_name" in tool.inputSchema.get("required", ["doc_name"])
            return await client.call_tool("sweep_pipe", params)
...
def test_sweep_pipe_sends_authenticated_json_rpc_to_freecad(monkeypatch):
    transport = _RecordingFreeCADTransport()
    result = _invoke_registered(monkeypatch, transport, {"doc_name": "AgentDocument", "path_wire": "Value", "diameter_mm": 1.0, "solid_name": "Value", "profile_mode": "Value", "if_exists": "error"})
    ...
    assert envelope["method"] == "sweep_pipe"
...
def test_mcp_schema_rejects_non_string_doc_name_before_json_rpc(monkeypatch):
    transport = _RecordingFreeCADTransport()
    params = dict({"doc_name": "AgentDocument", "path_wire": "Value", "diameter_mm": 1.0, "solid_name": "Value", "profile_mode": "Value", "if_exists": "error"})
    params["doc_name"] = 42
