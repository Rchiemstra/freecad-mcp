...
def test_common_volume_along_path_native_success_inspects_after_recompute(monkeypatch):
    ...
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops import common_volume_along_path as module

    document = FreeCAD.newDocument("MCPCommonVolumeAlongPathNative")
    events: list[str] = []
    ...
    monkeypatch.setattr(module, "apply_common_volume_along_path", tracked_apply)
    collaborators = _collaborators(FreeCAD, lambda _document: events.append("validate"))
    try:
        result = module.run_common_volume_along_path(collaborators, document.Name, "Mover", ["Wall"], None, 2, [{"x": 0, "y": 0, "z": 0}])
...
def test_common_volume_along_path_native_validation_failure_restores(monkeypatch):
    _require_native_collaboration()
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops import common_volume_along_path as module

    document = FreeCAD.newDocument("MCPCommonVolumeAlongPathNativeRollback")
    original_apply = module.apply_common_volume_along_path

    def tracked_apply(admitted_document, request):
        obj = admitted_document.addObject("App::FeaturePython", "RejectedNative")
        return original_apply(admitted_document, request) if False else module.CommonVolumeAlongPathReceipt(
...
def test_common_volume_along_path_native_missing_document_keeps_typed_error():
    ...
    result = run_common_volume_along_path(collaborators, "MissingNativeDoc", "Mover", ["Wall"], None, 2, [{"x": 0, "y": 0, "z": 0}])
    assert result["success"] is False
