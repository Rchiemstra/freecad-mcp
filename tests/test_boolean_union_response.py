"""Adversarial wire cases for the ``boolean_union`` contract."""
...
from freecad_mcp._shared.protocol.boolean_union_contract import (
    FeatureName,
...
from freecad_mcp.operations.parametric_ops.boolean_union import boolean_union_operation


def _success():
    return make_boolean_union_success(FeatureName("Feature001"), "Main Feature")


@pytest.mark.parametrize("raw", [None, [], 1, "timeout", {}, {1: "bad key"}])
def test_unknown_response_never_proves_rejection(raw):
    result = parse_boolean_union_response(raw)
...
def test_contract_version_must_be_the_exact_integer(version):
    assert parse_boolean_union_response(dict(_success(), contract_version=version))["success"] is False
...
def test_contradictory_or_malformed_success_cannot_escape(change):
    result = parse_boolean_union_response(dict(_success(), **change))
    assert result["success"] is False
    assert result["outcome"] == "uncertain"
    assert result["retry_safe"] is False
...
@pytest.mark.parametrize(
    ...
    [
        _success(),
        make_boolean_union_failure("BUSY", "Retry after recompute", native_status="Busy"),
        make_boolean_union_failure("INVALID_ARGUMENT", "Invalid name", retry_safe=False),
        *[
            make_boolean_union_uncertain(
                "STATE_UNKNOWN",
        ...
        make_boolean_union_uncertain(
            "BOOLEAN_UNION_ROLLBACK_UNCERTAIN",
            "Rollback failed",
            committed=None,
            native_status="RollbackFailed",
            rollback_failed=True,
...
def test_valid_variants_round_trip_without_changing_evidence(raw):
    assert parse_boolean_union_response(raw) == raw
    assert parse_boolean_union_response(parse_boolean_union_response(raw)) == raw
...
def test_transport_failure_preserves_unknown_model_state():
    ...
    response = boolean_union_operation(
        SimpleNamespace(boolean_union=lost_response), True, "Doc", shape1='Shape1', shape2='Shape2', result_name='Result'
    )
    assert response.isError is True
    data = response.structuredContent["data"]
    assert data["error_code"] == "BOOLEAN_UNION_TRANSPORT_UNCERTAIN"
