"""Agent-to-FreeCAD wire contract for the ``polar_pattern_feature`` MCP operation."""
...
from freecad_mcp.generated.capabilities.register_modules import tools_features_basic_2
...
_TOOL_PARAMS = {'doc_name': 'AgentDocument', 'feature_name': 'Pad', 'pattern_name': 'Polar', 'occurrences': 6}
_REQUIRED = {'doc_name', 'occurrences', 'feature_name', 'pattern_name'}
...
class _RecordingFreeCADTransport:
    ...
    def request(self, path, payload, headers):
        ...
        if body_result is _DEFAULT_RESPONSE:
            created = envelope["params"].get('pattern_name', 'pattern_name')
            body_result = {
                "contract_version": 1,
                "success": True,
                "ok": True,
...
def _invoke_registered(monkeypatch, transport, tool_params):
    ...
    monkeypatch.setattr(tools_features_basic_2, "server_connection", lambda: connection)
    monkeypatch.setattr(
    ...
    mcp = InstrumentedFastMCP("polar_pattern_feature-contract")
    exports = {}
    tools_features_basic_2._register_polar_pattern_feature(
        mcp,
        dependencies=object(),
        exports=exports,
    )
    ...
    async def invoke():
        async with create_connected_server_and_client_session(mcp._mcp_server) as client:
            ...
            tool = next(tool for tool in listing.tools if tool.name == "polar_pattern_feature")
            assert set(tool.inputSchema.get("required", [])) == _REQUIRED
            return await client.call_tool("polar_pattern_feature", tool_params)
...
def test_polar_pattern_feature_sends_exact_authenticated_json_rpc_values_to_freecad(monkeypatch):
    ...
    assert envelope["method"] == "polar_pattern_feature"
    assert envelope["session_token"] == "test-session-token"
    assert envelope["operation"] == {"name": 'Polar pattern feature'}
...
def test_public_route_marks_committed_but_invalid_response_non_retryable(monkeypatch):
    ...
    result = _invoke_registered(monkeypatch, transport, dict(_TOOL_PARAMS))
    assert result.isError is True
    assert result.structuredContent["error_code"] == "POLAR_PATTERN_FEATURE_COMMITTED_RESPONSE_INVALID"
