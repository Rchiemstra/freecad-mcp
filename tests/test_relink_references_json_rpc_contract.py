"""Agent-to-FreeCAD wire contract for the ``relink_references`` MCP operation."""
...
from freecad_mcp.generated.capabilities.register_modules import tools_advanced_b
...
class _RecordingFreeCADTransport:
    ...
    def request(self, path, payload, headers):
        ...
        if body_result is _DEFAULT_RESPONSE:
            body_result = dict({"contract_version": 1, "success": True, "ok": True, "outcome": "committed", "committed": True, "retry_safe": False, "from_obj": "Value", "to_obj": "Value"}, **{
                key: envelope["params"][key]
                for key in envelope["params"]
                if key in ['from_obj', 'to_obj']
            })
            body_result = {"contract_version": 1, "success": True, "ok": True, "outcome": "committed", "committed": True, "retry_safe": False, "from_obj": "Value", "to_obj": "Value"}
        response = {
            "jsonrpc": "2.0",
            "id": request["id"],
            "result": {
...
def _invoke_registered(monkeypatch, transport, params):
    ...
    monkeypatch.setattr(tools_advanced_b, "server_connection", lambda: connection)
    monkeypatch.setattr(tools_advanced_b, "server_state", lambda: SimpleNamespace(only_text_feedback=True))
    bind_instrumented_fast_mcp(InstrumentedFastMCP)
    mcp = InstrumentedFastMCP("relink_references-contract")
    exports = {}
    tools_advanced_b._register_relink_references(mcp, dependencies=object(), exports=exports)

    async def invoke():
        async with create_connected_server_and_client_session(mcp._mcp_server) as client:
            listing = await client.list_tools()
            tool = next(tool for tool in listing.tools if tool.name == "relink_references")
            assert "doc_name" in tool.inputSchema.get("required", ["doc_name"])
            return await client.call_tool("relink_references", params)
...
def test_relink_references_sends_authenticated_json_rpc_to_freecad(monkeypatch):
    transport = _RecordingFreeCADTransport()
    result = _invoke_registered(monkeypatch, transport, {"doc_name": "AgentDocument", "from_obj": "Value", "to_obj": "Value"})
    ...
    assert envelope["method"] == "relink_references"
...
def test_mcp_schema_rejects_non_string_doc_name_before_json_rpc(monkeypatch):
    transport = _RecordingFreeCADTransport()
    params = dict({"doc_name": "AgentDocument", "from_obj": "Value", "to_obj": "Value"})
    params["doc_name"] = 42
