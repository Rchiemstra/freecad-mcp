"""Adversarial wire cases for the ``insert_part_from_library`` contract."""
...
def _success():
    return make_insert_part_from_library_success(DocumentName("Doc"), "Parts/Box.FCStd")


@pytest.mark.parametrize("raw", [None, [], 1, "timeout", {}, {1: "bad key"}])
def test_unknown_response_never_proves_rejection(raw):
    result = parse_insert_part_from_library_response(raw)
...
def test_contract_version_must_be_the_exact_integer(version):
    assert parse_insert_part_from_library_response(dict(_success(), contract_version=version))["success"] is False
...
def test_contradictory_or_malformed_success_cannot_escape(change):
    result = parse_insert_part_from_library_response(dict(_success(), **change))
    assert result["success"] is False
    assert result["outcome"] == "uncertain"
    assert result["retry_safe"] is False
...
def test_valid_success_round_trips():
    raw = _success()
    assert parse_insert_part_from_library_response(raw) == raw


def test_valid_failure_round_trips():
    raw = make_insert_part_from_library_failure("BUSY", "Retry", native_status="Busy")
    assert parse_insert_part_from_library_response(raw) == raw


@pytest.mark.parametrize("committed", [None, False, True])
def test_valid_uncertain_round_trips(committed):
    raw = make_insert_part_from_library_uncertain("STATE_UNKNOWN", "Reconcile", committed=committed)
    assert parse_insert_part_from_library_response(raw) == raw
...
def test_transport_failure_preserves_unknown_model_state():
    ...
    client = SimpleNamespace(
        insert_part_from_library=lost_response,
        get_active_screenshot=lambda: None,
    )
    response = insert_part_from_library_operation(client, True, "Doc", "Parts/Box.FCStd")
