"""Unit coverage for the typed ``insert_part_from_library`` mutation slice."""
...
from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops.insert_part_from_library_mutation import (
    _insert_part_from_library_native_result,
...
def test_insert_part_from_library_runs_apply_recompute_validate_then_commits():
    events: list[str] = []
    document = FakeDocument(events)
    collab, _api = collaborators(document, events)

    result = run_insert_part_from_library(collab, "Doc", "Parts/Box.FCStd")

    assert result["success"] is True
    assert result['relative_path'] == 'Parts/Box.FCStd'
...
def test_invalid_arguments_abort_without_commit():
    ...
    result = run_insert_part_from_library(collab, "Doc", "")
...
def test_missing_document_fails_without_entering_apply():
    ...
    if "insert_part_from_library" in {"create_document", "open_document"}:
...
def test_native_capability_is_required_before_apply():
    ...
    if "insert_part_from_library" not in {"create_document", "open_document"}:
...
def test_unknown_or_contradictory_native_evidence_cannot_release_success(native_result):
    result = _insert_part_from_library_native_result(native_result, _NativeMutationState(postcondition_passed=True))
    assert result["success"] is False
    assert result["outcome"] == "uncertain"
    assert result["retry_safe"] is False


def test_insert_part_from_library_has_apply_entry_point():
    assert callable(apply_insert_part_from_library)
