"""Agent-to-FreeCAD wire contract for the ``insert_part_from_library`` MCP operation."""
...
from freecad_mcp.generated.capabilities.register_modules import tools_gui_document_a
...
class _RecordingFreeCADTransport:
    ...
    def request(self, path, payload, headers):
        ...
        if body_result is _DEFAULT_RESPONSE:
            ...
            extra = {"document_name": envelope["params"]["doc_name"], "relative_path": envelope["params"]["relative_path"]}
...
def _invoke_registered(monkeypatch, transport, arguments):
    ...
    monkeypatch.setattr(tools_gui_document_a, "server_connection", lambda: connection)
    if hasattr(tools_gui_document_a, "server_state"):
        monkeypatch.setattr(
    ...
    mcp = InstrumentedFastMCP("insert_part_from_library-contract")
    exports = {}
    tools_gui_document_a._register_insert_part_from_library(mcp, dependencies=object(), exports=exports)

    async def invoke():
        async with create_connected_server_and_client_session(mcp._mcp_server) as client:
            listing = await client.list_tools()
            tool = next(tool for tool in listing.tools if tool.name == "insert_part_from_library")
            assert set(tool.inputSchema["required"]) == {'relative_path', 'doc_name'}
            return await client.call_tool("insert_part_from_library", arguments)
...
def test_insert_part_from_library_sends_exact_authenticated_json_rpc_values_to_freecad(monkeypatch):
    transport = _RecordingFreeCADTransport()
    result = _invoke_registered(monkeypatch, transport, {'doc_name': 'AgentDocument', 'relative_path': 'Parts/Box.FCStd'})
    ...
    assert envelope["method"] == "insert_part_from_library"
    assert envelope["params"] == {'doc_name': 'AgentDocument', 'relative_path': 'Parts/Box.FCStd'}
    assert envelope["operation"] == {"name": 'Insert part from library'}
...
def test_public_route_never_turns_rejection_into_success(monkeypatch):
    arguments = dict({'doc_name': 'AgentDocument', 'relative_path': 'Parts/Box.FCStd'})
    if "relative_path" in arguments and isinstance(arguments["relative_path"], str):
...
def test_public_route_marks_committed_but_invalid_response_non_retryable(monkeypatch):
    ...
    result = _invoke_registered(monkeypatch, transport, {'doc_name': 'AgentDocument', 'relative_path': 'Parts/Box.FCStd'})
    assert result.isError is True
    assert result.structuredContent["error_code"] == 'INSERT_PART_FROM_LIBRARY_COMMITTED_RESPONSE_INVALID'
