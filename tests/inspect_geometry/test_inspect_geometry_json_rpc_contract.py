"""Agent-to-FreeCAD wire contract for the ``inspect_geometry`` MCP operation."""
...
class _RecordingFreeCADTransport:
    ...
    def request(self, path, payload, headers):
        ...
        if body_result is _DEFAULT_RESPONSE:
            from tests.inspect_geometry.test_inspect_geometry_response import _success
...
def _invoke(monkeypatch, transport, arguments):
    ...
    mcp = InstrumentedFastMCP("inspect_geometry-contract")
    exports = {}
    tools_advanced_a._register_inspect_geometry(mcp, dependencies=object(), exports=exports)

    async def invoke():
        async with create_connected_server_and_client_session(mcp._mcp_server) as client:
            listing = await client.list_tools()
            tool = next(tool for tool in listing.tools if tool.name == "inspect_geometry")
            return await client.call_tool("inspect_geometry", arguments)
...
def test_inspect_geometry_sends_authenticated_json_rpc_to_freecad(monkeypatch):
    ...
    assert envelope["method"] == "inspect_geometry"
    assert envelope["params"]["doc_name"] == "AgentDocument"
    assert envelope["params"]["object_name"] == "Box"
