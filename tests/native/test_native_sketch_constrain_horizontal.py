"""Native FreeCAD qualification for ``sketch_constrain_horizontal``."""
...
def test_sketch_constrain_horizontal_native_success_on_idle_sketch():
    _require_native_collaboration()
    import FreeCAD
    import Part
    import Sketcher

    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops.sketch_constrain_horizontal import run_sketch_constrain_horizontal

    document, sketch = _idle_sketch(FreeCAD, Part, "MCPSketchConstrainHorizontalNativeSuccess")
    before = sketch.GeometryCount, sketch.ConstraintCount
    try:
        result = run_sketch_constrain_horizontal(
            _collaborators(FreeCAD, Part, Sketcher, lambda _doc: None),
            document.Name, sketch.Name, 0,
        )
...
def test_sketch_constrain_horizontal_native_validation_failure_restores_geometry_counts():
    _require_native_collaboration()
    import FreeCAD
    import Part
    import Sketcher
    ...
    document, sketch = _idle_sketch(FreeCAD, Part, "MCPSketchConstrainHorizontalNativeValidation")
    ...
    try:
        result = run_sketch_constrain_horizontal(_collaborators(FreeCAD, Part, Sketcher, fail), document.Name, sketch.Name, 0)
...
def test_sketch_constrain_horizontal_native_missing_document_keeps_typed_error():
    _require_native_collaboration()
    import FreeCAD
    import Part
    import Sketcher
...
def test_sketch_constrain_horizontal_native_failure_isolated_and_healthy_document_succeeds():
    _require_native_collaboration()
    import FreeCAD
    import Part
    import Sketcher
    ...
    failed_document, failed_sketch = _idle_sketch(FreeCAD, Part, "MCPSketchConstrainHorizontalIsolatedFailure")
    healthy_document, healthy_sketch = _idle_sketch(FreeCAD, Part, "MCPSketchConstrainHorizontalHealthyDocument")
    ...
    try:
        failed = run_sketch_constrain_horizontal(collab, failed_document.Name, failed_sketch.Name, 0)
        isolated = run_sketch_constrain_horizontal(collab, healthy_document.Name, healthy_sketch.Name, 0)
...
def test_sketch_constrain_horizontal_native_postcondition_cannot_write():
    _require_native_collaboration()
    import FreeCAD
    import Part
    import Sketcher
    ...
    document, sketch = _idle_sketch(FreeCAD, Part, "MCPSketchConstrainHorizontalReadOnlyPostcondition")
