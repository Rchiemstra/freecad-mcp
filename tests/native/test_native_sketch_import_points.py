"""Native FreeCAD qualification for ``sketch_import_points``."""
...
def test_sketch_import_points_native_success_on_idle_sketch():
    _require_native_collaboration()
    import FreeCAD
    import Part
    import Sketcher

    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops.sketch_import_points import run_sketch_import_points

    document, sketch = _idle_sketch(FreeCAD, Part, "MCPSketchImportPointsNativeSuccess")
    before = sketch.GeometryCount, sketch.ConstraintCount
    try:
        result = run_sketch_import_points(
            _collaborators(FreeCAD, Part, Sketcher, lambda _doc: None),
            document.Name, sketch.Name, [{'x': 1.0, 'y': 2.0}, {'x': 3.0, 'y': 4.0}], False,
        )
...
def test_sketch_import_points_native_validation_failure_restores_geometry_counts():
    _require_native_collaboration()
    import FreeCAD
    import Part
    import Sketcher
    ...
    document, sketch = _idle_sketch(FreeCAD, Part, "MCPSketchImportPointsNativeValidation")
    ...
    try:
        result = run_sketch_import_points(_collaborators(FreeCAD, Part, Sketcher, fail), document.Name, sketch.Name, [{'x': 1.0, 'y': 2.0}, {'x': 3.0, 'y': 4.0}], False)
...
def test_sketch_import_points_native_missing_document_keeps_typed_error():
    _require_native_collaboration()
    import FreeCAD
    import Part
    import Sketcher
    ...
    result = run_sketch_import_points(
        ...
        "Sketch",
        [{'x': 1.0, 'y': 2.0}, {'x': 3.0, 'y': 4.0}], False,
    )
    assert result["error_code"] == "DOCUMENT_NOT_FOUND"
    assert result["outcome"] == "rejected"


def test_sketch_import_points_native_failure_isolated_and_healthy_document_succeeds():
    _require_native_collaboration()
    import FreeCAD
    import Part
    import Sketcher
    ...
    failed_document, failed_sketch = _idle_sketch(FreeCAD, Part, "MCPSketchImportPointsIsolatedFailure")
    healthy_document, healthy_sketch = _idle_sketch(FreeCAD, Part, "MCPSketchImportPointsHealthyDocument")
    ...
    try:
        failed = run_sketch_import_points(collab, failed_document.Name, failed_sketch.Name, [{'x': 1.0, 'y': 2.0}, {'x': 3.0, 'y': 4.0}], False)
        isolated = run_sketch_import_points(collab, healthy_document.Name, healthy_sketch.Name, [{'x': 1.0, 'y': 2.0}, {'x': 3.0, 'y': 4.0}], False)
...
def test_sketch_import_points_native_postcondition_cannot_write():
    _require_native_collaboration()
    import FreeCAD
    import Part
    import Sketcher
    ...
    document, sketch = _idle_sketch(FreeCAD, Part, "MCPSketchImportPointsReadOnlyPostcondition")
