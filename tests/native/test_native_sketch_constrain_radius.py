"""Native FreeCAD qualification for ``sketch_constrain_radius``."""
...
def _idle_sketch(FreeCAD, Part, name: str):
    ...
    sketch.addGeometry(
        Part.Circle(FreeCAD.Vector(0, 0, 0), FreeCAD.Vector(0, 0, 1), 5), False
    )
    sketch.addGeometry(
        Part.Circle(FreeCAD.Vector(20, 0, 0), FreeCAD.Vector(0, 0, 1), 5), False
    )
    document.recompute()
    return document, sketch


def test_sketch_constrain_radius_native_success_on_idle_sketch():
    _require_native_collaboration()
    import FreeCAD
    import Part
    import Sketcher

    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops.sketch_constrain_radius import run_sketch_constrain_radius

    document, sketch = _idle_sketch(FreeCAD, Part, "MCPSketchConstrainRadiusNativeSuccess")
    before = sketch.GeometryCount, sketch.ConstraintCount
    try:
        result = run_sketch_constrain_radius(
            _collaborators(FreeCAD, Part, Sketcher, lambda _doc: None),
            document.Name, sketch.Name, 0, 5.0, None,
        )
...
def test_sketch_constrain_radius_native_validation_failure_restores_geometry_counts():
    _require_native_collaboration()
    import FreeCAD
    import Part
    import Sketcher
    ...
    document, sketch = _idle_sketch(FreeCAD, Part, "MCPSketchConstrainRadiusNativeValidation")
    ...
    try:
        result = run_sketch_constrain_radius(_collaborators(FreeCAD, Part, Sketcher, fail), document.Name, sketch.Name, 0, 5.0, None)
...
def test_sketch_constrain_radius_native_missing_document_keeps_typed_error():
    _require_native_collaboration()
    import FreeCAD
    import Part
    import Sketcher
...
def test_sketch_constrain_radius_native_failure_isolated_and_healthy_document_succeeds():
    _require_native_collaboration()
    import FreeCAD
    import Part
    import Sketcher
    ...
    failed_document, failed_sketch = _idle_sketch(FreeCAD, Part, "MCPSketchConstrainRadiusIsolatedFailure")
    healthy_document, healthy_sketch = _idle_sketch(FreeCAD, Part, "MCPSketchConstrainRadiusHealthyDocument")
    ...
    try:
        failed = run_sketch_constrain_radius(collab, failed_document.Name, failed_sketch.Name, 0, 5.0, None)
        isolated = run_sketch_constrain_radius(collab, healthy_document.Name, healthy_sketch.Name, 0, 5.0, None)
...
def test_sketch_constrain_radius_native_postcondition_cannot_write():
    _require_native_collaboration()
    import FreeCAD
    import Part
    import Sketcher
    ...
    document, sketch = _idle_sketch(FreeCAD, Part, "MCPSketchConstrainRadiusReadOnlyPostcondition")
