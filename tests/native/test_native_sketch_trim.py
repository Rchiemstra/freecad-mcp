"""Native FreeCAD qualification for ``sketch_trim``."""
...
def test_sketch_trim_native_success_on_idle_sketch():
    _require_native_collaboration()
    import FreeCAD
    import Part
    import Sketcher

    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops.sketch_trim import run_sketch_trim

    document, sketch = _idle_sketch(FreeCAD, Part, "MCPSketchTrimNativeSuccess")
    before = sketch.GeometryCount, sketch.ConstraintCount
    try:
        result = run_sketch_trim(
...
def test_sketch_trim_native_validation_failure_restores_geometry_counts():
    _require_native_collaboration()
    import FreeCAD
    import Part
    import Sketcher
    ...
    document, sketch = _idle_sketch(FreeCAD, Part, "MCPSketchTrimNativeValidation")
    ...
    try:
        result = run_sketch_trim(_collaborators(FreeCAD, Part, Sketcher, fail), document.Name, sketch.Name, 0, 5.0, 0.0)
...
def test_sketch_trim_native_missing_document_keeps_typed_error():
    _require_native_collaboration()
    import FreeCAD
    import Part
    import Sketcher
...
def test_sketch_trim_native_failure_isolated_and_healthy_document_succeeds():
    _require_native_collaboration()
    import FreeCAD
    import Part
    import Sketcher
    ...
    failed_document, failed_sketch = _idle_sketch(FreeCAD, Part, "MCPSketchTrimIsolatedFailure")
    healthy_document, healthy_sketch = _idle_sketch(FreeCAD, Part, "MCPSketchTrimHealthyDocument")
    ...
    try:
        failed = run_sketch_trim(collab, failed_document.Name, failed_sketch.Name, 0, 5.0, 0.0)
        isolated = run_sketch_trim(collab, healthy_document.Name, healthy_sketch.Name, 0, 5.0, 0.0)
...
def test_sketch_trim_native_postcondition_cannot_write():
    _require_native_collaboration()
    import FreeCAD
    import Part
    import Sketcher
    ...
    document, sketch = _idle_sketch(FreeCAD, Part, "MCPSketchTrimReadOnlyPostcondition")
