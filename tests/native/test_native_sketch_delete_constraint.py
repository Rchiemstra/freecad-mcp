...
"""Native collaboration coverage for the typed ``sketch_delete_constraint`` mutation."""
...
def _revision_state(document):
    ...
    session = document.beginEditSession("sketch_delete_constraint-native-state-probe")
    try:
...
def test_sketch_delete_constraint_native_success_inspects_after_recompute(monkeypatch):
    _require_native_collaboration()
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops import sketch_delete_constraint as body_create_module

    document = FreeCAD.newDocument("MCPSketchDeleteConstraintNativePhaseOrder")
    events = []
    ...
    _idle(document, 'delete_constraint')
    events.clear()
    original_apply = body_create_module.apply_sketch_delete_constraint
    original_read = body_create_module.read_sketch_delete_constraint_result
    ...
    monkeypatch.setattr(body_create_module, "apply_sketch_delete_constraint", tracked_apply)
    monkeypatch.setattr(body_create_module, "read_sketch_delete_constraint_result", tracked_read)
    ...
    try:
        ...
        result = body_create_module.run_sketch_delete_constraint(collaborators, document.Name, "Sketch", [], ["Extra"])
        ...
        assert created.ConstraintCount == before - 1
    finally:
        FreeCAD.closeDocument(document.Name)


def test_sketch_delete_constraint_native_validation_failure_restores_complete_state():
    _require_native_collaboration()
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops.sketch_delete_constraint import run_sketch_delete_constraint

    document = FreeCAD.newDocument("MCPSketchDeleteConstraintNativeRollback")
    _idle(document, 'delete_constraint')
    ...
    try:
        result = run_sketch_delete_constraint(collaborators, document.Name, "Sketch", [], ["Extra"])
...
def test_sketch_delete_constraint_native_apply_failure_restores_all_attempted_effects(monkeypatch):
    _require_native_collaboration()
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops import sketch_delete_constraint as body_create_module

    document = FreeCAD.newDocument("MCPSketchDeleteConstraintNativeApplyFailure")
    _idle(document, 'delete_constraint')
    state_before = _model_state(document)
    revision_before = _revision_state(document)
    original_apply = body_create_module.apply_sketch_delete_constraint
    ...
    monkeypatch.setattr(body_create_module, "apply_sketch_delete_constraint", mutates_then_raises)
...
def test_sketch_delete_constraint_native_recompute_failure_rolls_back(monkeypatch):
    _require_native_collaboration()
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops import sketch_delete_constraint as body_create_module

    document = FreeCAD.newDocument("MCPSketchDeleteConstraintNativeRecomputeFailure")
    ...
    monkeypatch.setattr(body_create_module, "apply_sketch_delete_constraint", arm_recompute_failure)
    collaborators = _collaborators(FreeCAD, lambda _document: None)
    try:
        result = body_create_module.run_sketch_delete_constraint(collaborators, document.Name, "Sketch", [], ["Extra"])
        assert result["success"] is False
...
def test_sketch_delete_constraint_native_rollback_failure_is_uncertain_and_fences_document(monkeypatch):
    _require_native_collaboration()
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops import sketch_delete_constraint as body_create_module

    document = FreeCAD.newDocument("MCPSketchDeleteConstraintNativeRollbackFailure")
    ...
    monkeypatch.setattr(body_create_module, "apply_sketch_delete_constraint", arm_persistent_failure)
    collaborators = _collaborators(FreeCAD, lambda _document: None)
    try:
        result = body_create_module.run_sketch_delete_constraint(collaborators, document.Name, "Sketch", [], ["Extra"])
        proxy.armed = False
        fenced = body_create_module.run_sketch_delete_constraint(collaborators, document.Name, "Sketch", [], ["Extra"])
        assert result["success"] is False
...
def test_sketch_delete_constraint_native_inspection_failure_rolls_back_after_recompute(monkeypatch):
    _require_native_collaboration()
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops import sketch_delete_constraint as body_create_module

    document = FreeCAD.newDocument("MCPSketchDeleteConstraintNativeInspectionFailure")
    _idle(document, 'delete_constraint')
    state_before = _model_state(document)
    revision_before = _revision_state(document)

    def reject_inspection(*_args, **_kwargs):
        raise body_create_module.SketchDeleteConstraintError(
            "CREATED_OBJECT_WRONG_TYPE", "forced post-recompute inspection failure"
        )

    monkeypatch.setattr(body_create_module, "read_sketch_delete_constraint_result", reject_inspection)
    collaborators = _collaborators(FreeCAD, lambda _document: None)
    try:
        result = body_create_module.run_sketch_delete_constraint(collaborators, document.Name, "Sketch", [], ["Extra"])
        assert result["success"] is False
...
def test_sketch_delete_constraint_native_failure_isolated_and_healthy_rollback_recovers():
    _require_native_collaboration()
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops.sketch_delete_constraint import run_sketch_delete_constraint

    failed_document = FreeCAD.newDocument("MCPSketchDeleteConstraintIsolatedFailure")
    healthy_document = FreeCAD.newDocument("MCPSketchDeleteConstraintHealthyDocument")
    _idle(failed_document, 'delete_constraint')
    _idle(healthy_document, 'delete_constraint')
    ...
    try:
        failed = run_sketch_delete_constraint(collaborators, failed_document.Name, "Sketch", [], ["Extra"])
        isolated = run_sketch_delete_constraint(collaborators, healthy_document.Name, "Sketch", [], ["Extra"])
        rejected_names.clear()
        recovered = run_sketch_delete_constraint(collaborators, failed_document.Name, "Sketch", [], ["Extra"])
...
def test_sketch_delete_constraint_native_duplicate_and_missing_document_keep_typed_errors():
    _require_native_collaboration()
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops.sketch_delete_constraint import run_sketch_delete_constraint

    document = FreeCAD.newDocument("MCPSketchDeleteConstraintTypedErrors")
    _idle(document, 'delete_constraint')
    collaborators = _collaborators(FreeCAD, lambda _: None)
    try:
        first = run_sketch_delete_constraint(collaborators, document.Name, "Sketch", [], ["Extra"])
        assert first["success"] is True
        before = _model_state(document), _revision_state(document)
        result = run_sketch_delete_constraint(collaborators, document.Name, "MissingSketch", [], ["Extra"])
        assert result["error_code"] == 'SKETCH_NOT_FOUND'
        assert result["retry_safe"] is True
        assert (_model_state(document), _revision_state(document)) == before
        missing = run_sketch_delete_constraint(collaborators, "NoSuchDeleteConstraintQualificationDocument", "Sketch", [], ["Extra"])
...
def test_sketch_delete_constraint_native_rollback_restores_rich_model(monkeypatch, stage):
    _require_native_collaboration()
    import FreeCAD
    import Part
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops import sketch_delete_constraint as subject

    document = FreeCAD.newDocument("MCPSketchDeleteConstraintRichRollback")
    ...
    _idle(document, 'delete_constraint')
    before = _model_state(document), _revision_state(document)
    original_apply = subject.apply_sketch_delete_constraint
    original_inspect = subject.read_sketch_delete_constraint_result
    ...
    def inspect(*args, **kwargs):
        if stage == "inspection":
            raise subject.SketchDeleteConstraintError("INSPECTION_FAILED", "injected inspection failure")
    ...
    monkeypatch.setattr(subject, "apply_sketch_delete_constraint", change_model)
    monkeypatch.setattr(subject, "read_sketch_delete_constraint_result", inspect)
    try:
        result = subject.run_sketch_delete_constraint(_collaborators(FreeCAD, validate), document.Name, "Sketch", [], ["Extra"])
...
def test_sketch_delete_constraint_native_postcondition_cannot_write(monkeypatch, write):
    _require_native_collaboration()
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops import sketch_delete_constraint as subject

    document = FreeCAD.newDocument("MCPSketchDeleteConstraintReadOnlyPostcondition")
