"""Native FreeCAD qualification for ``sketch_extend``."""
...
def test_sketch_extend_native_success_on_idle_sketch():
    _require_native_collaboration()
    import FreeCAD
    import Part
    import Sketcher

    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops.sketch_extend import run_sketch_extend

    document, sketch = _idle_sketch(FreeCAD, Part, "MCPSketchExtendNativeSuccess")
    before = sketch.GeometryCount, sketch.ConstraintCount
    try:
        result = run_sketch_extend(
            _collaborators(FreeCAD, Part, Sketcher, lambda _doc: None),
            document.Name, sketch.Name, 0, 2.0, 2,
        )
...
def test_sketch_extend_native_validation_failure_restores_geometry_counts():
    _require_native_collaboration()
    import FreeCAD
    import Part
    import Sketcher
    ...
    document, sketch = _idle_sketch(FreeCAD, Part, "MCPSketchExtendNativeValidation")
    ...
    try:
        result = run_sketch_extend(_collaborators(FreeCAD, Part, Sketcher, fail), document.Name, sketch.Name, 0, 2.0, 2)
...
def test_sketch_extend_native_missing_document_keeps_typed_error():
    _require_native_collaboration()
    import FreeCAD
    import Part
    import Sketcher
...
def test_sketch_extend_native_failure_isolated_and_healthy_document_succeeds():
    _require_native_collaboration()
    import FreeCAD
    import Part
    import Sketcher
    ...
    failed_document, failed_sketch = _idle_sketch(FreeCAD, Part, "MCPSketchExtendIsolatedFailure")
    healthy_document, healthy_sketch = _idle_sketch(FreeCAD, Part, "MCPSketchExtendHealthyDocument")
    ...
    try:
        failed = run_sketch_extend(collab, failed_document.Name, failed_sketch.Name, 0, 2.0, 2)
        isolated = run_sketch_extend(collab, healthy_document.Name, healthy_sketch.Name, 0, 2.0, 2)
...
def test_sketch_extend_native_postcondition_cannot_write():
    _require_native_collaboration()
    import FreeCAD
    import Part
    import Sketcher
    ...
    document, sketch = _idle_sketch(FreeCAD, Part, "MCPSketchExtendReadOnlyPostcondition")
