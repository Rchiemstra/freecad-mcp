"""Native FreeCAD qualification for ``sketch_constrain_distance``."""
...
def test_sketch_constrain_distance_native_success_on_idle_sketch():
    _require_native_collaboration()
    import FreeCAD
    import Part
    import Sketcher

    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops.sketch_constrain_distance import run_sketch_constrain_distance

    document, sketch = _idle_sketch(FreeCAD, Part, "MCPSketchConstrainDistanceNativeSuccess")
    before = sketch.GeometryCount, sketch.ConstraintCount
    try:
        result = run_sketch_constrain_distance(
            _collaborators(FreeCAD, Part, Sketcher, lambda _doc: None),
            document.Name, sketch.Name, 0, 10.0, None, None,
        )
...
def test_sketch_constrain_distance_native_validation_failure_restores_geometry_counts():
    _require_native_collaboration()
    import FreeCAD
    import Part
    import Sketcher
    ...
    document, sketch = _idle_sketch(FreeCAD, Part, "MCPSketchConstrainDistanceNativeValidation")
    ...
    try:
        result = run_sketch_constrain_distance(_collaborators(FreeCAD, Part, Sketcher, fail), document.Name, sketch.Name, 0, 10.0, None, None)
...
def test_sketch_constrain_distance_native_missing_document_keeps_typed_error():
    _require_native_collaboration()
    import FreeCAD
    import Part
    import Sketcher
...
def test_sketch_constrain_distance_native_failure_isolated_and_healthy_document_succeeds():
    _require_native_collaboration()
    import FreeCAD
    import Part
    import Sketcher
    ...
    failed_document, failed_sketch = _idle_sketch(FreeCAD, Part, "MCPSketchConstrainDistanceIsolatedFailure")
    healthy_document, healthy_sketch = _idle_sketch(FreeCAD, Part, "MCPSketchConstrainDistanceHealthyDocument")
    ...
    try:
        failed = run_sketch_constrain_distance(collab, failed_document.Name, failed_sketch.Name, 0, 10.0, None, None)
        isolated = run_sketch_constrain_distance(collab, healthy_document.Name, healthy_sketch.Name, 0, 10.0, None, None)
...
def test_sketch_constrain_distance_native_postcondition_cannot_write():
    _require_native_collaboration()
    import FreeCAD
    import Part
    import Sketcher
    ...
    document, sketch = _idle_sketch(FreeCAD, Part, "MCPSketchConstrainDistanceReadOnlyPostcondition")
