"""Native qualification matrix for ``sweep_pipe``."""
...
def test_sweep_pipe_native_success_inspects_after_recompute(monkeypatch):
    _require_native_collaboration()
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops import sweep_pipe as subject

    document = FreeCAD.newDocument("MCPSweepPipeNativeSuccess")
    events = []
    ...
    _prepare(document)
    events.clear()
    original_apply = subject.apply_sweep_pipe
    original_read = subject.read_sweep_pipe_result
    ...
    monkeypatch.setattr(subject, "apply_sweep_pipe", tracked_apply)
    monkeypatch.setattr(subject, "read_sweep_pipe_result", tracked_read)
    try:
        result = subject.run_sweep_pipe(_collaborators(FreeCAD, lambda _d: events.append("validate")), document.Name, "Seed", 1.5, "Created", "frenet", None, None, "error")
        assert result["success"] is True
        created = document.getObject("Created")
        assert created is not None
        assert "Part::Feature" in created.TypeId
        shape = created.Shape
        assert shape is not None and (shape.Volume > 0 or len(shape.Faces) > 0)
        assert events == ["apply", "recompute", "inspect", "validate"]
    finally:
        FreeCAD.closeDocument(document.Name)


def test_sweep_pipe_native_validation_failure_restores_complete_state():
    _require_native_collaboration()
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops.sweep_pipe import run_sweep_pipe

    document = FreeCAD.newDocument("MCPSweepPipeNativeRollback")
    _prepare(document)
    state_before = _model_state(document)
    try:
        result = run_sweep_pipe(
            _collaborators(FreeCAD, lambda _d: (_ for _ in ()).throw(RuntimeError("forced validation failure"))),
            document.Name, "Seed", 1.5, "Created", "frenet", None, None, "error",
        )
...
def test_sweep_pipe_native_apply_failure_restores(monkeypatch):
    _require_native_collaboration()
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops import sweep_pipe as subject

    document = FreeCAD.newDocument("MCPSweepPipeNativeApplyFailure")
    _prepare(document)
    state_before = _model_state(document)
    original = subject.apply_sweep_pipe
    ...
    monkeypatch.setattr(subject, "apply_sweep_pipe", mutates_then_raises)
    try:
        result = subject.run_sweep_pipe(_collaborators(FreeCAD, lambda _d: None), document.Name, "Seed", 1.5, "Created", "frenet", None, None, "error")
        assert result["success"] is False
...
def test_sweep_pipe_native_recompute_failure_rolls_back(monkeypatch):
    _require_native_collaboration()
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops import sweep_pipe as subject

    document = FreeCAD.newDocument("MCPSweepPipeNativeRecomputeFailure")
    ...
    monkeypatch.setattr(subject, "apply_sweep_pipe", arm)
    try:
        result = subject.run_sweep_pipe(_collaborators(FreeCAD, lambda _d: None), document.Name, "Seed", 1.5, "Created", "frenet", None, None, "error")
        assert result["success"] is False
...
def test_sweep_pipe_native_rollback_failure_is_uncertain_and_fences(monkeypatch):
    _require_native_collaboration()
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops import sweep_pipe as subject

    document = FreeCAD.newDocument("MCPSweepPipeNativeRollbackFailure")
    ...
    try:
        result = subject.run_sweep_pipe(collaborators, document.Name, "Seed", 1.5, "Created", "frenet", None, None, "error")
        proxy.armed = False
        fenced = subject.run_sweep_pipe(collaborators, document.Name, "Seed", 1.5, "Created", "frenet", None, None, "error")
...
def test_sweep_pipe_native_inspection_failure_rolls_back(monkeypatch):
    _require_native_collaboration()
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops import sweep_pipe as subject

    document = FreeCAD.newDocument("MCPSweepPipeNativeInspectionFailure")
    _prepare(document)
    state_before = _model_state(document)

    def reject(_document, _receipt):
        raise subject.SweepPipeError("CREATED_OBJECT_WRONG_TYPE", "forced inspection failure")

    monkeypatch.setattr(subject, "read_sweep_pipe_result", reject)
    try:
        result = subject.run_sweep_pipe(_collaborators(FreeCAD, lambda _d: None), document.Name, "Seed", 1.5, "Created", "frenet", None, None, "error")
        assert result["success"] is False
...
def test_sweep_pipe_native_postcondition_cannot_write():
    _require_native_collaboration()
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops.sweep_pipe import run_sweep_pipe

    document = FreeCAD.newDocument("MCPSweepPipeReadOnlyPostcondition")
    ...
    try:
        result = run_sweep_pipe(_collaborators(FreeCAD, validate), document.Name, "Seed", 1.5, "Created", "frenet", None, None, "error")
