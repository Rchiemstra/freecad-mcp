"""Native FreeCAD qualification for ``sketch_add_arc``."""
...
def test_sketch_add_arc_native_success_on_idle_sketch():
    _require_native_collaboration()
    import FreeCAD
    import Part
    import Sketcher

    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops.sketch_add_arc import run_sketch_add_arc

    document, sketch = _idle_sketch(FreeCAD, Part, "MCPSketchAddArcNativeSuccess")
    before = sketch.GeometryCount, sketch.ConstraintCount
    try:
        result = run_sketch_add_arc(
            _collaborators(FreeCAD, Part, Sketcher, lambda _doc: None),
            document.Name, sketch.Name, 0.0, 0.0, 5.0, 0.0, 90.0, False,
        )
...
def test_sketch_add_arc_native_validation_failure_restores_geometry_counts():
    _require_native_collaboration()
    import FreeCAD
    import Part
    import Sketcher
    ...
    document, sketch = _idle_sketch(FreeCAD, Part, "MCPSketchAddArcNativeValidation")
    ...
    try:
        result = run_sketch_add_arc(_collaborators(FreeCAD, Part, Sketcher, fail), document.Name, sketch.Name, 0.0, 0.0, 5.0, 0.0, 90.0, False)
...
def test_sketch_add_arc_native_missing_document_keeps_typed_error():
    _require_native_collaboration()
    import FreeCAD
    import Part
    import Sketcher
    ...
    result = run_sketch_add_arc(
        ...
        "Sketch",
        0.0, 0.0, 5.0, 0.0, 90.0, False,
    )
    assert result["error_code"] == "DOCUMENT_NOT_FOUND"
    assert result["outcome"] == "rejected"


def test_sketch_add_arc_native_failure_isolated_and_healthy_document_succeeds():
    _require_native_collaboration()
    import FreeCAD
    import Part
    import Sketcher
    ...
    failed_document, failed_sketch = _idle_sketch(FreeCAD, Part, "MCPSketchAddArcIsolatedFailure")
    healthy_document, healthy_sketch = _idle_sketch(FreeCAD, Part, "MCPSketchAddArcHealthyDocument")
    ...
    try:
        failed = run_sketch_add_arc(collab, failed_document.Name, failed_sketch.Name, 0.0, 0.0, 5.0, 0.0, 90.0, False)
        isolated = run_sketch_add_arc(collab, healthy_document.Name, healthy_sketch.Name, 0.0, 0.0, 5.0, 0.0, 90.0, False)
...
def test_sketch_add_arc_native_postcondition_cannot_write():
    _require_native_collaboration()
    import FreeCAD
    import Part
    import Sketcher
    ...
    document, sketch = _idle_sketch(FreeCAD, Part, "MCPSketchAddArcReadOnlyPostcondition")
