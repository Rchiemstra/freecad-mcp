"""Native FreeCAD qualification for ``sketch_constrain_tangent``."""
...
def test_sketch_constrain_tangent_native_success_on_idle_sketch():
    _require_native_collaboration()
    import FreeCAD
    import Part
    import Sketcher

    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops.sketch_constrain_tangent import run_sketch_constrain_tangent

    document, sketch = _idle_sketch(FreeCAD, Part, "MCPSketchConstrainTangentNativeSuccess")
    before = sketch.GeometryCount, sketch.ConstraintCount
    try:
        result = run_sketch_constrain_tangent(
...
def test_sketch_constrain_tangent_native_validation_failure_restores_geometry_counts():
    _require_native_collaboration()
    import FreeCAD
    import Part
    import Sketcher
    ...
    document, sketch = _idle_sketch(FreeCAD, Part, "MCPSketchConstrainTangentNativeValidation")
    ...
    try:
        result = run_sketch_constrain_tangent(_collaborators(FreeCAD, Part, Sketcher, fail), document.Name, sketch.Name, 0, 1)
...
def test_sketch_constrain_tangent_native_missing_document_keeps_typed_error():
    _require_native_collaboration()
    import FreeCAD
    import Part
    import Sketcher
...
def test_sketch_constrain_tangent_native_failure_isolated_and_healthy_document_succeeds():
    _require_native_collaboration()
    import FreeCAD
    import Part
    import Sketcher
    ...
    failed_document, failed_sketch = _idle_sketch(FreeCAD, Part, "MCPSketchConstrainTangentIsolatedFailure")
    healthy_document, healthy_sketch = _idle_sketch(FreeCAD, Part, "MCPSketchConstrainTangentHealthyDocument")
    ...
    try:
        failed = run_sketch_constrain_tangent(collab, failed_document.Name, failed_sketch.Name, 0, 1)
        isolated = run_sketch_constrain_tangent(collab, healthy_document.Name, healthy_sketch.Name, 0, 1)
...
def test_sketch_constrain_tangent_native_postcondition_cannot_write():
    _require_native_collaboration()
    import FreeCAD
    import Part
    import Sketcher
    ...
    document, sketch = _idle_sketch(FreeCAD, Part, "MCPSketchConstrainTangentReadOnlyPostcondition")
