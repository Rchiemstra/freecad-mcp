"""Native FreeCAD qualification for ``sketch_symmetry``."""
...
def test_sketch_symmetry_native_success_on_idle_sketch():
    _require_native_collaboration()
    import FreeCAD
    import Part
    import Sketcher

    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops.sketch_symmetry import run_sketch_symmetry

    document, sketch = _idle_sketch(FreeCAD, Part, "MCPSketchSymmetryNativeSuccess")
    before = sketch.GeometryCount, sketch.ConstraintCount
    try:
        result = run_sketch_symmetry(
            _collaborators(FreeCAD, Part, Sketcher, lambda _doc: None),
            document.Name, sketch.Name, [0], 1, True,
        )
...
def test_sketch_symmetry_native_validation_failure_restores_geometry_counts():
    _require_native_collaboration()
    import FreeCAD
    import Part
    import Sketcher
    ...
    document, sketch = _idle_sketch(FreeCAD, Part, "MCPSketchSymmetryNativeValidation")
    ...
    try:
        result = run_sketch_symmetry(_collaborators(FreeCAD, Part, Sketcher, fail), document.Name, sketch.Name, [0], 1, True)
...
def test_sketch_symmetry_native_missing_document_keeps_typed_error():
    _require_native_collaboration()
    import FreeCAD
    import Part
    import Sketcher
...
def test_sketch_symmetry_native_failure_isolated_and_healthy_document_succeeds():
    _require_native_collaboration()
    import FreeCAD
    import Part
    import Sketcher
    ...
    failed_document, failed_sketch = _idle_sketch(FreeCAD, Part, "MCPSketchSymmetryIsolatedFailure")
    healthy_document, healthy_sketch = _idle_sketch(FreeCAD, Part, "MCPSketchSymmetryHealthyDocument")
    ...
    try:
        failed = run_sketch_symmetry(collab, failed_document.Name, failed_sketch.Name, [0], 1, True)
        isolated = run_sketch_symmetry(collab, healthy_document.Name, healthy_sketch.Name, [0], 1, True)
...
def test_sketch_symmetry_native_postcondition_cannot_write():
    _require_native_collaboration()
    import FreeCAD
    import Part
    import Sketcher
    ...
    document, sketch = _idle_sketch(FreeCAD, Part, "MCPSketchSymmetryReadOnlyPostcondition")
