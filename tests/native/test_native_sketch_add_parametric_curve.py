"""Native FreeCAD qualification for ``sketch_add_parametric_curve``."""
...
def test_sketch_add_parametric_curve_native_success_on_idle_sketch():
    _require_native_collaboration()
    import FreeCAD
    import Part
    import Sketcher

    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops.sketch_add_parametric_curve import run_sketch_add_parametric_curve

    document, sketch = _idle_sketch(FreeCAD, Part, "MCPSketchAddParametricCurveNativeSuccess")
    before = sketch.GeometryCount, sketch.ConstraintCount
    try:
        result = run_sketch_add_parametric_curve(
            _collaborators(FreeCAD, Part, Sketcher, lambda _doc: None),
            document.Name, sketch.Name, 't', '0', 0.0, 10.0, 20, False,
        )
...
def test_sketch_add_parametric_curve_native_validation_failure_restores_geometry_counts():
    _require_native_collaboration()
    import FreeCAD
    import Part
    import Sketcher
    ...
    document, sketch = _idle_sketch(FreeCAD, Part, "MCPSketchAddParametricCurveNativeValidation")
    ...
    try:
        result = run_sketch_add_parametric_curve(_collaborators(FreeCAD, Part, Sketcher, fail), document.Name, sketch.Name, 't', '0', 0.0, 10.0, 20, False)
...
def test_sketch_add_parametric_curve_native_missing_document_keeps_typed_error():
    _require_native_collaboration()
    import FreeCAD
    import Part
    import Sketcher
    ...
    result = run_sketch_add_parametric_curve(
        ...
        "Sketch",
        't', '0', 0.0, 10.0, 20, False,
    )
    assert result["error_code"] == "DOCUMENT_NOT_FOUND"
    assert result["outcome"] == "rejected"


def test_sketch_add_parametric_curve_native_failure_isolated_and_healthy_document_succeeds():
    _require_native_collaboration()
    import FreeCAD
    import Part
    import Sketcher
    ...
    failed_document, failed_sketch = _idle_sketch(FreeCAD, Part, "MCPSketchAddParametricCurveIsolatedFailure")
    healthy_document, healthy_sketch = _idle_sketch(FreeCAD, Part, "MCPSketchAddParametricCurveHealthyDocument")
    ...
    try:
        failed = run_sketch_add_parametric_curve(collab, failed_document.Name, failed_sketch.Name, 't', '0', 0.0, 10.0, 20, False)
        isolated = run_sketch_add_parametric_curve(collab, healthy_document.Name, healthy_sketch.Name, 't', '0', 0.0, 10.0, 20, False)
...
def test_sketch_add_parametric_curve_native_postcondition_cannot_write():
    _require_native_collaboration()
    import FreeCAD
    import Part
    import Sketcher
    ...
    document, sketch = _idle_sketch(FreeCAD, Part, "MCPSketchAddParametricCurveReadOnlyPostcondition")
