"""Native qualification matrix for ``set_expression``."""
...
def test_set_expression_native_success_inspects_after_recompute(monkeypatch):
    _require_native_collaboration()
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops import set_expression as subject

    document = FreeCAD.newDocument("MCPSetExpressionNativeSuccess")
    events = []
    ...
    monkeypatch.setattr(subject, "apply_set_expression", tracked_apply)
    monkeypatch.setattr(subject, "read_set_expression_result", tracked_read)
    try:
        result = subject.run_set_expression(_collaborators(FreeCAD, lambda _d: events.append("validate")), document.Name, "Seed", "Length", "1+1")
...
def test_set_expression_native_validation_failure_restores_complete_state():
    _require_native_collaboration()
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops.set_expression import run_set_expression

    document = FreeCAD.newDocument("MCPSetExpressionNativeRollback")
    _prepare(document)
    state_before = _model_state(document)
    try:
        result = run_set_expression(
            _collaborators(FreeCAD, lambda _d: (_ for _ in ()).throw(RuntimeError("forced validation failure"))),
            document.Name, "Seed", "Length", "1+1",
        )
...
def test_set_expression_native_apply_failure_restores(monkeypatch):
    _require_native_collaboration()
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops import set_expression as subject

    document = FreeCAD.newDocument("MCPSetExpressionNativeApplyFailure")
    _prepare(document)
    state_before = _model_state(document)
    ...
    monkeypatch.setattr(subject, "apply_set_expression", mutates_then_raises)
    try:
        result = subject.run_set_expression(_collaborators(FreeCAD, lambda _d: None), document.Name, "Seed", "Length", "1+1")
        assert result["success"] is False
...
def test_set_expression_native_recompute_failure_rolls_back(monkeypatch):
    _require_native_collaboration()
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops import set_expression as subject

    document = FreeCAD.newDocument("MCPSetExpressionNativeRecomputeFailure")
    ...
    monkeypatch.setattr(subject, "apply_set_expression", arm)
    try:
        result = subject.run_set_expression(_collaborators(FreeCAD, lambda _d: None), document.Name, "Seed", "Length", "1+1")
        assert result["success"] is False
...
def test_set_expression_native_rollback_failure_is_uncertain_and_fences(monkeypatch):
    _require_native_collaboration()
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops import set_expression as subject

    document = FreeCAD.newDocument("MCPSetExpressionNativeRollbackFailure")
    ...
    try:
        result = subject.run_set_expression(collaborators, document.Name, "Seed", "Length", "1+1")
        proxy.armed = False
        fenced = subject.run_set_expression(collaborators, document.Name, "Seed", "Length", "1+1")
...
def test_set_expression_native_inspection_failure_rolls_back(monkeypatch):
    _require_native_collaboration()
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops import set_expression as subject

    document = FreeCAD.newDocument("MCPSetExpressionNativeInspectionFailure")
    _prepare(document)
    state_before = _model_state(document)

    def reject(_document, _receipt):
        raise subject.SetExpressionError("CREATED_OBJECT_WRONG_TYPE", "forced inspection failure")

    monkeypatch.setattr(subject, "read_set_expression_result", reject)
    try:
        result = subject.run_set_expression(_collaborators(FreeCAD, lambda _d: None), document.Name, "Seed", "Length", "1+1")
        assert result["success"] is False
...
def test_set_expression_native_postcondition_cannot_write():
    _require_native_collaboration()
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops.set_expression import run_set_expression

    document = FreeCAD.newDocument("MCPSetExpressionReadOnlyPostcondition")
    ...
    try:
        result = run_set_expression(_collaborators(FreeCAD, validate), document.Name, "Seed", "Length", "1+1")
