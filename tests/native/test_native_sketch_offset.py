"""Native FreeCAD qualification for ``sketch_offset``."""
...
def test_sketch_offset_native_success_on_idle_sketch():
    _require_native_collaboration()
    import FreeCAD
    import Part
    import Sketcher

    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops.sketch_offset import run_sketch_offset

    document, sketch = _idle_sketch(FreeCAD, Part, "MCPSketchOffsetNativeSuccess")
    before = sketch.GeometryCount, sketch.ConstraintCount
    try:
        result = run_sketch_offset(
            _collaborators(FreeCAD, Part, Sketcher, lambda _doc: None),
            document.Name, sketch.Name, [0], 1.0, True, False,
        )
...
def test_sketch_offset_native_validation_failure_restores_geometry_counts():
    _require_native_collaboration()
    import FreeCAD
    import Part
    import Sketcher
    ...
    document, sketch = _idle_sketch(FreeCAD, Part, "MCPSketchOffsetNativeValidation")
    ...
    try:
        result = run_sketch_offset(_collaborators(FreeCAD, Part, Sketcher, fail), document.Name, sketch.Name, [0], 1.0, True, False)
...
def test_sketch_offset_native_missing_document_keeps_typed_error():
    _require_native_collaboration()
    import FreeCAD
    import Part
    import Sketcher
...
def test_sketch_offset_native_failure_isolated_and_healthy_document_succeeds():
    _require_native_collaboration()
    import FreeCAD
    import Part
    import Sketcher
    ...
    failed_document, failed_sketch = _idle_sketch(FreeCAD, Part, "MCPSketchOffsetIsolatedFailure")
    healthy_document, healthy_sketch = _idle_sketch(FreeCAD, Part, "MCPSketchOffsetHealthyDocument")
    ...
    try:
        failed = run_sketch_offset(collab, failed_document.Name, failed_sketch.Name, [0], 1.0, True, False)
        isolated = run_sketch_offset(collab, healthy_document.Name, healthy_sketch.Name, [0], 1.0, True, False)
        assert failed["success"] is False
        assert isolated["success"] is True, isolated
    finally:
        FreeCAD.closeDocument(failed_document.Name)
        FreeCAD.closeDocument(healthy_document.Name)


def test_sketch_offset_native_postcondition_cannot_write():
    _require_native_collaboration()
    import FreeCAD
    import Part
    import Sketcher
    ...
    document, sketch = _idle_sketch(FreeCAD, Part, "MCPSketchOffsetReadOnlyPostcondition")
