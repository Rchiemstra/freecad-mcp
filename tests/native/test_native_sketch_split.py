"""Native FreeCAD qualification for ``sketch_split``."""
...
def test_sketch_split_native_success_on_idle_sketch():
    _require_native_collaboration()
    import FreeCAD
    import Part
    import Sketcher

    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops.sketch_split import run_sketch_split

    document, sketch = _idle_sketch(FreeCAD, Part, "MCPSketchSplitNativeSuccess")
    before = sketch.GeometryCount, sketch.ConstraintCount
    try:
        result = run_sketch_split(
            _collaborators(FreeCAD, Part, Sketcher, lambda _doc: None),
            document.Name, sketch.Name, 0, 5.0, 0.0,
        )
...
def test_sketch_split_native_validation_failure_restores_geometry_counts():
    _require_native_collaboration()
    import FreeCAD
    import Part
    import Sketcher
    ...
    document, sketch = _idle_sketch(FreeCAD, Part, "MCPSketchSplitNativeValidation")
    ...
    try:
        result = run_sketch_split(_collaborators(FreeCAD, Part, Sketcher, fail), document.Name, sketch.Name, 0, 5.0, 0.0)
...
def test_sketch_split_native_missing_document_keeps_typed_error():
    _require_native_collaboration()
    import FreeCAD
    import Part
    import Sketcher
...
def test_sketch_split_native_failure_isolated_and_healthy_document_succeeds():
    _require_native_collaboration()
    import FreeCAD
    import Part
    import Sketcher
    ...
    failed_document, failed_sketch = _idle_sketch(FreeCAD, Part, "MCPSketchSplitIsolatedFailure")
    healthy_document, healthy_sketch = _idle_sketch(FreeCAD, Part, "MCPSketchSplitHealthyDocument")
    ...
    try:
        failed = run_sketch_split(collab, failed_document.Name, failed_sketch.Name, 0, 5.0, 0.0)
        isolated = run_sketch_split(collab, healthy_document.Name, healthy_sketch.Name, 0, 5.0, 0.0)
...
def test_sketch_split_native_postcondition_cannot_write():
    _require_native_collaboration()
    import FreeCAD
    import Part
    import Sketcher
    ...
    document, sketch = _idle_sketch(FreeCAD, Part, "MCPSketchSplitReadOnlyPostcondition")
