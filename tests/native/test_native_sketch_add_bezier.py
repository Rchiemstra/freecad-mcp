"""Native FreeCAD qualification for ``sketch_add_bezier``."""
...
def test_sketch_add_bezier_native_success_on_idle_sketch():
    _require_native_collaboration()
    import FreeCAD
    import Part
    import Sketcher

    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops.sketch_add_bezier import run_sketch_add_bezier

    document, sketch = _idle_sketch(FreeCAD, Part, "MCPSketchAddBezierNativeSuccess")
    before = sketch.GeometryCount, sketch.ConstraintCount
    try:
        result = run_sketch_add_bezier(
            _collaborators(FreeCAD, Part, Sketcher, lambda _doc: None),
            document.Name, sketch.Name, [{'x': 0.0, 'y': 0.0}, {'x': 5.0, 'y': 4.0}, {'x': 10.0, 'y': 0.0}], False,
        )
...
def test_sketch_add_bezier_native_validation_failure_restores_geometry_counts():
    _require_native_collaboration()
    import FreeCAD
    import Part
    import Sketcher
    ...
    document, sketch = _idle_sketch(FreeCAD, Part, "MCPSketchAddBezierNativeValidation")
    ...
    try:
        result = run_sketch_add_bezier(_collaborators(FreeCAD, Part, Sketcher, fail), document.Name, sketch.Name, [{'x': 0.0, 'y': 0.0}, {'x': 5.0, 'y': 4.0}, {'x': 10.0, 'y': 0.0}], False)
...
def test_sketch_add_bezier_native_missing_document_keeps_typed_error():
    _require_native_collaboration()
    import FreeCAD
    import Part
    import Sketcher
    ...
    result = run_sketch_add_bezier(
        ...
        "Sketch",
        [{'x': 0.0, 'y': 0.0}, {'x': 5.0, 'y': 4.0}, {'x': 10.0, 'y': 0.0}], False,
    )
    assert result["error_code"] == "DOCUMENT_NOT_FOUND"
    assert result["outcome"] == "rejected"


def test_sketch_add_bezier_native_failure_isolated_and_healthy_document_succeeds():
    _require_native_collaboration()
    import FreeCAD
    import Part
    import Sketcher
    ...
    failed_document, failed_sketch = _idle_sketch(FreeCAD, Part, "MCPSketchAddBezierIsolatedFailure")
    healthy_document, healthy_sketch = _idle_sketch(FreeCAD, Part, "MCPSketchAddBezierHealthyDocument")
    ...
    try:
        failed = run_sketch_add_bezier(collab, failed_document.Name, failed_sketch.Name, [{'x': 0.0, 'y': 0.0}, {'x': 5.0, 'y': 4.0}, {'x': 10.0, 'y': 0.0}], False)
        isolated = run_sketch_add_bezier(collab, healthy_document.Name, healthy_sketch.Name, [{'x': 0.0, 'y': 0.0}, {'x': 5.0, 'y': 4.0}, {'x': 10.0, 'y': 0.0}], False)
...
def test_sketch_add_bezier_native_postcondition_cannot_write():
    _require_native_collaboration()
    import FreeCAD
    import Part
    import Sketcher
    ...
    document, sketch = _idle_sketch(FreeCAD, Part, "MCPSketchAddBezierReadOnlyPostcondition")
