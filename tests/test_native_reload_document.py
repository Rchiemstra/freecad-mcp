...
def test_reload_document_native_commit_succeeds():
    ...
    document = FreeCAD.newDocument("MCPTypedReloadDocument")
    events: list[str] = []
    saved_path = ""
    try:
        
        if "reload_document" == "reload_document":
            saved_path = document.Name + ".FCStd"
            document.saveAs(saved_path)
        if "reload_document" == "open_document":
            saved_path = document.Name + ".FCStd"
            document.saveAs(saved_path)
        ...
        result = run_reload_document(collab, document.Name)
