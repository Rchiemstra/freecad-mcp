"""Native qualification matrix for ``create_part_container``."""
...
def test_create_part_container_native_success_inspects_after_recompute(monkeypatch):
    _require_native_collaboration()
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops import create_part_container as subject

    document = FreeCAD.newDocument("MCPCreatePartContainerNativeSuccess")
    events = []
    ...
    _prepare(document)
    events.clear()
    original_apply = subject.apply_create_part_container
    original_read = subject.read_create_part_container_result
    ...
    monkeypatch.setattr(subject, "apply_create_part_container", tracked_apply)
    monkeypatch.setattr(subject, "read_create_part_container_result", tracked_read)
    try:
        result = subject.run_create_part_container(_collaborators(FreeCAD, lambda _d: events.append("validate")), document.Name, "Created", None, "error")
...
def test_create_part_container_native_validation_failure_restores_complete_state():
    _require_native_collaboration()
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops.create_part_container import run_create_part_container

    document = FreeCAD.newDocument("MCPCreatePartContainerNativeRollback")
    _prepare(document)
    state_before = _model_state(document)
    try:
        result = run_create_part_container(
            _collaborators(FreeCAD, lambda _d: (_ for _ in ()).throw(RuntimeError("forced validation failure"))),
            document.Name, "Created", None, "error",
        )
...
def test_create_part_container_native_apply_failure_restores(monkeypatch):
    _require_native_collaboration()
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops import create_part_container as subject

    document = FreeCAD.newDocument("MCPCreatePartContainerNativeApplyFailure")
    _prepare(document)
    state_before = _model_state(document)
    original = subject.apply_create_part_container
    ...
    monkeypatch.setattr(subject, "apply_create_part_container", mutates_then_raises)
    try:
        result = subject.run_create_part_container(_collaborators(FreeCAD, lambda _d: None), document.Name, "Created", None, "error")
        assert result["success"] is False
...
def test_create_part_container_native_recompute_failure_rolls_back(monkeypatch):
    _require_native_collaboration()
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops import create_part_container as subject

    document = FreeCAD.newDocument("MCPCreatePartContainerNativeRecomputeFailure")
    ...
    monkeypatch.setattr(subject, "apply_create_part_container", arm)
    try:
        result = subject.run_create_part_container(_collaborators(FreeCAD, lambda _d: None), document.Name, "Created", None, "error")
        assert result["success"] is False
...
def test_create_part_container_native_rollback_failure_is_uncertain_and_fences(monkeypatch):
    _require_native_collaboration()
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops import create_part_container as subject

    document = FreeCAD.newDocument("MCPCreatePartContainerNativeRollbackFailure")
    ...
    try:
        result = subject.run_create_part_container(collaborators, document.Name, "Created", None, "error")
        proxy.armed = False
        fenced = subject.run_create_part_container(collaborators, document.Name, "Created", None, "error")
...
def test_create_part_container_native_inspection_failure_rolls_back(monkeypatch):
    _require_native_collaboration()
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops import create_part_container as subject

    document = FreeCAD.newDocument("MCPCreatePartContainerNativeInspectionFailure")
    _prepare(document)
    state_before = _model_state(document)

    def reject(_document, _receipt):
        raise subject.CreatePartContainerError("CREATED_OBJECT_WRONG_TYPE", "forced inspection failure")

    monkeypatch.setattr(subject, "read_create_part_container_result", reject)
    try:
        result = subject.run_create_part_container(_collaborators(FreeCAD, lambda _d: None), document.Name, "Created", None, "error")
        assert result["success"] is False
...
def test_create_part_container_native_postcondition_cannot_write():
    _require_native_collaboration()
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops.create_part_container import run_create_part_container

    document = FreeCAD.newDocument("MCPCreatePartContainerReadOnlyPostcondition")
    ...
    try:
        result = run_create_part_container(_collaborators(FreeCAD, validate), document.Name, "Created", None, "error")
