"""Static contract examples for the boolean_difference collaborator boundary."""
...
from addon.FreeCADMCP._shared.protocol.boolean_difference_contract import (
...
class CompleteBooleanDifferenceDouble:
    """A test double that satisfies every boolean_difference dependency."""
...
complete: BooleanDifferenceCollaborators = CompleteBooleanDifferenceDouble()
...
missing_postcondition: BooleanDifferenceCollaborators = MissingPostconditionDouble()  # type: ignore[assignment]

document_name = DocumentName("Model")
feature_name = FeatureName("Feature")
incomplete_success: BooleanDifferenceSuccess = {  # type: ignore[typeddict-item]
    "contract_version": 1,
    "success": True,
    "ok": True,
}
...
def public_client_preserves_names(client: FreeCADConnection) -> None:
    ...
    assert_type(client.boolean_difference(document, feature, feature, feature), object)
    assert_type(boolean_difference_operation(client, True, document, feature, feature, feature), CallToolResult)
    client.boolean_difference(feature, document, document, document)  # type: ignore[arg-type]
    client.boolean_difference("Doc", "Feature", "Feature", "Feature")  # type: ignore[arg-type]
