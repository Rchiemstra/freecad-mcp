"""Unit coverage for the typed ``scale`` mutation slice."""
...
from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops import scale as subject
from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops.scale import run_scale
...
def _collaborators(document, events, *, validator=None, final_result=None, monkeypatch=None):
    if monkeypatch is not None:
        monkeypatch.setattr(measure_io_actions, "scale", _fake_payload)
...
def test_scale_runs_apply_recompute_inspect_validate_then_commits(monkeypatch):
    events = []
    document = _Document(events)
    collaborators, api = _collaborators(document, events, monkeypatch=monkeypatch)
    result = run_scale(collaborators, "Doc", "Box", 1.0, 1.0, 1.0)
...
def test_invalid_names_abort_without_recompute_or_commit(monkeypatch):
    ...
    result = run_scale(collaborators, "Doc", "", 1.0, 1.0, 1.0)
...
def test_scale_has_typed_rpc_handler():
    assert subject.TYPED_RPC_HANDLER[0] == "scale"
    assert callable(subject.TYPED_RPC_HANDLER[1])


def test_unknown_native_evidence_cannot_release_success():
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops.scale_mutation import (
        _scale_native_result,
        _NativeMutationState,
    )
    result = _scale_native_result({"status": "FutureStatus", "committed": False}, _NativeMutationState(postcondition_passed=True))
    assert result["success"] is False
