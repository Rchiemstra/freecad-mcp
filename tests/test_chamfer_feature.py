"""Unit coverage for the typed ``chamfer_feature`` mutation slice."""
...
from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops import chamfer_feature as subject
from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops.chamfer_feature import run_chamfer_feature
from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops.chamfer_feature_mutation import (
    _ChamferFeatureNativeMutationState,
    _chamfer_feature_native_result,
)
...
def test_creation_that_changes_then_raises_is_rolled_back():
    ...
    assert result["success"] is False
    assert result["error_code"] == "CHAMFER_FEATURE_FAILED"
...
def test_apply_and_inspect_use_the_native_admitted_document():
    ...
    prepare_document('edge_feature', admitted)
...
def test_unknown_or_contradictory_native_evidence_cannot_release_success(native_result):
    result = _chamfer_feature_native_result(
        native_result, _ChamferFeatureNativeMutationState(postcondition_passed=True)
    )
    assert result["success"] is False
...
def test_typed_rpc_handler_is_registered():
    ...
    assert "chamfer_feature" in discover_typed_rpc_handlers()
    assert subject.TYPED_RPC_HANDLER[0] == "chamfer_feature"
