...
"""Native collaboration coverage for the typed ``sketch_add_constraint`` mutation."""
...
def _revision_state(document):
    ...
    session = document.beginEditSession("sketch_add_constraint-native-state-probe")
    try:
...
def test_sketch_add_constraint_native_success_inspects_after_recompute(monkeypatch):
    _require_native_collaboration()
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops import sketch_add_constraint as body_create_module

    document = FreeCAD.newDocument("MCPSketchAddConstraintNativePhaseOrder")
    events = []
    ...
    monkeypatch.setattr(body_create_module, "apply_sketch_add_constraint", tracked_apply)
    monkeypatch.setattr(body_create_module, "read_sketch_add_constraint_result", tracked_read)
    collaborators = _collaborators(FreeCAD, lambda _document: events.append("validate"))
    try:
        result = body_create_module.run_sketch_add_constraint(collaborators, document.Name, "Sketch", [{"type": "Radius", "geo": 0, "value": 10.0, "name": "D"}])
...
def test_sketch_add_constraint_native_validation_failure_restores_complete_state():
    _require_native_collaboration()
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops.sketch_add_constraint import run_sketch_add_constraint

    document = FreeCAD.newDocument("MCPSketchAddConstraintNativeRollback")
    _idle(document, 'constraint')
    ...
    try:
        result = run_sketch_add_constraint(collaborators, document.Name, "Sketch", [{"type": "Radius", "geo": 0, "value": 10.0, "name": "D"}])
...
def test_sketch_add_constraint_native_apply_failure_restores_all_attempted_effects(monkeypatch):
    _require_native_collaboration()
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops import sketch_add_constraint as body_create_module

    document = FreeCAD.newDocument("MCPSketchAddConstraintNativeApplyFailure")
    _idle(document, 'constraint')
    state_before = _model_state(document)
    revision_before = _revision_state(document)
    original_apply = body_create_module.apply_sketch_add_constraint
    ...
    monkeypatch.setattr(body_create_module, "apply_sketch_add_constraint", mutates_then_raises)
...
def test_sketch_add_constraint_native_recompute_failure_rolls_back(monkeypatch):
    _require_native_collaboration()
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops import sketch_add_constraint as body_create_module

    document = FreeCAD.newDocument("MCPSketchAddConstraintNativeRecomputeFailure")
    ...
    monkeypatch.setattr(body_create_module, "apply_sketch_add_constraint", arm_recompute_failure)
    collaborators = _collaborators(FreeCAD, lambda _document: None)
    try:
        result = body_create_module.run_sketch_add_constraint(collaborators, document.Name, "Sketch", [{"type": "Radius", "geo": 0, "value": 10.0, "name": "D"}])
        assert result["success"] is False
...
def test_sketch_add_constraint_native_rollback_failure_is_uncertain_and_fences_document(monkeypatch):
    _require_native_collaboration()
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops import sketch_add_constraint as body_create_module

    document = FreeCAD.newDocument("MCPSketchAddConstraintNativeRollbackFailure")
    ...
    monkeypatch.setattr(body_create_module, "apply_sketch_add_constraint", arm_persistent_failure)
    collaborators = _collaborators(FreeCAD, lambda _document: None)
    try:
        result = body_create_module.run_sketch_add_constraint(collaborators, document.Name, "Sketch", [{"type": "Radius", "geo": 0, "value": 10.0, "name": "D"}])
        proxy.armed = False
        fenced = body_create_module.run_sketch_add_constraint(collaborators, document.Name, "Sketch", [{"type": "Radius", "geo": 0, "value": 10.0, "name": "D"}])
        assert result["success"] is False
...
def test_sketch_add_constraint_native_inspection_failure_rolls_back_after_recompute(monkeypatch):
    _require_native_collaboration()
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops import sketch_add_constraint as body_create_module

    document = FreeCAD.newDocument("MCPSketchAddConstraintNativeInspectionFailure")
    _idle(document, 'constraint')
    state_before = _model_state(document)
    revision_before = _revision_state(document)

    def reject_inspection(*_args, **_kwargs):
        raise body_create_module.SketchAddConstraintError(
            "CREATED_OBJECT_WRONG_TYPE", "forced post-recompute inspection failure"
        )

    monkeypatch.setattr(body_create_module, "read_sketch_add_constraint_result", reject_inspection)
    collaborators = _collaborators(FreeCAD, lambda _document: None)
    try:
        result = body_create_module.run_sketch_add_constraint(collaborators, document.Name, "Sketch", [{"type": "Radius", "geo": 0, "value": 10.0, "name": "D"}])
        assert result["success"] is False
...
def test_sketch_add_constraint_native_failure_isolated_and_healthy_rollback_recovers():
    _require_native_collaboration()
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops.sketch_add_constraint import run_sketch_add_constraint

    failed_document = FreeCAD.newDocument("MCPSketchAddConstraintIsolatedFailure")
    healthy_document = FreeCAD.newDocument("MCPSketchAddConstraintHealthyDocument")
    _idle(failed_document, 'constraint')
    _idle(healthy_document, 'constraint')
    ...
    try:
        failed = run_sketch_add_constraint(collaborators, failed_document.Name, "Sketch", [{"type": "Radius", "geo": 0, "value": 10.0, "name": "D"}])
        isolated = run_sketch_add_constraint(collaborators, healthy_document.Name, "Sketch", [{"type": "Radius", "geo": 0, "value": 10.0, "name": "D"}])
        rejected_names.clear()
        recovered = run_sketch_add_constraint(collaborators, failed_document.Name, "Sketch", [{"type": "Radius", "geo": 0, "value": 10.0, "name": "D"}])
...
def test_sketch_add_constraint_native_duplicate_and_missing_document_keep_typed_errors():
    _require_native_collaboration()
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops.sketch_add_constraint import run_sketch_add_constraint

    document = FreeCAD.newDocument("MCPSketchAddConstraintTypedErrors")
    _idle(document, 'constraint')
    collaborators = _collaborators(FreeCAD, lambda _: None)
    try:
        first = run_sketch_add_constraint(collaborators, document.Name, "Sketch", [{"type": "Radius", "geo": 0, "value": 10.0, "name": "D"}])
        assert first["success"] is True
        before = _model_state(document), _revision_state(document)
        result = run_sketch_add_constraint(collaborators, document.Name, "MissingSketch", [{"type": "Radius", "geo": 0, "value": 10.0, "name": "D"}])
        assert result["error_code"] == 'SKETCH_NOT_FOUND'
        assert result["retry_safe"] is True
        assert (_model_state(document), _revision_state(document)) == before
        missing = run_sketch_add_constraint(collaborators, "NoSuchAddConstraintQualificationDocument", "Sketch", [{"type": "Radius", "geo": 0, "value": 10.0, "name": "D"}])
...
def test_sketch_add_constraint_native_rollback_restores_rich_model(monkeypatch, stage):
    _require_native_collaboration()
    import FreeCAD
    import Part
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops import sketch_add_constraint as subject

    document = FreeCAD.newDocument("MCPSketchAddConstraintRichRollback")
    ...
    _idle(document, 'constraint')
    before = _model_state(document), _revision_state(document)
    original_apply = subject.apply_sketch_add_constraint
    original_inspect = subject.read_sketch_add_constraint_result
    ...
    def inspect(*args, **kwargs):
        if stage == "inspection":
            raise subject.SketchAddConstraintError("INSPECTION_FAILED", "injected inspection failure")
    ...
    monkeypatch.setattr(subject, "apply_sketch_add_constraint", change_model)
    monkeypatch.setattr(subject, "read_sketch_add_constraint_result", inspect)
    try:
        result = subject.run_sketch_add_constraint(_collaborators(FreeCAD, validate), document.Name, "Sketch", [{"type": "Radius", "geo": 0, "value": 10.0, "name": "D"}])
...
def test_sketch_add_constraint_native_postcondition_cannot_write(monkeypatch, write):
    _require_native_collaboration()
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops import sketch_add_constraint as subject

    document = FreeCAD.newDocument("MCPSketchAddConstraintReadOnlyPostcondition")
