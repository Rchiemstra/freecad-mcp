...
def _call(collaborators, *extra):
    if extra:
        args = list(('Doc', 'Sketch', [{'type': 'circle', 'center': {'x': 0, 'y': 0}, 'radius': 5}]))
        args[1] = extra[0]
        return run_sketch_add_geometry(collaborators, *tuple(args))
    return run_sketch_add_geometry(collaborators, *('Doc', 'Sketch', [{'type': 'circle', 'center': {'x': 0, 'y': 0}, 'radius': 5}]))


def test_sketch_add_geometry_runs_apply_recompute_inspect_validate_then_commits():
    events = []
...
def test_creation_that_changes_then_raises_is_rolled_back():
    ...
    assert result["success"] is False
    assert result["error_code"] == 'SKETCH_ADD_GEOMETRY_FAILED'
    assert "commit" not in events
...
def test_sketch_add_geometry_has_no_uncoordinated_source_entry_point():
...
def test_unknown_or_contradictory_native_evidence_cannot_release_success(native_result):
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops.sketch_add_geometry_mutation import (
        _sketch_add_geometry_native_result,
        _NativeSketchAddGeometryMutationState,
    )

    result = _sketch_add_geometry_native_result(native_result, _NativeSketchAddGeometryMutationState(postcondition_passed=True))
    assert result["success"] is False
