"""Agent-to-FreeCAD wire contract for the ``match_subshape`` MCP operation."""
...
class _RecordingFreeCADTransport:
    ...
    def request(self, path, payload, headers):
        ...
        if body_result is _DEFAULT_RESPONSE:
            from tests.match_subshape.test_match_subshape_response import _success
...
def _invoke(monkeypatch, transport, arguments):
    ...
    mcp = InstrumentedFastMCP("match_subshape-contract")
    exports = {}
    tools_advanced_a._register_match_subshape(mcp, dependencies=object(), exports=exports)

    async def invoke():
        async with create_connected_server_and_client_session(mcp._mcp_server) as client:
            listing = await client.list_tools()
            tool = next(tool for tool in listing.tools if tool.name == "match_subshape")
            return await client.call_tool("match_subshape", arguments)
...
def test_match_subshape_sends_authenticated_json_rpc_to_freecad(monkeypatch):
    ...
    assert envelope["method"] == "match_subshape"
    assert envelope["params"]["doc_name"] == "AgentDocument"
    assert envelope["params"]["source_object"] == "Box"
