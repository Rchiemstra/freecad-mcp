"""Unit coverage for the typed ``close_document`` mutation slice."""
...
from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops.close_document_mutation import (
    _close_document_native_result,
...
def test_close_document_runs_apply_recompute_validate_then_commits():
    events: list[str] = []
    document = FakeDocument(events)
    collab, _api = collaborators(document, events)

    result = run_close_document(collab, "Doc")
...
def test_invalid_arguments_abort_without_commit():
    ...
    result = run_close_document(collab, "")
...
def test_missing_document_fails_without_entering_apply():
    ...
    if "close_document" in {"create_document", "open_document"}:
...
def test_native_capability_is_required_before_apply():
    ...
    if "close_document" not in {"create_document", "open_document"}:
...
def test_unknown_or_contradictory_native_evidence_cannot_release_success(native_result):
    result = _close_document_native_result(native_result, _NativeMutationState(postcondition_passed=True))
    assert result["success"] is False
    assert result["outcome"] == "uncertain"
    assert result["retry_safe"] is False


def test_close_document_has_apply_entry_point():
    assert callable(apply_close_document)
