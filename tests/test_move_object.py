"""Unit coverage for the typed ``move_object`` mutation slice."""
...
def test_move_object_runs_apply_recompute_inspect_validate_then_commits():
    events: list[str] = []
    document = _Document(events)
    _seed(document)
    collaborators, _api = _collaborators(document, events)

    result = run_move_object(collaborators, "Doc", "Seed", "Body", True)
...
def test_creation_that_changes_then_raises_is_rolled_back(monkeypatch):
    ...
    monkeypatch.setattr(subject, "apply_move_object", mutate_then_raise)
    result = run_move_object(collaborators, "Doc", "Seed", "Body", True)
    assert result["success"] is False
...
def test_unknown_or_contradictory_native_evidence_cannot_release_success():
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops.move_object_mutation import (
        _move_object_native_result,
    ...
    for native_result in (
        ...
        result = _move_object_native_result(native_result, _NativeMutationState(postcondition_passed=True))
        assert result["success"] is False
