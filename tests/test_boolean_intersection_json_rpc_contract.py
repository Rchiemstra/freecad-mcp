"""Agent-to-FreeCAD wire contract for the ``boolean_intersection`` MCP operation."""
...
from freecad_mcp.generated.capabilities.register_modules import tools_features_boolean
...
_TOOL_PARAMS = {'doc_name': 'AgentDocument', 'shape1': 'Shape1', 'shape2': 'Shape2', 'result_name': 'Result'}
_REQUIRED = {'doc_name', 'shape2', 'shape1', 'result_name'}
...
class _RecordingFreeCADTransport:
    ...
    def request(self, path, payload, headers):
        ...
        if body_result is _DEFAULT_RESPONSE:
            created = envelope["params"].get('result_name', 'result_name')
            body_result = {
                "contract_version": 1,
                "success": True,
                "ok": True,
...
def _invoke_registered(monkeypatch, transport, tool_params):
    ...
    monkeypatch.setattr(tools_features_boolean, "server_connection", lambda: connection)
    monkeypatch.setattr(
    ...
    mcp = InstrumentedFastMCP("boolean_intersection-contract")
    exports = {}
    tools_features_boolean._register_boolean_intersection(
        mcp,
        dependencies=object(),
        exports=exports,
    )
    ...
    async def invoke():
        async with create_connected_server_and_client_session(mcp._mcp_server) as client:
            ...
            tool = next(tool for tool in listing.tools if tool.name == "boolean_intersection")
            assert set(tool.inputSchema.get("required", [])) == _REQUIRED
            return await client.call_tool("boolean_intersection", tool_params)
...
def test_boolean_intersection_sends_exact_authenticated_json_rpc_values_to_freecad(monkeypatch):
    ...
    assert envelope["method"] == "boolean_intersection"
    assert envelope["session_token"] == "test-session-token"
    assert envelope["operation"] == {"name": 'Boolean intersection'}
...
def test_public_route_marks_committed_but_invalid_response_non_retryable(monkeypatch):
    ...
    result = _invoke_registered(monkeypatch, transport, dict(_TOOL_PARAMS))
    assert result.isError is True
    assert result.structuredContent["error_code"] == "BOOLEAN_INTERSECTION_COMMITTED_RESPONSE_INVALID"
