    ...
    def request(self, path, payload, headers):
        ...
        if body_result is _DEFAULT_RESPONSE:
            body_result = {
                ...
                "committed": True,
                "retry_safe": False,
                "from_obj": envelope["params"]["from_obj"],
                "to_obj": envelope["params"]["to_obj"],
            }
        response = {
            "jsonrpc": "2.0",
            "id": request["id"],
...
def _invoke_registered(monkeypatch, transport, arguments):
    ...
    async def invoke():
        async with create_connected_server_and_client_session(mcp._mcp_server) as client:
            ...
            assert set(tool.inputSchema["required"]) == {"doc_name", "from_obj", "to_obj"}
            return await client.call_tool("relink_references", arguments)
...
def test_relink_references_sends_exact_authenticated_json_rpc_values_to_freecad(monkeypatch):
    transport = _RecordingFreeCADTransport()
    result = _invoke_registered(
        monkeypatch,
        transport,
        {"doc_name": "AgentDocument", "from_obj": "Seed", "to_obj": "Target"},
    )
...
def test_public_route_marks_committed_but_invalid_response_non_retryable(monkeypatch):
    ...
    assert result.isError is True
    assert result.structuredContent["error_code"] == "RELINK_REFERENCES_COMMITTED_RESPONSE_INVALID"
...
def test_mcp_schema_rejects_non_string_doc_name_before_json_rpc(monkeypatch):
    ...
    result = _invoke_registered(
        monkeypatch,
        transport,
        {"doc_name": 42, "from_obj": "Seed", "to_obj": "Target"},
    )
    assert result.isError is True
