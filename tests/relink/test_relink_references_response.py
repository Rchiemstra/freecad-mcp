"""Adversarial wire cases for the ``relink_references`` contract."""
...
def _success():
    return make_relink_references_success(from_obj="Value", to_obj="Value")


@pytest.mark.parametrize("raw", [None, [], 1, "timeout", {}, {1: "bad key"}])
def test_unknown_response_never_proves_rejection(raw):
    result = parse_relink_references_response(raw)
...
def test_contract_version_must_be_the_exact_integer(version):
    assert parse_relink_references_response(dict(_success(), contract_version=version))["success"] is False
...
def test_contradictory_or_malformed_success_cannot_escape(change):
    result = parse_relink_references_response(dict(_success(), **change))
    assert result["success"] is False
    assert result["outcome"] == "uncertain"
    assert result["retry_safe"] is False
...
def test_valid_success_round_trips():
    raw = _success()
    assert parse_relink_references_response(raw) == raw
...
def test_transport_failure_preserves_unknown_model_state():
    ...
    response = relink_references_operation(_Conn(), True, "Doc", "Value", "Value")
    assert response.isError is True
    data = response.structuredContent["data"]
    assert data["error_code"] == "RELINK_REFERENCES_TRANSPORT_UNCERTAIN"
