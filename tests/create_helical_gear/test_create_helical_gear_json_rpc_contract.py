"""Agent-to-FreeCAD wire contract for the ``create_helical_gear`` MCP operation."""
...
from freecad_mcp._shared.protocol.create_helical_gear_contract import make_create_helical_gear_success
...
class _RecordingFreeCADTransport:
    ...
    def request(self, path, payload, headers):
        ...
        if body_result is _DEFAULT_RESPONSE:
            from tests.create_helical_gear.test_create_helical_gear_response import _success
...
def _invoke(monkeypatch, transport, arguments):
    ...
    mcp = InstrumentedFastMCP("create_helical_gear-contract")
    exports = {}
    tools_gear_1._register_create_helical_gear(mcp, dependencies=object(), exports=exports)

    async def invoke():
        async with create_connected_server_and_client_session(mcp._mcp_server) as client:
            listing = await client.list_tools()
            tool = next(tool for tool in listing.tools if tool.name == "create_helical_gear")
            return await client.call_tool("create_helical_gear", arguments)
...
def test_create_helical_gear_sends_authenticated_json_rpc_to_freecad(monkeypatch):
    ...
    if "create_helical_gear" == "create_assembly_grounded_joint":
        arguments.update({"assembly_name": "Assembly", "component_name": "Base"})
    elif "create_helical_gear" == "create_assembly_joint":
        arguments.update({
    ...
    elif "create_helical_gear" == "solve_assembly":
        arguments.update({"assembly_name": "Assembly"})
    elif "create_helical_gear" in {"create_helical_gear", "create_involute_gear", "create_spur_gear"}:
        arguments.update({"gear_name": "Gear", "teeth": 8, "module": 2.0, "width": 10.0})
    elif "create_helical_gear" == "export_brep":
        arguments.update({"obj_name": "Box", "file_path": "/tmp/out.brep"})
    elif "create_helical_gear" in {"export_step", "export_stl"}:
        arguments.update({"file_path": "/tmp/out"})
    elif "create_helical_gear" in {"import_brep", "import_step"}:
        arguments.update({"file_path": "/tmp/in"})
    elif "create_helical_gear" in {"bounding_box", "center_of_mass"}:
        arguments.update({"obj_name": "Box"})
    elif "create_helical_gear" == "common_volume_along_path":
        arguments.update({"moving_object": "Mover", "obstacle_objects": ["Wall"], "samples": [{"x": 0, "y": 0, "z": 0}]})
    elif "create_helical_gear" == "rotate":
        arguments.update({"obj_name": "Box", "axis_x": 0, "axis_y": 0, "axis_z": 1, "angle_deg": 90})
    elif "create_helical_gear" == "scale":
        arguments.update({"obj_name": "Box", "sx": 1, "sy": 1, "sz": 1})
    elif "create_helical_gear" == "translate":
