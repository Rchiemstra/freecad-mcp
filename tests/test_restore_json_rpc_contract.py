"""Agent-to-FreeCAD wire contract for the ``restore`` MCP operation."""
...
class _RecordingFreeCADTransport:
    ...
    def request(self, path, payload, headers):
        ...
        if body_result is _DEFAULT_RESPONSE:
            body_result = dict({"contract_version": 1, "success": True, "ok": True, "outcome": "committed", "committed": True, "retry_safe": False, "restored_id": "Value", "doc": "Value"}, **{
                key: envelope["params"][key]
                for key in envelope["params"]
                if key in ['restored_id', 'doc']
            })
            body_result = {"contract_version": 1, "success": True, "ok": True, "outcome": "committed", "committed": True, "retry_safe": False, "restored_id": "Value", "doc": "Value"}
        response = {
            "jsonrpc": "2.0",
            "id": request["id"],
            "result": {
...
def _invoke_registered(monkeypatch, transport, params):
    ...
    mcp = InstrumentedFastMCP("restore-contract")
    exports = {}
    tools_advanced_b2._register_restore(mcp, dependencies=object(), exports=exports)

    async def invoke():
        async with create_connected_server_and_client_session(mcp._mcp_server) as client:
            listing = await client.list_tools()
            tool = next(tool for tool in listing.tools if tool.name == "restore")
            assert "doc_name" in tool.inputSchema.get("required", ["doc_name"])
            return await client.call_tool("restore", params)
...
def test_restore_sends_authenticated_json_rpc_to_freecad(monkeypatch):
    ...
    assert envelope["method"] == "restore"
