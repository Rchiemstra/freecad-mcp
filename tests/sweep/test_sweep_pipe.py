"""Unit coverage for the typed ``sweep_pipe`` mutation slice."""
...
def test_sweep_pipe_runs_apply_recompute_inspect_validate_then_commits():
    events: list[str] = []
    document = _Document(events)
    _seed(document)
    collaborators, _api = _collaborators(document, events)

    result = run_sweep_pipe(collaborators, "Doc", "Seed", 1.5, "Created", "frenet", None, None, "error")
...
def test_creation_that_changes_then_raises_is_rolled_back(monkeypatch):
    ...
    monkeypatch.setattr(subject, "apply_sweep_pipe", mutate_then_raise)
    result = run_sweep_pipe(collaborators, "Doc", "Seed", 1.5, "Created", "frenet", None, None, "error")
    assert result["success"] is False
...
def test_unknown_or_contradictory_native_evidence_cannot_release_success():
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops.sweep_pipe_mutation import (
        _sweep_pipe_native_result,
    ...
    for native_result in (
        ...
        result = _sweep_pipe_native_result(native_result, _NativeMutationState(postcondition_passed=True))
        assert result["success"] is False
