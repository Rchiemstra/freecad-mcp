"""Agent-to-FreeCAD wire contract for the ``sweep_feature`` MCP operation."""
...
from freecad_mcp.generated.capabilities.register_modules import tools_features_advanced_a
...
_TOOL_PARAMS = {'doc_name': 'AgentDocument', 'profile_sketch': 'Sketch', 'path_sketch': 'Path', 'sweep_name': 'Sweep'}
_REQUIRED = {'doc_name', 'path_sketch', 'sweep_name', 'profile_sketch'}
...
class _RecordingFreeCADTransport:
    ...
    def request(self, path, payload, headers):
        ...
        if body_result is _DEFAULT_RESPONSE:
            created = envelope["params"].get('sweep_name', 'sweep_name')
            body_result = {
                "contract_version": 1,
                "success": True,
                "ok": True,
...
def _invoke_registered(monkeypatch, transport, tool_params):
    ...
    monkeypatch.setattr(tools_features_advanced_a, "server_connection", lambda: connection)
    monkeypatch.setattr(
    ...
    mcp = InstrumentedFastMCP("sweep_feature-contract")
    exports = {}
    tools_features_advanced_a._register_sweep_feature(
        mcp,
        dependencies=object(),
        exports=exports,
    )
    ...
    async def invoke():
        async with create_connected_server_and_client_session(mcp._mcp_server) as client:
            ...
            tool = next(tool for tool in listing.tools if tool.name == "sweep_feature")
            assert set(tool.inputSchema.get("required", [])) == _REQUIRED
            return await client.call_tool("sweep_feature", tool_params)
...
def test_sweep_feature_sends_exact_authenticated_json_rpc_values_to_freecad(monkeypatch):
    ...
    assert envelope["method"] == "sweep_feature"
    assert envelope["session_token"] == "test-session-token"
    assert envelope["operation"] == {"name": 'Sweep feature'}
...
def test_public_route_marks_committed_but_invalid_response_non_retryable(monkeypatch):
    ...
    result = _invoke_registered(monkeypatch, transport, dict(_TOOL_PARAMS))
    assert result.isError is True
    assert result.structuredContent["error_code"] == "SWEEP_FEATURE_COMMITTED_RESPONSE_INVALID"
