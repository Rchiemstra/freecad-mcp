"""Adversarial wire cases for the ``create_document`` contract."""
...
def test_contract_version_must_be_the_exact_integer(version):
    assert parse_create_document_response(dict(_success(), contract_version=version))["success"] is False
...
def test_contradictory_or_malformed_success_cannot_escape(change):
    result = parse_create_document_response(dict(_success(), **change))
    assert result["success"] is False
    assert result["outcome"] == "uncertain"
    assert result["retry_safe"] is False
...
def test_valid_success_round_trips():
    raw = _success()
    assert parse_create_document_response(raw) == raw


def test_valid_failure_round_trips():
    raw = make_create_document_failure("BUSY", "Retry", native_status="Busy")
    assert parse_create_document_response(raw) == raw


@pytest.mark.parametrize("committed", [None, False, True])
def test_valid_uncertain_round_trips(committed):
    raw = make_create_document_uncertain("STATE_UNKNOWN", "Reconcile", committed=committed)
    assert parse_create_document_response(raw) == raw
...
def test_transport_failure_preserves_unknown_model_state():
    ...
    client = SimpleNamespace(
        create_document=lost_response,
        get_active_screenshot=lambda: None,
    )
    response = create_document_operation(client, "Doc")
