"""Unit coverage for the typed ``create_document`` mutation slice."""
...
from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops.create_document_mutation import (
    _create_document_native_result,
...
def test_create_document_runs_apply_recompute_validate_then_commits():
    events: list[str] = []
...
def test_missing_document_fails_without_entering_apply():
    events: list[str] = []
    collab, _api = collaborators(None, events)
    if "create_document" in {"create_document", "open_document"}:
...
def test_native_capability_is_required_before_apply():
    ...
    if "create_document" not in {"create_document", "open_document"}:
...
def test_unknown_or_contradictory_native_evidence_cannot_release_success(native_result):
    result = _create_document_native_result(native_result, _NativeMutationState(postcondition_passed=True))
    assert result["success"] is False
    assert result["outcome"] == "uncertain"
    assert result["retry_safe"] is False


def test_create_document_has_apply_entry_point():
    assert callable(apply_create_document)
