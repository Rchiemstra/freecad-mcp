"""Agent-to-FreeCAD wire contract for the ``create_document`` MCP operation."""
...
def _invoke_registered(monkeypatch, transport, arguments):
    ...
    mcp = InstrumentedFastMCP("create_document-contract")
    exports = {}
    tools_core_document._register_create_document(mcp, dependencies=object(), exports=exports)

    async def invoke():
        async with create_connected_server_and_client_session(mcp._mcp_server) as client:
            listing = await client.list_tools()
            tool = next(tool for tool in listing.tools if tool.name == "create_document")
            assert set(tool.inputSchema["required"]) == {'name'}
            return await client.call_tool("create_document", arguments)
...
def test_create_document_sends_exact_authenticated_json_rpc_values_to_freecad(monkeypatch):
    ...
    assert envelope["method"] == "create_document"
    assert envelope["params"] == {'name': 'AgentDocument'}
    assert envelope["operation"] == {"name": 'Create document'}
...
def test_public_route_never_turns_rejection_into_success(monkeypatch):
    arguments = dict({'name': 'AgentDocument'})
    if "name" in arguments and isinstance(arguments["name"], str):
        arguments["name"] = ""
    transport = _RecordingFreeCADTransport(
        {
            "contract_version": 1,
