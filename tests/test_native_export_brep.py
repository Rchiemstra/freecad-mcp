...
def test_export_brep_native_success_inspects_after_recompute(monkeypatch):
    ...
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops import export_brep as module

    document = FreeCAD.newDocument("MCPExportBrepNative")
    events: list[str] = []
    ...
    monkeypatch.setattr(module, "apply_export_brep", tracked_apply)
    collaborators = _collaborators(FreeCAD, lambda _document: events.append("validate"))
    try:
        result = module.run_export_brep(collaborators, document.Name, "Box", "/tmp/out.brep")
...
def test_export_brep_native_validation_failure_restores(monkeypatch):
    _require_native_collaboration()
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops import export_brep as module

    document = FreeCAD.newDocument("MCPExportBrepNativeRollback")
    original_apply = module.apply_export_brep

    def tracked_apply(admitted_document, request):
        obj = admitted_document.addObject("App::FeaturePython", "RejectedNative")
        return original_apply(admitted_document, request) if False else module.ExportBrepReceipt(
...
def test_export_brep_native_missing_document_keeps_typed_error():
    ...
    result = run_export_brep(collaborators, "MissingNativeDoc", "Box", "/tmp/out.brep")
    assert result["success"] is False
