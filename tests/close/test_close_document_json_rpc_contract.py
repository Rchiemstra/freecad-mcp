"""Agent-to-FreeCAD wire contract for the ``close_document`` MCP operation."""
...
def _invoke_registered(monkeypatch, transport, arguments):
    ...
    mcp = InstrumentedFastMCP("close_document-contract")
    exports = {}
    tools_document_history._register_close_document(mcp, dependencies=object(), exports=exports)

    async def invoke():
        async with create_connected_server_and_client_session(mcp._mcp_server) as client:
            listing = await client.list_tools()
            tool = next(tool for tool in listing.tools if tool.name == "close_document")
            assert set(tool.inputSchema["required"]) == {'doc_name'}
            return await client.call_tool("close_document", arguments)
...
def test_close_document_sends_exact_authenticated_json_rpc_values_to_freecad(monkeypatch):
    ...
    assert envelope["method"] == "close_document"
    assert envelope["params"] == {'doc_name': 'AgentDocument'}
    assert envelope["operation"] == {"name": 'Close document'}
...
def test_public_route_marks_committed_but_invalid_response_non_retryable(monkeypatch):
    ...
    result = _invoke_registered(monkeypatch, transport, {'doc_name': 'AgentDocument'})
    assert result.isError is True
    assert result.structuredContent["error_code"] == 'CLOSE_DOCUMENT_COMMITTED_RESPONSE_INVALID'
