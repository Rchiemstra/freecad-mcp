...
def _call(collaborators, *extra):
    if extra:
        args = list(('Doc', 'Sketch', 'Pocket', 5.0, 'Body'))
        args[2] = extra[0]
        return run_pocket_feature(collaborators, *tuple(args))
    return run_pocket_feature(collaborators, *('Doc', 'Sketch', 'Pocket', 5.0, 'Body'))


def test_pocket_feature_runs_apply_recompute_inspect_validate_then_commits():
    events = []
    ...
    assert result["pocket"] == "Pocket"
    assert result["label"] == "Label for Pocket"
...
def test_duplicate_or_missing_target_aborts_without_recompute_or_commit():
    events = []
    document = _Document(events)
    document.objects["Pocket"] = _target("Pocket", "PartDesign::Pocket")
    collaborators, _api = _collaborators(document, events)
...
def test_missing_body_aborts_without_recompute_or_commit():
    ...
    result = run_pocket_feature(collaborators, "Doc", "Sketch", "Pocket", 10.0, "Body")
...
def test_creation_that_changes_then_raises_is_rolled_back():
    ...
    assert result["success"] is False
    assert result["error_code"] == 'POCKET_FEATURE_FAILED'
    assert "commit" not in events
...
def test_response_uses_the_actual_name_assigned_by_freecad():
    events = []
    document = _Document(events, assigned_name='Pocket001')
    collaborators, _api = _collaborators(document, events)

    result = _call(collaborators)

    assert result["success"] is True
    assert result["pocket"] == "Pocket001"


def test_pocket_feature_has_no_uncoordinated_source_entry_point():
    assert not hasattr(subject, 'pocket_feature_gui')
...
def test_unknown_or_contradictory_native_evidence_cannot_release_success(native_result):
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops.pocket_feature_mutation import (
        _pocket_feature_native_result,
        _NativePocketFeatureMutationState,
    )

    result = _pocket_feature_native_result(native_result, _NativePocketFeatureMutationState(postcondition_passed=True))
    assert result["success"] is False
