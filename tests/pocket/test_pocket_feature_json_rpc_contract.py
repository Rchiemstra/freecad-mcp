"""Agent-to-FreeCAD wire contract for the ``pocket_feature`` MCP operation."""
    ...
    def request(self, path, payload, headers):
        ...
        if body_result is _DEFAULT_RESPONSE:
            body_result = {
                ...
                "pocket": envelope["params"]["pocket_name"],
                "label": envelope["params"]["pocket_name"],
            }
        response = {
            "jsonrpc": "2.0",
            "id": request["id"],
...
def _invoke_registered(monkeypatch, transport, arguments):
    ...
    mcp = InstrumentedFastMCP("pocket_feature-contract")
    exports = {}
    tools_features_basic_1._register_pocket_feature(
        mcp,
        dependencies=object(),
        exports=exports,
    )
    ...
    async def invoke():
        async with create_connected_server_and_client_session(mcp._mcp_server) as client:
            ...
            tool = next(tool for tool in listing.tools if tool.name == "pocket_feature")
            assert set(tool.inputSchema["required"]) == {'doc_name', 'length', 'pocket_name', 'sketch_name'}
            return await client.call_tool("pocket_feature", arguments)
...
def test_pocket_feature_sends_exact_authenticated_json_rpc_values_to_freecad(monkeypatch):
    transport = _RecordingFreeCADTransport()
    result = _invoke_registered(monkeypatch, transport, {'doc_name': 'AgentDocument', 'sketch_name': 'Profile', 'pocket_name': 'MainPocket', 'length': 5.0})
...
def test_public_pocket_feature_route_never_turns_rejection_into_success(monkeypatch):
    response_result = {
        "contract_version": 1,
        "success": False,
        "ok": False,
...
def test_public_route_marks_committed_but_invalid_response_non_retryable(monkeypatch):
    ...
    result = _invoke_registered(monkeypatch, transport, {'doc_name': 'AgentDocument', 'sketch_name': 'Profile', 'pocket_name': 'MainPocket', 'length': 5.0})
    assert result.isError is True
    assert result.structuredContent["error_code"] == "POCKET_FEATURE_COMMITTED_RESPONSE_INVALID"
...
@pytest.mark.parametrize("pocket_name", [None, 42, [], {}])
def test_mcp_schema_rejects_non_string_before_json_rpc(monkeypatch, pocket_name):
    transport = _RecordingFreeCADTransport()
    arguments = dict({'doc_name': 'AgentDocument', 'sketch_name': 'Profile', 'pocket_name': 'MainPocket', 'length': 5.0})
    arguments["pocket_name"] = pocket_name
