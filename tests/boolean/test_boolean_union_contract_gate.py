"""Sensitivity checks for the ``boolean_union`` static and architecture gate."""
...
LEAF = "addon/FreeCADMCP/rpc_server/methods/cad_methods_ops/boolean_union.py"
MUTATION = "addon/FreeCADMCP/rpc_server/methods/cad_methods_ops/boolean_union_mutation.py"
BRIDGE = "addon/FreeCADMCP/collaboration_api.py"
PUBLIC_ADAPTER = "src/freecad_mcp/operations/parametric_ops/boolean_union.py"
...
def test_boolean_union_architecture_gate_accepts_the_production_path() -> None:
...
@pytest.mark.parametrize(
    ...
    [
        (
            ...
            "            self.inspect,",
            "            self.apply,",
            "BOOLEAN_UNION004 missing typed postcondition=inspect",
        ),
        (
            PUBLIC_ADAPTER,
            "result = parse_boolean_union_response(raw_result)",
            "result = raw_result",
            "BOOLEAN_UNION008 public adapter bypasses response validation",
        ),
        (
            MUTATION,
            ...
            "    if committed is not False or status not in _REJECTED_STATUSES:",
            "BOOLEAN_UNION007 cached success escaped native rejection",
        ),
        (
            MUTATION,
            "if state.postcondition_passed and state.failure is None:",
            "if True:",
            "BOOLEAN_UNION015 commit escaped without a successful postcondition",
        ),
    ],
)
...
def test_gate_rejects_transaction_or_recompute_ownership_in_the_leaf() -> None:
    ...
    assert "BOOLEAN_UNION001 leaf owns forbidden execution: recompute" in (
...
def test_gate_rejects_any_on_the_typed_surface() -> None:
    source = _read(LEAF)
    old = "def build_boolean_union_request("
    broken = "from typing import Any\n\ndef build_boolean_union_request(\n    unused: Any,"
    assert source.count(old) == 1
    assert "BOOLEAN_UNION014 typed boolean_union surface contains Any: boolean_union leaf" in (
        scan_feature_architecture(
            ROOT,
            "boolean_union",
