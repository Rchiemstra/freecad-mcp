"""Unit coverage for the typed ``boolean_intersection`` mutation slice."""
...
from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops import boolean_intersection as subject
from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops.boolean_intersection import run_boolean_intersection
from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops.boolean_intersection_mutation import (
    _BooleanIntersectionNativeMutationState,
    _boolean_intersection_native_result,
)
...
def _run(collab, **overrides):
    ...
    return run_boolean_intersection(collab, **payload)


def test_boolean_intersection_runs_apply_recompute_inspect_validate_then_commits():
    events: list[str] = []
    document = FeatureDocument(events)
    prepare_document('boolean', document)
...
def test_creation_that_changes_then_raises_is_rolled_back():
    ...
    assert result["success"] is False
    assert result["error_code"] == "BOOLEAN_INTERSECTION_FAILED"
...
def test_apply_and_inspect_use_the_native_admitted_document():
    ...
    prepare_document('boolean', admitted)
...
def test_unknown_or_contradictory_native_evidence_cannot_release_success(native_result):
    result = _boolean_intersection_native_result(
        native_result, _BooleanIntersectionNativeMutationState(postcondition_passed=True)
    )
    assert result["success"] is False
...
def test_typed_rpc_handler_is_registered():
    ...
    assert "boolean_intersection" in discover_typed_rpc_handlers()
    assert subject.TYPED_RPC_HANDLER[0] == "boolean_intersection"
