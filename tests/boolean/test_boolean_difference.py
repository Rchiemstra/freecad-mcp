"""Unit coverage for the typed ``boolean_difference`` mutation slice."""
...
from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops import boolean_difference as subject
from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops.boolean_difference import run_boolean_difference
from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops.boolean_difference_mutation import (
    _BooleanDifferenceNativeMutationState,
    _boolean_difference_native_result,
)
...
def _run(collab, **overrides):
    ...
    return run_boolean_difference(collab, **payload)


def test_boolean_difference_runs_apply_recompute_inspect_validate_then_commits():
    events: list[str] = []
...
def test_creation_that_changes_then_raises_is_rolled_back():
    ...
    assert result["success"] is False
    assert result["error_code"] == "BOOLEAN_DIFFERENCE_FAILED"
...
def test_unknown_or_contradictory_native_evidence_cannot_release_success(native_result):
    result = _boolean_difference_native_result(
        native_result, _BooleanDifferenceNativeMutationState(postcondition_passed=True)
    )
    assert result["success"] is False
...
def test_typed_rpc_handler_is_registered():
    ...
    assert "boolean_difference" in discover_typed_rpc_handlers()
    assert subject.TYPED_RPC_HANDLER[0] == "boolean_difference"
