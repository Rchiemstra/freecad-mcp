"""Unit coverage for the typed ``redo`` mutation slice."""
...
from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops.redo import (
    apply_redo,
    run_redo,
)
from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops.redo_mutation import (
    _redo_native_result,
...
def test_redo_runs_apply_recompute_validate_then_commits():
    events: list[str] = []
...
def test_missing_document_fails_without_entering_apply():
    ...
    if "redo" in {"create_document", "open_document"}:
...
def test_native_capability_is_required_before_apply():
    ...
    if "redo" not in {"create_document", "open_document"}:
...
def test_unknown_or_contradictory_native_evidence_cannot_release_success(native_result):
    result = _redo_native_result(native_result, _NativeMutationState(postcondition_passed=True))
    assert result["success"] is False
    assert result["outcome"] == "uncertain"
    assert result["retry_safe"] is False


def test_redo_has_apply_entry_point():
    assert callable(apply_redo)
