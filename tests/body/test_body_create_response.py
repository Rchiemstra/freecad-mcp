"""Adversarial wire cases for the Body reference contract."""
...
def test_contract_version_must_be_the_exact_integer(version):
    assert (
        parse_body_create_response(dict(_success(), contract_version=version))["success"] is False
    )
...
def test_contradictory_or_malformed_success_cannot_escape(change):
    result = parse_body_create_response(dict(_success(), **change))
    assert result["success"] is False
    assert result["outcome"] == "uncertain"
    assert result["retry_safe"] is False
...
@pytest.mark.parametrize(
    ...
    [
        _success(),
        make_body_create_failure("BUSY", "Retry after recompute", native_status="Busy"),
        make_body_create_failure("INVALID_ARGUMENT", "Invalid name", retry_safe=False),
        *[
            make_body_create_uncertain(
                "STATE_UNKNOWN",
...
def test_valid_variants_round_trip_without_changing_evidence(raw):
    assert parse_body_create_response(raw) == raw
    assert parse_body_create_response(parse_body_create_response(raw)) == raw
...
def test_contradictory_failure_never_proves_rejection(change):
    result = parse_body_create_response(
        dict(make_body_create_failure("REJECTED", "Rejected"), **change)
    )
    assert result["outcome"] == "uncertain"
    assert result["retry_safe"] is False
...
def test_gui_completion_timeout_preserves_unknown_model_state():
    ...
    response = body_create_operation(
        SimpleNamespace(body_create=lambda *_: raw), True, "Doc", "Body"
    )
...
def test_transport_failure_preserves_unknown_model_state():
    ...
    response = body_create_operation(
        SimpleNamespace(body_create=lost_response), True, "Doc", "Body"
    )
    assert response.isError is True
    data = response.structuredContent["data"]
    assert data["error_code"] == "BODY_CREATE_TRANSPORT_UNCERTAIN"
