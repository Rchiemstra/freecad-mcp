"""Agent-to-FreeCAD wire contract for the ``create_datum_plane`` MCP operation."""
...
class _RecordingFreeCADTransport:
    ...
    def request(self, path, payload, headers):
        ...
        if body_result is _DEFAULT_RESPONSE:
            body_result = dict({"contract_version": 1, "success": True, "ok": True, "outcome": "committed", "committed": True, "retry_safe": False, "plane_name": "Value", "body_name": "Value"}, **{
                key: envelope["params"][key]
                for key in envelope["params"]
                if key in ['plane_name', 'body_name']
            })
            body_result = {"contract_version": 1, "success": True, "ok": True, "outcome": "committed", "committed": True, "retry_safe": False, "plane_name": "Value", "body_name": "Value"}
        response = {
            "jsonrpc": "2.0",
            "id": request["id"],
            "result": {
...
def _invoke_registered(monkeypatch, transport, params):
    ...
    mcp = InstrumentedFastMCP("create_datum_plane-contract")
    exports = {}
    tools_partdesign_a._register_create_datum_plane(mcp, dependencies=object(), exports=exports)

    async def invoke():
        async with create_connected_server_and_client_session(mcp._mcp_server) as client:
            listing = await client.list_tools()
            tool = next(tool for tool in listing.tools if tool.name == "create_datum_plane")
            required = tool.inputSchema.get("required") or []
            assert "doc_name" in required
            return await client.call_tool("create_datum_plane", params)
...
def test_create_datum_plane_sends_authenticated_json_rpc_to_freecad(monkeypatch):
    transport = _RecordingFreeCADTransport()
    result = _invoke_registered(monkeypatch, transport, {"doc_name": "AgentDocument", "plane_name": "Value", "body_name": "Value", "mode": "through_point", "map_mode": "Value", "if_exists": "error"})
    ...
    assert envelope["method"] == "create_datum_plane"
...
def test_mcp_schema_rejects_non_string_doc_name_before_json_rpc(monkeypatch):
    transport = _RecordingFreeCADTransport()
    params = dict({"doc_name": "AgentDocument", "plane_name": "Value", "body_name": "Value", "mode": "through_point", "map_mode": "Value", "if_exists": "error"})
    params["doc_name"] = 42
