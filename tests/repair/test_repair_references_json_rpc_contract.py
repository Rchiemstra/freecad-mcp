"""Agent-to-FreeCAD wire contract for the ``repair_references`` MCP operation."""
...
from freecad_mcp.generated.capabilities.register_modules import tools_core_objects
...
class _RecordingFreeCADTransport:
    ...
    def request(self, path, payload, headers):
        ...
        if body_result is _DEFAULT_RESPONSE:
            ...
            extra = {"document_name": envelope["params"]["doc_name"], "repaired_count": 1}
...
def _invoke_registered(monkeypatch, transport, arguments):
    ...
    monkeypatch.setattr(tools_core_objects, "server_connection", lambda: connection)
    if hasattr(tools_core_objects, "server_state"):
        monkeypatch.setattr(
            tools_core_objects,
            "server_state",
    ...
    mcp = InstrumentedFastMCP("repair_references-contract")
    exports = {}
    tools_core_objects._register_repair_references(mcp, dependencies=object(), exports=exports)

    async def invoke():
        async with create_connected_server_and_client_session(mcp._mcp_server) as client:
            listing = await client.list_tools()
            tool = next(tool for tool in listing.tools if tool.name == "repair_references")
            assert set(tool.inputSchema["required"]) == {'doc_name', 'repairs'}
            return await client.call_tool("repair_references", arguments)
...
def test_repair_references_sends_exact_authenticated_json_rpc_values_to_freecad(monkeypatch):
    transport = _RecordingFreeCADTransport()
    result = _invoke_registered(monkeypatch, transport, {'doc_name': 'AgentDocument', 'repairs': [{'object': 'Sketch', 'property': 'Support', 'references': [{'object': 'Pad', 'subelements': ['Face1']}]}]})
    ...
    assert envelope["method"] == "repair_references"
    assert envelope["params"] == {'doc_name': 'AgentDocument', 'repairs': [{'object': 'Sketch', 'property': 'Support', 'references': [{'object': 'Pad', 'subelements': ['Face1']}]}], 'recompute': False, 'validate': False}
    assert envelope["operation"] == {"name": 'Repair references'}
...
def test_public_route_never_turns_rejection_into_success(monkeypatch):
    arguments = dict({'doc_name': 'AgentDocument', 'repairs': [{'object': 'Sketch', 'property': 'Support', 'references': [{'object': 'Pad', 'subelements': ['Face1']}]}]})
    if "repairs" in arguments and isinstance(arguments["repairs"], str):
        arguments["repairs"] = ""
    transport = _RecordingFreeCADTransport(
        {
            "contract_version": 1,
...
def test_public_route_marks_committed_but_invalid_response_non_retryable(monkeypatch):
    ...
    result = _invoke_registered(monkeypatch, transport, {'doc_name': 'AgentDocument', 'repairs': [{'object': 'Sketch', 'property': 'Support', 'references': [{'object': 'Pad', 'subelements': ['Face1']}]}]})
    assert result.isError is True
    assert result.structuredContent["error_code"] == 'REPAIR_REFERENCES_COMMITTED_RESPONSE_INVALID'
