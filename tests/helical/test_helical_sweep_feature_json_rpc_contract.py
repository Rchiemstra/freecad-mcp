"""Agent-to-FreeCAD wire contract for the ``helical_sweep_feature`` MCP operation."""
...
from freecad_mcp.generated.capabilities.register_modules import tools_features_advanced_b
...
_TOOL_PARAMS = {'doc_name': 'AgentDocument', 'profile_sketch': 'Sketch', 'helix_name': 'Helix', 'pitch': 2.0, 'height': 10.0, 'radius': 5.0}
_REQUIRED = {'pitch', 'radius', 'height', 'doc_name', 'helix_name', 'profile_sketch'}
...
class _RecordingFreeCADTransport:
    ...
    def request(self, path, payload, headers):
        ...
        if body_result is _DEFAULT_RESPONSE:
            created = envelope["params"].get('helix_name', 'helix_name')
            body_result = {
                "contract_version": 1,
                "success": True,
                "ok": True,
...
def _invoke_registered(monkeypatch, transport, tool_params):
    ...
    monkeypatch.setattr(tools_features_advanced_b, "server_connection", lambda: connection)
    monkeypatch.setattr(
    ...
    mcp = InstrumentedFastMCP("helical_sweep_feature-contract")
    exports = {}
    tools_features_advanced_b._register_helical_sweep_feature(
        mcp,
        dependencies=object(),
        exports=exports,
    )
    ...
    async def invoke():
        async with create_connected_server_and_client_session(mcp._mcp_server) as client:
            ...
            tool = next(tool for tool in listing.tools if tool.name == "helical_sweep_feature")
            assert set(tool.inputSchema.get("required", [])) == _REQUIRED
            return await client.call_tool("helical_sweep_feature", tool_params)
...
def test_helical_sweep_feature_sends_exact_authenticated_json_rpc_values_to_freecad(monkeypatch):
    ...
    assert envelope["method"] == "helical_sweep_feature"
    assert envelope["session_token"] == "test-session-token"
    assert envelope["operation"] == {"name": 'Helical sweep feature'}
...
def test_public_route_marks_committed_but_invalid_response_non_retryable(monkeypatch):
    ...
    result = _invoke_registered(monkeypatch, transport, dict(_TOOL_PARAMS))
    assert result.isError is True
    assert result.structuredContent["error_code"] == "HELICAL_SWEEP_FEATURE_COMMITTED_RESPONSE_INVALID"
...
def test_mcp_schema_rejects_non_string_before_json_rpc(monkeypatch):
    ...
    string_key = next(key for key in ("doc_name",) if key in params)
    params[string_key] = None
