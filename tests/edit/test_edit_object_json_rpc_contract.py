"""Agent-to-FreeCAD wire contract for the ``edit_object`` MCP operation."""
...
class _RecordingFreeCADTransport:
    ...
    def request(self, path, payload, headers):
        ...
        if body_result is _DEFAULT_RESPONSE:
            ...
            extra = {"object_name": envelope["params"]["obj_name"], "label": envelope["params"]["obj_name"]}
...
def _invoke_registered(monkeypatch, transport, arguments):
    ...
    mcp = InstrumentedFastMCP("edit_object-contract")
    exports = {}
    tools_core_objects._register_edit_object(mcp, dependencies=object(), exports=exports)

    async def invoke():
        async with create_connected_server_and_client_session(mcp._mcp_server) as client:
            listing = await client.list_tools()
            tool = next(tool for tool in listing.tools if tool.name == "edit_object")
            assert set(tool.inputSchema["required"]) == {'obj_name', 'obj_properties', 'doc_name'}
            return await client.call_tool("edit_object", arguments)
...
def test_edit_object_sends_exact_authenticated_json_rpc_values_to_freecad(monkeypatch):
    transport = _RecordingFreeCADTransport()
    result = _invoke_registered(monkeypatch, transport, {'doc_name': 'AgentDocument', 'obj_name': 'Box', 'obj_properties': {'Label': 'Box'}})
...
def test_public_route_never_turns_rejection_into_success(monkeypatch):
    arguments = dict({'doc_name': 'AgentDocument', 'obj_name': 'Box', 'obj_properties': {'Label': 'Box'}})
...
def test_public_route_marks_committed_but_invalid_response_non_retryable(monkeypatch):
    ...
    result = _invoke_registered(monkeypatch, transport, {'doc_name': 'AgentDocument', 'obj_name': 'Box', 'obj_properties': {'Label': 'Box'}})
    assert result.isError is True
    assert result.structuredContent["error_code"] == 'EDIT_OBJECT_COMMITTED_RESPONSE_INVALID'
