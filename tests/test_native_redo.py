...
def test_redo_native_commit_succeeds():
    ...
    document = FreeCAD.newDocument("MCPTypedRedo")
    events: list[str] = []
    saved_path = ""
    try:
        
        if "redo" == "reload_document":
            saved_path = document.Name + ".FCStd"
            document.saveAs(saved_path)
        if "redo" == "open_document":
            saved_path = document.Name + ".FCStd"
            document.saveAs(saved_path)
        ...
        result = run_redo(collab, document.Name)
