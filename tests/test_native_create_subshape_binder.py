"""Native qualification matrix for ``create_subshape_binder``."""
...
def test_create_subshape_binder_native_success_inspects_after_recompute(monkeypatch):
    _require_native()
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops import create_subshape_binder as subject

    document = FreeCAD.newDocument("MCPCreateSubshapeBinderNativeSuccess")
    events = []
...
def test_create_subshape_binder_native_validation_failure_restores_complete_state():
    _require_native()
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops.create_subshape_binder import run_create_subshape_binder

    document = FreeCAD.newDocument("MCPCreateSubshapeBinderNativeRollback")
    _prepare(document)
    ...
    try:
        result = run_create_subshape_binder(
            ...
            document.Name, "Created", "Seed", None, None, "Body", False, True, "error",
        )
...
def test_create_subshape_binder_native_apply_failure_restores(monkeypatch):
    _require_native()
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops import create_subshape_binder as subject

    document = FreeCAD.newDocument("MCPCreateSubshapeBinderNativeApplyFailure")
    _prepare(document)
    state_before = _model_state(document)
    original = subject.apply_create_subshape_binder
    ...
    monkeypatch.setattr(subject, "apply_create_subshape_binder", mutates_then_raises)
    try:
        result = subject.run_create_subshape_binder(_collaborators(FreeCAD, lambda _d: None), document.Name, "Created", "Seed", None, None, "Body", False, True, "error")
        assert result["success"] is False
...
def test_create_subshape_binder_native_recompute_failure_rolls_back(monkeypatch):
    _require_native()
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops import create_subshape_binder as subject

    document = FreeCAD.newDocument("MCPCreateSubshapeBinderNativeRecomputeFailure")
    ...
    monkeypatch.setattr(subject, "apply_create_subshape_binder", arm)
    try:
        result = subject.run_create_subshape_binder(_collaborators(FreeCAD, lambda _d: None), document.Name, "Created", "Seed", None, None, "Body", False, True, "error")
        assert result["success"] is False
...
def test_create_subshape_binder_native_rollback_failure_is_uncertain_and_fences(monkeypatch):
    _require_native()
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops import create_subshape_binder as subject

    document = FreeCAD.newDocument("MCPCreateSubshapeBinderNativeRollbackFailure")
    ...
    try:
        result = subject.run_create_subshape_binder(collaborators, document.Name, "Created", "Seed", None, None, "Body", False, True, "error")
        proxy.armed = False
        fenced = subject.run_create_subshape_binder(collaborators, document.Name, "Created", "Seed", None, None, "Body", False, True, "error")
...
def test_create_subshape_binder_native_inspection_failure_rolls_back(monkeypatch):
    _require_native()
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops import create_subshape_binder as subject

    document = FreeCAD.newDocument("MCPCreateSubshapeBinderNativeInspectionFailure")
    _prepare(document)
    state_before = _model_state(document)

    def reject(_document, _receipt):
        raise subject.CreateSubshapeBinderError("CREATED_OBJECT_WRONG_TYPE", "forced inspection failure")

    monkeypatch.setattr(subject, "read_create_subshape_binder_result", reject)
    try:
        result = subject.run_create_subshape_binder(_collaborators(FreeCAD, lambda _d: None), document.Name, "Created", "Seed", None, None, "Body", False, True, "error")
        assert result["success"] is False
...
def test_create_subshape_binder_native_postcondition_cannot_write():
    _require_native()
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops.create_subshape_binder import run_create_subshape_binder

    document = FreeCAD.newDocument("MCPCreateSubshapeBinderReadOnlyPostcondition")
    ...
    try:
        result = run_create_subshape_binder(_collaborators(FreeCAD, validate), document.Name, "Created", "Seed", None, None, "Body", False, True, "error")
