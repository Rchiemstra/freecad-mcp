...
def _call(collaborators, *extra):
    if extra:
        args = list(('Doc', 'Sketch', 'Pad', 10.0, 'Body'))
        args[2] = extra[0]
        return run_pad_feature(collaborators, *tuple(args))
    return run_pad_feature(collaborators, *('Doc', 'Sketch', 'Pad', 10.0, 'Body'))


def test_pad_feature_runs_apply_recompute_inspect_validate_then_commits():
    events = []
    ...
    assert result["label"] == "Label for Pad"
...
def test_duplicate_or_missing_target_aborts_without_recompute_or_commit():
    events = []
    document = _Document(events)
    document.objects["Pad"] = _target("Pad", "PartDesign::Pad")
    collaborators, _api = _collaborators(document, events)
...
def test_creation_that_changes_then_raises_is_rolled_back():
    ...
    assert result["success"] is False
    assert result["error_code"] == 'PAD_FEATURE_FAILED'
    assert "commit" not in events
...
def test_response_uses_the_actual_name_assigned_by_freecad():
    events = []
    document = _Document(events, assigned_name='Pad001')
    collaborators, _api = _collaborators(document, events)

    result = _call(collaborators)

    assert result["success"] is True
    assert result["pad"] == "Pad001"


def test_pad_feature_has_no_uncoordinated_source_entry_point():
    assert not hasattr(subject, 'pad_feature_gui')
...
def test_unknown_or_contradictory_native_evidence_cannot_release_success(native_result):
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops.pad_feature_mutation import (
        _pad_feature_native_result,
        _NativePadFeatureMutationState,
    )

    result = _pad_feature_native_result(native_result, _NativePadFeatureMutationState(postcondition_passed=True))
    assert result["success"] is False
