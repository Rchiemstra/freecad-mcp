"""Unit coverage for the typed ``measure_angle`` query slice."""
...
from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops.measure_angle import run_measure_angle
from tests.typed_rpc_fakes import FakeDocument, FakeObject, collaborators

pytestmark = pytest.mark.unit


def test_measure_angle_observed_without_native_mutation():
    events: list[str] = []
    ...
    with patch.object(measure_io_actions, "measure_angle", return_value=payload):
        result = run_measure_angle(collab, "Doc", "Box:Edge1", "Box:Edge2")
