"""Agent-to-FreeCAD wire contract for the ``measure_angle`` MCP operation."""
...
class _RecordingFreeCADTransport:
    ...
    def request(self, path, payload, headers):
        ...
        if body_result is _DEFAULT_RESPONSE:
            from tests.angle.test_measure_angle_response import _success
...
def _invoke(monkeypatch, transport, arguments):
    ...
    mcp = InstrumentedFastMCP("measure_angle-contract")
    exports = {}
    tools_measure_a._register_measure_angle(mcp, dependencies=object(), exports=exports)

    async def invoke():
        async with create_connected_server_and_client_session(mcp._mcp_server) as client:
            listing = await client.list_tools()
            tool = next(tool for tool in listing.tools if tool.name == "measure_angle")
            return await client.call_tool("measure_angle", arguments)
...
def test_measure_angle_sends_authenticated_json_rpc_to_freecad(monkeypatch):
    ...
    assert envelope["method"] == "measure_angle"
    assert envelope["params"]["doc_name"] == "AgentDocument"
    assert envelope["params"]["edge1_ref"] == "Box:Edge1"
