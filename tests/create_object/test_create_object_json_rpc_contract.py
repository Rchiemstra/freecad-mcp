"""Agent-to-FreeCAD wire contract for the ``create_object`` MCP operation."""
...
from freecad_mcp.generated.capabilities.register_modules import tools_core_document
...
class _RecordingFreeCADTransport:
    ...
    def request(self, path, payload, headers):
        ...
        if body_result is _DEFAULT_RESPONSE:
            ...
            data = envelope['params']['obj_data']
            extra = {"object_name": data["Name"], "object_type": data["Type"], "label": data["Name"]}
...
def _invoke_registered(monkeypatch, transport, arguments):
    ...
    monkeypatch.setattr(tools_core_document, "server_connection", lambda: connection)
    if hasattr(tools_core_document, "server_state"):
        monkeypatch.setattr(
            tools_core_document,
            "server_state",
    ...
    mcp = InstrumentedFastMCP("create_object-contract")
    exports = {}
    tools_core_document._register_create_object(mcp, dependencies=object(), exports=exports)

    async def invoke():
        async with create_connected_server_and_client_session(mcp._mcp_server) as client:
            listing = await client.list_tools()
            tool = next(tool for tool in listing.tools if tool.name == "create_object")
            assert set(tool.inputSchema["required"]) == {'obj_name', 'doc_name', 'obj_type'}
            return await client.call_tool("create_object", arguments)
...
def test_create_object_sends_exact_authenticated_json_rpc_values_to_freecad(monkeypatch):
    transport = _RecordingFreeCADTransport()
    result = _invoke_registered(monkeypatch, transport, {'doc_name': 'AgentDocument', 'obj_type': 'Part::Box', 'obj_name': 'Box'})
    ...
    assert envelope["method"] == "create_object"
    assert envelope["params"] == {'doc_name': 'AgentDocument', 'obj_data': {'Name': 'Box', 'Type': 'Part::Box'}}
    assert envelope["operation"] == {"name": 'Create object'}
...
def test_public_route_never_turns_rejection_into_success(monkeypatch):
    arguments = dict({'doc_name': 'AgentDocument', 'obj_type': 'Part::Box', 'obj_name': 'Box'})
    if "obj_name" in arguments and isinstance(arguments["obj_name"], str):
        arguments["obj_name"] = ""
    transport = _RecordingFreeCADTransport(
        {
            "contract_version": 1,
...
def test_public_route_marks_committed_but_invalid_response_non_retryable(monkeypatch):
    ...
    result = _invoke_registered(monkeypatch, transport, {'doc_name': 'AgentDocument', 'obj_type': 'Part::Box', 'obj_name': 'Box'})
    assert result.isError is True
    assert result.structuredContent["error_code"] == 'CREATE_OBJECT_COMMITTED_RESPONSE_INVALID'
