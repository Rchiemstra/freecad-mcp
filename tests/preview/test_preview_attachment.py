"""Unit coverage for the typed ``preview_attachment`` mutation slice."""
...
def test_preview_attachment_runs_apply_recompute_inspect_validate_then_commits():
    events: list[str] = []
...
def test_creation_that_changes_then_raises_is_rolled_back(monkeypatch):
    ...
    monkeypatch.setattr(subject, "apply_preview_attachment", mutate_then_raise)
    result = run_preview_attachment(collaborators, "Doc", "Target")
    assert result["success"] is False
...
def test_unknown_or_contradictory_native_evidence_cannot_release_success():
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops.preview_attachment_mutation import (
        _preview_attachment_native_result,
    ...
    for native_result in (
        ...
        result = _preview_attachment_native_result(native_result, _NativeMutationState(postcondition_passed=True))
        assert result["success"] is False
