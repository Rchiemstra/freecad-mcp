"""Agent-to-FreeCAD wire contract for the ``sketch_import_points`` MCP operation."""
...
_TOOL_PARAMS = {'doc_name': 'AgentDocument', 'sketch_name': 'Sketch', 'points': [{'x': 1.0, 'y': 2.0}, {'x': 3.0, 'y': 4.0}], 'construction': False}
...
def _invoke_registered(monkeypatch, transport, arguments):
    ...
    mcp = InstrumentedFastMCP("sketch_import_points-contract")
    exports = {}
    tools_sketch_curves_b._register_sketch_import_points(mcp, dependencies=object(), exports=exports)

    async def invoke():
        async with create_connected_server_and_client_session(mcp._mcp_server) as client:
            listing = await client.list_tools()
            tool = next(tool for tool in listing.tools if tool.name == "sketch_import_points")
            assert set(tool.inputSchema["required"]) == {'doc_name', 'sketch_name', 'points'}
            return await client.call_tool("sketch_import_points", arguments)
...
def test_sketch_import_points_sends_exact_authenticated_json_rpc_values_to_freecad(monkeypatch):
    ...
    assert envelope["method"] == "sketch_import_points"
