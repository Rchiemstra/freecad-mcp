"""Unit coverage for the typed ``translate`` mutation slice."""
...
from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops import translate as subject
from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops.translate import run_translate
...
def _fake_payload(document, obj_name, dx, dy, dz, **kwargs):
...
def _collaborators(document, events, *, validator=None, final_result=None, monkeypatch=None):
    if monkeypatch is not None:
        monkeypatch.setattr(measure_io_actions, "translate", _fake_payload)
...
def test_translate_runs_apply_recompute_inspect_validate_then_commits(monkeypatch):
    events = []
    document = _Document(events)
    collaborators, api = _collaborators(document, events, monkeypatch=monkeypatch)
    result = run_translate(collaborators, "Doc", "Box", 1.0, 0.0, 0.0)
...
def test_invalid_names_abort_without_recompute_or_commit(monkeypatch):
    ...
    result = run_translate(collaborators, "Doc", "", 1.0, 0.0, 0.0)
...
def test_translate_has_typed_rpc_handler():
    assert subject.TYPED_RPC_HANDLER[0] == "translate"
    assert callable(subject.TYPED_RPC_HANDLER[1])


def test_unknown_native_evidence_cannot_release_success():
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops.translate_mutation import (
        _translate_native_result,
        _NativeMutationState,
    )
    result = _translate_native_result({"status": "FutureStatus", "committed": False}, _NativeMutationState(postcondition_passed=True))
    assert result["success"] is False
