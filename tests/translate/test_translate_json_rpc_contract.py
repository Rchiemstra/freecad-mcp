"""Agent-to-FreeCAD wire contract for the ``translate`` MCP operation."""
...
from freecad_mcp._shared.protocol.translate_contract import make_translate_success
...
class _RecordingFreeCADTransport:
    ...
    def request(self, path, payload, headers):
        ...
        if body_result is _DEFAULT_RESPONSE:
            from tests.translate.test_translate_response import _success
...
def _invoke(monkeypatch, transport, arguments):
    ...
    mcp = InstrumentedFastMCP("translate-contract")
    exports = {}
    tools_transform._register_translate(mcp, dependencies=object(), exports=exports)

    async def invoke():
        async with create_connected_server_and_client_session(mcp._mcp_server) as client:
            listing = await client.list_tools()
            tool = next(tool for tool in listing.tools if tool.name == "translate")
            return await client.call_tool("translate", arguments)
...
def test_translate_sends_authenticated_json_rpc_to_freecad(monkeypatch):
    ...
    if "translate" == "create_assembly_grounded_joint":
        arguments.update({"assembly_name": "Assembly", "component_name": "Base"})
    elif "translate" == "create_assembly_joint":
        arguments.update({
