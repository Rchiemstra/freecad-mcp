"""Agent-to-FreeCAD wire contract for the ``create_placement_datum`` MCP operation."""
...
class _RecordingFreeCADTransport:
    ...
    def request(self, path, payload, headers):
        ...
        if body_result is _DEFAULT_RESPONSE:
            body_result = dict({"contract_version": 1, "success": True, "ok": True, "outcome": "committed", "committed": True, "retry_safe": False, "datum_name": "Value"}, **{
                key: envelope["params"][key]
                for key in envelope["params"]
                if key in ['datum_name']
            })
            body_result = {"contract_version": 1, "success": True, "ok": True, "outcome": "committed", "committed": True, "retry_safe": False, "datum_name": "Value"}
        response = {
            "jsonrpc": "2.0",
            "id": request["id"],
            "result": {
...
def _invoke_registered(monkeypatch, transport, params):
    ...
    mcp = InstrumentedFastMCP("create_placement_datum-contract")
    exports = {}
    tools_partdesign_b2._register_create_placement_datum(mcp, dependencies=object(), exports=exports)

    async def invoke():
        async with create_connected_server_and_client_session(mcp._mcp_server) as client:
            listing = await client.list_tools()
            tool = next(tool for tool in listing.tools if tool.name == "create_placement_datum")
            required = tool.inputSchema.get("required") or []
            assert "doc_name" in required
            return await client.call_tool("create_placement_datum", params)
...
def test_create_placement_datum_sends_authenticated_json_rpc_to_freecad(monkeypatch):
    transport = _RecordingFreeCADTransport()
    result = _invoke_registered(monkeypatch, transport, {"doc_name": "AgentDocument", "owner_body": "Value", "name": "Value", "source": "Value", "relative": True})
    ...
    assert envelope["method"] == "create_placement_datum"
...
def test_mcp_schema_rejects_non_string_doc_name_before_json_rpc(monkeypatch):
    transport = _RecordingFreeCADTransport()
    params = dict({"doc_name": "AgentDocument", "owner_body": "Value", "name": "Value", "source": "Value", "relative": True})
    params["doc_name"] = 42
