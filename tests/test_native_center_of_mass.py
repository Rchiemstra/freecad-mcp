...
def test_center_of_mass_native_success_inspects_after_recompute(monkeypatch):
    ...
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops import center_of_mass as module

    document = FreeCAD.newDocument("MCPCenterOfMassNative")
    events: list[str] = []
    ...
    monkeypatch.setattr(module, "apply_center_of_mass", tracked_apply)
    collaborators = _collaborators(FreeCAD, lambda _document: events.append("validate"))
    try:
        result = module.run_center_of_mass(collaborators, document.Name, "Box")
...
def test_center_of_mass_native_validation_failure_restores(monkeypatch):
    _require_native_collaboration()
    import FreeCAD
    from addon.FreeCADMCP.rpc_server.methods.cad_methods_ops import center_of_mass as module

    document = FreeCAD.newDocument("MCPCenterOfMassNativeRollback")
    original_apply = module.apply_center_of_mass

    def tracked_apply(admitted_document, request):
        obj = admitted_document.addObject("App::FeaturePython", "RejectedNative")
        return original_apply(admitted_document, request) if False else module.CenterOfMassReceipt(
...
def test_center_of_mass_native_missing_document_keeps_typed_error():
    ...
    result = run_center_of_mass(collaborators, "MissingNativeDoc", "Box")
    assert result["success"] is False
