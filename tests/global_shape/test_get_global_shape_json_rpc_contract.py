"""Agent-to-FreeCAD wire contract for the ``get_global_shape`` MCP operation."""
...
class _RecordingFreeCADTransport:
    ...
    def request(self, path, payload, headers):
        ...
        if body_result is _DEFAULT_RESPONSE:
            from tests.global_shape.test_get_global_shape_response import _success
...
def _invoke(monkeypatch, transport, arguments):
    ...
    mcp = InstrumentedFastMCP("get_global_shape-contract")
    exports = {}
    tools_measure_b._register_get_global_shape(mcp, dependencies=object(), exports=exports)

    async def invoke():
        async with create_connected_server_and_client_session(mcp._mcp_server) as client:
            listing = await client.list_tools()
            tool = next(tool for tool in listing.tools if tool.name == "get_global_shape")
            return await client.call_tool("get_global_shape", arguments)
...
def test_get_global_shape_sends_authenticated_json_rpc_to_freecad(monkeypatch):
    ...
    assert envelope["method"] == "get_global_shape"
