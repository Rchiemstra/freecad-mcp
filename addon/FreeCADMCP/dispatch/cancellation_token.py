...
from .request_cancellation_error import (
...
class CancellationToken:
    ...
    def __init__(self, session_id: str, request_id: str, method: str) -> None:
        ...
        self._cancellation_resolution: _Any = None
        self._cancellation_resolution_complete = _threading.Event()
        self._cancellation_begin_claimed = False
        self._cancellation_begin_complete = _threading.Event()
        self._recovery_incident_id: str | None = None
        self._lock = _threading.RLock()

    def _snapshot_locked(self) -> _InflightSnapshot:
    ...
    def snapshot(self) -> _InflightSnapshot:
        with self._lock:
            return self._snapshot_locked()

    def set_phase(self, phase: str) -> _InflightSnapshot:
        with self._lock:
            if not self._terminal:
                self._phase = str(phase)[:128]
            return self._snapshot_locked()

    def checkpoint(self, phase: str) -> _InflightSnapshot:
        ...
        if snapshot.cancellation_requested:
            raise _RequestCancellationError(snapshot)
        return snapshot

    def request_cancel(self) -> tuple[bool, _InflightSnapshot]:
        with self._lock:
            newly_requested = (
            ...
            if not self._terminal and self._accepting_cancellation:
                ...
                if self._cancel_requested_at is None:
                    self._cancel_requested_at = _time.monotonic()
            return newly_requested, self._snapshot_locked()

    def mark_mutation_started(self, phase: str = "mutation_started") -> _InflightSnapshot:
    ...
    def begin_mutation(self, phase: str = "mutation_started") -> _InflightSnapshot:
        """Atomically reject cancellation or cross the may-mutate boundary."""

        with self._lock:
            if self._cancellation_requested:
                raise _RequestCancellationError(self._snapshot_locked())
            self._mutation_started = True
            self._phase = str(phase)[:128]
            return self._snapshot_locked()

    def begin_irreversible(self, phase: str) -> _InflightSnapshot:
    ...
    def mark_uncertain(self, phase: str = "completion_uncertain") -> _InflightSnapshot:
        with self._lock:
            self._uncertain = True
            self._phase = str(phase)[:128]
            if self._recovery_incident_id is None:
                self._recovery_incident_id = str(_uuid.uuid4())
            return self._snapshot_locked()

    def mark_recovered(self, phase: str = "recovery_completed") -> _InflightSnapshot:
        with self._lock:
            self._uncertain = False
            self._phase = str(phase)[:128]
            return self._snapshot_locked()

    def claim_cancellation_resolution(self) -> tuple[bool, _Any]:
        """Permit exactly one caller to perform lease/CAS cancellation work."""

        with self._lock:
            if self._cancellation_resolved:
                return False, _copy.deepcopy(self._cancellation_resolution)
    ...
    def finish_cancellation_resolution(self, result: _Any) -> _Any:
        with self._lock:
            if not self._cancellation_resolved:
                self._cancellation_resolution = _copy.deepcopy(result)
            self._cancellation_resolved = True
            self._cancellation_resolving = False
            self._maybe_terminal_locked()
            resolved = _copy.deepcopy(self._cancellation_resolution)
    ...
    def cancellation_resolution(self) -> _Any:
        with self._lock:
            return _copy.deepcopy(self._cancellation_resolution)

    def begin_gui_phase(self, phase: str) -> _InflightSnapshot:
        with self._lock:
            self._active_gui_phases += 1
            self._phase = str(phase)[:128]
            return self._snapshot_locked()

    def end_gui_phase(self) -> _InflightSnapshot:
        with self._lock:
    ...
    def finish_handler(self, status: str) -> _InflightSnapshot:
        with self._lock:
