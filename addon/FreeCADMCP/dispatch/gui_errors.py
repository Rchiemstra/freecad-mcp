"""Errors reported by the GUI dispatch boundary.

This module is deliberately standard-library-only.  GUI-framework adapters
may translate scheduling and thread ownership into this stable error surface.
"""
...
class GuiDispatchError(RuntimeError):
    """Base error raised to an RPC handler by GUI dispatch."""
...
class GuiBusyAfterTimeout(GuiDispatchError):
    """A timed-out request is still occupying the GUI owner thread."""

    error_code = "GUI_BUSY_AFTER_TIMEOUT"


class GuiTaskError(GuiDispatchError):
    """A callable raised while executing on the GUI owner thread."""
