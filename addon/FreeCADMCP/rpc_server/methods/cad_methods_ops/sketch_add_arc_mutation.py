"""Fixed native transaction policy for the typed ``sketch_add_arc`` mutation."""
...
class SketchAddArcError(RuntimeError):
    def __init__(self, code: str, message: str) -> None:
        super().__init__(message)
        self.code = code


class _AbortSketchAddArcMutation(RuntimeError):
    """Ask the native coordinator to roll back a failed apply callback."""


@dataclass(slots=True)
class _NativeSketchAddArcMutationState:
    document: SketchDocument | None = None
    failure: SketchAddArcError | None = None
...
def _sketch_add_arc_native_result(
    native_result: object, state: _NativeSketchAddArcMutationState
) -> Literal[True] | SketchAddArcFailure | SketchAddArcUncertain:
    if not isinstance(native_result, dict):
        return make_sketch_add_arc_uncertain(
            "INVALID_NATIVE_SKETCH_ADD_ARC_RESULT",
    ...
    message = message_value if isinstance(message_value, str) else "Native sketch_add_arc mutation failed"
    ...
    if rollback_failed:
        return make_sketch_add_arc_uncertain(
            "SKETCH_ADD_ARC_ROLLBACK_UNCERTAIN",
            message,
            committed=True if committed is True else None,
            native_status=status,
    ...
    if status == "Committed" and committed is True and "rollback_succeeded" not in native_result:
        ...
        return make_sketch_add_arc_uncertain(
            ...
            "Native commit completed without a successful sketch_add_arc postcondition",
            committed=True,
            native_status=status,
        )
...
def run_sketch_add_arc_native_mutation(
    ...
    def native_apply(document: object) -> None:
        ...
        try:
            ...
            state.failure = (
                exc
                if isinstance(exc, SketchAddArcError)
                else SketchAddArcError("SKETCH_ADD_ARC_FAILED", str({exc}) or type({exc}).__name__)
            )
            raise _AbortSketchAddArcMutation from exc
    ...
    def native_postcondition(document: object) -> bool:
        ...
        if state.document is not document or state.failure is not None:
            state.failure = SketchAddArcError(
        ...
        try:
            ...
            state.failure = (
                exc
                if isinstance(exc, SketchAddArcError)
                else SketchAddArcError("SKETCH_ADD_ARC_RESULT_FAILED", str({exc}) or type({exc}).__name__)
            )
            return False
        try:
    ...
    try:
        ...
        return make_sketch_add_arc_uncertain(
            "SKETCH_ADD_ARC_NATIVE_EXCEPTION",
            str({exc}) or type({exc}).__name__,
            committed=None,
        )
    except Exception as exc:
    ...
    return _sketch_add_arc_native_result(native_result, state)
