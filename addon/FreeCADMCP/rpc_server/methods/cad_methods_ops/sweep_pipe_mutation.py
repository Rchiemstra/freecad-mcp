"""Fixed native transaction policy for the ``sweep_pipe`` mutation.
...
class _NativeMutationState:
    document: SweepPipeDocument | None = None
    failure: SweepPipeError | None = None
...
def _sweep_pipe_native_result(
    ...
    if not isinstance(native_result, dict):
        return make_sweep_pipe_uncertain(
            "INVALID_NATIVE_SWEEP_PIPE_RESULT",
    ...
    message = message_value if isinstance(message_value, str) else "Native sweep_pipe mutation failed"
...
def run_sweep_pipe_native_mutation(
    ...
    def native_apply(document: object) -> None:
        typed = cast(SweepPipeDocument, document)
        ...
        try:
            ...
            state.failure = (
                exc
                if isinstance(exc, SweepPipeError)
                else SweepPipeError("SWEEP_PIPE_FAILED", str(exc) or type(exc).__name__)
            )
            raise _AbortMutation from exc

    def native_postcondition(document: object) -> bool:
        typed = cast(SweepPipeReadDocument, document)
        if state.document is not typed or state.failure is not None:
            state.failure = SweepPipeError(
        ...
        try:
            ...
            state.failure = (
                exc
                if isinstance(exc, SweepPipeError)
                else SweepPipeError("SWEEP_PIPE_RESULT_FAILED", str(exc) or type(exc).__name__)
            )
            return False
        try:
