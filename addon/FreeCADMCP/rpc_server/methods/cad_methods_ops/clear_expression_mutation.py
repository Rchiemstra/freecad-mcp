"""Fixed native transaction policy for the ``clear_expression`` mutation.
...
class _NativeMutationState:
    document: ClearExpressionDocument | None = None
    failure: ClearExpressionError | None = None
...
def _clear_expression_native_result(
    native_result: object, state: _NativeMutationState
) -> Literal[True] | ClearExpressionFailure | ClearExpressionUncertain:
    if not isinstance(native_result, dict):
        return make_clear_expression_uncertain(
            "INVALID_NATIVE_CLEAR_EXPRESSION_RESULT",
    ...
    message = message_value if isinstance(message_value, str) else "Native clear_expression mutation failed"
    ...
    if rollback_failed:
        return make_clear_expression_uncertain(
            "CLEAR_EXPRESSION_ROLLBACK_UNCERTAIN",
            message,
            committed=True if committed is True else None,
            native_status=status,
...
def run_clear_expression_native_mutation(
    collaborators: ClearExpressionCollaborators,
    document_name: DocumentName,
    apply: Callable[[ClearExpressionDocument], None],
    postcondition: Callable[[ClearExpressionReadDocument], None],
) -> Literal[True] | ClearExpressionFailure | ClearExpressionUncertain:
    ...
    def native_apply(document: object) -> None:
        typed = cast(ClearExpressionDocument, document)
        ...
        try:
            ...
            state.failure = (
                exc
                if isinstance(exc, ClearExpressionError)
                else ClearExpressionError("CLEAR_EXPRESSION_FAILED", str(exc) or type(exc).__name__)
            )
            raise _AbortMutation from exc

    def native_postcondition(document: object) -> bool:
        typed = cast(ClearExpressionReadDocument, document)
        if state.document is not typed or state.failure is not None:
            state.failure = ClearExpressionError(
        ...
        try:
            ...
            state.failure = (
                exc
                if isinstance(exc, ClearExpressionError)
                else ClearExpressionError("CLEAR_EXPRESSION_RESULT_FAILED", str(exc) or type(exc).__name__)
            )
            return False
        try:
    ...
    try:
        ...
        return make_clear_expression_uncertain(
            "CLEAR_EXPRESSION_NATIVE_EXCEPTION",
            str(exc) or type(exc).__name__,
            committed=None,
        )
    except Exception as exc:
    ...
    return _clear_expression_native_result(native_result, state)
