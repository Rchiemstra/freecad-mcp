"""Native transaction policy for the typed ``create_object`` mutation."""
...
class _NativeMutationState:
    ...
    failure: CreateObjectError | None = None
...
def _create_object_native_result(
    ...
    if not isinstance(native_result, dict):
        return make_create_object_uncertain(
            "INVALID_NATIVE_CREATE_OBJECT_RESULT",
    ...
    message = message_value if isinstance(message_value, str) else "Native create_object mutation failed"
    ...
    if rollback_failed:
        return make_create_object_uncertain(
            "CREATE_OBJECT_ROLLBACK_UNCERTAIN",
            message,
            committed=True if committed is True else None,
            native_status=status,
    ...
    if status == "Committed" and committed is True and "rollback_succeeded" not in native_result:
        ...
        return make_create_object_uncertain(
            ...
            "Native commit completed without a successful create_object postcondition",
            committed=True,
            native_status=status,
        )
...
def run_create_object_native_mutation(
    ...
    def native_apply(document: object) -> None:
        ...
        try:
            ...
            state.failure = (
                exc
                if isinstance(exc, CreateObjectError)
                else CreateObjectError("CREATE_OBJECT_FAILED", str(exc) or type(exc).__name__)
            )
    ...
    def native_postcondition(document: object) -> bool:
        if state.document is not document or state.failure is not None:
            state.failure = CreateObjectError(
        ...
        try:
            ...
            state.failure = (
                exc
                if isinstance(exc, CreateObjectError)
                else CreateObjectError("CREATE_OBJECT_RESULT_FAILED", str(exc) or type(exc).__name__)
            )
            return False
        try:
            collaborators.validate_document_invariants(cast(CreateObjectReadDocument, document))
        except Exception as exc:
            state.failure = CreateObjectError(
    ...
    try:
        ...
        return make_create_object_uncertain(
            "CREATE_OBJECT_NATIVE_EXCEPTION",
            str(exc) or type(exc).__name__,
            committed=None,
        )
    except Exception as exc:
    ...
    return _create_object_native_result(native_result, state)
