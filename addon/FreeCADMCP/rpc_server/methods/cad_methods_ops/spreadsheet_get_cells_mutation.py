"""Fixed native transaction policy for the ``spreadsheet_get_cells`` mutation.
...
class _NativeMutationState:
    document: SpreadsheetGetCellsDocument | None = None
    failure: SpreadsheetGetCellsError | None = None
...
def _spreadsheet_get_cells_native_result(
    native_result: object, state: _NativeMutationState
) -> Literal[True] | SpreadsheetGetCellsFailure | SpreadsheetGetCellsUncertain:
    if not isinstance(native_result, dict):
        return make_spreadsheet_get_cells_uncertain(
            "INVALID_NATIVE_SPREADSHEET_GET_CELLS_RESULT",
    ...
    message = message_value if isinstance(message_value, str) else "Native spreadsheet_get_cells mutation failed"
    ...
    if rollback_failed:
        return make_spreadsheet_get_cells_uncertain(
            "SPREADSHEET_GET_CELLS_ROLLBACK_UNCERTAIN",
            message,
            committed=True if committed is True else None,
            native_status=status,
...
def run_spreadsheet_get_cells_native_mutation(
    collaborators: SpreadsheetGetCellsCollaborators,
    document_name: DocumentName,
    apply: Callable[[SpreadsheetGetCellsDocument], None],
    postcondition: Callable[[SpreadsheetGetCellsReadDocument], None],
) -> Literal[True] | SpreadsheetGetCellsFailure | SpreadsheetGetCellsUncertain:
    ...
    def native_apply(document: object) -> None:
        typed = cast(SpreadsheetGetCellsDocument, document)
        ...
        try:
            ...
            state.failure = (
                exc
                if isinstance(exc, SpreadsheetGetCellsError)
                else SpreadsheetGetCellsError("SPREADSHEET_GET_CELLS_FAILED", str(exc) or type(exc).__name__)
            )
            raise _AbortMutation from exc

    def native_postcondition(document: object) -> bool:
        typed = cast(SpreadsheetGetCellsReadDocument, document)
        if state.document is not typed or state.failure is not None:
            state.failure = SpreadsheetGetCellsError(
        ...
        try:
            ...
            state.failure = (
                exc
                if isinstance(exc, SpreadsheetGetCellsError)
                else SpreadsheetGetCellsError("SPREADSHEET_GET_CELLS_RESULT_FAILED", str(exc) or type(exc).__name__)
            )
            return False
        try:
    ...
    try:
        ...
        return make_spreadsheet_get_cells_uncertain(
            "SPREADSHEET_GET_CELLS_NATIVE_EXCEPTION",
            str(exc) or type(exc).__name__,
            committed=None,
        )
    except Exception as exc:
    ...
    return _spreadsheet_get_cells_native_result(native_result, state)
