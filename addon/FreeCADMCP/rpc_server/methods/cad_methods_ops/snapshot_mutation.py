"""Fixed native transaction policy for the ``snapshot`` mutation.
...
class _NativeMutationState:
    document: SnapshotDocument | None = None
...
def _snapshot_native_result(
    native_result: object, state: _NativeMutationState
) -> Literal[True] | SnapshotFailure | SnapshotUncertain:
    if not isinstance(native_result, dict):
        return make_snapshot_uncertain(
            "INVALID_NATIVE_SNAPSHOT_RESULT",
    ...
    message = message_value if isinstance(message_value, str) else "Native snapshot mutation failed"
...
def run_snapshot_native_mutation(
    collaborators: SnapshotCollaborators,
    document_name: DocumentName,
    apply: Callable[[SnapshotDocument], None],
    postcondition: Callable[[SnapshotReadDocument], None],
) -> Literal[True] | SnapshotFailure | SnapshotUncertain:
    ...
    def native_apply(document: object) -> None:
        typed = cast(SnapshotDocument, document)
    ...
    def native_postcondition(document: object) -> bool:
        ...
        try:
            ...
            state.failure = (
                exc
                if isinstance(exc, SnapshotError)
                else SnapshotError("SNAPSHOT_RESULT_FAILED", str(exc) or type(exc).__name__)
            )
            return False
        try:
    ...
    return _snapshot_native_result(native_result, state)
