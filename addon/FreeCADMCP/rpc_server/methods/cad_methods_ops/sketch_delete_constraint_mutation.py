"""Fixed native transaction policy for the typed ``sketch_delete_constraint`` mutation.
...
class SketchDeleteConstraintError(RuntimeError):
...
class _AbortSketchDeleteConstraintMutation(RuntimeError):
    """Ask the native coordinator to roll back a failed apply callback."""


@dataclass(slots=True)
class _NativeSketchDeleteConstraintMutationState:
    document: object | None = None
    failure: SketchDeleteConstraintError | None = None
...
def _sketch_delete_constraint_native_result(
    native_result: object, state: _NativeSketchDeleteConstraintMutationState
) -> Literal[True] | SketchDeleteConstraintFailure | SketchDeleteConstraintUncertain:
    if not isinstance(native_result, dict):
        return make_sketch_delete_constraint_uncertain(
            "INVALID_NATIVE_SKETCH_DELETE_CONSTRAINT_RESULT",
    ...
    message = message_value if isinstance(message_value, str) else "Native sketch_delete_constraint mutation failed"
    ...
    if rollback_failed:
        return make_sketch_delete_constraint_uncertain(
            "SKETCH_DELETE_CONSTRAINT_ROLLBACK_UNCERTAIN",
            message,
            committed=True if committed is True else None,
            native_status=status,
    ...
    if status == "Committed" and committed is True and "rollback_succeeded" not in native_result:
        ...
        return make_sketch_delete_constraint_uncertain(
            ...
            "Native commit completed without a successful sketch_delete_constraint postcondition",
            committed=True,
            native_status=status,
        )
...
def run_sketch_delete_constraint_native_mutation(
    collaborators: SketchDeleteConstraintCollaborators,
    document_name: str,
    apply: Callable[[SketchDeleteConstraintDocument], None],
    postcondition: Callable[[SketchDeleteConstraintReadDocument], None],
) -> Literal[True] | SketchDeleteConstraintFailure | SketchDeleteConstraintUncertain:
    """Apply, recompute, inspect and validate on the same admitted document."""

    state = _NativeSketchDeleteConstraintMutationState()

    def native_apply(document: object) -> None:
        state.document = document
        try:
            apply(cast(SketchDeleteConstraintDocument, document))
        except Exception as exc:
            state.failure = (
                exc
                if isinstance(exc, SketchDeleteConstraintError)
                else SketchDeleteConstraintError("SKETCH_DELETE_CONSTRAINT_FAILED", str(exc) or type(exc).__name__)
            )
            raise _AbortSketchDeleteConstraintMutation from exc

    def native_postcondition(document: object) -> bool:
        if state.document is not document or state.failure is not None:
            state.failure = SketchDeleteConstraintError(
                "DOCUMENT_IDENTITY_MISMATCH",
                "Native mutation callbacks did not receive the same admitted document",
            )
            return False
        admitted = cast(SketchDeleteConstraintReadDocument, document)
        try:
            ...
            state.failure = (
                exc
                if isinstance(exc, SketchDeleteConstraintError)
                else SketchDeleteConstraintError("SKETCH_DELETE_CONSTRAINT_RESULT_FAILED", str(exc) or type(exc).__name__)
            )
            return False
        try:
    ...
    try:
        ...
        return make_sketch_delete_constraint_uncertain(
            "SKETCH_DELETE_CONSTRAINT_NATIVE_EXCEPTION",
            str(exc) or type(exc).__name__,
            committed=None,
        )
    except Exception as exc:
    ...
    return _sketch_delete_constraint_native_result(native_result, state)
