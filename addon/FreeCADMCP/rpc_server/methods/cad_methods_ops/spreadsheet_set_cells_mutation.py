"""Fixed native transaction policy for the ``spreadsheet_set_cells`` mutation.
...
class _NativeMutationState:
    document: SpreadsheetSetCellsDocument | None = None
    failure: SpreadsheetSetCellsError | None = None
...
def _spreadsheet_set_cells_native_result(
    native_result: object, state: _NativeMutationState
) -> Literal[True] | SpreadsheetSetCellsFailure | SpreadsheetSetCellsUncertain:
    if not isinstance(native_result, dict):
        return make_spreadsheet_set_cells_uncertain(
            "INVALID_NATIVE_SPREADSHEET_SET_CELLS_RESULT",
    ...
    message = message_value if isinstance(message_value, str) else "Native spreadsheet_set_cells mutation failed"
    ...
    if rollback_failed:
        return make_spreadsheet_set_cells_uncertain(
            "SPREADSHEET_SET_CELLS_ROLLBACK_UNCERTAIN",
            message,
            committed=True if committed is True else None,
            native_status=status,
...
def run_spreadsheet_set_cells_native_mutation(
    collaborators: SpreadsheetSetCellsCollaborators,
    document_name: DocumentName,
    apply: Callable[[SpreadsheetSetCellsDocument], None],
    postcondition: Callable[[SpreadsheetSetCellsReadDocument], None],
) -> Literal[True] | SpreadsheetSetCellsFailure | SpreadsheetSetCellsUncertain:
    ...
    def native_apply(document: object) -> None:
        typed = cast(SpreadsheetSetCellsDocument, document)
        ...
        try:
            ...
            state.failure = (
                exc
                if isinstance(exc, SpreadsheetSetCellsError)
                else SpreadsheetSetCellsError("SPREADSHEET_SET_CELLS_FAILED", str(exc) or type(exc).__name__)
            )
            raise _AbortMutation from exc

    def native_postcondition(document: object) -> bool:
        typed = cast(SpreadsheetSetCellsReadDocument, document)
        if state.document is not typed or state.failure is not None:
            state.failure = SpreadsheetSetCellsError(
        ...
        try:
            ...
            state.failure = (
                exc
                if isinstance(exc, SpreadsheetSetCellsError)
                else SpreadsheetSetCellsError("SPREADSHEET_SET_CELLS_RESULT_FAILED", str(exc) or type(exc).__name__)
            )
            return False
        try:
    ...
    try:
        ...
        return make_spreadsheet_set_cells_uncertain(
            "SPREADSHEET_SET_CELLS_NATIVE_EXCEPTION",
            str(exc) or type(exc).__name__,
            committed=None,
        )
    except Exception as exc:
    ...
    return _spreadsheet_set_cells_native_result(native_result, state)
