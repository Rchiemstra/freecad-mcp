"""Fixed native transaction policy for the typed ``sketch_constrain_perpendicular`` mutation."""
...
class SketchConstrainPerpendicularError(RuntimeError):
    def __init__(self, code: str, message: str) -> None:
        super().__init__(message)
        self.code = code


class _AbortSketchConstrainPerpendicularMutation(RuntimeError):
    """Ask the native coordinator to roll back a failed apply callback."""


@dataclass(slots=True)
class _NativeSketchConstrainPerpendicularMutationState:
    document: SketchDocument | None = None
    failure: SketchConstrainPerpendicularError | None = None
...
def _sketch_constrain_perpendicular_native_result(
    native_result: object, state: _NativeSketchConstrainPerpendicularMutationState
) -> Literal[True] | SketchConstrainPerpendicularFailure | SketchConstrainPerpendicularUncertain:
    if not isinstance(native_result, dict):
        return make_sketch_constrain_perpendicular_uncertain(
            "INVALID_NATIVE_SKETCH_CONSTRAIN_PERPENDICULAR_RESULT",
    ...
    message = message_value if isinstance(message_value, str) else "Native sketch_constrain_perpendicular mutation failed"
    ...
    if rollback_failed:
        return make_sketch_constrain_perpendicular_uncertain(
            "SKETCH_CONSTRAIN_PERPENDICULAR_ROLLBACK_UNCERTAIN",
            message,
            committed=True if committed is True else None,
            native_status=status,
    ...
    if status == "Committed" and committed is True and "rollback_succeeded" not in native_result:
        ...
        return make_sketch_constrain_perpendicular_uncertain(
            ...
            "Native commit completed without a successful sketch_constrain_perpendicular postcondition",
            committed=True,
            native_status=status,
        )
...
def run_sketch_constrain_perpendicular_native_mutation(
    ...
    state = _NativeSketchConstrainPerpendicularMutationState()
    ...
    def native_apply(document: object) -> None:
        ...
        try:
            ...
            state.failure = (
                exc
                if isinstance(exc, SketchConstrainPerpendicularError)
                else SketchConstrainPerpendicularError("SKETCH_CONSTRAIN_PERPENDICULAR_FAILED", str(exc) or type(exc).__name__)
            )
            raise _AbortSketchConstrainPerpendicularMutation from exc
    ...
    def native_postcondition(document: object) -> bool:
        ...
        if state.document is not document or state.failure is not None:
            state.failure = SketchConstrainPerpendicularError(
        ...
        try:
            ...
            state.failure = (
                exc
                if isinstance(exc, SketchConstrainPerpendicularError)
                else SketchConstrainPerpendicularError("SKETCH_CONSTRAIN_PERPENDICULAR_RESULT_FAILED", str(exc) or type(exc).__name__)
            )
            return False
        try:
    ...
    try:
        ...
        return make_sketch_constrain_perpendicular_uncertain(
            "SKETCH_CONSTRAIN_PERPENDICULAR_NATIVE_EXCEPTION",
            str(exc) or type(exc).__name__,
            committed=None,
        )
    except Exception as exc:
    ...
    return _sketch_constrain_perpendicular_native_result(native_result, state)
