"""Fixed native transaction policy for the typed ``common_volume_along_path`` mutation."""
...
class CommonVolumeAlongPathError(TypedMutationError):
...
def _common_volume_along_path_native_result(
    native_result: object, state: _NativeMutationState
) -> Literal[True] | CommonVolumeAlongPathFailure | CommonVolumeAlongPathUncertain:
    if not isinstance(native_result, dict):
        return make_common_volume_along_path_uncertain(
            "INVALID_NATIVE_COMMON_VOLUME_ALONG_PATH_RESULT",
    ...
    message = message_value if isinstance(message_value, str) else "Native common_volume_along_path mutation failed"
    ...
    if rollback_failed:
        return make_common_volume_along_path_uncertain(
            "COMMON_VOLUME_ALONG_PATH_ROLLBACK_UNCERTAIN",
            message,
            committed=True if committed is True else None,
            native_status=status,
    ...
    if status == "Committed" and committed is True and "rollback_succeeded" not in native_result:
        ...
        return make_common_volume_along_path_uncertain(
            ...
            "Native commit completed without a successful common_volume_along_path postcondition",
            committed=True,
            native_status=status,
        )
...
def run_common_volume_along_path_native_mutation(
    ...
    def native_apply(document: object) -> None:
        ...
        try:
            ...
            state.failure = (
                ...
                else CommonVolumeAlongPathError("COMMON_VOLUME_ALONG_PATH_FAILED", str(exc) or type(exc).__name__)
            )
    ...
    def native_postcondition(document: object) -> bool:
        if state.document is not document or state.failure is not None:
            state.failure = CommonVolumeAlongPathError(
        ...
        try:
            ...
            state.failure = (
                exc
                if isinstance(exc, TypedMutationError)
                else CommonVolumeAlongPathError("COMMON_VOLUME_ALONG_PATH_RESULT_FAILED", str(exc) or type(exc).__name__)
            )
            return False
        try:
    ...
    try:
        ...
        return make_common_volume_along_path_uncertain(
            "COMMON_VOLUME_ALONG_PATH_NATIVE_EXCEPTION",
            str(exc) or type(exc).__name__,
            committed=None,
        )
    except Exception as exc:
    ...
    return _common_volume_along_path_native_result(native_result, state)
