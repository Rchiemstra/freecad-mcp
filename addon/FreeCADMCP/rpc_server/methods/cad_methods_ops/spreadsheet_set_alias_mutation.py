"""Fixed native transaction policy for the ``spreadsheet_set_alias`` mutation.
...
class SpreadsheetSetAliasError(RuntimeError):
...
class _NativeMutationState:
    document: SpreadsheetSetAliasDocument | None = None
    failure: SpreadsheetSetAliasError | None = None
...
def _spreadsheet_set_alias_native_result(
    native_result: object, state: _NativeMutationState
) -> Literal[True] | SpreadsheetSetAliasFailure | SpreadsheetSetAliasUncertain:
    if not isinstance(native_result, dict):
        return make_spreadsheet_set_alias_uncertain(
            "INVALID_NATIVE_SPREADSHEET_SET_ALIAS_RESULT",
    ...
    message = message_value if isinstance(message_value, str) else "Native spreadsheet_set_alias mutation failed"
    ...
    if rollback_failed:
        return make_spreadsheet_set_alias_uncertain(
            "SPREADSHEET_SET_ALIAS_ROLLBACK_UNCERTAIN",
            message,
            committed=True if committed is True else None,
            native_status=status,
...
def run_spreadsheet_set_alias_native_mutation(
    collaborators: SpreadsheetSetAliasCollaborators,
    document_name: DocumentName,
    apply: Callable[[SpreadsheetSetAliasDocument], None],
    postcondition: Callable[[SpreadsheetSetAliasReadDocument], None],
) -> Literal[True] | SpreadsheetSetAliasFailure | SpreadsheetSetAliasUncertain:
    ...
    def native_apply(document: object) -> None:
        typed = cast(SpreadsheetSetAliasDocument, document)
        ...
        try:
            ...
            state.failure = (
                exc
                if isinstance(exc, SpreadsheetSetAliasError)
                else SpreadsheetSetAliasError("SPREADSHEET_SET_ALIAS_FAILED", str(exc) or type(exc).__name__)
            )
            raise _AbortMutation from exc

    def native_postcondition(document: object) -> bool:
        typed = cast(SpreadsheetSetAliasReadDocument, document)
        if state.document is not typed or state.failure is not None:
            state.failure = SpreadsheetSetAliasError(
        ...
        try:
            ...
            state.failure = (
                exc
                if isinstance(exc, SpreadsheetSetAliasError)
                else SpreadsheetSetAliasError("SPREADSHEET_SET_ALIAS_RESULT_FAILED", str(exc) or type(exc).__name__)
            )
            return False
        try:
    ...
    try:
        ...
        return make_spreadsheet_set_alias_uncertain(
            "SPREADSHEET_SET_ALIAS_NATIVE_EXCEPTION",
            str(exc) or type(exc).__name__,
            committed=None,
        )
    except Exception as exc:
    ...
    return _spreadsheet_set_alias_native_result(native_result, state)
