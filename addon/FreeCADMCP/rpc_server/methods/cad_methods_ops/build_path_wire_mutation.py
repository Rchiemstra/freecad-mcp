"""Fixed native transaction policy for the ``build_path_wire`` mutation.
...
class _NativeMutationState:
    document: BuildPathWireDocument | None = None
    failure: BuildPathWireError | None = None
...
def _build_path_wire_native_result(
    native_result: object, state: _NativeMutationState
) -> Literal[True] | BuildPathWireFailure | BuildPathWireUncertain:
    if not isinstance(native_result, dict):
        return make_build_path_wire_uncertain(
            "INVALID_NATIVE_BUILD_PATH_WIRE_RESULT",
    ...
    message = message_value if isinstance(message_value, str) else "Native build_path_wire mutation failed"
    ...
    if rollback_failed:
        return make_build_path_wire_uncertain(
            "BUILD_PATH_WIRE_ROLLBACK_UNCERTAIN",
            message,
            committed=True if committed is True else None,
            native_status=status,
...
def run_build_path_wire_native_mutation(
    collaborators: BuildPathWireCollaborators,
    document_name: DocumentName,
    apply: Callable[[BuildPathWireDocument], None],
    postcondition: Callable[[BuildPathWireReadDocument], None],
) -> Literal[True] | BuildPathWireFailure | BuildPathWireUncertain:
    ...
    def native_apply(document: object) -> None:
        typed = cast(BuildPathWireDocument, document)
        ...
        try:
            ...
            state.failure = (
                exc
                if isinstance(exc, BuildPathWireError)
                else BuildPathWireError("BUILD_PATH_WIRE_FAILED", str(exc) or type(exc).__name__)
            )
            raise _AbortMutation from exc

    def native_postcondition(document: object) -> bool:
        typed = cast(BuildPathWireReadDocument, document)
        if state.document is not typed or state.failure is not None:
            state.failure = BuildPathWireError(
        ...
        try:
            ...
            state.failure = (
                exc
                if isinstance(exc, BuildPathWireError)
                else BuildPathWireError("BUILD_PATH_WIRE_RESULT_FAILED", str(exc) or type(exc).__name__)
            )
            return False
        try:
    ...
    try:
        ...
        return make_build_path_wire_uncertain(
            "BUILD_PATH_WIRE_NATIVE_EXCEPTION",
            str(exc) or type(exc).__name__,
            committed=None,
        )
    except Exception as exc:
    ...
    return _build_path_wire_native_result(native_result, state)
