"""Fixed native transaction policy for the typed ``sketch_constrain_radius`` mutation."""
...
class SketchConstrainRadiusError(RuntimeError):
    def __init__(self, code: str, message: str) -> None:
        super().__init__(message)
        self.code = code


class _AbortSketchConstrainRadiusMutation(RuntimeError):
    """Ask the native coordinator to roll back a failed apply callback."""


@dataclass(slots=True)
class _NativeSketchConstrainRadiusMutationState:
    document: SketchDocument | None = None
    failure: SketchConstrainRadiusError | None = None
...
def _sketch_constrain_radius_native_result(
    native_result: object, state: _NativeSketchConstrainRadiusMutationState
) -> Literal[True] | SketchConstrainRadiusFailure | SketchConstrainRadiusUncertain:
    if not isinstance(native_result, dict):
        return make_sketch_constrain_radius_uncertain(
            "INVALID_NATIVE_SKETCH_CONSTRAIN_RADIUS_RESULT",
    ...
    message = message_value if isinstance(message_value, str) else "Native sketch_constrain_radius mutation failed"
    ...
    if rollback_failed:
        return make_sketch_constrain_radius_uncertain(
            "SKETCH_CONSTRAIN_RADIUS_ROLLBACK_UNCERTAIN",
            message,
            committed=True if committed is True else None,
            native_status=status,
    ...
    if status == "Committed" and committed is True and "rollback_succeeded" not in native_result:
        ...
        return make_sketch_constrain_radius_uncertain(
            ...
            "Native commit completed without a successful sketch_constrain_radius postcondition",
            committed=True,
            native_status=status,
        )
...
def run_sketch_constrain_radius_native_mutation(
    ...
    state = _NativeSketchConstrainRadiusMutationState()
    ...
    def native_apply(document: object) -> None:
        ...
        try:
            ...
            state.failure = (
                exc
                if isinstance(exc, SketchConstrainRadiusError)
                else SketchConstrainRadiusError("SKETCH_CONSTRAIN_RADIUS_FAILED", str(exc) or type(exc).__name__)
            )
            raise _AbortSketchConstrainRadiusMutation from exc
    ...
    def native_postcondition(document: object) -> bool:
        ...
        if state.document is not document or state.failure is not None:
            state.failure = SketchConstrainRadiusError(
        ...
        try:
            ...
            state.failure = (
                exc
                if isinstance(exc, SketchConstrainRadiusError)
                else SketchConstrainRadiusError("SKETCH_CONSTRAIN_RADIUS_RESULT_FAILED", str(exc) or type(exc).__name__)
            )
            return False
        try:
    ...
    try:
        ...
        return make_sketch_constrain_radius_uncertain(
            "SKETCH_CONSTRAIN_RADIUS_NATIVE_EXCEPTION",
            str(exc) or type(exc).__name__,
            committed=None,
        )
    except Exception as exc:
    ...
    return _sketch_constrain_radius_native_result(native_result, state)
