"""Fixed native transaction policy for the typed ``sketch_offset`` mutation."""
...
class SketchOffsetError(RuntimeError):
    def __init__(self, code: str, message: str) -> None:
        super().__init__(message)
        self.code = code


class _AbortSketchOffsetMutation(RuntimeError):
    """Ask the native coordinator to roll back a failed apply callback."""


@dataclass(slots=True)
class _NativeSketchOffsetMutationState:
    document: SketchDocument | None = None
    failure: SketchOffsetError | None = None
...
def _sketch_offset_native_result(
    native_result: object, state: _NativeSketchOffsetMutationState
) -> Literal[True] | SketchOffsetFailure | SketchOffsetUncertain:
    if not isinstance(native_result, dict):
        return make_sketch_offset_uncertain(
            "INVALID_NATIVE_SKETCH_OFFSET_RESULT",
    ...
    message = message_value if isinstance(message_value, str) else "Native sketch_offset mutation failed"
    ...
    if rollback_failed:
        return make_sketch_offset_uncertain(
            "SKETCH_OFFSET_ROLLBACK_UNCERTAIN",
            message,
            committed=True if committed is True else None,
            native_status=status,
    ...
    if status == "Committed" and committed is True and "rollback_succeeded" not in native_result:
        ...
        return make_sketch_offset_uncertain(
            ...
            "Native commit completed without a successful sketch_offset postcondition",
            committed=True,
            native_status=status,
        )
...
def run_sketch_offset_native_mutation(
    ...
    def native_apply(document: object) -> None:
        ...
        try:
            ...
            state.failure = (
                exc
                if isinstance(exc, SketchOffsetError)
                else SketchOffsetError("SKETCH_OFFSET_FAILED", str(exc) or type(exc).__name__)
            )
            raise _AbortSketchOffsetMutation from exc
    ...
    def native_postcondition(document: object) -> bool:
        ...
        if state.document is not document or state.failure is not None:
            state.failure = SketchOffsetError(
        ...
        try:
            ...
            state.failure = (
                exc
                if isinstance(exc, SketchOffsetError)
                else SketchOffsetError("SKETCH_OFFSET_RESULT_FAILED", str(exc) or type(exc).__name__)
            )
            return False
        try:
    ...
    try:
        ...
        return make_sketch_offset_uncertain(
            "SKETCH_OFFSET_NATIVE_EXCEPTION",
            str(exc) or type(exc).__name__,
            committed=None,
        )
    except Exception as exc:
    ...
    return _sketch_offset_native_result(native_result, state)
