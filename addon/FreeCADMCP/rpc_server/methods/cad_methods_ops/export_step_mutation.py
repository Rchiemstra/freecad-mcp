"""Fixed native transaction policy for the typed ``export_step`` mutation."""
...
class ExportStepError(TypedMutationError):
...
def _export_step_native_result(
    native_result: object, state: _NativeMutationState
) -> Literal[True] | ExportStepFailure | ExportStepUncertain:
    if not isinstance(native_result, dict):
        return make_export_step_uncertain(
            "INVALID_NATIVE_EXPORT_STEP_RESULT",
    ...
    message = message_value if isinstance(message_value, str) else "Native export_step mutation failed"
    ...
    if rollback_failed:
        return make_export_step_uncertain(
            "EXPORT_STEP_ROLLBACK_UNCERTAIN",
            message,
            committed=True if committed is True else None,
            native_status=status,
    ...
    if status == "Committed" and committed is True and "rollback_succeeded" not in native_result:
        ...
        return make_export_step_uncertain(
            ...
            "Native commit completed without a successful export_step postcondition",
            committed=True,
            native_status=status,
        )
...
def run_export_step_native_mutation(
    ...
    def native_apply(document: object) -> None:
        ...
        try:
            ...
            state.failure = (
                ...
                else ExportStepError("EXPORT_STEP_FAILED", str(exc) or type(exc).__name__)
            )
    ...
    def native_postcondition(document: object) -> bool:
        ...
        try:
            ...
            state.failure = (
                exc
                if isinstance(exc, TypedMutationError)
                else ExportStepError("EXPORT_STEP_RESULT_FAILED", str(exc) or type(exc).__name__)
            )
            return False
        try:
    ...
    return _export_step_native_result(native_result, state)
