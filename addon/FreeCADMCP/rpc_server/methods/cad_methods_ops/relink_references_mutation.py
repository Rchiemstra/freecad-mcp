"""Fixed native transaction policy for the ``relink_references`` mutation.
...
class _NativeMutationState:
    document: RelinkReferencesDocument | None = None
    failure: RelinkReferencesError | None = None
...
def _relink_references_native_result(
    native_result: object, state: _NativeMutationState
) -> Literal[True] | RelinkReferencesFailure | RelinkReferencesUncertain:
    if not isinstance(native_result, dict):
        return make_relink_references_uncertain(
            "INVALID_NATIVE_RELINK_REFERENCES_RESULT",
    ...
    message = message_value if isinstance(message_value, str) else "Native relink_references mutation failed"
    ...
    if rollback_failed:
        return make_relink_references_uncertain(
            "RELINK_REFERENCES_ROLLBACK_UNCERTAIN",
            message,
            committed=True if committed is True else None,
            native_status=status,
...
def run_relink_references_native_mutation(
    collaborators: RelinkReferencesCollaborators,
    document_name: DocumentName,
    apply: Callable[[RelinkReferencesDocument], None],
    postcondition: Callable[[RelinkReferencesReadDocument], None],
) -> Literal[True] | RelinkReferencesFailure | RelinkReferencesUncertain:
    ...
    def native_apply(document: object) -> None:
        typed = cast(RelinkReferencesDocument, document)
        ...
        try:
            ...
            state.failure = (
                exc
                if isinstance(exc, RelinkReferencesError)
                else RelinkReferencesError("RELINK_REFERENCES_FAILED", str(exc) or type(exc).__name__)
            )
            raise _AbortMutation from exc

    def native_postcondition(document: object) -> bool:
        typed = cast(RelinkReferencesReadDocument, document)
        if state.document is not typed or state.failure is not None:
            state.failure = RelinkReferencesError(
        ...
        try:
            ...
            state.failure = (
                exc
                if isinstance(exc, RelinkReferencesError)
                else RelinkReferencesError("RELINK_REFERENCES_RESULT_FAILED", str(exc) or type(exc).__name__)
            )
            return False
        try:
    ...
    try:
        ...
        return make_relink_references_uncertain(
            "RELINK_REFERENCES_NATIVE_EXCEPTION",
            str(exc) or type(exc).__name__,
            committed=None,
        )
    except Exception as exc:
    ...
    return _relink_references_native_result(native_result, state)
