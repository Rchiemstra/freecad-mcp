"""Fixed native transaction policy for the typed ``create_spur_gear`` mutation."""
...
class CreateSpurGearError(TypedMutationError):
...
def _create_spur_gear_native_result(
    ...
    if not isinstance(native_result, dict):
        return make_create_spur_gear_uncertain(
            "INVALID_NATIVE_CREATE_SPUR_GEAR_RESULT",
    ...
    message = message_value if isinstance(message_value, str) else "Native create_spur_gear mutation failed"
    ...
    if rollback_failed:
        return make_create_spur_gear_uncertain(
            "CREATE_SPUR_GEAR_ROLLBACK_UNCERTAIN",
            message,
            committed=True if committed is True else None,
            native_status=status,
    ...
    if status == "Committed" and committed is True and "rollback_succeeded" not in native_result:
        ...
        return make_create_spur_gear_uncertain(
            ...
            "Native commit completed without a successful create_spur_gear postcondition",
            committed=True,
            native_status=status,
        )
...
def run_create_spur_gear_native_mutation(
    ...
    def native_apply(document: object) -> None:
        ...
        try:
            ...
            state.failure = (
                ...
                else CreateSpurGearError("CREATE_SPUR_GEAR_FAILED", str(exc) or type(exc).__name__)
            )
    ...
    def native_postcondition(document: object) -> bool:
        if state.document is not document or state.failure is not None:
            state.failure = CreateSpurGearError(
        ...
        try:
            ...
            state.failure = (
                exc
                if isinstance(exc, TypedMutationError)
                else CreateSpurGearError("CREATE_SPUR_GEAR_RESULT_FAILED", str(exc) or type(exc).__name__)
            )
            return False
        try:
    ...
    try:
        ...
        return make_create_spur_gear_uncertain(
            "CREATE_SPUR_GEAR_NATIVE_EXCEPTION",
            str(exc) or type(exc).__name__,
            committed=None,
        )
    except Exception as exc:
