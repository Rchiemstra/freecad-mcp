"""Typed ``diagnose_helix`` query handler."""
...
def run_diagnose_helix(collaborators: object, doc_name: str, helix_name: str) -> dict[str, object]:
    if not isinstance(doc_name, str) or not doc_name.strip():
        return _failure("INVALID_ARGUMENT", "doc_name must be a nonempty string")
    if not isinstance(helix_name, str) or not helix_name.strip():
        return _failure("INVALID_ARGUMENT", "helix_name must be a nonempty string")
...
def rpc_diagnose_helix(self: _RpcFacade, doc_name: str, helix_name: str) -> dict[str, object]:
    res = self._dispatch_gui(lambda: run_diagnose_helix(self._cad_collaborators, doc_name, helix_name))
    return res if isinstance(res, dict) else {"success": False, "error": res}

TYPED_RPC_HANDLER = ("diagnose_helix", rpc_diagnose_helix)
__all__ = ["rpc_diagnose_helix", "run_diagnose_helix", "TYPED_RPC_HANDLER"]
