"""Native transaction policy for the typed ``close_document`` mutation."""
...
class _NativeMutationState:
    ...
    failure: CloseDocumentError | None = None
...
def _close_document_native_result(
    native_result: object, state: _NativeMutationState
) -> Literal[True] | CloseDocumentFailure | CloseDocumentUncertain:
    if not isinstance(native_result, dict):
        return make_close_document_uncertain(
            "INVALID_NATIVE_CLOSE_DOCUMENT_RESULT",
    ...
    message = message_value if isinstance(message_value, str) else "Native close_document mutation failed"
    ...
    if rollback_failed:
        return make_close_document_uncertain(
            "CLOSE_DOCUMENT_ROLLBACK_UNCERTAIN",
            message,
            committed=True if committed is True else None,
            native_status=status,
    ...
    if status == "Committed" and committed is True and "rollback_succeeded" not in native_result:
        ...
        return make_close_document_uncertain(
            ...
            "Native commit completed without a successful close_document postcondition",
            committed=True,
            native_status=status,
        )
...
def run_close_document_native_mutation(
    ...
    def native_apply(document: object) -> None:
        ...
        try:
            ...
            state.failure = (
                exc
                if isinstance(exc, CloseDocumentError)
                else CloseDocumentError("CLOSE_DOCUMENT_FAILED", str(exc) or type(exc).__name__)
            )
    ...
    def native_postcondition(document: object) -> bool:
        if state.document is not document or state.failure is not None:
            state.failure = CloseDocumentError(
        ...
        try:
            ...
            state.failure = (
                exc
                if isinstance(exc, CloseDocumentError)
                else CloseDocumentError("CLOSE_DOCUMENT_RESULT_FAILED", str(exc) or type(exc).__name__)
            )
            return False
        try:
            collaborators.validate_document_invariants(cast(CloseDocumentReadDocument, document))
        except Exception as exc:
            state.failure = CloseDocumentError(
    ...
    try:
        ...
        return make_close_document_uncertain(
            "CLOSE_DOCUMENT_NATIVE_EXCEPTION",
            str(exc) or type(exc).__name__,
            committed=None,
        )
    except Exception as exc:
    ...
    return _close_document_native_result(native_result, state)
