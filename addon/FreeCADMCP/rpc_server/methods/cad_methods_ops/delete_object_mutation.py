"""Native transaction policy for the typed ``delete_object`` mutation."""
...
class _NativeMutationState:
    ...
    failure: DeleteObjectError | None = None
...
def _delete_object_native_result(
    ...
    if not isinstance(native_result, dict):
        return make_delete_object_uncertain(
            "INVALID_NATIVE_DELETE_OBJECT_RESULT",
    ...
    message = message_value if isinstance(message_value, str) else "Native delete_object mutation failed"
    ...
    if rollback_failed:
        return make_delete_object_uncertain(
            "DELETE_OBJECT_ROLLBACK_UNCERTAIN",
            message,
            committed=True if committed is True else None,
            native_status=status,
    ...
    if status == "Committed" and committed is True and "rollback_succeeded" not in native_result:
        ...
        return make_delete_object_uncertain(
            ...
            "Native commit completed without a successful delete_object postcondition",
            committed=True,
            native_status=status,
        )
...
def run_delete_object_native_mutation(
    ...
    def native_apply(document: object) -> None:
        ...
        try:
            ...
            state.failure = (
                exc
                if isinstance(exc, DeleteObjectError)
                else DeleteObjectError("DELETE_OBJECT_FAILED", str(exc) or type(exc).__name__)
            )
    ...
    try:
        ...
        return make_delete_object_uncertain(
            "DELETE_OBJECT_NATIVE_EXCEPTION",
            str(exc) or type(exc).__name__,
            committed=None,
        )
    except Exception as exc:
