"""Fixed native transaction policy for the ``validate_movement_follow`` mutation.
...
class _NativeMutationState:
    document: ValidateMovementFollowDocument | None = None
    failure: ValidateMovementFollowError | None = None
...
def _validate_movement_follow_native_result(
    ...
    if not isinstance(native_result, dict):
        return make_validate_movement_follow_uncertain(
            "INVALID_NATIVE_VALIDATE_MOVEMENT_FOLLOW_RESULT",
    ...
    message = message_value if isinstance(message_value, str) else "Native validate_movement_follow mutation failed"
    ...
    if rollback_failed:
        return make_validate_movement_follow_uncertain(
            "VALIDATE_MOVEMENT_FOLLOW_ROLLBACK_UNCERTAIN",
            message,
            committed=True if committed is True else None,
            native_status=status,
...
def run_validate_movement_follow_native_mutation(
    ...
    def native_apply(document: object) -> None:
        typed = cast(ValidateMovementFollowDocument, document)
        ...
        try:
            ...
            state.failure = (
                exc
                if isinstance(exc, ValidateMovementFollowError)
                else ValidateMovementFollowError("VALIDATE_MOVEMENT_FOLLOW_FAILED", str(exc) or type(exc).__name__)
            )
            raise _AbortMutation from exc

    def native_postcondition(document: object) -> bool:
        typed = cast(ValidateMovementFollowReadDocument, document)
        if state.document is not typed or state.failure is not None:
            state.failure = ValidateMovementFollowError(
        ...
        try:
            ...
            state.failure = (
                exc
                if isinstance(exc, ValidateMovementFollowError)
                else ValidateMovementFollowError("VALIDATE_MOVEMENT_FOLLOW_RESULT_FAILED", str(exc) or type(exc).__name__)
            )
            return False
        try:
    ...
    try:
        ...
        return make_validate_movement_follow_uncertain(
            "VALIDATE_MOVEMENT_FOLLOW_NATIVE_EXCEPTION",
            str(exc) or type(exc).__name__,
            committed=None,
        )
    except Exception as exc:
