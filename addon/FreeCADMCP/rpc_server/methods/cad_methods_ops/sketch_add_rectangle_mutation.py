"""Fixed native transaction policy for the typed ``sketch_add_rectangle`` mutation."""
...
class SketchAddRectangleError(RuntimeError):
    def __init__(self, code: str, message: str) -> None:
        super().__init__(message)
        self.code = code


class _AbortSketchAddRectangleMutation(RuntimeError):
    """Ask the native coordinator to roll back a failed apply callback."""


@dataclass(slots=True)
class _NativeSketchAddRectangleMutationState:
    document: SketchDocument | None = None
    failure: SketchAddRectangleError | None = None
...
def _sketch_add_rectangle_native_result(
    native_result: object, state: _NativeSketchAddRectangleMutationState
) -> Literal[True] | SketchAddRectangleFailure | SketchAddRectangleUncertain:
    if not isinstance(native_result, dict):
        return make_sketch_add_rectangle_uncertain(
            "INVALID_NATIVE_SKETCH_ADD_RECTANGLE_RESULT",
    ...
    message = message_value if isinstance(message_value, str) else "Native sketch_add_rectangle mutation failed"
    ...
    if rollback_failed:
        return make_sketch_add_rectangle_uncertain(
            "SKETCH_ADD_RECTANGLE_ROLLBACK_UNCERTAIN",
            message,
            committed=True if committed is True else None,
            native_status=status,
    ...
    if status == "Committed" and committed is True and "rollback_succeeded" not in native_result:
        ...
        return make_sketch_add_rectangle_uncertain(
            ...
            "Native commit completed without a successful sketch_add_rectangle postcondition",
            committed=True,
            native_status=status,
        )
...
def run_sketch_add_rectangle_native_mutation(
    ...
    state = _NativeSketchAddRectangleMutationState()
    ...
    def native_apply(document: object) -> None:
        ...
        try:
            ...
            state.failure = (
                exc
                if isinstance(exc, SketchAddRectangleError)
                else SketchAddRectangleError("SKETCH_ADD_RECTANGLE_FAILED", str(exc) or type(exc).__name__)
            )
            raise _AbortSketchAddRectangleMutation from exc
    ...
    def native_postcondition(document: object) -> bool:
        ...
        if state.document is not document or state.failure is not None:
            state.failure = SketchAddRectangleError(
        ...
        try:
            ...
            state.failure = (
                exc
                if isinstance(exc, SketchAddRectangleError)
                else SketchAddRectangleError("SKETCH_ADD_RECTANGLE_RESULT_FAILED", str(exc) or type(exc).__name__)
            )
            return False
        try:
    ...
    try:
        ...
        return make_sketch_add_rectangle_uncertain(
            "SKETCH_ADD_RECTANGLE_NATIVE_EXCEPTION",
            str(exc) or type(exc).__name__,
            committed=None,
        )
    except Exception as exc:
    ...
    return _sketch_add_rectangle_native_result(native_result, state)
