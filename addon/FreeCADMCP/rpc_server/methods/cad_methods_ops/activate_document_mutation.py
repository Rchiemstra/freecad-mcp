"""Native transaction policy for the typed ``activate_document`` mutation."""
...
class _NativeMutationState:
    ...
    failure: ActivateDocumentError | None = None
...
def _activate_document_native_result(
    native_result: object, state: _NativeMutationState
) -> Literal[True] | ActivateDocumentFailure | ActivateDocumentUncertain:
    if not isinstance(native_result, dict):
        return make_activate_document_uncertain(
            "INVALID_NATIVE_ACTIVATE_DOCUMENT_RESULT",
    ...
    message = message_value if isinstance(message_value, str) else "Native activate_document mutation failed"
    ...
    if status == "Committed" and committed is True and "rollback_succeeded" not in native_result:
        ...
        return make_activate_document_uncertain(
            ...
            "Native commit completed without a successful activate_document postcondition",
            committed=True,
            native_status=status,
        )
...
def run_activate_document_native_mutation(
    ...
    def native_apply(document: object) -> None:
        ...
        try:
            ...
            state.failure = (
                exc
                if isinstance(exc, ActivateDocumentError)
                else ActivateDocumentError("ACTIVATE_DOCUMENT_FAILED", str(exc) or type(exc).__name__)
            )
    ...
    def native_postcondition(document: object) -> bool:
        if state.document is not document or state.failure is not None:
            state.failure = ActivateDocumentError(
        ...
        try:
            ...
            state.failure = (
                exc
                if isinstance(exc, ActivateDocumentError)
                else ActivateDocumentError("ACTIVATE_DOCUMENT_RESULT_FAILED", str(exc) or type(exc).__name__)
            )
            return False
        try:
            collaborators.validate_document_invariants(cast(ActivateDocumentReadDocument, document))
        except Exception as exc:
            state.failure = ActivateDocumentError(
    ...
    try:
        ...
        return make_activate_document_uncertain(
            "ACTIVATE_DOCUMENT_NATIVE_EXCEPTION",
            str(exc) or type(exc).__name__,
            committed=None,
        )
    except Exception as exc:
    ...
    return _activate_document_native_result(native_result, state)
