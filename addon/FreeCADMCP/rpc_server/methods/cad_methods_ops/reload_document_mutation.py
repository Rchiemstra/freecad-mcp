"""Native transaction policy for the typed ``reload_document`` mutation."""
...
class _NativeMutationState:
    ...
    failure: ReloadDocumentError | None = None
...
def _reload_document_native_result(
    native_result: object, state: _NativeMutationState
) -> Literal[True] | ReloadDocumentFailure | ReloadDocumentUncertain:
    if not isinstance(native_result, dict):
        return make_reload_document_uncertain(
            "INVALID_NATIVE_RELOAD_DOCUMENT_RESULT",
    ...
    message = message_value if isinstance(message_value, str) else "Native reload_document mutation failed"
    ...
    if rollback_failed:
        return make_reload_document_uncertain(
            "RELOAD_DOCUMENT_ROLLBACK_UNCERTAIN",
            message,
            committed=True if committed is True else None,
            native_status=status,
    ...
    if status == "Committed" and committed is True and "rollback_succeeded" not in native_result:
        ...
        return make_reload_document_uncertain(
            ...
            "Native commit completed without a successful reload_document postcondition",
            committed=True,
            native_status=status,
        )
...
def run_reload_document_native_mutation(
    ...
    def native_apply(document: object) -> None:
        ...
        try:
            ...
            state.failure = (
                exc
                if isinstance(exc, ReloadDocumentError)
                else ReloadDocumentError("RELOAD_DOCUMENT_FAILED", str(exc) or type(exc).__name__)
            )
    ...
    def native_postcondition(document: object) -> bool:
        if state.document is not document or state.failure is not None:
            state.failure = ReloadDocumentError(
        ...
        try:
            ...
            state.failure = (
                exc
                if isinstance(exc, ReloadDocumentError)
                else ReloadDocumentError("RELOAD_DOCUMENT_RESULT_FAILED", str(exc) or type(exc).__name__)
            )
            return False
        try:
            collaborators.validate_document_invariants(cast(ReloadDocumentReadDocument, document))
        except Exception as exc:
            state.failure = ReloadDocumentError(
    ...
    try:
        ...
        return make_reload_document_uncertain(
            "RELOAD_DOCUMENT_NATIVE_EXCEPTION",
            str(exc) or type(exc).__name__,
            committed=None,
        )
    except Exception as exc:
    ...
    return _reload_document_native_result(native_result, state)
