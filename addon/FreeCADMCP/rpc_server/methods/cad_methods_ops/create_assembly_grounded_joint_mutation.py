"""Fixed native transaction policy for the typed ``create_assembly_grounded_joint`` mutation."""
...
class CreateAssemblyGroundedJointError(TypedMutationError):
...
def _create_assembly_grounded_joint_native_result(
    native_result: object, state: _NativeMutationState
) -> Literal[True] | CreateAssemblyGroundedJointFailure | CreateAssemblyGroundedJointUncertain:
    if not isinstance(native_result, dict):
        return make_create_assembly_grounded_joint_uncertain(
            "INVALID_NATIVE_CREATE_ASSEMBLY_GROUNDED_JOINT_RESULT",
    ...
    message = message_value if isinstance(message_value, str) else "Native create_assembly_grounded_joint mutation failed"
    ...
    if rollback_failed:
        return make_create_assembly_grounded_joint_uncertain(
            "CREATE_ASSEMBLY_GROUNDED_JOINT_ROLLBACK_UNCERTAIN",
            message,
            committed=True if committed is True else None,
            native_status=status,
    ...
    if status == "Committed" and committed is True and "rollback_succeeded" not in native_result:
        ...
        return make_create_assembly_grounded_joint_uncertain(
            ...
            "Native commit completed without a successful create_assembly_grounded_joint postcondition",
            committed=True,
            native_status=status,
        )
...
def run_create_assembly_grounded_joint_native_mutation(
    ...
    def native_apply(document: object) -> None:
        ...
        try:
            ...
            state.failure = (
                ...
                else CreateAssemblyGroundedJointError("CREATE_ASSEMBLY_GROUNDED_JOINT_FAILED", str(exc) or type(exc).__name__)
            )
    ...
    def native_postcondition(document: object) -> bool:
        if state.document is not document or state.failure is not None:
            state.failure = CreateAssemblyGroundedJointError(
        ...
        try:
            ...
            state.failure = (
                exc
                if isinstance(exc, TypedMutationError)
                else CreateAssemblyGroundedJointError("CREATE_ASSEMBLY_GROUNDED_JOINT_RESULT_FAILED", str(exc) or type(exc).__name__)
            )
            return False
        try:
    ...
    try:
        ...
        return make_create_assembly_grounded_joint_uncertain(
            "CREATE_ASSEMBLY_GROUNDED_JOINT_NATIVE_EXCEPTION",
            str(exc) or type(exc).__name__,
            committed=None,
        )
    except Exception as exc:
    ...
    return _create_assembly_grounded_joint_native_result(native_result, state)
