"""Fixed native transaction policy for the typed ``boolean_intersection`` mutation."""
...
class _BooleanIntersectionAbortMutation(RuntimeError):
    """Ask the native coordinator to roll back a failed apply callback."""


@dataclass(slots=True)
class _BooleanIntersectionNativeMutationState:
    document: FeatureDocument | None = None
    failure: BooleanIntersectionError | None = None
...
def _boolean_intersection_native_result(
    native_result: object, state: _BooleanIntersectionNativeMutationState
) -> Literal[True] | BooleanIntersectionFailure | BooleanIntersectionUncertain:
    if not isinstance(native_result, dict):
        return make_boolean_intersection_uncertain(
            "INVALID_NATIVE_BOOLEAN_INTERSECTION_RESULT",
    ...
    message = (
        ...
        else "Native boolean_intersection mutation failed"
    )
    ...
    if status == "Committed" and committed is True and "rollback_succeeded" not in native_result:
        ...
        return make_boolean_intersection_uncertain(
            ...
            "Native commit completed without a successful boolean_intersection postcondition",
            committed=True,
            native_status=status,
        )
...
def run_boolean_intersection_native_mutation(
    ...
    state = _BooleanIntersectionNativeMutationState()
    ...
    def native_apply(document: FeatureDocument) -> None:
        ...
        try:
            ...
            state.failure = (
                exc
                if isinstance(exc, BooleanIntersectionError)
                else BooleanIntersectionError("BOOLEAN_INTERSECTION_FAILED", str(exc) or type(exc).__name__)
            )
            raise _BooleanIntersectionAbortMutation from exc

    def native_postcondition(document: FeatureReadDocument) -> bool:
        if state.document is not document or state.failure is not None:
            state.failure = BooleanIntersectionError(
        ...
        try:
            ...
            state.failure = (
                exc
                if isinstance(exc, BooleanIntersectionError)
                else BooleanIntersectionError("BOOLEAN_INTERSECTION_RESULT_FAILED", str(exc) or type(exc).__name__)
            )
            return False
        try:
    ...
    try:
        ...
        return make_boolean_intersection_uncertain(
            "BOOLEAN_INTERSECTION_NATIVE_EXCEPTION",
            str(exc) or type(exc).__name__,
            committed=None,
        )
    except Exception as exc:
    ...
    return _boolean_intersection_native_result(native_result, state)
