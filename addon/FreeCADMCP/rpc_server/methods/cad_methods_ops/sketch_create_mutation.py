"""Fixed native transaction policy for the typed ``sketch_create`` mutation.
...
class _AbortSketchCreateMutation(RuntimeError):
    """Ask the native coordinator to roll back a failed apply callback."""


@dataclass(slots=True)
class _NativeSketchCreateMutationState:
    document: object | None = None
    failure: SketchCreateError | None = None
...
def _sketch_create_native_result(
    native_result: object, state: _NativeSketchCreateMutationState
) -> Literal[True] | SketchCreateFailure | SketchCreateUncertain:
    if not isinstance(native_result, dict):
        return make_sketch_create_uncertain(
            "INVALID_NATIVE_SKETCH_CREATE_RESULT",
    ...
    message = message_value if isinstance(message_value, str) else "Native sketch_create mutation failed"
    ...
    if rollback_failed:
        return make_sketch_create_uncertain(
            "SKETCH_CREATE_ROLLBACK_UNCERTAIN",
            message,
            committed=True if committed is True else None,
            native_status=status,
    ...
    if status == "Committed" and committed is True and "rollback_succeeded" not in native_result:
        ...
        return make_sketch_create_uncertain(
            ...
            "Native commit completed without a successful sketch_create postcondition",
            committed=True,
            native_status=status,
        )
...
def run_sketch_create_native_mutation(
    collaborators: SketchCreateCollaborators,
    document_name: str,
    apply: Callable[[SketchCreateDocument], None],
    postcondition: Callable[[SketchCreateReadDocument], None],
) -> Literal[True] | SketchCreateFailure | SketchCreateUncertain:
    """Apply, recompute, inspect and validate on the same admitted document."""

    state = _NativeSketchCreateMutationState()

    def native_apply(document: object) -> None:
        state.document = document
        try:
            apply(cast(SketchCreateDocument, document))
        except Exception as exc:
            state.failure = (
                exc
                if isinstance(exc, SketchCreateError)
                else SketchCreateError("SKETCH_CREATE_FAILED", str(exc) or type(exc).__name__)
            )
            raise _AbortSketchCreateMutation from exc

    def native_postcondition(document: object) -> bool:
        if state.document is not document or state.failure is not None:
            state.failure = SketchCreateError(
                "DOCUMENT_IDENTITY_MISMATCH",
                "Native mutation callbacks did not receive the same admitted document",
            )
            return False
        admitted = cast(SketchCreateReadDocument, document)
        try:
            ...
            state.failure = (
                exc
                if isinstance(exc, SketchCreateError)
                else SketchCreateError("SKETCH_CREATE_RESULT_FAILED", str(exc) or type(exc).__name__)
            )
            return False
        try:
    ...
    try:
        ...
        return make_sketch_create_uncertain(
            "SKETCH_CREATE_NATIVE_EXCEPTION",
            str(exc) or type(exc).__name__,
            committed=None,
        )
    except Exception as exc:
    ...
    return _sketch_create_native_result(native_result, state)
