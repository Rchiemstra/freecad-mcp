"""Native transaction policy for the typed ``recompute_document`` mutation."""
...
class _NativeMutationState:
    ...
    failure: RecomputeDocumentError | None = None
...
def _recompute_document_native_result(
    native_result: object, state: _NativeMutationState
) -> Literal[True] | RecomputeDocumentFailure | RecomputeDocumentUncertain:
    if not isinstance(native_result, dict):
        return make_recompute_document_uncertain(
            "INVALID_NATIVE_RECOMPUTE_DOCUMENT_RESULT",
    ...
    message = message_value if isinstance(message_value, str) else "Native recompute_document mutation failed"
    ...
    if rollback_failed:
        return make_recompute_document_uncertain(
            "RECOMPUTE_DOCUMENT_ROLLBACK_UNCERTAIN",
            message,
            committed=True if committed is True else None,
            native_status=status,
    ...
    if status == "Committed" and committed is True and "rollback_succeeded" not in native_result:
        ...
        return make_recompute_document_uncertain(
            ...
            "Native commit completed without a successful recompute_document postcondition",
            committed=True,
            native_status=status,
        )
...
def run_recompute_document_native_mutation(
    ...
    def native_apply(document: object) -> None:
        ...
        try:
            ...
            state.failure = (
                exc
                if isinstance(exc, RecomputeDocumentError)
                else RecomputeDocumentError("RECOMPUTE_DOCUMENT_FAILED", str(exc) or type(exc).__name__)
            )
    ...
    def native_postcondition(document: object) -> bool:
        if state.document is not document or state.failure is not None:
            state.failure = RecomputeDocumentError(
        ...
        try:
            ...
            state.failure = (
                exc
                if isinstance(exc, RecomputeDocumentError)
                else RecomputeDocumentError("RECOMPUTE_DOCUMENT_RESULT_FAILED", str(exc) or type(exc).__name__)
            )
            return False
        try:
            collaborators.validate_document_invariants(cast(RecomputeDocumentReadDocument, document))
        except Exception as exc:
            state.failure = RecomputeDocumentError(
    ...
    try:
        ...
        return make_recompute_document_uncertain(
            "RECOMPUTE_DOCUMENT_NATIVE_EXCEPTION",
            str(exc) or type(exc).__name__,
            committed=None,
        )
    except Exception as exc:
    ...
    return _recompute_document_native_result(native_result, state)
