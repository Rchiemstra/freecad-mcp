"""Typed ``run_fem_analysis`` external-effect handler."""
...
def build_run_fem_analysis_request(
...
) -> RunFemAnalysisRequest | RunFemAnalysisFailure:
    ...
    if analysis_name_value is None:
        return _failure(
            RunFemAnalysisError("INVALID_ARGUMENT", "analysis_name must be a nonempty string")
        )
...
def _perform_fem_solver(
    collaborators: RunFemAnalysisCollaborators, request: RunFemAnalysisRequest
) -> RunFemAnalysisFailure | RunFemAnalysisUncertain | None:
    runner = getattr(collaborators, "run_fem_analysis", None)
    if runner is None:
        return _failure(RunFemAnalysisError("FEM_ANALYSIS_UNAVAILABLE", "run_fem_analysis collaborator is missing"))
    executor_result = invoke(runner, str(request.doc_name), request.analysis_name)
    if not isinstance(executor_result, dict) or executor_result.get("success") is not True:
        ...
        return make_run_fem_analysis_uncertain(code, message, committed=None)
    return None
...
def run_run_fem_analysis(
    ...
    if app is None:
        return _failure(RunFemAnalysisError("FREECAD_UNAVAILABLE", "FreeCAD collaborator is missing"))
    document = lookup_document(app, str(request.doc_name))
    if document is None:
        return _failure(
            RunFemAnalysisError("DOCUMENT_NOT_FOUND", f"Document not found: {request.doc_name!r}")
        )
    try:
    ...
    if lookup_document(app, str(request.doc_name)) is None:
        return make_run_fem_analysis_uncertain(
            "RUN_FEM_ANALYSIS_FAILED",
            f"Document {request.doc_name!r} is not open after FEM analysis",
            committed=None,
        )
    if lookup_object(document, str(request.analysis_name)) is None:
        return make_run_fem_analysis_uncertain(
            "RUN_FEM_ANALYSIS_FAILED",
            f"Analysis {request.analysis_name!r} is missing after solver run",
            committed=None,
        )
    return make_run_fem_analysis_success(
        analysis_name=RunFemAnalysisName(object_name(analysis) or request.analysis_name)
    )
