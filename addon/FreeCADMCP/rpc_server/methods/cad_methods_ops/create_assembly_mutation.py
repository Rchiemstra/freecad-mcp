"""Fixed native transaction policy for the typed ``create_assembly`` mutation."""
...
class CreateAssemblyError(TypedMutationError):
...
def _create_assembly_native_result(
    native_result: object, state: _NativeMutationState
) -> Literal[True] | CreateAssemblyFailure | CreateAssemblyUncertain:
    if not isinstance(native_result, dict):
        return make_create_assembly_uncertain(
            "INVALID_NATIVE_CREATE_ASSEMBLY_RESULT",
    ...
    message = message_value if isinstance(message_value, str) else "Native create_assembly mutation failed"
    ...
    if rollback_failed:
        return make_create_assembly_uncertain(
            "CREATE_ASSEMBLY_ROLLBACK_UNCERTAIN",
            message,
            committed=True if committed is True else None,
            native_status=status,
    ...
    if status == "Committed" and committed is True and "rollback_succeeded" not in native_result:
        ...
        return make_create_assembly_uncertain(
            ...
            "Native commit completed without a successful create_assembly postcondition",
            committed=True,
            native_status=status,
        )
...
def run_create_assembly_native_mutation(
    ...
    def native_apply(document: object) -> None:
        ...
        try:
            ...
            state.failure = (
                ...
                else CreateAssemblyError("CREATE_ASSEMBLY_FAILED", str(exc) or type(exc).__name__)
            )
    ...
    def native_postcondition(document: object) -> bool:
        if state.document is not document or state.failure is not None:
            state.failure = CreateAssemblyError(
        ...
        try:
            ...
            state.failure = (
                exc
                if isinstance(exc, TypedMutationError)
                else CreateAssemblyError("CREATE_ASSEMBLY_RESULT_FAILED", str(exc) or type(exc).__name__)
            )
            return False
        try:
    ...
    try:
        ...
        return make_create_assembly_uncertain(
            "CREATE_ASSEMBLY_NATIVE_EXCEPTION",
            str(exc) or type(exc).__name__,
            committed=None,
        )
    except Exception as exc:
    ...
    return _create_assembly_native_result(native_result, state)
