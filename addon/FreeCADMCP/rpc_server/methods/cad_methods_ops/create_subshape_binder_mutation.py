"""Fixed native transaction policy for the ``create_subshape_binder`` mutation.
...
class _NativeMutationState:
    document: CreateSubshapeBinderDocument | None = None
    failure: CreateSubshapeBinderError | None = None
...
def _create_subshape_binder_native_result(
    native_result: object, state: _NativeMutationState
) -> Literal[True] | CreateSubshapeBinderFailure | CreateSubshapeBinderUncertain:
    if not isinstance(native_result, dict):
        return make_create_subshape_binder_uncertain(
            "INVALID_NATIVE_CREATE_SUBSHAPE_BINDER_RESULT",
    ...
    message = message_value if isinstance(message_value, str) else "Native create_subshape_binder mutation failed"
    ...
    if rollback_failed:
        return make_create_subshape_binder_uncertain(
            "CREATE_SUBSHAPE_BINDER_ROLLBACK_UNCERTAIN",
            message,
            committed=True if committed is True else None,
            native_status=status,
...
def run_create_subshape_binder_native_mutation(
    collaborators: CreateSubshapeBinderCollaborators,
    document_name: DocumentName,
    apply: Callable[[CreateSubshapeBinderDocument], None],
    postcondition: Callable[[CreateSubshapeBinderReadDocument], None],
) -> Literal[True] | CreateSubshapeBinderFailure | CreateSubshapeBinderUncertain:
    ...
    def native_apply(document: object) -> None:
        typed = cast(CreateSubshapeBinderDocument, document)
        ...
        try:
            ...
            state.failure = (
                exc
                if isinstance(exc, CreateSubshapeBinderError)
                else CreateSubshapeBinderError("CREATE_SUBSHAPE_BINDER_FAILED", str(exc) or type(exc).__name__)
            )
            raise _AbortMutation from exc

    def native_postcondition(document: object) -> bool:
        typed = cast(CreateSubshapeBinderReadDocument, document)
        if state.document is not typed or state.failure is not None:
            state.failure = CreateSubshapeBinderError(
        ...
        try:
            ...
            state.failure = (
                exc
                if isinstance(exc, CreateSubshapeBinderError)
                else CreateSubshapeBinderError("CREATE_SUBSHAPE_BINDER_RESULT_FAILED", str(exc) or type(exc).__name__)
            )
            return False
        try:
    ...
    try:
        ...
        return make_create_subshape_binder_uncertain(
            "CREATE_SUBSHAPE_BINDER_NATIVE_EXCEPTION",
            str(exc) or type(exc).__name__,
            committed=None,
        )
    except Exception as exc:
