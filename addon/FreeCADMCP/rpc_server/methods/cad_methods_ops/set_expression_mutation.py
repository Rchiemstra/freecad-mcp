"""Fixed native transaction policy for the ``set_expression`` mutation.
...
class SetExpressionError(RuntimeError):
...
class _NativeMutationState:
    document: SetExpressionDocument | None = None
    failure: SetExpressionError | None = None
...
def _set_expression_native_result(
    native_result: object, state: _NativeMutationState
) -> Literal[True] | SetExpressionFailure | SetExpressionUncertain:
    if not isinstance(native_result, dict):
        return make_set_expression_uncertain(
            "INVALID_NATIVE_SET_EXPRESSION_RESULT",
    ...
    message = message_value if isinstance(message_value, str) else "Native set_expression mutation failed"
    ...
    if rollback_failed:
        return make_set_expression_uncertain(
            "SET_EXPRESSION_ROLLBACK_UNCERTAIN",
            message,
            committed=True if committed is True else None,
            native_status=status,
...
def run_set_expression_native_mutation(
    collaborators: SetExpressionCollaborators,
    document_name: DocumentName,
    apply: Callable[[SetExpressionDocument], None],
    postcondition: Callable[[SetExpressionReadDocument], None],
) -> Literal[True] | SetExpressionFailure | SetExpressionUncertain:
    ...
    def native_apply(document: object) -> None:
        typed = cast(SetExpressionDocument, document)
        ...
        try:
            ...
            state.failure = (
                exc
                if isinstance(exc, SetExpressionError)
                else SetExpressionError("SET_EXPRESSION_FAILED", str(exc) or type(exc).__name__)
            )
            raise _AbortMutation from exc

    def native_postcondition(document: object) -> bool:
        typed = cast(SetExpressionReadDocument, document)
        if state.document is not typed or state.failure is not None:
            state.failure = SetExpressionError(
        ...
        try:
            ...
            state.failure = (
                exc
                if isinstance(exc, SetExpressionError)
                else SetExpressionError("SET_EXPRESSION_RESULT_FAILED", str(exc) or type(exc).__name__)
            )
            return False
        try:
    ...
    try:
        ...
        return make_set_expression_uncertain(
            "SET_EXPRESSION_NATIVE_EXCEPTION",
            str(exc) or type(exc).__name__,
            committed=None,
        )
    except Exception as exc:
    ...
    return _set_expression_native_result(native_result, state)
