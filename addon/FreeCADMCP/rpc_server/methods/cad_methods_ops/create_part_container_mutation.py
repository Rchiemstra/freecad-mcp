"""Fixed native transaction policy for the ``create_part_container`` mutation.
...
class _NativeMutationState:
    document: CreatePartContainerDocument | None = None
    failure: CreatePartContainerError | None = None
...
def _create_part_container_native_result(
    native_result: object, state: _NativeMutationState
) -> Literal[True] | CreatePartContainerFailure | CreatePartContainerUncertain:
    if not isinstance(native_result, dict):
        return make_create_part_container_uncertain(
            "INVALID_NATIVE_CREATE_PART_CONTAINER_RESULT",
    ...
    message = message_value if isinstance(message_value, str) else "Native create_part_container mutation failed"
    ...
    if rollback_failed:
        return make_create_part_container_uncertain(
            "CREATE_PART_CONTAINER_ROLLBACK_UNCERTAIN",
            message,
            committed=True if committed is True else None,
            native_status=status,
...
def run_create_part_container_native_mutation(
    collaborators: CreatePartContainerCollaborators,
    document_name: DocumentName,
    apply: Callable[[CreatePartContainerDocument], None],
    postcondition: Callable[[CreatePartContainerReadDocument], None],
) -> Literal[True] | CreatePartContainerFailure | CreatePartContainerUncertain:
    ...
    def native_apply(document: object) -> None:
        typed = cast(CreatePartContainerDocument, document)
        ...
        try:
            ...
            state.failure = (
                exc
                if isinstance(exc, CreatePartContainerError)
                else CreatePartContainerError("CREATE_PART_CONTAINER_FAILED", str(exc) or type(exc).__name__)
            )
            raise _AbortMutation from exc

    def native_postcondition(document: object) -> bool:
        typed = cast(CreatePartContainerReadDocument, document)
        if state.document is not typed or state.failure is not None:
            state.failure = CreatePartContainerError(
        ...
        try:
            ...
            state.failure = (
                exc
                if isinstance(exc, CreatePartContainerError)
                else CreatePartContainerError("CREATE_PART_CONTAINER_RESULT_FAILED", str(exc) or type(exc).__name__)
            )
            return False
        try:
    ...
    try:
        ...
        return make_create_part_container_uncertain(
            "CREATE_PART_CONTAINER_NATIVE_EXCEPTION",
            str(exc) or type(exc).__name__,
            committed=None,
        )
    except Exception as exc:
    ...
    return _create_part_container_native_result(native_result, state)
