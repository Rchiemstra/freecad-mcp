"""Fixed native transaction policy for the typed ``sweep_feature`` mutation."""
...
class SweepFeatureError(RuntimeError):
...
class _SweepFeatureAbortMutation(RuntimeError):
    """Ask the native coordinator to roll back a failed apply callback."""


@dataclass(slots=True)
class _SweepFeatureNativeMutationState:
    document: FeatureDocument | None = None
    failure: SweepFeatureError | None = None
...
def _sweep_feature_native_result(
    native_result: object, state: _SweepFeatureNativeMutationState
) -> Literal[True] | SweepFeatureFailure | SweepFeatureUncertain:
    if not isinstance(native_result, dict):
        return make_sweep_feature_uncertain(
            "INVALID_NATIVE_SWEEP_FEATURE_RESULT",
    ...
    message = (
        ...
        else "Native sweep_feature mutation failed"
    )
    ...
    if rollback_failed:
        return make_sweep_feature_uncertain(
            "SWEEP_FEATURE_ROLLBACK_UNCERTAIN",
            message,
            committed=True if committed is True else None,
            native_status=status,
    ...
    if status == "Committed" and committed is True and "rollback_succeeded" not in native_result:
        ...
        return make_sweep_feature_uncertain(
            ...
            "Native commit completed without a successful sweep_feature postcondition",
            committed=True,
            native_status=status,
        )
...
def run_sweep_feature_native_mutation(
    ...
    state = _SweepFeatureNativeMutationState()
    ...
    def native_apply(document: FeatureDocument) -> None:
        ...
        try:
            ...
            state.failure = (
                exc
                if isinstance(exc, SweepFeatureError)
                else SweepFeatureError("SWEEP_FEATURE_FAILED", str(exc) or type(exc).__name__)
            )
            raise _SweepFeatureAbortMutation from exc

    def native_postcondition(document: FeatureReadDocument) -> bool:
        if state.document is not document or state.failure is not None:
            state.failure = SweepFeatureError(
        ...
        try:
            ...
            state.failure = (
                exc
                if isinstance(exc, SweepFeatureError)
                else SweepFeatureError("SWEEP_FEATURE_RESULT_FAILED", str(exc) or type(exc).__name__)
            )
            return False
        try:
    ...
    try:
        ...
        return make_sweep_feature_uncertain(
            "SWEEP_FEATURE_NATIVE_EXCEPTION",
            str(exc) or type(exc).__name__,
            committed=None,
        )
    except Exception as exc:
    ...
    return _sweep_feature_native_result(native_result, state)
