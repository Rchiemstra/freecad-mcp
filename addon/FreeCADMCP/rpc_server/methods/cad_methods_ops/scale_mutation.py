"""Fixed native transaction policy for the typed ``scale`` mutation."""
...
class ScaleError(TypedMutationError):
...
def _scale_native_result(
    native_result: object, state: _NativeMutationState
) -> Literal[True] | ScaleFailure | ScaleUncertain:
    if not isinstance(native_result, dict):
        return make_scale_uncertain(
            "INVALID_NATIVE_SCALE_RESULT",
    ...
    message = message_value if isinstance(message_value, str) else "Native scale mutation failed"
    ...
    if status == "Committed" and committed is True and "rollback_succeeded" not in native_result:
        ...
        return make_scale_uncertain(
            ...
            "Native commit completed without a successful scale postcondition",
            committed=True,
            native_status=status,
        )
...
def run_scale_native_mutation(
    ...
    def native_apply(document: object) -> None:
        ...
        try:
            ...
            state.failure = (
                ...
                else ScaleError("SCALE_FAILED", str(exc) or type(exc).__name__)
            )
    ...
    def native_postcondition(document: object) -> bool:
        ...
        try:
            ...
            state.failure = (
                exc
                if isinstance(exc, TypedMutationError)
                else ScaleError("SCALE_RESULT_FAILED", str(exc) or type(exc).__name__)
            )
            return False
        try:
    ...
    return _scale_native_result(native_result, state)
