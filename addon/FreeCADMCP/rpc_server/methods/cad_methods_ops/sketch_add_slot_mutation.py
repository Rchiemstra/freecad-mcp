"""Fixed native transaction policy for the typed ``sketch_add_slot`` mutation."""
...
class SketchAddSlotError(RuntimeError):
    def __init__(self, code: str, message: str) -> None:
        super().__init__(message)
        self.code = code


class _AbortSketchAddSlotMutation(RuntimeError):
    """Ask the native coordinator to roll back a failed apply callback."""


@dataclass(slots=True)
class _NativeSketchAddSlotMutationState:
    document: SketchDocument | None = None
    failure: SketchAddSlotError | None = None
...
def _sketch_add_slot_native_result(
    native_result: object, state: _NativeSketchAddSlotMutationState
) -> Literal[True] | SketchAddSlotFailure | SketchAddSlotUncertain:
    if not isinstance(native_result, dict):
        return make_sketch_add_slot_uncertain(
            "INVALID_NATIVE_SKETCH_ADD_SLOT_RESULT",
    ...
    message = message_value if isinstance(message_value, str) else "Native sketch_add_slot mutation failed"
    ...
    if rollback_failed:
        return make_sketch_add_slot_uncertain(
            "SKETCH_ADD_SLOT_ROLLBACK_UNCERTAIN",
            message,
            committed=True if committed is True else None,
            native_status=status,
    ...
    if status == "Committed" and committed is True and "rollback_succeeded" not in native_result:
        ...
        return make_sketch_add_slot_uncertain(
            ...
            "Native commit completed without a successful sketch_add_slot postcondition",
            committed=True,
            native_status=status,
        )
...
def run_sketch_add_slot_native_mutation(
    ...
    def native_apply(document: object) -> None:
        ...
        try:
            ...
            state.failure = (
                exc
                if isinstance(exc, SketchAddSlotError)
                else SketchAddSlotError("SKETCH_ADD_SLOT_FAILED", str({exc}) or type({exc}).__name__)
            )
            raise _AbortSketchAddSlotMutation from exc
    ...
    def native_postcondition(document: object) -> bool:
        ...
        if state.document is not document or state.failure is not None:
            state.failure = SketchAddSlotError(
        ...
        try:
            ...
            state.failure = (
                exc
                if isinstance(exc, SketchAddSlotError)
                else SketchAddSlotError("SKETCH_ADD_SLOT_RESULT_FAILED", str({exc}) or type({exc}).__name__)
            )
            return False
        try:
    ...
    try:
        ...
        return make_sketch_add_slot_uncertain(
            "SKETCH_ADD_SLOT_NATIVE_EXCEPTION",
            str({exc}) or type({exc}).__name__,
            committed=None,
        )
    except Exception as exc:
    ...
    return _sketch_add_slot_native_result(native_result, state)
