"""Fixed native transaction policy for the typed ``import_step`` mutation."""
...
class ImportStepError(TypedMutationError):
...
def _import_step_native_result(
    native_result: object, state: _NativeMutationState
) -> Literal[True] | ImportStepFailure | ImportStepUncertain:
    if not isinstance(native_result, dict):
        return make_import_step_uncertain(
            "INVALID_NATIVE_IMPORT_STEP_RESULT",
    ...
    message = message_value if isinstance(message_value, str) else "Native import_step mutation failed"
    ...
    if rollback_failed:
        return make_import_step_uncertain(
            "IMPORT_STEP_ROLLBACK_UNCERTAIN",
            message,
            committed=True if committed is True else None,
            native_status=status,
    ...
    if status == "Committed" and committed is True and "rollback_succeeded" not in native_result:
        ...
        return make_import_step_uncertain(
            ...
            "Native commit completed without a successful import_step postcondition",
            committed=True,
            native_status=status,
        )
...
def run_import_step_native_mutation(
    ...
    def native_apply(document: object) -> None:
        ...
        try:
            ...
            state.failure = (
                ...
                else ImportStepError("IMPORT_STEP_FAILED", str(exc) or type(exc).__name__)
            )
    ...
    def native_postcondition(document: object) -> bool:
        ...
        try:
            ...
            state.failure = (
                exc
                if isinstance(exc, TypedMutationError)
                else ImportStepError("IMPORT_STEP_RESULT_FAILED", str(exc) or type(exc).__name__)
            )
            return False
        try:
    ...
    return _import_step_native_result(native_result, state)
