"""Fixed native transaction policy for the typed ``sketch_add_regular_polygon`` mutation."""
...
class _AbortSketchAddRegularPolygonMutation(RuntimeError):
    """Ask the native coordinator to roll back a failed apply callback."""


@dataclass(slots=True)
class _NativeSketchAddRegularPolygonMutationState:
    document: SketchDocument | None = None
    failure: SketchAddRegularPolygonError | None = None
...
def _sketch_add_regular_polygon_native_result(
    native_result: object, state: _NativeSketchAddRegularPolygonMutationState
) -> Literal[True] | SketchAddRegularPolygonFailure | SketchAddRegularPolygonUncertain:
    if not isinstance(native_result, dict):
        return make_sketch_add_regular_polygon_uncertain(
            "INVALID_NATIVE_SKETCH_ADD_REGULAR_POLYGON_RESULT",
    ...
    message = message_value if isinstance(message_value, str) else "Native sketch_add_regular_polygon mutation failed"
    ...
    if rollback_failed:
        return make_sketch_add_regular_polygon_uncertain(
            "SKETCH_ADD_REGULAR_POLYGON_ROLLBACK_UNCERTAIN",
            message,
            committed=True if committed is True else None,
            native_status=status,
    ...
    if status == "Committed" and committed is True and "rollback_succeeded" not in native_result:
        ...
        return make_sketch_add_regular_polygon_uncertain(
            ...
            "Native commit completed without a successful sketch_add_regular_polygon postcondition",
            committed=True,
            native_status=status,
        )
...
def run_sketch_add_regular_polygon_native_mutation(
    ...
    state = _NativeSketchAddRegularPolygonMutationState()
    ...
    def native_apply(document: object) -> None:
        ...
        try:
            ...
            state.failure = (
                exc
                if isinstance(exc, SketchAddRegularPolygonError)
                else SketchAddRegularPolygonError("SKETCH_ADD_REGULAR_POLYGON_FAILED", str(exc) or type(exc).__name__)
            )
            raise _AbortSketchAddRegularPolygonMutation from exc
    ...
    def native_postcondition(document: object) -> bool:
        ...
        if state.document is not document or state.failure is not None:
            state.failure = SketchAddRegularPolygonError(
        ...
        try:
            ...
            state.failure = (
                exc
                if isinstance(exc, SketchAddRegularPolygonError)
                else SketchAddRegularPolygonError("SKETCH_ADD_REGULAR_POLYGON_RESULT_FAILED", str(exc) or type(exc).__name__)
            )
            return False
        try:
    ...
    try:
        ...
        return make_sketch_add_regular_polygon_uncertain(
            "SKETCH_ADD_REGULAR_POLYGON_NATIVE_EXCEPTION",
            str(exc) or type(exc).__name__,
            committed=None,
        )
    except Exception as exc:
    ...
    return _sketch_add_regular_polygon_native_result(native_result, state)
