"""Fixed native transaction policy for the ``create_placement_datum`` mutation.
...
class _NativeMutationState:
    document: CreatePlacementDatumDocument | None = None
    failure: CreatePlacementDatumError | None = None
...
def _create_placement_datum_native_result(
    native_result: object, state: _NativeMutationState
) -> Literal[True] | CreatePlacementDatumFailure | CreatePlacementDatumUncertain:
    if not isinstance(native_result, dict):
        return make_create_placement_datum_uncertain(
            "INVALID_NATIVE_CREATE_PLACEMENT_DATUM_RESULT",
    ...
    message = message_value if isinstance(message_value, str) else "Native create_placement_datum mutation failed"
    ...
    if rollback_failed:
        return make_create_placement_datum_uncertain(
            "CREATE_PLACEMENT_DATUM_ROLLBACK_UNCERTAIN",
            message,
            committed=True if committed is True else None,
            native_status=status,
...
def run_create_placement_datum_native_mutation(
    collaborators: CreatePlacementDatumCollaborators,
    document_name: DocumentName,
    apply: Callable[[CreatePlacementDatumDocument], None],
    postcondition: Callable[[CreatePlacementDatumReadDocument], None],
) -> Literal[True] | CreatePlacementDatumFailure | CreatePlacementDatumUncertain:
    ...
    def native_apply(document: object) -> None:
        typed = cast(CreatePlacementDatumDocument, document)
        ...
        try:
            ...
            state.failure = (
                exc
                if isinstance(exc, CreatePlacementDatumError)
                else CreatePlacementDatumError("CREATE_PLACEMENT_DATUM_FAILED", str(exc) or type(exc).__name__)
            )
            raise _AbortMutation from exc

    def native_postcondition(document: object) -> bool:
        typed = cast(CreatePlacementDatumReadDocument, document)
        if state.document is not typed or state.failure is not None:
            state.failure = CreatePlacementDatumError(
        ...
        try:
            ...
            state.failure = (
                exc
                if isinstance(exc, CreatePlacementDatumError)
                else CreatePlacementDatumError("CREATE_PLACEMENT_DATUM_RESULT_FAILED", str(exc) or type(exc).__name__)
            )
            return False
        try:
    ...
    try:
        ...
        return make_create_placement_datum_uncertain(
            "CREATE_PLACEMENT_DATUM_NATIVE_EXCEPTION",
            str(exc) or type(exc).__name__,
            committed=None,
        )
    except Exception as exc:
    ...
    return _create_placement_datum_native_result(native_result, state)
