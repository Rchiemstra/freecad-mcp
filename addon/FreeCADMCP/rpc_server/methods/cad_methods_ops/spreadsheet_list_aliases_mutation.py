"""Fixed native transaction policy for the ``spreadsheet_list_aliases`` mutation.
...
class _NativeMutationState:
    document: SpreadsheetListAliasesDocument | None = None
    failure: SpreadsheetListAliasesError | None = None
...
def _spreadsheet_list_aliases_native_result(
    native_result: object, state: _NativeMutationState
) -> Literal[True] | SpreadsheetListAliasesFailure | SpreadsheetListAliasesUncertain:
    if not isinstance(native_result, dict):
        return make_spreadsheet_list_aliases_uncertain(
            "INVALID_NATIVE_SPREADSHEET_LIST_ALIASES_RESULT",
    ...
    message = message_value if isinstance(message_value, str) else "Native spreadsheet_list_aliases mutation failed"
    ...
    if rollback_failed:
        return make_spreadsheet_list_aliases_uncertain(
            "SPREADSHEET_LIST_ALIASES_ROLLBACK_UNCERTAIN",
            message,
            committed=True if committed is True else None,
            native_status=status,
...
def run_spreadsheet_list_aliases_native_mutation(
    collaborators: SpreadsheetListAliasesCollaborators,
    document_name: DocumentName,
    apply: Callable[[SpreadsheetListAliasesDocument], None],
    postcondition: Callable[[SpreadsheetListAliasesReadDocument], None],
) -> Literal[True] | SpreadsheetListAliasesFailure | SpreadsheetListAliasesUncertain:
    ...
    def native_apply(document: object) -> None:
        typed = cast(SpreadsheetListAliasesDocument, document)
        ...
        try:
            ...
            state.failure = (
                exc
                if isinstance(exc, SpreadsheetListAliasesError)
                else SpreadsheetListAliasesError("SPREADSHEET_LIST_ALIASES_FAILED", str(exc) or type(exc).__name__)
            )
            raise _AbortMutation from exc

    def native_postcondition(document: object) -> bool:
        typed = cast(SpreadsheetListAliasesReadDocument, document)
        if state.document is not typed or state.failure is not None:
            state.failure = SpreadsheetListAliasesError(
        ...
        try:
            ...
            state.failure = (
                exc
                if isinstance(exc, SpreadsheetListAliasesError)
                else SpreadsheetListAliasesError("SPREADSHEET_LIST_ALIASES_RESULT_FAILED", str(exc) or type(exc).__name__)
            )
            return False
        try:
    ...
    try:
        ...
        return make_spreadsheet_list_aliases_uncertain(
            "SPREADSHEET_LIST_ALIASES_NATIVE_EXCEPTION",
            str(exc) or type(exc).__name__,
            committed=None,
        )
    except Exception as exc:
    ...
    return _spreadsheet_list_aliases_native_result(native_result, state)
