"""Native transaction policy for the typed ``repair_references`` mutation."""
...
class RepairReferencesError(RuntimeError):
...
class _NativeMutationState:
    ...
    failure: RepairReferencesError | None = None
...
def _repair_references_native_result(
    ...
    if not isinstance(native_result, dict):
        return make_repair_references_uncertain(
            "INVALID_NATIVE_REPAIR_REFERENCES_RESULT",
    ...
    message = message_value if isinstance(message_value, str) else "Native repair_references mutation failed"
    ...
    if rollback_failed:
        return make_repair_references_uncertain(
            "REPAIR_REFERENCES_ROLLBACK_UNCERTAIN",
            message,
            committed=True if committed is True else None,
            native_status=status,
    ...
    if status == "Committed" and committed is True and "rollback_succeeded" not in native_result:
        ...
        return make_repair_references_uncertain(
            ...
            "Native commit completed without a successful repair_references postcondition",
            committed=True,
            native_status=status,
        )
...
def run_repair_references_native_mutation(
    ...
    def native_apply(document: object) -> None:
        ...
        try:
            ...
            state.failure = (
                exc
                if isinstance(exc, RepairReferencesError)
                else RepairReferencesError("REPAIR_REFERENCES_FAILED", str(exc) or type(exc).__name__)
            )
    ...
    def native_postcondition(document: object) -> bool:
        if state.document is not document or state.failure is not None:
            state.failure = RepairReferencesError(
        ...
        try:
            ...
            state.failure = (
                exc
                if isinstance(exc, RepairReferencesError)
                else RepairReferencesError("REPAIR_REFERENCES_RESULT_FAILED", str(exc) or type(exc).__name__)
            )
            return False
        try:
            collaborators.validate_document_invariants(cast(RepairReferencesReadDocument, document))
        except Exception as exc:
            state.failure = RepairReferencesError(
    ...
    try:
        ...
        return make_repair_references_uncertain(
            "REPAIR_REFERENCES_NATIVE_EXCEPTION",
            str(exc) or type(exc).__name__,
            committed=None,
        )
    except Exception as exc:
