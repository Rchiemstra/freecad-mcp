"""Fixed native transaction policy for the typed ``center_of_mass`` mutation."""
...
class CenterOfMassError(TypedMutationError):
...
def _center_of_mass_native_result(
    native_result: object, state: _NativeMutationState
) -> Literal[True] | CenterOfMassFailure | CenterOfMassUncertain:
    if not isinstance(native_result, dict):
        return make_center_of_mass_uncertain(
            "INVALID_NATIVE_CENTER_OF_MASS_RESULT",
    ...
    message = message_value if isinstance(message_value, str) else "Native center_of_mass mutation failed"
    ...
    if rollback_failed:
        return make_center_of_mass_uncertain(
            "CENTER_OF_MASS_ROLLBACK_UNCERTAIN",
            message,
            committed=True if committed is True else None,
            native_status=status,
    ...
    if status == "Committed" and committed is True and "rollback_succeeded" not in native_result:
        ...
        return make_center_of_mass_uncertain(
            ...
            "Native commit completed without a successful center_of_mass postcondition",
            committed=True,
            native_status=status,
        )
...
def run_center_of_mass_native_mutation(
    ...
    def native_apply(document: object) -> None:
        ...
        try:
            ...
            state.failure = (
                ...
                else CenterOfMassError("CENTER_OF_MASS_FAILED", str(exc) or type(exc).__name__)
            )
    ...
    def native_postcondition(document: object) -> bool:
        if state.document is not document or state.failure is not None:
            state.failure = CenterOfMassError(
        ...
        try:
            ...
            state.failure = (
                exc
                if isinstance(exc, TypedMutationError)
                else CenterOfMassError("CENTER_OF_MASS_RESULT_FAILED", str(exc) or type(exc).__name__)
            )
            return False
        try:
    ...
    try:
        ...
        return make_center_of_mass_uncertain(
            "CENTER_OF_MASS_NATIVE_EXCEPTION",
            str(exc) or type(exc).__name__,
            committed=None,
        )
    except Exception as exc:
    ...
    return _center_of_mass_native_result(native_result, state)
