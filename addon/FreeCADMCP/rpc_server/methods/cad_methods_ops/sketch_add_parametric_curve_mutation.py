"""Fixed native transaction policy for the typed ``sketch_add_parametric_curve`` mutation."""
...
class SketchAddParametricCurveError(RuntimeError):
    def __init__(self, code: str, message: str) -> None:
        super().__init__(message)
        self.code = code


class _AbortSketchAddParametricCurveMutation(RuntimeError):
    """Ask the native coordinator to roll back a failed apply callback."""


@dataclass(slots=True)
class _NativeSketchAddParametricCurveMutationState:
    document: SketchDocument | None = None
    failure: SketchAddParametricCurveError | None = None
...
def _sketch_add_parametric_curve_native_result(
    native_result: object, state: _NativeSketchAddParametricCurveMutationState
    ...
    if not isinstance(native_result, dict):
        return make_sketch_add_parametric_curve_uncertain(
            "INVALID_NATIVE_SKETCH_ADD_PARAMETRIC_CURVE_RESULT",
    ...
    message = message_value if isinstance(message_value, str) else "Native sketch_add_parametric_curve mutation failed"
    ...
    if rollback_failed:
        return make_sketch_add_parametric_curve_uncertain(
            "SKETCH_ADD_PARAMETRIC_CURVE_ROLLBACK_UNCERTAIN",
            message,
            committed=True if committed is True else None,
            native_status=status,
    ...
    if status == "Committed" and committed is True and "rollback_succeeded" not in native_result:
        ...
        return make_sketch_add_parametric_curve_uncertain(
            ...
            "Native commit completed without a successful sketch_add_parametric_curve postcondition",
            committed=True,
            native_status=status,
        )
...
def run_sketch_add_parametric_curve_native_mutation(
