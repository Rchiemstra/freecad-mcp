"""Fixed native transaction policy for the typed ``sketch_split`` mutation."""
...
class SketchSplitError(RuntimeError):
    def __init__(self, code: str, message: str) -> None:
        super().__init__(message)
        self.code = code


class _AbortSketchSplitMutation(RuntimeError):
    """Ask the native coordinator to roll back a failed apply callback."""


@dataclass(slots=True)
class _NativeSketchSplitMutationState:
    document: SketchDocument | None = None
    failure: SketchSplitError | None = None
...
def _sketch_split_native_result(
    native_result: object, state: _NativeSketchSplitMutationState
) -> Literal[True] | SketchSplitFailure | SketchSplitUncertain:
    if not isinstance(native_result, dict):
        return make_sketch_split_uncertain(
            "INVALID_NATIVE_SKETCH_SPLIT_RESULT",
    ...
    message = message_value if isinstance(message_value, str) else "Native sketch_split mutation failed"
    ...
    if rollback_failed:
        return make_sketch_split_uncertain(
            "SKETCH_SPLIT_ROLLBACK_UNCERTAIN",
            message,
            committed=True if committed is True else None,
            native_status=status,
    ...
    if status == "Committed" and committed is True and "rollback_succeeded" not in native_result:
        ...
        return make_sketch_split_uncertain(
            ...
            "Native commit completed without a successful sketch_split postcondition",
            committed=True,
            native_status=status,
        )
