"""Fixed native transaction policy for the typed ``boolean_difference`` mutation."""
...
class BooleanDifferenceError(RuntimeError):
...
class _BooleanDifferenceAbortMutation(RuntimeError):
    """Ask the native coordinator to roll back a failed apply callback."""


@dataclass(slots=True)
class _BooleanDifferenceNativeMutationState:
    document: FeatureDocument | None = None
    failure: BooleanDifferenceError | None = None
...
def _boolean_difference_native_result(
    native_result: object, state: _BooleanDifferenceNativeMutationState
) -> Literal[True] | BooleanDifferenceFailure | BooleanDifferenceUncertain:
    if not isinstance(native_result, dict):
        return make_boolean_difference_uncertain(
            "INVALID_NATIVE_BOOLEAN_DIFFERENCE_RESULT",
    ...
    message = (
        ...
        else "Native boolean_difference mutation failed"
    )
    ...
    if status == "Committed" and committed is True and "rollback_succeeded" not in native_result:
        ...
        return make_boolean_difference_uncertain(
            ...
            "Native commit completed without a successful boolean_difference postcondition",
            committed=True,
            native_status=status,
        )
...
def run_boolean_difference_native_mutation(
    ...
    state = _BooleanDifferenceNativeMutationState()
    ...
    def native_apply(document: FeatureDocument) -> None:
        ...
        try:
            ...
            state.failure = (
                exc
                if isinstance(exc, BooleanDifferenceError)
                else BooleanDifferenceError("BOOLEAN_DIFFERENCE_FAILED", str(exc) or type(exc).__name__)
            )
            raise _BooleanDifferenceAbortMutation from exc

    def native_postcondition(document: FeatureReadDocument) -> bool:
        if state.document is not document or state.failure is not None:
            state.failure = BooleanDifferenceError(
        ...
        try:
            ...
            state.failure = (
                exc
                if isinstance(exc, BooleanDifferenceError)
                else BooleanDifferenceError("BOOLEAN_DIFFERENCE_RESULT_FAILED", str(exc) or type(exc).__name__)
            )
            return False
        try:
    ...
    try:
        ...
        return make_boolean_difference_uncertain(
            "BOOLEAN_DIFFERENCE_NATIVE_EXCEPTION",
            str(exc) or type(exc).__name__,
            committed=None,
        )
    except Exception as exc:
    ...
    return _boolean_difference_native_result(native_result, state)
