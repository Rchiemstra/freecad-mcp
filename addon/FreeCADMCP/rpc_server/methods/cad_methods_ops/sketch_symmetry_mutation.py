"""Fixed native transaction policy for the typed ``sketch_symmetry`` mutation."""
...
class SketchSymmetryError(RuntimeError):
    def __init__(self, code: str, message: str) -> None:
        super().__init__(message)
        self.code = code


class _AbortSketchSymmetryMutation(RuntimeError):
    """Ask the native coordinator to roll back a failed apply callback."""


@dataclass(slots=True)
class _NativeSketchSymmetryMutationState:
    document: SketchDocument | None = None
    failure: SketchSymmetryError | None = None
...
def _sketch_symmetry_native_result(
    native_result: object, state: _NativeSketchSymmetryMutationState
) -> Literal[True] | SketchSymmetryFailure | SketchSymmetryUncertain:
    if not isinstance(native_result, dict):
        return make_sketch_symmetry_uncertain(
            "INVALID_NATIVE_SKETCH_SYMMETRY_RESULT",
    ...
    message = message_value if isinstance(message_value, str) else "Native sketch_symmetry mutation failed"
    ...
    if rollback_failed:
        return make_sketch_symmetry_uncertain(
            "SKETCH_SYMMETRY_ROLLBACK_UNCERTAIN",
            message,
            committed=True if committed is True else None,
            native_status=status,
    ...
    if status == "Committed" and committed is True and "rollback_succeeded" not in native_result:
        ...
        return make_sketch_symmetry_uncertain(
            ...
            "Native commit completed without a successful sketch_symmetry postcondition",
            committed=True,
            native_status=status,
        )
...
def run_sketch_symmetry_native_mutation(
    ...
    state = _NativeSketchSymmetryMutationState()
    ...
    def native_apply(document: object) -> None:
        ...
        try:
            ...
            state.failure = (
                exc
                if isinstance(exc, SketchSymmetryError)
                else SketchSymmetryError("SKETCH_SYMMETRY_FAILED", str({exc}) or type({exc}).__name__)
            )
            raise _AbortSketchSymmetryMutation from exc
    ...
    def native_postcondition(document: object) -> bool:
        ...
        if state.document is not document or state.failure is not None:
            state.failure = SketchSymmetryError(
        ...
        try:
            ...
            state.failure = (
                exc
                if isinstance(exc, SketchSymmetryError)
                else SketchSymmetryError("SKETCH_SYMMETRY_RESULT_FAILED", str({exc}) or type({exc}).__name__)
            )
            return False
        try:
    ...
    try:
        ...
        return make_sketch_symmetry_uncertain(
            "SKETCH_SYMMETRY_NATIVE_EXCEPTION",
            str({exc}) or type({exc}).__name__,
            committed=None,
        )
    except Exception as exc:
    ...
    return _sketch_symmetry_native_result(native_result, state)
