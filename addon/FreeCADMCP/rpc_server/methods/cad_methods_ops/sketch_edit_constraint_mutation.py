"""Fixed native transaction policy for the typed ``sketch_edit_constraint`` mutation.
...
class _AbortSketchEditConstraintMutation(RuntimeError):
    """Ask the native coordinator to roll back a failed apply callback."""


@dataclass(slots=True)
class _NativeSketchEditConstraintMutationState:
    document: object | None = None
    failure: SketchEditConstraintError | None = None
...
def _sketch_edit_constraint_native_result(
    native_result: object, state: _NativeSketchEditConstraintMutationState
    ...
    if not isinstance(native_result, dict):
        return make_sketch_edit_constraint_uncertain(
            "INVALID_NATIVE_SKETCH_EDIT_CONSTRAINT_RESULT",
    ...
    message = message_value if isinstance(message_value, str) else "Native sketch_edit_constraint mutation failed"
    ...
    if status == "Committed" and committed is True and "rollback_succeeded" not in native_result:
        ...
        return make_sketch_edit_constraint_uncertain(
            ...
            "Native commit completed without a successful sketch_edit_constraint postcondition",
            committed=True,
            native_status=status,
        )
...
def run_sketch_edit_constraint_native_mutation(
    ...
    def native_apply(document: object) -> None:
        ...
        try:
            apply(cast(SketchEditConstraintDocument, document))
        except Exception as exc:
            state.failure = (
                exc
                if isinstance(exc, SketchEditConstraintError)
                else SketchEditConstraintError("SKETCH_EDIT_CONSTRAINT_FAILED", str(exc) or type(exc).__name__)
            )
            raise _AbortSketchEditConstraintMutation from exc

    def native_postcondition(document: object) -> bool:
        if state.document is not document or state.failure is not None:
            state.failure = SketchEditConstraintError(
                "DOCUMENT_IDENTITY_MISMATCH",
                "Native mutation callbacks did not receive the same admitted document",
            )
            return False
        admitted = cast(SketchEditConstraintReadDocument, document)
        try:
            ...
            state.failure = (
                exc
                if isinstance(exc, SketchEditConstraintError)
                else SketchEditConstraintError("SKETCH_EDIT_CONSTRAINT_RESULT_FAILED", str(exc) or type(exc).__name__)
            )
            return False
        try:
    ...
    try:
        ...
        return make_sketch_edit_constraint_uncertain(
            "SKETCH_EDIT_CONSTRAINT_NATIVE_EXCEPTION",
            str(exc) or type(exc).__name__,
            committed=None,
        )
    except Exception as exc:
