"""Fixed native transaction policy for the typed ``sketch_import_points`` mutation."""
...
class SketchImportPointsError(RuntimeError):
    def __init__(self, code: str, message: str) -> None:
        super().__init__(message)
        self.code = code


class _AbortSketchImportPointsMutation(RuntimeError):
    """Ask the native coordinator to roll back a failed apply callback."""


@dataclass(slots=True)
class _NativeSketchImportPointsMutationState:
    document: SketchDocument | None = None
    failure: SketchImportPointsError | None = None
...
def _sketch_import_points_native_result(
    native_result: object, state: _NativeSketchImportPointsMutationState
) -> Literal[True] | SketchImportPointsFailure | SketchImportPointsUncertain:
    if not isinstance(native_result, dict):
        return make_sketch_import_points_uncertain(
            "INVALID_NATIVE_SKETCH_IMPORT_POINTS_RESULT",
    ...
    message = message_value if isinstance(message_value, str) else "Native sketch_import_points mutation failed"
    ...
    if rollback_failed:
        return make_sketch_import_points_uncertain(
            "SKETCH_IMPORT_POINTS_ROLLBACK_UNCERTAIN",
            message,
            committed=True if committed is True else None,
            native_status=status,
    ...
    if status == "Committed" and committed is True and "rollback_succeeded" not in native_result:
        ...
        return make_sketch_import_points_uncertain(
            ...
            "Native commit completed without a successful sketch_import_points postcondition",
            committed=True,
            native_status=status,
        )
...
def run_sketch_import_points_native_mutation(
    ...
    state = _NativeSketchImportPointsMutationState()
    ...
    def native_apply(document: object) -> None:
        ...
        try:
            ...
            state.failure = (
                exc
                if isinstance(exc, SketchImportPointsError)
                else SketchImportPointsError("SKETCH_IMPORT_POINTS_FAILED", str(exc) or type(exc).__name__)
            )
            raise _AbortSketchImportPointsMutation from exc
    ...
    def native_postcondition(document: object) -> bool:
        ...
        if state.document is not document or state.failure is not None:
            state.failure = SketchImportPointsError(
        ...
        try:
            ...
            state.failure = (
                exc
                if isinstance(exc, SketchImportPointsError)
                else SketchImportPointsError("SKETCH_IMPORT_POINTS_RESULT_FAILED", str(exc) or type(exc).__name__)
            )
            return False
        try:
    ...
    try:
        ...
        return make_sketch_import_points_uncertain(
            "SKETCH_IMPORT_POINTS_NATIVE_EXCEPTION",
            str(exc) or type(exc).__name__,
            committed=None,
        )
    except Exception as exc:
    ...
    return _sketch_import_points_native_result(native_result, state)
