"""Fixed native transaction policy for the ``preview_attachment`` mutation.
...
class _NativeMutationState:
    document: PreviewAttachmentDocument | None = None
    failure: PreviewAttachmentError | None = None
...
def _preview_attachment_native_result(
    ...
    if not isinstance(native_result, dict):
        return make_preview_attachment_uncertain(
            "INVALID_NATIVE_PREVIEW_ATTACHMENT_RESULT",
    ...
    message = message_value if isinstance(message_value, str) else "Native preview_attachment mutation failed"
    ...
    if rollback_failed:
        return make_preview_attachment_uncertain(
            "PREVIEW_ATTACHMENT_ROLLBACK_UNCERTAIN",
            message,
            committed=True if committed is True else None,
            native_status=status,
...
def run_preview_attachment_native_mutation(
    ...
    def native_apply(document: object) -> None:
        typed = cast(PreviewAttachmentDocument, document)
        ...
        try:
            ...
            state.failure = (
                exc
                if isinstance(exc, PreviewAttachmentError)
                else PreviewAttachmentError("PREVIEW_ATTACHMENT_FAILED", str(exc) or type(exc).__name__)
            )
            raise _AbortMutation from exc

    def native_postcondition(document: object) -> bool:
        typed = cast(PreviewAttachmentReadDocument, document)
        if state.document is not typed or state.failure is not None:
            state.failure = PreviewAttachmentError(
        ...
        try:
            ...
            state.failure = (
                exc
                if isinstance(exc, PreviewAttachmentError)
                else PreviewAttachmentError("PREVIEW_ATTACHMENT_RESULT_FAILED", str(exc) or type(exc).__name__)
            )
            return False
        try:
    ...
    try:
        ...
        return make_preview_attachment_uncertain(
            "PREVIEW_ATTACHMENT_NATIVE_EXCEPTION",
            str(exc) or type(exc).__name__,
            committed=None,
        )
    except Exception as exc:
    ...
    return _preview_attachment_native_result(native_result, state)
