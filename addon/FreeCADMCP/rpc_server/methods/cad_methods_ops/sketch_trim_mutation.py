"""Fixed native transaction policy for the typed ``sketch_trim`` mutation."""
...
class SketchTrimError(RuntimeError):
    def __init__(self, code: str, message: str) -> None:
        super().__init__(message)
        self.code = code


class _AbortSketchTrimMutation(RuntimeError):
    """Ask the native coordinator to roll back a failed apply callback."""


@dataclass(slots=True)
class _NativeSketchTrimMutationState:
    document: SketchDocument | None = None
    failure: SketchTrimError | None = None
...
def _sketch_trim_native_result(
    native_result: object, state: _NativeSketchTrimMutationState
) -> Literal[True] | SketchTrimFailure | SketchTrimUncertain:
    if not isinstance(native_result, dict):
        return make_sketch_trim_uncertain(
            "INVALID_NATIVE_SKETCH_TRIM_RESULT",
    ...
    message = message_value if isinstance(message_value, str) else "Native sketch_trim mutation failed"
    ...
    if rollback_failed:
        return make_sketch_trim_uncertain(
            "SKETCH_TRIM_ROLLBACK_UNCERTAIN",
            message,
            committed=True if committed is True else None,
            native_status=status,
    ...
    if status == "Committed" and committed is True and "rollback_succeeded" not in native_result:
        ...
        return make_sketch_trim_uncertain(
            ...
            "Native commit completed without a successful sketch_trim postcondition",
            committed=True,
            native_status=status,
        )
