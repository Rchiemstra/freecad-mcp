"""Fixed native transaction policy for the typed ``loft_feature`` mutation."""
...
class LoftFeatureError(RuntimeError):
...
class _LoftFeatureAbortMutation(RuntimeError):
    """Ask the native coordinator to roll back a failed apply callback."""


@dataclass(slots=True)
class _LoftFeatureNativeMutationState:
    document: FeatureDocument | None = None
    failure: LoftFeatureError | None = None
...
def _loft_feature_native_result(
    native_result: object, state: _LoftFeatureNativeMutationState
) -> Literal[True] | LoftFeatureFailure | LoftFeatureUncertain:
    if not isinstance(native_result, dict):
        return make_loft_feature_uncertain(
            "INVALID_NATIVE_LOFT_FEATURE_RESULT",
    ...
    message = (
        ...
        else "Native loft_feature mutation failed"
    )
    ...
    if rollback_failed:
        return make_loft_feature_uncertain(
            "LOFT_FEATURE_ROLLBACK_UNCERTAIN",
            message,
            committed=True if committed is True else None,
            native_status=status,
    ...
    if status == "Committed" and committed is True and "rollback_succeeded" not in native_result:
        ...
        return make_loft_feature_uncertain(
            ...
            "Native commit completed without a successful loft_feature postcondition",
            committed=True,
            native_status=status,
        )
...
def run_loft_feature_native_mutation(
    ...
    state = _LoftFeatureNativeMutationState()
    ...
    def native_apply(document: FeatureDocument) -> None:
        ...
        try:
            ...
            state.failure = (
                exc
                if isinstance(exc, LoftFeatureError)
                else LoftFeatureError("LOFT_FEATURE_FAILED", str(exc) or type(exc).__name__)
            )
            raise _LoftFeatureAbortMutation from exc

    def native_postcondition(document: FeatureReadDocument) -> bool:
        if state.document is not document or state.failure is not None:
            state.failure = LoftFeatureError(
        ...
        try:
            ...
            state.failure = (
                exc
                if isinstance(exc, LoftFeatureError)
                else LoftFeatureError("LOFT_FEATURE_RESULT_FAILED", str(exc) or type(exc).__name__)
            )
            return False
        try:
    ...
    return _loft_feature_native_result(native_result, state)
