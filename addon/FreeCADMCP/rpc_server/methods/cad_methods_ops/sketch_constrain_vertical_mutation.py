"""Fixed native transaction policy for the typed ``sketch_constrain_vertical`` mutation."""
...
class _AbortSketchConstrainVerticalMutation(RuntimeError):
    """Ask the native coordinator to roll back a failed apply callback."""


@dataclass(slots=True)
class _NativeSketchConstrainVerticalMutationState:
    document: SketchDocument | None = None
    failure: SketchConstrainVerticalError | None = None
...
def _sketch_constrain_vertical_native_result(
    native_result: object, state: _NativeSketchConstrainVerticalMutationState
) -> Literal[True] | SketchConstrainVerticalFailure | SketchConstrainVerticalUncertain:
    if not isinstance(native_result, dict):
        return make_sketch_constrain_vertical_uncertain(
            "INVALID_NATIVE_SKETCH_CONSTRAIN_VERTICAL_RESULT",
    ...
    message = message_value if isinstance(message_value, str) else "Native sketch_constrain_vertical mutation failed"
    ...
    if rollback_failed:
        return make_sketch_constrain_vertical_uncertain(
            "SKETCH_CONSTRAIN_VERTICAL_ROLLBACK_UNCERTAIN",
            message,
            committed=True if committed is True else None,
            native_status=status,
    ...
    if status == "Committed" and committed is True and "rollback_succeeded" not in native_result:
        ...
        return make_sketch_constrain_vertical_uncertain(
            ...
            "Native commit completed without a successful sketch_constrain_vertical postcondition",
            committed=True,
            native_status=status,
        )
...
def run_sketch_constrain_vertical_native_mutation(
    ...
    state = _NativeSketchConstrainVerticalMutationState()
    ...
    def native_apply(document: object) -> None:
        ...
        try:
            ...
            state.failure = (
                exc
                if isinstance(exc, SketchConstrainVerticalError)
                else SketchConstrainVerticalError("SKETCH_CONSTRAIN_VERTICAL_FAILED", str(exc) or type(exc).__name__)
            )
            raise _AbortSketchConstrainVerticalMutation from exc
    ...
    def native_postcondition(document: object) -> bool:
        ...
        if state.document is not document or state.failure is not None:
            state.failure = SketchConstrainVerticalError(
        ...
        try:
            ...
            state.failure = (
                exc
                if isinstance(exc, SketchConstrainVerticalError)
                else SketchConstrainVerticalError("SKETCH_CONSTRAIN_VERTICAL_RESULT_FAILED", str(exc) or type(exc).__name__)
            )
            return False
        try:
    ...
    try:
        ...
        return make_sketch_constrain_vertical_uncertain(
            "SKETCH_CONSTRAIN_VERTICAL_NATIVE_EXCEPTION",
            str(exc) or type(exc).__name__,
            committed=None,
        )
    except Exception as exc:
    ...
    return _sketch_constrain_vertical_native_result(native_result, state)
