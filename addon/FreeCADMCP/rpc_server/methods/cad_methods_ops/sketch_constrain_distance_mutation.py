"""Fixed native transaction policy for the typed ``sketch_constrain_distance`` mutation."""
...
class SketchConstrainDistanceError(RuntimeError):
    def __init__(self, code: str, message: str) -> None:
        super().__init__(message)
        self.code = code


class _AbortSketchConstrainDistanceMutation(RuntimeError):
    """Ask the native coordinator to roll back a failed apply callback."""


@dataclass(slots=True)
class _NativeSketchConstrainDistanceMutationState:
    document: SketchDocument | None = None
    failure: SketchConstrainDistanceError | None = None
...
def _sketch_constrain_distance_native_result(
    native_result: object, state: _NativeSketchConstrainDistanceMutationState
    ...
    if not isinstance(native_result, dict):
        return make_sketch_constrain_distance_uncertain(
            "INVALID_NATIVE_SKETCH_CONSTRAIN_DISTANCE_RESULT",
    ...
    message = message_value if isinstance(message_value, str) else "Native sketch_constrain_distance mutation failed"
    ...
    if rollback_failed:
        return make_sketch_constrain_distance_uncertain(
            "SKETCH_CONSTRAIN_DISTANCE_ROLLBACK_UNCERTAIN",
            message,
            committed=True if committed is True else None,
            native_status=status,
    ...
    if status == "Committed" and committed is True and "rollback_succeeded" not in native_result:
        ...
        return make_sketch_constrain_distance_uncertain(
            ...
            "Native commit completed without a successful sketch_constrain_distance postcondition",
            committed=True,
            native_status=status,
        )
...
def run_sketch_constrain_distance_native_mutation(
