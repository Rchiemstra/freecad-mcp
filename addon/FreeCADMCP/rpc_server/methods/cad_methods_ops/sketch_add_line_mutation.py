"""Fixed native transaction policy for the typed ``sketch_add_line`` mutation."""
...
class SketchAddLineError(RuntimeError):
    def __init__(self, code: str, message: str) -> None:
        super().__init__(message)
        self.code = code


class _AbortSketchAddLineMutation(RuntimeError):
    """Ask the native coordinator to roll back a failed apply callback."""


@dataclass(slots=True)
class _NativeSketchAddLineMutationState:
    document: SketchDocument | None = None
    failure: SketchAddLineError | None = None
...
def _sketch_add_line_native_result(
    native_result: object, state: _NativeSketchAddLineMutationState
) -> Literal[True] | SketchAddLineFailure | SketchAddLineUncertain:
    if not isinstance(native_result, dict):
        return make_sketch_add_line_uncertain(
            "INVALID_NATIVE_SKETCH_ADD_LINE_RESULT",
    ...
    message = message_value if isinstance(message_value, str) else "Native sketch_add_line mutation failed"
    ...
    if rollback_failed:
        return make_sketch_add_line_uncertain(
            "SKETCH_ADD_LINE_ROLLBACK_UNCERTAIN",
            message,
            committed=True if committed is True else None,
            native_status=status,
    ...
    if status == "Committed" and committed is True and "rollback_succeeded" not in native_result:
        ...
        return make_sketch_add_line_uncertain(
            ...
            "Native commit completed without a successful sketch_add_line postcondition",
            committed=True,
            native_status=status,
        )
...
def run_sketch_add_line_native_mutation(
    ...
    def native_apply(document: object) -> None:
        ...
        try:
            ...
            state.failure = (
                exc
                if isinstance(exc, SketchAddLineError)
                else SketchAddLineError("SKETCH_ADD_LINE_FAILED", str(exc) or type(exc).__name__)
            )
            raise _AbortSketchAddLineMutation from exc
    ...
    def native_postcondition(document: object) -> bool:
        ...
        if state.document is not document or state.failure is not None:
            state.failure = SketchAddLineError(
        ...
        try:
            ...
            state.failure = (
                exc
                if isinstance(exc, SketchAddLineError)
                else SketchAddLineError("SKETCH_ADD_LINE_RESULT_FAILED", str(exc) or type(exc).__name__)
            )
            return False
        try:
    ...
    try:
        ...
        return make_sketch_add_line_uncertain(
            "SKETCH_ADD_LINE_NATIVE_EXCEPTION",
            str(exc) or type(exc).__name__,
            committed=None,
        )
    except Exception as exc:
    ...
    return _sketch_add_line_native_result(native_result, state)
