"""Fixed native transaction policy for the typed ``translate`` mutation."""
...
class TranslateError(TypedMutationError):
...
def _translate_native_result(
    native_result: object, state: _NativeMutationState
) -> Literal[True] | TranslateFailure | TranslateUncertain:
    if not isinstance(native_result, dict):
        return make_translate_uncertain(
            "INVALID_NATIVE_TRANSLATE_RESULT",
    ...
    message = message_value if isinstance(message_value, str) else "Native translate mutation failed"
    ...
    if status == "Committed" and committed is True and "rollback_succeeded" not in native_result:
        ...
        return make_translate_uncertain(
            ...
            "Native commit completed without a successful translate postcondition",
            committed=True,
            native_status=status,
        )
...
def run_translate_native_mutation(
    ...
    def native_apply(document: object) -> None:
        ...
        try:
            ...
            state.failure = (
                ...
                else TranslateError("TRANSLATE_FAILED", str(exc) or type(exc).__name__)
            )
    ...
    def native_postcondition(document: object) -> bool:
        ...
        try:
            ...
            state.failure = (
                exc
                if isinstance(exc, TypedMutationError)
                else TranslateError("TRANSLATE_RESULT_FAILED", str(exc) or type(exc).__name__)
            )
            return False
        try:
    ...
    return _translate_native_result(native_result, state)
