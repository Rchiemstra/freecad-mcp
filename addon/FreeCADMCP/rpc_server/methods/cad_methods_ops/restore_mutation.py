"""Fixed native transaction policy for the ``restore`` mutation.
...
class _NativeMutationState:
    document: RestoreDocument | None = None
...
def _restore_native_result(
...
) -> Literal[True] | RestoreFailure | RestoreUncertain:
    if not isinstance(native_result, dict):
        return make_restore_uncertain(
            "INVALID_NATIVE_RESTORE_RESULT",
    ...
    message = message_value if isinstance(message_value, str) else "Native restore mutation failed"
...
def run_restore_native_mutation(
    collaborators: RestoreCollaborators,
    document_name: DocumentName,
    apply: Callable[[RestoreDocument], None],
    postcondition: Callable[[RestoreReadDocument], None],
) -> Literal[True] | RestoreFailure | RestoreUncertain:
    ...
    def native_apply(document: object) -> None:
        typed = cast(RestoreDocument, document)
    ...
    def native_postcondition(document: object) -> bool:
        ...
        try:
            ...
            state.failure = (
                exc
                if isinstance(exc, RestoreError)
                else RestoreError("RESTORE_RESULT_FAILED", str(exc) or type(exc).__name__)
            )
            return False
        try:
    ...
    return _restore_native_result(native_result, state)
