"""Fixed native transaction policy for the typed ``sketch_extend`` mutation."""
...
class SketchExtendError(RuntimeError):
    def __init__(self, code: str, message: str) -> None:
        super().__init__(message)
        self.code = code


class _AbortSketchExtendMutation(RuntimeError):
    """Ask the native coordinator to roll back a failed apply callback."""


@dataclass(slots=True)
class _NativeSketchExtendMutationState:
    document: SketchDocument | None = None
    failure: SketchExtendError | None = None
...
def _sketch_extend_native_result(
    native_result: object, state: _NativeSketchExtendMutationState
) -> Literal[True] | SketchExtendFailure | SketchExtendUncertain:
    if not isinstance(native_result, dict):
        return make_sketch_extend_uncertain(
            "INVALID_NATIVE_SKETCH_EXTEND_RESULT",
    ...
    message = message_value if isinstance(message_value, str) else "Native sketch_extend mutation failed"
    ...
    if rollback_failed:
        return make_sketch_extend_uncertain(
            "SKETCH_EXTEND_ROLLBACK_UNCERTAIN",
            message,
            committed=True if committed is True else None,
            native_status=status,
    ...
    if status == "Committed" and committed is True and "rollback_succeeded" not in native_result:
        ...
        return make_sketch_extend_uncertain(
            ...
            "Native commit completed without a successful sketch_extend postcondition",
            committed=True,
            native_status=status,
        )
...
def run_sketch_extend_native_mutation(
    ...
    def native_apply(document: object) -> None:
        ...
        try:
            ...
            state.failure = (
                exc
                if isinstance(exc, SketchExtendError)
                else SketchExtendError("SKETCH_EXTEND_FAILED", str(exc) or type(exc).__name__)
            )
            raise _AbortSketchExtendMutation from exc
    ...
    def native_postcondition(document: object) -> bool:
        ...
        if state.document is not document or state.failure is not None:
            state.failure = SketchExtendError(
        ...
        try:
            ...
            state.failure = (
                exc
                if isinstance(exc, SketchExtendError)
                else SketchExtendError("SKETCH_EXTEND_RESULT_FAILED", str(exc) or type(exc).__name__)
            )
            return False
        try:
    ...
    try:
        ...
        return make_sketch_extend_uncertain(
            "SKETCH_EXTEND_NATIVE_EXCEPTION",
            str(exc) or type(exc).__name__,
            committed=None,
        )
    except Exception as exc:
    ...
    return _sketch_extend_native_result(native_result, state)
