"""Native transaction policy for the typed ``insert_part_from_library`` mutation."""
...
class _NativeMutationState:
    ...
    failure: InsertPartFromLibraryError | None = None
...
def _insert_part_from_library_native_result(
    ...
    if not isinstance(native_result, dict):
        return make_insert_part_from_library_uncertain(
            "INVALID_NATIVE_INSERT_PART_FROM_LIBRARY_RESULT",
    ...
    message = message_value if isinstance(message_value, str) else "Native insert_part_from_library mutation failed"
    ...
    if rollback_failed:
        return make_insert_part_from_library_uncertain(
            "INSERT_PART_FROM_LIBRARY_ROLLBACK_UNCERTAIN",
            message,
            committed=True if committed is True else None,
            native_status=status,
    ...
    if status == "Committed" and committed is True and "rollback_succeeded" not in native_result:
        ...
        return make_insert_part_from_library_uncertain(
            ...
            "Native commit completed without a successful insert_part_from_library postcondition",
            committed=True,
            native_status=status,
        )
...
def run_insert_part_from_library_native_mutation(
    ...
    def native_apply(document: object) -> None:
        ...
        try:
            ...
            state.failure = (
                exc
                if isinstance(exc, InsertPartFromLibraryError)
                else InsertPartFromLibraryError("INSERT_PART_FROM_LIBRARY_FAILED", str(exc) or type(exc).__name__)
            )
    ...
    def native_postcondition(document: object) -> bool:
        if state.document is not document or state.failure is not None:
            state.failure = InsertPartFromLibraryError(
        ...
        try:
            ...
            state.failure = (
                exc
                if isinstance(exc, InsertPartFromLibraryError)
                else InsertPartFromLibraryError("INSERT_PART_FROM_LIBRARY_RESULT_FAILED", str(exc) or type(exc).__name__)
            )
            return False
        try:
            collaborators.validate_document_invariants(cast(InsertPartFromLibraryReadDocument, document))
        except Exception as exc:
            state.failure = InsertPartFromLibraryError(
    ...
    try:
        ...
        return make_insert_part_from_library_uncertain(
            "INSERT_PART_FROM_LIBRARY_NATIVE_EXCEPTION",
            str(exc) or type(exc).__name__,
            committed=None,
        )
    except Exception as exc:
    ...
    return _insert_part_from_library_native_result(native_result, state)
