"""Native transaction policy for the typed ``open_document`` mutation."""
...
class _NativeMutationState:
    ...
    failure: OpenDocumentError | None = None
...
def _open_document_native_result(
    native_result: object, state: _NativeMutationState
) -> Literal[True] | OpenDocumentFailure | OpenDocumentUncertain:
    if not isinstance(native_result, dict):
        return make_open_document_uncertain(
            "INVALID_NATIVE_OPEN_DOCUMENT_RESULT",
    ...
    message = message_value if isinstance(message_value, str) else "Native open_document mutation failed"
    ...
    if status == "Committed" and committed is True and "rollback_succeeded" not in native_result:
        ...
        return make_open_document_uncertain(
            ...
            "Native commit completed without a successful open_document postcondition",
            committed=True,
            native_status=status,
        )
...
def run_open_document_native_mutation(
    ...
    def native_apply(document: object) -> None:
        ...
        try:
            ...
            state.failure = (
                exc
                if isinstance(exc, OpenDocumentError)
                else OpenDocumentError("OPEN_DOCUMENT_FAILED", str(exc) or type(exc).__name__)
            )
    ...
    def native_postcondition(document: object) -> bool:
        if state.document is not document or state.failure is not None:
            state.failure = OpenDocumentError(
        ...
        try:
            ...
            state.failure = (
                exc
                if isinstance(exc, OpenDocumentError)
                else OpenDocumentError("OPEN_DOCUMENT_RESULT_FAILED", str(exc) or type(exc).__name__)
            )
            return False
        try:
            collaborators.validate_document_invariants(cast(OpenDocumentReadDocument, document))
        except Exception as exc:
            state.failure = OpenDocumentError(
    ...
    try:
        ...
        return make_open_document_uncertain(
            "OPEN_DOCUMENT_NATIVE_EXCEPTION",
            str(exc) or type(exc).__name__,
            committed=None,
        )
    except Exception as exc:
    ...
    return _open_document_native_result(native_result, state)
