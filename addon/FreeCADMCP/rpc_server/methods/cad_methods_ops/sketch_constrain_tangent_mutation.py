"""Fixed native transaction policy for the typed ``sketch_constrain_tangent`` mutation."""
...
class SketchConstrainTangentError(RuntimeError):
    def __init__(self, code: str, message: str) -> None:
        super().__init__(message)
        self.code = code


class _AbortSketchConstrainTangentMutation(RuntimeError):
    """Ask the native coordinator to roll back a failed apply callback."""


@dataclass(slots=True)
class _NativeSketchConstrainTangentMutationState:
    document: SketchDocument | None = None
    failure: SketchConstrainTangentError | None = None
...
def _sketch_constrain_tangent_native_result(
    native_result: object, state: _NativeSketchConstrainTangentMutationState
) -> Literal[True] | SketchConstrainTangentFailure | SketchConstrainTangentUncertain:
    if not isinstance(native_result, dict):
        return make_sketch_constrain_tangent_uncertain(
            "INVALID_NATIVE_SKETCH_CONSTRAIN_TANGENT_RESULT",
    ...
    message = message_value if isinstance(message_value, str) else "Native sketch_constrain_tangent mutation failed"
    ...
    if rollback_failed:
        return make_sketch_constrain_tangent_uncertain(
            "SKETCH_CONSTRAIN_TANGENT_ROLLBACK_UNCERTAIN",
            message,
            committed=True if committed is True else None,
            native_status=status,
    ...
    if status == "Committed" and committed is True and "rollback_succeeded" not in native_result:
        ...
        return make_sketch_constrain_tangent_uncertain(
            ...
            "Native commit completed without a successful sketch_constrain_tangent postcondition",
            committed=True,
            native_status=status,
        )
...
def run_sketch_constrain_tangent_native_mutation(
    ...
    state = _NativeSketchConstrainTangentMutationState()
    ...
    def native_apply(document: object) -> None:
        ...
        try:
            ...
            state.failure = (
                exc
                if isinstance(exc, SketchConstrainTangentError)
                else SketchConstrainTangentError("SKETCH_CONSTRAIN_TANGENT_FAILED", str(exc) or type(exc).__name__)
            )
            raise _AbortSketchConstrainTangentMutation from exc
    ...
    def native_postcondition(document: object) -> bool:
        ...
        if state.document is not document or state.failure is not None:
            state.failure = SketchConstrainTangentError(
        ...
        try:
            ...
            state.failure = (
                exc
                if isinstance(exc, SketchConstrainTangentError)
                else SketchConstrainTangentError("SKETCH_CONSTRAIN_TANGENT_RESULT_FAILED", str(exc) or type(exc).__name__)
            )
            return False
        try:
    ...
    try:
        ...
        return make_sketch_constrain_tangent_uncertain(
            "SKETCH_CONSTRAIN_TANGENT_NATIVE_EXCEPTION",
            str(exc) or type(exc).__name__,
            committed=None,
        )
    except Exception as exc:
    ...
    return _sketch_constrain_tangent_native_result(native_result, state)
