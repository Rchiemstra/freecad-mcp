"""Fixed native transaction policy for the typed ``bounding_box`` mutation."""
...
class BoundingBoxError(TypedMutationError):
...
def _bounding_box_native_result(
    native_result: object, state: _NativeMutationState
) -> Literal[True] | BoundingBoxFailure | BoundingBoxUncertain:
    if not isinstance(native_result, dict):
        return make_bounding_box_uncertain(
            "INVALID_NATIVE_BOUNDING_BOX_RESULT",
    ...
    message = message_value if isinstance(message_value, str) else "Native bounding_box mutation failed"
    ...
    if rollback_failed:
        return make_bounding_box_uncertain(
            "BOUNDING_BOX_ROLLBACK_UNCERTAIN",
            message,
            committed=True if committed is True else None,
            native_status=status,
    ...
    if status == "Committed" and committed is True and "rollback_succeeded" not in native_result:
        ...
        return make_bounding_box_uncertain(
            ...
            "Native commit completed without a successful bounding_box postcondition",
            committed=True,
            native_status=status,
        )
...
def run_bounding_box_native_mutation(
    ...
    def native_apply(document: object) -> None:
        ...
        try:
            ...
            state.failure = (
                ...
                else BoundingBoxError("BOUNDING_BOX_FAILED", str(exc) or type(exc).__name__)
            )
    ...
    def native_postcondition(document: object) -> bool:
        if state.document is not document or state.failure is not None:
            state.failure = BoundingBoxError(
        ...
        try:
            ...
            state.failure = (
                exc
                if isinstance(exc, TypedMutationError)
                else BoundingBoxError("BOUNDING_BOX_RESULT_FAILED", str(exc) or type(exc).__name__)
            )
            return False
        try:
    ...
    return _bounding_box_native_result(native_result, state)
