"""Fixed native transaction policy for the typed ``import_brep`` mutation."""
...
class ImportBrepError(TypedMutationError):
...
def _import_brep_native_result(
    native_result: object, state: _NativeMutationState
) -> Literal[True] | ImportBrepFailure | ImportBrepUncertain:
    if not isinstance(native_result, dict):
        return make_import_brep_uncertain(
            "INVALID_NATIVE_IMPORT_BREP_RESULT",
    ...
    message = message_value if isinstance(message_value, str) else "Native import_brep mutation failed"
    ...
    if rollback_failed:
        return make_import_brep_uncertain(
            "IMPORT_BREP_ROLLBACK_UNCERTAIN",
            message,
            committed=True if committed is True else None,
            native_status=status,
    ...
    if status == "Committed" and committed is True and "rollback_succeeded" not in native_result:
        ...
        return make_import_brep_uncertain(
            ...
            "Native commit completed without a successful import_brep postcondition",
            committed=True,
            native_status=status,
        )
...
def run_import_brep_native_mutation(
    ...
    def native_apply(document: object) -> None:
        ...
        try:
            ...
            state.failure = (
                ...
                else ImportBrepError("IMPORT_BREP_FAILED", str(exc) or type(exc).__name__)
            )
    ...
    def native_postcondition(document: object) -> bool:
        ...
        try:
            ...
            state.failure = (
                exc
                if isinstance(exc, TypedMutationError)
                else ImportBrepError("IMPORT_BREP_RESULT_FAILED", str(exc) or type(exc).__name__)
            )
            return False
        try:
    ...
    return _import_brep_native_result(native_result, state)
