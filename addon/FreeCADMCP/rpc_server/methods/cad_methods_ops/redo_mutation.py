"""Native transaction policy for the typed ``redo`` mutation."""
...
def _redo_native_result(
    native_result: object, state: _NativeMutationState
) -> Literal[True] | RedoFailure | RedoUncertain:
    if not isinstance(native_result, dict):
        return make_redo_uncertain(
            "INVALID_NATIVE_REDO_RESULT",
    ...
    message = message_value if isinstance(message_value, str) else "Native redo mutation failed"
    ...
    if status == "Committed" and committed is True and "rollback_succeeded" not in native_result:
        ...
        return make_redo_uncertain(
            ...
            "Native commit completed without a successful redo postcondition",
            committed=True,
            native_status=status,
        )
...
def run_redo_native_mutation(
    ...
    def native_apply(document: object) -> None:
        ...
        try:
            ...
            state.failure = (
                exc
                if isinstance(exc, RedoError)
                else RedoError("REDO_FAILED", str(exc) or type(exc).__name__)
            )
    ...
    def native_postcondition(document: object) -> bool:
        ...
        try:
            ...
            state.failure = (
                exc
                if isinstance(exc, RedoError)
                else RedoError("REDO_RESULT_FAILED", str(exc) or type(exc).__name__)
            )
            return False
        try:
            collaborators.validate_document_invariants(cast(RedoReadDocument, document))
        except Exception as exc:
            state.failure = RedoError(
    ...
    return _redo_native_result(native_result, state)
