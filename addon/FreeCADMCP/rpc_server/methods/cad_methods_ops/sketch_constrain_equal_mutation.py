"""Fixed native transaction policy for the typed ``sketch_constrain_equal`` mutation."""
...
class SketchConstrainEqualError(RuntimeError):
    def __init__(self, code: str, message: str) -> None:
        super().__init__(message)
        self.code = code


class _AbortSketchConstrainEqualMutation(RuntimeError):
    """Ask the native coordinator to roll back a failed apply callback."""


@dataclass(slots=True)
class _NativeSketchConstrainEqualMutationState:
    document: SketchDocument | None = None
    failure: SketchConstrainEqualError | None = None
...
def _sketch_constrain_equal_native_result(
    native_result: object, state: _NativeSketchConstrainEqualMutationState
) -> Literal[True] | SketchConstrainEqualFailure | SketchConstrainEqualUncertain:
    if not isinstance(native_result, dict):
        return make_sketch_constrain_equal_uncertain(
            "INVALID_NATIVE_SKETCH_CONSTRAIN_EQUAL_RESULT",
    ...
    message = message_value if isinstance(message_value, str) else "Native sketch_constrain_equal mutation failed"
    ...
    if rollback_failed:
        return make_sketch_constrain_equal_uncertain(
            "SKETCH_CONSTRAIN_EQUAL_ROLLBACK_UNCERTAIN",
            message,
            committed=True if committed is True else None,
            native_status=status,
    ...
    if status == "Committed" and committed is True and "rollback_succeeded" not in native_result:
        ...
        return make_sketch_constrain_equal_uncertain(
            ...
            "Native commit completed without a successful sketch_constrain_equal postcondition",
            committed=True,
            native_status=status,
        )
...
def run_sketch_constrain_equal_native_mutation(
    ...
    state = _NativeSketchConstrainEqualMutationState()
    ...
    def native_apply(document: object) -> None:
        ...
        try:
            ...
            state.failure = (
                exc
                if isinstance(exc, SketchConstrainEqualError)
                else SketchConstrainEqualError("SKETCH_CONSTRAIN_EQUAL_FAILED", str(exc) or type(exc).__name__)
            )
            raise _AbortSketchConstrainEqualMutation from exc
    ...
    def native_postcondition(document: object) -> bool:
        ...
        if state.document is not document or state.failure is not None:
            state.failure = SketchConstrainEqualError(
        ...
        try:
            ...
            state.failure = (
                exc
                if isinstance(exc, SketchConstrainEqualError)
                else SketchConstrainEqualError("SKETCH_CONSTRAIN_EQUAL_RESULT_FAILED", str(exc) or type(exc).__name__)
            )
            return False
        try:
    ...
    try:
        ...
        return make_sketch_constrain_equal_uncertain(
            "SKETCH_CONSTRAIN_EQUAL_NATIVE_EXCEPTION",
            str(exc) or type(exc).__name__,
            committed=None,
        )
    except Exception as exc:
    ...
    return _sketch_constrain_equal_native_result(native_result, state)
