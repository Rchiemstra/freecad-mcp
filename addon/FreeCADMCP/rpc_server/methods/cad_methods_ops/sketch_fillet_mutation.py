"""Fixed native transaction policy for the typed ``sketch_fillet`` mutation."""
...
class SketchFilletError(RuntimeError):
    def __init__(self, code: str, message: str) -> None:
        super().__init__(message)
        self.code = code


class _AbortSketchFilletMutation(RuntimeError):
    """Ask the native coordinator to roll back a failed apply callback."""


@dataclass(slots=True)
class _NativeSketchFilletMutationState:
    document: SketchDocument | None = None
    failure: SketchFilletError | None = None
...
def _sketch_fillet_native_result(
    native_result: object, state: _NativeSketchFilletMutationState
) -> Literal[True] | SketchFilletFailure | SketchFilletUncertain:
    if not isinstance(native_result, dict):
        return make_sketch_fillet_uncertain(
            "INVALID_NATIVE_SKETCH_FILLET_RESULT",
    ...
    message = message_value if isinstance(message_value, str) else "Native sketch_fillet mutation failed"
    ...
    if rollback_failed:
        return make_sketch_fillet_uncertain(
            "SKETCH_FILLET_ROLLBACK_UNCERTAIN",
            message,
            committed=True if committed is True else None,
            native_status=status,
    ...
    if status == "Committed" and committed is True and "rollback_succeeded" not in native_result:
        ...
        return make_sketch_fillet_uncertain(
            ...
            "Native commit completed without a successful sketch_fillet postcondition",
            committed=True,
            native_status=status,
        )
...
def run_sketch_fillet_native_mutation(
    ...
    def native_apply(document: object) -> None:
        ...
        try:
            ...
            state.failure = (
                exc
                if isinstance(exc, SketchFilletError)
                else SketchFilletError("SKETCH_FILLET_FAILED", str({exc}) or type({exc}).__name__)
            )
            raise _AbortSketchFilletMutation from exc
    ...
    def native_postcondition(document: object) -> bool:
        ...
        if state.document is not document or state.failure is not None:
            state.failure = SketchFilletError(
        ...
        try:
            ...
            state.failure = (
                exc
                if isinstance(exc, SketchFilletError)
                else SketchFilletError("SKETCH_FILLET_RESULT_FAILED", str({exc}) or type({exc}).__name__)
            )
            return False
        try:
    ...
    try:
        ...
        return make_sketch_fillet_uncertain(
            "SKETCH_FILLET_NATIVE_EXCEPTION",
            str({exc}) or type({exc}).__name__,
            committed=None,
        )
    except Exception as exc:
    ...
    return _sketch_fillet_native_result(native_result, state)
