"""Fixed native transaction policy for the typed ``solve_assembly`` mutation."""
...
class SolveAssemblyError(TypedMutationError):
...
def _solve_assembly_native_result(
    native_result: object, state: _NativeMutationState
) -> Literal[True] | SolveAssemblyFailure | SolveAssemblyUncertain:
    if not isinstance(native_result, dict):
        return make_solve_assembly_uncertain(
            "INVALID_NATIVE_SOLVE_ASSEMBLY_RESULT",
    ...
    message = message_value if isinstance(message_value, str) else "Native solve_assembly mutation failed"
    ...
    if rollback_failed:
        return make_solve_assembly_uncertain(
            "SOLVE_ASSEMBLY_ROLLBACK_UNCERTAIN",
            message,
            committed=True if committed is True else None,
            native_status=status,
    ...
    if status == "Committed" and committed is True and "rollback_succeeded" not in native_result:
        ...
        return make_solve_assembly_uncertain(
            ...
            "Native commit completed without a successful solve_assembly postcondition",
            committed=True,
            native_status=status,
        )
...
def run_solve_assembly_native_mutation(
    ...
    def native_apply(document: object) -> None:
        ...
        try:
            ...
            state.failure = (
                ...
                else SolveAssemblyError("SOLVE_ASSEMBLY_FAILED", str(exc) or type(exc).__name__)
            )
    ...
    def native_postcondition(document: object) -> bool:
        if state.document is not document or state.failure is not None:
            state.failure = SolveAssemblyError(
        ...
        try:
            ...
            state.failure = (
                exc
                if isinstance(exc, TypedMutationError)
                else SolveAssemblyError("SOLVE_ASSEMBLY_RESULT_FAILED", str(exc) or type(exc).__name__)
            )
            return False
        try:
    ...
    try:
        ...
        return make_solve_assembly_uncertain(
            "SOLVE_ASSEMBLY_NATIVE_EXCEPTION",
            str(exc) or type(exc).__name__,
            committed=None,
        )
    except Exception as exc:
    ...
    return _solve_assembly_native_result(native_result, state)
