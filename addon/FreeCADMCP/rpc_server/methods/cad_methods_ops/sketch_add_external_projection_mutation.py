"""Fixed native transaction policy for the ``sketch_add_external_projection`` mutation.
...
class SketchAddExternalProjectionError(RuntimeError):
...
class _NativeMutationState:
    document: SketchAddExternalProjectionDocument | None = None
    failure: SketchAddExternalProjectionError | None = None
...
def _sketch_add_external_projection_native_result(
    native_result: object, state: _NativeMutationState
) -> Literal[True] | SketchAddExternalProjectionFailure | SketchAddExternalProjectionUncertain:
    if not isinstance(native_result, dict):
        return make_sketch_add_external_projection_uncertain(
            "INVALID_NATIVE_SKETCH_ADD_EXTERNAL_PROJECTION_RESULT",
    ...
    message = message_value if isinstance(message_value, str) else "Native sketch_add_external_projection mutation failed"
    ...
    if rollback_failed:
        return make_sketch_add_external_projection_uncertain(
            "SKETCH_ADD_EXTERNAL_PROJECTION_ROLLBACK_UNCERTAIN",
            message,
            committed=True if committed is True else None,
            native_status=status,
...
def run_sketch_add_external_projection_native_mutation(
    collaborators: SketchAddExternalProjectionCollaborators,
    document_name: DocumentName,
    apply: Callable[[SketchAddExternalProjectionDocument], None],
    postcondition: Callable[[SketchAddExternalProjectionReadDocument], None],
) -> Literal[True] | SketchAddExternalProjectionFailure | SketchAddExternalProjectionUncertain:
    ...
    def native_apply(document: object) -> None:
        typed = cast(SketchAddExternalProjectionDocument, document)
        ...
        try:
            ...
            state.failure = (
                exc
                if isinstance(exc, SketchAddExternalProjectionError)
                else SketchAddExternalProjectionError("SKETCH_ADD_EXTERNAL_PROJECTION_FAILED", str(exc) or type(exc).__name__)
            )
            raise _AbortMutation from exc

    def native_postcondition(document: object) -> bool:
        typed = cast(SketchAddExternalProjectionReadDocument, document)
        if state.document is not typed or state.failure is not None:
            state.failure = SketchAddExternalProjectionError(
        ...
        try:
            ...
            state.failure = (
                exc
                if isinstance(exc, SketchAddExternalProjectionError)
                else SketchAddExternalProjectionError("SKETCH_ADD_EXTERNAL_PROJECTION_RESULT_FAILED", str(exc) or type(exc).__name__)
            )
            return False
        try:
    ...
    try:
        ...
        return make_sketch_add_external_projection_uncertain(
            "SKETCH_ADD_EXTERNAL_PROJECTION_NATIVE_EXCEPTION",
            str(exc) or type(exc).__name__,
            committed=None,
        )
    except Exception as exc:
    ...
    return _sketch_add_external_projection_native_result(native_result, state)
