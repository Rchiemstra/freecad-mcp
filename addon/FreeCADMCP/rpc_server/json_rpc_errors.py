...
def _error_data(result: Mapping[str, Any], semantic_code: str) -> dict[str, Any]:
    ...
    for key, value in result.items():
        if key not in _CONTROL_FIELDS:
            data.setdefault(key, value)
    # JSON-RPC requires an integer ``error.code``.  Retain the stable semantic
    # identifier used by existing callers in data so no diagnostic is lost.
    data["error_code"] = semantic_code
    return redact_sensitive(data)


def json_rpc_error_from_result(result: object) -> dict[str, Any] | None:
    """Return a JSON-RPC error object for an explicit legacy failure.

    Only the frozen ``ok: false`` and ``success: false`` indicators are
    failures.  A mapping that merely contains error-like fields is still a
    successful result, as are all non-mapping values.
    """
