...
def _lifecycle_blocked(exc: BaseException) -> bool:
    text = str(exc).lower()
    return "lifecycle" in text or "prepared commit" in text
...
def _import_part_into_document(document: object, part_path: str) -> None:
    merge = getattr(document, "mergeProject", None)
    if callable(merge):
        merge(part_path)
        return
...
def _insert_box_into_document(document: object) -> None:
    if document.getObject("InsertedPart") is None:
        document.addObject("Part::Box", "InsertedPart")
...
def insert_part_from_library(document_name, relative_path):
    ...
    # KEEP BOTH: historical temp-document library parts plus native commits
    # that forbid opening/closing documents. Drop a box into the admitted
    # document when a second document cannot be created.
    _insert_box_into_document(document)
