...
def wire_status_details_click(status: Any) -> None:
    def show_details() -> None:
        if state._dock_widget is not None:
            state._dock_widget.show()
            state._dock_widget.raise_()
...
def wire_refresh_timer(main: Any, qt_core: Any) -> None:
    ...
    timer.start()
    state._refresh_timer = timer


def wire_gui_update_callback() -> None:
    try:
...
def mount_dock_widget(main: Any, dock: Any, qt_core: Any) -> None:
    try:
        ...
        state._dock_widget = dock
    except Exception:
        state._dock_widget = None
