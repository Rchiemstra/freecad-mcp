"""Versioned, stdlib-only wire contract for ``sketch_constrain_horizontal``."""
...
SKETCH_CONSTRAIN_HORIZONTAL_CONTRACT_VERSION: Literal[1] = 1
...
class SketchConstrainHorizontalCollaborators(Protocol):
    @property
...
class SketchConstrainHorizontalRequest:
    doc_name: DocumentName
    sketch_name: SketchName
    geo: int


class SketchConstrainHorizontalSuccess(TypedDict):
...
class SketchConstrainHorizontalFailure(TypedDict):
    contract_version: Literal[1]
...
class SketchConstrainHorizontalUncertain(TypedDict):
    contract_version: Literal[1]
    success: Literal[False]
    ok: Literal[False]
...
SketchConstrainHorizontalResult = SketchConstrainHorizontalSuccess | SketchConstrainHorizontalFailure | SketchConstrainHorizontalUncertain
...
def make_sketch_constrain_horizontal_success(sketch: SketchName, constraint_index: int) -> SketchConstrainHorizontalSuccess:
    return {
        "contract_version": SKETCH_CONSTRAIN_HORIZONTAL_CONTRACT_VERSION,
        "success": True,
        "ok": True,
        "outcome": "committed",
        "committed": True,
...
def make_sketch_constrain_horizontal_failure(
    error_code: str,
    error: str,
    *,
    ...
    result: SketchConstrainHorizontalFailure = {
        "contract_version": SKETCH_CONSTRAIN_HORIZONTAL_CONTRACT_VERSION,
        "success": False,
        "ok": False,
        "outcome": "rejected",
...
def make_sketch_constrain_horizontal_uncertain(
    error_code: str,
    error: str,
    *,
    committed: bool | None,
    ...
    result: SketchConstrainHorizontalUncertain = {
        "contract_version": SKETCH_CONSTRAIN_HORIZONTAL_CONTRACT_VERSION,
        "success": False,
        "ok": False,
        "outcome": "uncertain",
...
def _invalid_response(response: dict[str, object]) -> SketchConstrainHorizontalUncertain:
    ...
    code = (
        ...
        if committed
        else "SKETCH_CONSTRAIN_HORIZONTAL_ROLLBACK_UNCERTAIN"
        if rollback_failed
        else "INVALID_SKETCH_CONSTRAIN_HORIZONTAL_RESPONSE"
    )
    return make_sketch_constrain_horizontal_uncertain(
        code,
        "sketch_constrain_horizontal returned an invalid contract response; document state requires reconciliation",
        committed=True if committed else None,
        diagnostics={"response": response},
    )


def parse_sketch_constrain_horizontal_response(raw_response: object) -> SketchConstrainHorizontalResult:
    response = _response_object(raw_response)
    if response is None:
        return make_sketch_constrain_horizontal_uncertain(
            "INVALID_RPC_RESPONSE",
            "sketch_constrain_horizontal returned a non-object response",
            committed=None,
        )
    version = response.get("contract_version")
    details = _response_details(response)
    if type(version) is not int or version != SKETCH_CONSTRAIN_HORIZONTAL_CONTRACT_VERSION or details is None:
    ...
    if (
        ...
        return make_sketch_constrain_horizontal_success(SketchName(sketch), constraint_index)
    ...
    if (
        ...
        if _valid_uncertain(response):
            ...
            return make_sketch_constrain_horizontal_uncertain(error_code, error, committed=committed, **details)
    return _invalid_response(response)


__all__ = [
    "SKETCH_CONSTRAIN_HORIZONTAL_CONTRACT_VERSION",
