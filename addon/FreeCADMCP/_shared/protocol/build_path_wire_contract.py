"""Versioned, stdlib-only wire contract for ``build_path_wire``."""
...
BUILD_PATH_WIRE_CONTRACT_VERSION: Literal[1] = 1

DocumentName = NewType("DocumentName", str)
BuildPathWireName = NewType("BuildPathWireName", str)


class BuildPathWireObject(Protocol):
...
class BuildPathWireReadDocument(Protocol):
    """Read-only document surface available after native recompute."""

    @property
    def Name(self) -> str: ...

    def getObject(self, name: str) -> BuildPathWireObject | None: ...


class BuildPathWireDocument(BuildPathWireReadDocument, Protocol):
    """Narrow mutation surface available only to the apply callback."""

    def addObject(self, object_type: str, name: str) -> BuildPathWireObject: ...


@runtime_checkable
class NativeBuildPathWireDocument(BuildPathWireDocument, Protocol):
...
class BuildPathWireCollaborators(Protocol):
...
class BuildPathWireSuccess(TypedDict):
...
class BuildPathWireFailure(TypedDict):
...
class BuildPathWireUncertain(TypedDict):
...
BuildPathWireResult = BuildPathWireSuccess | BuildPathWireFailure | BuildPathWireUncertain

_CORE_KEYS = frozenset(
    {
        "contract_version", "success", "ok", "outcome", "committed", "retry_safe", 'wire_name', "error_code", "error", "native_status", "native_message", "rollback_succeeded", "rollback_failed", "diagnostics"
    }
)


def make_build_path_wire_success(wire_name: str) -> BuildPathWireSuccess:
    """Construct a complete committed result."""

    return {
        "contract_version": BUILD_PATH_WIRE_CONTRACT_VERSION,
        "success": True,
        "ok": True,
        "outcome": "committed",
        "committed": True,
...
def make_build_path_wire_failure(
    error_code: str,
    error: str,
    *,
...
def make_build_path_wire_uncertain(
    error_code: str,
    error: str,
    *,
    committed: bool | None,
...
def _invalid_response(response: dict[str, object]) -> BuildPathWireUncertain:
    ...
    code = (
        ...
        if committed
        else "BUILD_PATH_WIRE_ROLLBACK_UNCERTAIN"
        if rollback_failed
        else "INVALID_BUILD_PATH_WIRE_RESPONSE"
    )
    return make_build_path_wire_uncertain(
        code,
        "build_path_wire returned an invalid contract response; document state requires reconciliation",
        committed=True if committed else None,
        diagnostics={"response": response},
    )


def parse_build_path_wire_response(raw_response: object) -> BuildPathWireResult:
    """Validate all three wire variants; unknown state always stays uncertain."""
    response = _response_object(raw_response)
    if response is None:
        return make_build_path_wire_uncertain(
            "INVALID_RPC_RESPONSE",
            "build_path_wire returned a non-object response",
            committed=None,
        )
    version = response.get("contract_version")
    details = _response_details(response)
    if type(version) is not int or version != BUILD_PATH_WIRE_CONTRACT_VERSION or details is None:
        return _invalid_response(response)

    wire_name = response.get('wire_name')
    if (
        _valid_success(response)
        and isinstance(wire_name, str) and wire_name.strip()
    ):
        return make_build_path_wire_success(str(wire_name))
    ...
    if (
        ...
        and error.strip()
        and 'wire_name' not in response
    ):
        if _valid_rejection(response):
            return make_build_path_wire_failure(
                error_code,
                error,
        ...
        if _valid_uncertain(response):
            ...
            return make_build_path_wire_uncertain(error_code, error, committed=committed, **details)
    return _invalid_response(response)


__all__ = [
    "BUILD_PATH_WIRE_CONTRACT_VERSION",
