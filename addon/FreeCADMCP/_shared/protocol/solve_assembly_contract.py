"""Versioned, stdlib-only wire contract for ``solve_assembly``."""
...
SOLVE_ASSEMBLY_CONTRACT_VERSION: Literal[1] = 1
...
class SolveAssemblyCollaborators(Protocol):
...
class SolveAssemblySuccess(TypedDict):
...
class SolveAssemblyUncertain(TypedDict):
...
SolveAssemblyResult = SolveAssemblySuccess | SolveAssemblyFailure | SolveAssemblyUncertain
...
def make_solve_assembly_success(assembly: str, method: str, status: str | None) -> SolveAssemblySuccess:
    """Construct a complete committed result."""

    return {
        "contract_version": SOLVE_ASSEMBLY_CONTRACT_VERSION,
        "success": True,
        "ok": True,
        "outcome": "committed",
        "committed": True,
...
def make_solve_assembly_failure(
    error_code: str,
    error: str,
    *,
...
def make_solve_assembly_uncertain(
    error_code: str,
    error: str,
    *,
    committed: bool | None,
...
def _invalid_response(response: dict[str, object]) -> SolveAssemblyUncertain:
    ...
    code = (
        ...
        if committed
        else "SOLVE_ASSEMBLY_ROLLBACK_UNCERTAIN"
        if rollback_failed
        else "INVALID_SOLVE_ASSEMBLY_RESPONSE"
    )
    return make_solve_assembly_uncertain(
        code,
        "solve_assembly returned an invalid contract response; document state requires reconciliation",
        committed=True if committed else None,
        diagnostics={"response": response},
    )


def parse_solve_assembly_response(raw_response: object) -> SolveAssemblyResult:
    """Validate all three wire variants; unknown state always stays uncertain."""
    response = _response_object(raw_response)
    if response is None:
        return make_solve_assembly_uncertain(
            "INVALID_RPC_RESPONSE",
            "solve_assembly returned a non-object response",
            committed=None,
        )
    version = response.get("contract_version")
    details = _response_details(response)
    if type(version) is not int or version != SOLVE_ASSEMBLY_CONTRACT_VERSION or details is None:
    ...
    if (
        ...
        and isinstance(assembly, str) and assembly.strip() and isinstance(method, str) and (status is None or isinstance(status, str))
    ):
        return make_solve_assembly_success(assembly, method, status)
    ...
    if (
        ...
        if _valid_rejection(response):
            return make_solve_assembly_failure(
                error_code,
                error,
        ...
        if _valid_uncertain(response):
            ...
            return make_solve_assembly_uncertain(error_code, error, committed=committed, **details)
    return _invalid_response(response)


__all__ = [
    "SOLVE_ASSEMBLY_CONTRACT_VERSION",
