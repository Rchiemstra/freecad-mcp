"""Versioned, stdlib-only wire contract for ``measure_distance``."""
...
MEASURE_DISTANCE_CONTRACT_VERSION: Literal[1] = 1
...
class MeasureDistanceCollaborators(Protocol):
...
class MeasureDistanceSuccess(TypedDict):
...
class MeasureDistanceFailure(TypedDict):
...
class MeasureDistanceUncertain(TypedDict):
...
MeasureDistanceResult = MeasureDistanceSuccess | MeasureDistanceFailure | MeasureDistanceUncertain
...
def make_measure_distance_success(
    distance: float, unit: str,
) -> MeasureDistanceSuccess:
    """Construct a complete observed result."""

    return {
        "contract_version": MEASURE_DISTANCE_CONTRACT_VERSION,
        "success": True,
        "ok": True,
        "outcome": "observed",
        "retry_safe": False,
...
def make_measure_distance_failure(
    error_code: str,
    error: str,
    *,
...
def make_measure_distance_uncertain(
    error_code: str,
    error: str,
    *,
    committed: bool | None,
...
def _invalid_response(response: dict[str, object]) -> MeasureDistanceUncertain:
    ...
    code = (
        "MEASURE_DISTANCE_COMMITTED_RESPONSE_INVALID"
        if committed
        else "MEASURE_DISTANCE_ROLLBACK_UNCERTAIN"
        if rollback_failed
        else "INVALID_MEASURE_DISTANCE_RESPONSE"
    )
    return make_measure_distance_uncertain(
        code,
        "measure_distance returned an invalid contract response; document state requires reconciliation",
        committed=True if committed else None,
        diagnostics={"response": response},
    )


def parse_measure_distance_response(raw_response: object) -> MeasureDistanceResult:
    """Validate all three wire variants; unknown state always stays uncertain."""
    response = _response_object(raw_response)
    if response is None:
        return make_measure_distance_uncertain(
            "INVALID_RPC_RESPONSE",
            "measure_distance returned a non-object response",
            committed=None,
        )
    version = response.get("contract_version")
    details = _response_details(response)
    if type(version) is not int or version != MEASURE_DISTANCE_CONTRACT_VERSION or details is None:
        return _invalid_response(response)

    distance = response.get("distance")
    unit = response.get("unit")
    if _valid_success(response) and isinstance(distance, (int, float)) and not isinstance(distance, bool) and isinstance(unit, str):
        return make_measure_distance_success(float(distance), unit)

    error_code = response.get("error_code")
    error = response.get("error")
    success_keys = {'distance', 'unit'}
    if (
        ...
        if _valid_rejection(response):
            return make_measure_distance_failure(
                error_code,
                error,
        ...
        if _valid_uncertain(response):
            ...
            return make_measure_distance_uncertain(error_code, error, committed=committed, **details)
    return _invalid_response(response)


__all__ = [
    "MEASURE_DISTANCE_CONTRACT_VERSION",
