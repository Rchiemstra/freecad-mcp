"""Versioned, stdlib-only wire contract for ``undo``."""
...
UNDO_CONTRACT_VERSION: Literal[1] = 1

DocumentName = NewType("DocumentName", str)


class UndoReadDocument(Protocol):
...
UndoResult = UndoSuccess | UndoFailure | UndoUncertain
...
def make_undo_success(document_name: DocumentName) -> UndoSuccess:
...
def _invalid_response(response: dict[str, object]) -> UndoUncertain:
    ...
    code = (
        ...
        if committed
        else "UNDO_ROLLBACK_UNCERTAIN"
        if rollback_failed
        else "INVALID_UNDO_RESPONSE"
    )
    return make_undo_uncertain(
        code,
        "undo returned an invalid contract response; document state requires reconciliation",
...
def parse_undo_response(raw_response: object, *, document_name: str | None = None) -> UndoResult:
    ...
    if response is None:
        return make_undo_uncertain(
            ...
            "undo returned a non-object response",
            committed=None,
        )
    ...
    if (
        ...
        return make_undo_success(DocumentName(response_document_name))
    ...
    if (
        ...
        if _valid_uncertain(response):
            ...
            return make_undo_uncertain(error_code, error, committed=committed, **details)
    return _invalid_response(response)
