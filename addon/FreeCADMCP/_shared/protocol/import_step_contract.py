"""Versioned, stdlib-only wire contract for ``import_step``."""
...
IMPORT_STEP_CONTRACT_VERSION: Literal[1] = 1
...
class ImportStepCollaborators(Protocol):
...
class ImportStepSuccess(TypedDict):
...
class ImportStepFailure(TypedDict):
...
class ImportStepUncertain(TypedDict):
...
ImportStepResult = ImportStepSuccess | ImportStepFailure | ImportStepUncertain
...
def make_import_step_success(path: str, imported: bool) -> ImportStepSuccess:
    """Construct a complete committed result."""

    return {
        "contract_version": IMPORT_STEP_CONTRACT_VERSION,
        "success": True,
        "ok": True,
        "outcome": "committed",
        "committed": True,
...
def make_import_step_uncertain(
    error_code: str,
    error: str,
    *,
    committed: bool | None,
...
def _invalid_response(response: dict[str, object]) -> ImportStepUncertain:
    ...
    code = (
        ...
        if committed
        else "IMPORT_STEP_ROLLBACK_UNCERTAIN"
        if rollback_failed
        else "INVALID_IMPORT_STEP_RESPONSE"
    )
    return make_import_step_uncertain(
        code,
        "import_step returned an invalid contract response; document state requires reconciliation",
        committed=True if committed else None,
        diagnostics={"response": response},
    )


def parse_import_step_response(raw_response: object) -> ImportStepResult:
    """Validate all three wire variants; unknown state always stays uncertain."""
    response = _response_object(raw_response)
    if response is None:
        return make_import_step_uncertain(
            "INVALID_RPC_RESPONSE",
            "import_step returned a non-object response",
            committed=None,
        )
    version = response.get("contract_version")
    details = _response_details(response)
    if type(version) is not int or version != IMPORT_STEP_CONTRACT_VERSION or details is None:
    ...
    if (
        ...
        and isinstance(path, str) and isinstance(imported, bool)
    ):
        return make_import_step_success(path, imported)

    error_code = response.get("error_code")
    error = response.get("error")
    success_keys = {'path', 'imported'}
    if (
        ...
        if _valid_rejection(response):
            return make_import_step_failure(
                error_code,
                error,
        ...
        if _valid_uncertain(response):
            ...
            return make_import_step_uncertain(error_code, error, committed=committed, **details)
    return _invalid_response(response)


__all__ = [
    "IMPORT_STEP_CONTRACT_VERSION",
