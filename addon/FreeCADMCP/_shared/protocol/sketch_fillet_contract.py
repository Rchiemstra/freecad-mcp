"""Versioned, stdlib-only wire contract for ``sketch_fillet``."""
...
SKETCH_FILLET_CONTRACT_VERSION: Literal[1] = 1
...
class SketchObject(Protocol):
    ...
    def getPoint(self, geo_index: int, pos: int) -> SketchVector: ...
...
class SketchFilletCollaborators(Protocol):
    @property
...
class SketchFilletSuccess(TypedDict):
...
class SketchFilletFailure(TypedDict):
    contract_version: Literal[1]
...
class SketchFilletUncertain(TypedDict):
    contract_version: Literal[1]
    success: Literal[False]
    ok: Literal[False]
...
SketchFilletResult = SketchFilletSuccess | SketchFilletFailure | SketchFilletUncertain
...
def make_sketch_fillet_success(sketch: SketchName) -> SketchFilletSuccess:
    return {
        "contract_version": SKETCH_FILLET_CONTRACT_VERSION,
        "success": True,
        "ok": True,
        "outcome": "committed",
        "committed": True,
...
def make_sketch_fillet_failure(
    error_code: str,
    error: str,
    *,
...
def make_sketch_fillet_uncertain(
    error_code: str,
    error: str,
    *,
    committed: bool | None,
...
def _invalid_response(response: dict[str, object]) -> SketchFilletUncertain:
    ...
    code = (
        ...
        if committed
        else "SKETCH_FILLET_ROLLBACK_UNCERTAIN"
        if rollback_failed
        else "INVALID_SKETCH_FILLET_RESPONSE"
    )
    return make_sketch_fillet_uncertain(
        code,
        "sketch_fillet returned an invalid contract response; document state requires reconciliation",
        committed=True if committed else None,
        diagnostics={"response": response},
    )


def parse_sketch_fillet_response(raw_response: object) -> SketchFilletResult:
    response = _response_object(raw_response)
    if response is None:
        return make_sketch_fillet_uncertain(
            "INVALID_RPC_RESPONSE",
            "sketch_fillet returned a non-object response",
            committed=None,
        )
    version = response.get("contract_version")
    details = _response_details(response)
    if type(version) is not int or version != SKETCH_FILLET_CONTRACT_VERSION or details is None:
    ...
    if _valid_success(response) and isinstance(sketch, str) and sketch.strip():
        return make_sketch_fillet_success(SketchName(sketch))
    ...
    if (
        ...
        if _valid_rejection(response):
            return make_sketch_fillet_failure(
                error_code,
                error,
        ...
        if _valid_uncertain(response):
            ...
            return make_sketch_fillet_uncertain(error_code, error, committed=committed, **details)
    return _invalid_response(response)


__all__ = [
    "SKETCH_FILLET_CONTRACT_VERSION",
