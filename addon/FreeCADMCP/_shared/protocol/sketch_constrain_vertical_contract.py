"""Versioned, stdlib-only wire contract for ``sketch_constrain_vertical``."""
...
SKETCH_CONSTRAIN_VERTICAL_CONTRACT_VERSION: Literal[1] = 1
...
class SketchConstrainVerticalCollaborators(Protocol):
    @property
...
class SketchConstrainVerticalRequest:
    doc_name: DocumentName
    sketch_name: SketchName
    geo: int


class SketchConstrainVerticalSuccess(TypedDict):
...
class SketchConstrainVerticalFailure(TypedDict):
    contract_version: Literal[1]
...
class SketchConstrainVerticalUncertain(TypedDict):
    contract_version: Literal[1]
    success: Literal[False]
    ok: Literal[False]
...
SketchConstrainVerticalResult = SketchConstrainVerticalSuccess | SketchConstrainVerticalFailure | SketchConstrainVerticalUncertain
...
def make_sketch_constrain_vertical_success(sketch: SketchName, constraint_index: int) -> SketchConstrainVerticalSuccess:
    return {
        "contract_version": SKETCH_CONSTRAIN_VERTICAL_CONTRACT_VERSION,
        "success": True,
        "ok": True,
        "outcome": "committed",
        "committed": True,
...
def make_sketch_constrain_vertical_failure(
    error_code: str,
    error: str,
    *,
    ...
    result: SketchConstrainVerticalFailure = {
        "contract_version": SKETCH_CONSTRAIN_VERTICAL_CONTRACT_VERSION,
        "success": False,
        "ok": False,
        "outcome": "rejected",
...
def make_sketch_constrain_vertical_uncertain(
    error_code: str,
    error: str,
    *,
    committed: bool | None,
    ...
    result: SketchConstrainVerticalUncertain = {
        "contract_version": SKETCH_CONSTRAIN_VERTICAL_CONTRACT_VERSION,
        "success": False,
        "ok": False,
        "outcome": "uncertain",
...
def _invalid_response(response: dict[str, object]) -> SketchConstrainVerticalUncertain:
    ...
    code = (
        ...
        if committed
        else "SKETCH_CONSTRAIN_VERTICAL_ROLLBACK_UNCERTAIN"
        if rollback_failed
        else "INVALID_SKETCH_CONSTRAIN_VERTICAL_RESPONSE"
    )
    return make_sketch_constrain_vertical_uncertain(
        code,
        "sketch_constrain_vertical returned an invalid contract response; document state requires reconciliation",
        committed=True if committed else None,
        diagnostics={"response": response},
    )


def parse_sketch_constrain_vertical_response(raw_response: object) -> SketchConstrainVerticalResult:
    response = _response_object(raw_response)
    if response is None:
        return make_sketch_constrain_vertical_uncertain(
            "INVALID_RPC_RESPONSE",
            "sketch_constrain_vertical returned a non-object response",
            committed=None,
        )
    version = response.get("contract_version")
    details = _response_details(response)
    if type(version) is not int or version != SKETCH_CONSTRAIN_VERTICAL_CONTRACT_VERSION or details is None:
    ...
    if (
        ...
        return make_sketch_constrain_vertical_success(SketchName(sketch), constraint_index)
    ...
    if (
        ...
        if _valid_rejection(response):
            return make_sketch_constrain_vertical_failure(
                error_code,
                error,
        ...
        if _valid_uncertain(response):
            ...
            return make_sketch_constrain_vertical_uncertain(error_code, error, committed=committed, **details)
    return _invalid_response(response)


__all__ = [
    "SKETCH_CONSTRAIN_VERTICAL_CONTRACT_VERSION",
    "SketchConstrainVerticalCollaborators",
    "SketchConstrainVerticalFailure",
    "SketchConstrainVerticalRequest",
    "SketchConstrainVerticalResult",
    "SketchConstrainVerticalSuccess",
    "SketchConstrainVerticalUncertain",
    "DocumentName",
