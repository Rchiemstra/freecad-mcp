"""Versioned, stdlib-only wire contract for ``redo``."""
...
REDO_CONTRACT_VERSION: Literal[1] = 1

DocumentName = NewType("DocumentName", str)


class RedoReadDocument(Protocol):
...
RedoResult = RedoSuccess | RedoFailure | RedoUncertain
...
def make_redo_success(document_name: DocumentName) -> RedoSuccess:
    """Construct a complete committed result."""

    return {
        "contract_version": REDO_CONTRACT_VERSION,
        "success": True,
        "ok": True,
        "outcome": "committed",
        "committed": True,
...
def _invalid_response(response: dict[str, object]) -> RedoUncertain:
    ...
    code = (
        ...
        if committed
        else "REDO_ROLLBACK_UNCERTAIN"
        if rollback_failed
        else "INVALID_REDO_RESPONSE"
    )
    return make_redo_uncertain(
        code,
        "redo returned an invalid contract response; document state requires reconciliation",
...
def parse_redo_response(raw_response: object) -> RedoResult:
    ...
    if response is None:
        return make_redo_uncertain(
            ...
            "redo returned a non-object response",
            committed=None,
        )
    ...
    if (
        ...
        if _valid_uncertain(response):
            ...
            return make_redo_uncertain(error_code, error, committed=committed, **details)
    return _invalid_response(response)
