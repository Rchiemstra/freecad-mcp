"""Versioned, stdlib-only wire contract for ``edit_object``."""
...
EDIT_OBJECT_CONTRACT_VERSION: Literal[1] = 1

DocumentName = NewType("DocumentName", str)
ObjectName = NewType("ObjectName", str)


class EditObjectPayload(TypedDict):
    """JSON object payload accepted by the edit_object RPC method."""

    Properties: NotRequired[dict[str, object]]


class EditObjectReadDocument(Protocol):
...
class EditObjectSuccess(TypedDict):
...
class EditObjectFailure(TypedDict):
...
class EditObjectUncertain(TypedDict):
...
EditObjectResult = EditObjectSuccess | EditObjectFailure | EditObjectUncertain
...
def make_edit_object_success(object_name: ObjectName, label: str) -> EditObjectSuccess:
    """Construct a complete committed result."""

    return {
        "contract_version": EDIT_OBJECT_CONTRACT_VERSION,
        "success": True,
        "ok": True,
        "outcome": "committed",
        "committed": True,
...
def make_edit_object_uncertain(
    error_code: str,
    error: str,
    *,
    committed: bool | None,
...
def _invalid_response(response: dict[str, object]) -> EditObjectUncertain:
    ...
    code = (
        ...
        if committed
        else "EDIT_OBJECT_ROLLBACK_UNCERTAIN"
        if rollback_failed
        else "INVALID_EDIT_OBJECT_RESPONSE"
    )
    return make_edit_object_uncertain(
        code,
        "edit_object returned an invalid contract response; document state requires reconciliation",
        committed=True if committed else None,
        diagnostics={"response": response},
    )


def parse_edit_object_response(raw_response: object) -> EditObjectResult:
    """Validate all three wire variants; unknown state always stays uncertain."""
    response = _response_object(raw_response)
    if response is None:
        return make_edit_object_uncertain(
            "INVALID_RPC_RESPONSE",
            "edit_object returned a non-object response",
            committed=None,
        )
    version = response.get("contract_version")
    details = _response_details(response)
    if type(version) is not int or version != EDIT_OBJECT_CONTRACT_VERSION or details is None:
    ...
    if (
        ...
        return make_edit_object_success(ObjectName(object_name), label)
    ...
    if (
        ...
        and error.strip()
        and "object_name" not in response and "label" not in response
    ):
        ...
        if _valid_uncertain(response):
            ...
            return make_edit_object_uncertain(error_code, error, committed=committed, **details)
    return _invalid_response(response)


__all__ = [
    "EDIT_OBJECT_CONTRACT_VERSION",
