"""Versioned, stdlib-only wire contract for ``translate``."""
...
TRANSLATE_CONTRACT_VERSION: Literal[1] = 1
...
class TranslateCollaborators(Protocol):
...
class TranslateSuccess(TypedDict):
...
class TranslateFailure(TypedDict):
...
class TranslateUncertain(TypedDict):
...
TranslateResult = TranslateSuccess | TranslateFailure | TranslateUncertain
...
def make_translate_success(object: str, label: str) -> TranslateSuccess:
    """Construct a complete committed result."""

    return {
        "contract_version": TRANSLATE_CONTRACT_VERSION,
        "success": True,
        "ok": True,
        "outcome": "committed",
        "committed": True,
...
def _invalid_response(response: dict[str, object]) -> TranslateUncertain:
    ...
    code = (
        ...
        if committed
        else "TRANSLATE_ROLLBACK_UNCERTAIN"
        if rollback_failed
        else "INVALID_TRANSLATE_RESPONSE"
    )
    return make_translate_uncertain(
        code,
        "translate returned an invalid contract response; document state requires reconciliation",
        committed=True if committed else None,
        diagnostics={"response": response},
    )


def parse_translate_response(raw_response: object) -> TranslateResult:
    """Validate all three wire variants; unknown state always stays uncertain."""
    response = _response_object(raw_response)
    if response is None:
        return make_translate_uncertain(
            "INVALID_RPC_RESPONSE",
            "translate returned a non-object response",
            committed=None,
        )
    version = response.get("contract_version")
    details = _response_details(response)
    if type(version) is not int or version != TRANSLATE_CONTRACT_VERSION or details is None:
    ...
    if (
        ...
        and isinstance(object, str) and object.strip() and isinstance(label, str)
    ):
        return make_translate_success(object, label)

    error_code = response.get("error_code")
    error = response.get("error")
    success_keys = {'object', 'label'}
    if (
        ...
        if _valid_uncertain(response):
            ...
            return make_translate_uncertain(error_code, error, committed=committed, **details)
    return _invalid_response(response)


__all__ = [
    "TRANSLATE_CONTRACT_VERSION",
