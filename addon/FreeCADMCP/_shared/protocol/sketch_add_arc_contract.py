"""Versioned, stdlib-only wire contract for ``sketch_add_arc``."""
...
SKETCH_ADD_ARC_CONTRACT_VERSION: Literal[1] = 1
...
class SketchAddArcCollaborators(Protocol):
    @property
...
class SketchAddArcSuccess(TypedDict):
...
class SketchAddArcFailure(TypedDict):
    contract_version: Literal[1]
...
class SketchAddArcUncertain(TypedDict):
    contract_version: Literal[1]
    success: Literal[False]
    ok: Literal[False]
...
SketchAddArcResult = SketchAddArcSuccess | SketchAddArcFailure | SketchAddArcUncertain
...
def make_sketch_add_arc_success(sketch: SketchName, geometry_index: int) -> SketchAddArcSuccess:
    return {
        "contract_version": SKETCH_ADD_ARC_CONTRACT_VERSION,
        "success": True,
        "ok": True,
        "outcome": "committed",
        "committed": True,
...
def make_sketch_add_arc_failure(
    error_code: str,
    error: str,
    *,
...
def make_sketch_add_arc_uncertain(
    error_code: str,
    error: str,
    *,
    committed: bool | None,
    ...
    rollback_failed: bool | None = None,
    diagnostics: dict[str, object] | None = None,
) -> SketchAddArcUncertain:
    result: SketchAddArcUncertain = {
        "contract_version": SKETCH_ADD_ARC_CONTRACT_VERSION,
        "success": False,
        "ok": False,
        "outcome": "uncertain",
...
def _invalid_response(response: dict[str, object]) -> SketchAddArcUncertain:
    ...
    code = (
        ...
        if committed
        else "SKETCH_ADD_ARC_ROLLBACK_UNCERTAIN"
        if rollback_failed
        else "INVALID_SKETCH_ADD_ARC_RESPONSE"
    )
    return make_sketch_add_arc_uncertain(
        code,
        "sketch_add_arc returned an invalid contract response; document state requires reconciliation",
        committed=True if committed else None,
        diagnostics={"response": response},
    )


def parse_sketch_add_arc_response(raw_response: object) -> SketchAddArcResult:
    response = _response_object(raw_response)
    if response is None:
        return make_sketch_add_arc_uncertain(
            "INVALID_RPC_RESPONSE",
            "sketch_add_arc returned a non-object response",
            committed=None,
        )
    version = response.get("contract_version")
    details = _response_details(response)
    if type(version) is not int or version != SKETCH_ADD_ARC_CONTRACT_VERSION or details is None:
    ...
    if (
        ...
        return make_sketch_add_arc_success(SketchName(sketch), geometry_index)
    ...
    if (
        ...
        if _valid_rejection(response):
            return make_sketch_add_arc_failure(
                error_code,
                error,
        ...
        if _valid_uncertain(response):
            ...
            return make_sketch_add_arc_uncertain(error_code, error, committed=committed, **details)
    return _invalid_response(response)


__all__ = [
    "SKETCH_ADD_ARC_CONTRACT_VERSION",
