"""Versioned, stdlib-only wire contract for ``create_subshape_binder``."""
...
CREATE_SUBSHAPE_BINDER_CONTRACT_VERSION: Literal[1] = 1

DocumentName = NewType("DocumentName", str)
CreateSubshapeBinderName = NewType("CreateSubshapeBinderName", str)


class CreateSubshapeBinderObject(Protocol):
...
class CreateSubshapeBinderReadDocument(Protocol):
    """Read-only document surface available after native recompute."""

    @property
    def Name(self) -> str: ...

    def getObject(self, name: str) -> CreateSubshapeBinderObject | None: ...


class CreateSubshapeBinderDocument(CreateSubshapeBinderReadDocument, Protocol):
    """Narrow mutation surface available only to the apply callback."""

    def addObject(self, object_type: str, name: str) -> CreateSubshapeBinderObject: ...


@runtime_checkable
class NativeCreateSubshapeBinderDocument(CreateSubshapeBinderDocument, Protocol):
...
class CreateSubshapeBinderCollaborators(Protocol):
...
class CreateSubshapeBinderRequest:
...
class CreateSubshapeBinderSuccess(TypedDict):
...
class CreateSubshapeBinderFailure(TypedDict):
...
class CreateSubshapeBinderUncertain(TypedDict):
...
CreateSubshapeBinderResult = CreateSubshapeBinderSuccess | CreateSubshapeBinderFailure | CreateSubshapeBinderUncertain

_CORE_KEYS = frozenset(
    {
        "contract_version", "success", "ok", "outcome", "committed", "retry_safe", 'binder_name', "error_code", "error", "native_status", "native_message", "rollback_succeeded", "rollback_failed", "diagnostics"
    }
)


def make_create_subshape_binder_success(binder_name: str) -> CreateSubshapeBinderSuccess:
    """Construct a complete committed result."""

    return {
        "contract_version": CREATE_SUBSHAPE_BINDER_CONTRACT_VERSION,
        "success": True,
        "ok": True,
        "outcome": "committed",
        "committed": True,
...
def make_create_subshape_binder_failure(
    error_code: str,
    error: str,
    *,
    ...
    result: CreateSubshapeBinderFailure = {
        "contract_version": CREATE_SUBSHAPE_BINDER_CONTRACT_VERSION,
        "success": False,
        "ok": False,
        "outcome": "rejected",
...
def make_create_subshape_binder_uncertain(
    error_code: str,
    error: str,
    *,
    committed: bool | None,
    ...
    result: CreateSubshapeBinderUncertain = {
        "contract_version": CREATE_SUBSHAPE_BINDER_CONTRACT_VERSION,
        "success": False,
        "ok": False,
        "outcome": "uncertain",
...
def _invalid_response(response: dict[str, object]) -> CreateSubshapeBinderUncertain:
    ...
    code = (
        ...
        if committed
        else "CREATE_SUBSHAPE_BINDER_ROLLBACK_UNCERTAIN"
        if rollback_failed
        else "INVALID_CREATE_SUBSHAPE_BINDER_RESPONSE"
    )
    return make_create_subshape_binder_uncertain(
        code,
        "create_subshape_binder returned an invalid contract response; document state requires reconciliation",
        committed=True if committed else None,
        diagnostics={"response": response},
    )


def parse_create_subshape_binder_response(raw_response: object) -> CreateSubshapeBinderResult:
    """Validate all three wire variants; unknown state always stays uncertain."""
    response = _response_object(raw_response)
    if response is None:
        return make_create_subshape_binder_uncertain(
            "INVALID_RPC_RESPONSE",
            "create_subshape_binder returned a non-object response",
            committed=None,
        )
    version = response.get("contract_version")
    details = _response_details(response)
    if type(version) is not int or version != CREATE_SUBSHAPE_BINDER_CONTRACT_VERSION or details is None:
        return _invalid_response(response)

    binder_name = response.get('binder_name')
    if (
        _valid_success(response)
        and isinstance(binder_name, str) and binder_name.strip()
    ):
        return make_create_subshape_binder_success(str(binder_name))
    ...
    if (
        ...
        and error.strip()
        and 'binder_name' not in response
    ):
        if _valid_rejection(response):
            return make_create_subshape_binder_failure(
                error_code,
                error,
        ...
        if _valid_uncertain(response):
            ...
            return make_create_subshape_binder_uncertain(error_code, error, committed=committed, **details)
    return _invalid_response(response)


__all__ = [
    "CREATE_SUBSHAPE_BINDER_CONTRACT_VERSION",
