"""Versioned, stdlib-only wire contract for ``set_expression``."""
...
SET_EXPRESSION_CONTRACT_VERSION: Literal[1] = 1

DocumentName = NewType("DocumentName", str)
SetExpressionName = NewType("SetExpressionName", str)


class SetExpressionObject(Protocol):
...
class SetExpressionReadDocument(Protocol):
    """Read-only document surface available after native recompute."""

    @property
    def Name(self) -> str: ...

    def getObject(self, name: str) -> SetExpressionObject | None: ...


class SetExpressionDocument(SetExpressionReadDocument, Protocol):
    """Narrow mutation surface available only to the apply callback."""

    def addObject(self, object_type: str, name: str) -> SetExpressionObject: ...


@runtime_checkable
class NativeSetExpressionDocument(SetExpressionDocument, Protocol):
...
class SetExpressionCollaborators(Protocol):
...
class SetExpressionSuccess(TypedDict):
...
class SetExpressionFailure(TypedDict):
...
class SetExpressionUncertain(TypedDict):
...
SetExpressionResult = SetExpressionSuccess | SetExpressionFailure | SetExpressionUncertain

_CORE_KEYS = frozenset(
    {
        "contract_version", "success", "ok", "outcome", "committed", "retry_safe", 'object', 'prop_path', 'expression', "error_code", "error", "native_status", "native_message", "rollback_succeeded", "rollback_failed", "diagnostics"
    }
)


def make_set_expression_success(object: str, prop_path: str, expression: str) -> SetExpressionSuccess:
    """Construct a complete committed result."""

    return {
        "contract_version": SET_EXPRESSION_CONTRACT_VERSION,
        "success": True,
        "ok": True,
        "outcome": "committed",
        "committed": True,
...
def make_set_expression_failure(
    error_code: str,
    error: str,
    *,
...
def make_set_expression_uncertain(
    error_code: str,
    error: str,
    *,
    committed: bool | None,
...
def _invalid_response(response: dict[str, object]) -> SetExpressionUncertain:
    ...
    code = (
        ...
        if committed
        else "SET_EXPRESSION_ROLLBACK_UNCERTAIN"
        if rollback_failed
        else "INVALID_SET_EXPRESSION_RESPONSE"
    )
    return make_set_expression_uncertain(
        code,
        "set_expression returned an invalid contract response; document state requires reconciliation",
        committed=True if committed else None,
        diagnostics={"response": response},
    )


def parse_set_expression_response(raw_response: object) -> SetExpressionResult:
    """Validate all three wire variants; unknown state always stays uncertain."""
    response = _response_object(raw_response)
    if response is None:
        return make_set_expression_uncertain(
            "INVALID_RPC_RESPONSE",
            "set_expression returned a non-object response",
            committed=None,
        )
    version = response.get("contract_version")
    details = _response_details(response)
    if type(version) is not int or version != SET_EXPRESSION_CONTRACT_VERSION or details is None:
    ...
    if (
        ...
        and isinstance(expression, str) and expression.strip()
    ):
        return make_set_expression_success(str(object), str(prop_path), str(expression))
    ...
    if (
        ...
        and error.strip()
        and 'object' not in response and 'prop_path' not in response and 'expression' not in response
    ):
        if _valid_rejection(response):
            return make_set_expression_failure(
                error_code,
                error,
        ...
        if _valid_uncertain(response):
            ...
            return make_set_expression_uncertain(error_code, error, committed=committed, **details)
    return _invalid_response(response)


__all__ = [
    "SET_EXPRESSION_CONTRACT_VERSION",
