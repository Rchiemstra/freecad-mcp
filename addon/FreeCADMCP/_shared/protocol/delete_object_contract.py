"""Versioned, stdlib-only wire contract for ``delete_object``."""
...
DELETE_OBJECT_CONTRACT_VERSION: Literal[1] = 1

DocumentName = NewType("DocumentName", str)
ObjectName = NewType("ObjectName", str)


class DeleteObjectReadDocument(Protocol):
...
class DeleteObjectSuccess(TypedDict):
...
class DeleteObjectUncertain(TypedDict):
...
DeleteObjectResult = DeleteObjectSuccess | DeleteObjectFailure | DeleteObjectUncertain
...
def make_delete_object_uncertain(
    error_code: str,
    error: str,
    *,
    committed: bool | None,
...
def _invalid_response(response: dict[str, object]) -> DeleteObjectUncertain:
    ...
    code = (
        ...
        if committed
        else "DELETE_OBJECT_ROLLBACK_UNCERTAIN"
        if rollback_failed
        else "INVALID_DELETE_OBJECT_RESPONSE"
    )
    return make_delete_object_uncertain(
        code,
        "delete_object returned an invalid contract response; document state requires reconciliation",
        committed=True if committed else None,
        diagnostics={"response": response},
    )


def parse_delete_object_response(raw_response: object) -> DeleteObjectResult:
    """Validate all three wire variants; unknown state always stays uncertain."""
    response = _response_object(raw_response)
    if response is None:
        return make_delete_object_uncertain(
            "INVALID_RPC_RESPONSE",
            "delete_object returned a non-object response",
            committed=None,
        )
    version = response.get("contract_version")
    details = _response_details(response)
    if type(version) is not int or version != DELETE_OBJECT_CONTRACT_VERSION or details is None:
    ...
    if (
        ...
        and error.strip()
        and "object_name" not in response and "deleted" not in response
    ):
        ...
        if _valid_uncertain(response):
            ...
            return make_delete_object_uncertain(error_code, error, committed=committed, **details)
    return _invalid_response(response)


__all__ = [
    "DELETE_OBJECT_CONTRACT_VERSION",
