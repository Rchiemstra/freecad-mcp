"""Versioned, stdlib-only wire contract for ``measure_angle``."""
...
MEASURE_ANGLE_CONTRACT_VERSION: Literal[1] = 1
...
class MeasureAngleCollaborators(Protocol):
...
class MeasureAngleSuccess(TypedDict):
...
class MeasureAngleFailure(TypedDict):
...
class MeasureAngleUncertain(TypedDict):
...
MeasureAngleResult = MeasureAngleSuccess | MeasureAngleFailure | MeasureAngleUncertain
...
def make_measure_angle_success(
    angle_deg: float, unit: str,
) -> MeasureAngleSuccess:
    """Construct a complete observed result."""

    return {
        "contract_version": MEASURE_ANGLE_CONTRACT_VERSION,
        "success": True,
        "ok": True,
        "outcome": "observed",
        "retry_safe": False,
...
def make_measure_angle_failure(
    error_code: str,
    error: str,
    *,
...
def make_measure_angle_uncertain(
    error_code: str,
    error: str,
    *,
    committed: bool | None,
...
def _invalid_response(response: dict[str, object]) -> MeasureAngleUncertain:
    ...
    code = (
        "MEASURE_ANGLE_COMMITTED_RESPONSE_INVALID"
        if committed
        else "MEASURE_ANGLE_ROLLBACK_UNCERTAIN"
        if rollback_failed
        else "INVALID_MEASURE_ANGLE_RESPONSE"
    )
    return make_measure_angle_uncertain(
        code,
        "measure_angle returned an invalid contract response; document state requires reconciliation",
        committed=True if committed else None,
        diagnostics={"response": response},
    )


def parse_measure_angle_response(raw_response: object) -> MeasureAngleResult:
    """Validate all three wire variants; unknown state always stays uncertain."""
    response = _response_object(raw_response)
    if response is None:
        return make_measure_angle_uncertain(
            "INVALID_RPC_RESPONSE",
            "measure_angle returned a non-object response",
            committed=None,
        )
    version = response.get("contract_version")
    details = _response_details(response)
    if type(version) is not int or version != MEASURE_ANGLE_CONTRACT_VERSION or details is None:
        return _invalid_response(response)

    angle_deg = response.get("angle_deg")
    unit = response.get("unit")
    if _valid_success(response) and isinstance(angle_deg, (int, float)) and not isinstance(angle_deg, bool) and isinstance(unit, str):
        return make_measure_angle_success(float(angle_deg), unit)

    error_code = response.get("error_code")
    error = response.get("error")
    success_keys = {'angle_deg', 'unit'}
    if (
        ...
        if _valid_rejection(response):
            return make_measure_angle_failure(
                error_code,
                error,
        ...
        if _valid_uncertain(response):
            ...
            return make_measure_angle_uncertain(error_code, error, committed=committed, **details)
    return _invalid_response(response)


__all__ = [
    "MEASURE_ANGLE_CONTRACT_VERSION",
