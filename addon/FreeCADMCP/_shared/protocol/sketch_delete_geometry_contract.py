"""Versioned, stdlib-only wire contract for ``sketch_delete_geometry``."""
...
SKETCH_DELETE_GEOMETRY_CONTRACT_VERSION: Literal[1] = 1

DocumentName = NewType("DocumentName", str)
SketchName = NewType("SketchName", str)


class SketchDeleteGeometryObject(Protocol):
...
class SketchDeleteGeometryReadDocument(Protocol):
    """Read-only document surface available after native recompute."""

    @property
    def Name(self) -> str: ...

    def getObject(self, name: str) -> SketchDeleteGeometryObject | None: ...


class SketchDeleteGeometryDocument(SketchDeleteGeometryReadDocument, Protocol):
    """Narrow mutation surface available only to the apply callback."""

    def addObject(self, object_type: str, name: str) -> SketchDeleteGeometryObject: ...


@runtime_checkable
class NativeSketchDeleteGeometryDocument(SketchDeleteGeometryDocument, Protocol):
...
class SketchDeleteGeometryCollaborators(Protocol):
    ...
    def validate_document_invariants(self, document: SketchDeleteGeometryReadDocument) -> object:
...
class SketchDeleteGeometryRequest:
...
class SketchDeleteGeometrySuccess(TypedDict):
...
class SketchDeleteGeometryFailure(TypedDict):
...
class SketchDeleteGeometryUncertain(TypedDict):
...
SketchDeleteGeometryResult = SketchDeleteGeometrySuccess | SketchDeleteGeometryFailure | SketchDeleteGeometryUncertain
...
def make_sketch_delete_geometry_success(sketch: SketchName, deleted_count: int) -> SketchDeleteGeometrySuccess:
    """Construct a complete committed result using the actual assigned identity."""

    return {
        "contract_version": SKETCH_DELETE_GEOMETRY_CONTRACT_VERSION,
        "success": True,
        "ok": True,
        "outcome": "committed",
        "committed": True,
...
def make_sketch_delete_geometry_failure(
    error_code: str,
    error: str,
    *,
    ...
    result: SketchDeleteGeometryFailure = {
        "contract_version": SKETCH_DELETE_GEOMETRY_CONTRACT_VERSION,
        "success": False,
        "ok": False,
        "outcome": "rejected",
...
def make_sketch_delete_geometry_uncertain(
    error_code: str,
    error: str,
    *,
    committed: bool | None,
    ...
    result: SketchDeleteGeometryUncertain = {
        "contract_version": SKETCH_DELETE_GEOMETRY_CONTRACT_VERSION,
        "success": False,
        "ok": False,
        "outcome": "uncertain",
...
def _invalid_response(response: dict[str, object]) -> SketchDeleteGeometryUncertain:
    ...
    code = (
        ...
        if committed
        else "SKETCH_DELETE_GEOMETRY_ROLLBACK_UNCERTAIN"
        if rollback_failed
        else "INVALID_SKETCH_DELETE_GEOMETRY_RESPONSE"
    )
    return make_sketch_delete_geometry_uncertain(
        code,
        "sketch_delete_geometry returned an invalid contract response; document state requires reconciliation",
        committed=True if committed else None,
        diagnostics={"response": response},
    )


def parse_sketch_delete_geometry_response(raw_response: object) -> SketchDeleteGeometryResult:
    """Validate all three wire variants; unknown state always stays uncertain."""
    response = _response_object(raw_response)
    if response is None:
        return make_sketch_delete_geometry_uncertain(
            "INVALID_RPC_RESPONSE",
            "sketch_delete_geometry returned a non-object response",
            committed=None,
        )
    version = response.get("contract_version")
    details = _response_details(response)
    if type(version) is not int or version != SKETCH_DELETE_GEOMETRY_CONTRACT_VERSION or details is None:
    ...
    if (
        ...
        return make_sketch_delete_geometry_success(SketchName(sketch), deleted_count)
    ...
    if (
        ...
        if _valid_uncertain(response):
            ...
            return make_sketch_delete_geometry_uncertain(error_code, error, committed=committed, **details)
    return _invalid_response(response)


__all__ = [
    "SKETCH_DELETE_GEOMETRY_CONTRACT_VERSION",
    "SketchDeleteGeometryCollaborators",
    "SketchDeleteGeometryFailure",
    "SketchDeleteGeometryRequest",
    "SketchDeleteGeometryResult",
    "SketchDeleteGeometrySuccess",
    "SketchDeleteGeometryUncertain",
    ...
    "parse_sketch_delete_geometry_response",
]
