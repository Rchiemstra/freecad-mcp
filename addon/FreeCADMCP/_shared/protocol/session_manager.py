...
from .constants import (
    HANDSHAKE_RESPONSE_KIND as _HANDSHAKE_RESPONSE_KIND,
)
from .constants import (
    MAX_HANDSHAKE_NONCES as _MAX_HANDSHAKE_NONCES,
)
from .constants import (
    MAX_SESSION_TTL_SECONDS as _MAX_SESSION_TTL_SECONDS,
)
from .constants import (
    PROTOCOL_VERSION as _PROTOCOL_VERSION,
)
from .handshake_request import verify_handshake_request as _verify_handshake_request
from .handshake_response import sign_handshake_response as _sign_handshake_response
...
from .runtime_manifest import RuntimeManifest as _RuntimeManifest
from .session_context import SessionContext as _SessionContext
...
class SessionManager:
    ...
    def __init__(
        ...
        *,
        ...
        utcnow: _Callable[[], _datetime] = lambda: _datetime.now(_UTC),
    ) -> None:
        if not 1 <= session_ttl_seconds <= _MAX_SESSION_TTL_SECONDS:
    ...
    def perform_handshake(self, payload: _Mapping[str, _Any]) -> dict[str, _Any]:
        verified = _verify_handshake_request(
        ...
        with self._lock:
            ...
            if len(self._seen_nonces) >= _MAX_HANDSHAKE_NONCES:
                raise _ProtocolError(
            ...
            issued_dt = issued_dt.astimezone(_UTC)
            expires_dt = issued_dt + _timedelta(seconds=self._session_ttl)
            negotiated = tuple(
        ...
        response = {
            "kind": _HANDSHAKE_RESPONSE_KIND,
            "protocol_version": _PROTOCOL_VERSION,
            "client_nonce": verified.client_nonce,
            "server_nonce": _secrets.token_urlsafe(32),
            "session_id": session_id,
            "session_token": token,
        ...
        return _sign_handshake_response(response, self._secret)

    def authenticate(self, session_token: str, *, mcp_runtime_id: str) -> _SessionContext:
        ...
        with self._lock:
            ...
            if record is None or not _hmac.compare_digest(record.token_digest, digest):
                raise _ProtocolError(
            ...
            if now_mono >= record.expires_monotonic:
                raise _ProtocolError("SESSION_EXPIRED", "RPC session has expired")
            if not _hmac.compare_digest(record.context.mcp.runtime_id, runtime_id):
                raise _ProtocolError(
    ...
    def authenticate_envelope(
        self,
        payload: _Mapping[str, _Any] | _RequestEnvelope,
        *,
        transport_mcp_runtime_id: str | None = None,
    ) -> tuple[_SessionContext, _RequestEnvelope]:
        envelope = (
            payload
            if isinstance(payload, _RequestEnvelope)
            else _RequestEnvelope.from_dict(payload)
        )
        if (
