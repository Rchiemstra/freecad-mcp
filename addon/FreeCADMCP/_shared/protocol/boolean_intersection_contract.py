"""Versioned, stdlib-only wire contract for ``boolean_intersection``."""
...
BOOLEAN_INTERSECTION_CONTRACT_VERSION: Literal[1] = 1
...
class BooleanIntersectionCollaborators(Protocol):
...
class BooleanIntersectionRequest:
...
class BooleanIntersectionSuccess(TypedDict):
...
class BooleanIntersectionFailure(TypedDict):
...
class BooleanIntersectionUncertain(TypedDict):
...
BooleanIntersectionResult = BooleanIntersectionSuccess | BooleanIntersectionFailure | BooleanIntersectionUncertain
...
def make_boolean_intersection_success(feature: FeatureName, label: str) -> BooleanIntersectionSuccess:
    """Construct a complete committed result using the actual assigned name."""

    return {
        "contract_version": BOOLEAN_INTERSECTION_CONTRACT_VERSION,
        "success": True,
        "ok": True,
        "outcome": "committed",
        "committed": True,
...
def make_boolean_intersection_failure(
    error_code: str,
    error: str,
    *,
    ...
    result: BooleanIntersectionFailure = {
        "contract_version": BOOLEAN_INTERSECTION_CONTRACT_VERSION,
        "success": False,
        "ok": False,
        "outcome": "rejected",
...
def make_boolean_intersection_uncertain(
    error_code: str,
    error: str,
    *,
    committed: bool | None,
    ...
    result: BooleanIntersectionUncertain = {
        "contract_version": BOOLEAN_INTERSECTION_CONTRACT_VERSION,
        "success": False,
        "ok": False,
        "outcome": "uncertain",
...
def _invalid_response(response: dict[str, object]) -> BooleanIntersectionUncertain:
    ...
    code = (
        ...
        if committed
        else "BOOLEAN_INTERSECTION_ROLLBACK_UNCERTAIN"
        if rollback_failed
        else "INVALID_BOOLEAN_INTERSECTION_RESPONSE"
    )
    return make_boolean_intersection_uncertain(
        code,
        "boolean_intersection returned an invalid contract response; document state requires reconciliation",
        committed=True if committed else None,
        diagnostics={"response": response},
    )


def parse_boolean_intersection_response(raw_response: object) -> BooleanIntersectionResult:
    """Validate all three wire variants; unknown state always stays uncertain."""
    response = _response_object(raw_response)
    if response is None:
        return make_boolean_intersection_uncertain(
            "INVALID_RPC_RESPONSE",
            "boolean_intersection returned a non-object response",
            committed=None,
        )
    version = response.get("contract_version")
    details = _response_details(response)
    if type(version) is not int or version != BOOLEAN_INTERSECTION_CONTRACT_VERSION or details is None:
    ...
    if (
        ...
        return make_boolean_intersection_success(FeatureName(feature), label)
    ...
    if (
        ...
        if _valid_rejection(response):
            return make_boolean_intersection_failure(
                error_code,
                error,
        ...
        if _valid_uncertain(response):
            ...
            return make_boolean_intersection_uncertain(error_code, error, committed=committed, **details)
    return _invalid_response(response)


__all__ = [
    "BOOLEAN_INTERSECTION_CONTRACT_VERSION",
