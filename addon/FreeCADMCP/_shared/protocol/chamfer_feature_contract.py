"""Versioned, stdlib-only wire contract for ``chamfer_feature``."""
...
CHAMFER_FEATURE_CONTRACT_VERSION: Literal[1] = 1
...
class ChamferFeatureCollaborators(Protocol):
...
class ChamferFeatureRequest:
    """Validated internal request."""

    doc_name: DocumentName
    base_feature: FeatureName
    chamfer_name: FeatureName
    size: float
    edge_refs: tuple[str, ...]
    body_name: FeatureName | None


class ChamferFeatureSuccess(TypedDict):
...
class ChamferFeatureFailure(TypedDict):
...
class ChamferFeatureUncertain(TypedDict):
...
ChamferFeatureResult = ChamferFeatureSuccess | ChamferFeatureFailure | ChamferFeatureUncertain
...
def make_chamfer_feature_success(feature: FeatureName, label: str) -> ChamferFeatureSuccess:
    """Construct a complete committed result using the actual assigned name."""

    return {
        "contract_version": CHAMFER_FEATURE_CONTRACT_VERSION,
        "success": True,
        "ok": True,
        "outcome": "committed",
        "committed": True,
...
def make_chamfer_feature_failure(
    error_code: str,
    error: str,
    *,
...
def make_chamfer_feature_uncertain(
    error_code: str,
    error: str,
    *,
    committed: bool | None,
    ...
    result: ChamferFeatureUncertain = {
        "contract_version": CHAMFER_FEATURE_CONTRACT_VERSION,
        "success": False,
        "ok": False,
        "outcome": "uncertain",
...
def _invalid_response(response: dict[str, object]) -> ChamferFeatureUncertain:
    ...
    code = (
        ...
        if committed
        else "CHAMFER_FEATURE_ROLLBACK_UNCERTAIN"
        if rollback_failed
        else "INVALID_CHAMFER_FEATURE_RESPONSE"
    )
    return make_chamfer_feature_uncertain(
        code,
        "chamfer_feature returned an invalid contract response; document state requires reconciliation",
        committed=True if committed else None,
        diagnostics={"response": response},
    )


def parse_chamfer_feature_response(raw_response: object) -> ChamferFeatureResult:
    """Validate all three wire variants; unknown state always stays uncertain."""
    response = _response_object(raw_response)
    if response is None:
        return make_chamfer_feature_uncertain(
            "INVALID_RPC_RESPONSE",
            "chamfer_feature returned a non-object response",
            committed=None,
        )
    version = response.get("contract_version")
    details = _response_details(response)
    if type(version) is not int or version != CHAMFER_FEATURE_CONTRACT_VERSION or details is None:
    ...
    if (
        ...
        return make_chamfer_feature_success(FeatureName(feature), label)
    ...
    if (
        ...
        if _valid_uncertain(response):
            ...
            return make_chamfer_feature_uncertain(error_code, error, committed=committed, **details)
    return _invalid_response(response)


__all__ = [
    "CHAMFER_FEATURE_CONTRACT_VERSION",
    "ChamferFeatureCollaborators",
    "ChamferFeatureFailure",
    "ChamferFeatureRequest",
    "ChamferFeatureResult",
    "ChamferFeatureSuccess",
    "ChamferFeatureUncertain",
    "FeatureDocument",
    "FeatureName",
    "FeatureObject",
