"""Versioned, stdlib-only wire contract for ``create_document``."""
...
CREATE_DOCUMENT_CONTRACT_VERSION: Literal[1] = 1

DocumentName = NewType("DocumentName", str)


class CreateDocumentReadDocument(Protocol):
...
class CreateDocumentSuccess(TypedDict):
...
class CreateDocumentFailure(TypedDict):
...
def make_create_document_failure(
    error_code: str,
    error: str,
    *,
...
def make_create_document_uncertain(
    ...
    *,
    ...
    result: CreateDocumentUncertain = {
        "contract_version": CREATE_DOCUMENT_CONTRACT_VERSION,
        "success": False,
        "ok": False,
        "outcome": "uncertain",
...
def _invalid_response(response: dict[str, object]) -> CreateDocumentUncertain:
    ...
    code = (
        ...
        if committed
        else "CREATE_DOCUMENT_ROLLBACK_UNCERTAIN"
        if rollback_failed
        else "INVALID_CREATE_DOCUMENT_RESPONSE"
    )
    return make_create_document_uncertain(
        code,
        "create_document returned an invalid contract response; document state requires reconciliation",
        committed=True if committed else None,
        diagnostics={"response": response},
    )


def parse_create_document_response(raw_response: object) -> CreateDocumentResult:
    """Validate all three wire variants; unknown state always stays uncertain."""
    response = _response_object(raw_response)
    if response is None:
        return make_create_document_uncertain(
            "INVALID_RPC_RESPONSE",
            "create_document returned a non-object response",
            committed=None,
        )
    version = response.get("contract_version")
    details = _response_details(response)
    if type(version) is not int or version != CREATE_DOCUMENT_CONTRACT_VERSION or details is None:
    ...
    if (
        ...
        return make_create_document_success(DocumentName(document_name))
    ...
    if (
        ...
        if _valid_uncertain(response):
            ...
            return make_create_document_uncertain(error_code, error, committed=committed, **details)
    return _invalid_response(response)


__all__ = [
    "CREATE_DOCUMENT_CONTRACT_VERSION",
