"""Versioned, stdlib-only wire contract for ``sketch_add_geometry``."""
...
SKETCH_ADD_GEOMETRY_CONTRACT_VERSION: Literal[1] = 1

DocumentName = NewType("DocumentName", str)
SketchName = NewType("SketchName", str)


class SketchAddGeometryObject(Protocol):
...
class SketchAddGeometryReadDocument(Protocol):
    """Read-only document surface available after native recompute."""

    @property
    def Name(self) -> str: ...

    def getObject(self, name: str) -> SketchAddGeometryObject | None: ...


class SketchAddGeometryDocument(SketchAddGeometryReadDocument, Protocol):
    """Narrow mutation surface available only to the apply callback."""

    def addObject(self, object_type: str, name: str) -> SketchAddGeometryObject: ...


@runtime_checkable
class NativeSketchAddGeometryDocument(SketchAddGeometryDocument, Protocol):
...
class SketchAddGeometryCollaborators(Protocol):
...
class SketchAddGeometryRequest:
...
class SketchAddGeometrySuccess(TypedDict):
...
class SketchAddGeometryFailure(TypedDict):
...
class SketchAddGeometryUncertain(TypedDict):
...
SketchAddGeometryResult = SketchAddGeometrySuccess | SketchAddGeometryFailure | SketchAddGeometryUncertain
...
def make_sketch_add_geometry_success(sketch: SketchName, indices: list[int]) -> SketchAddGeometrySuccess:
    """Construct a complete committed result using the actual assigned identity."""

    return {
        "contract_version": SKETCH_ADD_GEOMETRY_CONTRACT_VERSION,
        "success": True,
        "ok": True,
        "outcome": "committed",
        "committed": True,
...
def make_sketch_add_geometry_failure(
    error_code: str,
    error: str,
    *,
    ...
    result: SketchAddGeometryFailure = {
        "contract_version": SKETCH_ADD_GEOMETRY_CONTRACT_VERSION,
        "success": False,
        "ok": False,
        "outcome": "rejected",
...
def make_sketch_add_geometry_uncertain(
    error_code: str,
    error: str,
    *,
    committed: bool | None,
    ...
    result: SketchAddGeometryUncertain = {
        "contract_version": SKETCH_ADD_GEOMETRY_CONTRACT_VERSION,
        "success": False,
        "ok": False,
        "outcome": "uncertain",
...
def _invalid_response(response: dict[str, object]) -> SketchAddGeometryUncertain:
    ...
    code = (
        ...
        if committed
        else "SKETCH_ADD_GEOMETRY_ROLLBACK_UNCERTAIN"
        if rollback_failed
        else "INVALID_SKETCH_ADD_GEOMETRY_RESPONSE"
    )
    return make_sketch_add_geometry_uncertain(
        code,
        "sketch_add_geometry returned an invalid contract response; document state requires reconciliation",
        committed=True if committed else None,
        diagnostics={"response": response},
    )


def parse_sketch_add_geometry_response(raw_response: object) -> SketchAddGeometryResult:
    """Validate all three wire variants; unknown state always stays uncertain."""
    response = _response_object(raw_response)
    if response is None:
        return make_sketch_add_geometry_uncertain(
            "INVALID_RPC_RESPONSE",
            "sketch_add_geometry returned a non-object response",
            committed=None,
        )
    version = response.get("contract_version")
    details = _response_details(response)
    if type(version) is not int or version != SKETCH_ADD_GEOMETRY_CONTRACT_VERSION or details is None:
    ...
    if (
        ...
        and sketch.strip()
        and isinstance(indices, list) and all(
            isinstance(item, int) and not isinstance(item, bool) for item in indices
        )
    ):
        return make_sketch_add_geometry_success(SketchName(sketch), indices)
    ...
    if (
        ...
        and error.strip()
        and "sketch" not in response and "indices" not in response
    ):
        ...
        if _valid_uncertain(response):
            ...
            return make_sketch_add_geometry_uncertain(error_code, error, committed=committed, **details)
    return _invalid_response(response)


__all__ = [
    "SKETCH_ADD_GEOMETRY_CONTRACT_VERSION",
