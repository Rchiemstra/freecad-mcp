"""Versioned, stdlib-only wire contract for ``recompute_document``."""
...
RECOMPUTE_DOCUMENT_CONTRACT_VERSION: Literal[1] = 1

DocumentName = NewType("DocumentName", str)


class RecomputeDocumentReadDocument(Protocol):
...
class RecomputeDocumentDocument(RecomputeDocumentReadDocument, Protocol):
    """Narrow mutation surface available only to the apply callback."""

    def addObject(self, object_type: str, name: str) -> object: ...


class RecomputeDocumentCollaborators(Protocol):
...
class RecomputeDocumentRequest:
    """Validated internal request constructed from untyped JSON arguments."""

    doc_name: DocumentName


class RecomputeDocumentSuccess(TypedDict):
...
class RecomputeDocumentFailure(TypedDict):
...
class RecomputeDocumentUncertain(TypedDict):
...
RecomputeDocumentResult = RecomputeDocumentSuccess | RecomputeDocumentFailure | RecomputeDocumentUncertain
...
def make_recompute_document_success(document_name: DocumentName) -> RecomputeDocumentSuccess:
    """Construct a complete committed result."""

    return {
        "contract_version": RECOMPUTE_DOCUMENT_CONTRACT_VERSION,
        "success": True,
        "ok": True,
        "outcome": "committed",
        "committed": True,
...
def make_recompute_document_failure(
    error_code: str,
    error: str,
    *,
    ...
    result: RecomputeDocumentFailure = {
        "contract_version": RECOMPUTE_DOCUMENT_CONTRACT_VERSION,
        "success": False,
        "ok": False,
        "outcome": "rejected",
...
def make_recompute_document_uncertain(
    error_code: str,
    error: str,
    *,
    committed: bool | None,
    ...
    result: RecomputeDocumentUncertain = {
        "contract_version": RECOMPUTE_DOCUMENT_CONTRACT_VERSION,
        "success": False,
        "ok": False,
        "outcome": "uncertain",
...
def _invalid_response(response: dict[str, object]) -> RecomputeDocumentUncertain:
    ...
    code = (
        ...
        if committed
        else "RECOMPUTE_DOCUMENT_ROLLBACK_UNCERTAIN"
        if rollback_failed
        else "INVALID_RECOMPUTE_DOCUMENT_RESPONSE"
    )
    return make_recompute_document_uncertain(
        code,
        "recompute_document returned an invalid contract response; document state requires reconciliation",
        committed=True if committed else None,
        diagnostics={"response": response},
    )


def parse_recompute_document_response(raw_response: object) -> RecomputeDocumentResult:
    """Validate all three wire variants; unknown state always stays uncertain."""
    response = _response_object(raw_response)
    if response is None:
        return make_recompute_document_uncertain(
            "INVALID_RPC_RESPONSE",
            "recompute_document returned a non-object response",
            committed=None,
        )
    version = response.get("contract_version")
    details = _response_details(response)
    if type(version) is not int or version != RECOMPUTE_DOCUMENT_CONTRACT_VERSION or details is None:
    ...
    if (
        ...
        return make_recompute_document_success(DocumentName(document_name))
    ...
    if (
        ...
        and error.strip()
        and "document_name" not in response
    ):
        ...
        if _valid_uncertain(response):
            ...
            return make_recompute_document_uncertain(error_code, error, committed=committed, **details)
    return _invalid_response(response)


__all__ = [
    "RECOMPUTE_DOCUMENT_CONTRACT_VERSION",
    "RecomputeDocumentCollaborators",
    "RecomputeDocumentDocument",
    "RecomputeDocumentFailure",
