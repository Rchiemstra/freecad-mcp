"""Versioned, stdlib-only wire contract for ``helical_sweep_feature``."""
...
HELICAL_SWEEP_FEATURE_CONTRACT_VERSION: Literal[1] = 1
...
class HelicalSweepFeatureCollaborators(Protocol):
...
class HelicalSweepFeatureRequest:
    """Validated internal request."""

    doc_name: DocumentName
    profile_sketch: FeatureName
    helix_name: FeatureName
    pitch: float
    height: float
    radius: float
...
class HelicalSweepFeatureSuccess(TypedDict):
...
class HelicalSweepFeatureFailure(TypedDict):
...
class HelicalSweepFeatureUncertain(TypedDict):
...
HelicalSweepFeatureResult = HelicalSweepFeatureSuccess | HelicalSweepFeatureFailure | HelicalSweepFeatureUncertain
...
def make_helical_sweep_feature_success(feature: FeatureName, label: str) -> HelicalSweepFeatureSuccess:
    """Construct a complete committed result using the actual assigned name."""

    return {
        "contract_version": HELICAL_SWEEP_FEATURE_CONTRACT_VERSION,
        "success": True,
        "ok": True,
        "outcome": "committed",
        "committed": True,
...
def make_helical_sweep_feature_failure(
    error_code: str,
    error: str,
    *,
    ...
    result: HelicalSweepFeatureFailure = {
        "contract_version": HELICAL_SWEEP_FEATURE_CONTRACT_VERSION,
        "success": False,
        "ok": False,
        "outcome": "rejected",
...
def make_helical_sweep_feature_uncertain(
    error_code: str,
    error: str,
    *,
    committed: bool | None,
    ...
    result: HelicalSweepFeatureUncertain = {
        "contract_version": HELICAL_SWEEP_FEATURE_CONTRACT_VERSION,
        "success": False,
        "ok": False,
        "outcome": "uncertain",
...
def _invalid_response(response: dict[str, object]) -> HelicalSweepFeatureUncertain:
    ...
    code = (
        ...
        if committed
        else "HELICAL_SWEEP_FEATURE_ROLLBACK_UNCERTAIN"
        if rollback_failed
        else "INVALID_HELICAL_SWEEP_FEATURE_RESPONSE"
    )
    return make_helical_sweep_feature_uncertain(
        code,
        "helical_sweep_feature returned an invalid contract response; document state requires reconciliation",
        committed=True if committed else None,
        diagnostics={"response": response},
    )


def parse_helical_sweep_feature_response(raw_response: object) -> HelicalSweepFeatureResult:
    """Validate all three wire variants; unknown state always stays uncertain."""
    response = _response_object(raw_response)
    if response is None:
        return make_helical_sweep_feature_uncertain(
            "INVALID_RPC_RESPONSE",
            "helical_sweep_feature returned a non-object response",
            committed=None,
        )
    version = response.get("contract_version")
    details = _response_details(response)
    if type(version) is not int or version != HELICAL_SWEEP_FEATURE_CONTRACT_VERSION or details is None:
    ...
    if (
        ...
        return make_helical_sweep_feature_success(FeatureName(feature), label)
    ...
    if (
        ...
        if _valid_uncertain(response):
            ...
            return make_helical_sweep_feature_uncertain(error_code, error, committed=committed, **details)
    return _invalid_response(response)


__all__ = [
    "HELICAL_SWEEP_FEATURE_CONTRACT_VERSION",
