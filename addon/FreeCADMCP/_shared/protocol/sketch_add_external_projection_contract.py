"""Versioned, stdlib-only wire contract for ``sketch_add_external_projection``."""
...
SKETCH_ADD_EXTERNAL_PROJECTION_CONTRACT_VERSION: Literal[1] = 1

DocumentName = NewType("DocumentName", str)
SketchAddExternalProjectionName = NewType("SketchAddExternalProjectionName", str)


class SketchAddExternalProjectionObject(Protocol):
...
class SketchAddExternalProjectionReadDocument(Protocol):
    """Read-only document surface available after native recompute."""

    @property
    def Name(self) -> str: ...

    def getObject(self, name: str) -> SketchAddExternalProjectionObject | None: ...


class SketchAddExternalProjectionDocument(SketchAddExternalProjectionReadDocument, Protocol):
    """Narrow mutation surface available only to the apply callback."""

    def addObject(self, object_type: str, name: str) -> SketchAddExternalProjectionObject: ...


@runtime_checkable
class NativeSketchAddExternalProjectionDocument(SketchAddExternalProjectionDocument, Protocol):
...
class SketchAddExternalProjectionCollaborators(Protocol):
...
class SketchAddExternalProjectionRequest:
    ...
    projection_mode: str
    defining: bool
    allow_gui_geometry_loop: bool


class SketchAddExternalProjectionSuccess(TypedDict):
...
class SketchAddExternalProjectionFailure(TypedDict):
...
class SketchAddExternalProjectionUncertain(TypedDict):
...
SketchAddExternalProjectionResult = SketchAddExternalProjectionSuccess | SketchAddExternalProjectionFailure | SketchAddExternalProjectionUncertain

_CORE_KEYS = frozenset(
    {
        "contract_version", "success", "ok", "outcome", "committed", "retry_safe", 'sketch_name', "error_code", "error", "native_status", "native_message", "rollback_succeeded", "rollback_failed", "diagnostics"
    }
)


def make_sketch_add_external_projection_success(sketch_name: str) -> SketchAddExternalProjectionSuccess:
    """Construct a complete committed result."""

    return {
        "contract_version": SKETCH_ADD_EXTERNAL_PROJECTION_CONTRACT_VERSION,
        "success": True,
        "ok": True,
        "outcome": "committed",
        "committed": True,
...
def make_sketch_add_external_projection_failure(
    error_code: str,
    error: str,
    *,
    ...
    result: SketchAddExternalProjectionFailure = {
        "contract_version": SKETCH_ADD_EXTERNAL_PROJECTION_CONTRACT_VERSION,
        "success": False,
        "ok": False,
        "outcome": "rejected",
...
def make_sketch_add_external_projection_uncertain(
    error_code: str,
    error: str,
    *,
    committed: bool | None,
    ...
    result: SketchAddExternalProjectionUncertain = {
        "contract_version": SKETCH_ADD_EXTERNAL_PROJECTION_CONTRACT_VERSION,
        "success": False,
        "ok": False,
        "outcome": "uncertain",
...
def _invalid_response(response: dict[str, object]) -> SketchAddExternalProjectionUncertain:
    ...
    code = (
        ...
        if committed
        else "SKETCH_ADD_EXTERNAL_PROJECTION_ROLLBACK_UNCERTAIN"
        if rollback_failed
        else "INVALID_SKETCH_ADD_EXTERNAL_PROJECTION_RESPONSE"
    )
    return make_sketch_add_external_projection_uncertain(
        code,
        "sketch_add_external_projection returned an invalid contract response; document state requires reconciliation",
        committed=True if committed else None,
        diagnostics={"response": response},
    )


def parse_sketch_add_external_projection_response(raw_response: object) -> SketchAddExternalProjectionResult:
    """Validate all three wire variants; unknown state always stays uncertain."""
    response = _response_object(raw_response)
    if response is None:
        return make_sketch_add_external_projection_uncertain(
            "INVALID_RPC_RESPONSE",
            "sketch_add_external_projection returned a non-object response",
            committed=None,
        )
    version = response.get("contract_version")
    details = _response_details(response)
    if type(version) is not int or version != SKETCH_ADD_EXTERNAL_PROJECTION_CONTRACT_VERSION or details is None:
        return _invalid_response(response)

    sketch_name = response.get('sketch_name')
    if (
        _valid_success(response)
        and isinstance(sketch_name, str) and sketch_name.strip()
    ):
        return make_sketch_add_external_projection_success(str(sketch_name))
    ...
    if (
        ...
        and error.strip()
        and 'sketch_name' not in response
    ):
        if _valid_rejection(response):
            return make_sketch_add_external_projection_failure(
                error_code,
                error,
        ...
        if _valid_uncertain(response):
            ...
            return make_sketch_add_external_projection_uncertain(error_code, error, committed=committed, **details)
    return _invalid_response(response)


__all__ = [
    "SKETCH_ADD_EXTERNAL_PROJECTION_CONTRACT_VERSION",
    ...
    "SketchAddExternalProjectionObject",
