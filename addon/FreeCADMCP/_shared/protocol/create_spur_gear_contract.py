"""Versioned, stdlib-only wire contract for ``create_spur_gear``."""
...
CREATE_SPUR_GEAR_CONTRACT_VERSION: Literal[1] = 1
...
class CreateSpurGearCollaborators(Protocol):
...
class CreateSpurGearSuccess(TypedDict):
...
class CreateSpurGearFailure(TypedDict):
...
class CreateSpurGearUncertain(TypedDict):
...
CreateSpurGearResult = CreateSpurGearSuccess | CreateSpurGearFailure | CreateSpurGearUncertain
...
def make_create_spur_gear_success(body: str, sketch: str, feature: str, teeth: int, module: float) -> CreateSpurGearSuccess:
    """Construct a complete committed result."""

    return {
        "contract_version": CREATE_SPUR_GEAR_CONTRACT_VERSION,
        "success": True,
        "ok": True,
        "outcome": "committed",
        "committed": True,
...
def make_create_spur_gear_failure(
    error_code: str,
    error: str,
    *,
...
def make_create_spur_gear_uncertain(
    error_code: str,
    error: str,
    *,
    committed: bool | None,
    ...
    result: CreateSpurGearUncertain = {
        "contract_version": CREATE_SPUR_GEAR_CONTRACT_VERSION,
        "success": False,
        "ok": False,
        "outcome": "uncertain",
...
def _invalid_response(response: dict[str, object]) -> CreateSpurGearUncertain:
    ...
    code = (
        ...
        if committed
        else "CREATE_SPUR_GEAR_ROLLBACK_UNCERTAIN"
        if rollback_failed
        else "INVALID_CREATE_SPUR_GEAR_RESPONSE"
    )
    return make_create_spur_gear_uncertain(
        code,
        "create_spur_gear returned an invalid contract response; document state requires reconciliation",
        committed=True if committed else None,
        diagnostics={"response": response},
    )


def parse_create_spur_gear_response(raw_response: object) -> CreateSpurGearResult:
    """Validate all three wire variants; unknown state always stays uncertain."""
    response = _response_object(raw_response)
    if response is None:
        return make_create_spur_gear_uncertain(
            "INVALID_RPC_RESPONSE",
            "create_spur_gear returned a non-object response",
            committed=None,
        )
    version = response.get("contract_version")
    details = _response_details(response)
    if type(version) is not int or version != CREATE_SPUR_GEAR_CONTRACT_VERSION or details is None:
    ...
    if (
        ...
        return make_create_spur_gear_success(body, sketch, feature, teeth, float(module))
    ...
    if (
        ...
        if _valid_rejection(response):
            return make_create_spur_gear_failure(
                error_code,
                error,
        ...
        if _valid_uncertain(response):
            ...
            return make_create_spur_gear_uncertain(error_code, error, committed=committed, **details)
    return _invalid_response(response)


__all__ = [
    "CREATE_SPUR_GEAR_CONTRACT_VERSION",
