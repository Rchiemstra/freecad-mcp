"""Versioned, stdlib-only wire contract for ``close_document``."""
...
CLOSE_DOCUMENT_CONTRACT_VERSION: Literal[1] = 1

DocumentName = NewType("DocumentName", str)


class CloseDocumentReadDocument(Protocol):
...
class CloseDocumentDocument(CloseDocumentReadDocument, Protocol):
    """Narrow mutation surface available only to the apply callback."""

    def addObject(self, object_type: str, name: str) -> object: ...


class CloseDocumentCollaborators(Protocol):
    """Complete typed boundary between the handler and native bridge."""

    def validate_document_invariants(self, document: CloseDocumentReadDocument) -> object:
...
class CloseDocumentSuccess(TypedDict):
...
class CloseDocumentFailure(TypedDict):
...
class CloseDocumentUncertain(TypedDict):
...
CloseDocumentResult = CloseDocumentSuccess | CloseDocumentFailure | CloseDocumentUncertain
...
def make_close_document_success(document_name: DocumentName) -> CloseDocumentSuccess:
    """Construct a complete verified result."""

    return {
        "contract_version": CLOSE_DOCUMENT_CONTRACT_VERSION,
        "success": True,
        "ok": True,
        "outcome": "verified",
                "retry_safe": False,
...
def make_close_document_failure(
    error_code: str,
    error: str,
    *,
...
def make_close_document_uncertain(
    error_code: str,
    error: str,
    *,
    committed: bool | None,
...
def _invalid_response(response: dict[str, object]) -> CloseDocumentUncertain:
    ...
    code = (
        ...
        if committed
        else "CLOSE_DOCUMENT_ROLLBACK_UNCERTAIN"
        if rollback_failed
        else "INVALID_CLOSE_DOCUMENT_RESPONSE"
    )
    return make_close_document_uncertain(
        code,
        "close_document returned an invalid contract response; document state requires reconciliation",
        committed=True if committed else None,
        diagnostics={"response": response},
    )


def parse_close_document_response(raw_response: object) -> CloseDocumentResult:
    """Validate all three wire variants; unknown state always stays uncertain."""
    response = _response_object(raw_response)
    if response is None:
        return make_close_document_uncertain(
            "INVALID_RPC_RESPONSE",
            "close_document returned a non-object response",
            committed=None,
        )
    version = response.get("contract_version")
    details = _response_details(response)
    if type(version) is not int or version != CLOSE_DOCUMENT_CONTRACT_VERSION or details is None:
    ...
    if (
        ...
        return make_close_document_success(DocumentName(document_name))
    ...
    if (
        ...
        if _valid_uncertain(response):
            ...
            return make_close_document_uncertain(error_code, error, committed=committed, **details)
    return _invalid_response(response)


__all__ = [
    "CLOSE_DOCUMENT_CONTRACT_VERSION",
    "CloseDocumentCollaborators",
    "CloseDocumentDocument",
    "CloseDocumentFailure",
    "CloseDocumentReadDocument",
    "CloseDocumentRequest",
    "CloseDocumentResult",
    "CloseDocumentSuccess",
    "CloseDocumentUncertain",
    "DocumentName",
