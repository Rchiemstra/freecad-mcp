"""Versioned, stdlib-only wire contract for ``spreadsheet_create``."""
...
SPREADSHEET_CREATE_CONTRACT_VERSION: Literal[1] = 1

DocumentName = NewType("DocumentName", str)
SpreadsheetCreateName = NewType("SpreadsheetCreateName", str)


class SpreadsheetCreateObject(Protocol):
...
class SpreadsheetCreateReadDocument(Protocol):
    """Read-only document surface available after native recompute."""

    @property
    def Name(self) -> str: ...

    def getObject(self, name: str) -> SpreadsheetCreateObject | None: ...


class SpreadsheetCreateDocument(SpreadsheetCreateReadDocument, Protocol):
    """Narrow mutation surface available only to the apply callback."""

    def addObject(self, object_type: str, name: str) -> SpreadsheetCreateObject: ...


@runtime_checkable
class NativeSpreadsheetCreateDocument(SpreadsheetCreateDocument, Protocol):
...
class SpreadsheetCreateCollaborators(Protocol):
    """Complete typed boundary between the handler and native bridge."""

    def validate_document_invariants(self, document: SpreadsheetCreateReadDocument) -> object:
...
class SpreadsheetCreateRequest:
...
class SpreadsheetCreateSuccess(TypedDict):
...
class SpreadsheetCreateFailure(TypedDict):
...
class SpreadsheetCreateUncertain(TypedDict):
...
SpreadsheetCreateResult = SpreadsheetCreateSuccess | SpreadsheetCreateFailure | SpreadsheetCreateUncertain

_CORE_KEYS = frozenset(
    {
        "contract_version", "success", "ok", "outcome", "committed", "retry_safe", 'sheet', 'label', "error_code", "error", "native_status", "native_message", "rollback_succeeded", "rollback_failed", "diagnostics"
    }
)


def make_spreadsheet_create_success(sheet: str, label: str) -> SpreadsheetCreateSuccess:
    """Construct a complete committed result."""

    return {
        "contract_version": SPREADSHEET_CREATE_CONTRACT_VERSION,
        "success": True,
        "ok": True,
        "outcome": "committed",
        "committed": True,
...
def make_spreadsheet_create_failure(
    error_code: str,
    error: str,
    *,
    ...
    result: SpreadsheetCreateFailure = {
        "contract_version": SPREADSHEET_CREATE_CONTRACT_VERSION,
        "success": False,
        "ok": False,
        "outcome": "rejected",
...
def make_spreadsheet_create_uncertain(
    error_code: str,
    error: str,
    *,
    committed: bool | None,
    ...
    result: SpreadsheetCreateUncertain = {
        "contract_version": SPREADSHEET_CREATE_CONTRACT_VERSION,
        "success": False,
        "ok": False,
        "outcome": "uncertain",
...
def _invalid_response(response: dict[str, object]) -> SpreadsheetCreateUncertain:
    ...
    code = (
        ...
        if committed
        else "SPREADSHEET_CREATE_ROLLBACK_UNCERTAIN"
        if rollback_failed
        else "INVALID_SPREADSHEET_CREATE_RESPONSE"
    )
    return make_spreadsheet_create_uncertain(
        code,
        "spreadsheet_create returned an invalid contract response; document state requires reconciliation",
        committed=True if committed else None,
        diagnostics={"response": response},
    )


def parse_spreadsheet_create_response(raw_response: object) -> SpreadsheetCreateResult:
    """Validate all three wire variants; unknown state always stays uncertain."""
    response = _response_object(raw_response)
    if response is None:
        return make_spreadsheet_create_uncertain(
            "INVALID_RPC_RESPONSE",
            "spreadsheet_create returned a non-object response",
            committed=None,
        )
    version = response.get("contract_version")
    details = _response_details(response)
    if type(version) is not int or version != SPREADSHEET_CREATE_CONTRACT_VERSION or details is None:
    ...
    if (
        ...
        return make_spreadsheet_create_success(str(sheet), str(label))
    ...
    if (
        ...
        and error.strip()
        and 'sheet' not in response and 'label' not in response
    ):
        ...
        if _valid_uncertain(response):
            ...
            return make_spreadsheet_create_uncertain(error_code, error, committed=committed, **details)
    return _invalid_response(response)


__all__ = [
    "SPREADSHEET_CREATE_CONTRACT_VERSION",
