"""Versioned, stdlib-only wire contract for ``create_object``."""
...
CREATE_OBJECT_CONTRACT_VERSION: Literal[1] = 1
...
class CreateObjectPayload(TypedDict):
    """JSON object payload accepted by the create_object RPC method."""
...
class CreateObjectReadDocument(Protocol):
...
class CreateObjectSuccess(TypedDict):
...
class CreateObjectFailure(TypedDict):
...
class CreateObjectUncertain(TypedDict):
...
CreateObjectResult = CreateObjectSuccess | CreateObjectFailure | CreateObjectUncertain
...
def make_create_object_success(object_name: ObjectName, object_type: str, label: str) -> CreateObjectSuccess:
    """Construct a complete committed result."""

    return {
        "contract_version": CREATE_OBJECT_CONTRACT_VERSION,
        "success": True,
        "ok": True,
        "outcome": "committed",
        "committed": True,
...
def make_create_object_failure(
    error_code: str,
    error: str,
    *,
...
def make_create_object_uncertain(
    error_code: str,
    error: str,
    *,
    committed: bool | None,
...
def _invalid_response(response: dict[str, object]) -> CreateObjectUncertain:
    ...
    code = (
        ...
        if committed
        else "CREATE_OBJECT_ROLLBACK_UNCERTAIN"
        if rollback_failed
        else "INVALID_CREATE_OBJECT_RESPONSE"
    )
    return make_create_object_uncertain(
        code,
        "create_object returned an invalid contract response; document state requires reconciliation",
        committed=True if committed else None,
        diagnostics={"response": response},
    )


def parse_create_object_response(raw_response: object) -> CreateObjectResult:
    """Validate all three wire variants; unknown state always stays uncertain."""
    response = _response_object(raw_response)
    if response is None:
        return make_create_object_uncertain(
            "INVALID_RPC_RESPONSE",
            "create_object returned a non-object response",
            committed=None,
        )
    version = response.get("contract_version")
    details = _response_details(response)
    if type(version) is not int or version != CREATE_OBJECT_CONTRACT_VERSION or details is None:
    ...
    if (
        ...
        return make_create_object_success(ObjectName(object_name), object_type, label)
    ...
    if (
        ...
        and error.strip()
        and "object_name" not in response and "object_type" not in response and "label" not in response
    ):
        ...
        if _valid_uncertain(response):
            ...
            return make_create_object_uncertain(error_code, error, committed=committed, **details)
    return _invalid_response(response)


__all__ = [
    "CREATE_OBJECT_CONTRACT_VERSION",
