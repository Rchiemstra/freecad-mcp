"""Versioned, stdlib-only wire contract for ``capture_state``."""
...
CAPTURE_STATE_CONTRACT_VERSION: Literal[1] = 1

DocumentName = NewType("DocumentName", str)
CaptureStateName = NewType("CaptureStateName", str)


class CaptureStateObject(Protocol):
...
class CaptureStateReadDocument(Protocol):
    """Read-only document surface available after native recompute."""

    @property
    def Name(self) -> str: ...

    def getObject(self, name: str) -> CaptureStateObject | None: ...


class CaptureStateDocument(CaptureStateReadDocument, Protocol):
    """Narrow mutation surface available only to the apply callback."""

    def addObject(self, object_type: str, name: str) -> CaptureStateObject: ...


@runtime_checkable
class NativeCaptureStateDocument(CaptureStateDocument, Protocol):
...
class CaptureStateCollaborators(Protocol):
...
class CaptureStateUncertain(TypedDict):
...
CaptureStateResult = CaptureStateSuccess | CaptureStateFailure | CaptureStateUncertain
...
def make_capture_state_failure(
    error_code: str,
    error: str,
    *,
...
def make_capture_state_uncertain(
    error_code: str,
    error: str,
    *,
    committed: bool | None,
...
def _invalid_response(response: dict[str, object]) -> CaptureStateUncertain:
    ...
    code = (
        ...
        if committed
        else "CAPTURE_STATE_ROLLBACK_UNCERTAIN"
        if rollback_failed
        else "INVALID_CAPTURE_STATE_RESPONSE"
    )
    return make_capture_state_uncertain(
        code,
        "capture_state returned an invalid contract response; document state requires reconciliation",
        committed=True if committed else None,
        diagnostics={"response": response},
    )


def parse_capture_state_response(raw_response: object) -> CaptureStateResult:
    """Validate all three wire variants; unknown state always stays uncertain."""
    response = _response_object(raw_response)
    if response is None:
        return make_capture_state_uncertain(
            "INVALID_RPC_RESPONSE",
            "capture_state returned a non-object response",
            committed=None,
        )
    version = response.get("contract_version")
    details = _response_details(response)
    if type(version) is not int or version != CAPTURE_STATE_CONTRACT_VERSION or details is None:
    ...
    if (
        ...
        if _valid_uncertain(response):
            ...
            return make_capture_state_uncertain(error_code, error, committed=committed, **details)
    return _invalid_response(response)


__all__ = [
    "CAPTURE_STATE_CONTRACT_VERSION",
