"""Versioned, stdlib-only wire contract for ``create_assembly_joint``."""
...
CREATE_ASSEMBLY_JOINT_CONTRACT_VERSION: Literal[1] = 1
...
class CreateAssemblyJointCollaborators(Protocol):
...
class CreateAssemblyJointRequest:
...
class CreateAssemblyJointSuccess(TypedDict):
...
class CreateAssemblyJointFailure(TypedDict):
...
class CreateAssemblyJointUncertain(TypedDict):
...
CreateAssemblyJointResult = CreateAssemblyJointSuccess | CreateAssemblyJointFailure | CreateAssemblyJointUncertain
...
def make_create_assembly_joint_success(joint: str, label: str, joint_type: str, assembly: str) -> CreateAssemblyJointSuccess:
    """Construct a complete committed result."""

    return {
        "contract_version": CREATE_ASSEMBLY_JOINT_CONTRACT_VERSION,
        "success": True,
        "ok": True,
        "outcome": "committed",
        "committed": True,
...
def make_create_assembly_joint_failure(
    error_code: str,
    error: str,
    *,
    ...
    result: CreateAssemblyJointFailure = {
        "contract_version": CREATE_ASSEMBLY_JOINT_CONTRACT_VERSION,
        "success": False,
        "ok": False,
        "outcome": "rejected",
...
def make_create_assembly_joint_uncertain(
    error_code: str,
    error: str,
    *,
    committed: bool | None,
    ...
    rollback_failed: bool | None = None,
    diagnostics: dict[str, object] | None = None,
) -> CreateAssemblyJointUncertain:
    """Construct a result that explicitly forbids automatic replay."""

    result: CreateAssemblyJointUncertain = {
        "contract_version": CREATE_ASSEMBLY_JOINT_CONTRACT_VERSION,
        "success": False,
        "ok": False,
        "outcome": "uncertain",
...
def _invalid_response(response: dict[str, object]) -> CreateAssemblyJointUncertain:
    ...
    code = (
        ...
        if committed
        else "CREATE_ASSEMBLY_JOINT_ROLLBACK_UNCERTAIN"
        if rollback_failed
        else "INVALID_CREATE_ASSEMBLY_JOINT_RESPONSE"
    )
    return make_create_assembly_joint_uncertain(
        code,
        "create_assembly_joint returned an invalid contract response; document state requires reconciliation",
        committed=True if committed else None,
        diagnostics={"response": response},
    )


def parse_create_assembly_joint_response(raw_response: object) -> CreateAssemblyJointResult:
    """Validate all three wire variants; unknown state always stays uncertain."""
    response = _response_object(raw_response)
    if response is None:
        return make_create_assembly_joint_uncertain(
            "INVALID_RPC_RESPONSE",
            "create_assembly_joint returned a non-object response",
            committed=None,
        )
    version = response.get("contract_version")
    details = _response_details(response)
    if type(version) is not int or version != CREATE_ASSEMBLY_JOINT_CONTRACT_VERSION or details is None:
    ...
    if (
        ...
        and isinstance(joint, str) and joint.strip() and isinstance(label, str) and isinstance(joint_type, str) and isinstance(assembly, str) and assembly.strip()
    ):
        return make_create_assembly_joint_success(joint, label, joint_type, assembly)

    error_code = response.get("error_code")
    error = response.get("error")
    success_keys = {'joint', 'label', 'joint_type', 'assembly'}
    if (
        ...
        if _valid_rejection(response):
            return make_create_assembly_joint_failure(
                error_code,
                error,
        ...
        if _valid_uncertain(response):
            ...
            return make_create_assembly_joint_uncertain(error_code, error, committed=committed, **details)
    return _invalid_response(response)


__all__ = [
    "CREATE_ASSEMBLY_JOINT_CONTRACT_VERSION",
