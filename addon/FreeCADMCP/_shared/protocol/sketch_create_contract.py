"""Versioned, stdlib-only wire contract for ``sketch_create``."""
...
SKETCH_CREATE_CONTRACT_VERSION: Literal[1] = 1

DocumentName = NewType("DocumentName", str)
SketchName = NewType("SketchName", str)


class SketchCreateObject(Protocol):
...
class SketchCreateReadDocument(Protocol):
    """Read-only document surface available after native recompute."""

    @property
    def Name(self) -> str: ...

    def getObject(self, name: str) -> SketchCreateObject | None: ...


class SketchCreateDocument(SketchCreateReadDocument, Protocol):
    """Narrow mutation surface available only to the apply callback."""

    def addObject(self, object_type: str, name: str) -> SketchCreateObject: ...


@runtime_checkable
class NativeSketchCreateDocument(SketchCreateDocument, Protocol):
...
class SketchCreateCollaborators(Protocol):
...
class SketchCreateSuccess(TypedDict):
...
class SketchCreateFailure(TypedDict):
...
class SketchCreateUncertain(TypedDict):
...
SketchCreateResult = SketchCreateSuccess | SketchCreateFailure | SketchCreateUncertain
...
def make_sketch_create_success(sketch: SketchName, label: str) -> SketchCreateSuccess:
    """Construct a complete committed result using the actual assigned identity."""

    return {
        "contract_version": SKETCH_CREATE_CONTRACT_VERSION,
        "success": True,
        "ok": True,
        "outcome": "committed",
        "committed": True,
...
def make_sketch_create_failure(
    error_code: str,
    error: str,
    *,
...
def make_sketch_create_uncertain(
    error_code: str,
    error: str,
    *,
    committed: bool | None,
...
def _invalid_response(response: dict[str, object]) -> SketchCreateUncertain:
    ...
    code = (
        ...
        if committed
        else "SKETCH_CREATE_ROLLBACK_UNCERTAIN"
        if rollback_failed
        else "INVALID_SKETCH_CREATE_RESPONSE"
    )
    return make_sketch_create_uncertain(
        code,
        "sketch_create returned an invalid contract response; document state requires reconciliation",
        committed=True if committed else None,
        diagnostics={"response": response},
    )


def parse_sketch_create_response(raw_response: object) -> SketchCreateResult:
    """Validate all three wire variants; unknown state always stays uncertain."""
    response = _response_object(raw_response)
    if response is None:
        return make_sketch_create_uncertain(
            "INVALID_RPC_RESPONSE",
            "sketch_create returned a non-object response",
            committed=None,
        )
    version = response.get("contract_version")
    details = _response_details(response)
    if type(version) is not int or version != SKETCH_CREATE_CONTRACT_VERSION or details is None:
    ...
    if (
        ...
        return make_sketch_create_success(SketchName(sketch), label)
    ...
    if (
        ...
        and error.strip()
        and "sketch" not in response and "label" not in response
    ):
        if _valid_rejection(response):
            return make_sketch_create_failure(
                error_code,
                error,
        ...
        if _valid_uncertain(response):
            ...
            return make_sketch_create_uncertain(error_code, error, committed=committed, **details)
    return _invalid_response(response)


__all__ = [
    "SKETCH_CREATE_CONTRACT_VERSION",
