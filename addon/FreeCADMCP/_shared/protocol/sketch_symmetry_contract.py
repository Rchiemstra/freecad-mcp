"""Versioned, stdlib-only wire contract for ``sketch_symmetry``."""
...
SKETCH_SYMMETRY_CONTRACT_VERSION: Literal[1] = 1
...
class SketchSymmetryCollaborators(Protocol):
    @property
...
class SketchSymmetrySuccess(TypedDict):
...
class SketchSymmetryFailure(TypedDict):
    contract_version: Literal[1]
...
class SketchSymmetryUncertain(TypedDict):
    contract_version: Literal[1]
    success: Literal[False]
    ok: Literal[False]
...
SketchSymmetryResult = SketchSymmetrySuccess | SketchSymmetryFailure | SketchSymmetryUncertain
...
def make_sketch_symmetry_success(sketch: SketchName) -> SketchSymmetrySuccess:
    return {
        "contract_version": SKETCH_SYMMETRY_CONTRACT_VERSION,
        "success": True,
        "ok": True,
        "outcome": "committed",
        "committed": True,
...
def make_sketch_symmetry_failure(
    error_code: str,
    error: str,
    *,
...
def make_sketch_symmetry_uncertain(
    error_code: str,
    error: str,
    *,
    committed: bool | None,
    ...
    rollback_failed: bool | None = None,
    diagnostics: dict[str, object] | None = None,
) -> SketchSymmetryUncertain:
    result: SketchSymmetryUncertain = {
        "contract_version": SKETCH_SYMMETRY_CONTRACT_VERSION,
        "success": False,
        "ok": False,
        "outcome": "uncertain",
...
def _invalid_response(response: dict[str, object]) -> SketchSymmetryUncertain:
    ...
    code = (
        ...
        if committed
        else "SKETCH_SYMMETRY_ROLLBACK_UNCERTAIN"
        if rollback_failed
        else "INVALID_SKETCH_SYMMETRY_RESPONSE"
    )
    return make_sketch_symmetry_uncertain(
        code,
        "sketch_symmetry returned an invalid contract response; document state requires reconciliation",
        committed=True if committed else None,
        diagnostics={"response": response},
    )


def parse_sketch_symmetry_response(raw_response: object) -> SketchSymmetryResult:
    response = _response_object(raw_response)
    if response is None:
        return make_sketch_symmetry_uncertain(
            "INVALID_RPC_RESPONSE",
            "sketch_symmetry returned a non-object response",
            committed=None,
        )
    version = response.get("contract_version")
    details = _response_details(response)
    if type(version) is not int or version != SKETCH_SYMMETRY_CONTRACT_VERSION or details is None:
    ...
    if _valid_success(response) and isinstance(sketch, str) and sketch.strip():
        return make_sketch_symmetry_success(SketchName(sketch))
    ...
    if (
        ...
        if _valid_rejection(response):
            return make_sketch_symmetry_failure(
                error_code,
                error,
        ...
        if _valid_uncertain(response):
            ...
            return make_sketch_symmetry_uncertain(error_code, error, committed=committed, **details)
    return _invalid_response(response)


__all__ = [
    "SKETCH_SYMMETRY_CONTRACT_VERSION",
