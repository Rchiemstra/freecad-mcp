"""Versioned, stdlib-only wire contract for ``sketch_constrain_perpendicular``."""
...
SKETCH_CONSTRAIN_PERPENDICULAR_CONTRACT_VERSION: Literal[1] = 1
...
class SketchConstrainPerpendicularCollaborators(Protocol):
    @property
...
class SketchConstrainPerpendicularRequest:
...
class SketchConstrainPerpendicularSuccess(TypedDict):
...
class SketchConstrainPerpendicularFailure(TypedDict):
    contract_version: Literal[1]
...
class SketchConstrainPerpendicularUncertain(TypedDict):
    contract_version: Literal[1]
    success: Literal[False]
    ok: Literal[False]
...
SketchConstrainPerpendicularResult = SketchConstrainPerpendicularSuccess | SketchConstrainPerpendicularFailure | SketchConstrainPerpendicularUncertain
...
def make_sketch_constrain_perpendicular_success(sketch: SketchName, constraint_index: int) -> SketchConstrainPerpendicularSuccess:
    return {
        "contract_version": SKETCH_CONSTRAIN_PERPENDICULAR_CONTRACT_VERSION,
        "success": True,
        "ok": True,
        "outcome": "committed",
        "committed": True,
...
def make_sketch_constrain_perpendicular_failure(
    error_code: str,
    error: str,
    *,
    ...
    result: SketchConstrainPerpendicularFailure = {
        "contract_version": SKETCH_CONSTRAIN_PERPENDICULAR_CONTRACT_VERSION,
        "success": False,
        "ok": False,
        "outcome": "rejected",
...
def make_sketch_constrain_perpendicular_uncertain(
    error_code: str,
    error: str,
    *,
    committed: bool | None,
    ...
    result: SketchConstrainPerpendicularUncertain = {
        "contract_version": SKETCH_CONSTRAIN_PERPENDICULAR_CONTRACT_VERSION,
        "success": False,
        "ok": False,
        "outcome": "uncertain",
...
def _invalid_response(response: dict[str, object]) -> SketchConstrainPerpendicularUncertain:
    ...
    code = (
        ...
        if committed
        else "SKETCH_CONSTRAIN_PERPENDICULAR_ROLLBACK_UNCERTAIN"
        if rollback_failed
        else "INVALID_SKETCH_CONSTRAIN_PERPENDICULAR_RESPONSE"
    )
    return make_sketch_constrain_perpendicular_uncertain(
        code,
        "sketch_constrain_perpendicular returned an invalid contract response; document state requires reconciliation",
        committed=True if committed else None,
        diagnostics={"response": response},
    )


def parse_sketch_constrain_perpendicular_response(raw_response: object) -> SketchConstrainPerpendicularResult:
    response = _response_object(raw_response)
    if response is None:
        return make_sketch_constrain_perpendicular_uncertain(
            "INVALID_RPC_RESPONSE",
            "sketch_constrain_perpendicular returned a non-object response",
            committed=None,
        )
    version = response.get("contract_version")
    details = _response_details(response)
    if type(version) is not int or version != SKETCH_CONSTRAIN_PERPENDICULAR_CONTRACT_VERSION or details is None:
    ...
    if (
        ...
        return make_sketch_constrain_perpendicular_success(SketchName(sketch), constraint_index)
    ...
    if (
        ...
        if _valid_rejection(response):
            return make_sketch_constrain_perpendicular_failure(
                error_code,
                error,
        ...
        if _valid_uncertain(response):
            ...
            return make_sketch_constrain_perpendicular_uncertain(error_code, error, committed=committed, **details)
    return _invalid_response(response)


__all__ = [
    "SKETCH_CONSTRAIN_PERPENDICULAR_CONTRACT_VERSION",
    "SketchConstrainPerpendicularCollaborators",
    "SketchConstrainPerpendicularFailure",
    "SketchConstrainPerpendicularRequest",
    "SketchConstrainPerpendicularResult",
    "SketchConstrainPerpendicularSuccess",
    "SketchConstrainPerpendicularUncertain",
    "DocumentName",
    ...
    "parse_sketch_constrain_perpendicular_response",
]
