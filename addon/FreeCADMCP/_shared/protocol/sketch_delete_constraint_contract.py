"""Versioned, stdlib-only wire contract for ``sketch_delete_constraint``."""
...
SKETCH_DELETE_CONSTRAINT_CONTRACT_VERSION: Literal[1] = 1

DocumentName = NewType("DocumentName", str)
SketchName = NewType("SketchName", str)


class SketchDeleteConstraintObject(Protocol):
...
class SketchDeleteConstraintReadDocument(Protocol):
    """Read-only document surface available after native recompute."""

    @property
    def Name(self) -> str: ...

    def getObject(self, name: str) -> SketchDeleteConstraintObject | None: ...


class SketchDeleteConstraintDocument(SketchDeleteConstraintReadDocument, Protocol):
    """Narrow mutation surface available only to the apply callback."""

    def addObject(self, object_type: str, name: str) -> SketchDeleteConstraintObject: ...


@runtime_checkable
class NativeSketchDeleteConstraintDocument(SketchDeleteConstraintDocument, Protocol):
...
class SketchDeleteConstraintCollaborators(Protocol):
    ...
    def validate_document_invariants(self, document: SketchDeleteConstraintReadDocument) -> object:
...
class SketchDeleteConstraintRequest:
...
class SketchDeleteConstraintSuccess(TypedDict):
...
class SketchDeleteConstraintFailure(TypedDict):
...
class SketchDeleteConstraintUncertain(TypedDict):
...
SketchDeleteConstraintResult = SketchDeleteConstraintSuccess | SketchDeleteConstraintFailure | SketchDeleteConstraintUncertain
...
def make_sketch_delete_constraint_success(sketch: SketchName, deleted_count: int) -> SketchDeleteConstraintSuccess:
    """Construct a complete committed result using the actual assigned identity."""

    return {
        "contract_version": SKETCH_DELETE_CONSTRAINT_CONTRACT_VERSION,
        "success": True,
        "ok": True,
        "outcome": "committed",
        "committed": True,
...
def make_sketch_delete_constraint_failure(
    error_code: str,
    error: str,
    *,
    ...
    result: SketchDeleteConstraintFailure = {
        "contract_version": SKETCH_DELETE_CONSTRAINT_CONTRACT_VERSION,
        "success": False,
        "ok": False,
        "outcome": "rejected",
...
def make_sketch_delete_constraint_uncertain(
    error_code: str,
    error: str,
    *,
    committed: bool | None,
    ...
    result: SketchDeleteConstraintUncertain = {
        "contract_version": SKETCH_DELETE_CONSTRAINT_CONTRACT_VERSION,
        "success": False,
        "ok": False,
        "outcome": "uncertain",
...
def _invalid_response(response: dict[str, object]) -> SketchDeleteConstraintUncertain:
    ...
    code = (
        ...
        if committed
        else "SKETCH_DELETE_CONSTRAINT_ROLLBACK_UNCERTAIN"
        if rollback_failed
        else "INVALID_SKETCH_DELETE_CONSTRAINT_RESPONSE"
    )
    return make_sketch_delete_constraint_uncertain(
        code,
        "sketch_delete_constraint returned an invalid contract response; document state requires reconciliation",
        committed=True if committed else None,
        diagnostics={"response": response},
    )


def parse_sketch_delete_constraint_response(raw_response: object) -> SketchDeleteConstraintResult:
    """Validate all three wire variants; unknown state always stays uncertain."""
    response = _response_object(raw_response)
    if response is None:
        return make_sketch_delete_constraint_uncertain(
            "INVALID_RPC_RESPONSE",
            "sketch_delete_constraint returned a non-object response",
            committed=None,
        )
    version = response.get("contract_version")
    details = _response_details(response)
    if type(version) is not int or version != SKETCH_DELETE_CONSTRAINT_CONTRACT_VERSION or details is None:
    ...
    if (
        ...
        and sketch.strip()
        and isinstance(deleted_count, int) and not isinstance(deleted_count, bool) and deleted_count >= 0
    ):
        return make_sketch_delete_constraint_success(SketchName(sketch), deleted_count)
    ...
    if (
        ...
        and error.strip()
        and "sketch" not in response and "deleted_count" not in response
    ):
        if _valid_rejection(response):
            return make_sketch_delete_constraint_failure(
                error_code,
                error,
        ...
        if _valid_uncertain(response):
            ...
            return make_sketch_delete_constraint_uncertain(error_code, error, committed=committed, **details)
    return _invalid_response(response)


__all__ = [
    "SKETCH_DELETE_CONSTRAINT_CONTRACT_VERSION",
    "SketchDeleteConstraintCollaborators",
    "SketchDeleteConstraintFailure",
    "SketchDeleteConstraintRequest",
    "SketchDeleteConstraintResult",
    "SketchDeleteConstraintSuccess",
    "SketchDeleteConstraintUncertain",
