"""Versioned, stdlib-only wire contract for ``export_brep``."""
...
EXPORT_BREP_CONTRACT_VERSION: Literal[1] = 1
...
class ExportBrepCollaborators(Protocol):
...
class ExportBrepSuccess(TypedDict):
...
class ExportBrepFailure(TypedDict):
...
class ExportBrepUncertain(TypedDict):
...
ExportBrepResult = ExportBrepSuccess | ExportBrepFailure | ExportBrepUncertain
...
def make_export_brep_success(path: str, exported: bool, object: str) -> ExportBrepSuccess:
    """Construct a complete committed result."""

    return {
        "contract_version": EXPORT_BREP_CONTRACT_VERSION,
        "success": True,
        "ok": True,
        "outcome": "committed",
        "committed": True,
...
def make_export_brep_uncertain(
    error_code: str,
    error: str,
    *,
    committed: bool | None,
...
def _invalid_response(response: dict[str, object]) -> ExportBrepUncertain:
    ...
    code = (
        ...
        if committed
        else "EXPORT_BREP_ROLLBACK_UNCERTAIN"
        if rollback_failed
        else "INVALID_EXPORT_BREP_RESPONSE"
    )
    return make_export_brep_uncertain(
        code,
        "export_brep returned an invalid contract response; document state requires reconciliation",
        committed=True if committed else None,
        diagnostics={"response": response},
    )


def parse_export_brep_response(raw_response: object) -> ExportBrepResult:
    """Validate all three wire variants; unknown state always stays uncertain."""
    response = _response_object(raw_response)
    if response is None:
        return make_export_brep_uncertain(
            "INVALID_RPC_RESPONSE",
            "export_brep returned a non-object response",
            committed=None,
        )
    version = response.get("contract_version")
    details = _response_details(response)
    if type(version) is not int or version != EXPORT_BREP_CONTRACT_VERSION or details is None:
    ...
    if (
        ...
        and isinstance(path, str) and isinstance(exported, bool) and isinstance(object, str) and object.strip()
    ):
        return make_export_brep_success(path, exported, object)

    error_code = response.get("error_code")
    error = response.get("error")
    success_keys = {'path', 'exported', 'object'}
    if (
        ...
        if _valid_uncertain(response):
            ...
            return make_export_brep_uncertain(error_code, error, committed=committed, **details)
    return _invalid_response(response)


__all__ = [
    "EXPORT_BREP_CONTRACT_VERSION",
