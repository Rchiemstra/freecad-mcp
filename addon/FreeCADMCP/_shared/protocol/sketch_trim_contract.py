"""Versioned, stdlib-only wire contract for ``sketch_trim``."""
...
SKETCH_TRIM_CONTRACT_VERSION: Literal[1] = 1
...
class SketchTrimCollaborators(Protocol):
    @property
...
class SketchTrimSuccess(TypedDict):
...
class SketchTrimFailure(TypedDict):
    contract_version: Literal[1]
...
class SketchTrimUncertain(TypedDict):
    contract_version: Literal[1]
    success: Literal[False]
    ok: Literal[False]
...
SketchTrimResult = SketchTrimSuccess | SketchTrimFailure | SketchTrimUncertain
...
def make_sketch_trim_success(sketch: SketchName) -> SketchTrimSuccess:
    return {
        "contract_version": SKETCH_TRIM_CONTRACT_VERSION,
        "success": True,
        "ok": True,
        "outcome": "committed",
        "committed": True,
...
def make_sketch_trim_uncertain(
    error_code: str,
    error: str,
    *,
    committed: bool | None,
...
def _invalid_response(response: dict[str, object]) -> SketchTrimUncertain:
    ...
    code = (
        ...
        if committed
        else "SKETCH_TRIM_ROLLBACK_UNCERTAIN"
        if rollback_failed
        else "INVALID_SKETCH_TRIM_RESPONSE"
    )
    return make_sketch_trim_uncertain(
        code,
        "sketch_trim returned an invalid contract response; document state requires reconciliation",
        committed=True if committed else None,
        diagnostics={"response": response},
    )


def parse_sketch_trim_response(raw_response: object) -> SketchTrimResult:
    response = _response_object(raw_response)
    if response is None:
        return make_sketch_trim_uncertain(
            "INVALID_RPC_RESPONSE",
            "sketch_trim returned a non-object response",
            committed=None,
        )
    version = response.get("contract_version")
    details = _response_details(response)
    if type(version) is not int or version != SKETCH_TRIM_CONTRACT_VERSION or details is None:
    ...
    if _valid_success(response) and isinstance(sketch, str) and sketch.strip():
        return make_sketch_trim_success(SketchName(sketch))
    ...
    if (
        ...
        if _valid_rejection(response):
            return make_sketch_trim_failure(
                error_code,
                error,
        ...
        if _valid_uncertain(response):
            ...
            return make_sketch_trim_uncertain(error_code, error, committed=committed, **details)
    return _invalid_response(response)


__all__ = [
    "SKETCH_TRIM_CONTRACT_VERSION",
