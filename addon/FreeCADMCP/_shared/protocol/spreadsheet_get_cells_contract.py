"""Versioned, stdlib-only wire contract for ``spreadsheet_get_cells``."""
...
SPREADSHEET_GET_CELLS_CONTRACT_VERSION: Literal[1] = 1

DocumentName = NewType("DocumentName", str)
SpreadsheetGetCellsName = NewType("SpreadsheetGetCellsName", str)


class SpreadsheetGetCellsObject(Protocol):
...
class SpreadsheetGetCellsReadDocument(Protocol):
    """Read-only document surface available after native recompute."""

    @property
    def Name(self) -> str: ...

    def getObject(self, name: str) -> SpreadsheetGetCellsObject | None: ...


class SpreadsheetGetCellsDocument(SpreadsheetGetCellsReadDocument, Protocol):
    """Narrow mutation surface available only to the apply callback."""

    def addObject(self, object_type: str, name: str) -> SpreadsheetGetCellsObject: ...


@runtime_checkable
class NativeSpreadsheetGetCellsDocument(SpreadsheetGetCellsDocument, Protocol):
...
class SpreadsheetGetCellsCollaborators(Protocol):
...
class SpreadsheetGetCellsRequest:
...
class SpreadsheetGetCellsUncertain(TypedDict):
...
SpreadsheetGetCellsResult = SpreadsheetGetCellsSuccess | SpreadsheetGetCellsFailure | SpreadsheetGetCellsUncertain
...
def make_spreadsheet_get_cells_failure(
    error_code: str,
    error: str,
    *,
    ...
    result: SpreadsheetGetCellsFailure = {
        "contract_version": SPREADSHEET_GET_CELLS_CONTRACT_VERSION,
        "success": False,
        "ok": False,
        "outcome": "rejected",
...
def make_spreadsheet_get_cells_uncertain(
    error_code: str,
    error: str,
    *,
    committed: bool | None,
    ...
    rollback_failed: bool | None = None,
    diagnostics: dict[str, object] | None = None,
) -> SpreadsheetGetCellsUncertain:
    """Construct a result that explicitly forbids automatic replay."""

    result: SpreadsheetGetCellsUncertain = {
        "contract_version": SPREADSHEET_GET_CELLS_CONTRACT_VERSION,
        "success": False,
        "ok": False,
        "outcome": "uncertain",
...
def _invalid_response(response: dict[str, object]) -> SpreadsheetGetCellsUncertain:
    ...
    code = (
        ...
        if committed
        else "SPREADSHEET_GET_CELLS_ROLLBACK_UNCERTAIN"
        if rollback_failed
        else "INVALID_SPREADSHEET_GET_CELLS_RESPONSE"
    )
    return make_spreadsheet_get_cells_uncertain(
        code,
        "spreadsheet_get_cells returned an invalid contract response; document state requires reconciliation",
        committed=True if committed else None,
        diagnostics={"response": response},
    )


def parse_spreadsheet_get_cells_response(raw_response: object) -> SpreadsheetGetCellsResult:
    """Validate all three wire variants; unknown state always stays uncertain."""
    response = _response_object(raw_response)
    if response is None:
        return make_spreadsheet_get_cells_uncertain(
            "INVALID_RPC_RESPONSE",
            "spreadsheet_get_cells returned a non-object response",
            committed=None,
        )
    ...
    if (
        ...
        if _valid_uncertain(response):
            ...
            return make_spreadsheet_get_cells_uncertain(error_code, error, committed=committed, **details)
    return _invalid_response(response)


__all__ = [
    "SPREADSHEET_GET_CELLS_CONTRACT_VERSION",
