"""Versioned, stdlib-only wire contract for ``sketch_offset``."""
...
SKETCH_OFFSET_CONTRACT_VERSION: Literal[1] = 1
...
class SketchPart(Protocol):
    ...
    def Edge(self, geom: object) -> object: ...

    def Wire(self, shapes: list[object]) -> object: ...
...
class SketchOffsetCollaborators(Protocol):
    @property
...
class SketchOffsetSuccess(TypedDict):
...
class SketchOffsetFailure(TypedDict):
    contract_version: Literal[1]
...
class SketchOffsetUncertain(TypedDict):
    contract_version: Literal[1]
    success: Literal[False]
    ok: Literal[False]
...
SketchOffsetResult = SketchOffsetSuccess | SketchOffsetFailure | SketchOffsetUncertain
...
def make_sketch_offset_success(sketch: SketchName) -> SketchOffsetSuccess:
    return {
        "contract_version": SKETCH_OFFSET_CONTRACT_VERSION,
        "success": True,
        "ok": True,
        "outcome": "committed",
        "committed": True,
...
def make_sketch_offset_failure(
    error_code: str,
    error: str,
    *,
...
def make_sketch_offset_uncertain(
    error_code: str,
    error: str,
    *,
    committed: bool | None,
...
def _invalid_response(response: dict[str, object]) -> SketchOffsetUncertain:
    ...
    code = (
        ...
        if committed
        else "SKETCH_OFFSET_ROLLBACK_UNCERTAIN"
        if rollback_failed
        else "INVALID_SKETCH_OFFSET_RESPONSE"
    )
    return make_sketch_offset_uncertain(
        code,
        "sketch_offset returned an invalid contract response; document state requires reconciliation",
        committed=True if committed else None,
        diagnostics={"response": response},
    )


def parse_sketch_offset_response(raw_response: object) -> SketchOffsetResult:
    response = _response_object(raw_response)
    if response is None:
        return make_sketch_offset_uncertain(
            "INVALID_RPC_RESPONSE",
            "sketch_offset returned a non-object response",
            committed=None,
        )
    version = response.get("contract_version")
    details = _response_details(response)
    if type(version) is not int or version != SKETCH_OFFSET_CONTRACT_VERSION or details is None:
    ...
    if _valid_success(response) and isinstance(sketch, str) and sketch.strip():
        return make_sketch_offset_success(SketchName(sketch))
    ...
    if (
        ...
        if _valid_rejection(response):
            return make_sketch_offset_failure(
                error_code,
                error,
        ...
        if _valid_uncertain(response):
            ...
            return make_sketch_offset_uncertain(error_code, error, committed=committed, **details)
    return _invalid_response(response)


__all__ = [
    "SKETCH_OFFSET_CONTRACT_VERSION",
