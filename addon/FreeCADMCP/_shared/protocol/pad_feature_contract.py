"""Versioned, stdlib-only wire contract for ``pad_feature``."""
...
PAD_FEATURE_CONTRACT_VERSION: Literal[1] = 1

DocumentName = NewType("DocumentName", str)
PadName = NewType("PadName", str)


class PadFeatureObject(Protocol):
...
class PadFeatureReadDocument(Protocol):
    """Read-only document surface available after native recompute."""

    @property
    def Name(self) -> str: ...

    def getObject(self, name: str) -> PadFeatureObject | None: ...


class PadFeatureDocument(PadFeatureReadDocument, Protocol):
    """Narrow mutation surface available only to the apply callback."""

    def addObject(self, object_type: str, name: str) -> PadFeatureObject: ...


@runtime_checkable
class NativePadFeatureDocument(PadFeatureDocument, Protocol):
...
class PadFeatureCollaborators(Protocol):
...
class PadFeatureSuccess(TypedDict):
...
class PadFeatureFailure(TypedDict):
...
class PadFeatureUncertain(TypedDict):
...
PadFeatureResult = PadFeatureSuccess | PadFeatureFailure | PadFeatureUncertain
...
def make_pad_feature_success(pad: PadName, label: str) -> PadFeatureSuccess:
    """Construct a complete committed result using the actual assigned identity."""

    return {
        "contract_version": PAD_FEATURE_CONTRACT_VERSION,
        "success": True,
        "ok": True,
        "outcome": "committed",
        "committed": True,
...
def make_pad_feature_uncertain(
    error_code: str,
    error: str,
    *,
    committed: bool | None,
...
def _invalid_response(response: dict[str, object]) -> PadFeatureUncertain:
    ...
    code = (
        ...
        if committed
        else "PAD_FEATURE_ROLLBACK_UNCERTAIN"
        if rollback_failed
        else "INVALID_PAD_FEATURE_RESPONSE"
    )
    return make_pad_feature_uncertain(
        code,
        "pad_feature returned an invalid contract response; document state requires reconciliation",
        committed=True if committed else None,
        diagnostics={"response": response},
    )


def parse_pad_feature_response(raw_response: object) -> PadFeatureResult:
    """Validate all three wire variants; unknown state always stays uncertain."""
    response = _response_object(raw_response)
    if response is None:
        return make_pad_feature_uncertain(
            "INVALID_RPC_RESPONSE",
            "pad_feature returned a non-object response",
            committed=None,
        )
    version = response.get("contract_version")
    details = _response_details(response)
    if type(version) is not int or version != PAD_FEATURE_CONTRACT_VERSION or details is None:
    ...
    if (
        ...
        return make_pad_feature_success(PadName(pad), label)
    ...
    if (
        ...
        and error.strip()
        and "pad" not in response and "label" not in response
    ):
        ...
        if _valid_uncertain(response):
            ...
            return make_pad_feature_uncertain(error_code, error, committed=committed, **details)
    return _invalid_response(response)


__all__ = [
    "PAD_FEATURE_CONTRACT_VERSION",
