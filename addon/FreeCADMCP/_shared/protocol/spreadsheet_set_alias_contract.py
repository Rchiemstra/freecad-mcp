"""Versioned, stdlib-only wire contract for ``spreadsheet_set_alias``."""
...
SPREADSHEET_SET_ALIAS_CONTRACT_VERSION: Literal[1] = 1

DocumentName = NewType("DocumentName", str)
SpreadsheetSetAliasName = NewType("SpreadsheetSetAliasName", str)


class SpreadsheetSetAliasObject(Protocol):
...
class SpreadsheetSetAliasReadDocument(Protocol):
    """Read-only document surface available after native recompute."""

    @property
    def Name(self) -> str: ...

    def getObject(self, name: str) -> SpreadsheetSetAliasObject | None: ...


class SpreadsheetSetAliasDocument(SpreadsheetSetAliasReadDocument, Protocol):
    """Narrow mutation surface available only to the apply callback."""

    def addObject(self, object_type: str, name: str) -> SpreadsheetSetAliasObject: ...


@runtime_checkable
class NativeSpreadsheetSetAliasDocument(SpreadsheetSetAliasDocument, Protocol):
...
class SpreadsheetSetAliasCollaborators(Protocol):
...
class SpreadsheetSetAliasRequest:
...
class SpreadsheetSetAliasSuccess(TypedDict):
...
class SpreadsheetSetAliasFailure(TypedDict):
...
class SpreadsheetSetAliasUncertain(TypedDict):
...
SpreadsheetSetAliasResult = SpreadsheetSetAliasSuccess | SpreadsheetSetAliasFailure | SpreadsheetSetAliasUncertain

_CORE_KEYS = frozenset(
    {
        "contract_version", "success", "ok", "outcome", "committed", "retry_safe", 'sheet', 'address', 'alias', "error_code", "error", "native_status", "native_message", "rollback_succeeded", "rollback_failed", "diagnostics"
    }
)


def make_spreadsheet_set_alias_success(sheet: str, address: str, alias: str) -> SpreadsheetSetAliasSuccess:
    """Construct a complete committed result."""

    return {
        "contract_version": SPREADSHEET_SET_ALIAS_CONTRACT_VERSION,
        "success": True,
        "ok": True,
        "outcome": "committed",
        "committed": True,
...
def make_spreadsheet_set_alias_failure(
    error_code: str,
    error: str,
    *,
    ...
    result: SpreadsheetSetAliasFailure = {
        "contract_version": SPREADSHEET_SET_ALIAS_CONTRACT_VERSION,
        "success": False,
        "ok": False,
        "outcome": "rejected",
...
def make_spreadsheet_set_alias_uncertain(
    error_code: str,
    error: str,
    *,
    committed: bool | None,
    ...
    result: SpreadsheetSetAliasUncertain = {
        "contract_version": SPREADSHEET_SET_ALIAS_CONTRACT_VERSION,
        "success": False,
        "ok": False,
        "outcome": "uncertain",
...
def _invalid_response(response: dict[str, object]) -> SpreadsheetSetAliasUncertain:
    ...
    code = (
        ...
        if committed
        else "SPREADSHEET_SET_ALIAS_ROLLBACK_UNCERTAIN"
        if rollback_failed
        else "INVALID_SPREADSHEET_SET_ALIAS_RESPONSE"
    )
    return make_spreadsheet_set_alias_uncertain(
        code,
        "spreadsheet_set_alias returned an invalid contract response; document state requires reconciliation",
        committed=True if committed else None,
        diagnostics={"response": response},
    )


def parse_spreadsheet_set_alias_response(raw_response: object) -> SpreadsheetSetAliasResult:
    """Validate all three wire variants; unknown state always stays uncertain."""
    response = _response_object(raw_response)
    if response is None:
        return make_spreadsheet_set_alias_uncertain(
            "INVALID_RPC_RESPONSE",
            "spreadsheet_set_alias returned a non-object response",
            committed=None,
        )
    version = response.get("contract_version")
    details = _response_details(response)
    if type(version) is not int or version != SPREADSHEET_SET_ALIAS_CONTRACT_VERSION or details is None:
    ...
    if (
        ...
        return make_spreadsheet_set_alias_success(str(sheet), str(address), str(alias))
    ...
    if (
        ...
        and error.strip()
        and 'sheet' not in response and 'address' not in response and 'alias' not in response
    ):
        if _valid_rejection(response):
            return make_spreadsheet_set_alias_failure(
                error_code,
                error,
        ...
        if _valid_uncertain(response):
            ...
            return make_spreadsheet_set_alias_uncertain(error_code, error, committed=committed, **details)
    return _invalid_response(response)


__all__ = [
    "SPREADSHEET_SET_ALIAS_CONTRACT_VERSION",
