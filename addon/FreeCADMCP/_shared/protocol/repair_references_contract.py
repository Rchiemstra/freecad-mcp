"""Versioned, stdlib-only wire contract for ``repair_references``."""
...
REPAIR_REFERENCES_CONTRACT_VERSION: Literal[1] = 1

DocumentName = NewType("DocumentName", str)
ObjectName = NewType("ObjectName", str)


class RepairRefPayload(TypedDict):
    """One replacement link target."""

    object: str
    document: NotRequired[str]
    subelements: NotRequired[list[str]]


class RepairItemPayload(TypedDict):
    """One complete link-property replacement."""

    object: str
    property: str
    references: list[RepairRefPayload]


class RepairReferencesReadDocument(Protocol):
...
class RepairReferencesDocument(RepairReferencesReadDocument, Protocol):
    """Narrow mutation surface available only to the apply callback."""

    def addObject(self, object_type: str, name: str) -> object: ...


class RepairReferencesCollaborators(Protocol):
...
class RepairRef:
    """Resolved replacement target for one link entry."""
...
class RepairItem:
    ...
    object_name: ObjectName
    property_name: str
    references: tuple[RepairRef, ...]


@dataclass(frozen=True, slots=True, kw_only=True)
class RepairReferencesRequest:
...
class RepairReferencesSuccess(TypedDict):
...
class RepairReferencesUncertain(TypedDict):
...
RepairReferencesResult = RepairReferencesSuccess | RepairReferencesFailure | RepairReferencesUncertain
...
def make_repair_references_failure(
    ...
    *,
    ...
    result: RepairReferencesFailure = {
        "contract_version": REPAIR_REFERENCES_CONTRACT_VERSION,
        "success": False,
        "ok": False,
        "outcome": "rejected",
...
def make_repair_references_uncertain(
    error_code: str,
    error: str,
    *,
    committed: bool | None,
    ...
    result: RepairReferencesUncertain = {
        "contract_version": REPAIR_REFERENCES_CONTRACT_VERSION,
        "success": False,
        "ok": False,
        "outcome": "uncertain",
...
def _invalid_response(response: dict[str, object]) -> RepairReferencesUncertain:
    ...
    code = (
        ...
        if committed
        else "REPAIR_REFERENCES_ROLLBACK_UNCERTAIN"
        if rollback_failed
        else "INVALID_REPAIR_REFERENCES_RESPONSE"
    )
    return make_repair_references_uncertain(
        code,
        "repair_references returned an invalid contract response; document state requires reconciliation",
        committed=True if committed else None,
        diagnostics={"response": response},
    )


def parse_repair_references_response(raw_response: object) -> RepairReferencesResult:
    """Validate all three wire variants; unknown state always stays uncertain."""
    response = _response_object(raw_response)
    if response is None:
        return make_repair_references_uncertain(
            "INVALID_RPC_RESPONSE",
            "repair_references returned a non-object response",
            committed=None,
        )
    version = response.get("contract_version")
    details = _response_details(response)
    if type(version) is not int or version != REPAIR_REFERENCES_CONTRACT_VERSION or details is None:
        return _invalid_response(response)

    document_name = response.get('document_name')
    repaired_count = response.get('repaired_count')
    if (
    ...
    if (
        ...
        and error.strip()
        and "document_name" not in response and "repaired_count" not in response
    ):
        ...
        if _valid_uncertain(response):
            ...
            return make_repair_references_uncertain(error_code, error, committed=committed, **details)
    return _invalid_response(response)


__all__ = [
    "REPAIR_REFERENCES_CONTRACT_VERSION",
    "RepairReferencesCollaborators",
    "RepairReferencesDocument",
    "RepairReferencesFailure",
