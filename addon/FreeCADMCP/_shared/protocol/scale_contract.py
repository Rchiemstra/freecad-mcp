"""Versioned, stdlib-only wire contract for ``scale``."""
...
SCALE_CONTRACT_VERSION: Literal[1] = 1
...
class ScaleCollaborators(Protocol):
...
class ScaleUncertain(TypedDict):
...
ScaleResult = ScaleSuccess | ScaleFailure | ScaleUncertain
...
def make_scale_success(object: str, label: str) -> ScaleSuccess:
...
def _invalid_response(response: dict[str, object]) -> ScaleUncertain:
    ...
    code = (
        ...
        if committed
        else "SCALE_ROLLBACK_UNCERTAIN"
        if rollback_failed
        else "INVALID_SCALE_RESPONSE"
    )
    return make_scale_uncertain(
        code,
        "scale returned an invalid contract response; document state requires reconciliation",
        committed=True if committed else None,
        diagnostics={"response": response},
    )


def parse_scale_response(raw_response: object) -> ScaleResult:
    """Validate all three wire variants; unknown state always stays uncertain."""
    response = _response_object(raw_response)
    if response is None:
        return make_scale_uncertain(
            "INVALID_RPC_RESPONSE",
            "scale returned a non-object response",
            committed=None,
        )
    version = response.get("contract_version")
    details = _response_details(response)
    if type(version) is not int or version != SCALE_CONTRACT_VERSION or details is None:
    ...
    if (
        ...
        return make_scale_success(object, label)
    ...
    if (
        ...
        if _valid_uncertain(response):
            ...
            return make_scale_uncertain(error_code, error, committed=committed, **details)
    return _invalid_response(response)
