"""Versioned, stdlib-only wire contract for ``export_stl``."""
...
EXPORT_STL_CONTRACT_VERSION: Literal[1] = 1
...
class ExportStlCollaborators(Protocol):
...
class ExportStlSuccess(TypedDict):
...
class ExportStlFailure(TypedDict):
...
class ExportStlUncertain(TypedDict):
...
ExportStlResult = ExportStlSuccess | ExportStlFailure | ExportStlUncertain
...
def make_export_stl_success(path: str, exported: int, faces: int) -> ExportStlSuccess:
    """Construct a complete committed result."""

    return {
        "contract_version": EXPORT_STL_CONTRACT_VERSION,
        "success": True,
        "ok": True,
        "outcome": "committed",
        "committed": True,
...
def _invalid_response(response: dict[str, object]) -> ExportStlUncertain:
    ...
    code = (
        ...
        if committed
        else "EXPORT_STL_ROLLBACK_UNCERTAIN"
        if rollback_failed
        else "INVALID_EXPORT_STL_RESPONSE"
    )
    return make_export_stl_uncertain(
        code,
        "export_stl returned an invalid contract response; document state requires reconciliation",
        committed=True if committed else None,
        diagnostics={"response": response},
    )


def parse_export_stl_response(raw_response: object) -> ExportStlResult:
    """Validate all three wire variants; unknown state always stays uncertain."""
    response = _response_object(raw_response)
    if response is None:
        return make_export_stl_uncertain(
            "INVALID_RPC_RESPONSE",
            "export_stl returned a non-object response",
            committed=None,
        )
    version = response.get("contract_version")
    details = _response_details(response)
    if type(version) is not int or version != EXPORT_STL_CONTRACT_VERSION or details is None:
    ...
    if (
        ...
        and isinstance(path, str) and isinstance(exported, int) and not isinstance(exported, bool) and isinstance(faces, int) and not isinstance(faces, bool)
    ):
        return make_export_stl_success(path, exported, faces)

    error_code = response.get("error_code")
    error = response.get("error")
    success_keys = {'path', 'exported', 'faces'}
    if (
        ...
        if _valid_uncertain(response):
            ...
            return make_export_stl_uncertain(error_code, error, committed=committed, **details)
    return _invalid_response(response)


__all__ = [
    "EXPORT_STL_CONTRACT_VERSION",
