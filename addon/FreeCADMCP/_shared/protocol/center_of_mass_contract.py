"""Versioned, stdlib-only wire contract for ``center_of_mass``."""
...
CENTER_OF_MASS_CONTRACT_VERSION: Literal[1] = 1
...
class CenterOfMassCollaborators(Protocol):
...
class CenterOfMassSuccess(TypedDict):
...
class CenterOfMassFailure(TypedDict):
...
class CenterOfMassUncertain(TypedDict):
...
CenterOfMassResult = CenterOfMassSuccess | CenterOfMassFailure | CenterOfMassUncertain
...
def make_center_of_mass_failure(
    error_code: str,
    error: str,
    *,
...
def make_center_of_mass_uncertain(
    error_code: str,
    error: str,
    *,
    committed: bool | None,
...
def _invalid_response(response: dict[str, object]) -> CenterOfMassUncertain:
    ...
    code = (
        ...
        if committed
        else "CENTER_OF_MASS_ROLLBACK_UNCERTAIN"
        if rollback_failed
        else "INVALID_CENTER_OF_MASS_RESPONSE"
    )
    return make_center_of_mass_uncertain(
        code,
        "center_of_mass returned an invalid contract response; document state requires reconciliation",
        committed=True if committed else None,
        diagnostics={"response": response},
    )


def parse_center_of_mass_response(raw_response: object) -> CenterOfMassResult:
    """Validate all three wire variants; unknown state always stays uncertain."""
    response = _response_object(raw_response)
    if response is None:
        return make_center_of_mass_uncertain(
            "INVALID_RPC_RESPONSE",
            "center_of_mass returned a non-object response",
            committed=None,
        )
    version = response.get("contract_version")
    details = _response_details(response)
    if type(version) is not int or version != CENTER_OF_MASS_CONTRACT_VERSION or details is None:
    ...
    if (
        ...
        and isinstance(object, str) and object.strip() and isinstance(x, (int, float)) and not isinstance(x, bool) and isinstance(y, (int, float)) and not isinstance(y, bool) and isinstance(z, (int, float)) and not isinstance(z, bool) and isinstance(unit, str) and isinstance(method, str) and isinstance(frame, str)
    ):
        return make_center_of_mass_success(object, float(x), float(y), float(z), unit, method, frame)

    error_code = response.get("error_code")
    error = response.get("error")
    success_keys = {'object', 'x', 'y', 'z', 'unit', 'method', 'frame'}
    if (
        ...
        if _valid_rejection(response):
            return make_center_of_mass_failure(
                error_code,
                error,
        ...
        if _valid_uncertain(response):
            ...
            return make_center_of_mass_uncertain(error_code, error, committed=committed, **details)
    return _invalid_response(response)


__all__ = [
    "CENTER_OF_MASS_CONTRACT_VERSION",
