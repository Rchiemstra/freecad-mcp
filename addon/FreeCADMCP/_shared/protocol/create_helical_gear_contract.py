"""Versioned, stdlib-only wire contract for ``create_helical_gear``."""
...
CREATE_HELICAL_GEAR_CONTRACT_VERSION: Literal[1] = 1
...
class CreateHelicalGearCollaborators(Protocol):
...
class CreateHelicalGearRequest:
...
class CreateHelicalGearSuccess(TypedDict):
...
class CreateHelicalGearFailure(TypedDict):
...
class CreateHelicalGearUncertain(TypedDict):
...
CreateHelicalGearResult = CreateHelicalGearSuccess | CreateHelicalGearFailure | CreateHelicalGearUncertain
...
def make_create_helical_gear_success(body: str, sketch: str, feature: str, teeth: int, module: float) -> CreateHelicalGearSuccess:
    """Construct a complete committed result."""

    return {
        "contract_version": CREATE_HELICAL_GEAR_CONTRACT_VERSION,
        "success": True,
        "ok": True,
        "outcome": "committed",
        "committed": True,
...
def make_create_helical_gear_failure(
    error_code: str,
    error: str,
    *,
    ...
    result: CreateHelicalGearFailure = {
        "contract_version": CREATE_HELICAL_GEAR_CONTRACT_VERSION,
        "success": False,
        "ok": False,
        "outcome": "rejected",
...
def make_create_helical_gear_uncertain(
    error_code: str,
    error: str,
    *,
    committed: bool | None,
    ...
    rollback_failed: bool | None = None,
    diagnostics: dict[str, object] | None = None,
) -> CreateHelicalGearUncertain:
    """Construct a result that explicitly forbids automatic replay."""

    result: CreateHelicalGearUncertain = {
        "contract_version": CREATE_HELICAL_GEAR_CONTRACT_VERSION,
        "success": False,
        "ok": False,
        "outcome": "uncertain",
...
def _invalid_response(response: dict[str, object]) -> CreateHelicalGearUncertain:
    ...
    code = (
        ...
        if committed
        else "CREATE_HELICAL_GEAR_ROLLBACK_UNCERTAIN"
        if rollback_failed
        else "INVALID_CREATE_HELICAL_GEAR_RESPONSE"
    )
    return make_create_helical_gear_uncertain(
        code,
        "create_helical_gear returned an invalid contract response; document state requires reconciliation",
        committed=True if committed else None,
        diagnostics={"response": response},
    )


def parse_create_helical_gear_response(raw_response: object) -> CreateHelicalGearResult:
    """Validate all three wire variants; unknown state always stays uncertain."""
    response = _response_object(raw_response)
    if response is None:
        return make_create_helical_gear_uncertain(
            "INVALID_RPC_RESPONSE",
            "create_helical_gear returned a non-object response",
            committed=None,
        )
    version = response.get("contract_version")
    details = _response_details(response)
    if type(version) is not int or version != CREATE_HELICAL_GEAR_CONTRACT_VERSION or details is None:
    ...
    if (
        ...
        return make_create_helical_gear_success(body, sketch, feature, teeth, float(module))
    ...
    if (
        ...
        if _valid_rejection(response):
            return make_create_helical_gear_failure(
                error_code,
                error,
        ...
        if _valid_uncertain(response):
            ...
            return make_create_helical_gear_uncertain(error_code, error, committed=committed, **details)
    return _invalid_response(response)


__all__ = [
    "CREATE_HELICAL_GEAR_CONTRACT_VERSION",
