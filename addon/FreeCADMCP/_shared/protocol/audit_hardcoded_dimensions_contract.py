"""Versioned, stdlib-only wire contract for ``audit_hardcoded_dimensions``."""
...
AUDIT_HARDCODED_DIMENSIONS_CONTRACT_VERSION: Literal[1] = 1
...
class AuditHardcodedDimensionsCollaborators(Protocol):
...
class AuditHardcodedDimensionsRequest:
    """Validated internal request."""

    doc_name: DocumentName
    body_name: ObjectName
    flag_aliases: bool


class AuditHardcodedDimensionsSuccess(TypedDict):
...
class AuditHardcodedDimensionsFailure(TypedDict):
...
class AuditHardcodedDimensionsUncertain(TypedDict):
...
AuditHardcodedDimensionsResult = AuditHardcodedDimensionsSuccess | AuditHardcodedDimensionsFailure | AuditHardcodedDimensionsUncertain
...
def make_audit_hardcoded_dimensions_success(
    clean: bool, body: str, count: int, findings: object,
) -> AuditHardcodedDimensionsSuccess:
    """Construct a complete observed result."""

    return {
        "contract_version": AUDIT_HARDCODED_DIMENSIONS_CONTRACT_VERSION,
        "success": True,
        "ok": True,
        "outcome": "observed",
        "retry_safe": False,
...
def make_audit_hardcoded_dimensions_failure(
    error_code: str,
    error: str,
    *,
    ...
    result: AuditHardcodedDimensionsFailure = {
        "contract_version": AUDIT_HARDCODED_DIMENSIONS_CONTRACT_VERSION,
        "success": False,
        "ok": False,
        "outcome": "rejected",
...
def make_audit_hardcoded_dimensions_uncertain(
    error_code: str,
    error: str,
    *,
    committed: bool | None,
    ...
    rollback_failed: bool | None = None,
    diagnostics: dict[str, object] | None = None,
) -> AuditHardcodedDimensionsUncertain:
    """Construct a result that explicitly forbids automatic replay."""

    result: AuditHardcodedDimensionsUncertain = {
        "contract_version": AUDIT_HARDCODED_DIMENSIONS_CONTRACT_VERSION,
        "success": False,
        "ok": False,
        "outcome": "uncertain",
...
def _invalid_response(response: dict[str, object]) -> AuditHardcodedDimensionsUncertain:
    ...
    code = (
        "AUDIT_HARDCODED_DIMENSIONS_COMMITTED_RESPONSE_INVALID"
        if committed
        else "AUDIT_HARDCODED_DIMENSIONS_ROLLBACK_UNCERTAIN"
        if rollback_failed
        else "INVALID_AUDIT_HARDCODED_DIMENSIONS_RESPONSE"
    )
    return make_audit_hardcoded_dimensions_uncertain(
        code,
        "audit_hardcoded_dimensions returned an invalid contract response; document state requires reconciliation",
        committed=True if committed else None,
        diagnostics={"response": response},
    )


def parse_audit_hardcoded_dimensions_response(raw_response: object) -> AuditHardcodedDimensionsResult:
    """Validate all three wire variants; unknown state always stays uncertain."""
    response = _response_object(raw_response)
    if response is None:
        return make_audit_hardcoded_dimensions_uncertain(
            "INVALID_RPC_RESPONSE",
            "audit_hardcoded_dimensions returned a non-object response",
            committed=None,
        )
    version = response.get("contract_version")
    details = _response_details(response)
    if type(version) is not int or version != AUDIT_HARDCODED_DIMENSIONS_CONTRACT_VERSION or details is None:
    ...
    if _valid_success(response) and isinstance(clean, bool) and isinstance(body, str) and type(count) is int and findings is not None:
        return make_audit_hardcoded_dimensions_success(clean, body, count, findings)

    error_code = response.get("error_code")
    error = response.get("error")
    success_keys = {'clean', 'body', 'count', 'findings'}
    if (
        ...
        if _valid_rejection(response):
            return make_audit_hardcoded_dimensions_failure(
                error_code,
                error,
        ...
        if _valid_uncertain(response):
            ...
            return make_audit_hardcoded_dimensions_uncertain(error_code, error, committed=committed, **details)
    return _invalid_response(response)


__all__ = [
    "AUDIT_HARDCODED_DIMENSIONS_CONTRACT_VERSION",
