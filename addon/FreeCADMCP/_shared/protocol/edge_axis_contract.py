"""Versioned, stdlib-only wire contract for ``edge_axis``."""
...
EDGE_AXIS_CONTRACT_VERSION: Literal[1] = 1
...
class EdgeAxisCollaborators(Protocol):
...
class EdgeAxisSuccess(TypedDict):
...
class EdgeAxisFailure(TypedDict):
...
class EdgeAxisUncertain(TypedDict):
...
EdgeAxisResult = EdgeAxisSuccess | EdgeAxisFailure | EdgeAxisUncertain
...
def _invalid_response(response: dict[str, object]) -> EdgeAxisUncertain:
    ...
    code = (
        "EDGE_AXIS_COMMITTED_RESPONSE_INVALID"
        if committed
        else "EDGE_AXIS_ROLLBACK_UNCERTAIN"
        if rollback_failed
        else "INVALID_EDGE_AXIS_RESPONSE"
    )
    return make_edge_axis_uncertain(
        code,
        "edge_axis returned an invalid contract response; document state requires reconciliation",
        committed=True if committed else None,
        diagnostics={"response": response},
    )


def parse_edge_axis_response(raw_response: object) -> EdgeAxisResult:
    """Validate all three wire variants; unknown state always stays uncertain."""
    response = _response_object(raw_response)
    if response is None:
        return make_edge_axis_uncertain(
            "INVALID_RPC_RESPONSE",
            "edge_axis returned a non-object response",
            committed=None,
        )
    version = response.get("contract_version")
    details = _response_details(response)
    if type(version) is not int or version != EDGE_AXIS_CONTRACT_VERSION or details is None:
    ...
    if _valid_success(response) and isinstance(object_name, str) and isinstance(subshape, str) and isinstance(shape_type, str) and global_center is not None and global_normal is not None:
        return make_edge_axis_success(object_name, subshape, shape_type, global_center, global_normal)
    ...
    if (
        ...
        if _valid_uncertain(response):
            ...
            return make_edge_axis_uncertain(error_code, error, committed=committed, **details)
    return _invalid_response(response)


__all__ = [
    "EDGE_AXIS_CONTRACT_VERSION",
