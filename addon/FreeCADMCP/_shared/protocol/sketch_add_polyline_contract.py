"""Versioned, stdlib-only wire contract for ``sketch_add_polyline``."""
...
SKETCH_ADD_POLYLINE_CONTRACT_VERSION: Literal[1] = 1
...
class SketchAddPolylineCollaborators(Protocol):
    @property
...
class SketchAddPolylineRequest:
...
class SketchAddPolylineSuccess(TypedDict):
...
class SketchAddPolylineFailure(TypedDict):
    contract_version: Literal[1]
...
class SketchAddPolylineUncertain(TypedDict):
    contract_version: Literal[1]
    success: Literal[False]
    ok: Literal[False]
...
SketchAddPolylineResult = SketchAddPolylineSuccess | SketchAddPolylineFailure | SketchAddPolylineUncertain
...
def make_sketch_add_polyline_success(sketch: SketchName, geometry_indices: list[int]) -> SketchAddPolylineSuccess:
    return {
        "contract_version": SKETCH_ADD_POLYLINE_CONTRACT_VERSION,
        "success": True,
        "ok": True,
        "outcome": "committed",
        "committed": True,
...
def make_sketch_add_polyline_failure(
    error_code: str,
    error: str,
    *,
    ...
    result: SketchAddPolylineFailure = {
        "contract_version": SKETCH_ADD_POLYLINE_CONTRACT_VERSION,
        "success": False,
        "ok": False,
        "outcome": "rejected",
...
def make_sketch_add_polyline_uncertain(
    error_code: str,
    error: str,
    *,
    committed: bool | None,
    ...
    rollback_failed: bool | None = None,
    diagnostics: dict[str, object] | None = None,
) -> SketchAddPolylineUncertain:
    result: SketchAddPolylineUncertain = {
        "contract_version": SKETCH_ADD_POLYLINE_CONTRACT_VERSION,
        "success": False,
        "ok": False,
        "outcome": "uncertain",
...
def _invalid_response(response: dict[str, object]) -> SketchAddPolylineUncertain:
    ...
    code = (
        ...
        if committed
        else "SKETCH_ADD_POLYLINE_ROLLBACK_UNCERTAIN"
        if rollback_failed
        else "INVALID_SKETCH_ADD_POLYLINE_RESPONSE"
    )
    return make_sketch_add_polyline_uncertain(
        code,
        "sketch_add_polyline returned an invalid contract response; document state requires reconciliation",
        committed=True if committed else None,
        diagnostics={"response": response},
    )


def parse_sketch_add_polyline_response(raw_response: object) -> SketchAddPolylineResult:
    response = _response_object(raw_response)
    if response is None:
        return make_sketch_add_polyline_uncertain(
            "INVALID_RPC_RESPONSE",
            "sketch_add_polyline returned a non-object response",
            committed=None,
        )
    version = response.get("contract_version")
    details = _response_details(response)
    if type(version) is not int or version != SKETCH_ADD_POLYLINE_CONTRACT_VERSION or details is None:
    ...
    if (
        ...
        return make_sketch_add_polyline_success(SketchName(sketch), list(geometry_indices))
    ...
    if (
        ...
        if _valid_rejection(response):
            return make_sketch_add_polyline_failure(
                error_code,
                error,
        ...
        if _valid_uncertain(response):
            ...
            return make_sketch_add_polyline_uncertain(error_code, error, committed=committed, **details)
    return _invalid_response(response)


__all__ = [
    "SKETCH_ADD_POLYLINE_CONTRACT_VERSION",
