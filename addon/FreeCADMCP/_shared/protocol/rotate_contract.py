"""Versioned, stdlib-only wire contract for ``rotate``."""
...
ROTATE_CONTRACT_VERSION: Literal[1] = 1
...
class RotateCollaborators(Protocol):
...
class RotateUncertain(TypedDict):
...
RotateResult = RotateSuccess | RotateFailure | RotateUncertain
...
def make_rotate_success(object: str, label: str) -> RotateSuccess:
    """Construct a complete committed result."""

    return {
        "contract_version": ROTATE_CONTRACT_VERSION,
        "success": True,
        "ok": True,
        "outcome": "committed",
        "committed": True,
...
def _invalid_response(response: dict[str, object]) -> RotateUncertain:
    ...
    code = (
        ...
        if committed
        else "ROTATE_ROLLBACK_UNCERTAIN"
        if rollback_failed
        else "INVALID_ROTATE_RESPONSE"
    )
    return make_rotate_uncertain(
        code,
        "rotate returned an invalid contract response; document state requires reconciliation",
        committed=True if committed else None,
        diagnostics={"response": response},
    )


def parse_rotate_response(raw_response: object) -> RotateResult:
    """Validate all three wire variants; unknown state always stays uncertain."""
    response = _response_object(raw_response)
    if response is None:
        return make_rotate_uncertain(
            "INVALID_RPC_RESPONSE",
            "rotate returned a non-object response",
            committed=None,
        )
    version = response.get("contract_version")
    details = _response_details(response)
    if type(version) is not int or version != ROTATE_CONTRACT_VERSION or details is None:
    ...
    if (
        ...
        return make_rotate_success(object, label)
    ...
    if (
        ...
        if _valid_uncertain(response):
            ...
            return make_rotate_uncertain(error_code, error, committed=committed, **details)
    return _invalid_response(response)


__all__ = [
    "ROTATE_CONTRACT_VERSION",
    "RotateCollaborators",
    "RotateFailure",
    "RotateRequest",
    "RotateResult",
