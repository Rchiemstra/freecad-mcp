"""Versioned, stdlib-only wire contract for ``sketch_import_points``."""
...
SKETCH_IMPORT_POINTS_CONTRACT_VERSION: Literal[1] = 1
...
class SketchImportPointsCollaborators(Protocol):
    @property
...
class SketchImportPointsRequest:
...
class SketchImportPointsSuccess(TypedDict):
...
class SketchImportPointsFailure(TypedDict):
    contract_version: Literal[1]
...
class SketchImportPointsUncertain(TypedDict):
    contract_version: Literal[1]
    success: Literal[False]
    ok: Literal[False]
...
SketchImportPointsResult = SketchImportPointsSuccess | SketchImportPointsFailure | SketchImportPointsUncertain
...
def make_sketch_import_points_success(sketch: SketchName, geometry_indices: list[int]) -> SketchImportPointsSuccess:
    return {
        "contract_version": SKETCH_IMPORT_POINTS_CONTRACT_VERSION,
        "success": True,
        "ok": True,
        "outcome": "committed",
        "committed": True,
...
def make_sketch_import_points_failure(
    error_code: str,
    error: str,
    *,
    ...
    result: SketchImportPointsFailure = {
        "contract_version": SKETCH_IMPORT_POINTS_CONTRACT_VERSION,
        "success": False,
        "ok": False,
        "outcome": "rejected",
...
def make_sketch_import_points_uncertain(
    error_code: str,
    error: str,
    *,
    committed: bool | None,
    ...
    rollback_failed: bool | None = None,
    diagnostics: dict[str, object] | None = None,
) -> SketchImportPointsUncertain:
    result: SketchImportPointsUncertain = {
        "contract_version": SKETCH_IMPORT_POINTS_CONTRACT_VERSION,
        "success": False,
        "ok": False,
        "outcome": "uncertain",
...
def _invalid_response(response: dict[str, object]) -> SketchImportPointsUncertain:
    ...
    code = (
        ...
        if committed
        else "SKETCH_IMPORT_POINTS_ROLLBACK_UNCERTAIN"
        if rollback_failed
        else "INVALID_SKETCH_IMPORT_POINTS_RESPONSE"
    )
    return make_sketch_import_points_uncertain(
        code,
        "sketch_import_points returned an invalid contract response; document state requires reconciliation",
        committed=True if committed else None,
        diagnostics={"response": response},
    )


def parse_sketch_import_points_response(raw_response: object) -> SketchImportPointsResult:
    response = _response_object(raw_response)
    if response is None:
        return make_sketch_import_points_uncertain(
            "INVALID_RPC_RESPONSE",
            "sketch_import_points returned a non-object response",
            committed=None,
        )
    version = response.get("contract_version")
    details = _response_details(response)
    if type(version) is not int or version != SKETCH_IMPORT_POINTS_CONTRACT_VERSION or details is None:
    ...
    if (
        ...
        return make_sketch_import_points_success(SketchName(sketch), list(geometry_indices))
    ...
    if (
        ...
        if _valid_rejection(response):
            return make_sketch_import_points_failure(
                error_code,
                error,
        ...
        if _valid_uncertain(response):
            ...
            return make_sketch_import_points_uncertain(error_code, error, committed=committed, **details)
    return _invalid_response(response)


__all__ = [
    "SKETCH_IMPORT_POINTS_CONTRACT_VERSION",
