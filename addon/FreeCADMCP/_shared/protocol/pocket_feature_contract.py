"""Versioned, stdlib-only wire contract for ``pocket_feature``."""
...
POCKET_FEATURE_CONTRACT_VERSION: Literal[1] = 1

DocumentName = NewType("DocumentName", str)
PocketName = NewType("PocketName", str)


class PocketFeatureObject(Protocol):
...
class PocketFeatureReadDocument(Protocol):
    """Read-only document surface available after native recompute."""

    @property
    def Name(self) -> str: ...

    def getObject(self, name: str) -> PocketFeatureObject | None: ...


class PocketFeatureDocument(PocketFeatureReadDocument, Protocol):
    """Narrow mutation surface available only to the apply callback."""

    def addObject(self, object_type: str, name: str) -> PocketFeatureObject: ...


@runtime_checkable
class NativePocketFeatureDocument(PocketFeatureDocument, Protocol):
...
class PocketFeatureCollaborators(Protocol):
...
class PocketFeatureSuccess(TypedDict):
...
class PocketFeatureFailure(TypedDict):
...
class PocketFeatureUncertain(TypedDict):
...
PocketFeatureResult = PocketFeatureSuccess | PocketFeatureFailure | PocketFeatureUncertain
...
def make_pocket_feature_success(pocket: PocketName, label: str) -> PocketFeatureSuccess:
    """Construct a complete committed result using the actual assigned identity."""

    return {
        "contract_version": POCKET_FEATURE_CONTRACT_VERSION,
        "success": True,
        "ok": True,
        "outcome": "committed",
        "committed": True,
...
def make_pocket_feature_failure(
    error_code: str,
    error: str,
    *,
...
def make_pocket_feature_uncertain(
    error_code: str,
    error: str,
    *,
    committed: bool | None,
...
def _invalid_response(response: dict[str, object]) -> PocketFeatureUncertain:
    ...
    code = (
        ...
        if committed
        else "POCKET_FEATURE_ROLLBACK_UNCERTAIN"
        if rollback_failed
        else "INVALID_POCKET_FEATURE_RESPONSE"
    )
    return make_pocket_feature_uncertain(
        code,
        "pocket_feature returned an invalid contract response; document state requires reconciliation",
        committed=True if committed else None,
        diagnostics={"response": response},
    )


def parse_pocket_feature_response(raw_response: object) -> PocketFeatureResult:
    """Validate all three wire variants; unknown state always stays uncertain."""
    response = _response_object(raw_response)
    if response is None:
        return make_pocket_feature_uncertain(
            "INVALID_RPC_RESPONSE",
            "pocket_feature returned a non-object response",
            committed=None,
        )
    version = response.get("contract_version")
    details = _response_details(response)
    if type(version) is not int or version != POCKET_FEATURE_CONTRACT_VERSION or details is None:
        return _invalid_response(response)

    pocket = response.get("pocket")
    label = response.get("label")
    if (
        _valid_success(response)
        and isinstance(pocket, str)
        and pocket.strip()
        and isinstance(label, str)
    ):
        return make_pocket_feature_success(PocketName(pocket), label)
    ...
    if (
        ...
        and error.strip()
        and "pocket" not in response and "label" not in response
    ):
        ...
        if _valid_uncertain(response):
            ...
            return make_pocket_feature_uncertain(error_code, error, committed=committed, **details)
    return _invalid_response(response)


__all__ = [
    "POCKET_FEATURE_CONTRACT_VERSION",
