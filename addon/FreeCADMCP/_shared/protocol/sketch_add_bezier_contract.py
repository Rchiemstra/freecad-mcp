"""Versioned, stdlib-only wire contract for ``sketch_add_bezier``."""
...
SKETCH_ADD_BEZIER_CONTRACT_VERSION: Literal[1] = 1
...
class SketchAddBezierCollaborators(Protocol):
    @property
...
class SketchAddBezierSuccess(TypedDict):
...
class SketchAddBezierFailure(TypedDict):
    contract_version: Literal[1]
...
class SketchAddBezierUncertain(TypedDict):
    contract_version: Literal[1]
    success: Literal[False]
    ok: Literal[False]
...
SketchAddBezierResult = SketchAddBezierSuccess | SketchAddBezierFailure | SketchAddBezierUncertain
...
def make_sketch_add_bezier_success(sketch: SketchName, geometry_index: int) -> SketchAddBezierSuccess:
    return {
        "contract_version": SKETCH_ADD_BEZIER_CONTRACT_VERSION,
        "success": True,
        "ok": True,
        "outcome": "committed",
        "committed": True,
...
def make_sketch_add_bezier_failure(
    error_code: str,
    error: str,
    *,
...
def make_sketch_add_bezier_uncertain(
    error_code: str,
    error: str,
    *,
    committed: bool | None,
    ...
    result: SketchAddBezierUncertain = {
        "contract_version": SKETCH_ADD_BEZIER_CONTRACT_VERSION,
        "success": False,
        "ok": False,
        "outcome": "uncertain",
...
def _invalid_response(response: dict[str, object]) -> SketchAddBezierUncertain:
    ...
    code = (
        ...
        if committed
        else "SKETCH_ADD_BEZIER_ROLLBACK_UNCERTAIN"
        if rollback_failed
        else "INVALID_SKETCH_ADD_BEZIER_RESPONSE"
    )
    return make_sketch_add_bezier_uncertain(
        code,
        "sketch_add_bezier returned an invalid contract response; document state requires reconciliation",
        committed=True if committed else None,
        diagnostics={"response": response},
    )


def parse_sketch_add_bezier_response(raw_response: object) -> SketchAddBezierResult:
    response = _response_object(raw_response)
    if response is None:
        return make_sketch_add_bezier_uncertain(
            "INVALID_RPC_RESPONSE",
            "sketch_add_bezier returned a non-object response",
            committed=None,
        )
    version = response.get("contract_version")
    details = _response_details(response)
    if type(version) is not int or version != SKETCH_ADD_BEZIER_CONTRACT_VERSION or details is None:
    ...
    if (
        ...
        return make_sketch_add_bezier_success(SketchName(sketch), geometry_index)
    ...
    if (
        ...
        if _valid_rejection(response):
            return make_sketch_add_bezier_failure(
                error_code,
                error,
        ...
        if _valid_uncertain(response):
            ...
            return make_sketch_add_bezier_uncertain(error_code, error, committed=committed, **details)
    return _invalid_response(response)


__all__ = [
    "SKETCH_ADD_BEZIER_CONTRACT_VERSION",
    "SketchAddBezierCollaborators",
    "SketchAddBezierFailure",
    "SketchAddBezierRequest",
    "SketchAddBezierResult",
    "SketchAddBezierSuccess",
    "SketchAddBezierUncertain",
    "DocumentName",
