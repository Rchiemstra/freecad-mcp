"""Versioned, stdlib-only wire contract for ``sweep_pipe``."""
...
SWEEP_PIPE_CONTRACT_VERSION: Literal[1] = 1

DocumentName = NewType("DocumentName", str)
SweepPipeName = NewType("SweepPipeName", str)


class SweepPipeObject(Protocol):
...
class SweepPipeReadDocument(Protocol):
    """Read-only document surface available after native recompute."""

    @property
    def Name(self) -> str: ...

    def getObject(self, name: str) -> SweepPipeObject | None: ...


class SweepPipeDocument(SweepPipeReadDocument, Protocol):
    """Narrow mutation surface available only to the apply callback."""

    def addObject(self, object_type: str, name: str) -> SweepPipeObject: ...


@runtime_checkable
class NativeSweepPipeDocument(SweepPipeDocument, Protocol):
...
class SweepPipeCollaborators(Protocol):
...
class SweepPipeSuccess(TypedDict):
...
class SweepPipeFailure(TypedDict):
...
class SweepPipeUncertain(TypedDict):
...
SweepPipeResult = SweepPipeSuccess | SweepPipeFailure | SweepPipeUncertain

_CORE_KEYS = frozenset(
    {
        "contract_version", "success", "ok", "outcome", "committed", "retry_safe", 'solid_name', "error_code", "error", "native_status", "native_message", "rollback_succeeded", "rollback_failed", "diagnostics"
    }
)


def make_sweep_pipe_success(solid_name: str) -> SweepPipeSuccess:
    """Construct a complete committed result."""

    return {
        "contract_version": SWEEP_PIPE_CONTRACT_VERSION,
        "success": True,
        "ok": True,
        "outcome": "committed",
        "committed": True,
...
def _invalid_response(response: dict[str, object]) -> SweepPipeUncertain:
    ...
    code = (
        ...
        if committed
        else "SWEEP_PIPE_ROLLBACK_UNCERTAIN"
        if rollback_failed
        else "INVALID_SWEEP_PIPE_RESPONSE"
    )
    return make_sweep_pipe_uncertain(
        code,
        "sweep_pipe returned an invalid contract response; document state requires reconciliation",
        committed=True if committed else None,
        diagnostics={"response": response},
    )


def parse_sweep_pipe_response(raw_response: object) -> SweepPipeResult:
    """Validate all three wire variants; unknown state always stays uncertain."""
    response = _response_object(raw_response)
    if response is None:
        return make_sweep_pipe_uncertain(
            "INVALID_RPC_RESPONSE",
            "sweep_pipe returned a non-object response",
            committed=None,
        )
    version = response.get("contract_version")
    details = _response_details(response)
    if type(version) is not int or version != SWEEP_PIPE_CONTRACT_VERSION or details is None:
        return _invalid_response(response)

    solid_name = response.get('solid_name')
    if (
        _valid_success(response)
        and isinstance(solid_name, str) and solid_name.strip()
    ):
        return make_sweep_pipe_success(str(solid_name))
    ...
    if (
        ...
        and error.strip()
        and 'solid_name' not in response
    ):
        if _valid_rejection(response):
            return make_sweep_pipe_failure(
                error_code,
                error,
        ...
        if _valid_uncertain(response):
            ...
            return make_sweep_pipe_uncertain(error_code, error, committed=committed, **details)
    return _invalid_response(response)


__all__ = [
    "SWEEP_PIPE_CONTRACT_VERSION",
