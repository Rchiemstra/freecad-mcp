"""Versioned, stdlib-only wire contract for ``sweep_feature``."""
...
SWEEP_FEATURE_CONTRACT_VERSION: Literal[1] = 1
...
class SweepFeatureCollaborators(Protocol):
...
class SweepFeatureRequest:
    """Validated internal request."""

    doc_name: DocumentName
    profile_sketch: FeatureName
    path_sketch: FeatureName
    sweep_name: FeatureName
    body_name: FeatureName | None
    frenet: bool


class SweepFeatureSuccess(TypedDict):
...
class SweepFeatureFailure(TypedDict):
...
class SweepFeatureUncertain(TypedDict):
...
SweepFeatureResult = SweepFeatureSuccess | SweepFeatureFailure | SweepFeatureUncertain
...
def make_sweep_feature_success(feature: FeatureName, label: str) -> SweepFeatureSuccess:
    """Construct a complete committed result using the actual assigned name."""

    return {
        "contract_version": SWEEP_FEATURE_CONTRACT_VERSION,
        "success": True,
        "ok": True,
        "outcome": "committed",
        "committed": True,
...
def make_sweep_feature_failure(
    error_code: str,
    error: str,
    *,
...
def make_sweep_feature_uncertain(
    error_code: str,
    error: str,
    *,
    committed: bool | None,
...
def _invalid_response(response: dict[str, object]) -> SweepFeatureUncertain:
    ...
    code = (
        ...
        if committed
        else "SWEEP_FEATURE_ROLLBACK_UNCERTAIN"
        if rollback_failed
        else "INVALID_SWEEP_FEATURE_RESPONSE"
    )
    return make_sweep_feature_uncertain(
        code,
        "sweep_feature returned an invalid contract response; document state requires reconciliation",
        committed=True if committed else None,
        diagnostics={"response": response},
    )


def parse_sweep_feature_response(raw_response: object) -> SweepFeatureResult:
    """Validate all three wire variants; unknown state always stays uncertain."""
    response = _response_object(raw_response)
    if response is None:
        return make_sweep_feature_uncertain(
            "INVALID_RPC_RESPONSE",
            "sweep_feature returned a non-object response",
            committed=None,
        )
    version = response.get("contract_version")
    details = _response_details(response)
    if type(version) is not int or version != SWEEP_FEATURE_CONTRACT_VERSION or details is None:
    ...
    if (
        ...
        return make_sweep_feature_success(FeatureName(feature), label)
    ...
    if (
        ...
        if _valid_rejection(response):
            return make_sweep_feature_failure(
                error_code,
                error,
        ...
        if _valid_uncertain(response):
            ...
            return make_sweep_feature_uncertain(error_code, error, committed=committed, **details)
    return _invalid_response(response)


__all__ = [
    "SWEEP_FEATURE_CONTRACT_VERSION",
