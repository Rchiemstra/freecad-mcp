"""Versioned, stdlib-only wire contract for ``boolean_union``."""
...
BOOLEAN_UNION_CONTRACT_VERSION: Literal[1] = 1
...
class BooleanUnionCollaborators(Protocol):
...
class BooleanUnionSuccess(TypedDict):
...
class BooleanUnionFailure(TypedDict):
...
class BooleanUnionUncertain(TypedDict):
...
BooleanUnionResult = BooleanUnionSuccess | BooleanUnionFailure | BooleanUnionUncertain
...
def make_boolean_union_success(feature: FeatureName, label: str) -> BooleanUnionSuccess:
    """Construct a complete committed result using the actual assigned name."""

    return {
        "contract_version": BOOLEAN_UNION_CONTRACT_VERSION,
        "success": True,
        "ok": True,
        "outcome": "committed",
        "committed": True,
...
def make_boolean_union_failure(
    error_code: str,
    error: str,
    *,
...
def make_boolean_union_uncertain(
    error_code: str,
    error: str,
    *,
    committed: bool | None,
...
def _invalid_response(response: dict[str, object]) -> BooleanUnionUncertain:
    ...
    code = (
        ...
        if committed
        else "BOOLEAN_UNION_ROLLBACK_UNCERTAIN"
        if rollback_failed
        else "INVALID_BOOLEAN_UNION_RESPONSE"
    )
    return make_boolean_union_uncertain(
        code,
        "boolean_union returned an invalid contract response; document state requires reconciliation",
        committed=True if committed else None,
        diagnostics={"response": response},
    )


def parse_boolean_union_response(raw_response: object) -> BooleanUnionResult:
    """Validate all three wire variants; unknown state always stays uncertain."""
    response = _response_object(raw_response)
    if response is None:
        return make_boolean_union_uncertain(
            "INVALID_RPC_RESPONSE",
            "boolean_union returned a non-object response",
            committed=None,
        )
    version = response.get("contract_version")
    details = _response_details(response)
    if type(version) is not int or version != BOOLEAN_UNION_CONTRACT_VERSION or details is None:
    ...
    if (
        ...
        return make_boolean_union_success(FeatureName(feature), label)
    ...
    if (
        ...
        if _valid_rejection(response):
            return make_boolean_union_failure(
                error_code,
                error,
        ...
        if _valid_uncertain(response):
            ...
            return make_boolean_union_uncertain(error_code, error, committed=committed, **details)
    return _invalid_response(response)


__all__ = [
    "BOOLEAN_UNION_CONTRACT_VERSION",
