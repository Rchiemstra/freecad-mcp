"""Versioned, stdlib-only wire contract for ``inspect_geometry``."""
...
INSPECT_GEOMETRY_CONTRACT_VERSION: Literal[1] = 1
...
class InspectGeometryCollaborators(Protocol):
...
class InspectGeometryUncertain(TypedDict):
...
InspectGeometryResult = InspectGeometrySuccess | InspectGeometryFailure | InspectGeometryUncertain
...
def make_inspect_geometry_uncertain(
    error_code: str,
    error: str,
    *,
    committed: bool | None,
...
def _invalid_response(response: dict[str, object]) -> InspectGeometryUncertain:
    ...
    code = (
        "INSPECT_GEOMETRY_COMMITTED_RESPONSE_INVALID"
        if committed
        else "INSPECT_GEOMETRY_ROLLBACK_UNCERTAIN"
        if rollback_failed
        else "INVALID_INSPECT_GEOMETRY_RESPONSE"
    )
    return make_inspect_geometry_uncertain(
        code,
        "inspect_geometry returned an invalid contract response; document state requires reconciliation",
...
def parse_inspect_geometry_response(raw_response: object) -> InspectGeometryResult:
    ...
    if response is None:
        return make_inspect_geometry_uncertain(
            ...
            "inspect_geometry returned a non-object response",
            committed=None,
        )
    version = response.get("contract_version")
    details = _response_details(response)
    if type(version) is not int or version != INSPECT_GEOMETRY_CONTRACT_VERSION or details is None:
    ...
    global_placement = response.get("global_placement")
    parent_chain = response.get("parent_chain")
    ...
    success_keys = {'object', 'type_id', 'placement', 'global_placement', 'parent_chain', 'local_bbox', 'global_bbox'}
    if (
        ...
        if _valid_rejection(response):
            return make_inspect_geometry_failure(
                error_code,
                error,
        ...
        if _valid_uncertain(response):
            ...
            return make_inspect_geometry_uncertain(error_code, error, committed=committed, **details)
    return _invalid_response(response)


__all__ = [
    "INSPECT_GEOMETRY_CONTRACT_VERSION",
