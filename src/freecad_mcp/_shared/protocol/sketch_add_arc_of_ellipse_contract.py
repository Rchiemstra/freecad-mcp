"""Versioned, stdlib-only wire contract for ``sketch_add_arc_of_ellipse``."""
...
SKETCH_ADD_ARC_OF_ELLIPSE_CONTRACT_VERSION: Literal[1] = 1
...
class SketchPart(Protocol):
    ...
    def Ellipse(
        self, center: SketchVector, major_radius: float, minor_radius: float
    ) -> object: ...
...
class SketchAddArcOfEllipseCollaborators(Protocol):
    @property
...
class SketchAddArcOfEllipseRequest:
...
class SketchAddArcOfEllipseSuccess(TypedDict):
...
class SketchAddArcOfEllipseFailure(TypedDict):
    contract_version: Literal[1]
...
class SketchAddArcOfEllipseUncertain(TypedDict):
    contract_version: Literal[1]
    success: Literal[False]
    ok: Literal[False]
...
SketchAddArcOfEllipseResult = SketchAddArcOfEllipseSuccess | SketchAddArcOfEllipseFailure | SketchAddArcOfEllipseUncertain
...
def make_sketch_add_arc_of_ellipse_success(sketch: SketchName, geometry_index: int) -> SketchAddArcOfEllipseSuccess:
    return {
        "contract_version": SKETCH_ADD_ARC_OF_ELLIPSE_CONTRACT_VERSION,
        "success": True,
        "ok": True,
        "outcome": "committed",
        "committed": True,
...
def make_sketch_add_arc_of_ellipse_failure(
    error_code: str,
    error: str,
    *,
    ...
    result: SketchAddArcOfEllipseFailure = {
        "contract_version": SKETCH_ADD_ARC_OF_ELLIPSE_CONTRACT_VERSION,
        "success": False,
        "ok": False,
        "outcome": "rejected",
...
def make_sketch_add_arc_of_ellipse_uncertain(
    error_code: str,
    error: str,
    *,
    committed: bool | None,
    ...
    result: SketchAddArcOfEllipseUncertain = {
        "contract_version": SKETCH_ADD_ARC_OF_ELLIPSE_CONTRACT_VERSION,
        "success": False,
        "ok": False,
        "outcome": "uncertain",
...
def _invalid_response(response: dict[str, object]) -> SketchAddArcOfEllipseUncertain:
    ...
    code = (
        ...
        if committed
        else "SKETCH_ADD_ARC_OF_ELLIPSE_ROLLBACK_UNCERTAIN"
        if rollback_failed
        else "INVALID_SKETCH_ADD_ARC_OF_ELLIPSE_RESPONSE"
    )
    return make_sketch_add_arc_of_ellipse_uncertain(
        code,
        "sketch_add_arc_of_ellipse returned an invalid contract response; document state requires reconciliation",
        committed=True if committed else None,
        diagnostics={"response": response},
    )


def parse_sketch_add_arc_of_ellipse_response(raw_response: object) -> SketchAddArcOfEllipseResult:
    response = _response_object(raw_response)
    if response is None:
        return make_sketch_add_arc_of_ellipse_uncertain(
            "INVALID_RPC_RESPONSE",
            "sketch_add_arc_of_ellipse returned a non-object response",
            committed=None,
        )
    version = response.get("contract_version")
    details = _response_details(response)
    if type(version) is not int or version != SKETCH_ADD_ARC_OF_ELLIPSE_CONTRACT_VERSION or details is None:
    ...
    if (
        ...
        return make_sketch_add_arc_of_ellipse_success(SketchName(sketch), geometry_index)
    ...
    if (
        ...
        if _valid_uncertain(response):
            ...
            return make_sketch_add_arc_of_ellipse_uncertain(error_code, error, committed=committed, **details)
    return _invalid_response(response)


__all__ = [
    "SKETCH_ADD_ARC_OF_ELLIPSE_CONTRACT_VERSION",
    ...
    "parse_sketch_add_arc_of_ellipse_response",
]
