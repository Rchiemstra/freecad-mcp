"""Versioned, stdlib-only wire contract for ``import_brep``."""
...
IMPORT_BREP_CONTRACT_VERSION: Literal[1] = 1
...
class ImportBrepCollaborators(Protocol):
...
class ImportBrepSuccess(TypedDict):
...
class ImportBrepFailure(TypedDict):
...
class ImportBrepUncertain(TypedDict):
...
ImportBrepResult = ImportBrepSuccess | ImportBrepFailure | ImportBrepUncertain
...
def make_import_brep_success(path: str, object: str, imported: bool) -> ImportBrepSuccess:
    """Construct a complete committed result."""

    return {
        "contract_version": IMPORT_BREP_CONTRACT_VERSION,
        "success": True,
        "ok": True,
        "outcome": "committed",
        "committed": True,
...
def make_import_brep_uncertain(
    error_code: str,
    error: str,
    *,
    committed: bool | None,
...
def _invalid_response(response: dict[str, object]) -> ImportBrepUncertain:
    ...
    code = (
        ...
        if committed
        else "IMPORT_BREP_ROLLBACK_UNCERTAIN"
        if rollback_failed
        else "INVALID_IMPORT_BREP_RESPONSE"
    )
    return make_import_brep_uncertain(
        code,
        "import_brep returned an invalid contract response; document state requires reconciliation",
        committed=True if committed else None,
        diagnostics={"response": response},
    )


def parse_import_brep_response(raw_response: object) -> ImportBrepResult:
    """Validate all three wire variants; unknown state always stays uncertain."""
    response = _response_object(raw_response)
    if response is None:
        return make_import_brep_uncertain(
            "INVALID_RPC_RESPONSE",
            "import_brep returned a non-object response",
            committed=None,
        )
    version = response.get("contract_version")
    details = _response_details(response)
    if type(version) is not int or version != IMPORT_BREP_CONTRACT_VERSION or details is None:
    ...
    if (
        ...
        and isinstance(path, str) and isinstance(object, str) and object.strip() and isinstance(imported, bool)
    ):
        return make_import_brep_success(path, object, imported)

    error_code = response.get("error_code")
    error = response.get("error")
    success_keys = {'path', 'object', 'imported'}
    if (
        ...
        if _valid_rejection(response):
            return make_import_brep_failure(
                error_code,
                error,
        ...
        if _valid_uncertain(response):
            ...
            return make_import_brep_uncertain(error_code, error, committed=committed, **details)
    return _invalid_response(response)


__all__ = [
    "IMPORT_BREP_CONTRACT_VERSION",
