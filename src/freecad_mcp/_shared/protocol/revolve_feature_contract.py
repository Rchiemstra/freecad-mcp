"""Versioned, stdlib-only wire contract for ``revolve_feature``."""
...
REVOLVE_FEATURE_CONTRACT_VERSION: Literal[1] = 1
...
class RevolveFeatureCollaborators(Protocol):
...
class RevolveFeatureRequest:
    """Validated internal request."""

    doc_name: DocumentName
    sketch_name: FeatureName
    revolve_name: FeatureName
    angle: float
    axis: str
...
class RevolveFeatureSuccess(TypedDict):
...
class RevolveFeatureFailure(TypedDict):
...
class RevolveFeatureUncertain(TypedDict):
...
RevolveFeatureResult = RevolveFeatureSuccess | RevolveFeatureFailure | RevolveFeatureUncertain
...
def make_revolve_feature_success(feature: FeatureName, label: str) -> RevolveFeatureSuccess:
    """Construct a complete committed result using the actual assigned name."""

    return {
        "contract_version": REVOLVE_FEATURE_CONTRACT_VERSION,
        "success": True,
        "ok": True,
        "outcome": "committed",
        "committed": True,
...
def make_revolve_feature_failure(
    error_code: str,
    error: str,
    *,
...
def make_revolve_feature_uncertain(
    error_code: str,
    error: str,
    *,
    committed: bool | None,
    ...
    result: RevolveFeatureUncertain = {
        "contract_version": REVOLVE_FEATURE_CONTRACT_VERSION,
        "success": False,
        "ok": False,
        "outcome": "uncertain",
...
def _invalid_response(response: dict[str, object]) -> RevolveFeatureUncertain:
    ...
    code = (
        ...
        if committed
        else "REVOLVE_FEATURE_ROLLBACK_UNCERTAIN"
        if rollback_failed
        else "INVALID_REVOLVE_FEATURE_RESPONSE"
    )
    return make_revolve_feature_uncertain(
        code,
        "revolve_feature returned an invalid contract response; document state requires reconciliation",
        committed=True if committed else None,
        diagnostics={"response": response},
    )


def parse_revolve_feature_response(raw_response: object) -> RevolveFeatureResult:
    """Validate all three wire variants; unknown state always stays uncertain."""
    response = _response_object(raw_response)
    if response is None:
        return make_revolve_feature_uncertain(
            "INVALID_RPC_RESPONSE",
            "revolve_feature returned a non-object response",
            committed=None,
        )
    version = response.get("contract_version")
    details = _response_details(response)
    if type(version) is not int or version != REVOLVE_FEATURE_CONTRACT_VERSION or details is None:
    ...
    if (
        ...
        return make_revolve_feature_success(FeatureName(feature), label)
    ...
    if (
        ...
        if _valid_uncertain(response):
            ...
            return make_revolve_feature_uncertain(error_code, error, committed=committed, **details)
    return _invalid_response(response)


__all__ = [
    "REVOLVE_FEATURE_CONTRACT_VERSION",
