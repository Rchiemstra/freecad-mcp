"""Versioned, stdlib-only wire contract for ``sketch_add_regular_polygon``."""
...
SKETCH_ADD_REGULAR_POLYGON_CONTRACT_VERSION: Literal[1] = 1
...
class SketchAddRegularPolygonCollaborators(Protocol):
    @property
...
class SketchAddRegularPolygonRequest:
...
class SketchAddRegularPolygonSuccess(TypedDict):
...
class SketchAddRegularPolygonFailure(TypedDict):
    contract_version: Literal[1]
...
class SketchAddRegularPolygonUncertain(TypedDict):
    contract_version: Literal[1]
    success: Literal[False]
    ok: Literal[False]
...
SketchAddRegularPolygonResult = SketchAddRegularPolygonSuccess | SketchAddRegularPolygonFailure | SketchAddRegularPolygonUncertain
...
def make_sketch_add_regular_polygon_success(sketch: SketchName, geometry_indices: list[int]) -> SketchAddRegularPolygonSuccess:
    return {
        "contract_version": SKETCH_ADD_REGULAR_POLYGON_CONTRACT_VERSION,
        "success": True,
        "ok": True,
        "outcome": "committed",
        "committed": True,
...
def make_sketch_add_regular_polygon_failure(
    error_code: str,
    error: str,
    *,
    ...
    result: SketchAddRegularPolygonFailure = {
        "contract_version": SKETCH_ADD_REGULAR_POLYGON_CONTRACT_VERSION,
        "success": False,
        "ok": False,
        "outcome": "rejected",
...
def make_sketch_add_regular_polygon_uncertain(
    error_code: str,
    error: str,
    *,
    committed: bool | None,
    ...
    result: SketchAddRegularPolygonUncertain = {
        "contract_version": SKETCH_ADD_REGULAR_POLYGON_CONTRACT_VERSION,
        "success": False,
        "ok": False,
        "outcome": "uncertain",
...
def _invalid_response(response: dict[str, object]) -> SketchAddRegularPolygonUncertain:
    ...
    code = (
        ...
        if committed
        else "SKETCH_ADD_REGULAR_POLYGON_ROLLBACK_UNCERTAIN"
        if rollback_failed
        else "INVALID_SKETCH_ADD_REGULAR_POLYGON_RESPONSE"
    )
    return make_sketch_add_regular_polygon_uncertain(
        code,
        "sketch_add_regular_polygon returned an invalid contract response; document state requires reconciliation",
        committed=True if committed else None,
        diagnostics={"response": response},
    )


def parse_sketch_add_regular_polygon_response(raw_response: object) -> SketchAddRegularPolygonResult:
    response = _response_object(raw_response)
    if response is None:
        return make_sketch_add_regular_polygon_uncertain(
            "INVALID_RPC_RESPONSE",
            "sketch_add_regular_polygon returned a non-object response",
            committed=None,
        )
    version = response.get("contract_version")
    details = _response_details(response)
    if type(version) is not int or version != SKETCH_ADD_REGULAR_POLYGON_CONTRACT_VERSION or details is None:
        return _invalid_response(response)

    sketch = response.get("sketch")
    geometry_indices = response.get("geometry_indices")
    if (
        _valid_success(response)
        and isinstance(sketch, str)
        and sketch.strip()
        and isinstance(geometry_indices, list)
        and all(type(index) is int for index in geometry_indices)
    ):
        return make_sketch_add_regular_polygon_success(SketchName(sketch), list(geometry_indices))
    ...
    if (
        ...
        and "geometry_indices" not in response
    ):
        if _valid_rejection(response):
            return make_sketch_add_regular_polygon_failure(
                error_code,
                error,
        ...
        if _valid_uncertain(response):
            ...
            return make_sketch_add_regular_polygon_uncertain(error_code, error, committed=committed, **details)
    return _invalid_response(response)


__all__ = [
    "SKETCH_ADD_REGULAR_POLYGON_CONTRACT_VERSION",
