"""Versioned, stdlib-only wire contract for ``match_subshape``."""
...
MATCH_SUBSHAPE_CONTRACT_VERSION: Literal[1] = 1
...
class MatchSubshapeCollaborators(Protocol):
...
class MatchSubshapeSuccess(TypedDict):
...
class MatchSubshapeFailure(TypedDict):
...
class MatchSubshapeUncertain(TypedDict):
...
MatchSubshapeResult = MatchSubshapeSuccess | MatchSubshapeFailure | MatchSubshapeUncertain
...
def make_match_subshape_success(
    source: str, target: str, matches: object,
) -> MatchSubshapeSuccess:
    """Construct a complete observed result."""

    return {
        "contract_version": MATCH_SUBSHAPE_CONTRACT_VERSION,
        "success": True,
        "ok": True,
        "outcome": "observed",
        "retry_safe": False,
...
def make_match_subshape_failure(
    error_code: str,
    error: str,
    *,
...
def make_match_subshape_uncertain(
    error_code: str,
    error: str,
    *,
    committed: bool | None,
...
def _invalid_response(response: dict[str, object]) -> MatchSubshapeUncertain:
    ...
    code = (
        "MATCH_SUBSHAPE_COMMITTED_RESPONSE_INVALID"
        if committed
        else "MATCH_SUBSHAPE_ROLLBACK_UNCERTAIN"
        if rollback_failed
        else "INVALID_MATCH_SUBSHAPE_RESPONSE"
    )
    return make_match_subshape_uncertain(
        code,
        "match_subshape returned an invalid contract response; document state requires reconciliation",
        committed=True if committed else None,
        diagnostics={"response": response},
    )


def parse_match_subshape_response(raw_response: object) -> MatchSubshapeResult:
    """Validate all three wire variants; unknown state always stays uncertain."""
    response = _response_object(raw_response)
    if response is None:
        return make_match_subshape_uncertain(
            "INVALID_RPC_RESPONSE",
            "match_subshape returned a non-object response",
            committed=None,
        )
    version = response.get("contract_version")
    details = _response_details(response)
    if type(version) is not int or version != MATCH_SUBSHAPE_CONTRACT_VERSION or details is None:
        return _invalid_response(response)

    source = response.get("source")
    target = response.get("target")
    matches = response.get("matches")
    if _valid_success(response) and isinstance(source, str) and isinstance(target, str) and matches is not None:
        return make_match_subshape_success(source, target, matches)

    error_code = response.get("error_code")
    error = response.get("error")
    success_keys = {'source', 'target', 'matches'}
    if (
        ...
        if _valid_rejection(response):
            return make_match_subshape_failure(
                error_code,
                error,
        ...
        if _valid_uncertain(response):
            ...
            return make_match_subshape_uncertain(error_code, error, committed=committed, **details)
    return _invalid_response(response)


__all__ = [
    "MATCH_SUBSHAPE_CONTRACT_VERSION",
