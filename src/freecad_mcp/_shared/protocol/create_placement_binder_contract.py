"""Versioned, stdlib-only wire contract for ``create_placement_binder``."""
...
CREATE_PLACEMENT_BINDER_CONTRACT_VERSION: Literal[1] = 1

DocumentName = NewType("DocumentName", str)
CreatePlacementBinderName = NewType("CreatePlacementBinderName", str)


class CreatePlacementBinderObject(Protocol):
...
class CreatePlacementBinderReadDocument(Protocol):
    """Read-only document surface available after native recompute."""

    @property
    def Name(self) -> str: ...

    def getObject(self, name: str) -> CreatePlacementBinderObject | None: ...


class CreatePlacementBinderDocument(CreatePlacementBinderReadDocument, Protocol):
    """Narrow mutation surface available only to the apply callback."""

    def addObject(self, object_type: str, name: str) -> CreatePlacementBinderObject: ...


@runtime_checkable
class NativeCreatePlacementBinderDocument(CreatePlacementBinderDocument, Protocol):
...
class CreatePlacementBinderCollaborators(Protocol):
...
class CreatePlacementBinderRequest:
...
class CreatePlacementBinderSuccess(TypedDict):
...
class CreatePlacementBinderFailure(TypedDict):
...
class CreatePlacementBinderUncertain(TypedDict):
...
CreatePlacementBinderResult = CreatePlacementBinderSuccess | CreatePlacementBinderFailure | CreatePlacementBinderUncertain
...
def make_create_placement_binder_success(binder_name: str) -> CreatePlacementBinderSuccess:
    """Construct a complete committed result."""

    return {
        "contract_version": CREATE_PLACEMENT_BINDER_CONTRACT_VERSION,
        "success": True,
        "ok": True,
        "outcome": "committed",
        "committed": True,
...
def make_create_placement_binder_failure(
    error_code: str,
    error: str,
    *,
    ...
    result: CreatePlacementBinderFailure = {
        "contract_version": CREATE_PLACEMENT_BINDER_CONTRACT_VERSION,
        "success": False,
        "ok": False,
        "outcome": "rejected",
...
def make_create_placement_binder_uncertain(
    error_code: str,
    error: str,
    *,
    committed: bool | None,
    ...
    result: CreatePlacementBinderUncertain = {
        "contract_version": CREATE_PLACEMENT_BINDER_CONTRACT_VERSION,
        "success": False,
        "ok": False,
        "outcome": "uncertain",
...
def _invalid_response(response: dict[str, object]) -> CreatePlacementBinderUncertain:
    ...
    code = (
        ...
        if committed
        else "CREATE_PLACEMENT_BINDER_ROLLBACK_UNCERTAIN"
        if rollback_failed
        else "INVALID_CREATE_PLACEMENT_BINDER_RESPONSE"
    )
    return make_create_placement_binder_uncertain(
        code,
        "create_placement_binder returned an invalid contract response; document state requires reconciliation",
        committed=True if committed else None,
        diagnostics={"response": response},
    )


def parse_create_placement_binder_response(raw_response: object) -> CreatePlacementBinderResult:
    """Validate all three wire variants; unknown state always stays uncertain."""
    response = _response_object(raw_response)
    if response is None:
        return make_create_placement_binder_uncertain(
            "INVALID_RPC_RESPONSE",
            "create_placement_binder returned a non-object response",
            committed=None,
        )
    version = response.get("contract_version")
    details = _response_details(response)
    if type(version) is not int or version != CREATE_PLACEMENT_BINDER_CONTRACT_VERSION or details is None:
    ...
    if (
        ...
        return make_create_placement_binder_success(str(binder_name))
    ...
    if (
        ...
        if _valid_uncertain(response):
            ...
            return make_create_placement_binder_uncertain(error_code, error, committed=committed, **details)
    return _invalid_response(response)


__all__ = [
    "CREATE_PLACEMENT_BINDER_CONTRACT_VERSION",
