"""Versioned, stdlib-only wire contract for ``recompute_and_wait``."""
...
RECOMPUTE_AND_WAIT_CONTRACT_VERSION: Literal[1] = 1

DocumentName = NewType("DocumentName", str)


class RecomputeAndWaitReadDocument(Protocol):
...
class RecomputeAndWaitDocument(RecomputeAndWaitReadDocument, Protocol):
    """Narrow mutation surface available only to the apply callback."""

    def addObject(self, object_type: str, name: str) -> object: ...


class RecomputeAndWaitCollaborators(Protocol):
...
class RecomputeAndWaitFailure(TypedDict):
...
class RecomputeAndWaitUncertain(TypedDict):
...
RecomputeAndWaitResult = RecomputeAndWaitSuccess | RecomputeAndWaitFailure | RecomputeAndWaitUncertain
...
def make_recompute_and_wait_success(document_name: DocumentName, settled: bool) -> RecomputeAndWaitSuccess:
    """Construct a complete committed result."""

    return {
        "contract_version": RECOMPUTE_AND_WAIT_CONTRACT_VERSION,
        "success": True,
        "ok": True,
        "outcome": "committed",
        "committed": True,
...
def make_recompute_and_wait_failure(
    error_code: str,
    error: str,
    *,
    ...
    result: RecomputeAndWaitFailure = {
        "contract_version": RECOMPUTE_AND_WAIT_CONTRACT_VERSION,
        "success": False,
        "ok": False,
        "outcome": "rejected",
...
def make_recompute_and_wait_uncertain(
    error_code: str,
    error: str,
    *,
    committed: bool | None,
    ...
    result: RecomputeAndWaitUncertain = {
        "contract_version": RECOMPUTE_AND_WAIT_CONTRACT_VERSION,
        "success": False,
        "ok": False,
        "outcome": "uncertain",
...
def _invalid_response(response: dict[str, object]) -> RecomputeAndWaitUncertain:
    ...
    code = (
        ...
        if committed
        else "RECOMPUTE_AND_WAIT_ROLLBACK_UNCERTAIN"
        if rollback_failed
        else "INVALID_RECOMPUTE_AND_WAIT_RESPONSE"
    )
    return make_recompute_and_wait_uncertain(
        code,
        "recompute_and_wait returned an invalid contract response; document state requires reconciliation",
        committed=True if committed else None,
        diagnostics={"response": response},
    )


def parse_recompute_and_wait_response(raw_response: object) -> RecomputeAndWaitResult:
    """Validate all three wire variants; unknown state always stays uncertain."""
    response = _response_object(raw_response)
    if response is None:
        return make_recompute_and_wait_uncertain(
            "INVALID_RPC_RESPONSE",
            "recompute_and_wait returned a non-object response",
            committed=None,
        )
    version = response.get("contract_version")
    details = _response_details(response)
    if type(version) is not int or version != RECOMPUTE_AND_WAIT_CONTRACT_VERSION or details is None:
    ...
    if (
        ...
        and isinstance(settled, bool)
    ):
        return make_recompute_and_wait_success(DocumentName(document_name), settled)
    ...
    if (
        ...
        and error.strip()
        and "document_name" not in response and "settled" not in response
    ):
        ...
        if _valid_uncertain(response):
            ...
            return make_recompute_and_wait_uncertain(error_code, error, committed=committed, **details)
    return _invalid_response(response)


__all__ = [
    "RECOMPUTE_AND_WAIT_CONTRACT_VERSION",
    "RecomputeAndWaitCollaborators",
    "RecomputeAndWaitDocument",
    "RecomputeAndWaitFailure",
