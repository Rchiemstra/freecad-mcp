"""Versioned, stdlib-only wire contract for ``reload_document``."""
...
RELOAD_DOCUMENT_CONTRACT_VERSION: Literal[1] = 1

DocumentName = NewType("DocumentName", str)


class ReloadDocumentReadDocument(Protocol):
...
class ReloadDocumentDocument(ReloadDocumentReadDocument, Protocol):
    """Narrow mutation surface available only to the apply callback."""

    def addObject(self, object_type: str, name: str) -> object: ...


class ReloadDocumentCollaborators(Protocol):
    """Complete typed boundary between the handler and native bridge."""

    def validate_document_invariants(self, document: ReloadDocumentReadDocument) -> object:
...
class ReloadDocumentSuccess(TypedDict):
...
class ReloadDocumentUncertain(TypedDict):
...
ReloadDocumentResult = ReloadDocumentSuccess | ReloadDocumentFailure | ReloadDocumentUncertain
...
def make_reload_document_uncertain(
    error_code: str,
    error: str,
    *,
    committed: bool | None,
    ...
    result: ReloadDocumentUncertain = {
        "contract_version": RELOAD_DOCUMENT_CONTRACT_VERSION,
        "success": False,
        "ok": False,
        "outcome": "uncertain",
...
def _invalid_response(response: dict[str, object]) -> ReloadDocumentUncertain:
    ...
    code = (
        ...
        if committed
        else "RELOAD_DOCUMENT_ROLLBACK_UNCERTAIN"
        if rollback_failed
        else "INVALID_RELOAD_DOCUMENT_RESPONSE"
    )
    return make_reload_document_uncertain(
        code,
        "reload_document returned an invalid contract response; document state requires reconciliation",
        committed=True if committed else None,
        diagnostics={"response": response},
    )


def parse_reload_document_response(raw_response: object) -> ReloadDocumentResult:
    """Validate all three wire variants; unknown state always stays uncertain."""
    response = _response_object(raw_response)
    if response is None:
        return make_reload_document_uncertain(
            "INVALID_RPC_RESPONSE",
            "reload_document returned a non-object response",
            committed=None,
        )
    version = response.get("contract_version")
    details = _response_details(response)
    if type(version) is not int or version != RELOAD_DOCUMENT_CONTRACT_VERSION or details is None:
    ...
    if (
        ...
        if _valid_uncertain(response):
            ...
            return make_reload_document_uncertain(error_code, error, committed=committed, **details)
    return _invalid_response(response)


__all__ = [
    "RELOAD_DOCUMENT_CONTRACT_VERSION",
    "ReloadDocumentCollaborators",
    "ReloadDocumentDocument",
    "ReloadDocumentFailure",
    "ReloadDocumentReadDocument",
