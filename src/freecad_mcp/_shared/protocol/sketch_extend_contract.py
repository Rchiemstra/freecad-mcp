"""Versioned, stdlib-only wire contract for ``sketch_extend``."""
...
SKETCH_EXTEND_CONTRACT_VERSION: Literal[1] = 1
...
class SketchExtendCollaborators(Protocol):
    @property
...
class SketchExtendSuccess(TypedDict):
...
class SketchExtendFailure(TypedDict):
    contract_version: Literal[1]
...
class SketchExtendUncertain(TypedDict):
    contract_version: Literal[1]
    success: Literal[False]
    ok: Literal[False]
...
SketchExtendResult = SketchExtendSuccess | SketchExtendFailure | SketchExtendUncertain
...
def make_sketch_extend_success(sketch: SketchName) -> SketchExtendSuccess:
    return {
        "contract_version": SKETCH_EXTEND_CONTRACT_VERSION,
        "success": True,
        "ok": True,
        "outcome": "committed",
        "committed": True,
...
def make_sketch_extend_failure(
    error_code: str,
    error: str,
    *,
...
def make_sketch_extend_uncertain(
    error_code: str,
    error: str,
    *,
    committed: bool | None,
    ...
    rollback_failed: bool | None = None,
    diagnostics: dict[str, object] | None = None,
) -> SketchExtendUncertain:
    result: SketchExtendUncertain = {
        "contract_version": SKETCH_EXTEND_CONTRACT_VERSION,
        "success": False,
        "ok": False,
        "outcome": "uncertain",
...
def _invalid_response(response: dict[str, object]) -> SketchExtendUncertain:
    ...
    code = (
        ...
        if committed
        else "SKETCH_EXTEND_ROLLBACK_UNCERTAIN"
        if rollback_failed
        else "INVALID_SKETCH_EXTEND_RESPONSE"
    )
    return make_sketch_extend_uncertain(
        code,
        "sketch_extend returned an invalid contract response; document state requires reconciliation",
        committed=True if committed else None,
        diagnostics={"response": response},
    )


def parse_sketch_extend_response(raw_response: object) -> SketchExtendResult:
    response = _response_object(raw_response)
    if response is None:
        return make_sketch_extend_uncertain(
            "INVALID_RPC_RESPONSE",
            "sketch_extend returned a non-object response",
            committed=None,
        )
    version = response.get("contract_version")
    details = _response_details(response)
    if type(version) is not int or version != SKETCH_EXTEND_CONTRACT_VERSION or details is None:
    ...
    if (
        ...
        if _valid_rejection(response):
            return make_sketch_extend_failure(
                error_code,
                error,
        ...
        if _valid_uncertain(response):
            ...
            return make_sketch_extend_uncertain(error_code, error, committed=committed, **details)
    return _invalid_response(response)


__all__ = [
    "SKETCH_EXTEND_CONTRACT_VERSION",
