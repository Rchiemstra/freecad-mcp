"""Versioned, stdlib-only wire contract for ``restore``."""
...
RESTORE_CONTRACT_VERSION: Literal[1] = 1

DocumentName = NewType("DocumentName", str)
RestoreName = NewType("RestoreName", str)


class RestoreObject(Protocol):
...
class RestoreReadDocument(Protocol):
    """Read-only document surface available after native recompute."""

    @property
    def Name(self) -> str: ...

    def getObject(self, name: str) -> RestoreObject | None: ...


class RestoreDocument(RestoreReadDocument, Protocol):
    """Narrow mutation surface available only to the apply callback."""

    def addObject(self, object_type: str, name: str) -> RestoreObject: ...


@runtime_checkable
class NativeRestoreDocument(RestoreDocument, Protocol):
...
class RestoreCollaborators(Protocol):
...
class RestoreSuccess(TypedDict):
...
class RestoreUncertain(TypedDict):
...
RestoreResult = RestoreSuccess | RestoreFailure | RestoreUncertain
...
def _invalid_response(response: dict[str, object]) -> RestoreUncertain:
    ...
    code = (
        ...
        if committed
        else "RESTORE_ROLLBACK_UNCERTAIN"
        if rollback_failed
        else "INVALID_RESTORE_RESPONSE"
    )
    return make_restore_uncertain(
        code,
        "restore returned an invalid contract response; document state requires reconciliation",
        committed=True if committed else None,
        diagnostics={"response": response},
    )


def parse_restore_response(raw_response: object) -> RestoreResult:
    """Validate all three wire variants; unknown state always stays uncertain."""
    response = _response_object(raw_response)
    if response is None:
        return make_restore_uncertain(
            "INVALID_RPC_RESPONSE",
            "restore returned a non-object response",
            committed=None,
        )
    version = response.get("contract_version")
    details = _response_details(response)
    if type(version) is not int or version != RESTORE_CONTRACT_VERSION or details is None:
    ...
    if (
        ...
        if _valid_uncertain(response):
            ...
            return make_restore_uncertain(error_code, error, committed=committed, **details)
    return _invalid_response(response)
