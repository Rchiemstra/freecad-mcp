"""Versioned, stdlib-only wire contract for ``sketch_constrain_parallel``."""
...
SKETCH_CONSTRAIN_PARALLEL_CONTRACT_VERSION: Literal[1] = 1
...
class SketchConstrainParallelCollaborators(Protocol):
    @property
...
class SketchConstrainParallelRequest:
...
class SketchConstrainParallelSuccess(TypedDict):
...
class SketchConstrainParallelFailure(TypedDict):
    contract_version: Literal[1]
...
class SketchConstrainParallelUncertain(TypedDict):
    contract_version: Literal[1]
    success: Literal[False]
    ok: Literal[False]
...
SketchConstrainParallelResult = SketchConstrainParallelSuccess | SketchConstrainParallelFailure | SketchConstrainParallelUncertain
...
def make_sketch_constrain_parallel_success(sketch: SketchName, constraint_index: int) -> SketchConstrainParallelSuccess:
    return {
        "contract_version": SKETCH_CONSTRAIN_PARALLEL_CONTRACT_VERSION,
        "success": True,
        "ok": True,
        "outcome": "committed",
        "committed": True,
...
def make_sketch_constrain_parallel_failure(
    error_code: str,
    error: str,
    *,
    ...
    result: SketchConstrainParallelFailure = {
        "contract_version": SKETCH_CONSTRAIN_PARALLEL_CONTRACT_VERSION,
        "success": False,
        "ok": False,
        "outcome": "rejected",
...
def make_sketch_constrain_parallel_uncertain(
    error_code: str,
    error: str,
    *,
    committed: bool | None,
    ...
    result: SketchConstrainParallelUncertain = {
        "contract_version": SKETCH_CONSTRAIN_PARALLEL_CONTRACT_VERSION,
        "success": False,
        "ok": False,
        "outcome": "uncertain",
...
def _invalid_response(response: dict[str, object]) -> SketchConstrainParallelUncertain:
    ...
    code = (
        ...
        if committed
        else "SKETCH_CONSTRAIN_PARALLEL_ROLLBACK_UNCERTAIN"
        if rollback_failed
        else "INVALID_SKETCH_CONSTRAIN_PARALLEL_RESPONSE"
    )
    return make_sketch_constrain_parallel_uncertain(
        code,
        "sketch_constrain_parallel returned an invalid contract response; document state requires reconciliation",
        committed=True if committed else None,
        diagnostics={"response": response},
    )


def parse_sketch_constrain_parallel_response(raw_response: object) -> SketchConstrainParallelResult:
    response = _response_object(raw_response)
    if response is None:
        return make_sketch_constrain_parallel_uncertain(
            "INVALID_RPC_RESPONSE",
            "sketch_constrain_parallel returned a non-object response",
            committed=None,
        )
    version = response.get("contract_version")
    details = _response_details(response)
    if type(version) is not int or version != SKETCH_CONSTRAIN_PARALLEL_CONTRACT_VERSION or details is None:
    ...
    if (
        ...
        return make_sketch_constrain_parallel_success(SketchName(sketch), constraint_index)
    ...
    if (
        ...
        if _valid_rejection(response):
            return make_sketch_constrain_parallel_failure(
                error_code,
                error,
        ...
        if _valid_uncertain(response):
            ...
            return make_sketch_constrain_parallel_uncertain(error_code, error, committed=committed, **details)
    return _invalid_response(response)


__all__ = [
    "SKETCH_CONSTRAIN_PARALLEL_CONTRACT_VERSION",
    "SketchConstrainParallelCollaborators",
    "SketchConstrainParallelFailure",
    "SketchConstrainParallelRequest",
    "SketchConstrainParallelResult",
    "SketchConstrainParallelSuccess",
    "SketchConstrainParallelUncertain",
    "DocumentName",
