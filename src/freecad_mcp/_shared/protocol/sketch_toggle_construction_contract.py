"""Versioned, stdlib-only wire contract for ``sketch_toggle_construction``."""
...
SKETCH_TOGGLE_CONSTRUCTION_CONTRACT_VERSION: Literal[1] = 1
...
class SketchToggleConstructionCollaborators(Protocol):
    @property
...
class SketchToggleConstructionRequest:
    doc_name: DocumentName
    sketch_name: SketchName
    geo_indices: tuple[int, ...]
    construction: bool


class SketchToggleConstructionSuccess(TypedDict):
...
class SketchToggleConstructionFailure(TypedDict):
    contract_version: Literal[1]
...
class SketchToggleConstructionUncertain(TypedDict):
    contract_version: Literal[1]
    success: Literal[False]
    ok: Literal[False]
...
SketchToggleConstructionResult = SketchToggleConstructionSuccess | SketchToggleConstructionFailure | SketchToggleConstructionUncertain
...
def make_sketch_toggle_construction_success(sketch: SketchName) -> SketchToggleConstructionSuccess:
    return {
        "contract_version": SKETCH_TOGGLE_CONSTRUCTION_CONTRACT_VERSION,
        "success": True,
        "ok": True,
        "outcome": "committed",
        "committed": True,
...
def make_sketch_toggle_construction_failure(
    error_code: str,
    error: str,
    *,
    ...
    result: SketchToggleConstructionFailure = {
        "contract_version": SKETCH_TOGGLE_CONSTRUCTION_CONTRACT_VERSION,
        "success": False,
        "ok": False,
        "outcome": "rejected",
...
def make_sketch_toggle_construction_uncertain(
    error_code: str,
    error: str,
    *,
    committed: bool | None,
    ...
    result: SketchToggleConstructionUncertain = {
        "contract_version": SKETCH_TOGGLE_CONSTRUCTION_CONTRACT_VERSION,
        "success": False,
        "ok": False,
        "outcome": "uncertain",
...
def _invalid_response(response: dict[str, object]) -> SketchToggleConstructionUncertain:
    ...
    code = (
        ...
        if committed
        else "SKETCH_TOGGLE_CONSTRUCTION_ROLLBACK_UNCERTAIN"
        if rollback_failed
        else "INVALID_SKETCH_TOGGLE_CONSTRUCTION_RESPONSE"
    )
    return make_sketch_toggle_construction_uncertain(
        code,
        "sketch_toggle_construction returned an invalid contract response; document state requires reconciliation",
        committed=True if committed else None,
        diagnostics={"response": response},
    )


def parse_sketch_toggle_construction_response(raw_response: object) -> SketchToggleConstructionResult:
    response = _response_object(raw_response)
    if response is None:
        return make_sketch_toggle_construction_uncertain(
            "INVALID_RPC_RESPONSE",
            "sketch_toggle_construction returned a non-object response",
            committed=None,
        )
    version = response.get("contract_version")
    details = _response_details(response)
    if type(version) is not int or version != SKETCH_TOGGLE_CONSTRUCTION_CONTRACT_VERSION or details is None:
        return _invalid_response(response)

    sketch = response.get("sketch")
    if _valid_success(response) and isinstance(sketch, str) and sketch.strip():
        return make_sketch_toggle_construction_success(SketchName(sketch))
    ...
    if (
        ...
        if _valid_rejection(response):
            return make_sketch_toggle_construction_failure(
                error_code,
                error,
        ...
        if _valid_uncertain(response):
            ...
            return make_sketch_toggle_construction_uncertain(error_code, error, committed=committed, **details)
    return _invalid_response(response)


__all__ = [
    "SKETCH_TOGGLE_CONSTRUCTION_CONTRACT_VERSION",
