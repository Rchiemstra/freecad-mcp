"""Versioned, stdlib-only wire contract for ``linear_pattern_feature``."""
...
LINEAR_PATTERN_FEATURE_CONTRACT_VERSION: Literal[1] = 1
...
class LinearPatternFeatureCollaborators(Protocol):
...
class LinearPatternFeatureRequest:
    """Validated internal request."""

    doc_name: DocumentName
    feature_name: FeatureName
    pattern_name: FeatureName
    length: float
    occurrences: int
    direction: str
...
class LinearPatternFeatureSuccess(TypedDict):
...
class LinearPatternFeatureFailure(TypedDict):
...
class LinearPatternFeatureUncertain(TypedDict):
...
LinearPatternFeatureResult = LinearPatternFeatureSuccess | LinearPatternFeatureFailure | LinearPatternFeatureUncertain
...
def make_linear_pattern_feature_success(feature: FeatureName, label: str) -> LinearPatternFeatureSuccess:
    """Construct a complete committed result using the actual assigned name."""

    return {
        "contract_version": LINEAR_PATTERN_FEATURE_CONTRACT_VERSION,
        "success": True,
        "ok": True,
        "outcome": "committed",
        "committed": True,
...
def make_linear_pattern_feature_failure(
    error_code: str,
    error: str,
    *,
    ...
    result: LinearPatternFeatureFailure = {
        "contract_version": LINEAR_PATTERN_FEATURE_CONTRACT_VERSION,
        "success": False,
        "ok": False,
        "outcome": "rejected",
...
def make_linear_pattern_feature_uncertain(
    error_code: str,
    error: str,
    *,
    committed: bool | None,
    ...
    result: LinearPatternFeatureUncertain = {
        "contract_version": LINEAR_PATTERN_FEATURE_CONTRACT_VERSION,
        "success": False,
        "ok": False,
        "outcome": "uncertain",
...
def _invalid_response(response: dict[str, object]) -> LinearPatternFeatureUncertain:
    ...
    code = (
        ...
        if committed
        else "LINEAR_PATTERN_FEATURE_ROLLBACK_UNCERTAIN"
        if rollback_failed
        else "INVALID_LINEAR_PATTERN_FEATURE_RESPONSE"
    )
    return make_linear_pattern_feature_uncertain(
        code,
        "linear_pattern_feature returned an invalid contract response; document state requires reconciliation",
        committed=True if committed else None,
        diagnostics={"response": response},
    )


def parse_linear_pattern_feature_response(raw_response: object) -> LinearPatternFeatureResult:
    """Validate all three wire variants; unknown state always stays uncertain."""
    response = _response_object(raw_response)
    if response is None:
        return make_linear_pattern_feature_uncertain(
            "INVALID_RPC_RESPONSE",
            "linear_pattern_feature returned a non-object response",
            committed=None,
        )
    version = response.get("contract_version")
    details = _response_details(response)
    if type(version) is not int or version != LINEAR_PATTERN_FEATURE_CONTRACT_VERSION or details is None:
    ...
    if (
        ...
        return make_linear_pattern_feature_success(FeatureName(feature), label)
    ...
    if (
        ...
        if _valid_uncertain(response):
            ...
            return make_linear_pattern_feature_uncertain(error_code, error, committed=committed, **details)
    return _invalid_response(response)


__all__ = [
    "LINEAR_PATTERN_FEATURE_CONTRACT_VERSION",
