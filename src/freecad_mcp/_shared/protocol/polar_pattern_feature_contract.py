"""Versioned, stdlib-only wire contract for ``polar_pattern_feature``."""
...
POLAR_PATTERN_FEATURE_CONTRACT_VERSION: Literal[1] = 1
...
class PolarPatternFeatureCollaborators(Protocol):
...
class PolarPatternFeatureRequest:
...
class PolarPatternFeatureSuccess(TypedDict):
...
class PolarPatternFeatureFailure(TypedDict):
...
class PolarPatternFeatureUncertain(TypedDict):
...
PolarPatternFeatureResult = PolarPatternFeatureSuccess | PolarPatternFeatureFailure | PolarPatternFeatureUncertain
...
def make_polar_pattern_feature_success(feature: FeatureName, label: str) -> PolarPatternFeatureSuccess:
    """Construct a complete committed result using the actual assigned name."""

    return {
        "contract_version": POLAR_PATTERN_FEATURE_CONTRACT_VERSION,
        "success": True,
        "ok": True,
        "outcome": "committed",
        "committed": True,
...
def make_polar_pattern_feature_failure(
    error_code: str,
    error: str,
    *,
    ...
    result: PolarPatternFeatureFailure = {
        "contract_version": POLAR_PATTERN_FEATURE_CONTRACT_VERSION,
        "success": False,
        "ok": False,
        "outcome": "rejected",
...
def make_polar_pattern_feature_uncertain(
    error_code: str,
    error: str,
    *,
    committed: bool | None,
    ...
    result: PolarPatternFeatureUncertain = {
        "contract_version": POLAR_PATTERN_FEATURE_CONTRACT_VERSION,
        "success": False,
        "ok": False,
        "outcome": "uncertain",
...
def _invalid_response(response: dict[str, object]) -> PolarPatternFeatureUncertain:
    ...
    code = (
        ...
        if committed
        else "POLAR_PATTERN_FEATURE_ROLLBACK_UNCERTAIN"
        if rollback_failed
        else "INVALID_POLAR_PATTERN_FEATURE_RESPONSE"
    )
    return make_polar_pattern_feature_uncertain(
        code,
        "polar_pattern_feature returned an invalid contract response; document state requires reconciliation",
        committed=True if committed else None,
        diagnostics={"response": response},
    )


def parse_polar_pattern_feature_response(raw_response: object) -> PolarPatternFeatureResult:
    """Validate all three wire variants; unknown state always stays uncertain."""
    response = _response_object(raw_response)
    if response is None:
        return make_polar_pattern_feature_uncertain(
            "INVALID_RPC_RESPONSE",
            "polar_pattern_feature returned a non-object response",
            committed=None,
        )
    version = response.get("contract_version")
    details = _response_details(response)
    if type(version) is not int or version != POLAR_PATTERN_FEATURE_CONTRACT_VERSION or details is None:
    ...
    if (
        ...
        return make_polar_pattern_feature_success(FeatureName(feature), label)
    ...
    if (
        ...
        if _valid_uncertain(response):
            ...
            return make_polar_pattern_feature_uncertain(error_code, error, committed=committed, **details)
    return _invalid_response(response)


__all__ = [
    "POLAR_PATTERN_FEATURE_CONTRACT_VERSION",
