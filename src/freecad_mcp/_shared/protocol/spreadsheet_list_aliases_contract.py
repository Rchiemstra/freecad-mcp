"""Versioned, stdlib-only wire contract for ``spreadsheet_list_aliases``."""
...
SPREADSHEET_LIST_ALIASES_CONTRACT_VERSION: Literal[1] = 1

DocumentName = NewType("DocumentName", str)
SpreadsheetListAliasesName = NewType("SpreadsheetListAliasesName", str)


class SpreadsheetListAliasesObject(Protocol):
...
class SpreadsheetListAliasesReadDocument(Protocol):
    """Read-only document surface available after native recompute."""

    @property
    def Name(self) -> str: ...

    def getObject(self, name: str) -> SpreadsheetListAliasesObject | None: ...


class SpreadsheetListAliasesDocument(SpreadsheetListAliasesReadDocument, Protocol):
    """Narrow mutation surface available only to the apply callback."""

    def addObject(self, object_type: str, name: str) -> SpreadsheetListAliasesObject: ...


@runtime_checkable
class NativeSpreadsheetListAliasesDocument(SpreadsheetListAliasesDocument, Protocol):
...
class SpreadsheetListAliasesCollaborators(Protocol):
    """Complete typed boundary between the handler and native bridge."""

    def validate_document_invariants(self, document: SpreadsheetListAliasesReadDocument) -> object:
...
class SpreadsheetListAliasesRequest:
...
class SpreadsheetListAliasesSuccess(TypedDict):
...
class SpreadsheetListAliasesUncertain(TypedDict):
...
SpreadsheetListAliasesResult = SpreadsheetListAliasesSuccess | SpreadsheetListAliasesFailure | SpreadsheetListAliasesUncertain
...
def make_spreadsheet_list_aliases_failure(
    error_code: str,
    error: str,
    *,
    ...
    result: SpreadsheetListAliasesFailure = {
        "contract_version": SPREADSHEET_LIST_ALIASES_CONTRACT_VERSION,
        "success": False,
        "ok": False,
        "outcome": "rejected",
...
def make_spreadsheet_list_aliases_uncertain(
    error_code: str,
    error: str,
    *,
    committed: bool | None,
    ...
    rollback_failed: bool | None = None,
    diagnostics: dict[str, object] | None = None,
) -> SpreadsheetListAliasesUncertain:
    """Construct a result that explicitly forbids automatic replay."""

    result: SpreadsheetListAliasesUncertain = {
        "contract_version": SPREADSHEET_LIST_ALIASES_CONTRACT_VERSION,
        "success": False,
        "ok": False,
        "outcome": "uncertain",
...
def _invalid_response(response: dict[str, object]) -> SpreadsheetListAliasesUncertain:
    ...
    code = (
        ...
        if committed
        else "SPREADSHEET_LIST_ALIASES_ROLLBACK_UNCERTAIN"
        if rollback_failed
        else "INVALID_SPREADSHEET_LIST_ALIASES_RESPONSE"
    )
    return make_spreadsheet_list_aliases_uncertain(
        code,
        "spreadsheet_list_aliases returned an invalid contract response; document state requires reconciliation",
        committed=True if committed else None,
        diagnostics={"response": response},
    )


def parse_spreadsheet_list_aliases_response(raw_response: object) -> SpreadsheetListAliasesResult:
    """Validate all three wire variants; unknown state always stays uncertain."""
    response = _response_object(raw_response)
    if response is None:
        return make_spreadsheet_list_aliases_uncertain(
            "INVALID_RPC_RESPONSE",
            "spreadsheet_list_aliases returned a non-object response",
            committed=None,
        )
    ...
    if (
        ...
        if _valid_uncertain(response):
            ...
            return make_spreadsheet_list_aliases_uncertain(error_code, error, committed=committed, **details)
    return _invalid_response(response)


__all__ = [
    "SPREADSHEET_LIST_ALIASES_CONTRACT_VERSION",
    "SpreadsheetListAliasesCollaborators",
    "SpreadsheetListAliasesFailure",
    "SpreadsheetListAliasesRequest",
    "SpreadsheetListAliasesResult",
    "SpreadsheetListAliasesSuccess",
    "SpreadsheetListAliasesUncertain",
    "SpreadsheetListAliasesDocument",
    "SpreadsheetListAliasesName",
    "SpreadsheetListAliasesObject",
