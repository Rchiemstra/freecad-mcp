"""Versioned, stdlib-only wire contract for ``create_datum_plane``."""
...
CREATE_DATUM_PLANE_CONTRACT_VERSION: Literal[1] = 1

DocumentName = NewType("DocumentName", str)
CreateDatumPlaneName = NewType("CreateDatumPlaneName", str)


class CreateDatumPlaneObject(Protocol):
...
class CreateDatumPlaneReadDocument(Protocol):
    """Read-only document surface available after native recompute."""

    @property
    def Name(self) -> str: ...

    def getObject(self, name: str) -> CreateDatumPlaneObject | None: ...


class CreateDatumPlaneDocument(CreateDatumPlaneReadDocument, Protocol):
    """Narrow mutation surface available only to the apply callback."""

    def addObject(self, object_type: str, name: str) -> CreateDatumPlaneObject: ...


@runtime_checkable
class NativeCreateDatumPlaneDocument(CreateDatumPlaneDocument, Protocol):
...
class CreateDatumPlaneCollaborators(Protocol):
...
class CreateDatumPlaneRequest:
    ...
    face_a: str | None
    face_b: str | None
    offset_along_normal: object
    map_mode: str
    if_exists: str


class CreateDatumPlaneSuccess(TypedDict):
...
class CreateDatumPlaneUncertain(TypedDict):
...
CreateDatumPlaneResult = CreateDatumPlaneSuccess | CreateDatumPlaneFailure | CreateDatumPlaneUncertain
...
def make_create_datum_plane_failure(
    ...
    *,
    ...
    result: CreateDatumPlaneFailure = {
        "contract_version": CREATE_DATUM_PLANE_CONTRACT_VERSION,
        "success": False,
        "ok": False,
        "outcome": "rejected",
...
def make_create_datum_plane_uncertain(
    error_code: str,
    error: str,
    *,
    committed: bool | None,
    ...
    result: CreateDatumPlaneUncertain = {
        "contract_version": CREATE_DATUM_PLANE_CONTRACT_VERSION,
        "success": False,
        "ok": False,
        "outcome": "uncertain",
...
def _invalid_response(response: dict[str, object]) -> CreateDatumPlaneUncertain:
    ...
    code = (
        ...
        if committed
        else "CREATE_DATUM_PLANE_ROLLBACK_UNCERTAIN"
        if rollback_failed
        else "INVALID_CREATE_DATUM_PLANE_RESPONSE"
    )
    return make_create_datum_plane_uncertain(
        code,
        "create_datum_plane returned an invalid contract response; document state requires reconciliation",
        committed=True if committed else None,
        diagnostics={"response": response},
    )


def parse_create_datum_plane_response(raw_response: object) -> CreateDatumPlaneResult:
    """Validate all three wire variants; unknown state always stays uncertain."""
    response = _response_object(raw_response)
    if response is None:
        return make_create_datum_plane_uncertain(
            "INVALID_RPC_RESPONSE",
            "create_datum_plane returned a non-object response",
            committed=None,
        )
    version = response.get("contract_version")
    details = _response_details(response)
    if type(version) is not int or version != CREATE_DATUM_PLANE_CONTRACT_VERSION or details is None:
        return _invalid_response(response)

    plane_name = response.get('plane_name')
    body_name = response.get('body_name')
    if (
    ...
    if (
        ...
        and error.strip()
        and 'plane_name' not in response and 'body_name' not in response
    ):
        ...
        if _valid_uncertain(response):
            ...
            return make_create_datum_plane_uncertain(error_code, error, committed=committed, **details)
    return _invalid_response(response)


__all__ = [
    "CREATE_DATUM_PLANE_CONTRACT_VERSION",
