"""Versioned, stdlib-only wire contract for ``run_fem_analysis``."""
...
RUN_FEM_ANALYSIS_CONTRACT_VERSION: Literal[1] = 1

DocumentName = NewType("DocumentName", str)
RunFemAnalysisName = NewType("RunFemAnalysisName", str)


class RunFemAnalysisObject(Protocol):
...
class RunFemAnalysisReadDocument(Protocol):
    """Read-only document surface available after native recompute."""

    @property
    def Name(self) -> str: ...

    def getObject(self, name: str) -> RunFemAnalysisObject | None: ...


class RunFemAnalysisDocument(RunFemAnalysisReadDocument, Protocol):
    """Narrow mutation surface available only to the apply callback."""

    def addObject(self, object_type: str, name: str) -> RunFemAnalysisObject: ...


@runtime_checkable
class NativeRunFemAnalysisDocument(RunFemAnalysisDocument, Protocol):
...
class RunFemAnalysisCollaborators(Protocol):
...
class RunFemAnalysisSuccess(TypedDict):
...
class RunFemAnalysisFailure(TypedDict):
...
class RunFemAnalysisUncertain(TypedDict):
...
RunFemAnalysisResult = RunFemAnalysisSuccess | RunFemAnalysisFailure | RunFemAnalysisUncertain

_CORE_KEYS = frozenset(
    {
        "contract_version", "success", "ok", "outcome", "committed", "retry_safe", 'analysis_name', "error_code", "error", "native_status", "native_message", "rollback_succeeded", "rollback_failed", "diagnostics"
    }
)
...
def make_run_fem_analysis_failure(
    error_code: str,
    error: str,
    *,
...
def make_run_fem_analysis_uncertain(
    error_code: str,
    error: str,
    *,
    committed: bool | None,
    ...
    result: RunFemAnalysisUncertain = {
        "contract_version": RUN_FEM_ANALYSIS_CONTRACT_VERSION,
        "success": False,
        "ok": False,
        "outcome": "uncertain",
...
def _invalid_response(response: dict[str, object]) -> RunFemAnalysisUncertain:
    ...
    code = (
        ...
        if committed
        else "RUN_FEM_ANALYSIS_ROLLBACK_UNCERTAIN"
        if rollback_failed
        else "INVALID_RUN_FEM_ANALYSIS_RESPONSE"
    )
    return make_run_fem_analysis_uncertain(
        code,
        "run_fem_analysis returned an invalid contract response; document state requires reconciliation",
        committed=True if committed else None,
        diagnostics={"response": response},
    )


def parse_run_fem_analysis_response(raw_response: object) -> RunFemAnalysisResult:
    """Validate all three wire variants; unknown state always stays uncertain."""
    response = _response_object(raw_response)
    if response is None:
        return make_run_fem_analysis_uncertain(
            "INVALID_RPC_RESPONSE",
            "run_fem_analysis returned a non-object response",
            committed=None,
        )
    version = response.get("contract_version")
    details = _response_details(response)
    if type(version) is not int or version != RUN_FEM_ANALYSIS_CONTRACT_VERSION or details is None:
        return _invalid_response(response)

    analysis_name = response.get('analysis_name')
    if (
        _valid_success(response)
        and isinstance(analysis_name, str) and analysis_name.strip()
    ):
        return make_run_fem_analysis_success(str(analysis_name))
    ...
    if (
        ...
        and error.strip()
        and 'analysis_name' not in response
    ):
        ...
        if _valid_uncertain(response):
            ...
            return make_run_fem_analysis_uncertain(error_code, error, committed=committed, **details)
    return _invalid_response(response)


__all__ = [
    "RUN_FEM_ANALYSIS_CONTRACT_VERSION",
