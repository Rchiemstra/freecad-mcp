"""Versioned, stdlib-only wire contract for ``sketch_split``."""
...
SKETCH_SPLIT_CONTRACT_VERSION: Literal[1] = 1
...
class SketchSplitCollaborators(Protocol):
    @property
...
class SketchSplitSuccess(TypedDict):
...
class SketchSplitFailure(TypedDict):
    contract_version: Literal[1]
...
class SketchSplitUncertain(TypedDict):
    contract_version: Literal[1]
    success: Literal[False]
    ok: Literal[False]
...
SketchSplitResult = SketchSplitSuccess | SketchSplitFailure | SketchSplitUncertain
...
def make_sketch_split_success(sketch: SketchName) -> SketchSplitSuccess:
    return {
        "contract_version": SKETCH_SPLIT_CONTRACT_VERSION,
        "success": True,
        "ok": True,
        "outcome": "committed",
        "committed": True,
...
def make_sketch_split_uncertain(
    error_code: str,
    error: str,
    *,
    committed: bool | None,
...
def _invalid_response(response: dict[str, object]) -> SketchSplitUncertain:
    ...
    code = (
        ...
        if committed
        else "SKETCH_SPLIT_ROLLBACK_UNCERTAIN"
        if rollback_failed
        else "INVALID_SKETCH_SPLIT_RESPONSE"
    )
    return make_sketch_split_uncertain(
        code,
        "sketch_split returned an invalid contract response; document state requires reconciliation",
        committed=True if committed else None,
        diagnostics={"response": response},
    )


def parse_sketch_split_response(raw_response: object) -> SketchSplitResult:
    response = _response_object(raw_response)
    if response is None:
        return make_sketch_split_uncertain(
            "INVALID_RPC_RESPONSE",
            "sketch_split returned a non-object response",
            committed=None,
        )
    version = response.get("contract_version")
    details = _response_details(response)
    if type(version) is not int or version != SKETCH_SPLIT_CONTRACT_VERSION or details is None:
    ...
    if _valid_success(response) and isinstance(sketch, str) and sketch.strip():
        return make_sketch_split_success(SketchName(sketch))
    ...
    if (
        ...
        if _valid_rejection(response):
            return make_sketch_split_failure(
                error_code,
                error,
        ...
        if _valid_uncertain(response):
            ...
            return make_sketch_split_uncertain(error_code, error, committed=committed, **details)
    return _invalid_response(response)


__all__ = [
    "SKETCH_SPLIT_CONTRACT_VERSION",
