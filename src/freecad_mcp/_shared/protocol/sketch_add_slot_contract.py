"""Versioned, stdlib-only wire contract for ``sketch_add_slot``."""
...
SKETCH_ADD_SLOT_CONTRACT_VERSION: Literal[1] = 1
...
class SketchAddSlotCollaborators(Protocol):
    @property
...
class SketchAddSlotSuccess(TypedDict):
...
class SketchAddSlotFailure(TypedDict):
    contract_version: Literal[1]
...
class SketchAddSlotUncertain(TypedDict):
    contract_version: Literal[1]
    success: Literal[False]
    ok: Literal[False]
...
SketchAddSlotResult = SketchAddSlotSuccess | SketchAddSlotFailure | SketchAddSlotUncertain
...
def make_sketch_add_slot_success(sketch: SketchName, geometry_indices: list[int]) -> SketchAddSlotSuccess:
    return {
        "contract_version": SKETCH_ADD_SLOT_CONTRACT_VERSION,
        "success": True,
        "ok": True,
        "outcome": "committed",
        "committed": True,
...
def make_sketch_add_slot_failure(
    error_code: str,
    error: str,
    *,
...
def make_sketch_add_slot_uncertain(
    error_code: str,
    error: str,
    *,
    committed: bool | None,
    ...
    rollback_failed: bool | None = None,
    diagnostics: dict[str, object] | None = None,
) -> SketchAddSlotUncertain:
    result: SketchAddSlotUncertain = {
        "contract_version": SKETCH_ADD_SLOT_CONTRACT_VERSION,
        "success": False,
        "ok": False,
        "outcome": "uncertain",
...
def _invalid_response(response: dict[str, object]) -> SketchAddSlotUncertain:
    ...
    code = (
        ...
        if committed
        else "SKETCH_ADD_SLOT_ROLLBACK_UNCERTAIN"
        if rollback_failed
        else "INVALID_SKETCH_ADD_SLOT_RESPONSE"
    )
    return make_sketch_add_slot_uncertain(
        code,
        "sketch_add_slot returned an invalid contract response; document state requires reconciliation",
        committed=True if committed else None,
        diagnostics={"response": response},
    )


def parse_sketch_add_slot_response(raw_response: object) -> SketchAddSlotResult:
    response = _response_object(raw_response)
    if response is None:
        return make_sketch_add_slot_uncertain(
            "INVALID_RPC_RESPONSE",
            "sketch_add_slot returned a non-object response",
            committed=None,
        )
    version = response.get("contract_version")
    details = _response_details(response)
    if type(version) is not int or version != SKETCH_ADD_SLOT_CONTRACT_VERSION or details is None:
    ...
    if (
        ...
        return make_sketch_add_slot_success(SketchName(sketch), list(geometry_indices))
    ...
    if (
        ...
        if _valid_rejection(response):
            return make_sketch_add_slot_failure(
                error_code,
                error,
        ...
        if _valid_uncertain(response):
            ...
            return make_sketch_add_slot_uncertain(error_code, error, committed=committed, **details)
    return _invalid_response(response)


__all__ = [
    "SKETCH_ADD_SLOT_CONTRACT_VERSION",
    "SketchAddSlotCollaborators",
    "SketchAddSlotFailure",
    "SketchAddSlotRequest",
    "SketchAddSlotResult",
    "SketchAddSlotSuccess",
    "SketchAddSlotUncertain",
    "DocumentName",
    ...
    "parse_sketch_add_slot_response",
]
