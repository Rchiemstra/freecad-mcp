"""Versioned, stdlib-only wire contract for ``relink_references``."""
...
RELINK_REFERENCES_CONTRACT_VERSION: Literal[1] = 1

DocumentName = NewType("DocumentName", str)
RelinkReferencesName = NewType("RelinkReferencesName", str)


class RelinkReferencesObject(Protocol):
...
class RelinkReferencesReadDocument(Protocol):
    """Read-only document surface available after native recompute."""

    @property
    def Name(self) -> str: ...

    def getObject(self, name: str) -> RelinkReferencesObject | None: ...


class RelinkReferencesDocument(RelinkReferencesReadDocument, Protocol):
    """Narrow mutation surface available only to the apply callback."""

    def addObject(self, object_type: str, name: str) -> RelinkReferencesObject: ...


@runtime_checkable
class NativeRelinkReferencesDocument(RelinkReferencesDocument, Protocol):
...
class RelinkReferencesCollaborators(Protocol):
...
class RelinkReferencesSuccess(TypedDict):
...
class RelinkReferencesUncertain(TypedDict):
...
RelinkReferencesResult = RelinkReferencesSuccess | RelinkReferencesFailure | RelinkReferencesUncertain
...
def make_relink_references_failure(
    ...
    *,
    ...
    result: RelinkReferencesFailure = {
        "contract_version": RELINK_REFERENCES_CONTRACT_VERSION,
        "success": False,
        "ok": False,
        "outcome": "rejected",
...
def make_relink_references_uncertain(
    error_code: str,
    error: str,
    *,
    committed: bool | None,
    ...
    result: RelinkReferencesUncertain = {
        "contract_version": RELINK_REFERENCES_CONTRACT_VERSION,
        "success": False,
        "ok": False,
        "outcome": "uncertain",
...
def _invalid_response(response: dict[str, object]) -> RelinkReferencesUncertain:
    ...
    code = (
        ...
        if committed
        else "RELINK_REFERENCES_ROLLBACK_UNCERTAIN"
        if rollback_failed
        else "INVALID_RELINK_REFERENCES_RESPONSE"
    )
    return make_relink_references_uncertain(
        code,
        "relink_references returned an invalid contract response; document state requires reconciliation",
        committed=True if committed else None,
        diagnostics={"response": response},
    )


def parse_relink_references_response(raw_response: object) -> RelinkReferencesResult:
    """Validate all three wire variants; unknown state always stays uncertain."""
    response = _response_object(raw_response)
    if response is None:
        return make_relink_references_uncertain(
            "INVALID_RPC_RESPONSE",
            "relink_references returned a non-object response",
            committed=None,
        )
    version = response.get("contract_version")
    details = _response_details(response)
    if type(version) is not int or version != RELINK_REFERENCES_CONTRACT_VERSION or details is None:
        return _invalid_response(response)

    from_obj = response.get('from_obj')
    to_obj = response.get('to_obj')
    if (
    ...
    if (
        ...
        and error.strip()
        and 'from_obj' not in response and 'to_obj' not in response
    ):
        if _valid_rejection(response):
            return make_relink_references_failure(
                error_code,
                error,
        ...
        if _valid_uncertain(response):
            ...
            return make_relink_references_uncertain(error_code, error, committed=committed, **details)
    return _invalid_response(response)


__all__ = [
    "RELINK_REFERENCES_CONTRACT_VERSION",
