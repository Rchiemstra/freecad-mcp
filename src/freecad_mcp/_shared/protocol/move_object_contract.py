"""Versioned, stdlib-only wire contract for ``move_object``."""
...
MOVE_OBJECT_CONTRACT_VERSION: Literal[1] = 1

DocumentName = NewType("DocumentName", str)
MoveObjectName = NewType("MoveObjectName", str)


class MoveObjectObject(Protocol):
...
class MoveObjectReadDocument(Protocol):
    """Read-only document surface available after native recompute."""

    @property
    def Name(self) -> str: ...

    def getObject(self, name: str) -> MoveObjectObject | None: ...


class MoveObjectDocument(MoveObjectReadDocument, Protocol):
    """Narrow mutation surface available only to the apply callback."""

    def addObject(self, object_type: str, name: str) -> MoveObjectObject: ...


@runtime_checkable
class NativeMoveObjectDocument(MoveObjectDocument, Protocol):
...
class MoveObjectCollaborators(Protocol):
...
class MoveObjectRequest:
    ...
    remove_from_old_parent: bool


class MoveObjectSuccess(TypedDict):
...
class MoveObjectFailure(TypedDict):
...
class MoveObjectUncertain(TypedDict):
...
MoveObjectResult = MoveObjectSuccess | MoveObjectFailure | MoveObjectUncertain

_CORE_KEYS = frozenset(
    {
        "contract_version", "success", "ok", "outcome", "committed", "retry_safe", 'object_name', 'target_container', "error_code", "error", "native_status", "native_message", "rollback_succeeded", "rollback_failed", "diagnostics"
    }
)


def make_move_object_success(object_name: str, target_container: str) -> MoveObjectSuccess:
    """Construct a complete committed result."""

    return {
        "contract_version": MOVE_OBJECT_CONTRACT_VERSION,
        "success": True,
        "ok": True,
        "outcome": "committed",
        "committed": True,
...
def make_move_object_uncertain(
    error_code: str,
    error: str,
    *,
    committed: bool | None,
...
def _invalid_response(response: dict[str, object]) -> MoveObjectUncertain:
    ...
    code = (
        ...
        if committed
        else "MOVE_OBJECT_ROLLBACK_UNCERTAIN"
        if rollback_failed
        else "INVALID_MOVE_OBJECT_RESPONSE"
    )
    return make_move_object_uncertain(
        code,
        "move_object returned an invalid contract response; document state requires reconciliation",
        committed=True if committed else None,
        diagnostics={"response": response},
    )


def parse_move_object_response(raw_response: object) -> MoveObjectResult:
    """Validate all three wire variants; unknown state always stays uncertain."""
    response = _response_object(raw_response)
    if response is None:
        return make_move_object_uncertain(
            "INVALID_RPC_RESPONSE",
            "move_object returned a non-object response",
            committed=None,
        )
    version = response.get("contract_version")
    details = _response_details(response)
    if type(version) is not int or version != MOVE_OBJECT_CONTRACT_VERSION or details is None:
        return _invalid_response(response)

    object_name = response.get('object_name')
    target_container = response.get('target_container')
    if (
        _valid_success(response)
        and isinstance(object_name, str) and object_name.strip()
        and isinstance(target_container, str) and target_container.strip()
    ):
        return make_move_object_success(str(object_name), str(target_container))
    ...
    if (
        ...
        and error.strip()
        and 'object_name' not in response and 'target_container' not in response
    ):
        ...
        if _valid_uncertain(response):
            ...
            return make_move_object_uncertain(error_code, error, committed=committed, **details)
    return _invalid_response(response)


__all__ = [
    "MOVE_OBJECT_CONTRACT_VERSION",
