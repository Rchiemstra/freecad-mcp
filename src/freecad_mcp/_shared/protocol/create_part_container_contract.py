"""Versioned, stdlib-only wire contract for ``create_part_container``."""
...
CREATE_PART_CONTAINER_CONTRACT_VERSION: Literal[1] = 1

DocumentName = NewType("DocumentName", str)
CreatePartContainerName = NewType("CreatePartContainerName", str)


class CreatePartContainerObject(Protocol):
...
class CreatePartContainerReadDocument(Protocol):
    """Read-only document surface available after native recompute."""

    @property
    def Name(self) -> str: ...

    def getObject(self, name: str) -> CreatePartContainerObject | None: ...


class CreatePartContainerDocument(CreatePartContainerReadDocument, Protocol):
    """Narrow mutation surface available only to the apply callback."""

    def addObject(self, object_type: str, name: str) -> CreatePartContainerObject: ...


@runtime_checkable
class NativeCreatePartContainerDocument(CreatePartContainerDocument, Protocol):
...
class CreatePartContainerCollaborators(Protocol):
...
class CreatePartContainerRequest:
...
class CreatePartContainerSuccess(TypedDict):
...
class CreatePartContainerFailure(TypedDict):
...
class CreatePartContainerUncertain(TypedDict):
...
CreatePartContainerResult = CreatePartContainerSuccess | CreatePartContainerFailure | CreatePartContainerUncertain

_CORE_KEYS = frozenset(
    {
        "contract_version", "success", "ok", "outcome", "committed", "retry_safe", 'part_name', 'label', "error_code", "error", "native_status", "native_message", "rollback_succeeded", "rollback_failed", "diagnostics"
    }
)


def make_create_part_container_success(part_name: str, label: str) -> CreatePartContainerSuccess:
    """Construct a complete committed result."""

    return {
        "contract_version": CREATE_PART_CONTAINER_CONTRACT_VERSION,
        "success": True,
        "ok": True,
        "outcome": "committed",
        "committed": True,
...
def make_create_part_container_failure(
    error_code: str,
    error: str,
    *,
    ...
    result: CreatePartContainerFailure = {
        "contract_version": CREATE_PART_CONTAINER_CONTRACT_VERSION,
        "success": False,
        "ok": False,
        "outcome": "rejected",
...
def make_create_part_container_uncertain(
    error_code: str,
    error: str,
    *,
    committed: bool | None,
    ...
    result: CreatePartContainerUncertain = {
        "contract_version": CREATE_PART_CONTAINER_CONTRACT_VERSION,
        "success": False,
        "ok": False,
        "outcome": "uncertain",
...
def _invalid_response(response: dict[str, object]) -> CreatePartContainerUncertain:
    ...
    code = (
        ...
        if committed
        else "CREATE_PART_CONTAINER_ROLLBACK_UNCERTAIN"
        if rollback_failed
        else "INVALID_CREATE_PART_CONTAINER_RESPONSE"
    )
    return make_create_part_container_uncertain(
        code,
        "create_part_container returned an invalid contract response; document state requires reconciliation",
        committed=True if committed else None,
        diagnostics={"response": response},
    )


def parse_create_part_container_response(raw_response: object) -> CreatePartContainerResult:
    """Validate all three wire variants; unknown state always stays uncertain."""
    response = _response_object(raw_response)
    if response is None:
        return make_create_part_container_uncertain(
            "INVALID_RPC_RESPONSE",
            "create_part_container returned a non-object response",
            committed=None,
        )
    version = response.get("contract_version")
    details = _response_details(response)
    if type(version) is not int or version != CREATE_PART_CONTAINER_CONTRACT_VERSION or details is None:
        return _invalid_response(response)

    part_name = response.get('part_name')
    label = response.get('label')
    if (
        _valid_success(response)
        and isinstance(part_name, str) and part_name.strip()
        and isinstance(label, str) and label.strip()
    ):
        return make_create_part_container_success(str(part_name), str(label))
    ...
    if (
        ...
        and error.strip()
        and 'part_name' not in response and 'label' not in response
    ):
        if _valid_rejection(response):
            return make_create_part_container_failure(
                error_code,
                error,
        ...
        if _valid_uncertain(response):
            ...
            return make_create_part_container_uncertain(error_code, error, committed=committed, **details)
    return _invalid_response(response)


__all__ = [
    "CREATE_PART_CONTAINER_CONTRACT_VERSION",
