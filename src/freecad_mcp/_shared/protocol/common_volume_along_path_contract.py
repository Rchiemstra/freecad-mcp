"""Versioned, stdlib-only wire contract for ``common_volume_along_path``."""
...
COMMON_VOLUME_ALONG_PATH_CONTRACT_VERSION: Literal[1] = 1
...
class CommonVolumeAlongPathCollaborators(Protocol):
...
class CommonVolumeAlongPathRequest:
...
class CommonVolumeAlongPathSuccess(TypedDict):
    ...
    sample_count: int
    volume_threshold_mm3: float
    max_common_volume_mm3: float
    any_collision: bool
    samples: list[object]


class CommonVolumeAlongPathFailure(TypedDict):
...
class CommonVolumeAlongPathUncertain(TypedDict):
...
CommonVolumeAlongPathResult = CommonVolumeAlongPathSuccess | CommonVolumeAlongPathFailure | CommonVolumeAlongPathUncertain
...
def make_common_volume_along_path_success(moving_object: str, sample_count: int, volume_threshold_mm3: float, max_common_volume_mm3: float, any_collision: bool, samples: list[object]) -> CommonVolumeAlongPathSuccess:
    ...
    return {
        ...
        "max_common_volume_mm3": max_common_volume_mm3,
        "any_collision": any_collision,
        "samples": samples,
    }


def make_common_volume_along_path_failure(
    error_code: str,
    error: str,
    *,
    ...
    result: CommonVolumeAlongPathFailure = {
        "contract_version": COMMON_VOLUME_ALONG_PATH_CONTRACT_VERSION,
        "success": False,
        "ok": False,
        "outcome": "rejected",
...
def make_common_volume_along_path_uncertain(
    error_code: str,
    error: str,
    *,
    committed: bool | None,
    ...
    rollback_failed: bool | None = None,
    diagnostics: dict[str, object] | None = None,
) -> CommonVolumeAlongPathUncertain:
    """Construct a result that explicitly forbids automatic replay."""

    result: CommonVolumeAlongPathUncertain = {
        "contract_version": COMMON_VOLUME_ALONG_PATH_CONTRACT_VERSION,
        "success": False,
        "ok": False,
        "outcome": "uncertain",
...
def _invalid_response(response: dict[str, object]) -> CommonVolumeAlongPathUncertain:
    ...
    code = (
        ...
        if committed
        else "COMMON_VOLUME_ALONG_PATH_ROLLBACK_UNCERTAIN"
        if rollback_failed
        else "INVALID_COMMON_VOLUME_ALONG_PATH_RESPONSE"
    )
    return make_common_volume_along_path_uncertain(
        code,
        "common_volume_along_path returned an invalid contract response; document state requires reconciliation",
        committed=True if committed else None,
        diagnostics={"response": response},
    )


def parse_common_volume_along_path_response(raw_response: object) -> CommonVolumeAlongPathResult:
    """Validate all three wire variants; unknown state always stays uncertain."""
    response = _response_object(raw_response)
    if response is None:
        return make_common_volume_along_path_uncertain(
            "INVALID_RPC_RESPONSE",
            "common_volume_along_path returned a non-object response",
            committed=None,
        )
    version = response.get("contract_version")
    details = _response_details(response)
    if type(version) is not int or version != COMMON_VOLUME_ALONG_PATH_CONTRACT_VERSION or details is None:
        return _invalid_response(response)

    moving_object = response.get("moving_object")
    sample_count = response.get("sample_count")
    volume_threshold_mm3 = response.get("volume_threshold_mm3")
    max_common_volume_mm3 = response.get("max_common_volume_mm3")
    any_collision = response.get("any_collision")
    samples = response.get("samples")
    if (
        _valid_success(response)
        and isinstance(moving_object, str) and moving_object.strip() and isinstance(sample_count, int) and not isinstance(sample_count, bool) and isinstance(volume_threshold_mm3, (int, float)) and not isinstance(volume_threshold_mm3, bool) and isinstance(max_common_volume_mm3, (int, float)) and not isinstance(max_common_volume_mm3, bool) and isinstance(any_collision, bool) and isinstance(samples, list)
    ):
        return make_common_volume_along_path_success(moving_object, sample_count, float(volume_threshold_mm3), float(max_common_volume_mm3), any_collision, samples)

    error_code = response.get("error_code")
    error = response.get("error")
    success_keys = {'moving_object', 'sample_count', 'volume_threshold_mm3', 'max_common_volume_mm3', 'any_collision', 'samples'}
    if (
        ...
        if _valid_rejection(response):
            return make_common_volume_along_path_failure(
                error_code,
                error,
        ...
        if _valid_uncertain(response):
            ...
            return make_common_volume_along_path_uncertain(error_code, error, committed=committed, **details)
    return _invalid_response(response)


__all__ = [
    "COMMON_VOLUME_ALONG_PATH_CONTRACT_VERSION",
