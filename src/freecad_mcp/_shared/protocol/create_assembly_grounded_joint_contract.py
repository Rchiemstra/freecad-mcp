"""Versioned, stdlib-only wire contract for ``create_assembly_grounded_joint``."""
...
CREATE_ASSEMBLY_GROUNDED_JOINT_CONTRACT_VERSION: Literal[1] = 1

DocumentName = NewType("DocumentName", str)
AssemblyName = NewType("AssemblyName", str)
...
class CreateAssemblyGroundedJointCollaborators(Protocol):
...
class CreateAssemblyGroundedJointRequest:
...
class CreateAssemblyGroundedJointSuccess(TypedDict):
...
class CreateAssemblyGroundedJointFailure(TypedDict):
...
class CreateAssemblyGroundedJointUncertain(TypedDict):
...
CreateAssemblyGroundedJointResult = CreateAssemblyGroundedJointSuccess | CreateAssemblyGroundedJointFailure | CreateAssemblyGroundedJointUncertain
...
def make_create_assembly_grounded_joint_success(joint: str, label: str, joint_type: str, assembly: str, component: str) -> CreateAssemblyGroundedJointSuccess:
    """Construct a complete committed result."""

    return {
        "contract_version": CREATE_ASSEMBLY_GROUNDED_JOINT_CONTRACT_VERSION,
        "success": True,
        "ok": True,
        "outcome": "committed",
        "committed": True,
...
def make_create_assembly_grounded_joint_failure(
    error_code: str,
    error: str,
    *,
    ...
    result: CreateAssemblyGroundedJointFailure = {
        "contract_version": CREATE_ASSEMBLY_GROUNDED_JOINT_CONTRACT_VERSION,
        "success": False,
        "ok": False,
        "outcome": "rejected",
...
def make_create_assembly_grounded_joint_uncertain(
    error_code: str,
    error: str,
    *,
    committed: bool | None,
    ...
    result: CreateAssemblyGroundedJointUncertain = {
        "contract_version": CREATE_ASSEMBLY_GROUNDED_JOINT_CONTRACT_VERSION,
        "success": False,
        "ok": False,
        "outcome": "uncertain",
...
def _invalid_response(response: dict[str, object]) -> CreateAssemblyGroundedJointUncertain:
    ...
    code = (
        ...
        if committed
        else "CREATE_ASSEMBLY_GROUNDED_JOINT_ROLLBACK_UNCERTAIN"
        if rollback_failed
        else "INVALID_CREATE_ASSEMBLY_GROUNDED_JOINT_RESPONSE"
    )
    return make_create_assembly_grounded_joint_uncertain(
        code,
        "create_assembly_grounded_joint returned an invalid contract response; document state requires reconciliation",
        committed=True if committed else None,
        diagnostics={"response": response},
    )


def parse_create_assembly_grounded_joint_response(raw_response: object) -> CreateAssemblyGroundedJointResult:
    """Validate all three wire variants; unknown state always stays uncertain."""
    response = _response_object(raw_response)
    if response is None:
        return make_create_assembly_grounded_joint_uncertain(
            "INVALID_RPC_RESPONSE",
            "create_assembly_grounded_joint returned a non-object response",
            committed=None,
        )
    version = response.get("contract_version")
    details = _response_details(response)
    if type(version) is not int or version != CREATE_ASSEMBLY_GROUNDED_JOINT_CONTRACT_VERSION or details is None:
        return _invalid_response(response)

    joint = response.get("joint")
    label = response.get("label")
    joint_type = response.get("joint_type")
    assembly = response.get("assembly")
    component = response.get("component")
    if (
        _valid_success(response)
        and isinstance(joint, str) and joint.strip() and isinstance(label, str) and isinstance(joint_type, str) and isinstance(assembly, str) and assembly.strip() and isinstance(component, str) and component.strip()
    ):
        return make_create_assembly_grounded_joint_success(joint, label, joint_type, assembly, component)

    error_code = response.get("error_code")
    error = response.get("error")
    success_keys = {'joint', 'label', 'joint_type', 'assembly', 'component'}
    if (
        ...
        if _valid_rejection(response):
            return make_create_assembly_grounded_joint_failure(
                error_code,
                error,
        ...
        if _valid_uncertain(response):
            ...
            return make_create_assembly_grounded_joint_uncertain(error_code, error, committed=committed, **details)
    return _invalid_response(response)


__all__ = [
    "CREATE_ASSEMBLY_GROUNDED_JOINT_CONTRACT_VERSION",
