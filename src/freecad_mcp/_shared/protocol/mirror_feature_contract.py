"""Versioned, stdlib-only wire contract for ``mirror_feature``."""
...
MIRROR_FEATURE_CONTRACT_VERSION: Literal[1] = 1
...
class MirrorFeatureCollaborators(Protocol):
...
class MirrorFeatureRequest:
    """Validated internal request."""

    doc_name: DocumentName
    feature_name: FeatureName
    mirror_name: FeatureName
    plane: str
    body_name: FeatureName | None


class MirrorFeatureSuccess(TypedDict):
...
class MirrorFeatureFailure(TypedDict):
...
class MirrorFeatureUncertain(TypedDict):
...
MirrorFeatureResult = MirrorFeatureSuccess | MirrorFeatureFailure | MirrorFeatureUncertain
...
def make_mirror_feature_success(feature: FeatureName, label: str) -> MirrorFeatureSuccess:
    """Construct a complete committed result using the actual assigned name."""

    return {
        "contract_version": MIRROR_FEATURE_CONTRACT_VERSION,
        "success": True,
        "ok": True,
        "outcome": "committed",
        "committed": True,
...
def make_mirror_feature_failure(
    error_code: str,
    error: str,
    *,
...
def make_mirror_feature_uncertain(
    error_code: str,
    error: str,
    *,
    committed: bool | None,
...
def _invalid_response(response: dict[str, object]) -> MirrorFeatureUncertain:
    ...
    code = (
        ...
        if committed
        else "MIRROR_FEATURE_ROLLBACK_UNCERTAIN"
        if rollback_failed
        else "INVALID_MIRROR_FEATURE_RESPONSE"
    )
    return make_mirror_feature_uncertain(
        code,
        "mirror_feature returned an invalid contract response; document state requires reconciliation",
        committed=True if committed else None,
        diagnostics={"response": response},
    )


def parse_mirror_feature_response(raw_response: object) -> MirrorFeatureResult:
    """Validate all three wire variants; unknown state always stays uncertain."""
    response = _response_object(raw_response)
    if response is None:
        return make_mirror_feature_uncertain(
            "INVALID_RPC_RESPONSE",
            "mirror_feature returned a non-object response",
            committed=None,
        )
    version = response.get("contract_version")
    details = _response_details(response)
    if type(version) is not int or version != MIRROR_FEATURE_CONTRACT_VERSION or details is None:
    ...
    if (
        ...
        return make_mirror_feature_success(FeatureName(feature), label)
    ...
    if (
        ...
        if _valid_uncertain(response):
            ...
            return make_mirror_feature_uncertain(error_code, error, committed=committed, **details)
    return _invalid_response(response)


__all__ = [
    "MIRROR_FEATURE_CONTRACT_VERSION",
    "MirrorFeatureCollaborators",
    "MirrorFeatureFailure",
    "MirrorFeatureRequest",
    "MirrorFeatureResult",
    "MirrorFeatureSuccess",
    "MirrorFeatureUncertain",
    "FeatureDocument",
    "FeatureName",
    "FeatureObject",
