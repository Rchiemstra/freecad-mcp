"""Versioned, stdlib-only wire contract for ``sketch_add_constraint``."""
...
SKETCH_ADD_CONSTRAINT_CONTRACT_VERSION: Literal[1] = 1

DocumentName = NewType("DocumentName", str)
SketchName = NewType("SketchName", str)


class SketchAddConstraintObject(Protocol):
...
class SketchAddConstraintReadDocument(Protocol):
    """Read-only document surface available after native recompute."""

    @property
    def Name(self) -> str: ...

    def getObject(self, name: str) -> SketchAddConstraintObject | None: ...


class SketchAddConstraintDocument(SketchAddConstraintReadDocument, Protocol):
    """Narrow mutation surface available only to the apply callback."""

    def addObject(self, object_type: str, name: str) -> SketchAddConstraintObject: ...


@runtime_checkable
class NativeSketchAddConstraintDocument(SketchAddConstraintDocument, Protocol):
...
class SketchAddConstraintCollaborators(Protocol):
...
class SketchAddConstraintRequest:
...
class SketchAddConstraintSuccess(TypedDict):
...
class SketchAddConstraintFailure(TypedDict):
...
class SketchAddConstraintUncertain(TypedDict):
...
SketchAddConstraintResult = SketchAddConstraintSuccess | SketchAddConstraintFailure | SketchAddConstraintUncertain
...
def make_sketch_add_constraint_success(sketch: SketchName, added_count: int) -> SketchAddConstraintSuccess:
    """Construct a complete committed result using the actual assigned identity."""

    return {
        "contract_version": SKETCH_ADD_CONSTRAINT_CONTRACT_VERSION,
        "success": True,
        "ok": True,
        "outcome": "committed",
        "committed": True,
...
def make_sketch_add_constraint_failure(
    error_code: str,
    error: str,
    *,
    ...
    result: SketchAddConstraintFailure = {
        "contract_version": SKETCH_ADD_CONSTRAINT_CONTRACT_VERSION,
        "success": False,
        "ok": False,
        "outcome": "rejected",
...
def make_sketch_add_constraint_uncertain(
    error_code: str,
    error: str,
    *,
    committed: bool | None,
    ...
    result: SketchAddConstraintUncertain = {
        "contract_version": SKETCH_ADD_CONSTRAINT_CONTRACT_VERSION,
        "success": False,
        "ok": False,
        "outcome": "uncertain",
...
def _invalid_response(response: dict[str, object]) -> SketchAddConstraintUncertain:
    ...
    code = (
        ...
        if committed
        else "SKETCH_ADD_CONSTRAINT_ROLLBACK_UNCERTAIN"
        if rollback_failed
        else "INVALID_SKETCH_ADD_CONSTRAINT_RESPONSE"
    )
    return make_sketch_add_constraint_uncertain(
        code,
        "sketch_add_constraint returned an invalid contract response; document state requires reconciliation",
        committed=True if committed else None,
        diagnostics={"response": response},
    )


def parse_sketch_add_constraint_response(raw_response: object) -> SketchAddConstraintResult:
    """Validate all three wire variants; unknown state always stays uncertain."""
    response = _response_object(raw_response)
    if response is None:
        return make_sketch_add_constraint_uncertain(
            "INVALID_RPC_RESPONSE",
            "sketch_add_constraint returned a non-object response",
            committed=None,
        )
    version = response.get("contract_version")
    details = _response_details(response)
    if type(version) is not int or version != SKETCH_ADD_CONSTRAINT_CONTRACT_VERSION or details is None:
    ...
    if (
        ...
        and sketch.strip()
        and isinstance(added_count, int) and not isinstance(added_count, bool) and added_count >= 0
    ):
        return make_sketch_add_constraint_success(SketchName(sketch), added_count)
    ...
    if (
        ...
        and error.strip()
        and "sketch" not in response and "added_count" not in response
    ):
        ...
        if _valid_uncertain(response):
            ...
            return make_sketch_add_constraint_uncertain(error_code, error, committed=committed, **details)
    return _invalid_response(response)


__all__ = [
    "SKETCH_ADD_CONSTRAINT_CONTRACT_VERSION",
