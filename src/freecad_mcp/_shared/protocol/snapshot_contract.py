"""Versioned, stdlib-only wire contract for ``snapshot``."""
...
SNAPSHOT_CONTRACT_VERSION: Literal[1] = 1

DocumentName = NewType("DocumentName", str)
SnapshotName = NewType("SnapshotName", str)
...
class SnapshotReadDocument(Protocol):
    """Read-only document surface available after native recompute."""

    @property
    def Name(self) -> str: ...

    def getObject(self, name: str) -> SnapshotObject | None: ...


class SnapshotDocument(SnapshotReadDocument, Protocol):
    """Narrow mutation surface available only to the apply callback."""

    def addObject(self, object_type: str, name: str) -> SnapshotObject: ...


@runtime_checkable
class NativeSnapshotDocument(SnapshotDocument, Protocol):
...
class SnapshotCollaborators(Protocol):
    """Complete typed boundary between the handler and native bridge."""

    def validate_document_invariants(self, document: SnapshotReadDocument) -> object:
...
class SnapshotUncertain(TypedDict):
...
SnapshotResult = SnapshotSuccess | SnapshotFailure | SnapshotUncertain
...
def _invalid_response(response: dict[str, object]) -> SnapshotUncertain:
    ...
    code = (
        ...
        if committed
        else "SNAPSHOT_ROLLBACK_UNCERTAIN"
        if rollback_failed
        else "INVALID_SNAPSHOT_RESPONSE"
    )
    return make_snapshot_uncertain(
        code,
        "snapshot returned an invalid contract response; document state requires reconciliation",
        committed=True if committed else None,
        diagnostics={"response": response},
    )


def parse_snapshot_response(raw_response: object) -> SnapshotResult:
    """Validate all three wire variants; unknown state always stays uncertain."""
    response = _response_object(raw_response)
    if response is None:
        return make_snapshot_uncertain(
            "INVALID_RPC_RESPONSE",
            "snapshot returned a non-object response",
            committed=None,
        )
    version = response.get("contract_version")
    details = _response_details(response)
    if type(version) is not int or version != SNAPSHOT_CONTRACT_VERSION or details is None:
    ...
    if (
        ...
        if _valid_uncertain(response):
            ...
            return make_snapshot_uncertain(error_code, error, committed=committed, **details)
    return _invalid_response(response)
