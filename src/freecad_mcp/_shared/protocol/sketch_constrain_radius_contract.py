"""Versioned, stdlib-only wire contract for ``sketch_constrain_radius``."""
...
SKETCH_CONSTRAIN_RADIUS_CONTRACT_VERSION: Literal[1] = 1
...
class SketchConstrainRadiusCollaborators(Protocol):
    @property
...
class SketchConstrainRadiusRequest:
...
class SketchConstrainRadiusSuccess(TypedDict):
...
class SketchConstrainRadiusFailure(TypedDict):
    contract_version: Literal[1]
...
class SketchConstrainRadiusUncertain(TypedDict):
    contract_version: Literal[1]
    success: Literal[False]
    ok: Literal[False]
...
SketchConstrainRadiusResult = SketchConstrainRadiusSuccess | SketchConstrainRadiusFailure | SketchConstrainRadiusUncertain
...
def make_sketch_constrain_radius_success(sketch: SketchName, constraint_index: int) -> SketchConstrainRadiusSuccess:
    return {
        "contract_version": SKETCH_CONSTRAIN_RADIUS_CONTRACT_VERSION,
        "success": True,
        "ok": True,
        "outcome": "committed",
        "committed": True,
...
def make_sketch_constrain_radius_failure(
    error_code: str,
    error: str,
    *,
    ...
    result: SketchConstrainRadiusFailure = {
        "contract_version": SKETCH_CONSTRAIN_RADIUS_CONTRACT_VERSION,
        "success": False,
        "ok": False,
        "outcome": "rejected",
...
def make_sketch_constrain_radius_uncertain(
    error_code: str,
    error: str,
    *,
    committed: bool | None,
    ...
    result: SketchConstrainRadiusUncertain = {
        "contract_version": SKETCH_CONSTRAIN_RADIUS_CONTRACT_VERSION,
        "success": False,
        "ok": False,
        "outcome": "uncertain",
...
def _invalid_response(response: dict[str, object]) -> SketchConstrainRadiusUncertain:
    ...
    code = (
        ...
        if committed
        else "SKETCH_CONSTRAIN_RADIUS_ROLLBACK_UNCERTAIN"
        if rollback_failed
        else "INVALID_SKETCH_CONSTRAIN_RADIUS_RESPONSE"
    )
    return make_sketch_constrain_radius_uncertain(
        code,
        "sketch_constrain_radius returned an invalid contract response; document state requires reconciliation",
        committed=True if committed else None,
        diagnostics={"response": response},
    )


def parse_sketch_constrain_radius_response(raw_response: object) -> SketchConstrainRadiusResult:
    response = _response_object(raw_response)
    if response is None:
        return make_sketch_constrain_radius_uncertain(
            "INVALID_RPC_RESPONSE",
            "sketch_constrain_radius returned a non-object response",
            committed=None,
        )
    version = response.get("contract_version")
    details = _response_details(response)
    if type(version) is not int or version != SKETCH_CONSTRAIN_RADIUS_CONTRACT_VERSION or details is None:
    ...
    if (
        ...
        return make_sketch_constrain_radius_success(SketchName(sketch), constraint_index)
    ...
    if (
        ...
        if _valid_rejection(response):
            return make_sketch_constrain_radius_failure(
                error_code,
                error,
        ...
        if _valid_uncertain(response):
            ...
            return make_sketch_constrain_radius_uncertain(error_code, error, committed=committed, **details)
    return _invalid_response(response)


__all__ = [
    "SKETCH_CONSTRAIN_RADIUS_CONTRACT_VERSION",
