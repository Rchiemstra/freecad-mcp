"""Versioned, stdlib-only wire contract for ``clear_expression``."""
...
CLEAR_EXPRESSION_CONTRACT_VERSION: Literal[1] = 1

DocumentName = NewType("DocumentName", str)
ClearExpressionName = NewType("ClearExpressionName", str)


class ClearExpressionObject(Protocol):
...
class ClearExpressionReadDocument(Protocol):
    """Read-only document surface available after native recompute."""

    @property
    def Name(self) -> str: ...

    def getObject(self, name: str) -> ClearExpressionObject | None: ...


class ClearExpressionDocument(ClearExpressionReadDocument, Protocol):
    """Narrow mutation surface available only to the apply callback."""

    def addObject(self, object_type: str, name: str) -> ClearExpressionObject: ...


@runtime_checkable
class NativeClearExpressionDocument(ClearExpressionDocument, Protocol):
...
class ClearExpressionCollaborators(Protocol):
...
class ClearExpressionSuccess(TypedDict):
...
class ClearExpressionFailure(TypedDict):
...
class ClearExpressionUncertain(TypedDict):
...
ClearExpressionResult = ClearExpressionSuccess | ClearExpressionFailure | ClearExpressionUncertain

_CORE_KEYS = frozenset(
    {
        "contract_version", "success", "ok", "outcome", "committed", "retry_safe", 'object', 'prop_path', "error_code", "error", "native_status", "native_message", "rollback_succeeded", "rollback_failed", "diagnostics"
    }
)


def make_clear_expression_success(object: str, prop_path: str) -> ClearExpressionSuccess:
    """Construct a complete committed result."""

    return {
        "contract_version": CLEAR_EXPRESSION_CONTRACT_VERSION,
        "success": True,
        "ok": True,
        "outcome": "committed",
        "committed": True,
...
def make_clear_expression_failure(
    error_code: str,
    error: str,
    *,
...
def make_clear_expression_uncertain(
    error_code: str,
    error: str,
    *,
    committed: bool | None,
    ...
    result: ClearExpressionUncertain = {
        "contract_version": CLEAR_EXPRESSION_CONTRACT_VERSION,
        "success": False,
        "ok": False,
        "outcome": "uncertain",
...
def _invalid_response(response: dict[str, object]) -> ClearExpressionUncertain:
    ...
    code = (
        ...
        if committed
        else "CLEAR_EXPRESSION_ROLLBACK_UNCERTAIN"
        if rollback_failed
        else "INVALID_CLEAR_EXPRESSION_RESPONSE"
    )
    return make_clear_expression_uncertain(
        code,
        "clear_expression returned an invalid contract response; document state requires reconciliation",
        committed=True if committed else None,
        diagnostics={"response": response},
    )


def parse_clear_expression_response(raw_response: object) -> ClearExpressionResult:
    """Validate all three wire variants; unknown state always stays uncertain."""
    response = _response_object(raw_response)
    if response is None:
        return make_clear_expression_uncertain(
            "INVALID_RPC_RESPONSE",
            "clear_expression returned a non-object response",
            committed=None,
        )
    version = response.get("contract_version")
    details = _response_details(response)
    if type(version) is not int or version != CLEAR_EXPRESSION_CONTRACT_VERSION or details is None:
        return _invalid_response(response)

    object = response.get('object')
    prop_path = response.get('prop_path')
    if (
        _valid_success(response)
        and isinstance(object, str) and object.strip()
        and isinstance(prop_path, str) and prop_path.strip()
    ):
        return make_clear_expression_success(str(object), str(prop_path))
    ...
    if (
        ...
        and error.strip()
        and 'object' not in response and 'prop_path' not in response
    ):
        if _valid_rejection(response):
            return make_clear_expression_failure(
                error_code,
                error,
        ...
        if _valid_uncertain(response):
            ...
            return make_clear_expression_uncertain(error_code, error, committed=committed, **details)
    return _invalid_response(response)


__all__ = [
    "CLEAR_EXPRESSION_CONTRACT_VERSION",
