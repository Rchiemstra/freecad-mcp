"""Versioned, stdlib-only wire contract for ``measure_volume``."""
...
MEASURE_VOLUME_CONTRACT_VERSION: Literal[1] = 1
...
class MeasureVolumeCollaborators(Protocol):
...
class MeasureVolumeSuccess(TypedDict):
...
class MeasureVolumeUncertain(TypedDict):
...
MeasureVolumeResult = MeasureVolumeSuccess | MeasureVolumeFailure | MeasureVolumeUncertain
...
def make_measure_volume_success(
    ...
    return {
        "contract_version": MEASURE_VOLUME_CONTRACT_VERSION,
        "success": True,
        "ok": True,
        "outcome": "observed",
        "retry_safe": False,
...
def make_measure_volume_failure(
    error_code: str,
    error: str,
    *,
...
def make_measure_volume_uncertain(
    error_code: str,
    error: str,
    *,
    committed: bool | None,
...
def _invalid_response(response: dict[str, object]) -> MeasureVolumeUncertain:
    ...
    code = (
        "MEASURE_VOLUME_COMMITTED_RESPONSE_INVALID"
        if committed
        else "MEASURE_VOLUME_ROLLBACK_UNCERTAIN"
        if rollback_failed
        else "INVALID_MEASURE_VOLUME_RESPONSE"
    )
    return make_measure_volume_uncertain(
        code,
        "measure_volume returned an invalid contract response; document state requires reconciliation",
        committed=True if committed else None,
        diagnostics={"response": response},
    )


def parse_measure_volume_response(raw_response: object) -> MeasureVolumeResult:
    """Validate all three wire variants; unknown state always stays uncertain."""
    response = _response_object(raw_response)
    if response is None:
        return make_measure_volume_uncertain(
            "INVALID_RPC_RESPONSE",
            "measure_volume returned a non-object response",
            committed=None,
        )
    version = response.get("contract_version")
    details = _response_details(response)
    if type(version) is not int or version != MEASURE_VOLUME_CONTRACT_VERSION or details is None:
    ...
    if (
        ...
        if _valid_rejection(response):
            return make_measure_volume_failure(
                error_code,
                error,
        ...
        if _valid_uncertain(response):
            ...
            return make_measure_volume_uncertain(error_code, error, committed=committed, **details)
    return _invalid_response(response)


__all__ = [
    "MEASURE_VOLUME_CONTRACT_VERSION",
