"""Versioned, stdlib-only wire contract for ``get_global_shape``."""
...
GET_GLOBAL_SHAPE_CONTRACT_VERSION: Literal[1] = 1
...
class GetGlobalShapeCollaborators(Protocol):
...
class GetGlobalShapeSuccess(TypedDict):
...
class GetGlobalShapeFailure(TypedDict):
...
class GetGlobalShapeUncertain(TypedDict):
...
GetGlobalShapeResult = GetGlobalShapeSuccess | GetGlobalShapeFailure | GetGlobalShapeUncertain
...
def make_get_global_shape_success(
    object: str, frame: str, volume_mm3: float, area_mm2: float, center_of_mass: object, bbox: object, solids: int, faces: int, edges: int,
) -> GetGlobalShapeSuccess:
    """Construct a complete observed result."""

    return {
        "contract_version": GET_GLOBAL_SHAPE_CONTRACT_VERSION,
        "success": True,
        "ok": True,
        "outcome": "observed",
        "retry_safe": False,
...
def make_get_global_shape_failure(
    error_code: str,
    error: str,
    *,
...
def make_get_global_shape_uncertain(
    error_code: str,
    error: str,
    *,
    committed: bool | None,
...
def _invalid_response(response: dict[str, object]) -> GetGlobalShapeUncertain:
    ...
    code = (
        "GET_GLOBAL_SHAPE_COMMITTED_RESPONSE_INVALID"
        if committed
        else "GET_GLOBAL_SHAPE_ROLLBACK_UNCERTAIN"
        if rollback_failed
        else "INVALID_GET_GLOBAL_SHAPE_RESPONSE"
    )
    return make_get_global_shape_uncertain(
        code,
        "get_global_shape returned an invalid contract response; document state requires reconciliation",
        committed=True if committed else None,
        diagnostics={"response": response},
    )


def parse_get_global_shape_response(raw_response: object) -> GetGlobalShapeResult:
    """Validate all three wire variants; unknown state always stays uncertain."""
    response = _response_object(raw_response)
    if response is None:
        return make_get_global_shape_uncertain(
            "INVALID_RPC_RESPONSE",
            "get_global_shape returned a non-object response",
            committed=None,
        )
    version = response.get("contract_version")
    details = _response_details(response)
    if type(version) is not int or version != GET_GLOBAL_SHAPE_CONTRACT_VERSION or details is None:
    ...
    center_of_mass = response.get("center_of_mass")
    bbox = response.get("bbox")
    solids = response.get("solids")
    faces = response.get("faces")
    edges = response.get("edges")
    if _valid_success(response) and isinstance(object_name, str) and isinstance(frame, str) and isinstance(volume_mm3, (int, float)) and not isinstance(volume_mm3, bool) and isinstance(area_mm2, (int, float)) and not isinstance(area_mm2, bool) and center_of_mass is not None and bbox is not None and type(solids) is int and type(faces) is int and type(edges) is int:
        return make_get_global_shape_success(object_name, frame, float(volume_mm3), float(area_mm2), center_of_mass, bbox, solids, faces, edges)

    error_code = response.get("error_code")
    error = response.get("error")
    success_keys = {'object', 'frame', 'volume_mm3', 'area_mm2', 'center_of_mass', 'bbox', 'solids', 'faces', 'edges'}
    if (
        ...
        if _valid_rejection(response):
            return make_get_global_shape_failure(
                error_code,
                error,
        ...
        if _valid_uncertain(response):
            ...
            return make_get_global_shape_uncertain(error_code, error, committed=committed, **details)
    return _invalid_response(response)


__all__ = [
    "GET_GLOBAL_SHAPE_CONTRACT_VERSION",
