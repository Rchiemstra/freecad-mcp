"""Versioned, stdlib-only wire contract for ``get_dependency_graph``."""
...
GET_DEPENDENCY_GRAPH_CONTRACT_VERSION: Literal[1] = 1
...
class GetDependencyGraphCollaborators(Protocol):
...
class GetDependencyGraphRequest:
...
class GetDependencyGraphSuccess(TypedDict):
...
class GetDependencyGraphUncertain(TypedDict):
...
GetDependencyGraphResult = GetDependencyGraphSuccess | GetDependencyGraphFailure | GetDependencyGraphUncertain
...
def make_get_dependency_graph_failure(
    ...
    *,
    ...
    result: GetDependencyGraphFailure = {
        "contract_version": GET_DEPENDENCY_GRAPH_CONTRACT_VERSION,
        "success": False,
        "ok": False,
        "outcome": "rejected",
...
def make_get_dependency_graph_uncertain(
    error_code: str,
    error: str,
    *,
    committed: bool | None,
    ...
    rollback_failed: bool | None = None,
    diagnostics: dict[str, object] | None = None,
) -> GetDependencyGraphUncertain:
    """Construct a result that explicitly forbids automatic replay."""

    result: GetDependencyGraphUncertain = {
        "contract_version": GET_DEPENDENCY_GRAPH_CONTRACT_VERSION,
        "success": False,
        "ok": False,
        "outcome": "uncertain",
...
def _invalid_response(response: dict[str, object]) -> GetDependencyGraphUncertain:
    ...
    code = (
        "GET_DEPENDENCY_GRAPH_COMMITTED_RESPONSE_INVALID"
        if committed
        else "GET_DEPENDENCY_GRAPH_ROLLBACK_UNCERTAIN"
        if rollback_failed
        else "INVALID_GET_DEPENDENCY_GRAPH_RESPONSE"
    )
    return make_get_dependency_graph_uncertain(
        code,
        "get_dependency_graph returned an invalid contract response; document state requires reconciliation",
        committed=True if committed else None,
        diagnostics={"response": response},
    )


def parse_get_dependency_graph_response(raw_response: object) -> GetDependencyGraphResult:
    """Validate all three wire variants; unknown state always stays uncertain."""
    response = _response_object(raw_response)
    if response is None:
        return make_get_dependency_graph_uncertain(
            "INVALID_RPC_RESPONSE",
            "get_dependency_graph returned a non-object response",
            committed=None,
        )
    version = response.get("contract_version")
    details = _response_details(response)
    if type(version) is not int or version != GET_DEPENDENCY_GRAPH_CONTRACT_VERSION or details is None:
        return _invalid_response(response)

    root = response.get("root")
    history_order = response.get("history_order")
    ...
    if (
        ...
        if _valid_rejection(response):
            return make_get_dependency_graph_failure(
                error_code,
                error,
        ...
        if _valid_uncertain(response):
            ...
            return make_get_dependency_graph_uncertain(error_code, error, committed=committed, **details)
    return _invalid_response(response)


__all__ = [
    "GET_DEPENDENCY_GRAPH_CONTRACT_VERSION",
