"""Versioned, stdlib-only wire contract for ``fillet_feature``."""
...
FILLET_FEATURE_CONTRACT_VERSION: Literal[1] = 1
...
class FilletFeatureCollaborators(Protocol):
...
class FilletFeatureRequest:
    """Validated internal request."""

    doc_name: DocumentName
    base_feature: FeatureName
    fillet_name: FeatureName
    radius: float
    edge_refs: tuple[str, ...]
    body_name: FeatureName | None


class FilletFeatureSuccess(TypedDict):
...
class FilletFeatureFailure(TypedDict):
...
class FilletFeatureUncertain(TypedDict):
...
FilletFeatureResult = FilletFeatureSuccess | FilletFeatureFailure | FilletFeatureUncertain
...
def make_fillet_feature_success(feature: FeatureName, label: str) -> FilletFeatureSuccess:
    """Construct a complete committed result using the actual assigned name."""

    return {
        "contract_version": FILLET_FEATURE_CONTRACT_VERSION,
        "success": True,
        "ok": True,
        "outcome": "committed",
        "committed": True,
...
def make_fillet_feature_failure(
    error_code: str,
    error: str,
    *,
...
def make_fillet_feature_uncertain(
    error_code: str,
    error: str,
    *,
    committed: bool | None,
...
def _invalid_response(response: dict[str, object]) -> FilletFeatureUncertain:
    ...
    code = (
        ...
        if committed
        else "FILLET_FEATURE_ROLLBACK_UNCERTAIN"
        if rollback_failed
        else "INVALID_FILLET_FEATURE_RESPONSE"
    )
    return make_fillet_feature_uncertain(
        code,
        "fillet_feature returned an invalid contract response; document state requires reconciliation",
        committed=True if committed else None,
        diagnostics={"response": response},
    )


def parse_fillet_feature_response(raw_response: object) -> FilletFeatureResult:
    """Validate all three wire variants; unknown state always stays uncertain."""
    response = _response_object(raw_response)
    if response is None:
        return make_fillet_feature_uncertain(
            "INVALID_RPC_RESPONSE",
            "fillet_feature returned a non-object response",
            committed=None,
        )
    version = response.get("contract_version")
    details = _response_details(response)
    if type(version) is not int or version != FILLET_FEATURE_CONTRACT_VERSION or details is None:
    ...
    if (
        ...
        return make_fillet_feature_success(FeatureName(feature), label)
    ...
    if (
        ...
        if _valid_uncertain(response):
            ...
            return make_fillet_feature_uncertain(error_code, error, committed=committed, **details)
    return _invalid_response(response)


__all__ = [
    "FILLET_FEATURE_CONTRACT_VERSION",
    "FilletFeatureCollaborators",
    "FilletFeatureFailure",
    "FilletFeatureRequest",
    "FilletFeatureResult",
    "FilletFeatureSuccess",
    "FilletFeatureUncertain",
    "FeatureDocument",
    "FeatureName",
    "FeatureObject",
