"""Versioned, stdlib-only wire contract for ``measure_area``."""
...
MEASURE_AREA_CONTRACT_VERSION: Literal[1] = 1
...
class MeasureAreaCollaborators(Protocol):
...
class MeasureAreaSuccess(TypedDict):
...
class MeasureAreaFailure(TypedDict):
...
class MeasureAreaUncertain(TypedDict):
...
MeasureAreaResult = MeasureAreaSuccess | MeasureAreaFailure | MeasureAreaUncertain
...
def make_measure_area_success(
    object: str, area_mm2: float, area_cm2: float, unit: str, frame: str,
) -> MeasureAreaSuccess:
    """Construct a complete observed result."""

    return {
        "contract_version": MEASURE_AREA_CONTRACT_VERSION,
        "success": True,
        "ok": True,
        "outcome": "observed",
        "retry_safe": False,
...
def make_measure_area_uncertain(
    error_code: str,
    error: str,
    *,
    committed: bool | None,
...
def _invalid_response(response: dict[str, object]) -> MeasureAreaUncertain:
    ...
    code = (
        "MEASURE_AREA_COMMITTED_RESPONSE_INVALID"
        if committed
        else "MEASURE_AREA_ROLLBACK_UNCERTAIN"
        if rollback_failed
        else "INVALID_MEASURE_AREA_RESPONSE"
    )
    return make_measure_area_uncertain(
        code,
        "measure_area returned an invalid contract response; document state requires reconciliation",
        committed=True if committed else None,
        diagnostics={"response": response},
    )


def parse_measure_area_response(raw_response: object) -> MeasureAreaResult:
    """Validate all three wire variants; unknown state always stays uncertain."""
    response = _response_object(raw_response)
    if response is None:
        return make_measure_area_uncertain(
            "INVALID_RPC_RESPONSE",
            "measure_area returned a non-object response",
            committed=None,
        )
    version = response.get("contract_version")
    details = _response_details(response)
    if type(version) is not int or version != MEASURE_AREA_CONTRACT_VERSION or details is None:
        return _invalid_response(response)

    object_name = response.get("object")
    area_mm2 = response.get("area_mm2")
    area_cm2 = response.get("area_cm2")
    unit = response.get("unit")
    frame = response.get("frame")
    if _valid_success(response) and isinstance(object_name, str) and isinstance(area_mm2, (int, float)) and not isinstance(area_mm2, bool) and isinstance(area_cm2, (int, float)) and not isinstance(area_cm2, bool) and isinstance(unit, str) and isinstance(frame, str):
        return make_measure_area_success(object_name, float(area_mm2), float(area_cm2), unit, frame)

    error_code = response.get("error_code")
    error = response.get("error")
    success_keys = {'object', 'area_mm2', 'area_cm2', 'unit', 'frame'}
    if (
        ...
        if _valid_rejection(response):
            return make_measure_area_failure(
                error_code,
                error,
        ...
        if _valid_uncertain(response):
            ...
            return make_measure_area_uncertain(error_code, error, committed=committed, **details)
    return _invalid_response(response)


__all__ = [
    "MEASURE_AREA_CONTRACT_VERSION",
