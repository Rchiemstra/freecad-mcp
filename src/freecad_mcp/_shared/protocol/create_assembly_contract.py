"""Versioned, stdlib-only wire contract for ``create_assembly``."""
...
CREATE_ASSEMBLY_CONTRACT_VERSION: Literal[1] = 1
...
class CreateAssemblyCollaborators(Protocol):
...
class CreateAssemblySuccess(TypedDict):
    ...
    label: str
    type: str
    joint_group: str | None


class CreateAssemblyFailure(TypedDict):
...
class CreateAssemblyUncertain(TypedDict):
...
CreateAssemblyResult = CreateAssemblySuccess | CreateAssemblyFailure | CreateAssemblyUncertain
...
def make_create_assembly_success(assembly: str, label: str, type: str, joint_group: str | None) -> CreateAssemblySuccess:
    """Construct a complete committed result."""

    return {
        "contract_version": CREATE_ASSEMBLY_CONTRACT_VERSION,
        "success": True,
        "ok": True,
        "outcome": "committed",
        "committed": True,
...
def make_create_assembly_failure(
    error_code: str,
    error: str,
    *,
...
def make_create_assembly_uncertain(
    error_code: str,
    error: str,
    *,
    committed: bool | None,
...
def _invalid_response(response: dict[str, object]) -> CreateAssemblyUncertain:
    ...
    code = (
        ...
        if committed
        else "CREATE_ASSEMBLY_ROLLBACK_UNCERTAIN"
        if rollback_failed
        else "INVALID_CREATE_ASSEMBLY_RESPONSE"
    )
    return make_create_assembly_uncertain(
        code,
        "create_assembly returned an invalid contract response; document state requires reconciliation",
        committed=True if committed else None,
        diagnostics={"response": response},
    )


def parse_create_assembly_response(raw_response: object) -> CreateAssemblyResult:
    """Validate all three wire variants; unknown state always stays uncertain."""
    response = _response_object(raw_response)
    if response is None:
        return make_create_assembly_uncertain(
            "INVALID_RPC_RESPONSE",
            "create_assembly returned a non-object response",
            committed=None,
        )
    version = response.get("contract_version")
    details = _response_details(response)
    if type(version) is not int or version != CREATE_ASSEMBLY_CONTRACT_VERSION or details is None:
        return _invalid_response(response)

    assembly = response.get("assembly")
    label = response.get("label")
    object_type = response.get("type")
    joint_group = response.get("joint_group")
    if (
        _valid_success(response)
        and isinstance(assembly, str) and assembly.strip() and isinstance(label, str) and isinstance(object_type, str) and (joint_group is None or isinstance(joint_group, str))
    ):
        return make_create_assembly_success(AssemblyName(assembly), label, object_type, joint_group)

    error_code = response.get("error_code")
    error = response.get("error")
    success_keys = {'assembly', 'label', 'type', 'joint_group'}
    if (
        ...
        if _valid_rejection(response):
            return make_create_assembly_failure(
                error_code,
                error,
        ...
        if _valid_uncertain(response):
            ...
            return make_create_assembly_uncertain(error_code, error, committed=committed, **details)
    return _invalid_response(response)


__all__ = [
    "CREATE_ASSEMBLY_CONTRACT_VERSION",
