"""Versioned, stdlib-only wire contract for ``spreadsheet_set_cells``."""
...
SpreadsheetSetCellsName = NewType("SpreadsheetSetCellsName", str)


class SpreadsheetSetCellsObject(Protocol):
...
class SpreadsheetSetCellsReadDocument(Protocol):
    ...
    def getObject(self, name: str) -> SpreadsheetSetCellsObject | None: ...


class SpreadsheetSetCellsDocument(SpreadsheetSetCellsReadDocument, Protocol):
    """Narrow mutation surface available only to the apply callback."""

    def addObject(self, object_type: str, name: str) -> SpreadsheetSetCellsObject: ...


@runtime_checkable
class NativeSpreadsheetSetCellsDocument(SpreadsheetSetCellsDocument, Protocol):
...
class SpreadsheetSetCellsCollaborators(Protocol):
...
class SpreadsheetSetCellsRequest:
...
class SpreadsheetSetCellsUncertain(TypedDict):
...
SpreadsheetSetCellsResult = SpreadsheetSetCellsSuccess | SpreadsheetSetCellsFailure | SpreadsheetSetCellsUncertain
...
def make_spreadsheet_set_cells_success(sheet: str) -> SpreadsheetSetCellsSuccess:
    ...
    return {
        "contract_version": SPREADSHEET_SET_CELLS_CONTRACT_VERSION,
        "success": True,
        "ok": True,
        "outcome": "committed",
        "committed": True,
...
def make_spreadsheet_set_cells_failure(
    error_code: str,
    error: str,
    *,
    ...
    result: SpreadsheetSetCellsFailure = {
        "contract_version": SPREADSHEET_SET_CELLS_CONTRACT_VERSION,
        "success": False,
        "ok": False,
        "outcome": "rejected",
...
def make_spreadsheet_set_cells_uncertain(
    error_code: str,
    error: str,
    *,
    committed: bool | None,
    ...
    result: SpreadsheetSetCellsUncertain = {
        "contract_version": SPREADSHEET_SET_CELLS_CONTRACT_VERSION,
        "success": False,
        "ok": False,
        "outcome": "uncertain",
...
def _invalid_response(response: dict[str, object]) -> SpreadsheetSetCellsUncertain:
    ...
    code = (
        ...
        if committed
        else "SPREADSHEET_SET_CELLS_ROLLBACK_UNCERTAIN"
        if rollback_failed
        else "INVALID_SPREADSHEET_SET_CELLS_RESPONSE"
    )
    return make_spreadsheet_set_cells_uncertain(
        code,
        "spreadsheet_set_cells returned an invalid contract response; document state requires reconciliation",
        committed=True if committed else None,
        diagnostics={"response": response},
    )


def parse_spreadsheet_set_cells_response(raw_response: object) -> SpreadsheetSetCellsResult:
    """Validate all three wire variants; unknown state always stays uncertain."""
    response = _response_object(raw_response)
    if response is None:
        return make_spreadsheet_set_cells_uncertain(
            "INVALID_RPC_RESPONSE",
            "spreadsheet_set_cells returned a non-object response",
            committed=None,
        )
    ...
    if (
        ...
        if _valid_uncertain(response):
            ...
            return make_spreadsheet_set_cells_uncertain(error_code, error, committed=committed, **details)
    return _invalid_response(response)
