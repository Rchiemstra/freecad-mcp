"""Versioned, stdlib-only wire contract for ``sketch_add_parametric_curve``."""
...
SKETCH_ADD_PARAMETRIC_CURVE_CONTRACT_VERSION: Literal[1] = 1
...
class SketchAddParametricCurveCollaborators(Protocol):
    @property
...
class SketchAddParametricCurveRequest:
...
class SketchAddParametricCurveSuccess(TypedDict):
...
class SketchAddParametricCurveFailure(TypedDict):
    contract_version: Literal[1]
...
class SketchAddParametricCurveUncertain(TypedDict):
    contract_version: Literal[1]
    success: Literal[False]
    ok: Literal[False]
...
SketchAddParametricCurveResult = SketchAddParametricCurveSuccess | SketchAddParametricCurveFailure | SketchAddParametricCurveUncertain
...
def make_sketch_add_parametric_curve_success(sketch: SketchName, geometry_index: int, sample_count: int) -> SketchAddParametricCurveSuccess:
    return {
        "contract_version": SKETCH_ADD_PARAMETRIC_CURVE_CONTRACT_VERSION,
        "success": True,
        "ok": True,
        "outcome": "committed",
        "committed": True,
...
def make_sketch_add_parametric_curve_failure(
    error_code: str,
    error: str,
    *,
    ...
    result: SketchAddParametricCurveFailure = {
        "contract_version": SKETCH_ADD_PARAMETRIC_CURVE_CONTRACT_VERSION,
        "success": False,
        "ok": False,
        "outcome": "rejected",
...
def make_sketch_add_parametric_curve_uncertain(
    error_code: str,
    error: str,
    *,
    committed: bool | None,
    ...
    result: SketchAddParametricCurveUncertain = {
        "contract_version": SKETCH_ADD_PARAMETRIC_CURVE_CONTRACT_VERSION,
        "success": False,
        "ok": False,
        "outcome": "uncertain",
...
def _invalid_response(response: dict[str, object]) -> SketchAddParametricCurveUncertain:
    ...
    code = (
        ...
        if committed
        else "SKETCH_ADD_PARAMETRIC_CURVE_ROLLBACK_UNCERTAIN"
        if rollback_failed
        else "INVALID_SKETCH_ADD_PARAMETRIC_CURVE_RESPONSE"
    )
    return make_sketch_add_parametric_curve_uncertain(
        code,
        "sketch_add_parametric_curve returned an invalid contract response; document state requires reconciliation",
        committed=True if committed else None,
        diagnostics={"response": response},
    )


def parse_sketch_add_parametric_curve_response(raw_response: object) -> SketchAddParametricCurveResult:
    response = _response_object(raw_response)
    if response is None:
        return make_sketch_add_parametric_curve_uncertain(
            "INVALID_RPC_RESPONSE",
            "sketch_add_parametric_curve returned a non-object response",
            committed=None,
        )
    version = response.get("contract_version")
    details = _response_details(response)
    if type(version) is not int or version != SKETCH_ADD_PARAMETRIC_CURVE_CONTRACT_VERSION or details is None:
    ...
    if (
        ...
        return make_sketch_add_parametric_curve_success(SketchName(sketch), geometry_index, sample_count)
    ...
    if (
        ...
        and "sample_count" not in response
    ):
        if _valid_rejection(response):
            return make_sketch_add_parametric_curve_failure(
                error_code,
                error,
        ...
        if _valid_uncertain(response):
            ...
            return make_sketch_add_parametric_curve_uncertain(error_code, error, committed=committed, **details)
    return _invalid_response(response)


__all__ = [
    "SKETCH_ADD_PARAMETRIC_CURVE_CONTRACT_VERSION",
