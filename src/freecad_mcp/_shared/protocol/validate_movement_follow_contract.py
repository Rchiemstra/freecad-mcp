"""Versioned, stdlib-only wire contract for ``validate_movement_follow``."""
...
VALIDATE_MOVEMENT_FOLLOW_CONTRACT_VERSION: Literal[1] = 1

DocumentName = NewType("DocumentName", str)
ValidateMovementFollowName = NewType("ValidateMovementFollowName", str)


class ValidateMovementFollowObject(Protocol):
...
class ValidateMovementFollowReadDocument(Protocol):
    """Read-only document surface available after native recompute."""

    @property
    def Name(self) -> str: ...

    def getObject(self, name: str) -> ValidateMovementFollowObject | None: ...


class ValidateMovementFollowDocument(ValidateMovementFollowReadDocument, Protocol):
    """Narrow mutation surface available only to the apply callback."""

    def addObject(self, object_type: str, name: str) -> ValidateMovementFollowObject: ...


@runtime_checkable
class NativeValidateMovementFollowDocument(ValidateMovementFollowDocument, Protocol):
...
class ValidateMovementFollowCollaborators(Protocol):
...
class ValidateMovementFollowRequest:
...
class ValidateMovementFollowSuccess(TypedDict):
...
class ValidateMovementFollowFailure(TypedDict):
...
class ValidateMovementFollowUncertain(TypedDict):
...
ValidateMovementFollowResult = ValidateMovementFollowSuccess | ValidateMovementFollowFailure | ValidateMovementFollowUncertain

_CORE_KEYS = frozenset(
    {
        "contract_version", "success", "ok", "outcome", "committed", "retry_safe", 'source', "error_code", "error", "native_status", "native_message", "rollback_succeeded", "rollback_failed", "diagnostics"
    }
)


def make_validate_movement_follow_success(source: str) -> ValidateMovementFollowSuccess:
    """Construct a complete committed result."""

    return {
        "contract_version": VALIDATE_MOVEMENT_FOLLOW_CONTRACT_VERSION,
        "success": True,
        "ok": True,
        "outcome": "committed",
        "committed": True,
...
def make_validate_movement_follow_failure(
    error_code: str,
    error: str,
    *,
    ...
    result: ValidateMovementFollowFailure = {
        "contract_version": VALIDATE_MOVEMENT_FOLLOW_CONTRACT_VERSION,
        "success": False,
        "ok": False,
        "outcome": "rejected",
...
def make_validate_movement_follow_uncertain(
    error_code: str,
    error: str,
    *,
    committed: bool | None,
    ...
    result: ValidateMovementFollowUncertain = {
        "contract_version": VALIDATE_MOVEMENT_FOLLOW_CONTRACT_VERSION,
        "success": False,
        "ok": False,
        "outcome": "uncertain",
...
def _invalid_response(response: dict[str, object]) -> ValidateMovementFollowUncertain:
    ...
    code = (
        ...
        if committed
        else "VALIDATE_MOVEMENT_FOLLOW_ROLLBACK_UNCERTAIN"
        if rollback_failed
        else "INVALID_VALIDATE_MOVEMENT_FOLLOW_RESPONSE"
    )
    return make_validate_movement_follow_uncertain(
        code,
        "validate_movement_follow returned an invalid contract response; document state requires reconciliation",
        committed=True if committed else None,
        diagnostics={"response": response},
    )


def parse_validate_movement_follow_response(raw_response: object) -> ValidateMovementFollowResult:
    """Validate all three wire variants; unknown state always stays uncertain."""
    response = _response_object(raw_response)
    if response is None:
        return make_validate_movement_follow_uncertain(
            "INVALID_RPC_RESPONSE",
            "validate_movement_follow returned a non-object response",
            committed=None,
        )
    version = response.get("contract_version")
    details = _response_details(response)
    if type(version) is not int or version != VALIDATE_MOVEMENT_FOLLOW_CONTRACT_VERSION or details is None:
    ...
    if (
        ...
        return make_validate_movement_follow_success(str(source))
    ...
    if (
        ...
        and error.strip()
        and 'source' not in response
    ):
        if _valid_rejection(response):
            return make_validate_movement_follow_failure(
                error_code,
                error,
        ...
        if _valid_uncertain(response):
            ...
            return make_validate_movement_follow_uncertain(error_code, error, committed=committed, **details)
    return _invalid_response(response)


__all__ = [
    "VALIDATE_MOVEMENT_FOLLOW_CONTRACT_VERSION",
