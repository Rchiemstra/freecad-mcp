"""Versioned, stdlib-only wire contract for ``get_document_tree``."""
...
GET_DOCUMENT_TREE_CONTRACT_VERSION: Literal[1] = 1
...
class GetDocumentTreeCollaborators(Protocol):
...
class GetDocumentTreeSuccess(TypedDict):
...
class GetDocumentTreeFailure(TypedDict):
...
class GetDocumentTreeUncertain(TypedDict):
...
GetDocumentTreeResult = GetDocumentTreeSuccess | GetDocumentTreeFailure | GetDocumentTreeUncertain
...
def make_get_document_tree_success(
    doc_name: str, root_filter: str, max_depth: int, roots: object,
) -> GetDocumentTreeSuccess:
    return {
        "contract_version": GET_DOCUMENT_TREE_CONTRACT_VERSION,
        "success": True,
        "ok": True,
        "outcome": "observed",
        "retry_safe": False,
...
def make_get_document_tree_failure(
    error_code: str,
    error: str,
    *,
...
def make_get_document_tree_uncertain(
    error_code: str,
    error: str,
    *,
    committed: bool | None,
...
def _invalid_response(response: dict[str, object]) -> GetDocumentTreeUncertain:
    ...
    code = (
        "GET_DOCUMENT_TREE_COMMITTED_RESPONSE_INVALID"
        if committed
        else "GET_DOCUMENT_TREE_ROLLBACK_UNCERTAIN"
        if rollback_failed
        else "INVALID_GET_DOCUMENT_TREE_RESPONSE"
    )
    return make_get_document_tree_uncertain(
        code,
        "get_document_tree returned an invalid contract response; document state requires reconciliation",
        committed=True if committed else None,
        diagnostics={"response": response},
    )


def parse_get_document_tree_response(raw_response: object) -> GetDocumentTreeResult:
    """Validate all three wire variants; unknown state always stays uncertain."""
    response = _response_object(raw_response)
    if response is None:
        return make_get_document_tree_uncertain(
            "INVALID_RPC_RESPONSE",
            "get_document_tree returned a non-object response",
            committed=None,
        )
    version = response.get("contract_version")
    details = _response_details(response)
    if type(version) is not int or version != GET_DOCUMENT_TREE_CONTRACT_VERSION or details is None:
        return _invalid_response(response)

    doc_name = response.get("doc_name")
    root_filter = response.get("root_filter")
    max_depth = response.get("max_depth")
    roots = response.get("roots")
    if _valid_success(response) and isinstance(doc_name, str) and isinstance(root_filter, str) and type(max_depth) is int and roots is not None:
        return make_get_document_tree_success(doc_name, root_filter, max_depth, roots)

    error_code = response.get("error_code")
    error = response.get("error")
    success_keys = {'doc_name', 'root_filter', 'max_depth', 'roots'}
    if (
        ...
        if _valid_rejection(response):
            return make_get_document_tree_failure(
                error_code,
                error,
        ...
        if _valid_uncertain(response):
            ...
            return make_get_document_tree_uncertain(error_code, error, committed=committed, **details)
    return _invalid_response(response)


__all__ = [
    "GET_DOCUMENT_TREE_CONTRACT_VERSION",
