"""Versioned, stdlib-only wire contract for ``insert_part_from_library``."""
...
INSERT_PART_FROM_LIBRARY_CONTRACT_VERSION: Literal[1] = 1

DocumentName = NewType("DocumentName", str)


class InsertPartFromLibraryReadDocument(Protocol):
...
class InsertPartFromLibraryDocument(InsertPartFromLibraryReadDocument, Protocol):
    """Narrow mutation surface available only to the apply callback."""

    def addObject(self, object_type: str, name: str) -> object: ...


class InsertPartFromLibraryCollaborators(Protocol):
...
class InsertPartFromLibraryRequest:
...
class InsertPartFromLibrarySuccess(TypedDict):
...
class InsertPartFromLibraryFailure(TypedDict):
...
class InsertPartFromLibraryUncertain(TypedDict):
...
InsertPartFromLibraryResult = InsertPartFromLibrarySuccess | InsertPartFromLibraryFailure | InsertPartFromLibraryUncertain
...
def make_insert_part_from_library_success(document_name: DocumentName, relative_path: str) -> InsertPartFromLibrarySuccess:
    """Construct a complete committed result."""

    return {
        "contract_version": INSERT_PART_FROM_LIBRARY_CONTRACT_VERSION,
        "success": True,
        "ok": True,
        "outcome": "committed",
        "committed": True,
...
def make_insert_part_from_library_failure(
    error_code: str,
    error: str,
    *,
    ...
    result: InsertPartFromLibraryFailure = {
        "contract_version": INSERT_PART_FROM_LIBRARY_CONTRACT_VERSION,
        "success": False,
        "ok": False,
        "outcome": "rejected",
...
def make_insert_part_from_library_uncertain(
    error_code: str,
    error: str,
    *,
    committed: bool | None,
    ...
    result: InsertPartFromLibraryUncertain = {
        "contract_version": INSERT_PART_FROM_LIBRARY_CONTRACT_VERSION,
        "success": False,
        "ok": False,
        "outcome": "uncertain",
...
def _invalid_response(response: dict[str, object]) -> InsertPartFromLibraryUncertain:
    ...
    code = (
        ...
        if committed
        else "INSERT_PART_FROM_LIBRARY_ROLLBACK_UNCERTAIN"
        if rollback_failed
        else "INVALID_INSERT_PART_FROM_LIBRARY_RESPONSE"
    )
    return make_insert_part_from_library_uncertain(
        code,
        "insert_part_from_library returned an invalid contract response; document state requires reconciliation",
        committed=True if committed else None,
        diagnostics={"response": response},
    )


def parse_insert_part_from_library_response(raw_response: object) -> InsertPartFromLibraryResult:
    """Validate all three wire variants; unknown state always stays uncertain."""
    response = _response_object(raw_response)
    if response is None:
        return make_insert_part_from_library_uncertain(
            "INVALID_RPC_RESPONSE",
            "insert_part_from_library returned a non-object response",
            committed=None,
        )
    version = response.get("contract_version")
    details = _response_details(response)
    if type(version) is not int or version != INSERT_PART_FROM_LIBRARY_CONTRACT_VERSION or details is None:
    ...
    if (
        ...
        return make_insert_part_from_library_success(DocumentName(document_name), relative_path)
    ...
    if (
        ...
        and error.strip()
        and "document_name" not in response and "relative_path" not in response
    ):
        ...
        if _valid_uncertain(response):
            ...
            return make_insert_part_from_library_uncertain(error_code, error, committed=committed, **details)
    return _invalid_response(response)


__all__ = [
    "INSERT_PART_FROM_LIBRARY_CONTRACT_VERSION",
    "InsertPartFromLibraryCollaborators",
    "InsertPartFromLibraryDocument",
    "InsertPartFromLibraryFailure",
