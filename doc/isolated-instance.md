...
runtime but does not encrypt either the legacy XML-RPC or JSON-RPC 2.0 transport.
For remote use, keep the manifest endpoint local and connect an SSH tunnel or TLS
proxy to that loopback endpoint; do not replace `rpc_host` with a LAN address. See
