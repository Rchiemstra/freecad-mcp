#!/usr/bin/env python3
"""Fast static and architecture gate for the typed ``set_expression`` slice."""
...
_LEAF = "addon/FreeCADMCP/rpc_server/methods/cad_methods_ops/set_expression.py"
_MUTATION = "addon/FreeCADMCP/rpc_server/methods/cad_methods_ops/set_expression_mutation.py"
_PUBLIC_ADAPTER = "src/freecad_mcp/operations/parametric_ops/set_expression.py"
_ADDON_CONTRACT = "addon/FreeCADMCP/_shared/protocol/set_expression_contract.py"
_CLIENT_CONTRACT = "src/freecad_mcp/_shared/protocol/set_expression_contract.py"
_TEMPLATE = "src/freecad_mcp/templates/parametric/set_expression.py.txt"
...
def _scan_leaf(source: str) -> list[str]:
    ...
    apply_function = _function(tree, "apply_set_expression")
    apply_closure = _class_method(tree, "_SetExpressionExecution", "apply")
    run_function = _class_method(tree, "_SetExpressionExecution", "run")
    forbidden = {
        "abortTransaction",
        "commitTransaction",
        "execute_code",
    ...
    violations = ["SET_EXPRESSION001 leaf owns forbidden execution: " + ", ".join(found)] if found else []
    if "inspect" in _called_names(apply_closure):
        violations.append("SET_EXPRESSION002 inspection runs inside apply")
    mutation_calls = [
        node
        for node in ast.walk(run_function)
        if isinstance(node, ast.Call)
        and isinstance(node.func, ast.Name)
        and node.func.id == "run_set_expression_native_mutation"
    ]
    if len(mutation_calls) != 1:
        return [*violations, "SET_EXPRESSION003 must use exactly one typed mutation call"]
    ...
    if not (isinstance(postcondition, ast.Attribute) and postcondition.attr == "inspect"):
        violations.append("SET_EXPRESSION004 missing typed postcondition=inspect")
    return violations


def _scan_native_release(source: str) -> list[str]:
    tree = ast.parse(source, filename=_MUTATION)
    native_result = _function(tree, "_set_expression_native_result")
    runner = _function(tree, "run_set_expression_native_mutation")
    violations = []
    import types

    module = types.ModuleType("addon.FreeCADMCP.rpc_server.methods.cad_methods_ops._set_expression_gate")
    ...
    try:
        ...
        for status in (
            ...
            result = module._set_expression_native_result({"status": status, "committed": False}, state)
            if result is True:
                violations.append("SET_EXPRESSION007 cached success escaped native rejection")
                break
        state = module._NativeMutationState()
        result = module._set_expression_native_result({"status": "Committed", "committed": True}, state)
        if result is True:
            violations.append("SET_EXPRESSION015 commit escaped without a successful postcondition")
    finally:
    ...
    if len(commit_calls) != 1:
        violations.append("SET_EXPRESSION012 typed path must use exactly one native commit call")
        return violations
    call = commit_calls[0]
    ...
    if not isinstance(forwarded, ast.Name) or forwarded.id != "native_postcondition":
        violations.append("SET_EXPRESSION013 typed path dropped its native postcondition")
    if any(isinstance(node, ast.Name) and node.id == "Any" for node in ast.walk(runner)):
        violations.append("SET_EXPRESSION014 typed native path contains Any")
...
def _scan_public_adapter(source: str) -> list[str]:
    ...
    operation = _function(tree, "set_expression_operation")
    calls = set(_called_names(operation))
    violations = []
    if "parse_set_expression_response" not in calls:
        violations.append("SET_EXPRESSION008 public adapter bypasses response validation")
    if {"_run_json_code", "execute_code", "render_template_lines"} & calls:
        violations.append("SET_EXPRESSION009 public route returned to generated execution")
    return violations
...
def _scan_typed_surface(root: Path, overrides: Mapping[str, str]) -> list[str]:
    ...
    return [
        f"SET_EXPRESSION014 typed surface contains Any: {name}"
        for name, node in nodes.items()
        if _contains_any(node)
    ]


def scan_set_expression_architecture(
    root: Path,
    *,
    ...
    if addon_contract != client_contract:
        violations.append("SET_EXPRESSION010 protocol vendors differ")
    if _TEMPLATE and not overrides and (root / _TEMPLATE).exists():
        violations.append("SET_EXPRESSION011 generated source template exists")
    return violations
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    violations = scan_set_expression_architecture(root)
    ...
    print("set_expression contract: architecture and static types passed")
    return 0
