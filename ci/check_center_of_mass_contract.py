#!/usr/bin/env python3
"""Fast static and architecture gate for the typed ``center_of_mass`` slice."""
...
_LEAF = "addon/FreeCADMCP/rpc_server/methods/cad_methods_ops/center_of_mass.py"
_MUTATION = "addon/FreeCADMCP/rpc_server/methods/cad_methods_ops/center_of_mass_mutation.py"
...
_PUBLIC_ADAPTER = "src/freecad_mcp/operations/parametric_ops/center_of_mass.py"
_CLIENT = "src/freecad_mcp/freecad_client_ops/freecad_connection.py"
_ADDON_CONTRACT = "addon/FreeCADMCP/_shared/protocol/center_of_mass_contract.py"
_CLIENT_CONTRACT = "src/freecad_mcp/_shared/protocol/center_of_mass_contract.py"
...
def _scan_leaf(source: str) -> list[str]:
    ...
    apply_function = _function(tree, "apply_center_of_mass")
    apply_closure = _class_method(tree, "_CenterOfMassExecution", "apply")
    run_function = _class_method(tree, "_CenterOfMassExecution", "run")
    forbidden = {
        "abortTransaction",
        "commitTransaction",
        "execute_code",
    ...
    violations = ["CENTER_OF_MASS001 leaf owns forbidden execution: " + ", ".join(found)] if found else []
    if "inspect" in _called_names(apply_closure):
        violations.append("CENTER_OF_MASS002 inspection runs inside apply")
    mutation_calls = [
        node
        for node in ast.walk(run_function)
        if isinstance(node, ast.Call)
        and isinstance(node.func, ast.Name)
        and node.func.id == "run_center_of_mass_native_mutation"
    ]
    if len(mutation_calls) != 1:
        return [*violations, "CENTER_OF_MASS003 must use exactly one typed mutation call"]
    ...
    if not (isinstance(postcondition, ast.Attribute) and postcondition.attr == "inspect"):
        violations.append("CENTER_OF_MASS004 missing typed postcondition=inspect")
    return violations


def _scan_native_release(source: str) -> list[str]:
    tree = ast.parse(source, filename=_MUTATION)
    native_result = _function(tree, "_center_of_mass_native_result")
    runner = _function(tree, "run_center_of_mass_native_mutation")
    violations = []
    import types

    module = types.ModuleType("addon.FreeCADMCP.rpc_server.methods.cad_methods_ops._center_of_mass_gate")
    ...
    try:
        ...
        for status in (
            ...
            result = module._center_of_mass_native_result({"status": status, "committed": False}, state)
            if result is True:
                violations.append("CENTER_OF_MASS007 cached success escaped native rejection")
                break
        state = module._NativeMutationState()
        result = module._center_of_mass_native_result({"status": "Committed", "committed": True}, state)
        if result is True:
            violations.append("CENTER_OF_MASS015 commit escaped without a successful postcondition")
    finally:
    ...
    if len(commit_calls) != 1:
        violations.append("CENTER_OF_MASS012 typed path must use exactly one native commit call")
        return violations
    call = commit_calls[0]
    ...
    if not isinstance(forwarded, ast.Name) or forwarded.id != "native_postcondition":
        violations.append("CENTER_OF_MASS013 typed path dropped its native postcondition")
    ...
    if any(
        ...
        violations.append("CENTER_OF_MASS014 typed native path contains Any")
    return violations


def _scan_public_adapter(source: str) -> list[str]:
    tree = ast.parse(source, filename=_PUBLIC_ADAPTER)
    operation = _function(tree, "center_of_mass_operation")
    calls = set(_called_names(operation))
    violations = []
    if "parse_center_of_mass_response" not in calls:
        violations.append("CENTER_OF_MASS008 public adapter bypasses response validation")
    if {"_run_json_code", "execute_code", "render_template_lines"} & calls:
        violations.append("CENTER_OF_MASS009 public route returned to generated execution")
    return violations
...
def _scan_typed_surface(root: Path, overrides: Mapping[str, str]) -> list[str]:
    ...
    return [
        f"CENTER_OF_MASS014 typed surface contains Any: {name}"
        for name, node in nodes.items()
        if _contains_any(node)
    ]


def scan_center_of_mass_architecture(
    root: Path,
    *,
    ...
    if addon_contract != client_contract:
        violations.append("CENTER_OF_MASS010 protocol vendors differ")
    return violations
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    print("center_of_mass contract: architecture and static types passed")
    return 0
