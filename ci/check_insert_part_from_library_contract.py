#!/usr/bin/env python3
"""Fast static and architecture gate for the typed ``insert_part_from_library`` slice."""
...
_LEAF = "addon/FreeCADMCP/rpc_server/methods/cad_methods_ops/insert_part_from_library.py"
_MUTATION = "addon/FreeCADMCP/rpc_server/methods/cad_methods_ops/insert_part_from_library_mutation.py"
_PUBLIC_ADAPTER = "src/freecad_mcp/operations/parametric_ops/insert_part_from_library.py"
_CLIENT = "src/freecad_mcp/freecad_client_ops/freecad_connection.py"
_ADDON_CONTRACT = "addon/FreeCADMCP/_shared/protocol/insert_part_from_library_contract.py"
_CLIENT_CONTRACT = "src/freecad_mcp/_shared/protocol/insert_part_from_library_contract.py"
...
def _scan_leaf(source: str) -> list[str]:
    ...
    apply_function = _function(tree, "apply_insert_part_from_library")
    apply_closure = _class_method(tree, "_InsertPartFromLibraryExecution", "apply")
    run_function = _class_method(tree, "_InsertPartFromLibraryExecution", "run")
    forbidden = {
        "abortTransaction",
        "commitTransaction",
        "execute_code",
    ...
    violations = ["INSERT_PART_FROM_LIBRARY001 leaf owns forbidden execution: " + ", ".join(found)] if found else []
    if "inspect" in _called_names(apply_closure):
        violations.append("INSERT_PART_FROM_LIBRARY002 inspection runs inside apply")
    mutation_calls = [
        node
        for node in ast.walk(run_function)
        if isinstance(node, ast.Call)
        and isinstance(node.func, ast.Name)
        and node.func.id == "run_insert_part_from_library_native_mutation"
    ]
    if len(mutation_calls) != 1:
        return [*violations, "INSERT_PART_FROM_LIBRARY003 must use exactly one typed mutation call"]
    ...
    if not (isinstance(postcondition, ast.Attribute) and postcondition.attr == "inspect"):
        violations.append("INSERT_PART_FROM_LIBRARY004 missing typed postcondition=inspect")
    return violations


def _scan_native_release(source: str) -> list[str]:
    tree = ast.parse(source, filename=_MUTATION)
    native_result = _function(tree, "_insert_part_from_library_native_result")
    runner = _function(tree, "run_insert_part_from_library_native_mutation")
    violations: list[str] = []
    import types

    module = types.ModuleType("addon.FreeCADMCP.rpc_server.methods.cad_methods_ops._insert_part_from_library_gate")
    ...
    try:
        ...
        for status in (
            ...
            result = module._insert_part_from_library_native_result({"status": status, "committed": False}, state)
            if result is True:
                violations.append("INSERT_PART_FROM_LIBRARY007 cached success escaped native rejection")
                break
        state = module._NativeMutationState()
        result = module._insert_part_from_library_native_result({"status": "Committed", "committed": True}, state)
        if result is True:
            violations.append("INSERT_PART_FROM_LIBRARY015 commit escaped without a successful postcondition")
    finally:
    ...
    if len(commit_calls) != 1:
        violations.append("INSERT_PART_FROM_LIBRARY012 typed path must use exactly one native commit call")
        return violations
    call = commit_calls[0]
    ...
    if not isinstance(forwarded, ast.Name) or forwarded.id != "native_postcondition":
        violations.append("INSERT_PART_FROM_LIBRARY013 typed path dropped its native postcondition")
    ...
    if any(
        ...
        violations.append("INSERT_PART_FROM_LIBRARY014 typed native path contains Any")
    return violations


def _scan_public_adapter(source: str) -> list[str]:
    tree = ast.parse(source, filename=_PUBLIC_ADAPTER)
    operation = _function(tree, "insert_part_from_library_operation")
    calls = set(_called_names(operation))
    violations = []
    if "parse_insert_part_from_library_response" not in calls:
        violations.append("INSERT_PART_FROM_LIBRARY008 public adapter bypasses response validation")
    if {"_run_json_code", "execute_code", "render_template_lines"} & calls:
        violations.append("INSERT_PART_FROM_LIBRARY009 public route returned to generated execution")
    return violations
...
def _scan_typed_surface(root: Path, overrides: Mapping[str, str]) -> list[str]:
    ...
    return [
        f"INSERT_PART_FROM_LIBRARY014 typed surface contains Any: {label}"
        for label, node in nodes.items()
        if _contains_any(node)
    ]


def scan_insert_part_from_library_architecture(
    root: Path,
    *,
    ...
    if _source(root, _ADDON_CONTRACT, overrides).encode() != _source(
        ...
        violations.append("INSERT_PART_FROM_LIBRARY010 protocol vendors differ")
    return violations
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    violations = scan_insert_part_from_library_architecture(root)
    for violation in violations:
        print(violation)
    if violations:
        return 1
    print("insert_part_from_library contract: architecture passed")
    return 0
