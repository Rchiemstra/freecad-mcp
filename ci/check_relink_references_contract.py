#!/usr/bin/env python3
"""Fast static and architecture gate for the typed ``relink_references`` slice."""
...
_LEAF = "addon/FreeCADMCP/rpc_server/methods/cad_methods_ops/relink_references.py"
_MUTATION = "addon/FreeCADMCP/rpc_server/methods/cad_methods_ops/relink_references_mutation.py"
_PUBLIC_ADAPTER = "src/freecad_mcp/operations/parametric_ops/relink_references.py"
_ADDON_CONTRACT = "addon/FreeCADMCP/_shared/protocol/relink_references_contract.py"
_CLIENT_CONTRACT = "src/freecad_mcp/_shared/protocol/relink_references_contract.py"
_TEMPLATE = "src/freecad_mcp/templates/diagnostics/relink_references.py.txt"
...
def _scan_leaf(source: str) -> list[str]:
    ...
    apply_function = _function(tree, "apply_relink_references")
    apply_closure = _class_method(tree, "_RelinkReferencesExecution", "apply")
    run_function = _class_method(tree, "_RelinkReferencesExecution", "run")
    forbidden = {
        "abortTransaction",
        "commitTransaction",
        "execute_code",
    ...
    violations = ["RELINK_REFERENCES001 leaf owns forbidden execution: " + ", ".join(found)] if found else []
    if "inspect" in _called_names(apply_closure):
        violations.append("RELINK_REFERENCES002 inspection runs inside apply")
    mutation_calls = [
        node
        for node in ast.walk(run_function)
        if isinstance(node, ast.Call)
        and isinstance(node.func, ast.Name)
        and node.func.id == "run_relink_references_native_mutation"
    ]
    if len(mutation_calls) != 1:
        return [*violations, "RELINK_REFERENCES003 must use exactly one typed mutation call"]
    ...
    if not (isinstance(postcondition, ast.Attribute) and postcondition.attr == "inspect"):
        violations.append("RELINK_REFERENCES004 missing typed postcondition=inspect")
    return violations


def _scan_native_release(source: str) -> list[str]:
    tree = ast.parse(source, filename=_MUTATION)
    native_result = _function(tree, "_relink_references_native_result")
    runner = _function(tree, "run_relink_references_native_mutation")
    violations = []
    import types

    module = types.ModuleType("addon.FreeCADMCP.rpc_server.methods.cad_methods_ops._relink_references_gate")
    ...
    try:
        ...
        for status in (
            ...
            result = module._relink_references_native_result({"status": status, "committed": False}, state)
            if result is True:
                violations.append("RELINK_REFERENCES007 cached success escaped native rejection")
                break
        state = module._NativeMutationState()
        result = module._relink_references_native_result({"status": "Committed", "committed": True}, state)
        if result is True:
            violations.append("RELINK_REFERENCES015 commit escaped without a successful postcondition")
    finally:
    ...
    if len(commit_calls) != 1:
        violations.append("RELINK_REFERENCES012 typed path must use exactly one native commit call")
        return violations
    call = commit_calls[0]
    ...
    if not isinstance(forwarded, ast.Name) or forwarded.id != "native_postcondition":
        violations.append("RELINK_REFERENCES013 typed path dropped its native postcondition")
    if any(isinstance(node, ast.Name) and node.id == "Any" for node in ast.walk(runner)):
        violations.append("RELINK_REFERENCES014 typed native path contains Any")
...
def _scan_public_adapter(source: str) -> list[str]:
    ...
    operation = _function(tree, "relink_references_operation")
    calls = set(_called_names(operation))
    violations = []
    if "parse_relink_references_response" not in calls:
        violations.append("RELINK_REFERENCES008 public adapter bypasses response validation")
    if {"_run_json_code", "execute_code", "render_template_lines"} & calls:
        violations.append("RELINK_REFERENCES009 public route returned to generated execution")
    return violations
...
def _scan_typed_surface(root: Path, overrides: Mapping[str, str]) -> list[str]:
    ...
    return [
        f"RELINK_REFERENCES014 typed surface contains Any: {name}"
        for name, node in nodes.items()
        if _contains_any(node)
    ]


def scan_relink_references_architecture(
    root: Path,
    *,
    ...
    if addon_contract != client_contract:
        violations.append("RELINK_REFERENCES010 protocol vendors differ")
    if _TEMPLATE and not overrides and (root / _TEMPLATE).exists():
        violations.append("RELINK_REFERENCES011 generated source template exists")
    return violations
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    violations = scan_relink_references_architecture(root)
    ...
    print("relink_references contract: architecture and static types passed")
    return 0
