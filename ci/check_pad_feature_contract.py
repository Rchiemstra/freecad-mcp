#!/usr/bin/env python3
"""Fast static and architecture gate for the typed ``pad_feature`` slice."""
...
_LEAF = "addon/FreeCADMCP/rpc_server/methods/cad_methods_ops/pad_feature.py"
_MUTATION = "addon/FreeCADMCP/rpc_server/methods/cad_methods_ops/pad_feature_mutation.py"
...
_PUBLIC_ADAPTER = "src/freecad_mcp/operations/parametric_ops/pad_feature.py"
_CLIENT = "src/freecad_mcp/freecad_client_ops/freecad_connection.py"
_ADDON_CONTRACT = "addon/FreeCADMCP/_shared/protocol/pad_feature_contract.py"
_CLIENT_CONTRACT = "src/freecad_mcp/_shared/protocol/pad_feature_contract.py"
...
def _scan_leaf(source: str) -> list[str]:
    ...
    apply_function = _function(tree, "apply_pad_feature")
    apply_closure = _class_method(tree, "_PadFeatureExecution", "apply")
    run_function = _class_method(tree, "_PadFeatureExecution", "run")
    forbidden = {
        "abortTransaction",
        "commitTransaction",
        "execute_code",
    ...
    violations = ["PAD_001 leaf owns forbidden execution: " + ", ".join(found)] if found else []
    if "inspect" in _called_names(apply_closure):
        violations.append("PAD_002 inspection runs inside apply")
    mutation_calls = [
        node
        for node in ast.walk(run_function)
        if isinstance(node, ast.Call)
        and isinstance(node.func, ast.Name)
        and node.func.id == "run_pad_feature_native_mutation"
    ]
    if len(mutation_calls) != 1:
        return [*violations, "PAD_003 pad_feature must use exactly one typed mutation call"]
    ...
    if not (isinstance(postcondition, ast.Attribute) and postcondition.attr == "inspect"):
        violations.append("PAD_004 missing typed postcondition=inspect")
    return violations
...
def _scan_bridge(source: str) -> list[str]:
    ...
    if len(lookup_calls) != 1:
        violations.append("PAD_005 bridge must resolve the admitted document exactly once")
    ...
    if callback_count != 1 or "_commit_without_native" in _called_names(bridge_commit):
        violations.append("PAD_006 postcondition path can reach non-native callback fallback")
    native_calls = [
        node
        for node in ast.walk(bridge_commit)
        if isinstance(node, ast.Call)
    ...
    if len(native_calls) != 1:
        violations.append("PAD_012 typed bridge must use exactly one native commit call")
    else:
        ...
        if not (
            ...
            violations.append("PAD_013 typed bridge lost its fixed native policy")
    if any(isinstance(node, ast.Name) and node.id == "Any" for node in ast.walk(bridge_commit)):
        violations.append("PAD_014 typed native pad_feature bridge contains Any")
    return violations


def _scan_native_release(source: str) -> list[str]:
    tree = ast.parse(source, filename=_MUTATION)
    native_result = _function(tree, "_pad_feature_native_result")
    body_runner = _function(tree, "run_pad_feature_native_mutation")
    violations = []
    import types

    module = types.ModuleType("addon.FreeCADMCP.rpc_server.methods.cad_methods_ops._pad_feature_gate")
    ...
    try:
        ...
        state = module._NativePadFeatureMutationState(postcondition_passed=True)
        for status in (
            "Busy",
            "ApplyFailed",
            "PostconditionFailed",
            ...
            result = module._pad_feature_native_result({"status": status, "committed": False}, state)
            if result is True:
                violations.append("PAD_007 cached success escaped native rejection")
                break
        state = module._NativePadFeatureMutationState()
        result = module._pad_feature_native_result({"status": "Committed", "committed": True}, state)
        if result is True:
            violations.append("PAD_015 commit escaped without a successful postcondition")
    finally:
    ...
    if len(commit_calls) != 1:
        violations.append("PAD_012 typed path must use exactly one native commit call")
        return violations
    call = commit_calls[0]
    ...
    if not isinstance(forwarded, ast.Name) or forwarded.id != "native_postcondition":
        violations.append("PAD_013 typed path dropped its native postcondition")
    if any(
        ...
        violations.append("PAD_012 typed path must not call a per-op commit method")

    typed_nodes: list[ast.AST] = [body_runner, native_result]
    for class_node in tree.body:
        if isinstance(class_node, ast.ClassDef) and class_node.name == "_NativePadFeatureMutationState":
    ...
    if any(
        ...
        violations.append("PAD_014 typed native pad_feature path contains Any")
    return violations


def _scan_public_adapter(source: str) -> list[str]:
    tree = ast.parse(source, filename=_PUBLIC_ADAPTER)
    operation = _function(tree, "pad_feature_operation")
    calls = set(_called_names(operation))
    violations = []
    if "parse_pad_feature_response" not in calls:
        violations.append("PAD_008 public adapter bypasses response validation")
    if {"_run_json_code", "execute_code", "render_template_lines"} & calls:
        violations.append("PAD_009 public pad_feature route returned to generated execution")
    return violations
...
def _scan_typed_surface(root: Path, overrides: Mapping[str, str]) -> list[str]:
    nodes = {
        "PadFeature leaf": ast.parse(_source(root, _LEAF, overrides)),
    ...
    return [
        f"PAD_014 typed PadFeature surface contains Any: {name}"
        for name, node in nodes.items()
        if _contains_any(node)
    ]


def scan_pad_feature_architecture(
    root: Path,
    *,
    source_overrides: Mapping[str, str] | None = None,
) -> list[str]:
    """Return stable violations for the pad_feature execution contract."""
    ...
    if addon_contract != client_contract:
        violations.append("PAD_010 protocol vendors differ")
    return violations
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    violations = scan_pad_feature_architecture(root)
    ...
    print("pad_feature contract: architecture and static types passed")
    return 0
