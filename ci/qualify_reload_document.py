"""Repeatable acceptance lanes for typed ``reload_document``.
...
PYTHON_TESTS = (
    "tests/reload/test_reload_document.py",
    "tests/reload/test_reload_document_response.py",
    "tests/reload/test_reload_document_contract_gate.py",
    "tests/reload/test_reload_document_json_rpc_contract.py",
)
...
def main() -> int:
    ...
    if args.native:
        ...
        tests = ("tests/native/test_native_reload_document.py",)
    else:
        result = subprocess.run(
            [sys.executable, "ci/check_reload_document_contract.py"],
            cwd=root,
            check=False,
        )
