#!/usr/bin/env python3
"""Fast static and architecture gate for the typed ``sketch_add_geometry`` slice."""
...
_LEAF = "addon/FreeCADMCP/rpc_server/methods/cad_methods_ops/sketch_add_geometry.py"
_MUTATION = "addon/FreeCADMCP/rpc_server/methods/cad_methods_ops/sketch_add_geometry_mutation.py"
...
_PUBLIC_ADAPTER = "src/freecad_mcp/operations/parametric_ops/sketch_add_geometry.py"
_CLIENT = "src/freecad_mcp/freecad_client_ops/freecad_connection.py"
_ADDON_CONTRACT = "addon/FreeCADMCP/_shared/protocol/sketch_add_geometry_contract.py"
_CLIENT_CONTRACT = "src/freecad_mcp/_shared/protocol/sketch_add_geometry_contract.py"
...
def _scan_leaf(source: str) -> list[str]:
    ...
    apply_function = _function(tree, "apply_sketch_add_geometry")
    apply_closure = _class_method(tree, "_SketchAddGeometryExecution", "apply")
    run_function = _class_method(tree, "_SketchAddGeometryExecution", "run")
    forbidden = {
        "abortTransaction",
        "commitTransaction",
        "execute_code",
    ...
    mutation_calls = [
        ...
        and node.func.id == "run_sketch_add_geometry_native_mutation"
    ]
    if len(mutation_calls) != 1:
        return [*violations, "SKET003 sketch_add_geometry must use exactly one typed mutation call"]
...
def _scan_bridge(source: str) -> list[str]:
    ...
    if any(isinstance(node, ast.Name) and node.id == "Any" for node in ast.walk(bridge_commit)):
        violations.append("SKET014 typed native sketch_add_geometry bridge contains Any")
    return violations


def _scan_native_release(source: str) -> list[str]:
    tree = ast.parse(source, filename=_MUTATION)
    native_result = _function(tree, "_sketch_add_geometry_native_result")
    body_runner = _function(tree, "run_sketch_add_geometry_native_mutation")
    violations = []
    import types

    module = types.ModuleType("addon.FreeCADMCP.rpc_server.methods.cad_methods_ops._sketch_add_geometry_gate")
    ...
    try:
        ...
        state = module._NativeSketchAddGeometryMutationState(postcondition_passed=True)
        for status in (
            "Busy",
            "ApplyFailed",
            "PostconditionFailed",
            ...
            result = module._sketch_add_geometry_native_result({"status": status, "committed": False}, state)
            if result is True:
                violations.append("SKET007 cached success escaped native rejection")
                break
        state = module._NativeSketchAddGeometryMutationState()
        result = module._sketch_add_geometry_native_result({"status": "Committed", "committed": True}, state)
        if result is True:
    ...
    for class_node in tree.body:
        if isinstance(class_node, ast.ClassDef) and class_node.name == "_NativeSketchAddGeometryMutationState":
    ...
    if any(
        ...
        violations.append("SKET014 typed native sketch_add_geometry path contains Any")
    return violations


def _scan_public_adapter(source: str) -> list[str]:
    tree = ast.parse(source, filename=_PUBLIC_ADAPTER)
    operation = _function(tree, "sketch_add_geometry_operation")
    calls = set(_called_names(operation))
    violations = []
    if "parse_sketch_add_geometry_response" not in calls:
    ...
    if {"_run_json_code", "execute_code", "render_template_lines"} & calls:
        violations.append("SKET009 public sketch_add_geometry route returned to generated execution")
    return violations
...
def _scan_typed_surface(root: Path, overrides: Mapping[str, str]) -> list[str]:
    nodes = {
        "SketchAddGeometry leaf": ast.parse(_source(root, _LEAF, overrides)),
    ...
    return [
        f"SKET014 typed SketchAddGeometry surface contains Any: {name}"
        for name, node in nodes.items()
        if _contains_any(node)
    ]


def scan_sketch_add_geometry_architecture(
    root: Path,
    *,
    source_overrides: Mapping[str, str] | None = None,
) -> list[str]:
    """Return stable violations for the sketch_add_geometry execution contract."""
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    violations = scan_sketch_add_geometry_architecture(root)
    ...
    print("sketch_add_geometry contract: architecture and static types passed")
    return 0
