"""Repeatable acceptance lanes for the typed ``pocket_feature`` implementation.
...
PYTHON_TESTS = (
    "tests/pocket/test_pocket_feature.py",
    "tests/pocket/test_pocket_feature_response.py",
    "tests/pocket/test_pocket_feature_contract_gate.py",
    "tests/test_typed_platform_discovery.py",
    "tests/pocket/test_pocket_feature_json_rpc_contract.py",
...
def main() -> int:
    ...
    if args.native:
        ...
        tests = ("tests/native/test_native_pocket_feature.py",)
    else:
        result = subprocess.run(
            [sys.executable, "ci/check_pocket_feature_contract.py"],
            cwd=root,
            check=False,
        )
