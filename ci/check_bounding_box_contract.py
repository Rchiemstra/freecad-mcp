#!/usr/bin/env python3
"""Fast static and architecture gate for the typed ``bounding_box`` slice."""
...
_LEAF = "addon/FreeCADMCP/rpc_server/methods/cad_methods_ops/bounding_box.py"
_MUTATION = "addon/FreeCADMCP/rpc_server/methods/cad_methods_ops/bounding_box_mutation.py"
...
_PUBLIC_ADAPTER = "src/freecad_mcp/operations/parametric_ops/bounding_box.py"
_CLIENT = "src/freecad_mcp/freecad_client_ops/freecad_connection.py"
_ADDON_CONTRACT = "addon/FreeCADMCP/_shared/protocol/bounding_box_contract.py"
_CLIENT_CONTRACT = "src/freecad_mcp/_shared/protocol/bounding_box_contract.py"
...
def _scan_leaf(source: str) -> list[str]:
    ...
    apply_function = _function(tree, "apply_bounding_box")
    apply_closure = _class_method(tree, "_BoundingBoxExecution", "apply")
    run_function = _class_method(tree, "_BoundingBoxExecution", "run")
    forbidden = {
        "abortTransaction",
        "commitTransaction",
        "execute_code",
    ...
    violations = ["BOUNDING_BOX001 leaf owns forbidden execution: " + ", ".join(found)] if found else []
    if "inspect" in _called_names(apply_closure):
        violations.append("BOUNDING_BOX002 inspection runs inside apply")
    mutation_calls = [
        node
        for node in ast.walk(run_function)
        if isinstance(node, ast.Call)
        and isinstance(node.func, ast.Name)
        and node.func.id == "run_bounding_box_native_mutation"
    ]
    if len(mutation_calls) != 1:
        return [*violations, "BOUNDING_BOX003 must use exactly one typed mutation call"]
    ...
    if not (isinstance(postcondition, ast.Attribute) and postcondition.attr == "inspect"):
        violations.append("BOUNDING_BOX004 missing typed postcondition=inspect")
    return violations


def _scan_native_release(source: str) -> list[str]:
    tree = ast.parse(source, filename=_MUTATION)
    native_result = _function(tree, "_bounding_box_native_result")
    runner = _function(tree, "run_bounding_box_native_mutation")
    violations = []
    import types

    module = types.ModuleType("addon.FreeCADMCP.rpc_server.methods.cad_methods_ops._bounding_box_gate")
    ...
    try:
        ...
        for status in (
            ...
            result = module._bounding_box_native_result({"status": status, "committed": False}, state)
            if result is True:
                violations.append("BOUNDING_BOX007 cached success escaped native rejection")
                break
        state = module._NativeMutationState()
        result = module._bounding_box_native_result({"status": "Committed", "committed": True}, state)
        if result is True:
            violations.append("BOUNDING_BOX015 commit escaped without a successful postcondition")
    finally:
    ...
    if len(commit_calls) != 1:
        violations.append("BOUNDING_BOX012 typed path must use exactly one native commit call")
        return violations
    call = commit_calls[0]
    ...
    if not isinstance(forwarded, ast.Name) or forwarded.id != "native_postcondition":
        violations.append("BOUNDING_BOX013 typed path dropped its native postcondition")
    ...
    if any(
        ...
        violations.append("BOUNDING_BOX014 typed native path contains Any")
    return violations


def _scan_public_adapter(source: str) -> list[str]:
    tree = ast.parse(source, filename=_PUBLIC_ADAPTER)
    operation = _function(tree, "bounding_box_operation")
    calls = set(_called_names(operation))
    violations = []
    if "parse_bounding_box_response" not in calls:
        violations.append("BOUNDING_BOX008 public adapter bypasses response validation")
    if {"_run_json_code", "execute_code", "render_template_lines"} & calls:
        violations.append("BOUNDING_BOX009 public route returned to generated execution")
    return violations
...
def _scan_typed_surface(root: Path, overrides: Mapping[str, str]) -> list[str]:
    ...
    return [
        f"BOUNDING_BOX014 typed surface contains Any: {name}"
        for name, node in nodes.items()
        if _contains_any(node)
    ]


def scan_bounding_box_architecture(
    root: Path,
    *,
    ...
    if addon_contract != client_contract:
        violations.append("BOUNDING_BOX010 protocol vendors differ")
    return violations
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    print("bounding_box contract: architecture and static types passed")
    return 0
