...
_LEAF = "addon/FreeCADMCP/rpc_server/methods/cad_methods_ops/common_volume_along_path.py"
_MUTATION = "addon/FreeCADMCP/rpc_server/methods/cad_methods_ops/common_volume_along_path_mutation.py"
...
_PUBLIC_ADAPTER = "src/freecad_mcp/operations/parametric_ops/common_volume_along_path.py"
_CLIENT = "src/freecad_mcp/freecad_client_ops/freecad_connection.py"
_ADDON_CONTRACT = "addon/FreeCADMCP/_shared/protocol/common_volume_along_path_contract.py"
_CLIENT_CONTRACT = "src/freecad_mcp/_shared/protocol/common_volume_along_path_contract.py"
...
def _scan_leaf(source: str) -> list[str]:
    ...
    apply_function = _function(tree, "apply_common_volume_along_path")
    apply_closure = _class_method(tree, "_CommonVolumeAlongPathExecution", "apply")
    run_function = _class_method(tree, "_CommonVolumeAlongPathExecution", "run")
    forbidden = {
        "abortTransaction",
        "commitTransaction",
        "execute_code",
    ...
    violations = ["COMMON_VOLUME_ALONG_PATH001 leaf owns forbidden execution: " + ", ".join(found)] if found else []
    if "inspect" in _called_names(apply_closure):
        violations.append("COMMON_VOLUME_ALONG_PATH002 inspection runs inside apply")
    mutation_calls = [
        node
        for node in ast.walk(run_function)
        if isinstance(node, ast.Call)
        and isinstance(node.func, ast.Name)
        and node.func.id == "run_common_volume_along_path_native_mutation"
    ]
    if len(mutation_calls) != 1:
        return [*violations, "COMMON_VOLUME_ALONG_PATH003 must use exactly one typed mutation call"]
    ...
    if not (isinstance(postcondition, ast.Attribute) and postcondition.attr == "inspect"):
        violations.append("COMMON_VOLUME_ALONG_PATH004 missing typed postcondition=inspect")
    return violations


def _scan_native_release(source: str) -> list[str]:
    tree = ast.parse(source, filename=_MUTATION)
    native_result = _function(tree, "_common_volume_along_path_native_result")
    runner = _function(tree, "run_common_volume_along_path_native_mutation")
    violations = []
    import types

    module = types.ModuleType("addon.FreeCADMCP.rpc_server.methods.cad_methods_ops._common_volume_along_path_gate")
    ...
    try:
        ...
        for status in (
            ...
            result = module._common_volume_along_path_native_result({"status": status, "committed": False}, state)
            if result is True:
                violations.append("COMMON_VOLUME_ALONG_PATH007 cached success escaped native rejection")
                break
        state = module._NativeMutationState()
        result = module._common_volume_along_path_native_result({"status": "Committed", "committed": True}, state)
        if result is True:
            violations.append("COMMON_VOLUME_ALONG_PATH015 commit escaped without a successful postcondition")
    finally:
    ...
    if len(commit_calls) != 1:
        violations.append("COMMON_VOLUME_ALONG_PATH012 typed path must use exactly one native commit call")
        return violations
    call = commit_calls[0]
    ...
    if not isinstance(forwarded, ast.Name) or forwarded.id != "native_postcondition":
        violations.append("COMMON_VOLUME_ALONG_PATH013 typed path dropped its native postcondition")
    ...
    if any(
        ...
        violations.append("COMMON_VOLUME_ALONG_PATH014 typed native path contains Any")
    return violations


def _scan_public_adapter(source: str) -> list[str]:
    tree = ast.parse(source, filename=_PUBLIC_ADAPTER)
    operation = _function(tree, "common_volume_along_path_operation")
    calls = set(_called_names(operation))
    violations = []
    if "parse_common_volume_along_path_response" not in calls:
        violations.append("COMMON_VOLUME_ALONG_PATH008 public adapter bypasses response validation")
    if {"_run_json_code", "execute_code", "render_template_lines"} & calls:
        violations.append("COMMON_VOLUME_ALONG_PATH009 public route returned to generated execution")
    return violations
...
def _scan_typed_surface(root: Path, overrides: Mapping[str, str]) -> list[str]:
    ...
    return [
        f"COMMON_VOLUME_ALONG_PATH014 typed surface contains Any: {name}"
...
def scan_common_volume_along_path_architecture(
    ...
    *,
    ...
    if addon_contract != client_contract:
        violations.append("COMMON_VOLUME_ALONG_PATH010 protocol vendors differ")
    return violations
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    print("common_volume_along_path contract: architecture and static types passed")
    return 0
