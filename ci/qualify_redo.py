"""Repeatable acceptance lanes for typed ``redo``.
...
PYTHON_TESTS = (
    "tests/redo/test_redo.py",
    "tests/redo/test_redo_response.py",
    "tests/redo/test_redo_contract_gate.py",
    "tests/redo/test_redo_json_rpc_contract.py",
)
...
def main() -> int:
    ...
    if args.native:
        ...
        tests = ("tests/native/test_native_redo.py",)
    else:
        result = subprocess.run(
            [sys.executable, "ci/check_redo_contract.py"],
            cwd=root,
            check=False,
        )
