#!/usr/bin/env python3
"""Fast static and architecture gate for the typed ``import_step`` slice."""
...
_LEAF = "addon/FreeCADMCP/rpc_server/methods/cad_methods_ops/import_step.py"
_MUTATION = "addon/FreeCADMCP/rpc_server/methods/cad_methods_ops/import_step_mutation.py"
...
_PUBLIC_ADAPTER = "src/freecad_mcp/operations/parametric_ops/import_step.py"
_CLIENT = "src/freecad_mcp/freecad_client_ops/freecad_connection.py"
_ADDON_CONTRACT = "addon/FreeCADMCP/_shared/protocol/import_step_contract.py"
_CLIENT_CONTRACT = "src/freecad_mcp/_shared/protocol/import_step_contract.py"
...
def _scan_leaf(source: str) -> list[str]:
    ...
    apply_function = _function(tree, "apply_import_step")
    apply_closure = _class_method(tree, "_ImportStepExecution", "apply")
    run_function = _class_method(tree, "_ImportStepExecution", "run")
    forbidden = {
        "abortTransaction",
        "commitTransaction",
        "execute_code",
    ...
    violations = ["IMPORT_STEP001 leaf owns forbidden execution: " + ", ".join(found)] if found else []
    if "inspect" in _called_names(apply_closure):
        violations.append("IMPORT_STEP002 inspection runs inside apply")
    mutation_calls = [
        node
        for node in ast.walk(run_function)
        if isinstance(node, ast.Call)
        and isinstance(node.func, ast.Name)
        and node.func.id == "run_import_step_native_mutation"
    ]
    if len(mutation_calls) != 1:
        return [*violations, "IMPORT_STEP003 must use exactly one typed mutation call"]
    ...
    if not (isinstance(postcondition, ast.Attribute) and postcondition.attr == "inspect"):
        violations.append("IMPORT_STEP004 missing typed postcondition=inspect")
    return violations


def _scan_native_release(source: str) -> list[str]:
    tree = ast.parse(source, filename=_MUTATION)
    native_result = _function(tree, "_import_step_native_result")
    runner = _function(tree, "run_import_step_native_mutation")
    violations = []
    import types

    module = types.ModuleType("addon.FreeCADMCP.rpc_server.methods.cad_methods_ops._import_step_gate")
    ...
    try:
        ...
        for status in (
            ...
            result = module._import_step_native_result({"status": status, "committed": False}, state)
            if result is True:
                violations.append("IMPORT_STEP007 cached success escaped native rejection")
                break
        state = module._NativeMutationState()
        result = module._import_step_native_result({"status": "Committed", "committed": True}, state)
        if result is True:
            violations.append("IMPORT_STEP015 commit escaped without a successful postcondition")
    finally:
    ...
    if len(commit_calls) != 1:
        violations.append("IMPORT_STEP012 typed path must use exactly one native commit call")
        return violations
    call = commit_calls[0]
    ...
    if not isinstance(forwarded, ast.Name) or forwarded.id != "native_postcondition":
        violations.append("IMPORT_STEP013 typed path dropped its native postcondition")
    ...
    if any(
        ...
        violations.append("IMPORT_STEP014 typed native path contains Any")
    return violations


def _scan_public_adapter(source: str) -> list[str]:
    tree = ast.parse(source, filename=_PUBLIC_ADAPTER)
    operation = _function(tree, "import_step_operation")
    calls = set(_called_names(operation))
    violations = []
    if "parse_import_step_response" not in calls:
        violations.append("IMPORT_STEP008 public adapter bypasses response validation")
    if {"_run_json_code", "execute_code", "render_template_lines"} & calls:
        violations.append("IMPORT_STEP009 public route returned to generated execution")
    return violations
...
def _scan_typed_surface(root: Path, overrides: Mapping[str, str]) -> list[str]:
    ...
    return [
        f"IMPORT_STEP014 typed surface contains Any: {name}"
        for name, node in nodes.items()
        if _contains_any(node)
    ]


def scan_import_step_architecture(
    root: Path,
    *,
    ...
    if addon_contract != client_contract:
        violations.append("IMPORT_STEP010 protocol vendors differ")
    return violations
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    violations = scan_import_step_architecture(root)
    ...
    print("import_step contract: architecture and static types passed")
    return 0
