#!/usr/bin/env python3
"""Fast static and architecture gate for the typed ``sweep_pipe`` slice."""
...
_LEAF = "addon/FreeCADMCP/rpc_server/methods/cad_methods_ops/sweep_pipe.py"
_MUTATION = "addon/FreeCADMCP/rpc_server/methods/cad_methods_ops/sweep_pipe_mutation.py"
_PUBLIC_ADAPTER = "src/freecad_mcp/operations/parametric_ops/sweep_pipe.py"
_ADDON_CONTRACT = "addon/FreeCADMCP/_shared/protocol/sweep_pipe_contract.py"
_CLIENT_CONTRACT = "src/freecad_mcp/_shared/protocol/sweep_pipe_contract.py"
_TEMPLATE = "src/freecad_mcp/templates/p7_assembly/sweep_pipe.py.txt"
...
def _scan_leaf(source: str) -> list[str]:
    ...
    apply_function = _function(tree, "apply_sweep_pipe")
    apply_closure = _class_method(tree, "_SweepPipeExecution", "apply")
    run_function = _class_method(tree, "_SweepPipeExecution", "run")
    forbidden = {
        "abortTransaction",
        "commitTransaction",
        "execute_code",
    ...
    violations = ["SWEEP_PIPE001 leaf owns forbidden execution: " + ", ".join(found)] if found else []
    if "inspect" in _called_names(apply_closure):
        violations.append("SWEEP_PIPE002 inspection runs inside apply")
    mutation_calls = [
        node
        for node in ast.walk(run_function)
        if isinstance(node, ast.Call)
        and isinstance(node.func, ast.Name)
        and node.func.id == "run_sweep_pipe_native_mutation"
    ]
    if len(mutation_calls) != 1:
        return [*violations, "SWEEP_PIPE003 must use exactly one typed mutation call"]
    ...
    if not (isinstance(postcondition, ast.Attribute) and postcondition.attr == "inspect"):
        violations.append("SWEEP_PIPE004 missing typed postcondition=inspect")
    return violations


def _scan_native_release(source: str) -> list[str]:
    tree = ast.parse(source, filename=_MUTATION)
    native_result = _function(tree, "_sweep_pipe_native_result")
    runner = _function(tree, "run_sweep_pipe_native_mutation")
    violations = []
    import types

    module = types.ModuleType("addon.FreeCADMCP.rpc_server.methods.cad_methods_ops._sweep_pipe_gate")
    ...
    try:
        ...
        for status in (
            ...
            result = module._sweep_pipe_native_result({"status": status, "committed": False}, state)
            if result is True:
                violations.append("SWEEP_PIPE007 cached success escaped native rejection")
                break
        state = module._NativeMutationState()
        result = module._sweep_pipe_native_result({"status": "Committed", "committed": True}, state)
        if result is True:
            violations.append("SWEEP_PIPE015 commit escaped without a successful postcondition")
    finally:
    ...
    if len(commit_calls) != 1:
        violations.append("SWEEP_PIPE012 typed path must use exactly one native commit call")
        return violations
    call = commit_calls[0]
    ...
    if not isinstance(forwarded, ast.Name) or forwarded.id != "native_postcondition":
        violations.append("SWEEP_PIPE013 typed path dropped its native postcondition")
    if any(isinstance(node, ast.Name) and node.id == "Any" for node in ast.walk(runner)):
        violations.append("SWEEP_PIPE014 typed native path contains Any")
...
def _scan_public_adapter(source: str) -> list[str]:
    ...
    operation = _function(tree, "sweep_pipe_operation")
    calls = set(_called_names(operation))
    violations = []
    if "parse_sweep_pipe_response" not in calls:
        violations.append("SWEEP_PIPE008 public adapter bypasses response validation")
    if {"_run_json_code", "execute_code", "render_template_lines"} & calls:
        violations.append("SWEEP_PIPE009 public route returned to generated execution")
    return violations
...
def _scan_typed_surface(root: Path, overrides: Mapping[str, str]) -> list[str]:
    ...
    return [
        f"SWEEP_PIPE014 typed surface contains Any: {name}"
        for name, node in nodes.items()
        if _contains_any(node)
    ]


def scan_sweep_pipe_architecture(
    root: Path,
    *,
    ...
    if addon_contract != client_contract:
        violations.append("SWEEP_PIPE010 protocol vendors differ")
    if _TEMPLATE and not overrides and (root / _TEMPLATE).exists():
        violations.append("SWEEP_PIPE011 generated source template exists")
    return violations
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    violations = scan_sweep_pipe_architecture(root)
    ...
    print("sweep_pipe contract: architecture and static types passed")
    return 0
