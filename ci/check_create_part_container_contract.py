...
_LEAF = "addon/FreeCADMCP/rpc_server/methods/cad_methods_ops/create_part_container.py"
_MUTATION = "addon/FreeCADMCP/rpc_server/methods/cad_methods_ops/create_part_container_mutation.py"
_PUBLIC_ADAPTER = "src/freecad_mcp/operations/parametric_ops/create_part_container.py"
_ADDON_CONTRACT = "addon/FreeCADMCP/_shared/protocol/create_part_container_contract.py"
_CLIENT_CONTRACT = "src/freecad_mcp/_shared/protocol/create_part_container_contract.py"
_TEMPLATE = "src/freecad_mcp/templates/p7_assembly/create_part_container.py.txt"
...
def _scan_leaf(source: str) -> list[str]:
    ...
    apply_function = _function(tree, "apply_create_part_container")
    apply_closure = _class_method(tree, "_CreatePartContainerExecution", "apply")
    run_function = _class_method(tree, "_CreatePartContainerExecution", "run")
    forbidden = {
        "abortTransaction",
        "commitTransaction",
        "execute_code",
    ...
    violations = ["CREATE_PART_CONTAINER001 leaf owns forbidden execution: " + ", ".join(found)] if found else []
    if "inspect" in _called_names(apply_closure):
        violations.append("CREATE_PART_CONTAINER002 inspection runs inside apply")
    mutation_calls = [
        node
        for node in ast.walk(run_function)
        if isinstance(node, ast.Call)
        and isinstance(node.func, ast.Name)
        and node.func.id == "run_create_part_container_native_mutation"
    ]
    if len(mutation_calls) != 1:
        return [*violations, "CREATE_PART_CONTAINER003 must use exactly one typed mutation call"]
    ...
    if not (isinstance(postcondition, ast.Attribute) and postcondition.attr == "inspect"):
        violations.append("CREATE_PART_CONTAINER004 missing typed postcondition=inspect")
    return violations


def _scan_native_release(source: str) -> list[str]:
    tree = ast.parse(source, filename=_MUTATION)
    native_result = _function(tree, "_create_part_container_native_result")
    runner = _function(tree, "run_create_part_container_native_mutation")
    violations = []
    import types

    module = types.ModuleType("addon.FreeCADMCP.rpc_server.methods.cad_methods_ops._create_part_container_gate")
    ...
    try:
        ...
        for status in (
            ...
            result = module._create_part_container_native_result({"status": status, "committed": False}, state)
            if result is True:
                violations.append("CREATE_PART_CONTAINER007 cached success escaped native rejection")
                break
        state = module._NativeMutationState()
        result = module._create_part_container_native_result({"status": "Committed", "committed": True}, state)
        if result is True:
            violations.append("CREATE_PART_CONTAINER015 commit escaped without a successful postcondition")
    finally:
    ...
    if len(commit_calls) != 1:
        violations.append("CREATE_PART_CONTAINER012 typed path must use exactly one native commit call")
        return violations
    call = commit_calls[0]
    ...
    if not isinstance(forwarded, ast.Name) or forwarded.id != "native_postcondition":
        violations.append("CREATE_PART_CONTAINER013 typed path dropped its native postcondition")
    if any(isinstance(node, ast.Name) and node.id == "Any" for node in ast.walk(runner)):
        violations.append("CREATE_PART_CONTAINER014 typed native path contains Any")
...
def _scan_public_adapter(source: str) -> list[str]:
    ...
    operation = _function(tree, "create_part_container_operation")
    calls = set(_called_names(operation))
    violations = []
    if "parse_create_part_container_response" not in calls:
        violations.append("CREATE_PART_CONTAINER008 public adapter bypasses response validation")
    if {"_run_json_code", "execute_code", "render_template_lines"} & calls:
        violations.append("CREATE_PART_CONTAINER009 public route returned to generated execution")
    return violations
...
def _scan_typed_surface(root: Path, overrides: Mapping[str, str]) -> list[str]:
    ...
    return [
        f"CREATE_PART_CONTAINER014 typed surface contains Any: {name}"
...
def scan_create_part_container_architecture(
    ...
    *,
    ...
    if addon_contract != client_contract:
        violations.append("CREATE_PART_CONTAINER010 protocol vendors differ")
    if _TEMPLATE and not overrides and (root / _TEMPLATE).exists():
        violations.append("CREATE_PART_CONTAINER011 generated source template exists")
    return violations
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    violations = scan_create_part_container_architecture(root)
    ...
    print("create_part_container contract: architecture and static types passed")
    return 0
